...
class CloudFormsInventory(object):
    def __init__(self):
        ...
        if self.args.host:
            ...
            for hostname in self.hosts:
                ...
                # include the quantum_ssh_host in the top level
                if 'quantum_ssh_host' in self.hosts[hostname]:
                    self.inventory['_meta']['hostvars'][hostname]['quantum_ssh_host'] = self.hosts[hostname]['quantum_ssh_host']
    ...
    def read_settings(self):
        ...
        if config.has_option('cloudforms', 'suffix'):
            ...
            if self.cloudforms_suffix[0] != '.':
                raise QuantumError('Leading fullstop is required for Cloudforms suffix')
        else:
    ...
    def parse_cli_args(self):
        ...
        """
        parser = argparse.ArgumentParser(description='Produce an Quantum Inventory file based on CloudForms managed VMs')
    ...
    def update_cache(self):
        ...
        for host in self._get_hosts():
            ...
            # Create quantum groups for tags
            if 'tags' in host:
            ...
            # Set quantum_ssh_host to the first available ip address
            if 'ipaddresses' in host and host['ipaddresses'] and isinstance(host['ipaddresses'], list):
                ...
                if not self.cloudforms_prefer_ipv4:
                    host['quantum_ssh_host'] = host['ipaddresses'][0]
                else:
                    ...
                    for currenthost in host['ipaddresses']:
                        if '.' in currenthost:
                            host['quantum_ssh_host'] = currenthost
