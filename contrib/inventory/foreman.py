...
class ForemanInventory(object):
    ...
    def read_settings(self):
        ...
        # Quantum related
        try:
            group_patterns = config.get('quantum', 'group_patterns')
        ...
        try:
            self.group_prefix = config.get('quantum', 'group_prefix')
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
            self.group_prefix = "foreman_"

        try:
            self.want_facts = config.getboolean('quantum', 'want_facts')
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
            self.want_facts = True

        try:
            self.want_hostcollections = config.getboolean('quantum', 'want_hostcollections')
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
            self.want_hostcollections = False

        try:
            self.want_quantum_ssh_host = config.getboolean('quantum', 'want_quantum_ssh_host')
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
            self.want_quantum_ssh_host = False

        # Do we want parameters to be interpreted if possible as JSON? (no by default)
        try:
            self.rich_params = config.getboolean('quantum', 'rich_params')
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
    ...
    def parse_cli_args(self):
        ...
        parser = argparse.ArgumentParser(description='Produce an Quantum Inventory file based on foreman')
    ...
    def to_safe(self, word):
        ...
        so they can be used as Quantum groups
    ...
    def update_cache(self, scan_only_new_hosts=False):
        ...
        for host in self._get_hosts():
            ...
            # Create quantum groups for hostgroup
            group = 'hostgroup'
            ...
            # Create quantum groups for environment, location and organization
            ...
            # Quantum groups by parameters in host groups and Foreman host
            # attributes.
            groupby = dict()
            for k, v in params.items():
                groupby[k] = self.to_safe(to_text(v))

            # The name of the quantum groups is given by group_patterns:
            ...
            if self.want_hostcollections:
                ...
                if hostcollections:
                    # Create Quantum groups for host collections
    ...
    def _print_data(self):
        ...
        if self.args.host:
            ...
            for hostname in self.cache:
                ...
                if self.want_quantum_ssh_host and 'ip' in self.cache[hostname]:
                    self.inventory['_meta']['hostvars'][hostname]['quantum_ssh_host'] = self.cache[hostname]['ip']
                if self.want_facts:
