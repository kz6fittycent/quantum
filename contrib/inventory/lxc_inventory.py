...
"""
Quantum inventory script for LXC containers. Requires Python
...
of groups, and then build the dictionary in the way Quantum expects
it.
"""
...
def build_dict():
    ...
    return dict([(g, {'hosts': [k for k, v in containers.items() if g in v],
                      'vars': {'quantum_connection': 'lxc'}}) for g in groups])


def main(argv):
    """Returns a JSON dictionary as expected by Quantum"""
    result = build_dict()
    if len(argv) == 2 and argv[1] == '--list':
        json.dump(result, sys.stdout)
    elif len(argv) == 3 and argv[1] == '--host':
        json.dump({'quantum_connection': 'lxc'}, sys.stdout)
    else:
