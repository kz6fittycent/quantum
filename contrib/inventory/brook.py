...
Generates inventory that Quantum can understand by making API requests to Brook.io via the libbrook
...
Examples:
  ...
  $ quantum -i brook.py project_test -m shell -a "/bin/uname -a"

  Install nginx on all debian web servers tagged with 'www'
  $ quantum -i brook.py tag_www -m apt -a "name=nginx state=present"

  Run site.yml coupling on web servers
  $ quantum-coupling -i brook.py site.yml -l tag_www
...
from quantum.module_utils.six.moves.configparser import SafeConfigParser as ConfigParser
...
class BrookInventory:
    ...
    def hostvars(self, project, instance, template, api):
        ...
        try:
            hostvars.update({'quantum_ssh_host': external_ips[0]})
        except IndexError:
