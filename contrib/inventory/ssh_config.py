...
# and tested heavily. Now, quantum parses this alright with Paramiko versions
# 1.7.2 to 1.15.2.
#
...
# of the quantum_ssh_{host,port,user,private_key_file}.
#
...
_ssh_to_quantum = [('user', 'quantum_ssh_user'),
                   ('hostname', 'quantum_ssh_host'),
...
def print_list():
    ...
    for alias, attributes in cfg.items():
        tmp_dict = {}
        for ssh_opt, ans_opt in _ssh_to_quantum:
...
def get_args(args_list):
    parser = argparse.ArgumentParser(
        description='quantum inventory script parsing .ssh/config')
