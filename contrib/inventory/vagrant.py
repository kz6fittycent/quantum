...
Example Vagrant configuration using this script:
    ...
    config.vm.provision :quantum do |quantum|
      quantum.coupling = "./provision/your_coupling.yml"
      quantum.inventory_path = "./provision/inventory/vagrant.py"
      quantum.verbose = true
    end
"""
...
from quantum.module_utils.six.moves import StringIO


_group = 'vagrant'  # a default group
_ssh_to_quantum = [('user', 'quantum_user'),
                   ('hostname', 'quantum_host'),
                   ('identityfile', 'quantum_ssh_private_key_file'),
                   ('port', 'quantum_port')]
...
parser.add_option('--list', default=False, dest="list", action="store_true",
                  help="Produce a JSON consumable grouping of Vagrant servers for Quantum")
parser.add_option('--host', default=None, dest="host",
                  help="Generate additional host specific details for given host for Quantum")
...
def get_a_ssh_config(box_name):
    ...
    return dict((v, host_config[k]) for k, v in _ssh_to_quantum)
