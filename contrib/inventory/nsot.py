...
Quantum Dynamic Inventory to pull hosts from NSoT, a flexible CMDB by Dropbox
...
desired Quantum group-name hashed with single 'query' item to define the NSoT
attribute query.
...
Here are some examples shown from just calling the command directly::
   ...
   $ NSOT_INVENTORY_CONFIG=$PWD/test.yaml quantum_nsot --list | jq '.'
   {
     "routers": {
       "hosts": [
         "test1.example.com"
   ...
   $ NSOT_INVENTORY_CONFIG=$PWD/test.yaml quantum_nsot --host test1 | jq '.'
   {
      "make": "MX80",
      "site_id": 1,
      "id": 107
