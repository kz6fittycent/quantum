...
installed on the host running Quantum. To install docker-py:
...
creating a configuration file named docker.yml (sample provided in quantum/contrib/inventory). The order of precedence
...
class QuantumDockerClient(Client):
    def __init__(self, auth_params, debug):
        ...
        try:
            super(QuantumDockerClient, self).__init__(**self._connect_params)
...
class DockerInventory(object):
    ...
    def run(self):
        ...
        for host in docker_hosts:
            client = QuantumDockerClient(host, self._args.debug)
    ...
    def get_inventory(self, client, host):
        ...
        for container in containers:
            ...
            if not self._args.host or (self._args.host and self._args.host in [name, id, short_id]):
                ...
                facts = dict(
                    quantum_ssh_host=ip,
                    quantum_ssh_port=port.get('HostPort', int()),
                    docker_name=name,
    ...
    def _parse_cli_args(self):
        ...
        parser = argparse.ArgumentParser(
            description='Return Quantum inventory for one or more Docker hosts.')
