...
Quantum has a feature where instead of reading from /etc/quantum/hosts
...
automation like Quantum.  Extensive documentation on Collins, including a quickstart,
API docs, and a full reference manual, can be found here:

http://tumblr.github.io/collins

This script adds support to Quantum for obtaining a dynamic inventory of
assets in your infrastructure, grouping them in Quantum by their useful attributes,
...
To use it, copy it to your repo and pass -i <collins script> to the quantum or
quantum-coupling command; if you'd like to use it by default, simply copy collins.ini
to /etc/quantum and this script to /etc/quantum/hosts.
...
that will be used instead of the configured values if they are set:
 ...
 - COLLINS_ASSET_TYPE - specifies a Collins asset type to use during querying;
   this can be used to run Quantum automation against different asset classes than
...
Collins attributes are accessible as variables in quantum via the COLLINS['attribute_name'].

Tested against Quantum 1.8.2 and Collins 1.3.0.
"""
...
from quantum.module_utils.six.moves import configparser as ConfigParser
...
class CollinsInventory(object):
    ...
    def read_settings(self):
        ...
        self.cache_path_cache = cache_path + \
            '/quantum-collins-%s.cache' % self.collins_asset_type
        self.cache_path_inventory = cache_path + \
            '/quantum-collins-%s.index' % self.collins_asset_type
        self.cache_max_age = config.getint('collins', 'cache_max_age')

        log_path = config.get('collins', 'log_path')
        self.log_location = log_path + '/quantum-collins.log'

    def parse_cli_args(self):
        """ Command line argument processing """

        parser = argparse.ArgumentParser(
            description='Produces an Quantum Inventory file based on Collins')
    ...
    def update_cache(self):
        ...
        for asset in server_assets:
            ...
            # which will be used to index the asset throughout the Quantum inventory.
            ...
            # Adds an asset index to the Quantum inventory based upon unpacking
