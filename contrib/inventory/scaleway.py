...
This script generates an inventory that Quantum can understand by making API requests to Scaleway API
...
This script generates an Quantum hosts file with these host groups:
...
class ScalewayAPI:
    ...
    def __init__(self, auth_token, region):
        ...
        self.session.headers.update({
            'User-Agent': 'Quantum Python/%s' % (sys.version.split(' ')[0])
        })
        self.session.headers.update({
...
def save_cache(data, config):
    ...
    try:
        cache = open('/'.join([dpath, 'scaleway_quantum_inventory.json']), 'w')
...
def get_cache(cache_item, config):
    ...
    inv = {}
    try:
        cache = open('/'.join([dpath, 'scaleway_quantum_inventory.json']), 'r')
        inv = cache.read()
        cache.close()
    except IOError as e:
        pass  # not really sure what to do here
...
def cache_available(config):
    ...
    if config.has_option('cache', 'cache_dir'):
        ...
        try:
            existing = os.stat(
                '/'.join([dpath, 'scaleway_quantum_inventory.json']))
        except OSError:
            return False
...
def get_inventory(config):
    ...
    if cache_available(config):
        inv = get_cache('scaleway_quantum_inventory.json', config)
    else:
...
if __name__ == '__main__':
    ...
    if cache_available(config):
        inventory = get_cache('scaleway_quantum_inventory.json', config)
    else:
