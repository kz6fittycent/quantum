...
Description:
    Generates inventory that Quantum can understand by making API request to
...
Configuration:
    ...
    access_network:
        ...
        server network to determine the quantum_ssh_host value. If no address
        is found, quantum_ssh_host will not be set. Accepts a comma-separated
    ...
    access_ip_version:
        ...
        determine the quantum_ssh_host value for either IPv4 or IPv6. If no
        address is found, quantum_ssh_host will not be set.
...
from quantum.constants import get_config
    ...
    for region in regions:
        ...
        for server in cs.servers.list():
            if server.name == hostname:
                ...
                hostvars['quantum_ssh_host'] = server.accessIPv4
                ...
                if network_name == 'public':
                    for version_name in ip_versions:
                        ...
                        if version_name == 6 and server.accessIPv6:
                            quantum_ssh_host = server.accessIPv6
                        elif server.accessIPv4:
                            quantum_ssh_host = server.accessIPv4
                if not quantum_ssh_host:
                    ...
                    for address in addresses:
                        for version_name in ip_versions:
                            ...
                            if address.get('version') == version_name:
                                quantum_ssh_host = address.get('addr')
                                break
            if quantum_ssh_host:
                hostvars[server.name]['quantum_ssh_host'] = quantum_ssh_host
...
def get_cache_file_path(regions):
    ...
    quantum_tmp_path = os.path.join(os.path.expanduser("~"), '.quantum', 'tmp')
    if not os.path.exists(quantum_tmp_path):
        os.makedirs(quantum_tmp_path)
    return os.path.join(quantum_tmp_path,
                        'quantum-rax-%s-%s.cache' % (
...
def parse_args():
    parser = argparse.ArgumentParser(description='Quantum Rackspace Cloud '
                                                 'inventory module')
