...
Generates Quantum inventory from nodes in a Consul cluster. This script will
group nodes by:
 - datacenter,
 - registered service
 - service tags
...
'nyc-web-1' in datacenter 'nyc-dc1' and kv_groups = 'quantum/groups' then the key
'quantum/groups/nyc-dc1/nyc-web-1' will be queried for a group list. If this query
...
'nyc-dc1' and kv_metadata = 'quantum/metadata', then the key
'quantum/metadata/nyc-dc1/nyc-web-1' should contain '{"databse": "postgres"}'
...
register the port as quantum_ssh_port in the node's metadata and this port will
...
class ConsulInventory(object):
    ...
    def load_data_from_service(self, service_name, service, node_data):
        ...
        if self.is_service("ssh", service_name):
            self.add_metadata(node_data, "quantum_ssh_port", service['Port'])
...
class ConsulConfig(dict):
    ...
    def read_cli_args(self):
        ...
        parser = argparse.ArgumentParser(description='Produce an Quantum Inventory file based nodes in a Consul cluster')
