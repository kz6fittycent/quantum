...
# There are two quantum inventory specific options that can be set in
...
#
# Also it is possible to pass the correct user by setting an quantum_user: $myuser
# metadata attribute.
...
CONFIG_FILES = ['/etc/quantum/openstack.yaml', '/etc/quantum/openstack.yml']
...
def append_hostvars(hostvars, groups, key, server, namegroup=False):
    hostvars[key] = dict(
        quantum_ssh_host=server['interface_ip'],
        quantum_host=server['interface_ip'],
        openstack=server)

    metadata = server.get('metadata', {})
    if 'quantum_user' in metadata:
        hostvars[key]['quantum_user'] = metadata['quantum_user']
...
def get_cache_settings(cloud=None):
    ...
    cache_file = os.path.join(cache_path, 'quantum-inventory.cache')
...
def parse_args():
    ...
    parser.add_argument('--private',
                        action='store_true',
                        help='Use private address for quantum host')
