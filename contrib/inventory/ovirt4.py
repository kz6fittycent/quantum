...
Examples:
  # Execute update of system on webserver virtual machine:
    ...
    $ quantum -i contrib/inventory/ovirt4.py webserver -m yum -a "name=* state=latest"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Quantum dynamic inventory script for oVirt.',
    )
    parser.add_argument(
        '--list',
        action='store_true',
...
def get_dict_of_struct(connection, vm):
    ...
    return {
        ...
        'quantum_host': next((device.ips[0].address for device in devices if device.ips), None)
    }
