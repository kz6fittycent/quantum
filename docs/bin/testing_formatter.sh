...
The following sanity tests are available as \`\`--test\`\` options for \`\`quantum-test sanity\`\`.
This list is also available using \`\`quantum-test sanity --list-tests --allow-disabled\`\`.
...
$(for test in $(../../bin/quantum-test sanity --list-tests --allow-disabled); do echo "   ${test}"; done)
