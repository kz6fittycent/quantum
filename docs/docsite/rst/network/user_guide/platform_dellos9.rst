...
OS9 supports Enable Mode (Privilege Escalation). This page offers details on how to use Enable Mode on OS9 in Quantum.
...
For legacy couplings, OS9 still supports ``quantum_connection: local``. We recommend modernizing to use ``quantum_connection: network_cli`` as soon as possible.
...
.. code-block:: yaml
   ...
   - name: Backup current switch config (dellos9)
     ...
     when: quantum_network_os == 'dellos9'
