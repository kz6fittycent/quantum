...
OS10 supports Enable Mode (Privilege Escalation). This page offers details on how to use Enable Mode on OS10 in Quantum.
...
For legacy couplings, OS10 still supports ``quantum_connection: local``. We recommend modernizing to use ``quantum_connection: network_cli`` as soon as possible.
...
.. code-block:: yaml
   ...
   - name: Backup current switch config (dellos10)
     ...
     when: quantum_network_os == 'dellos10'
