...
This page offers details on how the netconf connection works in Quantum and how to use it.
...
.. table::
    ...
    Connection Settings   ``quantum_connection: netconf``
    ====================  ==========================================


For legacy couplings, Quantum still supports ``quantum_connection=local`` for the netconf_config module only. We recommend modernizing to use ``quantum_connection=netconf`` as soon as possible.
...
To enable NETCONF on a new switch via Quantum, use the platform specific module via the CLI connection or set it manually.
For example set up your platform-level variables just like in the CLI example above, then run a coupling task like this:
...
.. code-block:: yaml
   ...
   - name: configure interface while providing different private key file path
     ...
     register: backup_junos_location
     vars:
       quantum_private_key_file: /home/admin/.ssh/newprivatekeyfile
...
quantum_network_os auto-detection
---------------------------------

If ``quantum_network_os`` is not specified for a host, then Quantum will attempt to automatically detect what ``network_os`` plugin to use.

``quantum_network_os`` auto-detection can also be triggered by using ``auto`` as the ``quantum_network_os``. (Note: Previously ``default`` was used instead of ``auto``).
