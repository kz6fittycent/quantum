...
Pluribus NETVISOR Quantum modules only support CLI connections today. ``httpapi`` modules may be added in future.
This page offers details on how to use ``network_cli`` on NETVISOR in Quantum.
...
Pluribus NETVISOR does not support ``quantum_connection: local``. You must use ``quantum_connection: network_cli``.
...
.. code-block:: yaml
   ...
   - name: Create access list
     pn_access_list:
       ...
       state: "present"
     register: acc_list
     when: quantum_network_os == 'netvisor'
