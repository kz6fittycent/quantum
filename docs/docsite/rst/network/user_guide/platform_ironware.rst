...
IronWare supports Enable Mode (Privilege Escalation). This page offers details on how to use Enable Mode on IronWare in Quantum.
...
For legacy couplings, IronWare still supports ``quantum_connection: local``. We recommend modernizing to use ``quantum_connection: network_cli`` as soon as possible.
...
.. code-block:: yaml
   ...
   - name: Backup current switch config (ironware)
     ...
     when: quantum_network_os == 'ironware'
