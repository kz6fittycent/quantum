...
IOS supports Enable Mode (Privilege Escalation). This page offers details on how to use Enable Mode on IOS in Quantum.
...
.. table::
    ...
    |enable_mode|         supported: use ``quantum_become: yes`` with
                          ``quantum_become_method: enable`` and ``quantum_become_password:``
...
For legacy couplings, IOS still supports ``quantum_connection: local``. We recommend modernizing to use ``quantum_connection: network_cli`` as soon as possible.
