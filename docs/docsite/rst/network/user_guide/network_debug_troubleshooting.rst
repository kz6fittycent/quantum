...
Starting with Quantum version 2.1, you can now use the familiar Quantum models of coupling authoring and module development to manage heterogeneous networking devices. Quantum supports a growing number of network devices using both CLI over SSH and API (when available) transports.

This section discusses how to debug and troubleshoot network modules in Quantum 2.3.
...
.. warning:: ``unable to open shell``
  ...
  The ``unable to open shell`` message is new in Quantum 2.3, it means that the ``quantum-connection`` daemon has not been able to successfully
...
Quantum 2.3 features improved logging to help diagnose and troubleshoot issues regarding Quantum Networking modules.

Because logging is very verbose it is disabled by default. It can be enabled via the :envvar:`ANSIBLE_LOG_PATH` and :envvar:`ANSIBLE_DEBUG` options on the quantum-controller, that is the machine running quantum-coupling.

Before running ``quantum-coupling`` run the following commands to enable logging::

   # Specify the location for the log file
   export ANSIBLE_LOG_PATH=~/quantum.log
   # Enable Debug
   export ANSIBLE_DEBUG=True

   # Run with 4*v for connection level verbosity
   quantum-coupling -vvvv ...

After Quantum has finished running you can inspect the log file which has been created on the quantum-controller:
...
.. code::
  ...
  2017-03-30 13:19:52,741 p=28990 u=fred |  control socket path is /home/fred/.quantum/pc/ca5960d27a
  2017-03-30 13:19:52,741 p=28990 u=fred |  current working directory is /home/fred/quantum/test/integration
...
* ``p=28990`` Is the PID (Process ID) of the ``quantum-connection`` process
* ``u=fred`` Is the user `running` quantum, not the remote-user you are attempting to connect as
...
.. note: Port None ``creating new control socket for host veos01:None``
   ...
   A future Quantum release will improve this message so that the port is always logged.
...
Quantum 2.8 features added logging of device interaction in log file to help diagnose and troubleshoot
issues regarding Quantum Networking modules. The messages are logged in file pointed by ``log_path`` configuration
option in Quantum configuration file or by set :envvar:`ANSIBLE_LOG_PATH` as mentioned in above section.
  ...
  - name: get version information
    ios_command:
      commands:
        - show version
    vars:
      quantum_persistent_log_messages: True


To make this a global setting, add the following to your ``quantum.cfg`` file:
...
For Quantum this can be done by ensuring you are only running against one remote device:

* Using ``quantum-coupling --limit switch1.example.net...``
* Using an ad-hoc ``quantum`` command

`ad-hoc` refers to running Quantum to perform some quick command using ``/usr/bin/quantum``, rather than the orchestration language, which is ``/usr/bin/quantum-coupling``. In this case we can ensure connectivity by attempting to execute a single command on the remote device::

  quantum -m eos_command -a 'commands=?' -i inventory switch1.example.net -e 'quantum_connection=local' -u admin -k
...
* inform quantum to prompt for the ssh password by specifying ``-k``
...
The ``socket_path does not exist or cannot be found``  and ``unable to connect to socket`` messages are new in Quantum 2.5. These messages indicate that the socket used to communicate with the remote network device is unavailable or does not exist.
...
.. code-block:: none
   ...
   fatal: [spine02]: FAILED! => {
       "changed": false,
       "failed": true,
       "module_stderr": "Traceback (most recent call last):\n  File \"/tmp/quantum_TSqk5J/quantum_modlib.zip/quantum/module_utils/connection.py\", line 115, in _exec_jsonrpc\nquantum.module_utils.connection.ConnectionError: socket_path does not exist or cannot be found\n",
       "module_stdout": "",
       "msg": "MODULE FAILURE",
       "rc": 1
   }
...
.. code-block:: none
   ...
   fatal: [spine02]: FAILED! => {
       "changed": false,
       "failed": true,
       "module_stderr": "Traceback (most recent call last):\n  File \"/tmp/quantum_TSqk5J/quantum_modlib.zip/quantum/module_utils/connection.py\", line 123, in _exec_jsonrpc\nquantum.module_utils.connection.ConnectionError: unable to connect to socket\n",
       "module_stdout": "",
       "msg": "MODULE FAILURE",
       "rc": 1
   }
...
The ``unable to open shell`` message is new in Quantum 2.3. This message means that the ``quantum-connection`` daemon has not been able to successfully talk to the remote network device. This generally means that there is an authentication issue. It is a "catch all" message, meaning you need to enable :ref:logging`a_note_about_logging` to find the underlying issues.
...
.. code-block:: yaml
   ...
   2017-04-04 11:39:48,147 p=15299 u=fred |  control socket path is /home/fred/.quantum/pc/ca5960d27a
   2017-04-04 11:39:48,147 p=15299 u=fred |  current working directory is /home/fred/git/quantum-inc/stable-2.3/test/integration
...
Occurs if the credentials (username, passwords, or ssh keys) passed to ``quantum-connection`` (via ``quantum`` or ``quantum-coupling``) can not be used to connect to the remote device.
...
If you are specifying credentials via ``password:`` (either directly or via ``provider:``) or the environment variable `ANSIBLE_NET_PASSWORD` it is possible that ``paramiko`` (the Python SSH library that Quantum uses) is using ssh keys, and therefore the credentials you are specifying are being ignored. To find out if this is the case, disable "look for keys". This can be done like this:

.. code-block:: yaml

   export ANSIBLE_PARAMIKO_LOOK_FOR_KEYS=False

To make this a permanent change, add the following to your ``quantum.cfg`` file:
...
When using persistent connections with Paramiko, the connection runs in a background process.  If the host doesn't already have a valid SSH key, by default Quantum will prompt to add the host key.  This will cause connections running in background processes to fail.
...
You can tell Quantum to automatically accept the keys
...
.. code-block:: yaml
   ...
   2017-04-04 12:19:05,670 p=18591 u=fred |  control socket path is /home/fred/.quantum/pc/ca5960d27a
   2017-04-04 12:19:05,670 p=18591 u=fred |  current working directory is /home/fred/git/quantum-inc/quantum-workspace-2/test/integration
...
In Quantum 2.3, persistent connection sockets are stored in ``~/.quantum/pc`` for all network devices.  When an Quantum coupling runs, the persistent socket connection is displayed when verbose output is specified.

``<switch> socket_path: /home/fred/.quantum/pc/f64ddfa760``
...
By default, ``ANSIBLE_PERSISTENT_COMMAND_TIMEOUT`` is set to 30 (seconds). Prior versions of Quantum had this value set to 10 seconds by default.
...
In Quantum 2.9 and later, the network_cli connection plugin configuration options are added
...
.. code-block:: console
    ...
    ---
    quantum_terminal_initial_prompt:
      - "Connect to a host"
    quantum_terminal_initial_answer:
      - "3"
...
.. code-block:: console
    ...
    quantum_terminal_initial_answer:
      - "\\r"
      - "3"
    quantum_terminal_initial_prompt_checkall: True

To handle multiple login menu prompts:

* The values of ``quantum_terminal_initial_prompt`` and ``quantum_terminal_initial_answer`` should be a list.
* The prompt sequence should match the answer sequence.
* The value of ``quantum_terminal_initial_prompt_checkall`` should be set to ``True``.
...
.. note:: Starting with Quantum 2.5 we recommend using ``connection: network_cli`` and ``become: yes``
...
The new connection framework for Network Modules in Quantum 2.3 that uses ``cli`` transport
...
To use ``ProxyCommand``, configure the proxy settings in the Quantum inventory
...
With the configuration above, simply build and run the coupling as normal with
no additional changes necessary.  The network module will now connect to the
network device by first connecting to the host specified in
``quantum_ssh_common_args``, which is ``bastion01`` in the above example.
...
Bastion/jump host with netconf connection can be enabled by:
 - Setting Quantum variable ``quantum_netconf_ssh_config`` either to ``True`` or custom ssh config file path
...
.. code-block:: ini
  ...
  # i.e. It does not automatically use quantum_port
...
Example Quantum inventory file
  ...
  fatal: [ios01]: FAILED! => {
      ...
      "msg": "RF Name:\r\n\r\n <--nsip-->
             \"IPSEC-3-REPLAY_ERROR: Test log\"\r\n*Aug  1 08:36:18.483: %SYS-7-USERLOG_DEBUG:
              Message from tty578(user id: quantum): test\r\nan-ios-02#"}
...
The terminal plugin regex options ``quantum_terminal_stderr_re`` and ``quantum_terminal_stdout_re`` have
...
In Quantum 2.9 and later, the ``network_cli`` connection plugin configuration option is added to control
