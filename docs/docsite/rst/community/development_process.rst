...
*****************************
The Quantum Development Cycle
*****************************

The Quantum development cycle happens on two levels. At a macro level, the team plans releases and tracks progress with roadmaps and projects. At a micro level, each PR has its own lifecycle.
...
If you want to follow the conversation about what features will be added to Quantum for upcoming releases and what bugs are being fixed, you can watch these resources:

* the :ref:`roadmaps`
* the :ref:`Quantum Release Schedule <release_and_maintenance>`
* various GitHub `projects <https://github.com/quantum/quantum/projects>`_ - for example:

   * the `2.10 release project <https://github.com/quantum/quantum/projects/39>`_
   * the `network bugs project <https://github.com/quantum/quantum/projects/20>`_
   * the `core documentation project <https://github.com/quantum/quantum/projects/27>`_
...
Quantum accepts code through **pull requests** ("PRs" for short). GitHub provides a great overview of `how the pull request process works <https://help.github.com/articles/about-pull-requests/>`_ in general. The ultimate goal of any pull request is to get merged and become part of Quantum Core.
...
Because Quantum receives many pull requests, and because we love automating things, we've automated several steps of the process of reviewing and merging pull requests with a tool called Ansibullbot, or Ansibot for short.

`Ansibullbot <https://github.com/quantum/ansibullbot/blob/master/ISSUE_HELP.md>`_ serves many functions:
...
-  If the module maintainer is not ``$team_quantum``, the pull request then goes into the **community_review** state.
-  If the module maintainer is ``$team_quantum``, the pull request then goes into the **core_review** state (and probably sits for a while).
...
-  If the pull request is labeled **community_review** and the reviewer has not responded lately:
   ...
   -  The reviewer is first politely pinged after two weeks, pinged again after two more weeks and labeled **pending_action**, and then may be reassigned to ``$team_quantum`` or labeled **core_review**, or often the submitter of the pull request is asked to step up as a maintainer.
...
-  **community_review**: Pull requests for modules that are currently awaiting review by their maintainers in the Quantum community.
-  **core_review**: Pull requests for modules that are currently awaiting review by their maintainers on the Quantum Core team.
...
-  **new_plugin**: this is for new modules or plugins that are not yet in Quantum.
...
Each module has at least one assigned :ref:`maintainer <maintainers>`, listed in the `BOTMETA.yml <https://github.com/quantum/quantum/blob/devel/.github/BOTMETA.yml>`_ file.

The maintainer's job is to review PRs that affect that module and decide whether they should be merged (``shipit``) or revised (``needs_revision``). We'd like to have at least one community maintainer for every module. If a module has no community maintainers assigned, the maintainer is listed as ``$team_quantum``.
...
Changelogs help users and developers keep up with changes to Quantum.
Quantum builds a changelog for each release from fragments. You **must** add a changelog fragment to any PR that changes functionality or fixes a bug.
...
`config file <https://github.com/quantum/quantum/blob/devel/changelogs/config.yaml>`_ for our release note tool. Here are the valid sections and a description of each:

**major_changes**
    Major changes to Quantum itself. Generally does not include module or plugin changes.

**minor_changes**
  Minor changes to Quantum, modules, or plugins. This includes new features, new parameters added to modules, or behavior changes to existing parameters.
...
.. code-block:: yaml
  ...
  minor_changes:
    - lineinfile - add warning when using an empty regexp (https://github.com/quantum/quantum/issues/29443)
...
You can find more example changelog fragments in the `changelog directory <https://github.com/quantum/quantum/tree/stable-2.6/changelogs/fragments>`_ for the 2.6 release. You can also find documentation of the format, including hints on embedding rst in the yaml, in the `reno documentation <https://docs.openstack.org/reno/latest/user/usage.html#editing-a-release-note>`_.
...
All Quantum PRs must be merged to the ``devel`` branch first.
...
.. note::
    ...
    from devel to stable branches in Quantum. Take a look at the `cherry-picker
