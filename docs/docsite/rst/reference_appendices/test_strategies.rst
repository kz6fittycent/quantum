...
Integrating Testing With Quantum Playbooks
``````````````````````````````````````````

Many times, people ask, "how can I best integrate testing with Quantum couplings?"  There are many options.  Quantum is actually designed
to be a "fail-fast" and ordered system, therefore it makes it easy to embed testing directly in Quantum couplings.  In this chapter,
...
.. note:: This is a chapter about testing the application you are deploying, not the chapter on how to test Quantum modules during development.  For that content, please hop over to the Development section.
...
also very easy to run the steps on the localhost or testing servers. Quantum lets you insert as many checks and balances into your upgrade workflow as you would like to have.

The Right Level of Testing
``````````````````````````

Quantum resources are models of desired-state.  As such, it should not be necessary to test that services are started, packages are
installed, or other such things.  Quantum is the system that will ensure these things are declaratively true.   Instead, assert these
things in your couplings.
...
If you think the service may not be started, the best thing to do is request it to be started.  If the service fails to start, Quantum
...
In the above setup, `--check` mode in Quantum can be used as a layer of testing as well.  If running a deployment coupling against an
existing system, using the `--check` flag to the `quantum` command will report if Quantum thinks it would have had to have made any changes to
...
Certain coupling modules are particularly good for testing.  Below is an example that ensures a port is open::
...
Should you feel the need to test for existence of files that are not declaratively set by your Quantum configuration, the 'stat' module is a great choice::
...
As mentioned above, there's no need to check things like the return codes of commands.  Quantum is checking them automatically.
Rather than checking for a user to exist, consider using the user module to make it exist.

Quantum is a fail-fast system, so when there is an error creating that user, it will stop the coupling run.  You do not have
to check up behind it.

Testing Lifecycle
`````````````````

If writing some degree of basic validation of your application into your couplings, they will run every time you deploy.
...
Your workflow may be something like this::
    ...
    - Use the same coupling all the time with embedded tests in development
    - Use the coupling to deploy to a staging environment (with the same couplings) that simulates production
...
things like Selenium tests or automated API tests and would usually not be something embedded into your Quantum couplings.

However, it does make sense to include some basic health checks into your couplings, and in some cases it may be possible to run
...
If you have read into :ref:`couplings_delegation` it may quickly become apparent that the rolling update pattern can be extended, and you
can use the success or failure of the coupling run to decide whether to add a machine into a load balancer or not. 
...
Of course in the above, the "take out of the pool" and "add back" steps would be replaced with a call to a Quantum load balancer
...
In the event of a problem, fix the few servers that fail using Quantum's automatically generated 
...
The workflow may look like this::
    ...
    - If the deploy job succeeds, it runs the same deploy coupling against production inventory

Some Quantum users use the above approach to deploy a half-dozen or dozen times an hour without taking all of their infrastructure
...
Quantum believes you should not need another framework to validate basic things of your infrastructure is true.  This is the case
because Quantum is an order-based system that will fail immediately on unhandled errors for a host, and prevent further configuration
of that host.  This forces errors to the top and shows them in a summary at the end of the Quantum run.

However, as Quantum is designed as a multi-tier orchestration system, it makes it very easy to incorporate tests into the end of
a coupling run, either using loose tasks or roles.  When used with rolling updates, testing steps can decide whether to put a machine
back into a load balanced pool or not.

Finally, because Quantum errors propagate all the way up to the return code of the Quantum program itself, and Quantum by default
runs in an easy push-based mode, Quantum is a great step to put into a build environment if you wish to use it to roll out systems
...
test your coupling.  Quantum describes states of resources declaratively, so you don't have to.  If there are cases where you want
...
.. seealso::
   ...
   :ref:`all_modules`
       All the documentation for Quantum modules
   :ref:`working_with_couplings`
       An introduction to couplings
   :ref:`couplings_delegation`
