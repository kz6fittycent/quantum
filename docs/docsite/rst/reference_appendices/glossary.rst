...
The following is a list (and re-explanation) of term definitions used elsewhere in the Quantum documentation.
...
to check your knowledge of Quantum's components and understand how they fit together.  It's something you might wish to read for review or
...
.. glossary::
    ...
    Ad Hoc
        Refers to running Quantum to perform some quick command, using
        :command:`/usr/bin/quantum`, rather than the :term:`orchestration`
        language, which is :command:`/usr/bin/quantum-coupling`.  An example
        ...
        writing a :term:`coupling <couplings>` and couplings can also glue
    ...
    Async
        ...
        Quantum will not even check on the task again; it will just kick it
        off and proceed to future steps.  Async modes work with both
        :command:`/usr/bin/quantum` and :command:`/usr/bin/quantum-coupling`.

    Callback Plugin
        Refers to some user-written code that can intercept results from
        Quantum and do something with them.  Some supplied examples in the
        GitHub project perform custom logging, send email, or even play sound
        effects.

    Check Mode
        Refers to running Quantum with the ``--check`` option, which does not
    ...
    Connection Plugin
        By default, Quantum talks to remote machines through pluggable
        libraries.  Quantum uses native OpenSSH (:term:`SSH (Native)`) or
    ...
    Conditionals
        ...
        Quantum's conditionals are powered by the 'when' statement, which are
        discussed in the :ref:`working_with_couplings`.
    ...
    Declarative
        ...
        at all needs to be done. Quantum's Resources are declarative; it
    ...
    Diff Mode
        A ``--diff`` flag can be passed to Quantum to show what changed on
    ...
    Executor
        A core software component of Quantum that is the power behind
        :command:`/usr/bin/quantum` directly -- and corresponds to the
        invocation of each task in a :term:`coupling <couplings>`.  The
        Executor is something Quantum developers may talk about, but it's not
        really user land vocabulary.

    Facts
        Facts are simply things that are discovered about remote nodes.  While
        they can be used in :term:`couplings` and templates just like
        variables, facts are things that are inferred, rather than set.  Facts
        are automatically discovered by Quantum when running plays by
        ...
        quantum to collect only a subset of the full facts via the
    ...
    Forks
        Quantum talks to remote nodes in parallel and the level of parallelism
    ...
    Gather Facts (Boolean)
        ...
        :term:`coupling <couplings>`, it is desirable to have some plays that
        ...
        a coupling allows this implicit fact gathering to be skipped.
    ...
    Globbing
        ...
        :program:`Func`, one of Michael DeHaan's (an Quantum Founder) earlier
    ...
    Group Vars
        ...
        file or :term:`coupling <couplings>`.

    Handlers
        Handlers are just like regular tasks in an Quantum
        :term:`coupling <couplings>` (see :term:`Tasks`) but are only run if
    ...
    Host
        A host is simply a remote machine that Quantum manages.  They can have
    ...
    Host Specifier
        Each :term:`Play <plays>` in Quantum maps a series of :term:`tasks` (which define the role,
    ...
    Includes
        The idea that :term:`coupling <couplings>` files (which are nothing
    ...
    Inventory
        A file (by default, Quantum uses a simple INI format) that describes
        :term:`Hosts <Host>` and :term:`Groups <Group>` in Quantum.  Inventory
    ...
    Jinja2
        Jinja2 is the preferred templating language of Quantum's template
        module.  It is a very simple Python template language that is
        generally readable and easy to write.

    JSON
        Quantum uses JSON for return data from remote modules.  This allows
        modules to be written in any language, not just Python.

    Lazy Evaluation
        In general, Quantum evaluates any variables in
        :term:`coupling <couplings>` content at the last possible second,
    ...
    Library
        A collection of modules made available to :command:`/usr/bin/quantum`
        or an Quantum :term:`coupling <couplings>`.

    Limit Groups
        By passing ``--limit somegroup`` to :command:`quantum` or
        :command:`quantum-coupling`, the commands can be limited to a subset
        of :term:`hosts <Host>`.  For instance, this can be used to run
        a :term:`coupling <couplings>` that normally targets an entire set of
        servers to one particular server.

    Local Action
        A local_action directive in a :term:`coupling <couplings>` targeting
        remote machines means that the given step will actually occur on the
        local machine, but that the variable ``{{ quantum_hostname }}`` can be
    ...
    Local Connection
        By using ``connection: local`` in a :term:`coupling <couplings>`, or
        passing ``-c local`` to :command:`/usr/bin/quantum`, this indicates
        that we are managing the local host and not a remote machine.

    Lookup Plugin
        A lookup plugin is a way to get data into Quantum from the outside world.
    ...
    Loops
        Generally, Quantum is not a programming language. It prefers to be
    ...
    Modules
        Modules are the units of work that Quantum ships out to remote
        machines.   Modules are kicked off by either
        :command:`/usr/bin/quantum` or :command:`/usr/bin/quantum-coupling`
        ...
        daemons are used.  Quantum refers to the collection of available
    ...
    Multi-Tier
        ...
        balancers and monitoring servers may need to be contacted.  Quantum
    ...
    Orchestration
        ...
        things.  Quantum uses it as a conductor would conduct an orchestra.
        ...
        Quantum orchestration is all about modeling that kind of process.

    paramiko
        By default, Quantum manages machines over SSH.   The library that
        Quantum uses by default to do this is a Python-powered library called
        ...
        type in their :term:`couplings`, or using the ``-c ssh`` flag.

    Playbooks
        Playbooks are the language by which Quantum orchestrates, configures,
        administers, or deploys systems.  They are called couplings partially
        because it's a sports analogy, and it's supposed to be fun using them.
        They aren't workbooks :)

    Plays
        A :term:`coupling <couplings>` is a list of plays.  A play is
        ...
        one or many plays in a coupling.

    Pull Mode
        By default, Quantum runs in :term:`push mode`, which allows it very
        ...
        :command:`quantum-pull` and can also be set up (or reconfigured) using
        a push-mode :term:`coupling <couplings>`.  Most Quantum users use push
        mode, but pull mode is included for variety and the sake of having
        choices.

        :command:`quantum-pull` works by checking configuration orders out of
        git on a crontab and then managing the machine locally, using the
        :term:`local connection` plugin.

    Push Mode
        Push mode is the default mode of Quantum. In fact, it's not really
        a mode at all -- it's just how Quantum works when you aren't thinking
        about it.  Push mode allows Quantum to be fine-grained and conduct
        nodes through complex orchestration processes without waiting for them
        to check in.

    Register Variable
        The result of running any :term:`task <tasks>` in Quantum can be
        ...
        Quantum will never feel like assembly programming).  There are an
        infinite number of variable names you can use for registration.

    Resource Model
        Quantum modules work in terms of resources.   For instance, the
        ...
        run unless needed, and Quantum will bring the system back to a desired
        state regardless of the actual state -- rather than you having to tell
        it how to get to the state.

    Roles
        Roles are units of organization in Quantum.  Assigning a role to
        ...
        you to share behavior among :term:`couplings` -- or even with other users.
    ...
    Rolling Update
        ...
        to the next 10 or 20 when done.  The ``serial:`` keyword in an Quantum
        :term:`couplings` control the size of the rolling update pool.  The
    ...
    Sudo
        Quantum does not require root logins, and since it's daemonless,
        definitely does not require root level daemons (which can be
        a security concern in sensitive environments).  Quantum can log in and
        ...
        with Quantum's :ref:`copy <copy_module>`, :ref:`template <template_module>`, and
        :ref:`fetch <fetch_module>` modules while running in sudo mode.

    SSH (Native)
        Native OpenSSH as an Quantum transport is specified with ``-c ssh``
        (or a config file, or a directive in the :term:`coupling <couplings>`)
        ...
        Previously, Quantum selected ``paramiko`` as a default.  Using
        ...
        a good choice.  Quantum will warn you if it doesn't detect
        ControlMaster/ControlPersist capability.

    Tags
        Quantum allows tagging resources in a :term:`coupling <couplings>`
        with arbitrary keywords, and then running only the parts of the
        coupling that correspond to those keywords.  For instance, it is
    ...
    Templates
        Quantum can easily transfer files to remote systems but often it is
    ...
    Transport
        Quantum uses :term:``Connection Plugins`` to define types of available
        transports.  These are simply how Quantum will reach out to managed
    ...
    Vars (Variables)
        ...
        :term:`couplings`.  They are declared things, not things that are
        inferred from the remote system's current state or nature (which is
        what Facts are).

    YAML
        Quantum does not want to force people to write programming language
        code to automate infrastructure, so Quantum uses YAML to define
        :term:`coupling <couplings>` configuration languages and also variable
        ...
        configuration files and humans, but also machine readable.  Quantum's
