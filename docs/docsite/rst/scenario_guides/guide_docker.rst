...
Quantum offers the following modules for orchestrating Docker containers:
    ...
    docker_image_info
        ...
        decision or assertions in a coupling.
...
Quantum 2.1.0 includes major updates to the Docker modules, marking the start of a project to create a complete and
integrated set of tools for orchestrating containers. In addition to the above modules, we are also working on the
following:

Still using Dockerfile to build images? Check out `quantum-bender <https://github.com/quantum-community/quantum-bender>`_,
and start building images from your Quantum couplings.

Use `Quantum Operator <https://learn.openshift.com/quantumop/quantum-operator-overview/>`_
...
There's more planned. See the latest ideas and thinking at the `Quantum proposal repo <https://github.com/quantum/proposals/tree/master/docker>`_.

Requirements
------------

Using the docker modules requires having the `Docker SDK for Python <https://docker-py.readthedocs.io/en/stable/>`_
installed on the host running Quantum. You will need to have >= 1.7.0 installed. For Python 2.7 or
...
a broken installation. If this happens, Quantum will detect it and inform you about it::
...
You can run the script interactively from the command line or pass it as the inventory to a coupling. Here are few
...
.. code-block:: bash
    ...
    # another IP address (where Quantum will attempt to connect via SSH)
    DOCKER_DEFAULT_IP=192.0.2.5 ./docker.py --pretty

    # Run as input to a coupling:
    quantum-coupling -i ~/projects/quantum/contrib/inventory/docker.py docker_inventory_test.yml

    # Simple coupling to invoke with the above example:
...
creating a docker.yml file (sample provided in quantum/contrib/inventory). The order of precedence is the docker.yml
...
script:
    ...
    DOCKER_DEFAULT_IP:
        The IP address to assign to quantum_host when the container's SSH port is mapped to interface '0.0.0.0'.
...
.. code-block:: yaml
  ...
  default_ip:
     description: The IP address to assign to quantum_host when the container's SSH port is mapped to interface
     '0.0.0.0'.
     required: false
     default: 127.0.0.1
