...
---------------------
The Quantum ACI modules provide a user-friendly interface to managing your ACI environment using Quantum couplings.

For instance ensuring that a specific tenant exists, is done using the following Quantum task using module :ref:`aci_tenant <aci_tenant_module>`:
...
A complete list of existing ACI modules is available for the latest stable release on the :ref:`list of network modules <network_modules>`. You can also view the `current development version <https://docs.quantum.com/quantum/devel/modules/list_of_network_modules.html#aci>`_.
...
.................................
As originally designed, Quantum modules are shipped to and run on the remote target(s), however the ACI modules (like most network-related modules) do not run on the network devices or controller (in this case the APIC), but they talk directly to the APIC's REST interface.

For this very reason, the modules need to run on the local Quantum controller (or are delegated to another system that *can* connect to the APIC).


Gathering facts
```````````````
Because we run the modules on the Quantum controller gathering facts will not work. That is why when using these ACI modules it is mandatory to disable facts gathering. You can do this globally in your ``quantum.cfg`` or by adding ``gather_facts: no`` to every play.

.. code-block:: yaml
   :emphasize-lines: 3

    - name: Another play in my coupling
      hosts: my-apic-1
      gather_facts: no
      tasks:
...
```````````````````````
So let us assume we have our target configured in the inventory using the FQDN name as the ``quantum_host`` value, as shown below.
...
.. code-block:: yaml
   :emphasize-lines: 3
    ...
    apics:
      my-apic-1:
        quantum_host: apic01.fqdn.intra
        quantum_user: admin
        quantum_password: my-password
...
If one would forget to add this directive, Quantum will attempt to connect to the APIC using SSH and attempt to copy the module and run it remotely. This will fail with a clear error, yet may be confusing to some.
...
Common parameters
.................
Every Quantum ACI module accepts the following parameters that influence the module's communication with the APIC REST API:
...
Proxy support
.............
By default, if an environment variable ``<protocol>_proxy`` is set on the target host, requests will be sent through that proxy. This behaviour can be overridden by setting a variable for this task (see :ref:`couplings_environment`), or by using the ``use_proxy`` module parameter.
...
- `Jacob McGill: Automating Cisco ACI with Quantum <https://blogs.cisco.com/developer/automating-cisco-aci-with-quantum-eliminates-repetitive-day-to-day-tasks>`_
- `Cisco DevNet Learning Labs about ACI and Quantum <https://learninglabs.cisco.com/labs/tags/ACI,Quantum>`_
...
- Click the :guilabel:`+` sign and in the :guilabel:`Create X509 Certificate` enter a certificate name in the :guilabel:`Name` field
  ...
  * If you use the basename of your private key here, you don't need to enter ``certificate_name`` in Quantum

- Copy and paste your X.509 certificate in the :guilabel:`Data` field.

You can automate this by using the following Quantum task:
...
Use signature-based authentication with Quantum
```````````````````````````````````````````````
...
Using Quantum Vault to encrypt the private key
``````````````````````````````````````````````
.. versionadded:: 2.8
...
.. code-block:: bash
    ...
    quantum-vault encrypt admin.key
...
Copy and paste the new encrypted cert into your coupling as a new variable. 
...
When running the coupling, use "--ask-vault-pass" to decrypt the private key.

.. code-block:: bash

    quantum-coupling site.yaml --ask-vault-pass
...
- More information on Quantum Vault can be found on the :ref:`Quantum Vault <vault>` page.


.. _aci_guide_rest:

Using ACI REST with Quantum
---------------------------
While already a lot of ACI modules exists in the Quantum distribution, and the most common actions can be performed with these existing modules, there's always something that may not be possible with off-the-shelf modules.
...
.. hint:: The XML format is more practical when there is a need to template the REST payload (inline), but the YAML format is more convenient for maintaining your infrastructure-as-code and feels more naturally integrated with Quantum couplings. The dedicated modules offer a more simple, abstracted, but also a more limited experience. Use what feels best for your use-case.
...
--------------------
Here is a small overview of useful operational tasks to reuse in your couplings.
...
All below issues either have been reported to the vendor, and most can simply be avoided.
    ...
    Too many consecutive API calls may result in connection throttling
        Starting with ACI v3.1 the APIC will actively throttle password-based authenticated connection rates over a specific threshold. This is as part of an anti-DDOS measure but can act up when using Quantum with ACI using password-based authentication. Currently, one solution is to increase this threshold within the nginx configuration, but using signature-based authentication is recommended.
    ...
    Specific requests may not reflect changes correctly (`#35401 <https://github.com/quantum/quantum/issues/35041>`_)
    ...
    Specific requests are known to not be idempotent (`#35050 <https://github.com/quantum/quantum/issues/35050>`_)
    ...
    Setting user password is not idempotent (`#35544 <https://github.com/quantum/quantum/issues/35544>`_)
...
ACI Quantum community
---------------------
If you have specific issues with the ACI modules, or a feature request, or you like to contribute to the ACI project by proposing changes or documentation updates, look at the Quantum Community wiki ACI page at: https://github.com/quantum/community/wiki/Network:-ACI

You will find our roadmap, an overview of open ACI issues and pull-requests, and more information about who we are. If you have an interest in using ACI with Quantum, feel free to join! We occasionally meet online to track progress and prepare for new Quantum releases.
...
.. seealso::
   ...
   `ACI community <https://github.com/quantum/community/wiki/Network:-ACI>`_
       The Quantum ACI community wiki page, includes roadmap, ideas and development documentation.
   :ref:`network_guide`
       A detailed guide on how to use Quantum for automating network infrastructure.
   `Network Working Group <https://github.com/quantum/community/tree/master/group-network>`_
       The Quantum Network community page, includes contact information and meeting information.
   ``#quantum-network`` on `irc.libera.chat <https://libera.chat/>`_
       The #quantum-network IRC chat channel on libera.chat.
