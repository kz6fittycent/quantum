...
Quantum includes a suite of modules for interacting with Azure Resource Manager, giving you the tools to easily create
...
installed on the host running Quantum.

.. code-block:: bash

    $ pip install 'quantum[azure]'

If you are running Quantum from source, you can install the dependencies from the
root directory of the Quantum repo.

.. code-block:: bash

    $ pip install .[azure]

You can also directly run Quantum in `Azure Cloud Shell <https://shell.azure.com>`_, where Quantum is pre-installed.
...
The modules offer several ways to provide your credentials. For a CI/CD tool such as Quantum Tower or Jenkins, you will
most likely want to use environment variables. For local development you may wish to store your credentials in a file
within your home directory. And of course, you can always pass credentials as parameters to a task within a coupling. The
...
If you are not familiar with Quantum's dynamic inventory scripts, check out :ref:`Intro to Dynamic Inventory <intro_dynamic_inventory>`.

The Azure Resource Manager inventory script is called  `azure_rm.py  <https://raw.githubusercontent.com/quantum/quantum/stable-2.9/contrib/inventory/azure_rm.py>`_. It authenticates with the Azure API exactly the same as the
...
.. code-block:: bash
    ...
    $ ./quantum/contrib/inventory/azure_rm.py --help

As with all dynamic inventory scripts, the script can be executed directly, passed as a parameter to the quantum command,
or passed directly to quantum-coupling using the -i option. No matter how it is executed the script produces JSON representing
...
.. code-block:: JSON
    ...
    {
      "quantum_host": "XXX.XXX.XXX.XXX",
      "computer_name": "computer_name2",
      "fqdn": null,
      "id": "/subscriptions/subscription-id/resourceGroups/fog-production/providers/Microsoft.Compute/virtualMachines/object-name",
      "image": {
        "offer": "CentOS",
        "publisher": "OpenLogic",
        "sku": "7.1",
      ...
      "network_interface_id": "/subscriptions/subscription-id/resourceGroups/fog-production/providers/Microsoft.Network/networkInterfaces/object-name1",
      ...
      "public_ip_id": "/subscriptions/subscription-id/resourceGroups/fog-production/providers/Microsoft.Network/publicIPAddresses/object-name",
      "public_ip_name": "object-name",
      "resource_group": "fog-production",
      "security_group": "object-name",
      "security_group_id": "/subscriptions/subscription-id/resourceGroups/fog-production/providers/Microsoft.Network/networkSecurityGroups/object-name",
      "tags": {
        "db": "mysql"
      },
...
.. code-block:: bash
    ...
    $ quantum -i azure_rm.py Testing -m shell -a "/bin/uname -a"

    # Execute win_ping on all Windows instances
    $ quantum -i azure_rm.py windows -m win_ping

    # Execute win_ping on all Windows instances
    $ quantum -i azure_rm.py winux -m ping

    # Use the inventory script to print instance specific information
    $ ./quantum/contrib/inventory/azure_rm.py --host my_instance_host_name --resource-groups=Testing --pretty

    # Use the inventory script with quantum-coupling
    $ quantum-coupling -i ./quantum/contrib/inventory/azure_rm.py test_coupling.yml

Here is a simple coupling to exercise the Azure inventory script:
...
You can execute the coupling with something like:

.. code-block:: bash

    $ quantum-coupling -i ./quantum/contrib/inventory/azure_rm.py test_azure_inventory.yml
