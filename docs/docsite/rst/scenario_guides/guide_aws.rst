...
Quantum contains a number of modules for controlling Amazon Web Services (AWS).  The purpose of this
section is to explain how to put Quantum modules together (and use inventory scripts) to use Quantum in AWS context.
...
Whereas classically quantum will execute tasks in its host loop against multiple remote machines, most cloud-control steps occur on your local machine with reference to the regions to control.

In your coupling steps we'll typically be using the following pattern for provisioning steps::
...
For storing these in a vars_file, ideally encrypted with quantum-vault::
...
With the host group now created, a second play at the bottom of the same provisioning coupling file might now have some configuration steps::
...
This will also dynamically select nodes that were even created outside of Quantum, and allow Quantum to manage them.
...
in quantum can be used.  See :ref:`splitting_out_vars`.
...
Autoscaling with Quantum Pull
`````````````````````````````

Amazon Autoscaling features automatically increase or decrease capacity based on load.  There are also Quantum modules shown in the cloud documentation that
can configure autoscaling policy.

When nodes come online, it may not be sufficient to wait for the next cycle of an quantum command to come along and configure that node.  

To do this, pre-bake machine images which contain the necessary quantum-pull invocation.  Quantum-pull is a command line tool that fetches a coupling from a git server and runs it locally.  
...
Read :ref:`quantum-pull` for more information on pull-mode couplings.

.. _aws_autoscale:

Autoscaling with Quantum Tower
``````````````````````````````

:ref:`quantum_tower` also contains a very nice feature for auto-scaling use cases.  In this mode, a simple curl script can call
...
Quantum With (And Versus) CloudFormation
````````````````````````````````````````

CloudFormation is a Amazon technology for defining a cloud stack as a JSON or YAML document.   

Quantum modules provide an easier to use interface than CloudFormation in many examples, without defining a complex JSON/YAML document.
This is recommended for most users.

However, for users that have decided to use CloudFormation, there is an Quantum module that can be used to apply a CloudFormation template
to Amazon.

When using Quantum with CloudFormation, typically Quantum will be used with a tool like Packer to build images, and CloudFormation will launch
those images, or quantum will be invoked through user data once the image comes online, or a combination of the two.

Please see the examples in the Quantum CloudFormation module for more details.

.. _aws_image_build:

AWS Image Building With Quantum
```````````````````````````````
...
one of many programs can be used with Quantum couplings to define and upload a base image, which will then get its own AMI ID for usage with
the ec2 module or other Quantum AWS modules such as ec2_asg or the cloudformation module.   Possible tools include Packer, aminator, and Quantum's
ec2_ami module.  

Generally speaking, we find most users using Packer.

See the Packer documentation of the `Quantum local Packer provisioner <https://www.packer.io/docs/provisioners/quantum-local.html>`_ and `Quantum remote Packer provisioner <https://www.packer.io/docs/provisioners/quantum.html>`_.

If you do not want to adopt Packer at this time, configuring a base-image with Quantum after provisioning (as shown above) is acceptable.
...
Quantum ships with lots of modules for configuring a wide array of EC2 services.  Browse the "Cloud" category of the module
