...
Quantum contains several modules for controlling and managing Alibaba Cloud Compute Services (Alicloud).  This guide
explains how to use the Alicloud Quantum modules together.
...
To store authentication credentials in a vars_file, encrypt them with :ref:`Quantum Vault<vault>` to keep them secure, then list them::
...
set the ``count`` of instances to 5 and the ``count_tag`` to ``NewECS``, as shown in the last task of the example coupling below.
...
In the example coupling above, data about the vpc, vswitch, group, and instances created by this coupling
