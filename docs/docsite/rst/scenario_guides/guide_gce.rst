...
Quantum + Google have been working together on a set of auto-generated
Quantum modules designed to consistently and comprehensively cover the entirety
of the Google Cloud Platform (GCP).

Quantum contains modules for managing Google Cloud Platform resources,
...
and make them automatically available in your Quantum inventory.
...
Quantum community. You will find some overlapping functionality such as with
...
into the new "gcp_*" modules. Google is committed to ensuring the Quantum
...
Credentials
-----------
It's easy to create a GCP account with credentials for Quantum. You have multiple options to
...
* Machine Accounts: Use the permissions associated with the GCP Instance you're using Quantum on.
...
Once you have your credentials, there are two different ways to provide them to Quantum:
...
Set the following environment variables before running Quantum in order to configure your credentials:
...
The best way to interact with your hosts is to use the gcp_compute inventory plugin, which dynamically queries GCE and tells Quantum what nodes can be managed.

To be able to use this GCE dynamic inventory plugin, you need to enable it first by specifying the following in the ``quantum.cfg`` file:
...
Executing ``quantum-inventory --list -i <filename>.gcp.yml`` will create a list of GCP instances that are ready to be configured using Quantum.
...
The following coupling creates a GCE Instance. This instance relies on a GCP
...
Note that use of the "add_host" module above creates a temporary, in-memory group.  This means that a play in the same coupling can then manage machines
...
`````````````````````````````````
As of Quantum 2.8, we're encouraging everyone to move from the ``gce`` module to the
...
============================  ==========================================  ======================
 ...
 instance_names               Use a loop or multiple tasks.               Using loops is a more Quantum-centric way of creating multiple instances and gives you the most flexibility.
 ...
 num_instances                Use a loop                                  For maximum flexibility, we're encouraging users to use Quantum features to create multiple instances, rather than letting the module do it for you.
