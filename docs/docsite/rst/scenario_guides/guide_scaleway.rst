...
`Scaleway <https://scaleway.com>`_ is a cloud provider supported by Quantum, version 2.6 or higher via a dynamic inventory plugin and modules.
Those modules are:
...
.. note::
   This guide assumes you are familiar with Quantum and how it works.
   If you're not, have a look at :ref:`quantum_documentation` before getting started.
...
You can add an SSH key to your account by including the following task in a coupling:
...
The ``ssh_pub_key`` parameter contains your ssh public key as a string. Here is an example inside a coupling:


.. code-block:: yaml

    # SCW_API_KEY='XXX' quantum-coupling ./test/legacy/scaleway_ssh_coupling.yml
...
Take a look at this short coupling to see a working example using ``scaleway_compute``:

.. code-block:: yaml

    # SCW_TOKEN='XXX' quantum-coupling ./test/legacy/scaleway_compute.yml
...
Quantum ships with :ref:`scaleway_inventory`.
...
.. code-block:: bash
    ...
    $ quantum-inventory --list -i scaleway_inventory.yml
...
As the Scaleway API is S3 compatible, Quantum supports it natively through the modules: :ref:`s3_bucket_module`, :ref:`aws_s3_module`.
