...
Quantum offers a set of modules to interact with `Vultr <https://www.vultr.com>`_ cloud platform.
...
Before using the Quantum modules to interact with Vultr, ones need an API key.
...
.. code-block:: console
  ...
  #> VULTR_API_KEY=XXX quantum -m vultr_account_info localhost
  localhost | SUCCESS => {
    "changed": false,
    "vultr_account_info": {
        "balance": -8.9,
...
From that point on, only you creativity is the limit. Make sure to read the documentation of the `available modules <https://docs.quantum.com/quantum/latest/modules/list_of_cloud_modules.html#vultr>`_.


Dynamic Inventory
-----------------

Quantum provides a dynamic inventory plugin for `Vultr <https://www.vultr.com>`_.
The configuration process is exactly the same as the one for the modules.

To be able to use it you need to enable it first by specifying the following in the ``quantum.cfg`` file:
...
.. code-block:: console
  ...
  #> quantum-inventory -i vultr.yml --list
...
Quantum includes integration tests for all Vultr modules.
...
.. code-block:: shell
  ...
  $ cd quantum # location the quantum source is
...
.. code-block:: shell
  ...
  $ quantum-test integration cloud/vultr/ -v --diff --allow-unsupported
...
.. code-block:: shell
  ...
  $ quantum-test integration cloud/vultr/vultr_account_info -v --diff --allow-unsupported
