...
`Packet.net <https://packet.net>`_ is a bare metal infrastructure host that's supported by Quantum (>=2.3) via a dynamic inventory script and two cloud modules. The two modules are:
...
Note, this guide assumes you are familiar with Quantum and how it works. If you're not, have a look at their :ref:`docs <quantum_documentation>` before getting started.
...
In order to check the state of devices created by Quantum on Packet, it's a good idea to install one of the `Packet CLI clients <https://www.packet.net/developers/integrations/>`_. Otherwise you can check them via the `Packet portal <https://app.packet.net/portal>`_.
...
If you want to use an existing keypair, just copy the private and public key over to the coupling directory.


Device Creation
===============

The following code block is a simple coupling that creates one `Type 0 <https://www.packet.net/bare-metal/servers/type-0/>`_ server (the 'plan' parameter). You have to supply 'plan' and 'operating_system'. 'location' defaults to 'ewr1' (Parsippany, NJ). You can find all the possible values for the parameters via a `CLI client <https://www.packet.net/developers/integrations/>`_.
...
After running ``quantum-coupling coupling_create.yml``, you should have a server provisioned on Packet. You can verify via a CLI or in the `Packet portal <https://app.packet.net/portal#/projects/list/table>`__.
...
If your coupling acts on existing Packet devices, you can only pass the 'hostname' and 'device_ids' parameters. The following coupling shows how you can reboot a specific Packet device by setting the 'hostname' parameter:
...
You can also identify specific Packet devices with the 'device_ids' parameter. The device's UUID can be found in the `Packet Portal <https://app.packet.net/portal>`_ or by using a `CLI <https://www.packet.net/developers/integrations/>`_. The following coupling removes a Packet device using the 'device_ids' field:
...
The following coupling will create an SSH key, 3 Packet servers, and then wait until SSH is ready (or until 5 minutes passed). Make sure to substitute the discovery token URL in 'user_data', and the 'project_id' before running ``quantum-coupling``. Also, feel free to change 'plan' and 'facility'.
...
As with most Quantum modules, the default states of the Packet modules are idempotent, meaning the resources in your project will remain the same after re-runs of a coupling. Thus, we can keep the ``packet_sshkey`` module call in our coupling. If the public key is already in your Packet account, the call will have no effect.
...
The ``packet_device`` module has a ``wait_for_public_IPv`` that is used to specify the version of the IP address to wait for (valid values are ``4`` or ``6`` for IPv4 or IPv6). If specified, Quantum will wait until the GET API call for a device contains an Internet-routeable IP address of the specified version. When referring to an IP address of a created device in subsequent module calls, it's wise to use the ``wait_for_public_IPv`` parameter, or ``state: active`` in the packet_device module call.

Run the coupling:

.. code-block:: bash

    $ quantum-coupling coupling_coreos.yml

Once the coupling quits, your new devices should be reachable via SSH. Try to connect to one and check if etcd has started properly:
...
The dynamic inventory script queries the Packet API for a list of hosts, and exposes it to Quantum so you can easily identify and act on Packet devices. You can find it in Quantum's git repo at `contrib/inventory/packet_net.py <https://github.com/quantum/quantum/blob/stable-2.9/contrib/inventory/packet_net.py>`_.

The inventory script is configurable via a `ini file <https://github.com/quantum/quantum/blob/stable-2.9/contrib/inventory/packet_net.ini>`_.
...
.. code-block:: bash
    ...
    $ wget https://github.com/quantum/quantum/raw/stable-2.9/contrib/inventory/packet_net.py
    $ chmod +x packet_net.py
    $ wget https://github.com/quantum/quantum/raw/stable-2.9/contrib/inventory/packet_net.ini

In order to understand what the inventory script gives to Quantum you can run:
...
You can now target groups in couplings! The following coupling will install a role that supplies resources for an Quantum target into all devices in the "coreos_beta" group:

.. code-block:: yaml

    # coupling_bootstrap.yml
...
.. code-block:: bash
    ...
    $ quantum-coupling -u core -i packet_net.py coupling_bootstrap.yml
