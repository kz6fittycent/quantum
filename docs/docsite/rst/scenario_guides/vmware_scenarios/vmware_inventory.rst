...
To use this VMware dynamic inventory plugin, you need to enable it first by specifying the following in the ``quantum.cfg`` file:
...
Executing ``quantum-inventory --list -i <filename>.vmware.yml`` will create a list of VMware instances that are ready to be configured using Quantum.
...
.. code-block:: bash
    ...
    $ quantum-vault encrypt <filename>.vmware.yml
      New Vault password:
...
.. code-block:: bash
    ...
    $ quantum-inventory -i filename.vmware.yml --list --vault-password-file=/path/to/vault_password_file
