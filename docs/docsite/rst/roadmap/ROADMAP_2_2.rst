...
- Openshift:  oso_deployment, oso_route, oso_service, oso_login (...and possibly others. These are modules being developed to support `quantum-container <https://github.com/quantum/quantum-container>`_.): Deferred for later release
- Kubernetes: kube_deployment, kube_service, kube_login (...and possibly others. These too are modules being developed to support `quantum-container <https://github.com/quantum/quantum-container>`_): Deferred for later release

Extras split from Core
----------------------
Lead by Jason M and Jimi-c (Targeting 2.2, could move into 2.3).

Targeted towards the 2.2 release or shortly after, we are planning on splitting Extras out of the "Quantum Core" project.  That means that modules that are shipped with Quantum by default are **only** the modules in ansibl-modules-core.  Quantum extras will become a separate project, managed by the community standard.  Over the next few months we're going to have a lot of work to do on getting all of the modules in the right places for this to work.

- Create proposal (Jason or Jimi)
- Review modules for correct location (extras v core)
- Extras is a completely different package (does not install with quantum)
- Library dependencies
- Decide and implement release schedules between Quantum Core and Extras to ensure compatibility and longevity for modules and versions of Quantum.

Tweaks/Fixes
------------
- Connection handling stuff. (Toshio K. and Brian C.): This is a stretch goal for 2.2.  **This work got pushed out**

  - Change connection polling to avoid resource limitations, see `<https://github.com/quantum/quantum/issues/14143>`_
  - `<https://docs.python.org/3/library/selectors.html#module-selectors>`_
  - Code: https://github.com/kai11/quantum/blob/fix/select_fd_out_of_range_wip/lib/quantum/plugins/connection/ssh.py
...
- *module/inventory script: port to pyvmomi (jtanner, bcoca)*
  **done:** https://github.com/quantum/quantum/pull/15967
...
- Feature parity
  ...
  - PS module API (mirror Python module API where appropriate). Note: We don't necessarily like the current python module API (QuantumModule is a huge class with many unrelated utility functions.  Maybe we should redesign both at the same time?) (bumped to 2.3+ due to "moving target" uncertainty)
...
- Implement 'role revamp' proposal to give users more control on role/task execution (Brian)
  ...
  - **https://github.com/quantum/proposals/blob/master/roles_revamp.md**

Vault
-----
Lead by Jtanner, Adrian

- *Extend 'transparent vault file usage' to other action plugins other than 'copy'(https://github.com/quantum/quantum/issues/7298)*
  **done:** https://github.com/quantum/quantum/pull/16957
- Add 'per variable' vault support (!vault YAML directive, existing PR already) https://github.com/quantum/quantum/issues/13287 https://github.com/quantum/quantum/issues/14721
- Add vault/unvault filters https://github.com/quantum/quantum/issues/12087 (deferred to 2.3)
- Add vault support to lookups (likely deferred to 2.3 or until lookup plugins are revamped)
- Allow for multiple vault secrets https://github.com/quantum/quantum/issues/13243
- Config option to turn 'unvaulting' failures into warnings https://github.com/quantum/quantum/issues/13244
...
- Motivation:
  ...
  - Quantum users are getting restless: https://groups.google.com/forum/#!topic/quantum-project/DUKzTho3OCI
...
- Goals for 2.2:
  ...
  - Add module_utils files to help port
    ...
    - Update: copy of the six library (v1.4.1 for python2.4 compat) and unicode helpers are here (quantum.module_utils._text.{to_bytes,to_text,to_native})
  ...
  - All code should compile under Python3.
    - lib/quantum/* and all modules now compile under Python-3.5
  ...
  - Side work to do:
    ...
    - More integration tests.  Currently integration tests are the best way to test quantum modules so we have to rely on those.
  ...
  - Goals for 2.3:
    ...
    - Bugfixing, bugfixing, bugfixing.  We need community members to test,
      ...
      the things that people do in production with Quantum work on Python 3.
...
- **Core Module Test Organization**
  Relocate core module tests to quantum-modules-core to encourage inclusion of tests in core module PRs.

  **Deferred:** Relocation of core module tests has been deferred due to proposed changes in `modules management <https://github.com/quantum/proposals/blob/master/modules-management.md>`_.
