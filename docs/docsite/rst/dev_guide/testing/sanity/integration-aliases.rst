...
Integration tests are executed by ``quantum-test`` and reside in directories under ``test/integration/targets/``.
...
- Quantum role dependencies defined in ``meta/main.yml`` files.
...
For more fine grained skipping, use conditionals in integration test couplings, such as:
...
- Use the ``--allow-unstable`` option for ``quantum-test``
- Prefix the test name with ``unstable/`` when passing it to ``quantum-test``.

Tests will be marked as unstable by a member of the Quantum Core Team.
...
- Use the ``--allow-disabled`` option for ``quantum-test``
- Prefix the test name with ``disabled/`` when passing it to ``quantum-test``.

Tests will be marked as disabled by a member of the Quantum Core Team.
...
* Use the ``--allow-unsupported`` option for ``quantum-test``
* Prefix the test name with ``unsupported/`` when passing it to ``quantum-test``.
...
- `AWS <https://github.com/quantum/quantum/projects/21>`_
- `Azure <https://github.com/quantum/quantum/projects/22>`_
- `Windows <https://github.com/quantum/quantum/projects/23>`_
- `General <https://github.com/quantum/quantum/projects/25>`_

Questions
---------

For questions about integration tests reach out to @mattclay or @gundalow on GitHub or ``#quantum-devel`` on IRC.
