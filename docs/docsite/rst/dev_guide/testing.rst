...
Why test your Quantum contributions?
====================================
...
:integration:
  ...
  * Functional tests of modules and Quantum core functionality.
:units:
  * :ref:`testing_units`
...
immensely valuable.  Quantum users who understand writing couplings and roles should be
...
When Shippable detects an error and it can be linked back to a file that has been modified in the PR then the relevant lines will be added as a GitHub comment. For example::
   ...
   The test `quantum-test sanity --test pep8` failed with the following errors:

   lib/quantum/modules/network/foo/bar.py:509:17: E265 block comment should start with '# '

   The test `quantum-test sanity --test validate-modules` failed with the following errors:
   lib/quantum/modules/network/foo/bar.py:0:0: E307 version_added should be 2.4. Currently 2.3
   lib/quantum/modules/network/foo/bar.py:0:0: E316 ANSIBLE_METADATA.metadata_version: required key not provided @ data['metadata_version']. Got None
...
If you haven't already got Quantum available, use the local checkout by running::

  source hacking/env-setup

Then run the tests detailed in the GitHub comment::

  quantum-test sanity --test pep8
  quantum-test sanity --test validate-modules
...
If the issue persists, please contact us in the ``#quantum-devel`` irc channel on the `irc.libera.chat <https://libera.chat/>`_ IRC network.
...
Thankfully, helping to test Quantum is pretty straightforward, assuming you are familiar with how Quantum works.
...
Create a fresh area to work::
   ...
   git clone https://github.com/quantum/quantum.git quantum-pr-testing
   cd quantum-pr-testing
...
and destination repositories. It will look something like this::
   ...
   Someuser wants to merge 1 commit into quantum:devel from someuser:feature_branch_name

.. note:: Only test ``quantum:devel``

   It is important that the PR request target be ``quantum:devel``, as we do not accept pull requests into any other branch. Dot releases are cherry-picked manually by Quantum staff.
...
The username and branch at the end are the important parts, which will be turned into git commands as follows::
   ...
   git pull https://github.com/someuser/quantum.git feature_branch_name
...
The Quantum source includes a script that allows you to use Quantum directly from source without requiring a
full installation that is frequently used by developers on Quantum.
...
Any potential issues should be added as comments on the pull request (and it's acceptable to comment if the feature works as well), remembering to include the output of ``quantum --version``

Example::

   Works for me! Tested on `Quantum 2.3.0`.  I verified this on CentOS 6.5 and also Ubuntu 14.04.
...
`The online code coverage reports <https://codecov.io/gh/quantum/quantum>`_ are a good way
to identify areas for testing improvement in Quantum.  By following red colors you can
...
Quantum functions and increase testing coverage in areas where there is none is a valuable
way to help improve Quantum.

The code coverage reports only cover the ``devel`` branch of Quantum where new feature
development takes place.  Pull requests and new code will be missing from the codecov.io
coverage reports so local reporting is needed.  Most ``quantum-test`` commands allow you
...
If you'd like to know more about the plans for improving testing Quantum then why not join the
`Testing Working Group <https://github.com/quantum/community/blob/master/meetings/README.md>`_.
