...
As described in :ref:`dynamic_inventory`, Quantum can pull inventory information from dynamic sources,
...
While with :ref:`inventory plugins <inventory_plugins>` you can leverage the Quantum codebase to add these common features.
...
Inventory plugins normally only execute at the start of a run, before couplings/plays and roles are loaded,
...
.. code-block:: python
    ...
    class InventoryModule(BaseInventoryPlugin):
        ...
        NAME = 'myplugin'  # used internally by Quantum, it should match the file name but not required
...
This method is used by Quantum to make a quick determination if the inventory source is usable by the plugin. It does not need to be 100% accurate as there might be overlap in what plugins can handle and Quantum will try the enabled plugins (in order) by default.
...
It is up to the plugin now to deal with the inventory source provided and translate that into the Quantum inventory.
       ...
       def parse(self, inventory, loader, path, cache=True):
            ...
            for colo in mydata:
                for server in mydata[colo]['servers']:
                    ...
                    self.inventory.set_variable(server['name'], 'quantum_host', server['external_ip'])

The specifics will vary depending on API and structure returned. But one thing to keep in mind, if the inventory source or any other issue crops up you should ``raise QuantumParserError`` to let Quantum know that the source was invalid or the process failed.

For examples on how to implement an inventory plugin, see the source code here:
`lib/quantum/plugins/inventory <https://github.com/quantum/quantum/tree/devel/lib/quantum/plugins/inventory>`_.
...
Inventory scripts must accept the ``--list`` and ``--host <hostname>`` arguments, other arguments are allowed but Quantum will not use them.
...
When called with the argument ``--host <hostname>`` (where <hostname> is a host from above), the script must print either an empty JSON hash/dictionary, or a hash/dictionary of variables to make available to templates and couplings. For example::
...
The stock inventory script system detailed above works for all versions of Quantum,
...
To satisfy the requirements of using ``_meta``, to prevent quantum from calling your inventory with ``--host`` you must at least populate ``_meta`` with an empty ``hostvars`` dictionary.
For example::
...
An easy way to see how this should look is using :ref:`quantum-inventory`, which also supports ``--list`` and ``--host`` parameters like an inventory script would.
...
.. seealso::
   ...
   `AWX <https://github.com/quantum/awx>`_
       REST API endpoint and GUI for Quantum, syncs with dynamic inventory
