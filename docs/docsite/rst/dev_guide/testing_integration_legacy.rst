...
This page details how to run the integration tests that haven't been ported to the new ``quantum-test`` framework.
...
Over time the above list will be reduced as tests are ported to the ``quantum-test`` framework.
...
Subsets of tests may be run by ``#commenting`` out unnecessary roles in the appropriate coupling, such as ``test/integration/amazon.yml``.
...
In order to run the tests in an AWS account quantum needs fairly wide ranging powers which
...
the tests, a lambda role `quantum_integration_tests` has to be created which
...
If you haven't already got Quantum available use the local checkout by doing::
