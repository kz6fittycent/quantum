...
********************
Quantum and Python 3
********************

.. contents:: Topics
   :local:

Quantum maintains a single code base that runs on both
Python 2 and Python 3 because we want Quantum to be able to manage a wide
variety of machines.  Contributors to Quantum should be aware of the tips in
this document so that they can write code that will run on the same versions
of Python as the rest of Quantum.

To ensure that your code runs on Python 3 as well as on Python 2, learn the tips and tricks and idioms
described here. Most of these considerations apply to all three types of Quantum code:

1. controller-side code - code that runs on the machine where you invoke :command:`/usr/bin/quantum`
2. modules - the code which Quantum transmits to and invokes on the managed machine.
...
.. note:: Python 2.4 Module-side Support:
    ...
    Support for Python 2.4 and Python 2.5 was dropped in Quantum-2.4.  RHEL-5
    (and its rebuilds like CentOS-5) were supported until April of 2017.
    Quantum-2.3 was released in April of 2017 and was the last Quantum release
    to support Python 2.4 on the module-side.

Developing Quantum code that supports Python 2 and Python 3
===========================================================
...
Quantum uses different strategies for working with strings in controller-side code, in
...
.. note:: Much of Quantum assumes that all encoded text is UTF-8.  At some
...
One of the main places in Quantum's controller code that we interact with
...
easier on the community members who maintain so many of Quantum's
...
In ``module_utils`` code we use a hybrid string strategy. Although Quantum's
...
They convert their string parameters into text (using ``quantum.module_utils._text.to_text``)
at the beginning of the function, do their work, and then convert
the return values into the native string type (using ``quantum.module_utils._text.to_native``)
...
Import Quantum's bundled ``python-six`` library
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The third-party `python-six <https://pythonhosted.org/six/>`_ library exists
to help projects create code that runs on both Python 2 and Python 3.  Quantum
...
.. note:: Quantum can also use a system copy of six

    Quantum will use a system copy of six if the system copy is a later
    version than the one Quantum bundles.
...
added until Python 2.7, so you need to remember not to use it in Quantum code:
...
.. code-block:: python
    ...
    b_command_line = b'quantum-coupling --become-user %s -K %s' % (user, coupling_file)
...
.. note:: Percent formatting added in Python 3.5
    ...
    However, if you happen to be testing Quantum code with Python 3.4 or
...
Quantum modules are slightly harder to code to support Python 3 than normal code from other
projects. A lot of mocking has to go into unit testing an Quantum module, so
