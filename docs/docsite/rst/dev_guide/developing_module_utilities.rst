...
Quantum provides a number of module utilities, or snippets of shared code, that
...
Quantum library, and all Python Quantum modules must import something from
``quantum.module_utils``. A common option is to import ``QuantumModule``::

  from quantum.module_utils.basic import QuantumModule

The ``quantum.module_utils`` namespace is not a plain Python package: it is
constructed dynamically for each task invocation, by extracting imports and
resolving those matching the namespace against a :ref:`search path <quantum_search_path>` derived from the
...
duplicated code into one or more module utilities and import them into your modules. For example, if you have your own custom modules that import a ``my_shared_code`` library, you can place that into a ``./module_utils/my_shared_code.py`` file like this::
  ...
  from quantum.module_utils.my_shared_code import MySharedCodeClient

When you run ``quantum-coupling``, Quantum will merge any files in your local ``module_utils`` directories into the ``quantum.module_utils`` namespace in the order defined by the :ref:`Quantum search path <quantum_search_path>`.
...
* ``lib/quantum/module_utils/urls.py`` contains shared code for parsing URLs
* ``lib/quantum/module_utils/storage/emc/`` contains shared code related to EMC
*  ``lib/quantum/modules/storage/emc/`` contains modules related to EMC
...
Quantum ships with an extensive library of ``module_utils`` files.
You can find the module
utility source code in the ``lib/quantum/module_utils`` directory under
your main Quantum path. We've described the most widely used utilities below. For more details on any specific module utility,
please see the `source code for module_utils <https://github.com/quantum/quantum/tree/devel/lib/quantum/module_utils>`_.

.. include:: shared_snippets/licensing.txt

- ``api.py`` - Supports generic API modules
- ``basic.py`` - General definitions and helper utilities for Quantum modules
...
- ``facts/`` - Directory of utilities for modules that return facts. See `PR 23012 <https://github.com/quantum/quantum/pull/23012>`_ for more information
