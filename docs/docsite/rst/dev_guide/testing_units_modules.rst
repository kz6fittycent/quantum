...
This document explains why, how and when you should use unit tests for Quantum modules.
The document doesn't apply to other parts of Quantum for which the recommendations are
normally closer to the Python standard.  There is basic documentation for Quantum unit
tests in the developer guide :ref:`testing_units`.  This document should
be readable for a new Quantum module author. If you find it incomplete or confusing,
please open a bug or ask for help on Quantum IRC.

What Are Unit Tests?
====================

Quantum includes a set of unit tests in the :file:`test/units` directory. These tests primarily cover the
internals but can also can cover Quantum modules.  The structure of the unit tests matches
...
cases cannot be verified using integration tests.  This means that Quantum unit test cases
...
Quantum unit tests have advantages and disadvantages. It is important to understand these.
Advantages include:

* Most unit tests are much faster than most Quantum integration tests.  The complete suite
...
Normally the Quantum integration tests (which are written in Quantum YAML) provide better
...
* Situations where the integration tests cannot be run as part of the main Quantum
...
How to unit test Quantum modules
================================
...
simulating an API. As for 'six', the 'mock' python package is bundled with Quantum (use
``import quantum.compat.tests.mock``). See for example
...
above, either by throwing an exception or ensuring that they haven't been called. For example::
    ...
    module = MagicMock()
    module.exit_json.side_effect = QuantumExitJson(Exception)
    with self.assertRaises(QuantumExitJson) as result:
...
API interaction is usually best tested with the function tests defined in Quantum's
...
an API that Quantum uses but which are beyond the control of the user.
...
Quantum special cases for unit testing
======================================

There are a number of special cases for unit testing the environment of an Quantum module.
The most common are documented below, and suggestions for others can be found by looking
at the source code of the existing unit tests or asking on the Quantum IRC channel or mailing
lists.
...
.. This section should be updated once https://github.com/quantum/quantum/pull/31456 is
...
handled through the :class:`QuantumModule` object in the basic section of the utilities. Normally
...
the arguments as above, set up the appropriate exit exception and then run the module::
    ...
    import pytest
    import quantum.modules.module.group.my_module as my_module

    def test_main_function(monkeypatch):
        monkeypatch.setattr(my_module.QuantumModule, "exit_json", fake_exit_json)
        set_module_args({
            'activationkey': 'key',
            'username': 'user',
            'password': 'pass',
...
Module must use :meth:`QuantumModule.run_command` in order to execute an external command. This
method needs to be mocked:

Here is a simple mock of :meth:`QuantumModule.run_command` (taken from :file:`test/units/modules/packaging/os/test_rhn_register.py`)::

        with patch.object(basic.QuantumModule, 'run_command') as run_command:
...
mock for :meth:`Quantum.get_bin_path`::

    import json

    from quantum.compat.tests import unittest
    from quantum.compat.tests.mock import patch
    from quantum.module_utils import basic
    from quantum.module_utils._text import to_bytes
    from quantum.modules.namespace import my_module
    ...
    def get_bin_path(self, arg, required=False):
        """Mock QuantumModule.get_bin_path"""
    ...
    class TestMyModule(unittest.TestCase):
        ...
        def setUp(self):
            self.mock_module_helper = patch.multiple(basic.QuantumModule,
                                                     exit_json=exit_json,
                                                     fail_json=fail_json,
        ...
        def test_module_fail_when_required_args_missing(self):
            with self.assertRaises(QuantumFailJson):
                set_module_args({})
                self.module.main()
        ...
        def test_ensure_command_called(self):
            ...
            with patch.object(basic.QuantumModule, 'run_command') as mock_run_command:
...
This now makes it possible to run tests against the module initiation function::
    ...
    def test_rds_module_setup_fails_if_db_instance_identifier_parameter_missing():
        ...
        with self.assertRaises(QuantumFailJson) as result:
...
the code in Quantum to trigger that failure.
...
.. seealso::
   ...
   :ref:`testing_units`
       Quantum unit tests documentation
