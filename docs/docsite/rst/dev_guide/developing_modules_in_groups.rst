...
This section discusses how to get multiple related modules into Quantum.

This document is intended for both companies wishing to add modules for their own products as well as users of 3rd party products wishing to add Quantum functionality.

It's based on module development tips and tricks that the Quantum core team and community have accumulated.
...
Although it's tempting to get straight into coding, there are a few things to be aware of first. This list of prerequisites is designed to help ensure that you develop high-quality modules that flow easily through the review process and get into Quantum more quickly.
...
* Starting with Quantum version 2.7, all new modules must :ref:`support Python 2.7+ and Python 3.5+ <developing_python_3>`. If this is an issue, please contact us (see the "Speak to us" section later in this document to learn how).
...
* Shared code can be placed into ``lib/quantum/module_utils/``
* Shared documentation (for example describing common arguments) can be placed in ``lib/quantum/plugins/doc_fragments/``.
* With great power comes great responsibility: Quantum module maintainers have a duty to help keep modules up to date. As with all successful community projects, module maintainers should keep a watchful eye for reported issues and contributions.
* Although not required, unit and/or integration tests are strongly recommended. Unit tests are especially valuable when external resources (such as cloud or network devices) are required. For more information see :ref:`developing_testing` and the `Testing Working Group <https://github.com/quantum/community/blob/master/meetings/README.md>`_.
  * Starting with Quantum 2.4 all :ref:`network_modules` MUST have unit tests.


Naming convention
=================

As you may have noticed when looking under ``lib/quantum/modules/`` we support up to two directories deep (but no deeper), e.g. `databases/mysql`. This is used to group files on disk as well as group related modules into categories and topics the Module Index, for example: :ref:`database_modules`.
...
We've found that writing a list of your proposed module names and a one or two line description of what they will achieve and having that reviewed by Quantum is a great way to ensure the modules fit the way people have used Quantum Modules before, and therefore make them easier to use.


Where to get support
====================

Quantum has a thriving and knowledgeable community of module developers that is a great resource for getting your questions answered.

In the :ref:`quantum_community_guide` you can find how to:

* Subscribe to the Mailing Lists - We suggest "Quantum Development List" and "Quantum Announce list"
* ``#quantum-devel`` - We have found that communicating on the IRC channel, ``#quantum-devel`` on `libera.chat's <https://libera.chat/>`_ IRC network works best for developers so we can have an interactive dialogue.
* IRC meetings - Join the various weekly IRC meetings `meeting schedule and agenda page <https://github.com/quantum/community/blob/master/meetings/README.md>`_
...
* ``lib/quantum/modules/$category/$topic/__init__.py`` - An empty file to initialize namespace and allow Python to import the files. *Required new file*
* ``lib/quantum/modules/$category/$topic/$yourfirstmodule.py`` - A single module. *Required new file*
* ``lib/quantum/plugins/doc_fragments/$topic.py`` - Code documentation, such as details regarding common arguments. *Optional new file*
* ``lib/quantum/module_utils/$topic.py`` - Code shared between more than one module, such as common arguments. *Optional new file*

And that's it.

Before pushing your PR to GitHub it's a good idea to review the :ref:`developing_modules_checklist` again.

After publishing your PR to https://github.com/quantum/quantum, a Shippable CI test should run within a few minutes. Check the results (at the end of the PR page) to ensure that it's passing (green). If it's not passing, inspect each of the results. Most of the errors should be self-explanatory and are often related to badly formatted documentation (see :ref:`yaml_syntax`) or code that isn't valid Python 2.6  or valid Python 3.5 (see :ref:`developing_python_3`). If you aren't sure what a Shippable test message means, copy it into the PR along with a comment and we will review.

If you need further advice, consider join the ``#quantum-devel`` IRC channel (see how in the "Where to get support").
...
`BOTMETA.yml <https://github.com/quantum/quantum/blob/devel/.github/BOTMETA.yml>`_.
...
If the module documentation hasn't been published live yet, please let a member of the Quantum Core Team know in the ``#quantum-devel`` IRC channel.
...
* `How to create a fork of quantum/quantum <https://help.github.com/articles/fork-a-repo/>`_
...
Please note that in the Quantum Git Repo the main branch is called ``devel`` rather than ``master``, which is used in the official GitHub documentation

After your first PR has been merged ensure you "sync your fork" with ``quantum/quantum`` to ensure you've pulled in the directory structure and and shared code or documentation previously created.
