...
****************************
This is a brief walk-through of how to create new Cisco ACI modules for Quantum.
...
Introduction
============
Quantum already ships with a large collection of Cisco ACI modules, however the ACI object model is huge and covering all possible functionality would easily cover more than 1500 individual modules.
...
.. code-block:: python
    ...
    from quantum.module_utils.aci import ACIModule, aci_argument_spec
...
Using the QuantumModule object
------------------------------
The following section creates an QuantumModule instance. The module should support check-mode, so we pass the ``argument_spec`` and  ``supports_check_mode`` arguments. Since these modules support querying the APIC for all objects of the module's class, the object/parent IDs should only be required if ``state: absent`` or ``state: present``.
...
--------------------------
The ACIModule class handles most of the logic for the ACI modules. The ACIModule extends functionality to the QuantumModule object, so the module instance must be passed into the class instantiation.
...
.. code-block:: guess
    ...
    #!/usr/bin/python
    import json
    from quantum.module_utils.network.aci.aci import ACIModule

    # Just another class mimicing a bare QuantumModule class for construct_url() and payload() methods
    class AltModule():
        params = dict(
            host='dummy',
            port=123,
...
.. code-block:: bash
    ...
    $ quantum-test sanity --python 2.7 lib/quantum/modules/network/aci/aci_tenant.py
...
.. code-block:: bash
    ...
    $ quantum-test network-integration --continue-on-error --allow-unsupported --diff -v aci_tenant
...
.. code-block:: bash
    ...
    $ quantum-test network-integration --python 2.7 --allow-unsupported --coverage aci_tenant
    $ quantum-test coverage report
