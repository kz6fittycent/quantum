...
further information see the `team community page <https://github.com/quantum/community/wiki/VMware>`_.
...
By default, the tests run against a vCenter API simulator called `govcsim <https://github.com/vmware/govmomi/tree/master/vcsim>`_. ``quantum-test`` will automatically pull a `govcsim container <https://quay.io/repository/quantum/vcenter-test-container>` and use it to set-up the test environment.

You can trigger the test of a module manually with the ``quantum-test`` command. For example, to trigger ``vcenter_folder`` tests:

.. code-block:: shell

    source hacking/env-setup
    quantum-test integration --python 3.7 vcenter_folder
...
:file:`test/lib/quantum_test/config/cloud-config-vcenter.ini.template`. For instance, if you've deployed your lab with
...
- quantum-test: vcenter behind an HTTP proxy <https://github.com/quantum/quantum/pull/58208>
- pyvmomi: proxy support <https://github.com/vmware/pyvmomi/pull/799>
- VMware: add support for HTTP proxy in connection API <https://github.com/quantum/quantum/pull/52936>

Once you have incorporated the code from those PRs, specify the location of the proxy server with the two extra keys:

.. code-block:: ini

    vmware_proxy_host: esxi1-gw.ws.testing.quantum.com
...
.. code-block:: shell
    ...
    source hacking/env-setup
    VMWARE_TEST_PLATFORM=static quantum-test integration --python 3.7 vmware_host_firewall_manager

``vmware_host_firewall_manager`` is the name of the module to test.

``vmware_guest`` is much larger than any other test role and is rather slow. You can enable or disable some of its test couplings in
...
.. code-block:: shell
    ...
    source hacking/env-setup
    quantum-test units --tox --python 3.7 '.*vmware.*'
...
not realize that Quantum silently targets the wrong data center.
...
the standard Quantum :ref:`integration testing<testing_integration>` process.
