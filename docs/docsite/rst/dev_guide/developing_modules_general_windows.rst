...
Quantum, Windows modules need to be written and tested for Windows hosts.
While evaluation editions of Windows can be downloaded from
Microsoft, these images are usually not ready to be used by Quantum without
further modification. The easiest way to set up a Windows host so that it is
ready to by used by Quantum is to set up a virtual machine using Vagrant.
...
its process once the listeners are online, after which the VM can be used by Quantum.

Create an Quantum inventory
===========================

The following Quantum inventory file can be used to connect to the newly
created Windows VM:

.. code-block:: ini

    [windows]
    WindowsServer  quantum_host=127.0.0.1

    [windows:vars]
    quantum_user=vagrant
    quantum_password=vagrant
    quantum_port=55986
    quantum_connection=winrm
    quantum_winrm_transport=ntlm
...
at `quantum-windows <https://github.com/jborean93/quantum-windows/tree/master/vagrant>`_
can be used to create a test domain environment to be used in Quantum. This
repo contains three files which are used by both Quantum and Vagrant to create
...
- ``main.yml``: Quantum coupling called by Vagrant to provision the domain controller and join the child hosts to the domain
...
``quantum_host`` is the private IP that will be assigned to the VirtualBox host
...
.. code-block:: shell
    ...
    git clone https://github.com/jborean93/quantum-windows.git
    cd vagrant
    vagrant up

.. note:: Vagrant provisions each host sequentially so this can take some time
    to complete. If any errors occur during the Quantum phase of setting up the
...
.. note:: While an SSH server is available on all Windows hosts but Server
    2008 (non R2), it is not a support connection for Quantum managing Windows
    hosts and should not be used with Quantum.
...
- Most new modules require check mode and integration tests before they are merged into the main Quantum codebase
...
- Look for common functions in ``./lib/quantum/module_utils/powershell/`` and use the code there instead of duplicating work. These can be imported by adding the line ``#Requires -Module *`` where * is the filename to import, and will be automatically included with the module code sent to the Windows target when run via Quantum
- As well as PowerShell module utils, C# module utils are stored in ``./lib/quantum/module_utils/csharp/`` and are automatically imported in a module execution if the line ``#QuantumRequires -CSharpUtil *`` is present
...
- Quantum runs modules under strictmode version 2.0. Be sure to test with that enabled by putting ``Set-StrictMode -Version 2.0`` at the top of your dev script
...
A very basic powershell module `win_environment <https://github.com/quantum/quantum/blob/devel/lib/quantum/modules/windows/win_environment.ps1>`_ is included below. It demonstrates how to implement check-mode and diff-support, and also shows a warning to the user when a specific condition is met.

.. .. include:: ../../../../lib/quantum/modules/windows/win_environment.ps1
..    :code: powershell

.. literalinclude:: ../../../../lib/quantum/modules/windows/win_environment.ps1
   :language: powershell

A slightly more advanced module is `win_uri <https://github.com/quantum/quantum/blob/devel/lib/quantum/modules/windows/win_uri.ps1>`_ which additionally shows how to use different parameter types (bool, str, int, list, dict, path) and a selection of choices for parameters, how to fail a module and how to handle exceptions.

As part of the new ``QuantumModule`` wrapper, the input parameters are defined and validated based on an argument
...
- ``type``: The type of the module option, if not set then it defaults to ``str``. The valid types are;
    ...
    * ``raw``: No conversions occur on the value passed in by Quantum
...
into PowerShell as well as some Quantum-specific requirements specified by
``#QuantumRequires``. These statements can be placed at any point in the script,
...
These are the checks that can be used within Quantum modules:

- ``#Requires -Module Quantum.ModuleUtils.<module_util>``: Added in Quantum 2.4, specifies a module_util to load in for the module execution.
- ``#Requires -Version x.y``: Added in Quantum 2.5, specifies the version of PowerShell that is required by the module. The module will fail if this requirement is not met.
- ``#QuantumRequires -OSVersion x.y``: Added in Quantum 2.5, specifies the OS build version that is required by the module and will fail if this requirement is not met. The actual OS version is derived from ``[Environment]::OSVersion.Version``.
- ``#QuantumRequires -Become``: Added in Quantum 2.5, forces the exec runner to run the module with ``become``, which is primarily used to bypass WinRM restrictions. If ``quantum_become_user`` is not specified then the ``SYSTEM`` account is used instead.
- ``#QuantumRequires -CSharpUtil Quantum.<module_util>``: Added in Quantum 2.8, specifies a C# module_util to load in for the module execution.

C# module utils can reference other C# utils by adding the line
``using Quantum.<module_util>;`` to the top of the script with all the other
using statements.
...
This will import the module_util at ``./lib/quantum/module_utils/powershell/Quantum.ModuleUtils.Legacy.psm1``
and enable calling all of its functions. As of Quantum 2.8, Windows module
utils can also be written in C# and stored at ``lib/quantum/module_utils/csharp``.
...
This will import the module_util at ``./lib/quantum/module_utils/csharp/Quantum.Basic.cs``
...
The following is a list of module_utils that are packaged with Quantum and a general description of what
they do:
...
- Legacy: General definitions and helper utilities for Quantum module.
...
For more details on any specific module utility and their requirements, please see the `Quantum
module utilities source code <https://github.com/quantum/quantum/tree/devel/lib/quantum/module_utils/powershell>`_.

PowerShell module utilities can be stored outside of the standard Quantum
distribution for use with custom modules. Custom module_utils are placed in a
folder called ``module_utils`` located in the root folder of the coupling or role
directory.

C# module utilities can also be stored outside of the standard Quantum distribution for use with custom modules. Like
PowerShell utils, these are stored in a folder called ``module_utils`` and the filename must end in the extension
``.cs``, start with ``Quantum.``  and be named after the namespace defined in the util.

The below example is a role structure that contains two PowerShell custom module_utils called
``Quantum.ModuleUtils.ModuleUtil1``, ``Quantum.ModuleUtils.ModuleUtil2``, and a C# util containing the namespace
``Quantum.CustomUtil``::
    ...
    defaults/
      main.yml
    module_utils/
      Quantum.ModuleUtils.ModuleUtil1.psm1
      Quantum.ModuleUtils.ModuleUtil2.psm1
      Quantum.CustomUtil.cs
    tasks/
      main.yml
...
Windows coupling module testing
===============================

You can test a module with an Quantum coupling. For example:

- Create a coupling in any directory ``touch testmodule.yml``.
...
- Run the coupling ``quantum-coupling -i hosts testmodule.yml``

This can be useful for seeing how Quantum runs with
...
- Copy the folders ``./lib/quantum/module_utils/powershell`` and ``./lib/quantum/module_utils/csharp`` to the same directory as the script above
...
.. code-block:: powershell
    ...
    # Set $ErrorActionPreference to what's set during Quantum execution
    ...
    $complex_args = @{
        _quantum_check_mode = $false
        _quantum_diff = $false
        path = "C:\temp"
        state = "present"
    }

    # Import any C# utils referenced with '#QuantumRequires -CSharpUtil' or 'using Quantum.;
    # The $_csharp_utils entries should be the context of the C# util files and not the path
    Import-Module -Name "$($pwd.Path)\powershell\Quantum.ModuleUtils.AddType.psm1"
    $_csharp_utils = @(
        [System.IO.File]::ReadAllText("$($pwd.Path)\csharp\Quantum.Basic.cs")
    )
    Add-CSharpType -References $_csharp_utils -IncludeDebugInfo

    # Import any PowerShell modules referenced with '#Requires -Module`
    Import-Module -Name "$($pwd.Path)\powershell\Quantum.ModuleUtils.Legacy.psm1"
...
To be able to view the arguments as passed by Quantum to the module follow
these steps.

- Prefix the Quantum command with :envvar:`ANSIBLE_KEEP_REMOTE_FILES=1<ANSIBLE_KEEP_REMOTE_FILES>` to specify that Quantum should keep the exec files on the server.
- Log onto the Windows server using the same user account that Quantum used to execute the module.
- Navigate to ``%TEMP%\..``. It should contain a folder starting with ``quantum-tmp-``.
...
Currently there is no mechanism to run unit tests for Powershell modules under Quantum CI.


Windows integration testing
===========================

Integration tests for Quantum modules are typically written as Quantum roles. These test
roles are located in ``./test/integration/targets``. You must first set up your testing
environment, and configure a test inventory for Quantum to connect to.
...
- To execute the integration tests, run ``quantum-test windows-integration win_stat``; you can replace ``win_stat`` with the role you wish to test.
...
Join the ``#quantum-devel`` or ``#quantum-windows`` irc channels on `irc.libera.chat <https://libera.chat/>`_ for discussions about Quantum development for Windows.
