...
Python program to help test or validate Quantum modules.

``validate-modules`` is one of the ``quantum-test`` Sanity Tests, see :ref:`testing_sanity` for more information.
...
The ``validate-modules`` tool has a `schema.py <https://github.com/quantum/quantum/blob/devel/test/lib/quantum_test/_data/sanity/validate-modules/validate_modules/schema.py>`_ that is used to validate the YAML blocks, such as ``DOCUMENTATION`` and ``RETURNS``.
...
------------------------------------------------------------   ------------------   --------------------   -----------------------------------------------------------------------------------------
  quantum-module-not-initialized                               Syntax               Error                  Execution of the module did not result in initialization of QuantumModule
  ...
  invalid-extension                                            Naming               Error                  Official Quantum modules must have a ``.py`` extension for python modules or a ``.ps1`` for powershell modules
  ...
  invalid-module-schema                                        Documentation        Error                  ``QuantumModule`` schema validation error
  invalid-requires-extension                                   Naming               Error                  Module ``#QuantumRequires -CSharpUtil`` should not end in .cs, Module ``#Requires`` should not end in .psm1
  ...
  missing-module-utils-basic-import                            Imports              Warning                Did not find ``quantum.module_utils.basic`` import
  missing-module-utils-import-csharp-requirements              Imports              Error                  No ``Quantum.ModuleUtils`` or C# Quantum util requirements/imports found
  ...
  multiple-utils-per-requires                                  Imports              Error                  ``Quantum.ModuleUtils`` requirements do not support multiple modules per statement
  multiple-csharp-utils-per-requires                           Imports              Error                  Quantum C# util requirements do not support multiple utils per statement
  ...
  subdirectory-missing-init                                    Naming               Error                  Quantum module subdirectories must contain an ``__init__.py``
  ...
  use-module-utils-urls                                        Imports              Error                  ``requests`` import found, should use ``quantum.module_utils.urls`` instead
