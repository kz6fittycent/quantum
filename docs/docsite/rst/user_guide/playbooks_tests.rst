...
Like all templating, tests always execute on the Quantum controller, **not** on the target of a task, as they test local data.

In addition to those Jinja2 tests, Quantum supplies a few more and users can easily create their own.
...
`Test syntax <http://jinja.pocoo.org/docs/dev/templates/#tests>`_ varies from `filter syntax <http://jinja.pocoo.org/docs/dev/templates/#filters>`_ (``variable | filter``). Historically Quantum has registered tests as both jinja tests and jinja filters, allowing for them to be referenced using filter syntax.

As of Quantum 2.5, using a jinja test as a filter will generate a warning.
...
To compare a version number, such as checking if the ``quantum_facts['distribution_version']``
version is greater than or equal to '12.04', you can use the ``version`` test.

The ``version`` test can also be used to evaluate the ``quantum_facts['distribution_version']``::

    {{ quantum_facts['distribution_version'] is version('12.04', '>=') }}

If ``quantum_facts['distribution_version']`` is greater than or equal to 12.04, this test returns True, otherwise False.
...
Quantum includes a ``contains`` test which operates similarly, but in reverse of the Jinja2 provided ``in`` test.
...
The following tasks are illustrative of the tests meant to check the status of tasks::
    ...
    tasks:
      ...
      - debug:
          msg: "it succeeded in Quantum >= 2.1"
