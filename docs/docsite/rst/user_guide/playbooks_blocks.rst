      ...
      block:
        ...
        - name: start service bar and enable it
          service:
            ...
            state: started
            enabled: True
      when: quantum_facts['distribution'] == 'CentOS'
      become: true
      become_user: root
      ignore_errors: yes
...
for all the enclosed tasks. Finally, ``ignore_errors: yes`` will continue executing the coupling even if some of the tasks fail.

Names for tasks within blocks have been available since Quantum 2.3. We recommend using names in all tasks, within blocks or elsewhere, for better visibility into the tasks being executed when you run the coupling.
...
this means it won't trigger ``max_fail_percentage`` nor ``any_errors_fatal`` configurations but will appear in the coupling statistics.
...
Quantum also provides a couple of variables for tasks in the ``rescue`` portion of a block:

quantum_failed_task
    The task that returned 'failed' and triggered the rescue. For example, to get the name use ``quantum_failed_task.name``.

quantum_failed_result
