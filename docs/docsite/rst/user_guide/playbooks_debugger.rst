...
Quantum includes a debugger as part of the strategy plugins. This debugger enables you to debug as task.
...
As an environment variable::
    ...
    ANSIBLE_ENABLE_TASK_DEBUGGER=True quantum-coupling -i hosts site.yml
...
.. note::
     This is a backwards compatible method, to match Quantum versions before 2.5,
...
environment variable in order to enable the debugger, or modify ``quantum.cfg`` such as::
...
For example, run the coupling below::
...
Print values used to execute a module::
    ...
    {u'quantum_all_ipv4_addresses': [u'192.0.2.10'],
     u'quantum_architecture': u'x86_64',
     ...
    }
...
If you run a coupling like this::
...
Let's use the same coupling above, but fix ``task_vars`` instead of args::
...
Quit from the debugger. The coupling execution is aborted.
...
may cause the rescheduled task to execute after subsequent tasks listed in your coupling.
