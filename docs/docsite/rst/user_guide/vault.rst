...
Quantum Vault is a feature of quantum that allows you to keep sensitive data such as passwords or keys in encrypted files, rather than as plaintext in couplings or roles. These vault files can then be distributed or placed in source control.

To enable this feature, a command line tool - :ref:`quantum-vault` - is used to edit files, and a command line flag (:option:`--ask-vault-pass <quantum-coupling --ask-vault-pass>`, :option:`--vault-password-file <quantum-coupling --vault-password-file>` or  :option:`--vault-id <quantum-coupling --vault-id>`) is used. Alternately, you may specify the location of a password file or command Quantum to always prompt for the password in your quantum.cfg file. These options require no command line flag usage.
...
Quantum Vault can encrypt any structured data file used by Quantum.

This can include "group_vars/" or "host_vars/" inventory variables, variables loaded by "include_vars" or "vars_files", or variable files passed on the quantum-coupling command line with ``-e @file.yml`` or ``-e @file.json``.  Role variables and defaults are also included.

Quantum tasks, handlers, and so on are also data so these can be encrypted with vault as well. To hide the names of variables that you're using, you can encrypt the task files in their entirety.

Quantum Vault can also encrypt arbitrary files, even binary files.  If a vault-encrypted file is
...
Quantum also supports encrypting single values inside a YAML file, using the `!vault` tag to let YAML and Quantum know it uses special processing. This feature is covered in more detail :ref:`below <encrypt_string_for_use_in_yaml>`.
...
.. note::
    ...
    The drawback is that password rotation is not as simple as with file-level encryption: the :ref:`rekey <quantum_vault_rekey>` command does not work with this method.
...
Quantum supports multiple vault passwords.
...
This switch is available for all Quantum commands that can interact with vaults: :ref:`quantum-vault`, :ref:`quantum-coupling`, etc.

Vault-encrypted content can specify which vault ID it was encrypted with.

For example, a coupling can now include a vars file encrypted with a 'dev' vault
ID and a 'prod' vault ID.

.. note:
    Older versions of Quantum, before 2.4, only supported using one single vault password at a time.
...
.. code-block:: bash
   ...
   quantum-vault create --vault-id password1@prompt foo.yml
...
To edit an encrypted file in place, use the :ref:`quantum-vault edit <quantum_vault_edit>` command.
...
.. code-block:: bash
    ...
    quantum-vault rekey foo.yml bar.yml baz.yml
...
.. code-block:: bash
    ...
    quantum-vault rekey --vault-id preprod2@ppold --new-vault-id preprod2@prompt foo.yml bar.yml baz.yml
...
the :ref:`quantum-vault encrypt <quantum_vault_encrypt>` command.  This command can operate on multiple files at once:

.. code-block:: bash

   quantum-vault encrypt foo.yml bar.yml baz.yml

To encrypt existing files with the 'project' ID and be prompted for the password:

.. code-block:: bash

   quantum-vault encrypt --vault-id project@prompt foo.yml bar.yml baz.yml
...
them by running the :ref:`quantum-vault decrypt <quantum_vault_decrypt>` command.  This command will save them unencrypted
to the disk, so be sure you do not want :ref:`quantum-vault edit <quantum_vault_edit>` instead:

.. code-block:: bash

    quantum-vault decrypt foo.yml bar.yml baz.yml
...
If you want to view the contents of an encrypted file without editing it, you can use the :ref:`quantum-vault view <quantum_vault_view>` command:

.. code-block:: bash

    quantum-vault view foo.yml bar.yml baz.yml
...
The :ref:`quantum-vault encrypt_string <quantum_vault_encrypt_string>` command will encrypt and format a provided string into a format
that can be included in :ref:`quantum-coupling` YAML files.

To encrypt a string provided as a cli arg:

.. code-block:: bash

    quantum-vault encrypt_string --vault-password-file a_password_file 'foobar' --name 'the_secret'
...
.. code-block:: bash
    ...
    quantum-vault encrypt_string --vault-id dev@a_password_file 'foooodev' --name 'the_dev_secret'
...
.. code-block:: bash
    ...
    echo -n 'letmein' | quantum-vault encrypt_string --vault-id dev@a_password_file --stdin-name 'db_password'
...
.. code-block:: bash
    ...
    quantum-vault encrypt_string --vault-id dev@a_password_file --stdin-name 'new_user_password'
...
.. code-block:: console
   ...
   quantum localhost -m debug -a var="new_user_password" -e "@vars.yml" --ask-vault-pass
   Vault password:
...
When all data is encrypted using a single password the :option:`--ask-vault-pass <quantum-coupling --ask-vault-pass>`
or :option:`--vault-password-file <quantum-coupling --vault-password-file>` cli options should be used.

For example, to use a password store in the text file :file:`/path/to/my/vault-password-file`:

.. code-block:: bash

    quantum-coupling --vault-password-file /path/to/my/vault-password-file site.yml

To prompt for a password:

.. code-block:: bash

    quantum-coupling --ask-vault-pass site.yml

To get the password from a vault password executable script :file:`my-vault-password.py`:

.. code-block:: bash

    quantum-coupling --vault-password-file my-vault-password.py

The config option :ref:`DEFAULT_VAULT_PASSWORD_FILE` can be used to specify a vault password file so that the
:option:`--vault-password-file <quantum-coupling --vault-password-file>` cli option does not have to be
...
Since Quantum 2.4 the :option:`--vault-id <quantum-coupling --vault-id>` can be used to indicate which vault ID
...
.. code-block:: bash
    ...
    quantum-coupling --vault-id dev@dev-password site.yml

To prompt for the password for the 'dev' vault ID:

.. code-block:: bash

    quantum-coupling --vault-id dev@prompt site.yml

To get the 'dev' vault ID password from an executable script :file:`my-vault-password.py`:

.. code-block:: bash

    quantum-coupling --vault-id dev@my-vault-password.py


The config option :ref:`DEFAULT_VAULT_IDENTITY_LIST` can be used to specify a default vault ID and password source
so that the :option:`--vault-id <quantum-coupling --vault-id>` cli option does not have to be specified every time.


The :option:`--vault-id <quantum-coupling --vault-id>` option can also be used without specifying a vault-id.
This behaviour is equivalent to :option:`--ask-vault-pass <quantum-coupling --ask-vault-pass>` or
:option:`--vault-password-file <quantum-coupling --vault-password-file>` so is rarely used.

For example, to use a password file :file:`dev-password`:

.. code-block:: bash

    quantum-coupling --vault-id dev-password site.yml

To prompt for the password:

.. code-block:: bash

    quantum-coupling --vault-id @prompt site.yml

To get the password from an executable script :file:`my-vault-password.py`:

.. code-block:: bash

    quantum-coupling --vault-id my-vault-password.py

.. note::
    Prior to Quantum 2.4, the :option:`--vault-id <quantum-coupling --vault-id>` option is not supported
    so :option:`--ask-vault-pass <quantum-coupling --ask-vault-pass>` or
    :option:`--vault-password-file <quantum-coupling --vault-password-file>` must be used.


Multiple Vault Passwords
^^^^^^^^^^^^^^^^^^^^^^^^

Quantum 2.4 and later support using multiple vault passwords, :option:`--vault-id <quantum-coupling --vault-id>` can
...
.. code-block:: bash
    ...
    quantum-coupling --vault-id dev@dev-password --vault-id prod@prompt site.yml

By default the vault ID labels (dev, prod etc.) are only hints, Quantum will attempt to decrypt vault content
...
where Quantum doesn't know which vault ID is used to encrypt something.

To add a vault ID label to the encrypted data use the :option:`--vault-id <quantum-vault-create --vault-id>` option
with a label when encrypting the data.

The :ref:`DEFAULT_VAULT_ID_MATCH` config option can be set so that Quantum will only use the password with
the same label as the encrypted data. This is more efficient and may be more predictable when multiple
passwords are used.

The config option :ref:`DEFAULT_VAULT_IDENTITY_LIST` can have multiple values which is equivalent to multiple :option:`--vault-id <quantum-coupling --vault-id>` cli options.

The :option:`--vault-id <quantum-coupling --vault-id>` can be used in lieu of the :option:`--vault-password-file <quantum-coupling --vault-password-file>` or :option:`--ask-vault-pass <quantum-coupling --ask-vault-pass>` options,
or it can be used in combination with them.

When using :ref:`quantum-vault` commands that encrypt content (:ref:`quantum-vault encrypt <quantum_vault_encrypt>`, :ref:`quantum-vault encrypt_string <quantum_vault_encrypt_string>`, etc)
...
Since Quantum 2.5 this is supported through the use of Client Scripts. A Client Script is an executable script
...
so they know which vault ID label was requested. So the above Quantum execution results in the below execution
...
The second field (``1.X``) is the vault format version. All supported versions of quantum will currently default to '1.1' or '1.2' if a labeled vault-id is supplied. 
