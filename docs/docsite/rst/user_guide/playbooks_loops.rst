...
Sometimes you want to repeat a task multiple times. In computer programming, this is called a loop. Common Quantum loops include changing ownership on several files and/or directories with the :ref:`file module <file_module>`, creating multiple users with the :ref:`user module <user_module>`, and
repeating a polling step until a certain result is reached. Quantum offers two keywords for creating loops: ``loop`` and ``with_<lookup>``.

.. note::
   * We added ``loop`` in Quantum 2.5. It is not yet a full replacement for ``with_<lookup>``, but we recommend it for most use cases.
   * We have not deprecated the use of ``with_<lookup>`` - that syntax will still be valid for the foreseeable future.
   * We are looking to improve ``loop`` syntax - watch this page and the `changelog <https://github.com/quantum/quantum/tree/devel/changelogs>`_ for updates.
...
When combining :ref:`couplings_conditionals` with a loop, the ``when:`` statement is processed separately for each item.
...
To loop over your inventory, or just a subset of it, you can use a regular ``loop`` with the ``quantum_play_batch`` or ``groups`` variables::
...
The ``loop`` keyword requires a list as input, but the ``lookup`` keyword returns a string of comma-separated values by default. Quantum 2.5 introduced a new Jinja2 function named :ref:`query` that always returns a list, offering a simpler interface and more predictable output from lookup plugins when using the ``loop`` keyword.
...
You can nest two looping tasks using ``include_tasks``. However, by default Quantum sets the loop variable ``item`` for each loop. This means the inner, nested loop will overwrite the value of ``item`` from the outer loop.
...
.. note:: If Quantum detects that the current loop is using a variable which has already been defined, it will raise an error to fail the task.

Extended loop variables
-----------------------
.. versionadded:: 2.8

As of Quantum 2.8 you can get extended loop information using the ``extended`` option to loop control. This option will expose the following information.

==========================  ===========
Variable                    Description
--------------------------  -----------
``quantum_loop.allitems``   The list of all items in the loop
``quantum_loop.index``      The current iteration of the loop. (1 indexed)
``quantum_loop.index0``     The current iteration of the loop. (0 indexed)
``quantum_loop.revindex``   The number of iterations from the end of the loop (1 indexed)
``quantum_loop.revindex0``  The number of iterations from the end of the loop (0 indexed)
``quantum_loop.first``      ``True`` if first iteration
``quantum_loop.last``       ``True`` if last iteration
``quantum_loop.length``     The number of items in the loop
``quantum_loop.previtem``   The item from the previous iteration of the loop. Undefined during the first iteration.
``quantum_loop.nextitem``   The item from the following iteration of the loop. Undefined during the last iteration.
==========================  ===========
...
As of Quantum 2.8 you can get the name of the value provided to ``loop_control.loop_var`` using the ``quantum_loop_var`` variable
