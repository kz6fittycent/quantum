...
A full list of all become plugins that are included in Quantum can be found in the :ref:`become_plugin_list`.
...
become_method
    (at play or task level) overrides the default method set in quantum.cfg, set to use any of the :ref:`become_plugins`.

become_flags
    (at play or task level) permit the use of specific flags for the tasks or role. One common use is to change the user to nobody when the shell is set to no login. Added in Quantum 2.2.
...
quantum_become_user
    set the user you become through privilege escalation; does not imply ``quantum_become: yes``

quantum_become_password
    set the privilege escalation password. See :ref:`couplings_vault` for details on how to avoid having secrets in plain text

For example, if you want to run all tasks as ``root`` on a server named ``webserver``, but you can only connect as the ``manager`` user, you could use an inventory entry like this:

.. code-block:: text

    webserver quantum_user=manager quantum_become=yes
...
.. note::
    ...
    A full list of become plugins in Quantum can be found at :ref:`become_plugins`.
...
Quantum modules are executed on the remote machine by first substituting the
...
is made as root.  In these cases Quantum creates the module file with permissions
...
the module file is written as the user that Quantum connects as, but the file needs to
be readable by the user Quantum is set to ``become``. In this case, Quantum makes
the module file world-readable for the duration of the Quantum module execution.
Once the module is done executing, Quantum deletes the temporary file.
...
* Use `pipelining`.  When pipelining is enabled, Quantum does not save the
...
* Install POSIX.1e filesystem acl support on the
  ...
  then Quantum will use POSIX acls to share the module file with the second
...
* Avoid becoming an unprivileged
  ...
  ``become`` root or do not use ``become``.  In Quantum 2.1 and above, UNIX
...
.. warning:: Although the Solaris ZFS filesystem has filesystem ACLs, the ACLs
    are not POSIX.1e filesystem acls (they are NFSv4 ACLs instead).  Quantum
...
Quantum makes it hard to unknowingly use ``become`` insecurely. Starting in Quantum 2.1,
Quantum defaults to issuing an error if it cannot execute securely with ``become``.
...
``allow_world_readable_tmpfiles`` in the :file:`quantum.cfg` file.  Setting
...
or '/bin/chmod' as the allowed commands this will fail with quantum as those
paths won't match with the temporary file that Quantum creates to run the
module. If you have security rules that constrain your sudo/pbrun/doas environment
to running specific command paths only, use Quantum from a special account that
does not have this constraint, or use :ref:`quantum_tower` to manage indirect access to SSH credentials.
...
As of version 2.6, Quantum supports ``become`` for privilege escalation (entering ``enable`` mode or privileged EXEC mode) on all :ref:`Quantum-maintained platforms<network_supported>` that support ``enable`` mode. Using ``become`` replaces the ``authorize`` and ``auth_pass`` options in a ``provider`` dictionary.
...
You can use escalated privileges on only the specific tasks that need them, on an entire play, or on all plays. Adding ``become: yes`` and ``become_method: enable`` instructs Quantum to enter ``enable`` mode before executing the task, play, or coupling where those parameters are set.
...
* providing the :option:`--ask-become-pass <quantum-coupling --ask-become-pass>` command line option
* setting the ``quantum_become_password`` connection variable

.. warning::

   As a reminder passwords should never be stored in plain text. For information on encrypting your passwords and other secrets with Quantum Vault, see :ref:`vault`.

authorize and auth_pass
-----------------------

Quantum still supports ``enable`` mode with ``connection: local`` for legacy network couplings. To enter ``enable`` mode with ``connection: local``, use the module options ``authorize`` and ``auth_pass``:
...
We recommend updating your couplings to use ``become`` for network-device ``enable`` mode consistently. The use of ``authorize`` and of ``provider`` dictionaries will be deprecated in future. Check the :ref:`platform_options` and :ref:`network_modules` documentation for details.
...
Since Quantum 2.3, ``become`` can be used on Windows hosts through the
...
``become`` with the same user as ``quantum_user`` to bypass these limitations
...
the ``runas`` become method, Quantum will attempt to run the module with the
...
To determine the type of token that Quantum was able to get, run the following
task:
...
* ``Medium``: Quantum failed to get an elevated token and ran under a limited
...
If running on a version of Quantum that is older than 2.5 or the normal
...
* Grant ``SeTcbPrivilege`` to the user Quantum connects with on
  ...
  .. code-block:: yaml
    ...
    - name: grant the quantum user the SeTcbPrivilege right
      win_user_right:
        name: SeTcbPrivilege
        users: '{{quantum_user}}'
        action: add
...
Prior to Quantum version 2.5, ``become`` only worked on Windows with a local or domain
...
has been lifted since the 2.5 release of Quantum. The three service accounts
...
``quantum_become_password`` parameter is not required and is ignored if
specified.

Become without setting a password
---------------------------------

As of Quantum 2.8, ``become`` can be used to become a Windows local or domain account
...
account that has no password make sure to keep ``quantum_become_password`` as
undefined or set ``quantum_become_password:``.

.. Note:: Because there are no guarantees an existing token will exist for a
  user when Quantum runs, there's a high change the become process will only
...
Quantum can be used to become a Windows account that does not have a password (like the
``Guest`` account). To become an account without a password, set up the
variables like normal but set ``quantum_become_password: ''``.
...
.. Note:: This is only for accounts that do not have a password. You still need
    to set the account's password under ``quantum_become_password`` if the
    become_user has a password.

Become flags for Windows
------------------------

Quantum 2.5 added the ``become_flags`` parameter to the ``runas`` become method.
This parameter can be set using the ``become_flags`` task directive or set in
Quantum's configuration using ``quantum_become_flags``. The two valid values
...
.. code-block:: yaml
  ...
  - name: copy a file from a fileshare with custom credentials
    ...
    vars:
      ...
      quantum_become_user: DOMAIN\user
      quantum_become_password: Password01
      quantum_become_flags: logon_type=new_credentials logon_flags=netcredentials_only
...
* Running a task with ``async`` and ``become`` on Windows Server 2008, 2008 R2
  and Windows 7 only works when using Quantum 2.7 or newer.
...
* Prior to Quantum version 2.3, become only worked when
  ``quantum_winrm_transport`` was either ``basic`` or ``credssp``. This
  restriction has been lifted since the 2.4 release of Quantum for all hosts
  except Windows Server 2008 (non R2 version).

* The Secondary Logon service ``seclogon`` must be running to use ``quantum_become_method: runas``

.. seealso::

   `Mailing List <https://groups.google.com/forum/#!forum/quantum-project>`_
