...
Additional groups can be created to manage hosts based on whether the hosts match other criteria. This topic covers how conditionals are used in couplings.

.. note:: There are many options to control execution flow in Quantum. More examples of supported conditionals can be located here: http://jinja.pocoo.org/docs/dev/templates/#comparisons.
...
This is easy to do in Quantum with the `when` clause, which contains a raw Jinja2 expression without double curly braces (see :ref:`group_by_module`).
It's actually pretty simple::

    tasks:
      - name: "shut down Debian flavored systems"
        command: /sbin/shutdown -t now
        when: quantum_facts['os_family'] == "Debian"
...
You can also use parentheses to group conditions::
    ...
    tasks:
      - name: "shut down CentOS 6 and Debian 7 systems"
        command: /sbin/shutdown -t now
        when: (quantum_facts['distribution'] == "CentOS" and quantum_facts['distribution_major_version'] == "6") or
              (quantum_facts['distribution'] == "Debian" and quantum_facts['distribution_major_version'] == "7")
    ...
    tasks:
      - name: "shut down CentOS 6 systems"
        ...
        when:
          ...
          - quantum_facts['distribution_major_version'] == "6"

A number of Jinja2 "tests" and "filters" can also be used in when statements, some of which are unique
and provided by Quantum.  Suppose we want to ignore the error of one statement and then
decide to do something conditionally based on success or failure::
    ...
    tasks:
      ...
      # In older versions of quantum use ``success``, now both are valid but succeeded uses the correct tense.
...
To see what facts are available on a particular system, you can do the following in a coupling::
...
Tip: Sometimes you'll get back a variable that's a string and you'll want to do a math operation comparison on it.  You can do this like so::
    ...
    tasks:
      - shell: echo "only on Red Hat 6, derivatives, and later"
        when: quantum_facts['os_family'] == "RedHat" and quantum_facts['lsb']['major_release']|int >= 6

.. note:: the above example requires the lsb_release package on the target host in order to return the 'lsb major_release' fact.

Variables defined in the couplings or inventory can also be used, just make sure to apply the `|bool` filter to non boolean variables (ex: string variables with content like 'yes', 'on', '1', 'true').  An example may be the execution of a task based on a variable's boolean value::
...
``````````````````````
Combining `when` with loops (see :ref:`couplings_loops`), be aware that the `when` statement is processed separately for each item. This is by design::
...
.. note:: In versions prior to 2.0 this worked with task includes but not coupling includes.  2.0 allows it to work with both.
...
You will note a lot of 'skipped' output by default in Quantum when using this approach on systems that don't match the criteria.
...
For more information on the differences between ``include`` v ``import`` see :ref:`couplings_reuse`.
...
Sometimes you will want to do certain things differently in a coupling based on certain criteria.
Having one coupling that works on multiple platforms and OS versions is a good example.

As an example, the name of the Apache package may be different between CentOS and Debian,
but it is easily handled with a minimum of syntax in an Quantum Playbook::
    ...
    - hosts: all
      ...
      vars_files:
        - "vars/common.yml"
        - [ "vars/{{ quantum_facts['os_family'] }}.yml", "vars/os_defaults.yml" ]
      tasks:
      - name: make sure apache is started
        service: name={{ apache }} state=started

.. note::
   The variable "quantum_facts['os_family']" is being interpolated into
...
How does this work?  For Red Hat operating systems ('CentOS', for example), the first file Quantum tries to import
is 'vars/RedHat.yml'. If that file does not exist, Quantum attempts to load 'vars/os_defaults.yml'. If no files in
the list were found, an error is raised.

On Debian, Quantum first looks for 'vars/Debian.yml' instead of 'vars/RedHat.yml', before
falling back on 'vars/os_defaults.yml'.

Quantum's approach to configuration -- separating variables from tasks, keeping your couplings
...
The following example shows how to template out a configuration file that was very different between, say, CentOS and Debian::
    ...
    - name: template a file
      ...
      vars:
        myfiles:
          - "{{quantum_facts['distribution']}}.conf"
          -  default.conf
...
Often in a coupling it may be useful to store the result of a given command in a variable and access
...
.. _quantum_distribution:

quantum_facts['distribution']
-----------------------------
...
.. _quantum_distribution_major_version:

quantum_facts['distribution_major_version']
-------------------------------------------
...
.. Quantum checks `OS_FAMILY_MAP`; if there's no match, it returns the value of `platform.system()`.
