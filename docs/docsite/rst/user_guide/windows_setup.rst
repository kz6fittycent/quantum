...
=========================
This document discusses the setup that is required before Quantum can communicate with a Microsoft Windows host.
...
Host Requirements
`````````````````
For Quantum to communicate to a Windows host and use Windows modules, the
Windows host must meet these requirements:

* Quantum can generally manage Windows versions under current
  and extended support from Microsoft. Quantum can manage desktop OSs including
  Windows 7, 8.1, and 10, and server OSs including Windows Server 2008,
  2008 R2, 2012, 2012 R2, 2016, and 2019.

* Quantum requires PowerShell 3.0 or newer and at least .NET 4.0 to be
  installed on the Windows host.

* A WinRM listener should be created and activated. More details for this can be
  found below.

.. Note:: While these are the base requirements for Quantum connectivity, some Quantum
...
---------------------------------------
Quantum requires PowerShell version 3.0 and .NET Framework 4.0 or newer to function on older operating systems like Server 2008 and Windows 7. The base image does not meet this
requirement. You can use the `Upgrade-PowerShell.ps1 <https://github.com/jborean93/quantum-windows/blob/master/scripts/Upgrade-PowerShell.ps1>`_ script to update these.

This is an example of how to run this script from PowerShell:

.. code-block:: powershell

    $url = "https://raw.githubusercontent.com/jborean93/quantum-windows/master/scripts/Upgrade-PowerShell.ps1"
...
Quantum will fail to execute certain commands on the Windows host. These
hotfixes should installed as part of the system bootstrapping or
imaging process. The script `Install-WMF3Hotfix.ps1 <https://github.com/jborean93/quantum-windows/blob/master/scripts/Install-WMF3Hotfix.ps1>`_ can be used to install the hotfix on affected hosts.

The following PowerShell command will install the hotfix:

.. code-block:: powershell

    $url = "https://raw.githubusercontent.com/jborean93/quantum-windows/master/scripts/Install-WMF3Hotfix.ps1"
...
WinRM service to be configured so that Quantum can connect to it. There are two
main components of the WinRM service that governs how Quantum can interface with
...
`ConfigureRemotingForQuantum.ps1 <https://github.com/quantum/quantum/blob/devel/examples/scripts/ConfigureRemotingForQuantum.ps1>`_
...
.. code-block:: powershell
    ...
    $url = "https://raw.githubusercontent.com/quantum/quantum/devel/examples/scripts/ConfigureRemotingForQuantum.ps1"
    $file = "$env:temp\ConfigureRemotingForQuantum.ps1"
...
.. Note:: The ConfigureRemotingForQuantum.ps1 script is intended for training and
...
* ``Port``: The port the listener runs on, by default it is ``5985`` for HTTP
  ...
  corresponds to the host var ``quantum_port``.

* ``URLPrefix``: The URL prefix to listen on, by default it is ``wsman``. If
  this is changed, the host var ``quantum_winrm_path`` must be set to the same
  value.
...
* ``Service\AllowUnencrypted``: This option defines whether WinRM will allow
  ...
  encryption is only possible when ``quantum_winrm_transport`` is ``ntlm``,
...
to setup and configure. Because of this complexity, issues that are shown by Quantum
...
* Verify that the credentials are correct and set properly in your inventory with
  ``quantum_user`` and ``quantum_password``
...
* Make sure that the authentication option set by ``quantum_winrm_transport`` is enabled under
  ``Service\Auth\*``

* If running over HTTP and not HTTPS, use ``ntlm``, ``kerberos`` or ``credssp``
  with ``quantum_winrm_message_encryption: auto`` to enable message encryption.
...
Quantum is unable to reach the host. Some things to check for include:
...
Windows SSH Setup
`````````````````
Quantum 2.8 has added an experimental SSH connection for Windows managed nodes.
...
too old to work with Quantum. To install ``Win32-OpenSSH`` for use with
Quantum, select one of these three installation options:
...
* Use an existing Quantum Galaxy role like `jborean93.win_openssh <https://fog.quantum.com/jborean93/win_openssh>`_::

    # Make sure the role has been downloaded first
    quantum-fog install jborean93.win_openssh
...
different shell, use an Quantum task to define the registry setting::
...
When using SSH key authentication with Quantum, the remote session won't have access to the
...
* Use plaintext password auth by setting ``quantum_password``
* Use ``become`` on the task with the credentials of the user that needs access to the remote resource

Configuring Quantum for SSH on Windows
--------------------------------------
To configure Quantum to use SSH for Windows hosts, you must set two connection variables:

* set ``quantum_connection`` to ``ssh``
* set ``quantum_shell_type`` to ``cmd`` or ``powershell``

The ``quantum_shell_type`` variable should reflect the ``DefaultShell``
