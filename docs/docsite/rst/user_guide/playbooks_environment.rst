...
.. note::
   ``environment:`` does not affect Quantum itself, ONLY the context of the specific task action and this does not include
    Quantum's own configuration settings nor the execution of any other plugins, including lookups, filters, and so on.
...
Some language-specific version managers (such as rbenv and nvm) require environment variables be set while these tools are in use. When using these tools manually, they usually require sourcing some environment variables via a script or lines added to your shell configuration file. In Quantum, you can instead use the environment directive::
    ...
    - hosts: application
      ...
      environment:
        ...
        PATH: /var/local/nvm/versions/node/v4.2.1/bin:{{ quantum_env.PATH }}
...
.. note::
   ``quantum_env:`` is normally populated by fact gathering (M(gather_facts)) and the value of the variables depends on the user
...
You might also want to simply specify the environment for a single task::
    ...
    - name: install ruby 2.3.1
      ...
      environment:
        ...
        PATH: '{{ rbenv_root }}/bin:{{ rbenv_root }}/shims:{{ rbenv_plugins }}/ruby-build/bin:{{ quantum_env.PATH }}'
