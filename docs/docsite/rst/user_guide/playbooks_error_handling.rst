...
Quantum normally has defaults that make sure to check the return codes of commands and modules and
...
the default behavior of Quantum for certain tasks so output and error handling behavior is
as desired.
...
Generally couplings will stop executing any more steps on a host that has a task fail.
...
in quantum.cfg. When handlers are forced, they will run when notified even
...
Quantum lets you define what "failure" means in each task using the ``failed_when`` conditional. As with all conditionals in Quantum, lists of multiple ``failed_when`` conditions are joined with an implicit ``and``, meaning the task only fails when *all* conditions are met. If you want to trigger a failure when any of the conditions is met, you must define the conditions in a string with an explicit ``or`` operator.
...
In previous version of Quantum, this can still be accomplished as follows::
...
Most of what you can apply to a single task (with the exception of loops) can be applied at the :ref:`couplings_blocks` level, which also makes it much easier to set data or directives common to the tasks.
