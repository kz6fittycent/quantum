...
When quantum-coupling is executed with ``--check`` it will not make any changes on remote systems.  Instead, any module
...
Example::
    ...
    quantum-coupling foo.yml --check
...
1. Force a task to **run in check mode**, even when the coupling is called **without** ``--check``. This is called ``check_mode: yes``.
2. Force a task to **run in normal mode** and make changes to the system, even when the coupling is called **with** ``--check``. This is called ``check_mode: no``.
...
quantum modules, either to test the module itself or to the conditions under
which a module would make changes.
With ``register`` (see :ref:`couplings_conditionals`) you can check the
potential changes.
...
you can use a boolean magic variable ``quantum_check_mode``
...
Example::
  ...
  tasks:
    ...
    - name: this task will be skipped in check mode
      ...
      when: not quantum_check_mode

    - name: this task will ignore errors in check mode
      git:
        repo: ssh://git@github.com/mylogin/hello.git
        dest: /home/mylogin/hello
      ignore_errors: "{{ quantum_check_mode }}"
...
The ``--diff`` option to quantum-coupling works great with ``--check`` (detailed above) but can also be used by itself.
When this flag is supplied and the module supports this, Quantum will report back the changes made or, if used with ``--check``, the changes that would have been made.
...
Since the diff feature produces a large amount of output, it is best used when checking a single host at a time. For example::
    ...
    quantum-coupling foo.yml --check --diff --limit foo.example.com
