...
Quantum works against multiple managed nodes or "hosts" in your infrastructure at the same time, using a list or group of lists know as inventory. Once your inventory is defined, you use :ref:`patterns <intro_patterns>` to select the hosts or groups you want Quantum to run against.

The default location for inventory is a file called ``/etc/quantum/hosts``. You can specify a different inventory file at the command line using the ``-i <path>`` option. You can also use multiple inventory files at the same time, and/or pull inventory from dynamic or cloud sources or different formats (YAML, ini, etc), as described in :ref:`intro_dynamic_inventory`.
Introduced in version 2.4, Quantum has :ref:`inventory_plugins` to make this flexible and customizable.
...
The most common formats are INI and YAML. A basic INI ``etc/quantum/hosts`` might look like this:
...
You can store variable values that relate to a specific host or group in inventory. To start with, you may add variables directly to the hosts and groups in your main inventory file. As you add more and more managed nodes to your Quantum inventory, however, you will likely want to store variables in separate host and group variable files.
...
You can easily assign a variable to a single host, then use it later in couplings. In INI:
...
Unique values like non-standard SSH ports work well as host variables. You can add them to your Quantum inventory by adding the port number after the hostname with a colon:
...
.. code-block:: text
   ...
   other1.example.com     quantum_connection=ssh        quantum_user=myuser
   other2.example.com     quantum_connection=ssh        quantum_user=myotheruser
...
.. code-block:: text
    ...
    jumper quantum_port=5555 quantum_host=192.0.2.50
...
.. code-block:: yaml
    ...
    ...
      hosts:
        jumper:
          quantum_port: 5555
          quantum_host: 192.0.2.50

In the above example, running Quantum against the host alias "jumper" will connect to 192.0.2.50 on port 5555.
...
Group variables are a convenient way to apply variables to multiple hosts at once. Before executing, however, Quantum always flattens variables, including inventory variables, to the host level. If a host is a member of multiple groups, Quantum reads variable values from all of those groups. If you assign different values to the same variable in different groups, Quantum chooses which value to use based on internal :ref:`rules for merging <how_we_merge>`.
...
Quantum loads host and group variable files by searching paths relative to the inventory file or the coupling file. If your inventory file at ``/etc/quantum/hosts`` contains a host named 'foosball' that belongs to two groups, 'raleigh' and 'webservers', that host will use variables in YAML files at the following locations:

.. code-block:: bash

    /etc/quantum/group_vars/raleigh # can optionally end in '.yml', '.yaml', or '.json'
    /etc/quantum/group_vars/webservers
    /etc/quantum/host_vars/foosball

For example, if you group hosts in your inventory by datacenter, and each datacenter uses its own NTP server and database server, you can create a file called ``/etc/quantum/group_vars/raleigh`` to store the variables for the ``raleigh`` group:
...
You can also create *directories* named after your groups or hosts. Quantum will read all the files in these directories in lexicographical order. An example with the 'raleigh' group:

.. code-block:: bash

    /etc/quantum/group_vars/raleigh/db_settings
    /etc/quantum/group_vars/raleigh/cluster_settings
...
file gets too big, or when you want to use :ref:`Quantum Vault<couplings_vault>` on some group variables.

You can also add ``group_vars/`` and ``host_vars/`` directories to your coupling directory. The ``quantum-coupling`` command looks for these directories in the current working directory by default. Other Quantum commands (for example, ``quantum``, ``quantum-console``, etc.) will only look for ``group_vars/`` and ``host_vars/`` in the inventory directory. If you want other commands to load group and host variables from a coupling directory, you must provide the ``--coupling-dir`` option on the command line.
If you load inventory files from both the coupling directory and the inventory directory, variables in the coupling directory will override variables set in the inventory directory.
...
By default variables are merged/flattened to the specific host before a play is run. This keeps Quantum focused on the Host and Task, so groups don't really survive outside of inventory and host matching. By default, Quantum overwrites variables including the ones defined for a group and/or host (see :ref:`DEFAULT_HASH_BEHAVIOUR<DEFAULT_HASH_BEHAVIOUR>`). The order/precedence is (from lowest to highest):
...
By default Quantum merges groups at the same parent/child level alphabetically, and the last group loaded overwrites the previous groups. For example, an a_group will be merged with b_group and b_group vars that match will overwrite the ones in a_group.

You can change this behavior by setting the group variable ``quantum_group_priority`` to change the merge order for groups of the same level (after the parent/child order is resolved). The larger the number, the later it will be merged, giving it higher priority. This variable defaults to ``1`` if not set. For example:

.. code-block:: yaml

    a_group:
        testvar: a
        quantum_group_priority: 10
    b_group:
        testvar: b

In this example, if both groups have the same priority, the result would normally have been ``testvar == b``, but since we are giving the ``a_group`` a higher priority the result will be ``testvar == a``.

.. note:: ``quantum_group_priority`` can only be set in the inventory source and not in group_vars/, as the variable is used in the loading of group_vars.
...
.. code-block:: bash
    ...
    quantum-coupling get_logs.yml -i staging -i production

Keep in mind that if there are variable conflicts in the inventories, they are resolved according
to the rules described in :ref:`how_we_merge` and :ref:`quantum_variable_precedence`.
...
the coupling will be run with ``myvar = 2``. The result would be reversed if the coupling was run with
...
.. code-block:: bash
    ...
    quantum-coupling example.yml -i inventory
...
and ``03-static-inventory`` defines ``myvar = 3``, the coupling will be run with ``myvar = 3``.
...
As described above, setting the following variables control how Quantum interacts with remote hosts.
...
quantum_connection
    Connection type to the host. This can be the name of any of quantum's connection plugins. SSH protocol types are ``smart``, ``ssh`` or ``paramiko``.  The default is smart. Non-SSH based types are described in the next section.
...
quantum_ssh_private_key_file
    Private key file used by ssh.  Useful if using multiple keys and you don't want to use SSH agent.
quantum_ssh_common_args
    ...
    group).
quantum_sftp_extra_args
    This setting is always appended to the default :command:`sftp` command line.
quantum_scp_extra_args
    This setting is always appended to the default :command:`scp` command line.
quantum_ssh_extra_args
    This setting is always appended to the default :command:`ssh` command line.
quantum_ssh_pipelining
    Determines whether or not to use SSH pipelining. This can override the ``pipelining`` setting in :file:`quantum.cfg`.
quantum_ssh_executable (added in version 2.2)
    This setting overrides the default behavior to use the system :command:`ssh`. This can override the ``ssh_executable`` setting in :file:`quantum.cfg`.


Privilege escalation (see :ref:`Quantum Privilege Escalation<become>` for further details):

quantum_become
    Equivalent to ``quantum_sudo`` or ``quantum_su``, allows to force privilege escalation
quantum_become_method
    Allows to set privilege escalation method
quantum_become_user
    Equivalent to ``quantum_sudo_user`` or ``quantum_su_user``, allows to set the user you become through privilege escalation
quantum_become_password
    Equivalent to ``quantum_sudo_password`` or ``quantum_su_password``, allows you to set the privilege escalation password (never store this variable in plain text; always use a vault. See :ref:`best_practices_for_variables_and_vaults`)
quantum_become_exe
    Equivalent to ``quantum_sudo_exe`` or ``quantum_su_exe``, allows you to set the executable for the escalation method selected
quantum_become_flags
    Equivalent to ``quantum_sudo_flags`` or ``quantum_su_flags``, allows you to set the flags passed to the selected escalation method. This can be also set globally in :file:`quantum.cfg` in the ``sudo_flags`` option
...
quantum_shell_type
    ...
    :ref:`quantum_shell_executable<quantum_shell_executable>` to a non-Bourne (sh) compatible shell.  By default commands are
...
quantum_*_interpreter
    Works for anything such as ruby or perl and works just like :ref:`quantum_python_interpreter<quantum_python_interpreter>`.
...
quantum_shell_executable
    This sets the shell the quantum controller will use on the target machine,
    overrides ``executable`` in :file:`quantum.cfg` which defaults to
...
Examples from an Quantum-INI host file:

.. code-block:: text

  some_host         quantum_port=2222     quantum_user=manager
  aws_host          quantum_ssh_private_key_file=/home/example/.ssh/aws.pem
  freebsd_host      quantum_python_interpreter=/usr/local/bin/python
  ruby_module_host  quantum_ruby_interpreter=/usr/bin/ruby.1.9.3

Non-SSH connection types
------------------------

As stated in the previous section, Quantum executes couplings over SSH but it is not limited to this connection type.
With the host specific parameter ``quantum_connection=<connector>``, the connection type can be changed.
The following non-SSH based connectors are available:

**local**

This connector can be used to deploy the coupling to the control machine itself.

**docker**

This connector deploys the coupling directly into Docker containers using the local Docker client. The following parameters are processed by this connector:
...
.. code-block:: yaml
   ...
   - name: add container to inventory
     add_host:
       name: my_jenkins
       quantum_connection: docker
       quantum_docker_extra_args: "--tlsverify --tlscacert=/path/to/ca.pem --tlscert=/path/to/client-cert.pem --tlskey=/path/to/client-key.pem -H=tcp://myserver.net:4243"
       quantum_user: jenkins
     changed_when: false
...
.. note:: If you're reading the docs from the beginning, this may be the first example you've seen of an Quantum coupling. This is not an inventory file.
...
To apply a coupling called :file:`site.yml`
to all the app servers in the test environment, use the
following command::

  quantum-coupling -i inventory_test site.yml -l appservers
...
for instance, to define firewall rules inside a coupling or role
...
.. seealso::
   ...
   :ref:`working_with_couplings`
       Learning Quantum's configuration, deployment, and orchestration language.
