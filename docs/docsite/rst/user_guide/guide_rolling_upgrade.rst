...
Some Quantum users are deploying updates to their end users on an hourly or even more frequent basis -- sometimes every time
...
This document describes in detail how to achieve this goal, using one of Quantum's most complete example
couplings as a template: lamp_haproxy. This example uses a lot of Quantum features: roles, templates,
and group variables, and it also comes with an orchestration coupling that can do zero-downtime
rolling upgrades of the web application stack.

.. note::

   `Click here for the latest couplings for this example
   <https://github.com/quantum/quantum-examples/tree/master/lamp_haproxy>`_.

The couplings deploy Apache, PHP, MySQL, Nagios, and HAProxy to a CentOS-based set of servers.

We're not going to cover how to run these couplings here. Read the included README in the github project along with the
example for that information. Instead, we're going to take a close look at every part of the coupling and describe what it does.
...
Let's start with ``site.yml``. This is our site-wide deployment coupling. It can be used to initially deploy the site, as well
as push updates to all of the servers:

.. code-block:: yaml

    ---
    # This coupling deploys the whole application stack in this site.
...
.. note::
   ...
   If you're not familiar with terms like couplings and plays, you should review :ref:`working_with_couplings`.

In this coupling we have 5 plays. The first one targets ``all`` hosts and applies the ``common`` role to all of the hosts.
...
By now you should have a bit of understanding about roles and how they work in Quantum. Roles are a way to organize
...
You can read more about roles in the :ref:`couplings_reuse_roles` section.
...
couplings to customize behavior and to provide easily-changed settings and parameters. They are stored in
...
These variables are used in a variety of places. You can use them in couplings, like this, in ``roles/db/tasks/main.yml``:
...
.. code-block:: jinja
   ...
   -A INPUT -p tcp -s {{ hostvars[host].quantum_default_ipv4.address }} --dport 5666 -j ACCEPT
   {% endfor %}
...
can read more about Quantum variables in general in the :ref:`couplings_variables` section.
...
Now you have a fully-deployed site with web servers, a load balancer, and monitoring. How do you update it? This is where Quantum's
orchestration features come into play. While some applications use the term 'orchestration' to mean basic ordering or command-blasting, Quantum
refers to orchestration as 'conducting machines like an orchestra', and has a pretty sophisticated engine for it.

Quantum has the capability to do operations on multi-tier applications in a coordinated way, making it easy to orchestrate a sophisticated zero-downtime rolling upgrade of our web application. This is implemented in a separate coupling, called ``rolling_update.yml``.

Looking at the coupling, you can see it is made up of two plays. The first play is very simple and looks like this:
...
The ``delegate_to`` and ``loop`` arguments, used together, cause Quantum to loop over each monitoring server and load balancer, and perform that operation (delegate that operation) on the monitoring or load balancing server, "on behalf" of the webserver. In programming terms, the outer loop is the list of web servers, and the inner loop is the list of monitoring servers.

Note that the HAProxy step looks a little complicated.  We're using HAProxy in this example because it's freely available, though if you have (for instance) an F5 or Netscaler in your infrastructure (or maybe you have an AWS Elastic IP setup?), you can use modules included in core Quantum to communicate with them instead.  You might also wish to use other monitoring modules instead of nagios, but this just shows the main goal of the 'pre tasks' section -- take the server out of monitoring, and take it out of rotation.
...
In this example, we use the simple HAProxy load balancer to front-end the web servers. It's easy to configure and easy to manage. As we have mentioned, Quantum has built-in support for a variety of other load balancers like Citrix NetScaler, F5 BigIP, Amazon Elastic Load Balancers, and more. See the :ref:`working_with_modules` documentation for more information.

For other load balancers, you may need to send shell commands to them (like we do for HAProxy above), or call an API, if your load balancer exposes one. For the load balancers for which Quantum has modules, you may want to run them as a ``local_action`` if they contact an API. You can read more about local actions in the :ref:`couplings_delegation` section.  Should you develop anything interesting for some hardware where there is not a core module, it might make for a good module for core inclusion!
...
For integration with Continuous Integration systems, you can easily trigger coupling runs using the ``quantum-coupling`` command line tool, or, if you're using :ref:`quantum_tower`, the ``tower-cli`` or the built-in REST API.  (The tower-cli command 'joblaunch' will spawn a remote job over the REST API and is pretty slick).

This should give you a good idea of how to structure a multi-tier application with Quantum, and orchestrate operations upon that app, with the eventual goal of continuous delivery to your customers. You could extend the idea of the rolling upgrade to lots of different parts of the app; maybe add front-end web servers along with application servers, for instance, or replace the SQL database with something like MongoDB or Riak. Quantum gives you the capability to easily manage complicated environments and automate common operations.

.. seealso::

   `lamp_haproxy example <https://github.com/quantum/quantum-examples/tree/master/lamp_haproxy>`_
       The lamp_haproxy example discussed here.
   :ref:`working_with_couplings`
       An introduction to couplings
   :ref:`couplings_reuse_roles`
       An introduction to coupling roles
   :ref:`couplings_variables`
       An introduction to Quantum variables
   `Quantum.com: Continuous Delivery <https://www.quantum.com/use-cases/continuous-delivery>`_
       An introduction to Continuous Delivery with Quantum
