...
configured with WinRM. This topic covers how to configure and use WinRM with Quantum.
...
configuration is required to use WinRM with Quantum.

Quantum uses the `pywinrm <https://github.com/diyan/pywinrm>`_ package to
communicate with Windows servers over WinRM. It is not installed by default
with the Quantum package, but can be installed by running the following:
...
.. Note:: on distributions with multiple python versions, use pip2 or pip2.x,
    where x matches the python minor version Quantum is running under.
...
hosts or groups with the ``quantum_winrm_transport`` variable.
...
.. code-block:: yaml+jinja
    ...
    quantum_user: LocalUsername
    quantum_password: Password
    quantum_connection: winrm
    quantum_winrm_transport: basic
...
.. code-block:: yaml+jinja
    ...
    quantum_connection: winrm
    quantum_winrm_cert_pem: /path/to/certificate/public/key.pem
    quantum_winrm_cert_key_pem: /path/to/certificate/private/key.pem
    quantum_winrm_transport: certificate
...
.. Note:: Encrypted private keys cannot be used as the urllib3 library that
    is used by Quantum for WinRM does not support this functionality.
...
.. Note:: Using the PowerShell cmdlet ``New-SelfSignedCertificate`` to generate
    ...
    extract the private key from the PFX certificate to a PEM file for Quantum
    to use.
...
Once this is complete, the hostvar ``quantum_winrm_cert_pem`` should be set to
the path of the public key and the ``quantum_winrm_cert_key_pem`` variable should be set to
...
Kerberos requires some additional setup work on the Quantum host before it can be
used properly.
...
.. code-block:: yaml+jinja
    ...
    quantum_user: username@MY.DOMAIN.COM
    quantum_password: Password
    quantum_connection: winrm
    quantum_winrm_transport: kerberos

As of Quantum version 2.3, the Kerberos ticket will be created based on
``quantum_user`` and ``quantum_password``. If running on an older version of
Quantum or when ``quantum_winrm_kinit_mode`` is ``manual``, a Kerberos
ticket must already be obtained. See below for more details.

There are some extra host variables that can be set::

    quantum_winrm_kinit_mode: managed/manual (manual means Quantum will not obtain a ticket)
    quantum_winrm_kinit_cmd: the kinit binary to use to obtain a Kerberos ticket (default to kinit)
    quantum_winrm_service: overrides the SPN prefix that is used, the default is ``HTTP`` and should rarely ever need changing
    quantum_winrm_kerberos_delegation: allows the credentials to traverse multiple hops
    quantum_winrm_kerberos_hostname_override: the hostname to be used for the kerberos exchange
...
Add a line like the following for each domain that Quantum needs access for:
...
++++++++++++++++++++++++++++++++++++
Quantum version 2.3 and later defaults to automatically managing Kerberos tickets
when both ``quantum_user`` and ``quantum_password`` are specified for a host. In
...
To disable automatic ticket management, set ``quantum_winrm_kinit_mode=manual``
via the inventory.
...
``quantum_winrm_kinit_cmd`` hostvar to the fully qualified path to a MIT krbv5
...
* The Quantum host's clock is synchronized with the domain controller. Kerberos
...
.. code-block:: yaml+jinja
    ...
    quantum_user: Username
    quantum_password: Password
    quantum_connection: winrm
    quantum_winrm_transport: credssp

There are some extra host variables that can be set as shown below::

    quantum_winrm_credssp_disable_tlsv1_2: when true, will not use TLS 1.2 in the CredSSP auth process
...
* Set ``quantum_winrm_credssp_disable_tlsv1_2=True`` in the inventory to run
...
The ``ConfigureRemotingForQuantum.ps1`` creates a self-signed certificate and
...
message encryption is required, set ``quantum_winrm_message_encryption=always``
in the host vars.
...
from Quantum can be viewed, manipulated and also the remote session can completely
...
Inventory Options
`````````````````
Quantum's Windows support relies on a few standard variables to indicate the
...
.. code-block:: yaml+jinja
    ...
    # It is suggested that these be encrypted with quantum-vault:
    # quantum-vault edit group_vars/windows.yml
    quantum_connection: winrm

    # May also be passed on the command-line via --user
    quantum_user: Administrator

    # May also be supplied at runtime with --ask-pass
    quantum_password: SecretPasswordGoesHere


Using the variables above, Quantum will connect to the Windows host with Basic
authentication through HTTPS. If ``quantum_user`` has a UPN value like
``username@MY.DOMAIN.COM`` then the authentication option will automatically attempt
to use Kerberos unless ``quantum_winrm_transport`` has been set to something other than
``kerberos``.

The following custom inventory variables are also supported
for additional configuration of WinRM connections:

* ``quantum_port``: The port WinRM will run over, HTTPS is ``5986`` which is
  the default while HTTP is ``5985``

* ``quantum_winrm_scheme``: Specify the connection scheme (``http`` or
  ``https``) to use for the WinRM connection. Quantum uses ``https`` by default
  unless ``quantum_port`` is ``5985``

* ``quantum_winrm_path``: Specify an alternate path to the WinRM endpoint,
  Quantum uses ``/wsman`` by default

* ``quantum_winrm_realm``: Specify the realm to use for Kerberos
  authentication. If ``quantum_user`` contains ``@``, Quantum will use the part
  of the username after ``@`` by default

* ``quantum_winrm_transport``: Specify one or more authentication transport
  options as a comma-separated list. By default, Quantum will use ``kerberos,
  basic`` if the ``kerberos`` module is installed and a realm is defined,
  otherwise it will be ``plaintext``

* ``quantum_winrm_server_cert_validation``: Specify the server certificate
  validation mode (``ignore`` or ``validate``). Quantum defaults to
...
* ``quantum_winrm_operation_timeout_sec``: Increase the default timeout for
  WinRM operations, Quantum uses ``20`` by default

* ``quantum_winrm_read_timeout_sec``: Increase the WinRM read timeout, Quantum
  uses ``30`` by default. Useful if there are intermittent network issues and
  read timeout errors keep occurring

* ``quantum_winrm_message_encryption``: Specify the message encryption
  operation (``auto``, ``always``, ``never``) to use, Quantum uses ``auto`` by
  default. ``auto`` means message encryption is only used when
  ``quantum_winrm_scheme`` is ``http`` and ``quantum_winrm_transport`` supports
...
* ``quantum_winrm_ca_trust_path``: Used to specify a different cacert container
  than the one used in the ``certifi`` module. See the HTTPS Certificate
  Validation section for more details.

* ``quantum_winrm_send_cbt``: When using ``ntlm`` or ``kerberos`` over HTTPS,
...
* ``quantum_winrm_*``: Any additional keyword arguments supported by
...
.. Note:: Quantum 2.0 has deprecated the "ssh" from ``quantum_ssh_user``,
    ``quantum_ssh_pass``, ``quantum_ssh_host``, and ``quantum_ssh_port`` to
    become ``quantum_user``, ``quantum_password``, ``quantum_host``, and
    ``quantum_port``. If using a version of Quantum prior to 2.0, the older
    style (``quantum_ssh_*``) should be used instead. The shorter variables
    are ignored, without warning, in older versions of Quantum.

.. Note:: ``quantum_winrm_message_encryption`` is different from transport
    encryption done over TLS. The WinRM payload is still encrypted with TLS
    when run over HTTPS, even if ``quantum_winrm_message_encryption=never``.

IPv6 Addresses
``````````````
IPv6 addresses can be used instead of IPv4 addresses or hostnames. This option
is normally set in an inventory. Quantum will attempt to parse the address
...
``quantum_winrm_server_cert_validation: ignore`` these security mechanisms are
...
another authority, like AD CS, then Quantum can be set up to trust that
issuer as part of the TLS handshake.

To get Quantum to trust a Certificate Authority (CA) like AD CS, the issuer
certificate of the CA can be exported as a PEM encoded certificate. This
certificate can then be copied locally to the Quantum controller and used as a
...
the validation process, set ``quantum_winrm_ca_trust_path`` to the path of the
...
client and the server. If a match cannot be found then Quantum will error out
...
TLS v1.2 but it could also mean the Quantum controller has an older OpenSSL
version installed.
...
.. Note:: There is a bug with the TLS 1.2 patch for Server 2008 which will stop
    Quantum from connecting to the Windows host. This means that Server 2008
...
command on the Quantum controller::
...
The below Quantum tasks can also be used to enable TLS v1.2:
...
when using WinRM that can cause issues when creating couplings for Quantum.
These include:
...
* Set ``quantum_winrm_transport`` to ``credssp`` or ``kerberos`` (with
  ``quantum_winrm_kerberos_delegation=true``) to bypass the double hop issue
