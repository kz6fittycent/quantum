Using Quantum and Windows
=========================
When using Quantum to manage Windows, many of the syntax and rules that apply
...
This document covers details specific to using Quantum for Windows.
...
Use Cases
`````````
Quantum can be used to orchestrate a multitude of tasks on Windows servers.
Below are some examples and info about common tasks.

Installing Software
-------------------
There are three main ways that Quantum can be used to install software:
...
issues is to use ``become`` with the task. With ``become``, Quantum will run
...
.. Note:: Some installers restart the WinRM or HTTP services, or cause them to become temporarily unavailable, making Quantum assume the system is unreachable.
...
-----------------------
Quantum can be used to create Windows users and groups both locally and on a domain.
...
has none of the wrappers that Quantum typically uses, ``become``, ``async``
and environment variables do not work.

The ``script`` module executes a script from the Quantum controller on
...
.. code-block:: yaml+jinja
    ...
    # Due to YAML and Quantum parsing '\"' must be written as '{% raw %}\\{% endraw %}"'
...
Quantum version 2.5 added modules that make it easier to work with scheduled tasks in Windows.
...
.. Note:: The modules used in the above example were updated/added in Quantum
    version 2.5.
...
can cause major issues with how couplings are written, since ``\`` is often used
as an escape character on POSIX systems.

Quantum allows two different styles of syntax; each deals with path separators for Windows differently:
...
or inside couplings. The use of this style is discouraged within couplings
because backslash characters need to be escaped, making couplings harder to read.
The legacy syntax depends on the specific implementation in Quantum, and quoting
(both single and double) does not have any effect on how it is parsed by
Quantum.

The Quantum key=value parser parse_kv() considers the following escape
sequences:
