...
Httpapi plugins tell Quantum how to interact with a remote device's HTTP-based API and execute tasks on the
device.
...
You can extend Quantum to support other APIs by dropping a custom plugin into the ``httpapi_plugins`` directory. See :ref:`developing_plugins_httpapi` for details.
...
The httpapi plugin to use is determined automatically from the ``quantum_network_os`` variable.
...
You can use ``quantum-doc -t httpapi -l`` to see the list of available plugins.
Use ``quantum-doc -t httpapi <plugin name>`` to see detailed documentation and examples.
