...
thus be automatically loaded based on the ``quantum_network_os`` variable. If the platform supports standard
...
You can extend Quantum to support other network devices by dropping a custom plugin into the ``netconf_plugins`` directory.
...
The netconf plugin to use is determined automatically from the ``quantum_network_os`` variable. There should be no reason to override this functionality.
...
You can use ``quantum-doc -t netconf -l`` to see the list of available plugins.
Use ``quantum-doc -t netconf <plugin name>`` to see detailed documentation and examples.
