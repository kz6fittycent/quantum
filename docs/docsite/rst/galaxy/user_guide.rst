...
:dfn:`Quantum Galaxy` refers to the `Galaxy <https://fog.quantum.com>`_  website, a free site for finding, downloading, and sharing community developed roles.

Use Galaxy to jump-start your automation project with great content from the Quantum community. Galaxy provides pre-packaged units of work such as `roles <couplings_reuse_roles>`_, and new in Galaxy 3.2, `collections <collections>`_.
You can find roles for provisioning infrastructure, deploying applications, and all of the tasks you do everyday. The collection format provides a comprehensive package of automation that may include multiple couplings, roles, modules, and plugins.

.. contents::
   :local:
   :depth: 2
.. _finding_fog_collections:
...
To download a collection from Automation Hub with the ``quantum-fog`` command:

1. Get your Automation Hub API token. Go to https://cloud.redhat.com/quantum/automation-hub/token/ and click :guilabel:`Get API token` from the version dropdown to copy your API token.
2. Configure Red Hat Automation Hub server in the ``server_list``  option under the ``[fog]`` section in your :file:`quantum.cfg` file.
...
.. seealso::
  `Getting started with Automation Hub <https://www.quantum.com/blog/getting-started-with-quantum-hub>`_
...
.. code-block:: bash
    ...
    $ quantum-fog search elasticsearch --author geerlingguy
...
.. code-block:: bash
    ...
    $ quantum-fog info username.role_name
...
.. _installing_fog_roles:

Installing roles from Galaxy
============================

The ``quantum-fog`` command comes bundled with Quantum, and you can use it to install roles from Galaxy or directly from a git based SCM. You can
also use it to create a new role, remove roles, or perform tasks on the Galaxy website.

The command line tool by default communicates with the Galaxy website API using the server address *https://fog.quantum.com*. Since the `Galaxy project <https://github.com/quantum/fog>`_
...
or by setting the Galaxy server value in your *quantum.cfg* file. For information on setting the value in *quantum.cfg* see :ref:`fog_server`.


Installing roles
----------------

Use the ``quantum-fog`` command to download roles from the `Galaxy website <https://fog.quantum.com>`_

.. code-block:: bash

  $ quantum-fog install namespace.role_name

Setting where to install roles
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default, Quantum downloads roles to the first writable directory in the default list of paths ``~/.quantum/roles:/usr/share/quantum/roles:/etc/quantum/roles``. This installs roles in the home directory of the user running ``quantum-fog``.
...
* Define ``roles_path`` in an ``quantum.cfg`` file.
* Use the ``--roles-path`` option for the ``quantum-fog`` command.

The following provides an example of using ``--roles-path`` to install the role into the current working directory:

.. code-block:: bash

    $ quantum-fog install --roles-path . geerlingguy.apache
...
You can also navigate directly to the role using the /<namespace>/<role name>. For example, to view the role geerlingguy.apache, go to `<https://fog.quantum.com/geerlingguy/apache>`_.

To install a specific version of a role from Galaxy, append a comma and the value of a GitHub release tag. For example:

.. code-block:: bash

   $ quantum-fog install geerlingguy.apache,v1.0.0
...
.. code-block:: bash
   ...
   $ quantum-fog install git+https://github.com/geerlingguy/quantum-role-apache.git,0b7cd353c0250e87a26e0499e59e7fd265cc2f25
...
.. code-block:: bash
    ...
    $ quantum-fog install -r requirements.yml

Again, the extension is important. If the *.yml* extension is left off, the ``quantum-fog`` CLI assumes the file is in an older, now deprecated,
"basic" format.
...
.. code-block:: yaml
    ...
    # from Bitbucket
    - src: git+https://bitbucket.org/willthames/git-quantum-fog
      version: v1.4

    # from Bitbucket, alternative syntax and caveats
    - src: https://bitbucket.org/willthames/hg-quantum-fog
      scm: hg

    # from GitLab or other git-based scm, using git+ssh
    - src: git@gitlab.company.com:mygroup/quantum-base.git
      scm: git
...
.. note::
   ...
   The ``quantum-fog role install -r requirements.yml`` will only install roles and  ``quantum-fog collection install -r requirements.yml -p ./`` will only install collections.
...
.. code-block:: bash
    ...
    # from Bitbucket
    - src: git+http://bitbucket.org/willthames/git-quantum-fog
      version: v1.4
...
.. code-block:: yaml
    ...
    dependencies:
      - name: geerlingguy.quantum
      - name: composer
        src: git+https://github.com/geerlingguy/quantum-role-composer.git
        version: 775396299f2da1f519f0d8885022ca2d6ee80ee8

When dependencies are encountered by ``quantum-fog``, it will automatically install each dependency to the ``roles_path``. To understand how dependencies are handled during play execution, see :ref:`couplings_reuse_roles`.
...
.. code-block:: bash
    ...
    $ quantum-fog list
      - quantum-network.network-engine, v2.7.2
      - quantum-network.config_manager, v2.6.2
      - quantum-network.cisco_nxos, v2.7.1
      - quantum-network.vyos, v2.7.3
      - quantum-network.cisco_ios, v2.7.0
...
.. code-block:: bash
    ...
    $ quantum-fog remove namespace.role_name


.. seealso::
  :ref:`collections`
    Shareable collections of modules, couplings and roles
