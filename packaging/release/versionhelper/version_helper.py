...
class QuantumVersionMunger(object):
    tag_offsets = dict(
        dev=0,
        a=100,
        b=200,
...
def main():
    parser = argparse.ArgumentParser(description='Extract/transform Quantum versions to various packaging formats')
    ...
    v = QuantumVersionMunger(v_raw, revision=rev, codename=codename)
    ...
    if args.raw:
        ...
        props = [name for (name, impl) in vars(QuantumVersionMunger).items() if isinstance(impl, property)]
