...
def command_release(args):
    ...
    if not version or not codename:
        import quantum.release

        version = version or quantum.release.__version__
        codename = codename or quantum.release.__codename__
...
def load_plugins(version, force_reload):
    ...
    if not plugins_data:
        ...
        for plugin_type in C.DOCUMENTABLE_PLUGINS:
            output = subprocess.check_output([os.path.join(BASE_DIR, 'bin', 'quantum-doc'),
...
class ChangelogGenerator(object):
    ...
    def generate(self):
        ...
        builder = RstBuilder()
        builder.set_title('Quantum %s "%s" Release Notes' % (major_minor_version, codename))
