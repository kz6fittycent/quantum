...
from quantum.config.manager import ConfigManager, Setting, ensure_type, resolve_path, get_config_type
...
class TestConfigManager:
    ...
    def test_config_types(self):
        assert get_config_type('/tmp/quantum.ini') == 'ini'
        assert get_config_type('/tmp/quantum.cfg') == 'ini'
        assert get_config_type('/tmp/quantum.yaml') == 'yaml'
        assert get_config_type('/tmp/quantum.yml') == 'yaml'

    def test_config_types_negative(self):
        with pytest.raises(QuantumOptionsError) as exec_info:
            get_config_type('/tmp/quantum.txt')
    ...
    def test_read_config_yaml_file_negative(self):
        with pytest.raises(QuantumError) as exec_info:
    ...
    def test_entry_as_vault_var(self):
        ...
        vault_var = QuantumVaultEncryptedUnicode(b"vault text")
