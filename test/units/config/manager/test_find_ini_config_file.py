...
from quantum.config.manager import find_ini_config_file
...
cfg_in_cwd = os.path.join(working_dir, 'quantum.cfg')

cfg_dir = os.path.join(working_dir, 'data')
cfg_file = os.path.join(cfg_dir, 'quantum.cfg')
alt_cfg_file = os.path.join(cfg_dir, 'test.cfg')
cfg_in_homedir = os.path.expanduser('~/.quantum.cfg')
...
class TestFindIniFile:
    ...
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/quantum/quantum.cfg', cfg_in_homedir, cfg_in_cwd, alt_cfg_file, cfg_file)]],
    ...
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/quantum/quantum.cfg', cfg_in_homedir, cfg_in_cwd)]],
                             indirect=['setup_existing_files'])
    ...
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/quantum/quantum.cfg', cfg_in_homedir, cfg_in_cwd, cfg_file, alt_cfg_file)]],
                             indirect=['setup_existing_files'])
    ...
    # No config in cwd
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/quantum/quantum.cfg', cfg_in_homedir, cfg_file, alt_cfg_file)]],
                             indirect=['setup_existing_files'])
    ...
    @pytest.mark.parametrize('setup_env', [[None]], indirect=['setup_env'])
    # No config in cwd
    @pytest.mark.parametrize('setup_existing_files', [[('/etc/quantum/quantum.cfg', cfg_file, alt_cfg_file)]], indirect=['setup_existing_files'])
    def test_ini_in_systemdir(self, setup_env, setup_existing_files):
        """First config found is the system config"""
        warnings = set()
        assert find_ini_config_file(warnings) == '/etc/quantum/quantum.cfg'
        assert warnings == set()
    ...
    def test_cwd_warning_on_writable(self, setup_env, setup_existing_files, monkeypatch):
        ...
        assert u'Quantum is being run in a world writable directory' in warning
        assert u'ignoring it as an quantum.cfg source' in warning
