...
def stdin(mocker, request):
    old_args = quantum.module_utils.basic._ANSIBLE_ARGS
    quantum.module_utils.basic._ANSIBLE_ARGS = None
    old_argv = sys.argv
    sys.argv = ['quantum_unittest']
    ...
    if PY3:
        mocker.patch('quantum.module_utils.basic.sys.stdin', mocker.MagicMock())
        mocker.patch('quantum.module_utils.basic.sys.stdin.buffer', fake_stdin)
    else:
        mocker.patch('quantum.module_utils.basic.sys.stdin', fake_stdin)

    yield fake_stdin

    quantum.module_utils.basic._ANSIBLE_ARGS = old_args
    sys.argv = old_argv
...
def am(stdin, request):
    ...
    am = quantum.module_utils.basic.QuantumModule(
        argument_spec=argspec,
    )
    am._name = 'quantum_unittest'
