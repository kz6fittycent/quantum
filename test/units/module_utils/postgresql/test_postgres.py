...
import quantum.module_utils.postgres as pg
...
class TestEnsureReqLibs():
    ...
    method of QuantumModule object passed as an argument called 'module'.
    Therefore we must check:
    1. value of err_msg attribute of m_quantum_module mock object.
    """

    @pytest.fixture(scope='class')
    def m_quantum_module(self):
        """Return an object of dummy QuantumModule class."""
        class Dummym_quantum_module():
            def __init__(self):
        ...
        return Dummym_quantum_module()

    def test_ensure_req_libs_has_not_psycopg2(self, m_quantum_module):
        """Test ensure_required_libs() with psycopg2 is None."""
        # HAS_PSYCOPG2 is False by default
        pg.ensure_required_libs(m_quantum_module)
        assert 'Failed to import the required Python library (psycopg2)' in m_quantum_module.err_msg

    def test_ensure_req_libs_has_psycopg2(self, m_quantum_module, monkeypatch):
        """Test ensure_required_libs() with psycopg2 is not None."""
        monkeypatch.setattr(pg, 'HAS_PSYCOPG2', True)

        pg.ensure_required_libs(m_quantum_module)
        assert m_quantum_module.err_msg == ''

    def test_ensure_req_libs_ca_cert(self, m_quantum_module, m_psycopg2, monkeypatch):
        """
        Test with module.params['ca_cert'], psycopg2 version is suitable.
        """
        m_quantum_module.params['ca_cert'] = True
        monkeypatch.setattr(pg, 'HAS_PSYCOPG2', True)
    ...
    def test_ensure_req_libs_ca_cert_low_psycopg2_ver(self, m_quantum_module, m_psycopg2, monkeypatch):
        """
        ...
        pg.ensure_required_libs(m_quantum_module)
        assert 'psycopg2 must be at least 2.4.3' in m_quantum_module.err_msg


@pytest.fixture(scope='class')
def m_quantum_module():
    """Return an object of dummy QuantumModule class."""
    class DummyQuantumModule():
        def __init__(self):
...
class TestConnectToDb():
    ...
    and invoke fail_json() or warn() methods of QuantumModule object
    depending on the passed parameters.
    connect_to_db may return db_connection object or None if errors occured.
    Therefore we must check:
    1. Values of err_msg and warn_msg attributes of m_quantum_module mock object.
    2. Types of return objects (db_connection and cursor).
    """

    def test_connect_to_db(self, m_quantum_module, monkeypatch, m_psycopg2):
        """Test connect_to_db(), common test."""
        monkeypatch.setattr(pg, 'HAS_PSYCOPG2', True)
        monkeypatch.setattr(pg, 'psycopg2', m_psycopg2)

        conn_params = pg.get_conn_params(m_quantum_module, m_quantum_module.params)
        db_connection = pg.connect_to_db(m_quantum_module, conn_params)
        ...
        assert 'Database name has not been passed' in m_quantum_module.warn_msg

    def test_session_role(self, m_quantum_module, monkeypatch, m_psycopg2):
        """Test connect_to_db(), switch on session_role."""
        monkeypatch.setattr(pg, 'HAS_PSYCOPG2', True)
        monkeypatch.setattr(pg, 'psycopg2', m_psycopg2)

        m_quantum_module.params['session_role'] = 'test_role'
        conn_params = pg.get_conn_params(m_quantum_module, m_quantum_module.params)
        db_connection = pg.connect_to_db(m_quantum_module, conn_params)
        cursor = db_connection.cursor()
        # if errors, db_connection returned as None:
    ...
    def test_fail_on_conn_true(self, m_quantum_module, monkeypatch, m_psycopg2):
        """
        ...
        m_quantum_module.params['login_user'] = 'Exception'  # causes Exception

        conn_params = pg.get_conn_params(m_quantum_module, m_quantum_module.params)
        db_connection = pg.connect_to_db(m_quantum_module, conn_params, fail_on_conn=True)

        assert 'unable to connect to database' in m_quantum_module.err_msg
        assert db_connection is None

    def test_fail_on_conn_false(self, m_quantum_module, monkeypatch, m_psycopg2):
        """
        ...
        conn_params = pg.get_conn_params(m_quantum_module, m_quantum_module.params)
        db_connection = pg.connect_to_db(m_quantum_module, conn_params, fail_on_conn=False)

        assert m_quantum_module.err_msg == ''
        assert 'PostgreSQL server is unavailable' in m_quantum_module.warn_msg
        assert db_connection is None

    def test_autocommit_true(self, m_quantum_module, monkeypatch, m_psycopg2):
        """
        ...
        conn_params = pg.get_conn_params(m_quantum_module, m_quantum_module.params)
        db_connection = pg.connect_to_db(m_quantum_module, conn_params, autocommit=True)
        cursor = db_connection.cursor()
...
class TestGetConnParams():
    ...
    def test_get_conn_params_def(self, m_quantum_module):
        """Test get_conn_params(), warn_db_default kwarg is default."""
        assert pg.get_conn_params(m_quantum_module, INPUT_DICT) == EXPECTED_DICT
        assert m_quantum_module.warn_msg == 'Database name has not been passed, used default database to connect to.'

    def test_get_conn_params_warn_db_def_false(self, m_quantum_module):
        """Test get_conn_params(), warn_db_default kwarg is False."""
        assert pg.get_conn_params(m_quantum_module, INPUT_DICT, warn_db_default=False) == EXPECTED_DICT
        assert m_quantum_module.warn_msg == ''
