...
from quantum.module_utils.network.nso import nso
    ...
    '/an:id-name-leaf': '''
{
  "meta": {
    "prefix": "an",
    "namespace": "http://github.com/quantum/nso",
    "types": {
      "http://github.com/quantum/nso:id-name-t": [
        {
          "name": "http://github.com/quantum/nso:id-name-t",
          "enumeration": [
            {
              "label": "id-one"
            },
...
class TestJsonRpc(unittest.TestCase):
    @patch('quantum.module_utils.network.nso.nso.open_url')
...
class TestValueBuilder(unittest.TestCase):
    ...
    def test_identityref_leaf(self, open_url_mock):
        ...
        vb.build(parent, None, 'quantum-nso:id-two', schema)
    ...
    def test_identityref_key(self, open_url_mock):
        ...
        vb = nso.ValueBuilder(nso.JsonRpc('http://localhost:8080/jsonrpc', 10, False))
        vb.build(parent, 'id-name-value', [{'name': 'quantum-nso:id-one', 'value': '1'}], schema)
        values = list(vb.values)
        self.assertEquals(1, len(values))
        value = values[0]
