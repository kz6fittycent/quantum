...
def test_set_vm_power_state_bad_vm_ref(fake_quantum_module, xenserver, vm_ref):
    """Tests failure on bad vm_ref."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.set_vm_power_state(fake_quantum_module, vm_ref, None)

    assert exc_info.value.kwargs['msg'] == "Cannot set VM power state. Invalid VM reference supplied!"


def test_set_vm_power_state_xenapi_failure(mock_xenapi_failure, fake_quantum_module, xenserver):
    """Tests catching of XenAPI failures."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), "poweredon")

    assert exc_info.value.kwargs['msg'] == "XAPI ERROR: %s" % mock_xenapi_failure[1]


def test_set_vm_power_state_bad_power_state(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), "bad")
...
def test_set_vm_power_state_bad_transition(mocker, fake_quantum_module, XenAPI, xenserver, power_state_desired, power_state_current, error_msg):
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state_desired)
...
def test_set_vm_power_state_task_timeout(mocker, fake_quantum_module, XenAPI, xenserver, power_state, error_msg):
    ...
    mocker.patch('quantum.module_utils.xenserver.wait_for_task', return_value="timeout")

    with pytest.raises(FailJsonException) as exc_info:
        xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state, timeout=1)
...
def test_set_vm_power_state_no_transition(mocker, fake_quantum_module, XenAPI, xenserver, power_state_desired, power_state_current):
    ...
    result = xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state_desired)
...
def test_set_vm_power_state_transition(mocker,
    ...
    result = xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state_desired, timeout=0)
...
def test_set_vm_power_state_transition_async(mocker,
    ...
    mocker.patch('quantum.module_utils.xenserver.wait_for_task', return_value="")

    result = xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state_desired, timeout=1)
...
def test_set_vm_power_state_transition_check_mode(mocker,
    ...
    fake_quantum_module.check_mode = True
    result = xenserver.set_vm_power_state(fake_quantum_module, fake_xenapi_ref('VM'), power_state_desired, timeout=0)
