...
def test_wait_for_vm_ip_address_bad_vm_ref(fake_quantum_module, xenserver, vm_ref):
    """Tests failure on bad vm_ref."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.wait_for_vm_ip_address(fake_quantum_module, vm_ref)

    assert exc_info.value.kwargs['msg'] == "Cannot wait for VM IP address. Invalid VM reference supplied!"


def test_wait_for_vm_ip_address_xenapi_failure(mock_xenapi_failure, xenserver, fake_quantum_module):
    """Tests catching of XenAPI failures."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.wait_for_vm_ip_address(fake_quantum_module, fake_xenapi_ref('VM'))
...
def test_wait_for_vm_ip_address_bad_power_state(mocker, fake_quantum_module, XenAPI, xenserver, bad_power_state):
...
def test_wait_for_vm_ip_address_timeout(mocker, fake_quantum_module, XenAPI, xenserver, bad_guest_metrics_ref, bad_guest_metrics):
    """Tests timeout."""
    mocked_xenapi = mocker.patch.object(XenAPI.Session, 'xenapi', create=True)
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.wait_for_vm_ip_address(fake_quantum_module, fake_xenapi_ref('VM'), timeout=1)

    assert exc_info.value.kwargs['msg'] == "Timed out waiting for VM IP address!"


def test_wait_for_vm_ip_address(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    fake_guest_metrics = xenserver.wait_for_vm_ip_address(fake_quantum_module, fake_xenapi_ref('VM'))
...
def test_wait_for_task_bad_task_ref(fake_quantum_module, xenserver, task_ref):
    """Tests failure on bad task_ref."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.wait_for_task(fake_quantum_module, task_ref)

    assert exc_info.value.kwargs['msg'] == "Cannot wait for task. Invalid task reference supplied!"


def test_wait_for_task_xenapi_failure(mock_xenapi_failure, fake_quantum_module, xenserver):
    """Tests catching of XenAPI failures."""
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.wait_for_task(fake_quantum_module, fake_xenapi_ref('task'))

    assert exc_info.value.kwargs['msg'] == "XAPI ERROR: %s" % mock_xenapi_failure[1]


def test_wait_for_task_timeout(mocker, fake_quantum_module, XenAPI, xenserver):
    """Tests timeout."""
    mocked_xenapi = mocker.patch.object(XenAPI.Session, 'xenapi', create=True)
    ...
    fake_result = xenserver.wait_for_task(fake_quantum_module, fake_xenapi_ref('task'), timeout=1)
...
def test_wait_for_task(mocker, fake_quantum_module, XenAPI, xenserver, task_status, result):
    """Tests regular invocation."""
    mocked_xenapi = mocker.patch.object(XenAPI.Session, 'xenapi', create=True)
    ...
    fake_result = xenserver.wait_for_task(fake_quantum_module, fake_xenapi_ref('task'))
