...
def test_get_object_ref_xenapi_failure(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.get_object_ref(fake_quantum_module, "name")

    assert exc_info.value.kwargs['msg'] == "XAPI ERROR: Fake XAPI method call error!"


def test_get_object_ref_bad_uuid_and_name(mocker, fake_quantum_module, XenAPI, xenserver):
    """Tests failure on bad object uuid and/or name."""
    mocked_xenapi = mocker.patch.object(XenAPI.Session, 'xenapi_request')

    with pytest.raises(FailJsonException) as exc_info:
        xenserver.get_object_ref(fake_quantum_module, None, msg_prefix="Test: ")
...
def test_get_object_ref_uuid_not_found(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.get_object_ref(fake_quantum_module, "name", uuid="fake-uuid", msg_prefix="Test: ")

    assert exc_info.value.kwargs['msg'] == "Test: VM with UUID 'fake-uuid' not found!"
    assert xenserver.get_object_ref(fake_quantum_module, "name", uuid="fake-uuid", fail=False, msg_prefix="Test: ") is None


def test_get_object_ref_name_not_found(mocker, fake_quantum_module, XenAPI, xenserver):
    """Tests when object is not found by name."""
    mocked_xenapi = mocker.patch.object(XenAPI.Session, 'xenapi_request', return_value=[])

    with pytest.raises(FailJsonException) as exc_info:
        xenserver.get_object_ref(fake_quantum_module, "name", msg_prefix="Test: ")

    assert exc_info.value.kwargs['msg'] == "Test: VM with name 'name' not found!"
    assert xenserver.get_object_ref(fake_quantum_module, "name", fail=False, msg_prefix="Test: ") is None


def test_get_object_ref_name_multiple_found(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    with pytest.raises(FailJsonException) as exc_info:
        xenserver.get_object_ref(fake_quantum_module, "name", fail=False, msg_prefix="Test: ")
