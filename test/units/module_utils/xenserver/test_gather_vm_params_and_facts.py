...
testcase_gather_vm_params_and_facts = {
    "params": [
        ["quantum-test-vm-1-params.json", "quantum-test-vm-1-facts.json"],
        ["quantum-test-vm-2-params.json", "quantum-test-vm-2-facts.json"],
        ["quantum-test-vm-3-params.json", "quantum-test-vm-3-facts.json"],
    ],
    "ids": [
        "quantum-test-vm-1",
        "quantum-test-vm-2",
...
def test_gather_vm_params_bad_vm_ref(fake_quantum_module, xenserver, vm_ref):
    """Tests return of empty dict on bad vm_ref."""
    assert xenserver.gather_vm_params(fake_quantum_module, vm_ref) == {}


def test_gather_vm_facts_no_vm_params(fake_quantum_module, xenserver):
    """Tests return of empty facts dict when vm_params is not available"""
    assert xenserver.gather_vm_facts(fake_quantum_module, None) == {}
    assert xenserver.gather_vm_facts(fake_quantum_module, {}) == {}
...
@pytest.mark.parametrize('fixture_data_from_file',
                         ...
                         indirect=True)
def test_gather_vm_params_and_facts(mocker, fake_quantum_module, XenAPI, xenserver, fixture_data_from_file):
    ...
    assert xenserver.gather_vm_facts(fake_quantum_module, xenserver.gather_vm_params(fake_quantum_module, vm_ref)) == fixture_data_from_file[facts_file]
