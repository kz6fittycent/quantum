...
def test_xenserverobject_xenapi_lib_detection(mocker, fake_quantum_module, xenserver):
    """Tests XenAPI lib detection code."""
    mocker.patch('quantum.module_utils.xenserver.HAS_XENAPI', new=False)

    with pytest.raises(FailJsonException) as exc_info:
        xenserver.XenServerObject(fake_quantum_module)

    assert 'Failed to import the required Python library (XenAPI) on' in exc_info.value.kwargs['msg']


def test_xenserverobject_xenapi_failure(mock_xenapi_failure, fake_quantum_module, xenserver):
...
def test_xenserverobject(mocker, fake_quantum_module, XenAPI, xenserver):
    ...
    xso = xenserver.XenServerObject(fake_quantum_module)
