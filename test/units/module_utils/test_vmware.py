...
import quantum.module_utils.vmware as vmware_module_utils
...
def test_lib_loading_failure(monkeypatch, fake_quantum_module, key, libname):
    """ Test if Pyvmomi is present or not"""
    monkeypatch.setattr(vmware_module_utils, key, False)
    with pytest.raises(FailJsonException):
        vmware_module_utils.PyVmomi(fake_quantum_module)
    error_str = 'Failed to import the required Python library (%s)' % libname
    assert fake_quantum_module.fail_json.called_once()
    assert error_str in fake_quantum_module.fail_json.call_args[1]['msg']
...
def test_required_params(request, params, msg, fake_quantum_module):
    """ Test if required params are correct or not"""
    fake_quantum_module.params = params
    with pytest.raises(FailJsonException):
        vmware_module_utils.connect_to_api(fake_quantum_module)
    assert fake_quantum_module.fail_json.called_once()
    assert msg in fake_quantum_module.fail_json.call_args[1]['msg']


def test_validate_certs(monkeypatch, fake_quantum_module):
    """ Test if SSL is required or not"""
    fake_quantum_module.params = test_data[3][0]
...
def test_vmdk_disk_path_split(monkeypatch, fake_quantum_module):
    """ Test vmdk_disk_path_split function"""
    fake_quantum_module.params = test_data[0][0]

    monkeypatch.setattr(vmware_module_utils, 'connect_to_api', fake_connect_to_api)
    pyv = vmware_module_utils.PyVmomi(fake_quantum_module)
    v = pyv.vmdk_disk_path_split('[ds1] VM_0001/VM0001_0.vmdk')
    assert v == ('ds1', 'VM_0001/VM0001_0.vmdk', 'VM0001_0.vmdk', 'VM_0001')


def test_vmdk_disk_path_split_negative(monkeypatch, fake_quantum_module):
    """ Test vmdk_disk_path_split function"""
    fake_quantum_module.params = test_data[0][0]
    ...
    assert fake_quantum_module.fail_json.called_once()
    assert 'Bad path' in fake_quantum_module.fail_json.call_args[1]['msg']


@pytest.mark.skipif(sys.version_info < (2, 7), reason="requires python2.7 and greater")
def test_connect_to_api_validate_certs(monkeypatch, fake_quantum_module):
    ...
    fake_quantum_module.params['validate_certs'] = True
    vmware_module_utils.connect_to_api(fake_quantum_module)
    ...
    ssl_context = mock.Mock()
    monkeypatch.setattr(vmware_module_utils.ssl, 'SSLContext', MockSSLContext)
    fake_quantum_module.params['validate_certs'] = False
    vmware_module_utils.connect_to_api(fake_quantum_module)
    assert ssl_context.proto == ssl.PROTOCOL_SSLv23
    ...
    fake_quantum_module.fail_json.assert_called_once_with(msg=(
