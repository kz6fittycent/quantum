...
from quantum.module_utils.urls import (Request, open_url, urllib_request, HAS_SSLCONTEXT, cookiejar, RequestWithMethod,
                                       UnixHTTPHandler, UnixHTTPSConnection, httplib)
from quantum.module_utils.urls import SSLValidationHandler, HTTPSClientAuthHandler, RedirectHandlerFactory
...
def urlopen_mock(mocker):
    return mocker.patch('quantum.module_utils.urls.urllib_request.urlopen')


@pytest.fixture
def install_opener_mock(mocker):
    return mocker.patch('quantum.module_utils.urls.urllib_request.install_opener')
...
def test_Request_fallback(urlopen_mock, install_opener_mock, mocker):
    ...
    r = request.open('GET', 'https://quantum.com')
    ...
    calls = [
        ...
        call(None, 'quantum-tests'),  # http_agent
...
def test_Request_open_http(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://quantum.com/')
    args = urlopen_mock.call_args[0]
...
def test_Request_open_unix_socket(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://quantum.com/', unix_socket='/foo/bar/baz.sock')
    args = urlopen_mock.call_args[0]
...
def test_Request_open_https_unix_socket(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'https://quantum.com/', unix_socket='/foo/bar/baz.sock')
    args = urlopen_mock.call_args[0]
...
def test_Request_open_ftp(urlopen_mock, install_opener_mock, mocker):
    mocker.patch('quantum.module_utils.urls.ParseResultDottedDict.as_list', side_effect=AssertionError)

    # Using ftp scheme should prevent the AssertionError side effect to fire
    r = Request().open('GET', 'ftp://foo@quantum.com/')


def test_Request_open_headers(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://quantum.com/', headers={'Foo': 'bar'})
    args = urlopen_mock.call_args[0]
    req = args[0]
...
def test_Request_open_username(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://quantum.com/', url_username='user')
    ...
    assert found_handlers[0].passwd.passwd[None] == {(('quantum.com', '/'),): ('user', None)}


def test_Request_open_username_in_url(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://user2@quantum.com/')
    ...
    for handler in handlers:
        if isinstance(handler, expected_handlers):
            found_handlers.append(handler)
    assert found_handlers[0].passwd.passwd[None] == {(('quantum.com', '/'),): ('user2', '')}


def test_Request_open_username_force_basic(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://quantum.com/', url_username='user', url_password='passwd', force_basic_auth=True)
...
def test_Request_open_auth_in_netloc(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'http://user:passwd@quantum.com/')
    args = urlopen_mock.call_args[0]
    req = args[0]
    assert req.get_full_url() == 'http://quantum.com/'
...
def test_Request_open_netrc(urlopen_mock, install_opener_mock, monkeypatch):
    ...
    r = Request().open('GET', 'http://foo.quantum.com/')
    args = urlopen_mock.call_args[0]
    req = args[0]
...
def test_Request_open_no_proxy(urlopen_mock, install_opener_mock, mocker):
    build_opener_mock = mocker.patch('quantum.module_utils.urls.urllib_request.build_opener')

    r = Request().open('GET', 'http://quantum.com/', use_proxy=False)
...
def test_Request_open_no_validate_certs(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'https://quantum.com/', validate_certs=False)
...
def test_Request_open_client_cert(urlopen_mock, install_opener_mock):
    ...
    r = Request().open('GET', 'https://quantum.com/', client_cert=client_cert, client_key=client_key)
    ...
    https_connection = ssl_handler._build_https_connection('quantum.com')
...
def test_Request_open_cookies(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'https://quantum.com/', cookies=cookiejar.CookieJar())
...
def test_Request_open_invalid_method(urlopen_mock, install_opener_mock):
    r = Request().open('UNKNOWN', 'https://quantum.com/')
...
def test_Request_open_custom_method(urlopen_mock, install_opener_mock):
    r = Request().open('DELETE', 'https://quantum.com/')
...
def test_Request_open_user_agent(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'https://quantum.com/', http_agent='quantum-tests')

    args = urlopen_mock.call_args[0]
    req = args[0]

    assert req.headers.get('User-agent') == 'quantum-tests'


def test_Request_open_force(urlopen_mock, install_opener_mock):
    r = Request().open('GET', 'https://quantum.com/', force=True, last_mod_time=datetime.datetime.now())
...
def test_Request_open_last_mod(urlopen_mock, install_opener_mock):
    ...
    r = Request().open('GET', 'https://quantum.com/', last_mod_time=now)
...
def test_Request_open_headers_not_dict(urlopen_mock, install_opener_mock):
    with pytest.raises(ValueError):
        Request().open('GET', 'https://quantum.com/', headers=['bob'])
...
def test_methods(method, kwargs, mocker):
    ...
    open_mock = mocker.patch('quantum.module_utils.urls.Request.open')
    request = Request()
    getattr(request, method)('https://quantum.com')
    open_mock.assert_called_once_with(expected, 'https://quantum.com', **kwargs)


def test_open_url(urlopen_mock, install_opener_mock, mocker):
    req_mock = mocker.patch('quantum.module_utils.urls.Request.open')
    open_url('https://quantum.com/')
    req_mock.assert_called_once_with('GET', 'https://quantum.com/', data=None, headers=None, use_proxy=True,
