...
from quantum.module_utils import urls
...
def test_maybe_add_ssl_handler(mocker):
    ...
    with pytest.raises(urls.NoSSLError):
        urls.maybe_add_ssl_handler('https://quantum.com/', True)

    mocker.patch.object(urls, 'HAS_SSL', new=True)
    url = 'https://user:passwd@quantum.com/'
    handler = urls.maybe_add_ssl_handler(url, True)
    assert handler.hostname == 'quantum.com'
    assert handler.port == 443

    url = 'https://quantum.com:4433/'
    handler = urls.maybe_add_ssl_handler(url, True)
    assert handler.hostname == 'quantum.com'
    assert handler.port == 4433

    url = 'https://user:passwd@quantum.com:4433/'
    handler = urls.maybe_add_ssl_handler(url, True)
    assert handler.hostname == 'quantum.com'
    assert handler.port == 4433
...
def test_unix_socket_patch_httpconnection_connect(mocker):
    ...
    conn = urls.httplib.HTTPConnection('quantum.com')
