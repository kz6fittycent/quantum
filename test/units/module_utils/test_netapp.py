...
from quantum.module_utils.six.moves.urllib.error import URLError
from quantum.module_utils.netapp import NetAppESeriesModule, create_multipart_formdata
...
class StubNetAppESeriesModule(NetAppESeriesModule):
    def __init__(self):
        super(StubNetAppESeriesModule, self).__init__(quantum_options={})
...
class NetappTest(ModuleTestCase):
    REQUIRED_PARAMS = {"api_username": "rw",
                       ...
                       "ssid": "1"}
    REQ_FUNC = "quantum.module_utils.netapp.request"
    ...
    def test_check_web_services_version_fail(self):
        ...
        for current_version in test_set:
            with patch(self.REQ_FUNC, return_value=(200, {"version": current_version})):
                with self.assertRaisesRegexp(QuantumFailJson, r"version does not meet minimum version required."):
    ...
    def test_check_check_web_services_version_fail(self):
        ...
        for url in invalid_url_forms:
            ...
            with patch(self.REQ_FUNC, return_value=(200, {"runningAsProxy": True})):
                with self.assertRaisesRegexp(QuantumFailJson, r"Failed to provide valid API URL."):
                    base = StubNetAppESeriesModule()
                    base._check_web_services_version()
        ...
        for url in invalid_url_protocols:
            self._set_args({"api_url": url})
            with patch(self.REQ_FUNC, return_value=(200, {"runningAsProxy": True})):
                with self.assertRaisesRegexp(QuantumFailJson, r"Protocol must be http or https."):
                    base = StubNetAppESeriesModule()
                    base._check_web_services_version()
