...
from quantum.module_utils.facts import namespace
...
def _collectors(module,
    ...
    collector_meta_data_collector = \
        quantum_collector.CollectorMetaDataCollector(gather_subset=gather_subset,
                                                     module_setup=True)
...
ns = namespace.PrefixFactNamespace('quantum_facts', 'quantum_')
...
class TestInPlace(unittest.TestCase):
    ...
    def test(self):
        ...
        fact_collector = \
            quantum_collector.QuantumFactCollector(collectors=collectors,
                                                   namespace=ns)
...
class TestCollectedFacts(unittest.TestCase):
    ...
    # TODO: add quantum_cmdline, quantum_*_pubkey* back when TempFactCollector goes away
    ...
    @patch('quantum.module_utils.facts.system.service_mgr.get_file_content', return_value='systemd')
    ...
    # everything starts with quantum_ namespace
    def _assert_quantum_namespace(self, facts):
        ...
        for fact_key in facts:
            self.assertTrue(fact_key.startswith('quantum_'),
                            'The fact name "%s" does not startwith "quantum_"' % fact_key)
...
class TestCollectorDepsWithFilter(unittest.TestCase):
    ...
    def test_with_filter_no_match(self):
        _mock_module = mock_module(gather_subset=['all', '!facter', '!ohai'],
                                   filter='quantum_this_doesnt_exist')
    ...
    def test_concat_collector(self):
        ...
        fact_collector = \
            quantum_collector.QuantumFactCollector(collectors=_collectors,
                                                   namespace=ns,
