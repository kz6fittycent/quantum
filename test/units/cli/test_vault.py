...
from quantum import context, errors
from quantum.cli.vault import VaultCLI
...
class TestVaultCli(unittest.TestCase):
    def setUp(self):
        self.tty_patcher = patch('quantum.cli.sys.stdin.isatty', return_value=False)
    ...
    def test_parse_view_file(self):
        cli = VaultCLI(args=['quantum-vault', 'view', '/dev/null/foo'])
        cli.parse()

    @patch('quantum.cli.vault.VaultCLI.setup_vault_secrets')
    def test_view_missing_file_no_secret(self, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = []
        cli = VaultCLI(args=['quantum-vault', 'view', '/dev/null/foo'])
        cli.parse()
        self.assertRaisesRegexp(errors.QuantumOptionsError,
                                "A vault password is required to use Quantum's Vault",
                                cli.run)

    @patch('quantum.cli.vault.VaultCLI.setup_vault_secrets')
    def test_encrypt_missing_file_no_secret(self, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = []
        cli = VaultCLI(args=['quantum-vault', 'encrypt', '/dev/null/foo'])
        cli.parse()
        self.assertRaisesRegexp(errors.QuantumOptionsError,
                                "A vault password is required to use Quantum's Vault",
                                cli.run)
    ...
    @patch('quantum.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('quantum.cli.vault.VaultEditor')
    ...
    def test_encrypt_string(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault', 'encrypt_string',
    ...
    @patch('quantum.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('quantum.cli.vault.VaultEditor')
    @patch('quantum.cli.vault.display.prompt', return_value='a_prompt')
    def test_encrypt_string_prompt(self, mock_display, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault',
                             'encrypt_string',
                             '--prompt',
                             'some string to encrypt'])
        cli.parse()
    ...
    @patch('quantum.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('quantum.cli.vault.VaultEditor')
    @patch('quantum.cli.vault.sys.stdin.read', return_value='This is data from stdin')
    ...
    def test_create(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault', 'create', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_edit(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault', 'edit', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_decrypt(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault', 'decrypt', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_rekey(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['quantum-vault', 'rekey', '/dev/null/foo'])
        cli.parse()
        cli.run()


@pytest.mark.parametrize('cli_args, expected', [
    (['quantum-vault', 'view', 'vault.txt'], 0),
    (['quantum-vault', 'view', 'vault.txt', '-vvv'], 3),
    (['quantum-vault', '-vv', 'view', 'vault.txt'], 2),
    ...
    (['quantum-vault', '-vv', 'view', 'vault.txt', '-v'], 1),
    (['quantum-vault', '-vv', 'view', 'vault.txt', '-vvvv'], 4),
])
def test_verbosity_arguments(cli_args, expected, tmp_path_factory, monkeypatch):
    ...
    test_dir = to_text(tmp_path_factory.mktemp('test-quantum-vault'))
