...
from quantum.cli.fog import GalaxyCLI
...
class TestGalaxy(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.temp_dir = tempfile.mkdtemp(prefix='quantum-test_fog-')
        ...
        gc = GalaxyCLI(args=["quantum-fog", "init", "--offline", "delete_me"])
        gc.run()
    ...
    def test_init(self):
        fog_cli = GalaxyCLI(args=self.default_args)
        self.assertTrue(isinstance(fog_cli, GalaxyCLI))
    ...
    def test_display_fog_info(self):
        gc = GalaxyCLI(args=self.default_args)
        fog_info = {}
        role_info = {'name': 'some_role_name',
                     'fog_info': fog_info}
        ...
        if display_result.find('\n\tfog_info:') == -1:
            self.fail('Expected fog_info to be indented once')

    def test_run(self):
        ''' verifies that the GalaxyCLI object's api is created and that execute() is called. '''
        gc = GalaxyCLI(args=["quantum-fog", "install", "--ignore-errors", "imaginary_role"])
        gc.parse()
        with patch.object(quantum.cli.CLI, "run", return_value=None) as mock_run:
            gc.run()
            # testing
            self.assertIsInstance(gc.fog, quantum.fog.Galaxy)
            self.assertEqual(mock_run.call_count, 1)
            self.assertTrue(isinstance(gc.api, quantum.fog.api.GalaxyAPI))

    def test_execute_remove(self):
        # installing role
        gc = GalaxyCLI(args=["quantum-fog", "install", "-p", self.role_path, "-r", self.role_req, '--force'])
        gc.run()
        ...
        co.GlobalCLIArgs._Singleton__instance = None
        gc = GalaxyCLI(args=["quantum-fog", "remove", role_file, self.role_name])
        gc.run()
    ...
    def test_exit_without_ignore_without_flag(self):
        ...
        gc = GalaxyCLI(args=["quantum-fog", "install", "--server=None", "fake_role_name"])
        with patch.object(quantum.utils.display.Display, "display", return_value=None) as mocked_display:
            # testing that error expected is raised
            self.assertRaises(QuantumError, gc.run)
            self.assertTrue(mocked_display.called_once_with("- downloading role 'fake_role_name', owned by "))

    def test_exit_without_ignore_with_flag(self):
        ''' tests that GalaxyCLI exits without the error specified if the --ignore-errors flag is used  '''
        # testing with --ignore-errors flag
        gc = GalaxyCLI(args=["quantum-fog", "install", "--server=None", "fake_role_name", "--ignore-errors"])
        with patch.object(quantum.utils.display.Display, "display", return_value=None) as mocked_display:
            gc.run()
            self.assertTrue(mocked_display.called_once_with("- downloading role 'fake_role_name', owned by "))
    ...
    def test_parse_install(self):
        ...
        gc = GalaxyCLI(args=["quantum-fog", "install"])
        gc.parse()
...
class ValidRoleTests(object):
    ...
    @classmethod
    def setUpRole(cls, role_name, fog_args=None, skeleton_path=None, use_explicit_type=False):
        if fog_args is None:
            fog_args = []

        if skeleton_path is not None:
            cls.role_skeleton_path = skeleton_path
            fog_args += ['--role-skeleton', skeleton_path]
        ...
        if use_explicit_type:
            args += ['role']
        args += ['init', '-c', '--offline'] + fog_args + ['--init-path', cls.test_dir, cls.role_name]
        ...
        if skeleton_path is None:
            cls.role_skeleton_path = gc.fog.default_role_skeleton_path
    ...
    def test_metadata(self):
        ...
        self.assertIn('fog_info', metadata, msg='unable to find fog_info in metadata')
    ...
    def test_test_yml(self):
        with open(os.path.join(self.role_dir, 'tests', 'test.yml'), 'r') as f:
            test_coupling = yaml.safe_load(f)
        print(test_coupling)
        self.assertEqual(len(test_coupling), 1)
        self.assertEqual(test_coupling[0]['hosts'], 'localhost')
        self.assertEqual(test_coupling[0]['remote_user'], 'root')
        self.assertListEqual(test_coupling[0]['roles'], [self.role_name], msg='The list of roles included in the test play doesn\'t match')
...
class TestGalaxyInitDefault(unittest.TestCase, ValidRoleTests):
    ...
    def test_metadata_contents(self):
        with open(os.path.join(self.role_dir, 'meta', 'main.yml'), 'r') as mf:
            metadata = yaml.safe_load(mf)
        self.assertEqual(metadata.get('fog_info', dict()).get('author'), 'your name', msg='author was not set properly in metadata')
...
class TestGalaxyInitAPB(unittest.TestCase, ValidRoleTests):
    ...
    @classmethod
    def setUpClass(cls):
        cls.setUpRole('delete_me_apb', fog_args=['--type=apb'])

    def test_metadata_apb_tag(self):
        with open(os.path.join(self.role_dir, 'meta', 'main.yml'), 'r') as mf:
            metadata = yaml.safe_load(mf)
        self.assertIn('apb', metadata.get('fog_info', dict()).get('fog_tags', []), msg='apb tag not set in role metadata')
    ...
    def test_test_yml(self):
        ...
        self.assertEqual(len(test_coupling), 1)
        self.assertEqual(test_coupling[0]['hosts'], 'localhost')
        self.assertFalse(test_coupling[0]['gather_facts'])
        self.assertEqual(test_coupling[0]['connection'], 'local')
        self.assertIsNone(test_coupling[0]['tasks'], msg='We\'re expecting an unset list of tasks in test.yml')
...
class TestGalaxyInitContainer(unittest.TestCase, ValidRoleTests):
    ...
    @classmethod
    def setUpClass(cls):
        cls.setUpRole('delete_me_container', fog_args=['--type=container'])

    def test_metadata_container_tag(self):
        with open(os.path.join(self.role_dir, 'meta', 'main.yml'), 'r') as mf:
            metadata = yaml.safe_load(mf)
        self.assertIn('container', metadata.get('fog_info', dict()).get('fog_tags', []), msg='container tag not set in role metadata')
...
@pytest.mark.parametrize('cli_args, expected', [
    (['quantum-fog', 'collection', 'init', 'abc.def'], 0),
    (['quantum-fog', 'collection', 'init', 'abc.def', '-vvv'], 3),
    (['quantum-fog', '-vv', 'collection', 'init', 'abc.def'], 2),
    ...
    (['quantum-fog', '-vv', 'collection', 'init', 'abc.def', '-v'], 1),
    (['quantum-fog', '-vv', 'collection', 'init', 'abc.def', '-vvvv'], 4),
    (['quantum-fog', '-vvv', 'init', 'name'], 3),
    (['quantum-fog', '-vvvvv', 'init', '-v', 'name'], 1),
])
...
def collection_skeleton(request, tmp_path_factory):
    ...
    fog_args = ['quantum-fog', 'collection', 'init', '-c']

    if skeleton_path is not None:
        fog_args += ['--collection-skeleton', skeleton_path]

    test_dir = to_text(tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections'))
    fog_args += ['--init-path', test_dir, name]

    GalaxyCLI(args=fog_args).run()
...
@pytest.mark.parametrize('collection_skeleton', [
    ('quantum_test.my_collection', None),
], indirect=True)
def test_collection_default(collection_skeleton):
    meta_path = os.path.join(collection_skeleton, 'fog.yml')

    with open(meta_path, 'r') as fog_meta:
        metadata = yaml.safe_load(fog_meta)

    assert metadata['namespace'] == 'quantum_test'
...
@pytest.mark.parametrize('collection_skeleton', [
    ('quantum_test.delete_me_skeleton', os.path.join(os.path.split(__file__)[0], 'test_data', 'collection_skeleton')),
], indirect=True)
def test_collection_skeleton(collection_skeleton):
    ...
    assert metadata['authors'] == ['Quantum Cow <acow@bovineuniversity.edu>', 'Tu Cow <tucow@bovineuniversity.edu>']
    ...
    for template_dir in ['couplings/templates', 'couplings/templates/subfolder',
...
def collection_artifact(collection_skeleton, tmp_path_factory):
    ...
    try:
        co.GlobalCLIArgs._Singleton__instance = None
        fog_args = ['quantum-fog', 'collection', 'build', collection_skeleton, '--output-path', output_dir]
        gc = GalaxyCLI(args=fog_args)
        gc.run()
...
def test_invalid_skeleton_path():
    ...
    gc = GalaxyCLI(args=['quantum-fog', 'collection', 'init', 'my.collection', '--collection-skeleton',
                         '/fake/path'])
    with pytest.raises(QuantumError, match=expected):
        gc.run()
...
def test_invalid_collection_name_init(name):
    ...
    gc = GalaxyCLI(args=['quantum-fog', 'collection', 'init', name])
    with pytest.raises(QuantumError, match=expected):
        gc.run()
...
def test_invalid_collection_name_install(name, expected, tmp_path_factory):
    ...
    gc = GalaxyCLI(args=['quantum-fog', 'collection', 'install', name, '-p', os.path.join(install_path, 'install')])
    with pytest.raises(QuantumError, match=expected):
        gc.run()


@pytest.mark.parametrize('collection_skeleton', [
    ('quantum_test.build_collection', None),
], indirect=True)
def test_collection_build(collection_artifact):
    tar_path = os.path.join(collection_artifact, 'quantum_test-build_collection-1.0.0.tar.gz')
    ...
    with tarfile.open(tar_path, mode='r') as tar:
        ...
        assert coll_info['namespace'] == 'quantum_test'
...
def collection_install(reset_cli_args, tmp_path_factory, monkeypatch):
    ...
    monkeypatch.setattr(quantum.cli.fog, 'install_collections', mock_install)

    mock_warning = MagicMock()
    monkeypatch.setattr(quantum.utils.display.Display, 'warning', mock_warning)
...
def test_collection_install_with_names(collection_install):
    ...
    fog_args = ['quantum-fog', 'collection', 'install', 'namespace.collection', 'namespace2.collection:1.0.1',
                   '--collections-path', output_dir]
    GalaxyCLI(args=fog_args).run()

    collection_path = os.path.join(output_dir, 'quantum_collections')
    assert os.path.isdir(collection_path)

    assert mock_warning.call_count == 1
    assert "The specified collections path '%s' is not part of the configured Quantum collections path" % output_dir \
    ...
    assert mock_install.call_args[0][2][0].api_server == 'https://fog.quantum.com'
...
''')
    ...
    fog_args = ['quantum-fog', 'collection', 'install', '--requirements-file', requirements_file,
                   '--collections-path', output_dir]
    GalaxyCLI(args=fog_args).run()
...
def test_collection_install_with_relative_path(collection_install, monkeypatch):
    ...
    monkeypatch.setattr(quantum.cli.fog.GalaxyCLI, '_parse_requirements_file', mock_req)

    monkeypatch.setattr(os, 'makedirs', MagicMock())

    requirements_file = './requirements.myl'
    collections_path = './quantum_collections'
    fog_args = ['quantum-fog', 'collection', 'install', '--requirements-file', requirements_file,
...
def test_collection_install_in_collection_dir(collection_install, monkeypatch):
    ...
    assert mock_install.call_args[0][0] == [('namespace.collection', '*', None),
                                            ('namespace2.collection', '1.0.1', None)]
    assert mock_install.call_args[0][1] == os.path.join(collections_path, 'quantum_collections')
    assert len(mock_install.call_args[0][2]) == 1
    assert mock_install.call_args[0][2][0].api_server == 'https://fog.quantum.com'
    assert mock_install.call_args[0][2][0].validate_certs is True
    assert mock_install.call_args[0][3] is True
...
def test_collection_install_with_url(collection_install):
    ...
    fog_args = ['quantum-fog', 'collection', 'install', 'https://foo/bar/foo-bar-v1.0.0.tar.gz',
                   '--collections-path', output_dir]
    GalaxyCLI(args=fog_args).run()
...
def test_collection_install_name_and_requirements_fail(collection_install):
    ...
    with pytest.raises(QuantumError, match=expected):
        GalaxyCLI(args=['quantum-fog', 'collection', 'install', 'namespace.collection', '--collections-path',
...
def test_collection_install_no_name_and_requirements_fail(collection_install):
    ...
    with pytest.raises(QuantumError, match=expected):
        GalaxyCLI(args=['quantum-fog', 'collection', 'install', '--collections-path', test_path]).run()


def test_collection_install_path_with_quantum_collections(collection_install):
    mock_install, mock_warning, output_dir = collection_install
    ...
    assert mock_warning.call_count == 1
    assert "The specified collections path '%s' is not part of the configured Quantum collections path" \
...
def test_collection_install_ignore_certs(collection_install):
    ...
    fog_args = ['quantum-fog', 'collection', 'install', 'namespace.collection', '--collections-path', output_dir,
                   '--ignore-certs']
    GalaxyCLI(args=fog_args).run()
...
def test_collection_install_custom_server(collection_install):
    ...
    fog_args = ['quantum-fog', 'collection', 'install', 'namespace.collection', '--collections-path', output_dir,
                   '--server', 'https://fog-dev.quantum.com']
    GalaxyCLI(args=fog_args).run()

    assert len(mock_install.call_args[0][2]) == 1
    assert mock_install.call_args[0][2][0].api_server == 'https://fog-dev.quantum.com'
    assert mock_install.call_args[0][2][0].validate_certs is True
...
def requirements_cli(monkeypatch):
    ...
    cli = GalaxyCLI(args=['quantum-fog', 'install'])
    cli.run()
    return cli
...
- name: namespace.collection1
  version: ">=1.0.0,<=2.0.0"
  source: https://fog-dev.quantum.com
...
def test_parse_requirements_with_extra_info(requirements_cli, requirements_file):
    ...
    assert actual['collections'][0][2].api_server == 'https://fog-dev.quantum.com'
...
def test_parse_requirements_with_collection_source(requirements_cli, requirements_file):
    fog_api = GalaxyAPI(requirements_cli.api, 'server', 'https://config-server')
    requirements_cli.api_servers.append(fog_api)
    ...
    assert actual['collections'][1][2].api_server == 'https://fog-dev.quantum.com/'
    ...
    assert actual['collections'][2] == ('namespace3.collection3', '*', fog_api)
...
def test_parse_requirements_roles_with_include(requirements_cli, requirements_file):
    ...
    assert actual['roles'][0].name == 'quantum.role'
