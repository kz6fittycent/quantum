...
from quantum.template import Templar, QuantumContext, QuantumEnvironment, QuantumUndefined
from quantum.utils.unsafe_proxy import QuantumUnsafe, wrap_var
...
class BaseTemplar(object):
    def setUp(self):
        ...
        self._quantum_context = QuantumContext(self.templar.environment, {}, {}, {})

    def is_unsafe(self, obj):
        return self._quantum_context._is_unsafe(obj)
...
class SomeUnsafeClass(QuantumUnsafe):
    def __init__(self):
...
class TestTemplarTemplate(BaseTemplar, unittest.TestCase):
    ...
    def test_is_possible_template(self):
        ...
        self.assertRaises(QuantumError, self.templar.template, '{{ foo|default(False)) }}')
    ...
    def test_template_convert_bare_unsafe(self):
        ...
        # self.assertIsInstance(res, QuantumUnsafe)
    ...
    @patch('quantum.template.safe_eval', side_effect=QuantumError)
    ...
    def test_template_with_error(self):
        """Check that QuantumError is raised, fail if an unhandled exception is raised"""
        self.assertRaises(QuantumError, self.templar.template, "{{ str_with_error }}")
...
class TestTemplarMisc(BaseTemplar, unittest.TestCase):
    def test_templar_simple(self):
        ...
        # force errors
        self.assertRaises(QuantumUndefinedVariable, templar.template, "{{bad_var}}")
        self.assertRaises(QuantumUndefinedVariable, templar.template, "{{lookup('file', bad_var)}}")
        self.assertRaises(QuantumError, templar.template, "{{lookup('bad_lookup')}}")
        self.assertRaises(QuantumError, templar.template, "{{recursive}}")
        self.assertRaises(QuantumUndefinedVariable, templar.template, "{{foo-bar}}")
...
class TestTemplarLookup(BaseTemplar, unittest.TestCase):
    ...
    def test_lookup_jinja_undefined(self):
        self.assertRaisesRegexp(QuantumUndefinedVariable,
    ...
    def test_lookup_jinja_list_wantlist_unsafe(self):
        ...
        for lookup_result in res:
            ...
            # self.assertIsInstance(lookup_result, QuantumUnsafe)

        # Should this be an QuantumUnsafe
        # self.assertIsInstance(res, QuantumUnsafe)
    ...
    def test_lookup_jinja_dict(self):
        ...
        # self.assertIsInstance(res['{{ a_keyword }}'], QuantumUnsafe)
        # self.assertIsInstance(res, QuantumUnsafe)
    ...
    def test_lookup_jinja_dict_unsafe(self):
        ...
        # self.assertIsInstance(res['{{ some_unsafe_key }}'], QuantumUnsafe)
        # TODO: Should this be an QuantumUnsafe
        # self.assertIsInstance(res, QuantumUnsafe)
...
class TestQuantumContext(BaseTemplar, unittest.TestCase):
    def _context(self, variables=None):
        variables = variables or {}

        env = QuantumEnvironment()
        context = QuantumContext(env, parent={}, name='some_context',
                                 blocks={})
    ...
    def test(self):
        ...
        self.assertIsInstance(context, QuantumContext)
    ...
    def test_resolve_unsafe(self):
        ...
        # self.assertIsInstance(res, QuantumUnsafe)
        self.assertTrue(self.is_unsafe(res),
                        'return of QuantumContext.resolve (%s) was expected to be marked unsafe but was not' % res)

    def test_resolve_unsafe_list(self):
        context = self._context(variables={'some_unsafe_key': [wrap_var('some unsafe string 1')]})
        res = context.resolve('some_unsafe_key')
        # self.assertIsInstance(res[0], QuantumUnsafe)
        self.assertTrue(self.is_unsafe(res),
                        'return of QuantumContext.resolve (%s) was expected to be marked unsafe but was not' % res)
    ...
    def test_resolve_unsafe_dict(self):
        ...
        self.assertTrue(self.is_unsafe(res['an_unsafe_dict']),
                        'return of QuantumContext.resolve (%s) was expected to be marked unsafe but was not' % res['an_unsafe_dict'])

    def test_resolve(self):
        context = self._context(variables={'some_key': 'some_string'})
        res = context.resolve('some_key')
        self.assertEqual(res, 'some_string')
        # self.assertNotIsInstance(res, QuantumUnsafe)
        self.assertFalse(self.is_unsafe(res),
                         'return of QuantumContext.resolve (%s) was not expected to be marked unsafe but was' % res)
    ...
    def test_is_unsafe(self):
        context = self._context()
        self.assertFalse(context._is_unsafe(QuantumUndefined()))
