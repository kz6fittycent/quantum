...
from quantum.executor.task_executor import TaskExecutor, remove_omit
from quantum.plugins.loader import action_loader, lookup_loader
from quantum.parsing.yaml.objects import QuantumUnicode
...
class TestTaskExecutor(unittest.TestCase):
    ...
    def test_task_executor_run(self):
        ...
        te._get_loop_items = MagicMock(side_effect=QuantumError(""))
        res = te.run()
    ...
    def test_task_executor_run_clean_res(self):
        ...
        te._run_loop = MagicMock(
            return_value=[
                {
                    'unsafe_bytes': QuantumUnsafeBytes(b'{{ $bar }}'),
                    'unsafe_text': QuantumUnsafeText(u'{{ $bar }}'),
                    'bytes': b'bytes',
                    'text': u'text',
                    'int': 1,
                }
        ...
        res = te.run()
        data = res['results'][0]
        self.assertIsInstance(data['unsafe_bytes'], QuantumUnsafeText)
        self.assertIsInstance(data['unsafe_text'], QuantumUnsafeText)
    ...
    def test_task_executor_execute(self):
        ...
        mock_action.run.return_value = dict(quantum_facts=dict())
        res = te._execute()

        mock_task.changed_when = MagicMock(return_value=QuantumUnicode("1 == 1"))
        res = te._execute()

        mock_task.changed_when = None
        mock_task.failed_when = MagicMock(return_value=QuantumUnicode("1 == 1"))
        res = te._execute()
    ...
    def test_task_executor_poll_async_result(self):
        ...
        with patch.object(action_loader, 'get', _get):
            ...
            self.assertIn('failed', res)
            res = te._poll_async_result(result=dict(quantum_job_id=1), templar=mock_templar)
            self.assertIn('failed', res)
