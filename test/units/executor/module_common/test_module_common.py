...
from quantum.executor import module_common as amc
...
class TestSlurp:
    def test_slurp_nonexistent(self, mocker):
        ...
        with pytest.raises(quantum.errors.QuantumError):
            amc._slurp('no_file')
...
class TestGetShebang:
    ...
    def test_interpreter_set_in_task_vars(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'quantum_python_interpreter': u'/usr/bin/pypy'}, templar) == \
            (u'#!/usr/bin/pypy', u'/usr/bin/pypy')

    def test_non_python_interpreter_in_task_vars(self, templar):
        assert amc._get_shebang(u'/usr/bin/ruby', {u'quantum_ruby_interpreter': u'/usr/local/bin/ruby'}, templar) == \
            (u'#!/usr/local/bin/ruby', u'/usr/local/bin/ruby')

    def test_with_args(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'quantum_python_interpreter': u'/usr/bin/python3'}, templar, args=('-tt', '-OO')) == \
            (u'#!/usr/bin/python3 -tt -OO', u'/usr/bin/python3')

    def test_python_via_env(self, templar):
        assert amc._get_shebang(u'/usr/bin/python', {u'quantum_python_interpreter': u'/usr/bin/env python'}, templar) == \
...
class TestDetectionRegexes:
    ANSIBLE_MODULE_UTIL_STRINGS = (
        ...
        b'import quantum_collections.my_ns.my_col.plugins.module_utils.my_util',
        b'from quantum_collections.my_ns.my_col.plugins.module_utils import my_util',
        b'from quantum_collections.my_ns.my_col.plugins.module_utils.my_util import my_func',
        # Absolute core imports
        b'import quantum.module_utils.basic',
        b'from quantum.module_utils import basic',
        b'from quantum.module_utils.basic import QuantumModule',
        # Relative imports
        b'from ..module_utils import basic',
        b'from .. module_utils import basic',
        b'from ....module_utils import basic',
        b'from ..module_utils.basic import QuantumModule',
    )
    NOT_ANSIBLE_MODULE_UTIL_STRINGS = (
        b'from quantum import release',
        b'from ..release import __version__',
        b'from .. import release',
        b'from quantum.modules.system import ping',
        b'from quantum_collecitons.my_ns.my_col.plugins.modules import function',
    )

    OFFSET = os.path.dirname(os.path.dirname(amc.__file__))
    CORE_PATHS = (
        ('%s/modules/from_role.py' % OFFSET, 'quantum/modules/from_role'),
        ('%s/modules/system/ping.py' % OFFSET, 'quantum/modules/system/ping'),
        ('%s/modules/cloud/amazon/s3.py' % OFFSET, 'quantum/modules/cloud/amazon/s3'),
    )

    COLLECTION_PATHS = (
        ('/root/quantum_collections/ns/col/plugins/modules/ping.py',
         'quantum_collections/ns/col/plugins/modules/ping'),
        ('/root/quantum_collections/ns/col/plugins/modules/subdir/ping.py',
         'quantum_collections/ns/col/plugins/modules/subdir/ping'),
    )
