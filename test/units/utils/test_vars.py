...
# (c) 2015, Toshio Kuraotmi <tkuratomi@quantum.com>
#
...
from quantum.utils.vars import combine_vars, merge_hash
...
class TestVariableUtils(unittest.TestCase):
    ...
    def test_improper_args(self):
        with mock.patch('quantum.constants.DEFAULT_HASH_BEHAVIOUR', 'replace'):
            with self.assertRaises(QuantumError):
                combine_vars([1, 2, 3], dict(a=1))
            with self.assertRaises(QuantumError):
                combine_vars(dict(a=1), [1, 2, 3])

        with mock.patch('quantum.constants.DEFAULT_HASH_BEHAVIOUR', 'merge'):
            with self.assertRaises(QuantumError):
                combine_vars([1, 2, 3], dict(a=1))
            with self.assertRaises(QuantumError):
                combine_vars(dict(a=1), [1, 2, 3])
