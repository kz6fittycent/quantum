...
from quantum.plugins.filter.core import get_encrypted_password
from quantum.utils import encrypt
...
def assert_hash(expected, secret, algorithm, **settings):
    ...
    if encrypt.PASSLIB_AVAILABLE:
        ...
        assert encrypt.passlib_or_crypt(secret, algorithm, **settings) == expected
        with pytest.raises(QuantumError) as excinfo:
...
def test_password_hash_filter_no_passlib():
    with passlib_off():
        ...
        with pytest.raises(QuantumFilterError):
