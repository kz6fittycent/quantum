...
class TestErrors(unittest.TestCase):
    ...
    def test_error_with_kv(self, mock_method):
        ...
        self.obj.quantum_pos = ('foo.yml', 2, 1)

        mock_method.return_value = ['    line: foo\n', '- lineinfile: line=foo path=bar\n']

        e = QuantumError(self.message, self.obj)
        self.assertEqual(
            e.message,
    ...
    def test_error_with_object(self, mock_method):
        self.obj.quantum_pos = ('foo.yml', 1, 1)
    ...
    def test_get_error_lines_from_file(self):
        ...
        with patch('{0}.open'.format(BUILTINS), m):
            ...
            with patch('quantum.errors.to_text', side_effect=IndexError('Raised intentionally')):
                # raise an IndexError
                self.obj.quantum_pos = ('foo.yml', 2, 1)
                e = QuantumError(self.message, self.obj)
                self.assertEqual(
        ...
        with patch('{0}.open'.format(BUILTINS), m):
            # this line will be found in the file
            self.obj.quantum_pos = ('foo.yml', 1, 1)
            e = QuantumError(self.unicode_message, self.obj)
            self.assertEqual(
                e.message,
    ...
    def test_get_error_lines_error_in_last_line(self):
        ...
        with patch('{0}.open'.format(BUILTINS), m):
            ...
            self.obj.quantum_pos = ('foo.yml', 4, 1)
            e = QuantumError(self.message, self.obj)
            self.assertEqual(
                e.message,
    ...
    def test_get_error_lines_error_empty_lines_around_error(self):
        ...
        with patch('{0}.open'.format(BUILTINS), m):
            self.obj.quantum_pos = ('foo.yml', 5, 1)
            e = QuantumError(self.message, self.obj)
            self.assertEqual(
                e.message,
