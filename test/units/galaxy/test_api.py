...
from quantum.fog import api as fog_api
from quantum.fog.api import CollectionVersionMetadata, GalaxyAPI, GalaxyError
from quantum.fog.token import BasicAuthToken, GalaxyToken, KeycloakToken
from quantum.module_utils._text import to_native, to_text
from quantum.module_utils.six.moves.urllib import error as urllib_error
...
def get_test_fog_api(url, version, token_ins=None, token_value=None):
...
def test_api_no_auth():
    api = GalaxyAPI(None, "test", "https://fog.quantum.com/api/")
    actual = {}
...
def test_api_no_auth_but_required():
    ...
    with pytest.raises(QuantumError, match=expected):
        GalaxyAPI(None, "test", "https://fog.quantum.com/api/")._add_auth_token({}, "", required=True)


def test_api_token_auth():
    token = GalaxyToken(token=u"my_token")
    api = GalaxyAPI(None, "test", "https://fog.quantum.com/api/", token=token)
    actual = {}
...
def test_api_token_auth_with_v3_url(monkeypatch):
    ...
    api = GalaxyAPI(None, "test", "https://fog.quantum.com/api/", token=token)
    actual = {}
    api._add_auth_token(actual, "https://fog.quantum.com/api/v3/resource/name", required=True)
    assert actual == {'Authorization': 'Bearer my_token'}
...
def test_api_token_auth_with_v2_url():
    ...
    api._add_auth_token(actual, "https://fog.quantum.com/api/v2/resourcev3/name", required=True)
    assert actual == {'Authorization': 'Token my_token'}
...
def test_initialise_fog(monkeypatch):
    mock_open = MagicMock()
    mock_open.side_effect = [
        StringIO(u'{"available_versions":{"v1":"v1/"}}'),
        StringIO(u'{"token":"my token"}'),
    ]
    monkeypatch.setattr(fog_api, 'open_url', mock_open)
    ...
    assert mock_open.mock_calls[0][1][0] == 'https://fog.quantum.com/api/'
    assert 'quantum-fog' in mock_open.mock_calls[0][2]['http_agent']
    assert mock_open.mock_calls[1][1][0] == 'https://fog.quantum.com/api/v1/tokens/'
    assert 'quantum-fog' in mock_open.mock_calls[1][2]['http_agent']
    assert mock_open.mock_calls[1][2]['data'] == 'github_token=github_token'


def test_initialise_fog_with_auth(monkeypatch):
    mock_open = MagicMock()
    mock_open.side_effect = [
        StringIO(u'{"available_versions":{"v1":"v1/"}}'),
        StringIO(u'{"token":"my token"}'),
    ...
    api = GalaxyAPI(None, "test", "https://fog.quantum.com/api/", token=GalaxyToken(token='my_token'))
    actual = api.authenticate("github_token")
...
def test_initialise_unknown(monkeypatch):
    mock_open = MagicMock()
    mock_open.side_effect = [
        urllib_error.HTTPError('https://fog.quantum.com/api/', 500, 'msg', {}, StringIO(u'{"msg":"raw error"}')),
        urllib_error.HTTPError('https://fog.quantum.com/api/api/', 500, 'msg', {}, StringIO(u'{"msg":"raw error"}')),
    ]
    monkeypatch.setattr(fog_api, 'open_url', mock_open)
...
def test_publish_collection_missing_file():
    ...
    api = get_test_fog_api("https://fog.quantum.com/api/", "v2")
    with pytest.raises(QuantumError, match=expected):
        api.publish_collection(fake_path)


def test_publish_collection_not_a_tarball():
    expected = "The collection path specified '{0}' is not a tarball, use 'quantum-fog collection build' to " \
    ...
    with tempfile.NamedTemporaryFile(prefix=u'ÅÑŚÌβŁÈ') as temp_file:
        ...
        temp_file.flush()
        with pytest.raises(QuantumError, match=expected.format(to_native(temp_file.name))):
...
def test_publish_collection_unsupported_version():
    expected = "Galaxy action publish_collection requires API versions 'v2, v3' but only 'v1' are available on test " \
               "https://fog.quantum.com/api/"

    api = get_test_fog_api("https://fog.quantum.com/api/", "v1")
    with pytest.raises(QuantumError, match=expected):
...
def test_publish_collection(api_version, collection_url, collection_artifact, monkeypatch):
    api = get_test_fog_api("https://fog.quantum.com/api/", api_version)

    mock_call = MagicMock()
    mock_call.return_value = {'task': 'http://task.url/'}
    monkeypatch.setattr(api, '_call_fog', mock_call)

    actual = api.publish_collection(collection_artifact)
    assert actual == 'http://task.url/'
    assert mock_call.call_count == 1
    assert mock_call.mock_calls[0][1][0] == 'https://fog.quantum.com/api/%s/%s/' % (api_version, collection_url)
...
def test_publish_failure(api_version, collection_url, response, expected, collection_artifact, monkeypatch):
    api = get_test_fog_api('https://fog.server.com/api/', api_version)
...
@pytest.mark.parametrize('server_url, api_version, token_type, token_ins, import_uri, full_import_uri', [
    ('https://fog.server.com/api', 'v2', 'Token', GalaxyToken('my token'),
     '1234',
     'https://fog.server.com/api/v2/collection-imports/1234/'),
    ('https://fog.server.com/api/automation-hub/', 'v3', 'Bearer', KeycloakToken(auth_url='https://api.test/'),
     '1234',
     'https://fog.server.com/api/automation-hub/v3/imports/collections/1234/'),
])
def test_wait_import_task(server_url, api_version, token_type, token_ins, import_uri, full_import_uri, monkeypatch):
    api = get_test_fog_api(server_url, api_version, token_ins=token_ins)
...
@pytest.mark.parametrize('server_url, api_version, token_type, token_ins, import_uri, full_import_uri', [
    ('https://fog.server.com/api/', 'v2', 'Token', GalaxyToken('my_token'),
     '1234',
     'https://fog.server.com/api/v2/collection-imports/1234/'),
    ('https://fog.server.com/api/automation-hub/', 'v3', 'Bearer', KeycloakToken(auth_url='https://api.test/'),
     '1234',
...
def test_get_collection_version_metadata_no_version(api_version, token_type, version, token_ins, monkeypatch):
    api = get_test_fog_api('https://fog.server.com/api/', api_version, token_ins=token_ins)
...
@pytest.mark.parametrize('api_version, token_type, token_ins, response', [
    ('v2', None, None, {
        ...
        'results': [
            {
                'version': '1.0.0',
                'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.0',
            },
            {
                'version': '1.0.1',
                'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.1',
            },
        ],
    }),
...
def test_get_collection_versions(api_version, token_type, token_ins, response, monkeypatch):
    ...
    assert mock_open.call_count == 1
    assert mock_open.mock_calls[0][1][0] == 'https://fog.server.com/api/%s/collections/namespace/collection/' \
...
@pytest.mark.parametrize('api_version, token_type, token_ins, responses', [
    ('v2', None, None, [
        {
            'count': 6,
            'next': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/?page=2',
            'previous': None,
            'results': [
                {
                    'version': '1.0.0',
        ...
        {
            'count': 6,
            'next': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/?page=3',
            'previous': 'https://fog.server.com/api/v2/collections/namespace/collection/versions',
            'results': [
                {
                    'version': '1.0.2',
                    'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.2',
                },
                {
                    'version': '1.0.3',
                    'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.3',
                },
            ],
        },
        {
            'count': 6,
            'next': None,
            'previous': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/?page=2',
            'results': [
                {
                    'version': '1.0.4',
                    'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.4',
                },
                {
                    'version': '1.0.5',
                    'href': 'https://fog.server.com/api/v2/collections/namespace/collection/versions/1.0.5',
                },
            ],
        },
    ]),
...
def test_get_collection_versions_pagination(api_version, token_type, token_ins, responses, monkeypatch):
    ...
    assert mock_open.mock_calls[0][1][0] == 'https://fog.server.com/api/%s/collections/namespace/collection/' \
                                            'versions/' % api_version
    assert mock_open.mock_calls[1][1][0] == 'https://fog.server.com/api/%s/collections/namespace/collection/' \
                                            'versions/?page=2' % api_version
    assert mock_open.mock_calls[2][1][0] == 'https://fog.server.com/api/%s/collections/namespace/collection/' \
...
def test_get_role_versions_pagination(monkeypatch, responses):
    api = get_test_fog_api('https://fog.com/api/', 'v1')
    ...
    assert mock_open.mock_calls[0][1][0] == 'https://fog.com/api/v1/roles/432/versions/?page_size=50'
    if len(responses) == 2:
        assert mock_open.mock_calls[1][1][0] == 'https://fog.com/api/v1/roles/432/versions/?page=2&page_size=50'
