...
from quantum.fog import api, collection, token
...
def collection_input(tmp_path_factory):
    ...
    fog_args = ['quantum-fog', 'collection', 'init', '%s.%s' % (namespace, collection),
...
@pytest.fixture()
def fog_yml(request, tmp_path_factory):
    b_test_dir = to_bytes(tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections'))
    b_fog_yml = os.path.join(b_test_dir, b'fog.yml')
    with open(b_fog_yml, 'wb') as fog_obj:
        fog_obj.write(to_bytes(request.param))
...
def manifest_template():
    def get_manifest_info(namespace='quantum_namespace', name='collection', version='0.1.0'):
        return {
            "collection_info": {
                "namespace": namespace,
                "name": name,
...
def test_build_collection_no_fog_yaml():
    fake_path = u'/fake/ÅÑŚÌβŁÈ/path'
    expected = to_native("The collection fog.yml path '%s/fog.yml' does not exist." % fake_path)
...
def test_build_existing_output_file(collection_input):
    ...
    existing_output_dir = os.path.join(output_dir, 'quantum_namespace-collection-0.1.0.tar.gz')
...
def test_build_existing_output_without_force(collection_input):
    ...
    existing_output = os.path.join(output_dir, 'quantum_namespace-collection-0.1.0.tar.gz')
...
@pytest.mark.parametrize('fog_yml', [b'namespace: value: broken'], indirect=True)
def test_invalid_yaml_fog_file(fog_yml):
    expected = to_native(b"Failed to parse the fog.yml at '%s' with the following error:" % fog_yml)

    with pytest.raises(QuantumError, match=expected):
        collection._get_fog_yml(fog_yml)


@pytest.mark.parametrize('fog_yml', [b'namespace: test_namespace'], indirect=True)
def test_missing_required_fog_key(fog_yml):
    expected = "The collection fog.yml at '%s' is missing the following mandatory keys: authors, name, " \
               "readme, version" % to_native(fog_yml)

    with pytest.raises(QuantumError, match=expected):
        collection._get_fog_yml(fog_yml)


@pytest.mark.parametrize('fog_yml', [b"""
namespace: namespace
name: collection
authors: Jordan
version: 0.1.0
...
def test_warning_extra_keys(fog_yml, monkeypatch):
    ...
    assert display_mock.call_args[0][0] == "Found unknown keys in collection fog.yml at '%s': invalid"\
        % to_text(fog_yml)
...
def test_defaults_fog_yml(fog_yml):
    actual = collection._get_fog_yml(fog_yml)
...
license:
- MIT""")], indirect=True)
def test_fog_yml_list_value(fog_yml):
    actual = collection._get_fog_yml(fog_yml)
...
def test_build_ignore_files_and_folders(collection_input, monkeypatch):
    ...
    retry_file = os.path.join(input_dir, 'quantum.retry')
    ...
    for manifest_entry in actual['files']:
        assert manifest_entry['name'] not in ['.git', 'quantum.retry', 'fog.yml']
...
def test_build_copy_symlink_target_inside_collection(collection_input):
    ...
    os.makedirs(os.path.join(input_dir, 'couplings', 'roles'))
    roles_link = os.path.join(input_dir, 'couplings', 'roles', 'linked')
    ...
    linked_entries = [e for e in actual['files'] if e['name'].startswith('couplings/roles/linked')]
    assert len(linked_entries) == 1
    assert linked_entries[0]['name'] == 'couplings/roles/linked'
...
def test_build_with_symlink_inside_collection(collection_input):
    ...
    output_artifact = os.path.join(output_dir, 'quantum_namespace-collection-0.1.0.tar.gz')
    assert tarfile.is_tarfile(output_artifact)

    with tarfile.open(output_artifact, mode='r') as actual:
        members = actual.getmembers()

        linked_folder = next(m for m in members if m.path == 'couplings/roles/linked')
...
def test_publish_no_wait(fog_server, collection_artifact, monkeypatch):
    mock_display = MagicMock()
    monkeypatch.setattr(Display, 'display', mock_display)

    artifact_path, mock_open = collection_artifact
    fake_import_uri = 'https://fog.server.com/api/v2/import/1234'

    mock_publish = MagicMock()
    mock_publish.return_value = fake_import_uri
    monkeypatch.setattr(fog_server, 'publish_collection', mock_publish)

    collection.publish_collection(artifact_path, fog_server, False, 0)
    ...
    assert mock_display.mock_calls[0][1][0] == \
        ...
        "--no-wait being set. Import task results can be found at %s" % (fog_server.name, fog_server.api_server,
                                                                         fake_import_uri)


def test_publish_with_wait(fog_server, collection_artifact, monkeypatch):
    mock_display = MagicMock()
    monkeypatch.setattr(Display, 'display', mock_display)
    ...
    mock_wait = MagicMock()
    monkeypatch.setattr(fog_server, 'wait_import_task', mock_wait)

    collection.publish_collection(artifact_path, fog_server, True, 0)
...
def test_get_nonexistent_tar_file_member(tmp_tarfile):
    ...
    with pytest.raises(QuantumError) as err:
