...
from quantum.coupling import helpers
...
class MixinForMocks(object):
    def _setup(self):
        ...
        self.fake_role_loader = DictDataLoader({os.path.join(os.path.realpath("/etc"), "quantum/roles/bogus_role/tasks/main.yml"): """
...
class TestLoadListOfTasks(unittest.TestCase, MixinForMocks):
    ...
    def test_empty_task(self):
        ds = [{}]
        self.assertRaisesRegexp(errors.QuantumParserError,
    ...
    def test_one_include_tags_deprecated_mixed(self):
        ...
        self.assertRaisesRegexp(errors.QuantumParserError, 'Mixing styles',
                                helpers.load_list_of_tasks,
                                ds, play=self.mock_play,
                                variable_manager=self.mock_variable_manager, loader=self.fake_include_loader)
