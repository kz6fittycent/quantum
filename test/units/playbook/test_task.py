...
class TestTask(unittest.TestCase):
    ...
    @patch.object(errors.QuantumError, '_get_error_lines_from_file')
    def test_load_task_kv_form_error_36848(self, mock_get_err_lines):
        ds = objects.QuantumMapping(kv_bad_args_ds)
        ds.quantum_pos = ('test_task_faux_coupling.yml', 1, 1)
        mock_get_err_lines.return_value = (kv_bad_args_str, '')

        with self.assertRaises(errors.QuantumParserError) as cm:
            Task.load(ds)

        self.assertIsInstance(cm.exception, errors.QuantumParserError)
        self.assertEqual(cm.exception._obj, ds)
        self.assertEqual(cm.exception._obj, kv_bad_args_ds)
        self.assertIn("The error appears to be in 'test_task_faux_coupling.yml", cm.exception.message)
