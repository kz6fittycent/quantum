...
from quantum.modules.cloud.amazon.s3_bucket_notification import AmazonBucket, Config
from quantum.modules.cloud.amazon import s3_bucket_notification
try:
...
class TestModule(ModuleTestCase):
    def test_module_fail_when_required_args_missing(self):
        with pytest.raises(QuantumFailJson):
            set_module_args({})
            s3_bucket_notification.main()

    @patch('quantum.modules.cloud.amazon.s3_bucket_notification.QuantumAWSModule.client')
    def test_add_s3_bucket_notification(self, aws_client):
        ...
        with pytest.raises(QuantumExitJson) as context:
            s3_bucket_notification.main()
