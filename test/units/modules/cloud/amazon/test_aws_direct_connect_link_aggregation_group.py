...
from quantum.modules.cloud.amazon import aws_direct_connect_link_aggregation_group as lag_module
from quantum.module_utils.ec2 import get_aws_connection_info, boto3_conn
...
def dependencies():
    ...
    vanilla_params = {"name": "quantum_lag_1",
                      "location": "EqSe2",
                      "num_connections": 1,
                      "min_links": 0,
                      "bandwidth": "1Gbps"}

    for lag in ("quantum_lag_1", "quantum_lag_2"):
        params = dict(vanilla_params)
        params["name"] = lag
        if lag == "quantum_lag_1":
    ...
    if os.getenv("PLACEBO_RECORD"):
        ...
        lag_1.update(lag_id="dxlag-fgkk4dja", name="quantum_lag_1")
        lag_2.update(lag_id="dxlag-fgytkicv", name="quantum_lag_2")
        yield Dependencies(lag_1=lag_1, lag_2=lag_2)
...
def test_lag_status(placeboify, maybe_sleep, dependencies):
    ...
    assert status.get("lagName") == "quantum_lag_1"
...
def test_lag_changed_true_no(placeboify, maybe_sleep, dependencies):
    client = placeboify.client("directconnect")
    status = lag_module.lag_status(client=client, lag_id=dependencies.lag_1.get("lag_id"))
    assert not lag_module.lag_changed(status, "quantum_lag_1", 0)
...
def test_delete_lag_with_connections_without_force_delete(placeboify, maybe_sleep, dependencies):
    ...
    with pytest.raises(Exception) as error_message:
        lag_module.ensure_absent(client, dependencies.lag_1.get("lag_id"), "quantum_lag_1", False, True, True, 120)
...
def test_delete_lag_with_connections(placeboify, maybe_sleep, dependencies):
    client = placeboify.client("directconnect")
    changed = lag_module.ensure_absent(client, dependencies.lag_1.get("lag_id"), "quantum_lag_1", True, True, True, 120)
    assert changed
