...
from quantum.modules.cloud.amazon import ec2_vpc_vpn
from quantum.module_utils.ec2 import get_aws_connection_info, boto3_conn, boto3_tag_list_to_quantum_dict
...
def get_vgw(connection):
    ...
    vgw = connection.describe_vpn_gateways(Filters=[{'Name': 'tag:Quantum_VPN', 'Values': ['Test']}])
    ...
    for resource in (vgw_1, vgw_2):
        connection.create_tags(Resources=[resource['VpnGateway']['VpnGatewayId']], Tags=[{'Key': 'Quantum_VPN', 'Value': 'Test'}])
...
def get_cgw(connection):
    ...
    cgw = connection.describe_customer_gateways(DryRun=False, Filters=[{'Name': 'state', 'Values': ['available']},
                                                                       {'Name': 'tag:Name', 'Values': ['Quantum-CGW']}])
    ...
    for resource in (cgw_1, cgw_2):
        connection.create_tags(Resources=[resource['CustomerGateway']['CustomerGatewayId']], Tags=[{'Key': 'Quantum-CGW', 'Value': 'Test'}])
...
def test_check_for_update_tags(placeboify, maybe_sleep):
    ...
    flat_dict_changes = boto3_tag_list_to_quantum_dict(changes['tags_to_add'])
    correct_changes = boto3_tag_list_to_quantum_dict([{'Key': 'Three', 'Value': 'three'}, {'Key': 'Four', 'Value': 'four'}])
...
def test_add_tags(placeboify, maybe_sleep):
    ...
    ec2_vpc_vpn.add_tags(conn, vpn['VpnConnectionId'], add=[{'Key': 'Quantum-Test', 'Value': 'VPN'}])

    # assert tag is there
    current_vpn = ec2_vpc_vpn.find_connection(conn, params)
    assert current_vpn['Tags'] == [{'Key': 'Quantum-Test', 'Value': 'VPN'}]
...
def test_remove_tags(placeboify, maybe_sleep):
    ...
    ec2_vpc_vpn.remove_tags(conn, vpn['VpnConnectionId'], remove=['Quantum-Test'])
