...
from quantum.modules.cloud.amazon import data_pipeline
...
def dp_setup():
    ...
    if not os.getenv('PLACEBO_RECORD'):
        ...
        s3 = boto3.client('s3')
        data = s3.get_object(Bucket="quantum-test-datapipeline", Key="pipeline-object/new.json")
        objects = json.loads(to_text(data['Body'].read()))

    # create a module with vanilla data pipeline parameters
    params = {'name': 'quantum-test-create-pipeline',
              'description': 'quantum-datapipeline-unit-test',
              'state': 'present',
              'timeout': 300,
              'objects': [],
              'tags': {},
...
def test_create_pipeline_already_exists(placeboify, maybe_sleep, dp_setup):
    ...
    assert "Data Pipeline quantum-test-create-pipeline is present" in result['msg']
...
def test_deactivate_pipeline(placeboify, maybe_sleep, dp_setup):
    ...
    assert "Data Pipeline quantum-test-create-pipeline deactivated" in result['msg']
...
def test_create_pipeline(placeboify, maybe_sleep):
    connection = placeboify.client('datapipeline')
    params = {'name': 'quantum-unittest-create-pipeline',
              'description': 'quantum-datapipeline-unit-test',
              'state': 'present',
              'timeout': 300,
              'tags': {}}
    ...
    assert result['msg'] == "Data Pipeline quantum-unittest-create-pipeline created."
...
def test_create_pipeline_with_tags(placeboify, maybe_sleep):
    connection = placeboify.client('datapipeline')
    params = {'name': 'quantum-unittest-create-pipeline_tags',
              'description': 'quantum-datapipeline-unit-test',
              'state': 'present',
              'tags': {'quantum': 'test'},
              'timeout': 300}
    ...
    changed, result = data_pipeline.create_pipeline(connection, m)
    assert changed is True
    assert result['msg'] == "Data Pipeline quantum-unittest-create-pipeline_tags created."
...
def test_delete_nonexistent_pipeline(placeboify, maybe_sleep):
    connection = placeboify.client('datapipeline')
    params = {'name': 'quantum-test-nonexistent',
              'description': 'quantum-test-nonexistent',
              'state': 'absent',
              'objects': [],
              'tags': {'quantum': 'test'},
              'timeout': 300}
...
def test_build_unique_id_different():
    m = FakeModule(**{'name': 'quantum-unittest-1', 'description': 'test-unique-id'})
    m2 = FakeModule(**{'name': 'quantum-unittest-1', 'description': 'test-unique-id-different'})
    assert data_pipeline.build_unique_id(m) != data_pipeline.build_unique_id(m2)


def test_build_unique_id_same():
    m = FakeModule(**{'name': 'quantum-unittest-1', 'description': 'test-unique-id', 'tags': {'quantum': 'test'}})
    m2 = FakeModule(**{'name': 'quantum-unittest-1', 'description': 'test-unique-id', 'tags': {'quantum': 'test'}})
    assert data_pipeline.build_unique_id(m) == data_pipeline.build_unique_id(m2)
...
def test_build_unique_id_obj():
    ...
    m = FakeModule(**{'name': 'quantum-unittest-1', 'objects': [{'first': 'object'}]})
    m2 = FakeModule(**{'name': 'quantum-unittest-1', 'objects': [{'second': 'object'}]})
    assert data_pipeline.build_unique_id(m) == data_pipeline.build_unique_id(m2)
