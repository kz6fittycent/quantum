...
from quantum.modules.messaging.rabbitmq import rabbitmq_user
...
class TestRabbitMQUserModule(ModuleTestCase):
    ...
    def test_permissions_with_same_vhost(self):
        ...
        with patch('quantum.module_utils.basic.QuantumModule.get_bin_path') as get_bin_path:
            get_bin_path.return_value = '/rabbitmqctl'
            try:
                self.module.main()
            except QuantumFailJson as e:
                self._assert(e, 'failed', True)
                self._assert(e, 'msg',
                             "Error parsing permissions: You can't have two permission dicts for the same vhost")

    @patch('quantum.module_utils.basic.QuantumModule.get_bin_path')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.get')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.check_password')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.has_tags_modifications')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.has_permissions_modifications')
    def test_password_changes_only_when_needed(self, has_permissions_modifications, has_tags_modifications,
        ...
        try:
            self.module.main()
        except QuantumExitJson as e:
            self._assert(e, 'changed', False)
            self._assert(e, 'state', 'present')

    @patch('quantum.module_utils.basic.QuantumModule.get_bin_path')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser._exec')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser._get_permissions')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.has_tags_modifications')
    ...
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser._exec')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser._get_permissions')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.set_permissions')
    @patch('quantum.modules.messaging.rabbitmq.rabbitmq_user.RabbitMqUser.has_tags_modifications')
