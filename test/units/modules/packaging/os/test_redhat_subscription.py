...
from quantum.modules.packaging.os import redhat_subscription
...
def patch_redhat_subscription(mocker):
    ...
    """
    mocker.patch('quantum.modules.packaging.os.redhat_subscription.RegistrationBase.REDHAT_REPO')
    mocker.patch('quantum.modules.packaging.os.redhat_subscription.isfile', return_value=False)
    mocker.patch('quantum.modules.packaging.os.redhat_subscription.unlink', return_value=True)
    mocker.patch('quantum.modules.packaging.os.redhat_subscription.QuantumModule.get_bin_path',
...
@pytest.mark.parametrize('patch_quantum_module, testcase', TEST_CASES, ids=TEST_CASES_IDS, indirect=['patch_quantum_module'])
@pytest.mark.usefixtures('patch_quantum_module')
def test_redhat_subscribtion(mocker, capfd, patch_redhat_subscription, testcase):
    ...
    assert basic.QuantumModule.run_command.call_count == len(testcase['run_command.calls'])
    if basic.QuantumModule.run_command.call_count:
        call_args_list = [(item[0][0], item[1]) for item in basic.QuantumModule.run_command.call_args_list]
...
@pytest.mark.parametrize('patch_quantum_module, testcase', SYSPURPOSE_TEST_CASES, ids=SYSPURPOSE_TEST_CASES_IDS, indirect=['patch_quantum_module'])
@pytest.mark.usefixtures('patch_quantum_module')
def test_redhat_subscribtion_syspurpose(mocker, capfd, patch_redhat_subscription, patch_quantum_module, testcase, tmpdir):
    """
