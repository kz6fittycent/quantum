...
from quantum.module_utils.six.moves import xmlrpc_client
from quantum.modules.packaging.os import rhn_register
...
def import_libxml(mocker):
    ...
    if quantum.module_utils.six.PY3:
...
def test_systemid_with_requirements(capfd, mocker, patch_rhn):
    ...
    mocker.patch('quantum.modules.packaging.os.rhn_register.open', mock_open(read_data=SYSTEMID), create=True)
...
def test_register_parameters(mocker, capfd, mock_request, patch_rhn, testcase):
    ...
    mocker.patch.object(basic.QuantumModule, 'run_command', return_value=(0, '', ''))
    ...
    assert basic.QuantumModule.run_command.call_count == testcase['run_command.call_count']
    if basic.QuantumModule.run_command.call_count:
        assert basic.QuantumModule.run_command.call_args[0][0][0] == testcase['run_command.call_args']
