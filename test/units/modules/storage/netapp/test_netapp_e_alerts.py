...
from quantum.modules.storage.netapp.netapp_e_alerts import Alerts
...
class AlertsTest(ModuleTestCase):
    ...
    REQ_FUNC = 'quantum.modules.storage.netapp.netapp_e_alerts.request'
    ...
    def test_send_test_email_fail(self):
        ...
        ret_msg = 'fail'
        with self.assertRaisesRegexp(QuantumFailJson, ret_msg):
    ...
    def test_send_test_email_fail_connection(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"failed to send"):
    ...
    def test_update(self):
        ...
        alerts = Alerts()
        with self.assertRaisesRegexp(QuantumExitJson, r"enabled"):
            with mock.patch.object(alerts, 'update_configuration', return_value=True):
        ...
        alerts = Alerts()
        with self.assertRaisesRegexp(QuantumExitJson, r"disabled"):
            with mock.patch.object(alerts, 'update_configuration', return_value=True):
                with mock.patch.object(alerts, 'send_test_email') as test:
                    alerts.update()
                    self.assertFalse(test.called)
