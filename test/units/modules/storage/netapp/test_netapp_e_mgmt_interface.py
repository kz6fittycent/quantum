...
from quantum.modules.storage.netapp.netapp_e_mgmt_interface import MgmtInterface
...
class MgmtInterfaceTest(ModuleTestCase):
    ...
    REQ_FUNC = 'quantum.modules.storage.netapp.netapp_e_mgmt_interface.request'
    ...
    def test_controller_property_fail(self):
        """Verify controllers endpoint request failure causes QuantumFailJson exception."""
        initial = {
            "state": "enable",
            "controller": "A",
            "channel": "1",
        ...
        self._set_args(initial)
        mgmt_interface = MgmtInterface()
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to retrieve the controller settings."):
    ...
    def test_interface_property_request_exception_fail(self):
        """Verify ethernet-interfaces endpoint request failure results in QuantumFailJson exception."""
        initial = {
            "state": "enable",
            "controller": "A",
            "channel": "1",
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to retrieve defined management interfaces."):
    ...
    def test_interface_property_no_match_fail(self):
        ...
        self._set_args(initial)
        mgmt_interface = MgmtInterface()
        with self.assertRaisesRegexp(QuantumFailJson, r"We could not find an interface matching"):
            with mock.patch(self.REQ_FUNC, side_effect=[(200, self.TEST_DATA), (200, controller_request)]):
                iface = mgmt_interface.interface
    ...
    def test_update_array_ipv4_ipv6_disabled_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"This storage-system already has IPv6 connectivity disabled."):
    ...
    def test_update_array_request_error_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"We failed to configure the management interface."):
    ...
    def test_update_pass(self):
        ...
        with self.assertRaisesRegexp(QuantumExitJson, r"The interface settings have been updated."):
