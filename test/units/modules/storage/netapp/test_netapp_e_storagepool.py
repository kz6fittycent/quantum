...
from quantum.modules.storage.netapp.netapp_e_storagepool import NetAppESeriesStoragePool
...
class StoragePoolTest(ModuleTestCase):
    ...
    REQUEST_FUNC = "quantum.modules.storage.netapp.netapp_e_storagepool.request"
    NETAPP_REQUEST_FUNC = "quantum.module_utils.netapp.NetAppESeriesModule.request"
    VALIDATE_FUNC = "quantum.modules.storage.netapp.netapp_e_storagepool.NetAppESeriesModule.validate_instance"

    DRIVES_PROPERTY = "quantum.modules.storage.netapp.netapp_e_storagepool.NetAppESeriesStoragePool.drives"
    STORAGE_POOL_PROPERTY = "quantum.modules.storage.netapp.netapp_e_storagepool.NetAppESeriesStoragePool.storage_pool"
    ...
    def test_drives_fail(self):
        ...
        with patch(self.NETAPP_REQUEST_FUNC) as netapp_request:
            ...
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to fetch disk drives."):
    ...
    def test_apply_check_mode_unchange(self):
        ...
        with self.assertRaisesRegexp(QuantumExitJson, "'changed': False"):
            with patch(self.DRIVES_PROPERTY, new_callable=PropertyMock) as drives:
                drives.return_value = self.DRIVES_DATA
    ...
    def test_apply_check_mode_change(self):
        ...
        with self.assertRaisesRegexp(QuantumExitJson, "'changed': True"):
            with patch(self.DRIVES_PROPERTY, new_callable=PropertyMock) as drives:
                drives.return_value = self.DRIVES_DATA
                with patch(self.STORAGE_POOL_PROPERTY, new_callable=PropertyMock) as storage_pool:
                    storage_pool.return_value = self.STORAGE_POOL_DATA[0]
