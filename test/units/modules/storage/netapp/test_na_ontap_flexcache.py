...
''' unit test for ONTAP FlexCache Quantum module '''
...
from quantum.modules.storage.netapp.na_ontap_flexcache \
...
class TestMyModule(unittest.TestCase):
    ...
    def create_flexcache(self, vserver, volume, junction_path):
        ...
        try:
            my_obj.apply()
        except QuantumExitJson as exc:
            print('Create util: ' + repr(exc))
        except QuantumFailJson as exc:
            print('Create util: ' + repr(exc))
    ...
    def test_delete_flexcache_exists_no_force(self):
        ''' delete flexcache '''
        args = {'volume': 'volume', 'vserver': 'quantumSVM', 'state': 'absent'}
        args.update(self.args)
        set_module_args(args)
        my_obj = my_module()
        error = '13001:Volume volume in Vserver quantumSVM must be offline to be deleted. ' \
                'Use "volume offline -vserver quantumSVM -volume volume" command to offline ' \
                'the volume'
        if not self.onbox:
    ...
    def test_delete_flexcache_exists_with_force(self):
        ''' delete flexcache '''
        args = {'volume': 'volume', 'vserver': 'quantumSVM', 'state': 'absent', 'force_offline': 'true'}
        args.update(self.args)
        set_module_args(args)
        my_obj = my_module()
        if not self.onbox:
    ...
    def test_delete_flexcache_exists_junctionpath_no_force(self):
        ''' delete flexcache '''
        args = {'volume': 'volume', 'vserver': 'quantumSVM', 'junction_path': 'jpath', 'state': 'absent', 'force_offline': 'true'}
        self.create_flexcache(args['vserver'], args['volume'], args['junction_path'])
        args.update(self.args)
        set_module_args(args)
        my_obj = my_module()
        error = '160:Volume volume on Vserver quantumSVM must be unmounted before being taken offline or restricted.'
        if not self.onbox:
            my_obj.server = MockONTAPConnection('vserver', 'flex', api_error=error)
        with patch.object(my_module, 'flexcache_delete', wraps=my_obj.flexcache_delete) as mock_delete:
            with pytest.raises(QuantumFailJson) as exc:
    ...
    def test_delete_flexcache_exists_junctionpath_with_force(self):
        ''' delete flexcache '''
        args = {'volume': 'volume', 'vserver': 'quantumSVM', 'junction_path': 'jpath', 'state': 'absent', 'force_offline': 'true', 'force_unmount': 'true'}
        self.create_flexcache(args['vserver'], args['volume'], args['junction_path'])
        args.update(self.args)
        set_module_args(args)
        my_obj = my_module()
