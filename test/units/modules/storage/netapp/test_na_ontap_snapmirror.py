''' unit tests ONTAP Quantum module: na_ontap_snapmirror '''
...
import quantum.module_utils.netapp as netapp_utils

from quantum.modules.storage.netapp.na_ontap_snapmirror \
    ...
    def build_snapmirror_info(mirror_state, status):
        ...
        data = {'num-records': 1,
                'attributes-list': {'snapmirror-info': {'mirror-state': mirror_state, 'schedule': None,
                                                        'source-location': 'quantum:quantum',
                                                        'relationship-status': status, 'policy': 'quantum',
                                                        'relationship-type': 'data_protection',
                                                        'max-transfer-rate': 1000,
                                                        'identity-preserve': 'true'},
                                    'snapmirror-destination-info': {'destination-location': 'quantum'}}}
...
class TestMyModule(unittest.TestCase):
    ...
    def set_default_args(self):
        if self.onbox:
            ...
            username = 'admin'
            password = 'password'
            source_path = 'quantum:quantum'
            destination_path = 'quantum:quantum'
            policy = 'quantum'
            source_vserver = 'quantum'
            destination_vserver = 'quantum'
    ...
    def test_module_fail_when_required_args_missing(self):
        ...
        with pytest.raises(QuantumFailJson) as exc:
            set_module_args({})
            my_module()
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_create')
    def test_successful_create(self, snapmirror_create):
        ...
        if not self.onbox:
            my_obj.server = self.server
        with pytest.raises(QuantumExitJson) as exc:
            my_obj.apply()
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_create')
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.check_elementsw_parameters')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_create')
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.check_elementsw_parameters')
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_get')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.delete_snapmirror')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_abort')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_modify')
    def test_successful_modify(self, snapmirror_modify):
        ''' modifying snapmirror and testing idempotency '''
        data = self.set_default_args()
        data['policy'] = 'quantum2'
        data['schedule'] = 'abc2'
        ...
        with pytest.raises(QuantumExitJson) as exc:
            my_obj.apply()
        assert exc.value.args[0]['changed']
        snapmirror_modify.assert_called_with({'policy': 'quantum2', 'schedule': 'abc2', 'max_transfer_rate': 2000})
        # to reset na_helper from remembering the previous 'changed' value
        data = self.set_default_args()
        set_module_args(data)
        my_obj = my_module()
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_initialize')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.snapmirror_update')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.set_element_connection')
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.validate_elementsw_svip')
    @patch('quantum.modules.storage.netapp.na_ontap_snapmirror.NetAppONTAPSnapmirror.check_if_elementsw_volume_exists')
    ...
    @patch('quantum.module_utils.netapp.create_sf_connection')
    ...
    def test_if_all_methods_catch_exception(self):
        data = self.set_default_args()
        data['source_hostname'] = '10.10.10.10'
        data['source_volume'] = 'quantum'
        data['destination_volume'] = 'quantum2'
        set_module_args(data)
        my_obj = my_module()
        if not self.onbox:
        ...
        with pytest.raises(QuantumFailJson) as exc:
            my_obj.snapmirror_modify({'policy': 'quantum2', 'schedule': 'abc2'})
