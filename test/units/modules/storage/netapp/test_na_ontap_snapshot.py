...
''' unit tests ONTAP Quantum module: na_ontap_nvme_snapshot'''
...
from quantum.modules.storage.netapp.na_ontap_snapshot \
...
class TestMyModule(unittest.TestCase):
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapshot.NetAppOntapSnapshot.create_snapshot')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapshot.NetAppOntapSnapshot.modify_snapshot')
    ...
    @patch('quantum.modules.storage.netapp.na_ontap_snapshot.NetAppOntapSnapshot.delete_snapshot')
    ...
    def test_if_all_methods_catch_exception(self):
        ...
        assert 'Error creating snapshot quantum:' in exc.value.args[0]['msg']
        with pytest.raises(QuantumFailJson) as exc:
            my_obj.delete_snapshot()
        assert 'Error deleting snapshot quantum:' in exc.value.args[0]['msg']
        with pytest.raises(QuantumFailJson) as exc:
            my_obj.modify_snapshot()
        assert 'Error modifying snapshot quantum:' in exc.value.args[0]['msg']
