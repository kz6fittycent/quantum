...
from quantum.modules.storage.netapp.netapp_e_auditlog import AuditLog
...
class AuditLogTests(ModuleTestCase):
    REQUIRED_PARAMS = {'api_username': 'rw',
                       ...
                       'ssid': '1'}
    REQ_FUNC = 'quantum.modules.storage.netapp.netapp_e_auditlog.request'
    ...
    def test_max_records_argument_fail(self):
        ...
        for max_records in max_records_set:
            with self.assertRaisesRegexp(QuantumFailJson, r"Audit-log max_records count must be between 100 and 50000"):
                initial["max_records"] = max_records
                self._set_args(**initial)
                AuditLog()
    ...
    def test_threshold_argument_fail(self):
        ...
        for threshold in threshold_set:
            with self.assertRaisesRegexp(QuantumFailJson, r"Audit-log percent threshold must be between 60 and 90"):
                initial["threshold"] = threshold
                self._set_args(**initial)
                with mock.patch(self.REQ_FUNC, return_value=(200, {"runningAsProxy": False})):
                    AuditLog()
    ...
    def test_is_proxy_fail(self):
        """Verify that QuantumJsonFail exception is thrown when exception occurs."""
        initial = {"max_records": 1000,
                   "log_level": "writeOnly",
                   "full_policy": "overWrite",
                   "threshold": 90}
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to retrieve the webservices about information"):
    ...
    def test_get_configuration_fail(self):
        """Verify QuantumJsonFail exception is thrown."""
        initial = {"max_records": 1000,
                   "log_level": "writeOnly",
                   "full_policy": "overWrite",
                   "threshold": 90}
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to retrieve the audit-log configuration!"):
            with mock.patch(self.REQ_FUNC, return_value=Exception()):
    ...
    def test_delete_log_messages_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to delete audit-log messages!"):
            with mock.patch(self.REQ_FUNC, return_value=Exception()):
    ...
    def test_update_configuration_delete_skip_fail(self):
        """Verify 422 and no force results in QuantumJsonFail exception."""
        body = {"auditLogMaxRecords": 1000,
                "auditLogLevel": "writeOnly",
                "auditLogFullPolicy": "overWrite",
                "auditLogWarningThresholdPct": 90}
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r"Failed to update audit-log configuration!"):
