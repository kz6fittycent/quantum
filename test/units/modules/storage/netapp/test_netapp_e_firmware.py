...
from quantum.modules.storage.netapp.netapp_e_firmware import NetAppESeriesFirmware
from units.modules.utils import QuantumExitJson, QuantumFailJson, ModuleTestCase, set_module_args
...
class FirmwareTest(ModuleTestCase):
    REQUIRED_PARAMS = {"api_username": "username",
                       ...
                       "ssid": "1",
                       "validate_certs": "no"}
    REQUEST_FUNC = "quantum.modules.storage.netapp.netapp_e_firmware.NetAppESeriesFirmware.request"
    BASE_REQUEST_FUNC = "quantum.modules.storage.netapp.netapp_e_firmware.request"
    CREATE_MULTIPART_FORMDATA_FUNC = "quantum.modules.storage.netapp.netapp_e_firmware.create_multipart_formdata"
    SLEEP_FUNC = "quantum.modules.storage.netapp.netapp_e_firmware.sleep"
    ...
    def test_is_firmware_bundles_fail(self):
        ...
        with patch(builtin_path, mock_open(read_data=b"xxxxxxxxxxxxxxxx")) as mock_file:
            firmware = NetAppESeriesFirmware()
            with self.assertRaisesRegexp(QuantumFailJson, "Firmware file is invalid."):
    ...
    def test_check_system_health_fail(self):
        ...
        with patch("time.sleep", return_value=None):
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to initiate health check."):
                with patch(self.REQUEST_FUNC, return_value=(404, Exception())):
                    firmware.check_system_health()

            with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve health check status."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": "1"}),
                                                           (404, Exception())]):
                    firmware.check_system_health()

            with self.assertRaisesRegexp(QuantumFailJson, "Health check failed to complete."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": "1"}),
                                                           (200, {"healthCheckRunning": True,
    ...
    def test_embedded_check_nvsram_compatibility_fail(self):
        ...
        with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve NVSRAM compatibility results."):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.embedded_check_nvsram_compatibility()

            with self.assertRaisesRegexp(QuantumFailJson, "Invalid NVSRAM file."):
            ...
            with self.assertRaisesRegexp(QuantumFailJson, "Incompatible NVSRAM file."):
                with patch(self.REQUEST_FUNC, return_value=(200, {"signatureTestingPassed": True,
                                                                  "fileCompatible": False,
                                                                  "versionContents": [{"module": "nvsram",
                                                                                       "bundledVersion": "N280X-842834-D02",
    ...
    def test_embedded_check_firmware_compatibility_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve bundle compatibility results."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.embedded_check_bundle_compatibility()

        with self.assertRaisesRegexp(QuantumFailJson, "Invalid firmware bundle file."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
                with patch(self.REQUEST_FUNC, return_value=(200, {
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Incompatible firmware bundle file."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
                with patch(self.REQUEST_FUNC, return_value=(200, {
                    "signatureTestingPassed": True,
                    "fileCompatible": False,
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Downgrades are not permitted."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
                with patch(self.REQUEST_FUNC, return_value=(200, {
                    "signatureTestingPassed": True,
                    "fileCompatible": True,
    ...
    def test_embedded_wait_for_upgrade_fail(self):
        ...
        self._set_args({"firmware": "test.dlp", "nvsram": "test.dlp"})
        firmware = NetAppESeriesFirmware()
        with self.assertRaisesRegexp(QuantumFailJson, "Timeout waiting for Santricity Web Services Embedded."):
            with patch(self.SLEEP_FUNC, return_value=None):
    ...
    def test_embedded_upgrade_fail(self):
        ...
        self._set_args({"firmware": "test.dlp", "nvsram": "test.dlp"})
        firmware = NetAppESeriesFirmware()
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to upload and activate firmware."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("", {})):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.embedded_upgrade()
    ...
    def test_check_nvsram_compatibility_fail(self):
        ...
        firmware = NetAppESeriesFirmware()
        with patch(self.SLEEP_FUNC, return_value=None):
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to receive NVSRAM compatibility information."):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.proxy_check_nvsram_compatibility()

            with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve NVSRAM status update from proxy."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": 1}), Exception()]):
                    firmware.proxy_check_nvsram_compatibility()

            with self.assertRaisesRegexp(QuantumFailJson, "NVSRAM is not compatible."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": 1}),
                                                           (200, {"checkRunning": True}),
                                                           (200, {"checkRunning": False,
    ...
    def test_check_firmware_compatibility_fail(self):
        ...
        with patch(self.SLEEP_FUNC, return_value=None):
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to receive firmware compatibility information."):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.proxy_check_firmware_compatibility()

            with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve firmware status update from proxy."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": 1}), Exception()]):
                    firmware.proxy_check_firmware_compatibility()

            with self.assertRaisesRegexp(QuantumFailJson, "Firmware bundle is not compatible."):
                with patch(self.REQUEST_FUNC, side_effect=[(200, {"requestId": 1}),
                                                           (200, {"checkRunning": True}),
                                                           (200, {"checkRunning": False,
    ...
    def test_proxy_upload_and_check_compatibility_fail(self):
        ...
        firmware = NetAppESeriesFirmware()
        firmware.proxy_check_nvsram_compatibility = lambda: None
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve existing existing firmware files."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("headers", "data")):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.proxy_upload_and_check_compatibility()

        with self.assertRaisesRegexp(QuantumFailJson, "Failed to upload NVSRAM file."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("headers", "data")):
                with patch(self.REQUEST_FUNC, side_effect=[(200, [{"version": "XX.XX.XX.XX", "filename": "test"},
                                                                  {"version": "XXXXXXXXXX", "filename": "test.dlp"}]),
                                                           Exception()]):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to upload firmware bundle file."):
            with patch(self.CREATE_MULTIPART_FORMDATA_FUNC, return_value=("headers", "data")):
                with patch(self.REQUEST_FUNC, side_effect=[(200, [{"version": "XX.XX.XX.XX", "filename": "test"},
                                                                  {"version": "XXXXXXXXXX", "filename": "test.dlp"}]),
                                                           (200, None), Exception()]):
    ...
    def test_proxy_check_upgrade_required_fail(self):
        ...
        firmware.firmware_version = lambda: b"08.42.50.00"
        firmware.nvsram_version = lambda: b"not_nvsram_version"
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve controller firmware information."):
            with patch(self.REQUEST_FUNC, return_value=Exception()):
                firmware.proxy_check_upgrade_required()

        with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve storage system's NVSRAM version."):
    ...
    def test_proxy_wait_for_upgrade_fail(self):
        ...
        with patch(self.SLEEP_FUNC, return_value=None):
            ...
            firmware.is_firmware_bundled = lambda: True
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve firmware upgrade status."):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.proxy_wait_for_upgrade("1")

            firmware.is_firmware_bundled = lambda: False
            with self.assertRaisesRegexp(QuantumFailJson, "Timed out waiting for firmware upgrade to complete."):
                with patch(self.REQUEST_FUNC, return_value=Exception()):
                    firmware.proxy_wait_for_upgrade("1")

            firmware.is_firmware_bundled = lambda: True
            with self.assertRaisesRegexp(QuantumFailJson, "Firmware upgrade failed to complete."):
    ...
    def test_proxy_upgrade_fail(self):
        ...
        firmware.is_firmware_bundled = lambda: True
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to initiate firmware upgrade."):
            with patch(self.REQUEST_FUNC, return_value=Exception()):
