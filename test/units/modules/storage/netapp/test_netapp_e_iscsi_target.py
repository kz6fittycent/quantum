...
from quantum.modules.storage.netapp.netapp_e_iscsi_target import IscsiTarget
...
class IscsiTargetTest(ModuleTestCase):
    ...
    REQ_FUNC = 'quantum.modules.storage.netapp.netapp_e_iscsi_target.request'
    ...
    def test_invalid_chap_secret(self):
        for secret in [11 * 'a', 58 * 'a']:
            with self.assertRaisesRegexp(QuantumFailJson, r'.*?CHAP secret is not valid.*') as result:
                self._set_args(dict(chap=secret))
                tgt = IscsiTarget()
    ...
    def test_apply_iscsi_settings_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, r".*?update.*"):
