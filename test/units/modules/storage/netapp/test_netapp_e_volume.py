...
from quantum.module_utils.netapp import NetAppESeriesModule
from quantum.modules.storage.netapp.netapp_e_volume import NetAppESeriesVolume
from units.modules.utils import QuantumFailJson, ModuleTestCase, set_module_args
...
class NetAppESeriesVolumeTest(ModuleTestCase):
    ...
    WORKLOAD_GET_RESPONSE = [{"id": "4200000001000000000000000000000000000000", "name": "general_workload_1",
                             ...
                             {"id": "4200000002000000000000000000000000000000", "name": "employee_data",
                              "workloadAttributes": [{"key": "use", "value": "EmployeeData"},
                                                     ...
                                                     {"key": "profileId", "value": "quantum_workload_1"}]},
                             {"id": "4200000003000000000000000000000000000000", "name": "customer_database",
                              "workloadAttributes": [{"key": "use", "value": "customer_information"},
                                                     ...
                                                     {"key": "profileId", "value": "quantum_workload_2"}]},
                             {"id": "4200000004000000000000000000000000000000", "name": "product_database",
                              "workloadAttributes": [{"key": "use", "value": "production_information"},
                                                     ...
                                                     {"key": "location", "value": "global"},
                                                     {"key": "profileId", "value": "quantum_workload_4"}]}]

    REQUEST_FUNC = "quantum.modules.storage.netapp.netapp_e_volume.NetAppESeriesVolume.request"
    GET_VOLUME_FUNC = "quantum.modules.storage.netapp.netapp_e_volume.NetAppESeriesVolume.get_volume"
    SLEEP_FUNC = "quantum.modules.storage.netapp.netapp_e_volume.sleep"
    ...
    def test_get_volume_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to obtain list of thick volumes."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                self._set_args({"state": "present", "name": "Matthew", "storage_pool_name": "pool", "size": 100})
                volume_object = NetAppESeriesVolume()
                volume_object.get_volume()

        with self.assertRaisesRegexp(QuantumFailJson, "Failed to obtain list of thin volumes."):
    ...
    def tests_wait_for_volume_availability_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Timed out waiting for the volume"):
            with mock.patch(self.SLEEP_FUNC, return_value=None):
                volume_object.wait_for_volume_availability()
    ...
    def tests_wait_for_volume_action_fail(self):
        ...
        volume_object.volume_detail = {"id": "02000000600A098000A4B9D1000037315DXXXXXX",
                                       "storageVolumeRef": "02000000600A098000A4B9D1000037315D494C6F"}
        with mock.patch(self.SLEEP_FUNC, return_value=None):
            with self.assertRaisesRegexp(QuantumFailJson, "Failed to get volume expansion progress."):
                with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                    volume_object.wait_for_volume_action()

            with self.assertRaisesRegexp(QuantumFailJson, "Expansion action failed to complete."):
    ...
    def test_get_storage_pool_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to obtain list of storage pools."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                self._set_args({"state": "present", "name": "Matthew", "storage_pool_name": "pool", "size": 100})
                volume_object = NetAppESeriesVolume()
    ...
    def test_check_storage_pool_sufficiency_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Requested storage pool"):
            volume_object.check_storage_pool_sufficiency()

        with self.assertRaisesRegexp(QuantumFailJson,
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "requires the storage pool to be DA-compatible."):
    ...
    def test_update_workload_tags_fail(self):
        ...
        volume_object = NetAppESeriesVolume()
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to retrieve storage array workload tags."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
        ...
        volume_object = NetAppESeriesVolume()
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to create new workload tag."):
    ...
    def test_get_volume_property_changes_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Existing volume segment size is"):
    ...
    def test_get_expand_volume_changes_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Reducing the size of volumes is not permitted."):
        ...
        volume_object.volume_detail = {"capacity": str(100 * 1024 * 1024 * 1024), "thinProvisioned": True,
                                       "expansionPolicy": "manual",
                                       "currentProvisionedCapacity": str(500 * 1024 * 1024 * 1024)}
        with self.assertRaisesRegexp(QuantumFailJson, "The thin volume repository increase must be between or equal"):
            volume_object.get_expand_volume_changes()
    ...
    def test_create_volume_fail(self):
        ...
        volume_object = NetAppESeriesVolume()
        volume_object.pool_detail = {"id": "12345"}
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to create volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.create_volume()
        ...
        volume_object = NetAppESeriesVolume()
        volume_object.pool_detail = {"id": "12345"}
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to create thin volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.create_volume()
    ...
    def test_update_volume_properties_fail(self):
        ...
        volume_object.get_volume_property_changes = lambda: {
            ...
            'flashCached': True}
        volume_object.workload_id = "4200000001000000000000000000000000000000"
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to update volume properties."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                self.assertTrue(volume_object.update_volume_properties())
        ...
        volume_object.get_volume_property_changes = lambda: {
            ...
            'flashCached': True}
        volume_object.workload_id = "4200000001000000000000000000000000000000"
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to update thin volume properties."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                self.assertTrue(volume_object.update_volume_properties())
    ...
    def test_expand_volume_fail(self):
        ...
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to expand volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.expand_volume()
        ...
        volume_object.get_expand_volume_changes = lambda: {"sizeUnit": "bytes",
                                                           "expansionSize": 100 * 1024 * 1024 * 1024}
        volume_object.volume_detail = {"id": "12345", "thinProvisioned": True}
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to expand thin volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.expand_volume()
    ...
    def test_delete_volume_fail(self):
        ...
        self._set_args(
            {"state": "present", "name": "Matthew", "storage_pool_name": "pool", "size": 100, "thin_provision": False})
        volume_object = NetAppESeriesVolume()
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to delete volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.delete_volume()
        ...
        self._set_args(
            {"state": "present", "name": "Matthew", "storage_pool_name": "pool", "size": 100, "thin_provision": True})
        volume_object = NetAppESeriesVolume()
        with self.assertRaisesRegexp(QuantumFailJson, "Failed to delete thin volume."):
            with mock.patch(self.REQUEST_FUNC, return_value=Exception()):
                volume_object.delete_volume()
