...
from quantum.modules.storage.hpe3par import ss_3par_cpg
from quantum.module_utils.storage.hpe3par import hpe3par


@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.client')
@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.QuantumModule')
@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.create_cpg')
...
def test_main_exit_functionality_present_success_without_issue_attr_dict(mock_create_cpg, mock_module, mock_client):
    ...
    # This creates a instance of the QuantumModule mock.
    mock_module.params = PARAMS_FOR_PRESENT
    mock_module.return_value = mock_module
    ...
    ss_3par_cpg.main()
    # QuantumModule.exit_json should be called
    instance.exit_json.assert_called_with(
        changed=True, msg="Created CPG successfully.")
    # QuantumModule.fail_json should not be called
    assert instance.fail_json.call_count == 0


@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.client')
@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.QuantumModule')
@mock.patch('quantum.modules.storage.hpe3par.ss_3par_cpg.delete_cpg')
