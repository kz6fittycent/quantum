...
from quantum.modules.network.eos import eos_config
from quantum.plugins.cliconf.eos import Cliconf
...
class TestEosConfigModule(TestEosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.eos.eos_config.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_get_connection = patch('quantum.modules.network.eos.eos_config.get_connection')
        self.get_connection = self.mock_get_connection.start()

        self.mock_load_config = patch('quantum.modules.network.eos.eos_config.load_config')
        self.load_config = self.mock_load_config.start()
        self.mock_run_commands = patch('quantum.modules.network.eos.eos_config.run_commands')
        self.run_commands = self.mock_run_commands.start()

        self.mock_supports_sessions = patch('quantum.plugins.cliconf.eos.Cliconf.supports_sessions')
    ...
    def test_eos_config_lines(self):
        lines = ['hostname switch01', 'ip domain-name eng.quantum.com']
        args = dict(lines=lines)
        set_module_args(args)
    ...
    def test_eos_config_save_when(self):
        mock_run_commands = patch('quantum.modules.network.eos.eos_config.run_commands')
