...
from quantum.modules.network.nxos import nxos_telemetry
from quantum.module_utils.network.nxos.nxos import NxosCmdRef
from quantum.module_utils.network.nxos.config.telemetry.telemetry import Telemetry
...
import pytest
from quantum.module_utils.network.nxos.nxos import nxosCmdRef_import_check
...
class TestNxosTelemetryModule(TestNxosModule):
    ...
    def setUp(self):
        ...
        self.mock_edit_config = patch('quantum.module_utils.network.nxos.config.telemetry.telemetry.Telemetry.edit_config')
        self.edit_config = self.mock_edit_config.start()

        self.mock_execute_show_command = patch('quantum.module_utils.network.nxos.nxos.NxosCmdRef.execute_show_command')
        self.execute_show_command = self.mock_execute_show_command.start()

        self.mock_get_platform_shortname = patch('quantum.module_utils.network.nxos.nxos.NxosCmdRef.get_platform_shortname')
    ...
    def test_tms_destgroup_input_validation_1(self):
        ...
        with pytest.raises(QuantumFailJson) as errinfo:
            self.execute_module()
    ...
    def test_telemetry_deleted_input_validation_n9k(self):
        ...
        with pytest.raises(QuantumFailJson) as errinfo:
            self.execute_module()
        testdata = errinfo.value.args[0]
        assert 'Remove config key from coupling when state is <deleted>' in str(testdata['msg'])
        assert testdata['failed']

    def test_telemetry_deleted_n9k(self):
        # Assumes feature telemetry is enabled
        # TMS global config is present.
        # Make absent with all coupling keys provided
        self.execute_show_command.return_value = load_fixture('nxos_telemetry', 'N9K.cfg')
        self.get_platform_shortname.return_value = 'N9K'
        set_module_args(dict(
            state='deleted',
