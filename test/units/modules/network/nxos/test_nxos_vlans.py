...
from quantum.modules.network.nxos import nxos_vlans
from quantum.module_utils.network.nxos.config.vlans.vlans import Vlans
...
class TestNxosVlansModule(TestNxosModule):
    ...
    def setUp(self):
        ...
        self.mock_edit_config = patch('quantum.module_utils.network.nxos.config.vlans.vlans.Vlans.edit_config')
        self.edit_config = self.mock_edit_config.start()

        self.mock_get_device_data = patch('quantum.module_utils.network.nxos.facts.vlans.vlans.VlansFacts.get_device_data')
    ...
    def test_1(self):
        ...
        **NOTE** This config is for reference only! See fixtures files for real data.
        ...
        coupling = dict(config=[
            ...
            # vlan 3 is not present in coupling.
        ])
        ...
        deleted = [
            ...
            # this means deleting each vlan listed in the coupling and ignoring
    ...
    def test_2(self):
        # vlan 1 in coupling should raise
        coupling = dict(config=[dict(vlan_id=1)], state='merged')
        set_module_args(coupling, ignore_provider_arg)
        self.execute_module(failed=True)

    def test_3(self):
        # Test when no 'config' key is used in coupling.
        deleted = [
    ...
    def test_6(self):
        ...
        coupling['state'] = 'replaced'
        coupling['_quantum_check_mode'] = True
        set_module_args(coupling, ignore_provider_arg)
        self.execute_module(changed=True, commands=replaced)
