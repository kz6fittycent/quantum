...
from quantum.modules.network.nxos import nxos_vpc
...
class TestNxosVpcModule(TestNxosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.nxos.nxos_vpc.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.nxos.nxos_vpc.load_config')
        self.load_config = self.mock_load_config.start()

        self.mock_run_commands = patch('quantum.modules.network.nxos.nxos_vpc.run_commands')
    ...
    def test_nxos_vpc_vrf_3(self):
        ...
        # Existing pkl_src should be retained even though coupling does not specify it
        self.get_config.return_value = load_fixture('nxos_vpc', 'vrf_test_vpc_config')
        set_module_args(dict(
            domain=100,
            pkl_dest='192.168.1.1',
    ...
    def test_nxos_vpc_vrf_7(self):
        ...
        # pkl_src/pkl_vrf not in coupling but exists on device.
        self.get_config.return_value = load_fixture('nxos_vpc', 'vrf_test_vpc_config')
        set_module_args(dict(
            domain=100,
            pkl_dest='192.168.1.1',
    ...
    def test_nxos_vpc_vrf_8(self):
        ...
        # pkl_src not in coupling but exists on device.
        self.get_config.return_value = load_fixture('nxos_vpc', 'vrf_test_vpc_config')
        set_module_args(dict(
            domain=100,
            pkl_dest='192.168.1.1',
