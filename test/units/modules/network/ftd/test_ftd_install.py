...
from units.modules.utils import set_module_args, exit_json, fail_json, QuantumFailJson, QuantumExitJson

from quantum.modules.network.ftd import ftd_install
from quantum.module_utils.network.ftd.device import FtdModel
...
class TestFtdInstall(object):
    ...
    def module_mock(self, mocker):
        mocker.patch.multiple(basic.QuantumModule, exit_json=exit_json, fail_json=fail_json)
        mocker.patch.object(basic.QuantumModule, '_socket_path', new_callable=PropertyMock, create=True,
                            return_value=mocker.MagicMock())

    @pytest.fixture(autouse=True)
    def connection_mock(self, mocker):
        connection_class_mock = mocker.patch('quantum.modules.network.ftd.ftd_install.Connection')
        return connection_class_mock.return_value

    @pytest.fixture
    def config_resource_mock(self, mocker):
        resource_class_mock = mocker.patch('quantum.modules.network.ftd.ftd_install.BaseConfigurationResource')
        return resource_class_mock.return_value

    @pytest.fixture(autouse=True)
    def ftd_factory_mock(self, mocker):
        return mocker.patch('quantum.modules.network.ftd.ftd_install.FtdPlatformFactory')

    @pytest.fixture(autouse=True)
    def has_kick_mock(self, mocker):
        return mocker.patch('quantum.module_utils.network.ftd.device.HAS_KICK', True)

    def test_module_should_fail_when_kick_is_not_installed(self, mocker):
        mocker.patch('quantum.module_utils.network.ftd.device.HAS_KICK', False)

        set_module_args(dict(DEFAULT_MODULE_PARAMS))
        with pytest.raises(QuantumFailJson) as ex:
            self.module.main()
    ...
    def test_module_should_fail_when_device_model_is_missing_with_local_connection(self, mocker):
        mocker.patch.object(basic.QuantumModule, '_socket_path', create=True, return_value=None)
        module_params = dict(DEFAULT_MODULE_PARAMS)
        del module_params['device_model']
    ...
    def test_module_should_fill_management_ip_values_when_missing(self, config_resource_mock, ftd_factory_mock):
        ...
        set_module_args(module_params)
        with pytest.raises(QuantumExitJson):
            self.module.main()
