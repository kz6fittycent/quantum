...
from quantum.modules.network.cnos import cnos_user
...
class TestCnosUserModule(TestCnosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.cnos.cnos_user.get_config')
        self.get_config = self.mock_get_config.start()
        self.mock_load_config = patch('quantum.modules.network.cnos.cnos_user.load_config')
        self.load_config = self.mock_load_config.start()
        self.mock_run_commands = patch('quantum.modules.network.cnos.cnos_user.run_commands')
    ...
    def test_cnos_user_password(self):
        ...
        commands = ['username quantum', 'username quantum password test']
        self.execute_module(changed=True, commands=commands)
    ...
    def test_cnos_user_role(self):
        set_module_args(dict(name='quantum', role='network-admin', configured_password='test'))
        result = self.execute_module(changed=True)
        self.assertIn('username quantum role network-admin', result['commands'])

    def test_cnos_user_sshkey(self):
        set_module_args(dict(name='quantum', sshkey='test'))
        commands = ['username quantum', 'username quantum sshkey test']
        self.execute_module(changed=True, commands=commands)
