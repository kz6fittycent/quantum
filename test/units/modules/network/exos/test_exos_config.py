...
from quantum.plugins.cliconf.exos import Cliconf
from quantum.modules.network.exos import exos_config
...
class TestExosConfigModule(TestExosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.exos.exos_config.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.exos.exos_config.load_config')
        self.load_config = self.mock_load_config.start()

        self.mock_run_commands = patch('quantum.modules.network.exos.exos_config.run_commands')
        self.run_commands = self.mock_run_commands.start()

        self.mock_get_startup_config = patch('quantum.modules.network.exos.exos_config.get_startup_config')
        self.get_startup_config = self.mock_get_startup_config.start()

        self.cliconf_obj = Cliconf(MagicMock())

        self.mock_get_diff = patch('quantum.modules.network.exos.exos_config.get_diff')
    ...
    def test_exos_config_save_changed_true_check_mode(self):
        src = load_fixture('exos_config_src.cfg')
        set_module_args(dict(src=src, save_when='changed', _quantum_check_mode=True))
        commands = ['configure ports 1 description-string "IDS"',
                    'configure snmp sysName "marble"']
        self.get_diff.return_value = self.cliconf_obj.get_diff(src, self.running_config)
        self.execute_module(changed=True, commands=commands)
    ...
    def test_exos_diff_running_unchanged(self):
        args = dict(diff_against='running', _quantum_diff=True)
        set_module_args(args)
        self.execute_module(changed=False)
    ...
    def test_exos_diff_startup_unchanged(self):
        mock_get_startup_config = patch('quantum.modules.network.exos.exos_config.get_startup_config')
        get_startup_config = mock_get_startup_config.start()
        get_startup_config.return_value = load_fixture('exos_config_config.cfg')

        args = dict(diff_against='startup', _quantum_diff=True)
        set_module_args(args)
        self.execute_module(changed=False)
