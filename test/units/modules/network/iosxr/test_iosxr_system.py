...
from quantum.modules.network.iosxr import iosxr_system
...
class TestIosxrSystemModule(TestIosxrModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.iosxr.iosxr_system.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.iosxr.iosxr_system.load_config')
        self.load_config = self.mock_load_config.start()

        self.mock_is_cliconf = patch('quantum.modules.network.iosxr.iosxr_system.is_cliconf')
    ...
    def test_iosxr_system_domain_search(self):
        set_module_args(dict(domain_search=['quantum.com', 'redhat.com']))
        commands = ['domain list quantum.com', 'no domain list cisco.com', 'no domain lookup disable']
        self.execute_module(changed=True, commands=commands)
    ...
    def test_iosxr_system_no_change(self):
        set_module_args(dict(hostname='iosxr01', domain_name='eng.quantum.com', lookup_enabled=False))
        self.execute_module()
