...
from quantum.modules.network.iosxr import iosxr_user
...
class TestIosxrUserModule(TestIosxrModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.iosxr.iosxr_user.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.iosxr.iosxr_user.load_config')
        self.load_config = self.mock_load_config.start()

        self.mock_is_cliconf = patch('quantum.modules.network.iosxr.iosxr_user.is_cliconf')
    ...
    def test_iosxr_user_delete(self):
        ...
        self.assertEqual(result['commands'], ['no username quantum'])
    ...
    def test_iosxr_user_group(self):
        set_module_args(dict(name='quantum', group='sysadmin'))
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], ['username quantum group sysadmin'])
    ...
    def test_iosxr_user_admin_mode(self):
        set_module_args(dict(name='quantum-2', configured_password='test-2', admin=True))
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], ['username quantum-2', 'username quantum-2 secret test-2'])
