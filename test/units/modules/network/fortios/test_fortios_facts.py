...
try:
    from quantum.module_utils.network.fortios.facts.facts import Facts
    from quantum.modules.network.fortios import fortios_facts
except ImportError:
...
def connection_mock(mocker):
    connection_class_mock = mocker.patch('quantum.modules.network.fortios.fortios_facts.Connection')
...
def test_facts_get(mocker):
    ...
    monitor_method_mock = mocker.patch('quantum.module_utils.network.fortios.fortios.FortiOSHandler.monitor', return_value=monitor_method_result)
    mock_module = patch.multiple(basic.QuantumModule, exit_json=exit_json, fail_json=fail_json)
    ...
    assert response['quantum_network_resources']['system_status_select']['status'] == 'success'
    assert response['quantum_network_resources']['system_status_select']['http_status'] == 200
    ...
    assert response['quantum_network_resources']['system_interface_select']['status'] == 'success'
    assert response['quantum_network_resources']['system_interface_select']['http_status'] == 200
    ...
    assert response['quantum_network_resources']['system_ha-checksums_select']['status'] == 'success'
    assert response['quantum_network_resources']['system_ha-checksums_select']['http_status'] == 200
