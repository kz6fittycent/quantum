...
from quantum.modules.network.vyos import vyos_user
...
class TestVyosUserModule(TestVyosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.vyos.vyos_user.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.vyos.vyos_user.load_config')
    ...
    def test_vyos_user_password(self):
        ...
        self.assertEqual(result['commands'], ['set system login user quantum authentication plaintext-password test'])

    def test_vyos_user_delete(self):
        set_module_args(dict(name='quantum', state='absent'))
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], ['delete system login user quantum'])

    def test_vyos_user_level(self):
        set_module_args(dict(name='quantum', level='operator'))
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], ['set system login user quantum level operator'])

    def test_vyos_user_level_invalid(self):
        set_module_args(dict(name='quantum', level='sysadmin'))
        self.execute_module(failed=True)

    def test_vyos_user_purge(self):
        set_module_args(dict(purge=True))
        result = self.execute_module(changed=True)
        self.assertEqual(sorted(result['commands']), sorted(['delete system login user quantum',
