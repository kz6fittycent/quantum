...
from quantum.modules.network.icx import icx_system
...
class TestICXSystemModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.icx.icx_system.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.icx.icx_system.load_config')
        self.load_config = self.mock_load_config.start()

        self.mock_exec_command = patch('quantum.modules.network.icx.icx_system.exec_command')
    ...
    def test_icx_system_set_config(self):
        set_module_args(dict(hostname='ruckus', name_servers=['172.16.10.2', '11.22.22.4'], domain_search=['quantum.com', 'redhat.com']))
        if not self.ENV_ICX_USE_DIFF:
            commands = [
                'hostname ruckus',
                'ip dns domain-list quantum.com',
    ...
    def test_icx_aaa_servers_radius_set(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            commands = [
                ...
                'tacacs-server host quantum.com'
            ]
            self.execute_module(changed=True, commands=commands)
