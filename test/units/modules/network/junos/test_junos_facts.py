...
from quantum.modules.network.junos import junos_facts
...
class TestJunosCommandModule(TestJunosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.module_utils.network.junos.facts.legacy.base.get_configuration')
        self.get_config = self.mock_get_config.start()

        self.mock_netconf = patch('quantum.module_utils.network.junos.junos.NetconfConnection')
        self.netconf_conn = self.mock_netconf.start()

        self.mock_exec_rpc = patch('quantum.module_utils.network.junos.facts.legacy.base.exec_rpc')
        self.exec_rpc = self.mock_exec_rpc.start()

        self.mock_netconf_rpc = patch('quantum.module_utils.network.common.netconf.NetconfConnection')
        ...
        self.mock_get_capabilities = patch('quantum.module_utils.network.junos.facts.legacy.base.get_capabilities')
    ...
    def test_junos_get_facts(self):
        ...
        self.assertEqual(facts['quantum_net_hostname'], 'vsrx01')
        self.assertTrue('em0' in facts['quantum_net_interfaces'])
        self.assertEqual(facts['quantum_net_interfaces']['em0']['type'], 'Ethernet')
        self.assertEqual(facts['quantum_net_memtotal_mb'], 983500)
        self.assertEqual(facts['quantum_net_filesystems'][0], '/dev/vtbd0s1a')
        self.assertTrue('quantum_net_config' not in facts)
        self.assertEqual(facts['quantum_net_routing_engines']["0"]['model'], 'RE-S-EX9200-1800X4')
        self.assertEqual(facts['quantum_net_modules'][0]['name'], 'Midplane')
        self.assertTrue(facts['quantum_net_has_2RE'])
    ...
    def test_junos_get_facts_subset_config_set(self):
        ...
        self.assertTrue('quantum_net_config' in facts)
        self.assertTrue(facts['quantum_net_config'].startswith('set'))
        self.assertEqual(facts['quantum_net_hostname'], 'vsrx01')
        self.assertTrue('quantum_net_interfaces' not in facts)
    ...
    def test_junos_get_facts_subset_config_json(self):
        ...
        self.assertTrue('quantum_net_config' in facts)
        self.assertTrue('configuration' in facts['quantum_net_config'])
        self.assertEqual(facts['quantum_net_hostname'], 'vsrx01')
        self.assertTrue('quantum_net_interfaces' not in facts)
    ...
    def test_junos_get_facts_subset_list(self):
        ...
        self.assertTrue('quantum_net_config' not in facts)
        self.assertEqual(facts['quantum_net_interfaces']['em0']['oper-status'], 'up')
        self.assertEqual(facts['quantum_net_memfree_mb'], 200684)
