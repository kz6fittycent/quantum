...
from quantum.modules.network.ios import ios_system
...
class TestIosSystemModule(TestIosModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch('quantum.modules.network.ios.ios_system.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch('quantum.modules.network.ios.ios_system.load_config')
    ...
    def test_ios_system_domain_search(self):
        ...
        commands = ['no ip domain list vrf management example.net',
                    ...
                    'ip domain list quantum.com',
                    'ip domain list redhat.com']
        self.execute_module(changed=True, commands=commands, sort=False)

    def test_ios_system_domain_search_complex(self):
        set_module_args(dict(domain_search=[{'name': 'quantum.com', 'vrf': 'test'}]))
        commands = ['no ip domain list vrf management example.net',
                    'no ip domain list example.net',
                    'no ip domain list example.com',
                    'ip domain list vrf test quantum.com']
        self.execute_module(changed=True, commands=commands, sort=False)
