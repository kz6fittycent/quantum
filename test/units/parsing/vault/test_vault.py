...
class TestUnhexlify(unittest.TestCase):
    ...
    def test_odd_length(self):
        ...
        self.assertRaisesRegexp(vault.QuantumVaultFormatError,
...
class TestPromptVaultSecret(unittest.TestCase):
    ...
    @patch('quantum.parsing.vault.display.prompt', return_value='the_password')
    ...
    @patch('quantum.parsing.vault.display.prompt', side_effect=EOFError)
    def test_prompt_eoferror(self, mock_display_prompt):
        secret = vault.PromptVaultSecret(vault_id='test_id')
        self.assertRaisesRegexp(vault.QuantumVaultError,
                                'EOFError.*test_id',
                                secret.load)

    @patch('quantum.parsing.vault.display.prompt', side_effect=['first_password', 'second_password'])
    def test_prompt_passwords_dont_match(self, mock_display_prompt):
        secret = vault.PromptVaultSecret(vault_id='test_id',
                                         prompt_formats=['Vault password: ',
                                                         'Confirm Vault password: '])
        self.assertRaisesRegexp(errors.QuantumError,
...
class TestFileVaultSecret(unittest.TestCase):
    ...
    def test_file_empty(self):
        ...
        secret = vault.FileVaultSecret(loader=fake_loader, filename=tmp_file.name)
        self.assertRaisesRegexp(vault.QuantumVaultPasswordError,
    ...
    def test_file_encrypted(self):
        ...
        password = 'some password'
        # 'some password' encrypted with 'test-quantum-password'
...
class TestScriptVaultSecret(unittest.TestCase):
    ...
    @patch('quantum.parsing.vault.subprocess.Popen')
...
class TestVaultLib(unittest.TestCase):
    ...
    def test_parse_vaulttext_envelope(self):
        ...
        self.assertEqual(b_lines[0], b"quantum", msg="Payload was not properly split from the header")
    ...
    def test_encrypt_decrypt_aes256_none_secrets(self):
        vault_secrets = self._vault_secrets_from_password('default', 'quantum')
        v = vault.VaultLib(vault_secrets)
...
3138'''
    ...
    def test_decrypt_and_get_vault_id(self):
        ...
        vaulttext = '''$ANSIBLE_VAULT;1.2;AES256;quantum_devel
...
6264'''
        ...
        vault_secrets = self._vault_secrets_from_password('quantum_devel', 'quantum')
        v = vault.VaultLib(vault_secrets)
        ...
        self.assertEqual(vault_id_used, 'quantum_devel')
        self.assertEqual(vault_secret_used.text, 'quantum')
...
6264'''
        ...
        b_ciphertext, b_version, cipher_name, vault_id = vault.parse_vaulttext_envelope(b_vaulttext)
        self.assertEqual('quantum_devel', vault_id)
        self.assertEqual(b'1.2', b_version)

    def test_decrypt_decrypted(self):
        plaintext = u"quantum"
        self.assertRaises(errors.QuantumError, self.v.decrypt, plaintext)

        b_plaintext = b"quantum"
        self.assertRaises(errors.QuantumError, self.v.decrypt, b_plaintext)
