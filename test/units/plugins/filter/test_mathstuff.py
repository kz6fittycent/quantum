...
import quantum.plugins.filter.mathstuff as ms
...
class TestLogarithm:
    def test_log_non_number(self):
        ...
        with pytest.raises(QuantumFilterTypeError, match='log\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
            ms.logarithm('a')
        with pytest.raises(QuantumFilterTypeError, match='log\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
...
class TestPower:
    def test_power_non_number(self):
        # Message changed in python3.6
        with pytest.raises(QuantumFilterTypeError, match='pow\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
            ms.power('a', 10)
...
class TestInversePower:
    def test_root_non_number(self):
        ...
        with pytest.raises(QuantumFilterTypeError, match="root\\(\\) can only be used on numbers:"
                           " (invalid literal for float\\(\\): a"
                           "|could not convert string to float: a"
                           "|could not convert string to float: 'a')"):
            ms.inversepower(10, 'a')

        with pytest.raises(QuantumFilterTypeError, match="root\\(\\) can only be used on numbers: (a float is required|must be real number, not str)"):
...
class TestRekeyOnMember():
    ...
    INVALID_ENTRIES = (
        ...
        (QuantumFilterError, [{"proto": "eigrp", "state": "enabled"}], 'invalid_key', "Key invalid_key was not found"),
        (QuantumFilterError, {"eigrp": {"proto": "eigrp", "state": "enabled"}}, 'invalid_key', "Key invalid_key was not found"),
        # Fail when key is duplicated
        (QuantumFilterError, [{"proto": "eigrp"}, {"proto": "ospf"}, {"proto": "ospf"}],
         'proto', 'Key ospf is not unique, cannot correctly turn into dict'),
        # Fail when value is not a dict
        (QuantumFilterTypeError, ["string"], 'proto', "List item is not a valid dict"),
        (QuantumFilterTypeError, [123], 'proto', "List item is not a valid dict"),
        (QuantumFilterTypeError, [[{'proto': 1}]], 'proto', "List item is not a valid dict"),
        # Fail when we do not send a dict or list
        (QuantumFilterTypeError, "string", 'proto', "Type is not a valid list, set, or dict"),
        (QuantumFilterTypeError, 123, 'proto', "Type is not a valid list, set, or dict"),
    )
