...
from quantum.plugins.filter.ipaddr import (ipaddr, next_nth_usable, ipsubnet,
...
class TestIpFilter(unittest.TestCase):
    ...
    def test_ipmath(self):
        ...
        with self.assertRaises(QuantumFilterError) as exc:
            ipmath('invalid_ip', 8)
    ...
    def _test_ipsubnet(self, ipsubnet_args, expected_result):
        ...
        with self.assertRaisesRegexp(QuantumFilterError, 'You must pass a valid subnet or IP address; invalid_subnet is invalid'):
            ipsubnet('192.168.144.5', 'invalid_subnet')

        with self.assertRaisesRegexp(QuantumFilterError, '192.168.144.0/30 is not in the subnet 192.168.144.4/30'):
