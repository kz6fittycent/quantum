...
def laps_password():
    ...
    try:
        ...
        from quantum.plugins.lookup import laps_password
...
def test_missing_ldap(laps_password):
    ...
    with pytest.raises(QuantumLookupError) as err:
...
def test_gssapi_auth_with_credentials():
    ...
    assert str(err.value) == "Explicit credentials are not supported when auth='gssapi'. Call kinit outside of Quantum"
