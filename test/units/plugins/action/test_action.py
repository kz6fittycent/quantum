...
from quantum.module_utils.six import text_type
from quantum.module_utils.six.moves import shlex_quote, builtins
...
from quantum.vars.clean import clean_facts
...
class TestActionBase(unittest.TestCase):
        ...
        with patch.object(builtins, 'open', mock_open(read_data=to_bytes(python_module_replacers.strip(), encoding='utf-8'))):
            with patch.object(os, 'rename'):
                ...
                (style, shebang, data, path) = action_base._configure_module(mock_task.action, mock_task.args,
                                                                             task_vars=dict(quantum_python_interpreter='/usr/bin/python'))
                self.assertEqual(style, "new")
                self.assertEqual(shebang, u"#!/usr/bin/python")

                # test module not found
                self.assertRaises(QuantumError, action_base._configure_module, 'badmodule', mock_task.args, {})
    ...
    def test_action_base__compute_environment_string(self):
        ...
        self.assertRaises(QuantumError, action_base._compute_environment_string)
    ...
    def test_action_base__make_tmp_path(self):
        ...
        self.assertRaises(QuantumError, action_base._make_tmp_path, 'root')
    ...
    def test_action_base__remove_tmp_path(self):
        ...
        action_base._remove_tmp_path('/good/path/to/quantum-tmp-thing')
    ...
    def test_action_base__transfer_data(self, mock_mkstemp, mock_fdopen, mock_unlink):
        ...
        self.assertRaises(QuantumError, action_base._transfer_data, '/path/to/remote/file', '')
    ...
    def test_action_base__execute_remote_stat(self):
        ...
        self.assertRaises(QuantumError, action_base._execute_remote_stat, path='/path/to/file', all_vars=dict(), follow=False)
    ...
    def test_action_base__execute_module(self):
        ...
        self.assertEqual(action_base._execute_module(module_name=None, module_args=None), dict(_quantum_parsed=True, rc=0, stdout="ok", stdout_lines=['ok']))
        self.assertEqual(
        ...
        action_base._is_pipelining_enabled.return_value = False
        action_base._make_tmp_path.return_value = '/the/tmp/path'
        self.assertEqual(action_base._execute_module(), dict(_quantum_parsed=True, rc=0, stdout="ok", stdout_lines=['ok']))
        ...
        action_base._is_pipelining_enabled.return_value = False
        action_base._make_tmp_path.return_value = '/the/tmp/path'
        self.assertRaises(QuantumError, action_base._execute_module)
...
class TestActionBaseCleanReturnedData(unittest.TestCase):
    def test(self):
        ...
        connection_loader_paths = ['/tmp/asdfadf', '/usr/lib64/whatever',
                                   ...
                                   # '(.*)',
                                   '/path/to/quantum/lib/quantum/plugins/connection/custom_connection.py',
                                   '/path/to/quantum/lib/quantum/plugins/connection/ssh.py']
        ...
        action_base = DerivedActionBase(task=None,
                                        ...
                                        templar=None,
                                        shared_loader_obj=mock_shared_loader_obj)
        data = {'quantum_coupling_python': '/usr/bin/python',
                # 'quantum_rsync_path': '/usr/bin/rsync',
                'quantum_python_interpreter': '/usr/bin/python',
                'quantum_ssh_some_var': 'whatever',
                'quantum_ssh_host_key_somehost': 'some key here',
                'some_other_var': 'foo bar'}
        data = clean_facts(data)
        self.assertNotIn('quantum_coupling_python', data)
        self.assertNotIn('quantum_python_interpreter', data)
        self.assertIn('quantum_ssh_host_key_somehost', data)
...
class TestActionBaseParseReturnedData(unittest.TestCase):
    ...
    def test_fail_no_json(self):
        ...
        self.assertFalse(res['_quantum_parsed'])
    ...
    def test_json_empty(self):
        ...
        returned_data = {'rc': rc,
                         ...
                         'stderr': err}
        res = action_base._parse_returned_data(returned_data)
        del res['_quantum_parsed']  # we always have _quantum_parsed
    ...
    def test_json_facts(self):
        action_base = _action_base()
        rc = 0
        stdout = '{"quantum_facts": {"foo": "bar", "quantum_blip": "blip_value"}}\n'
        err = ''
        ...
        returned_data = {'rc': rc,
                         ...
                         'stderr': err}
        res = action_base._parse_returned_data(returned_data)
        self.assertTrue(res['quantum_facts'])
        self.assertIn('quantum_blip', res['quantum_facts'])
        # TODO: Should this be an QuantumUnsafe?
        # self.assertIsInstance(res['quantum_facts'], QuantumUnsafe)
