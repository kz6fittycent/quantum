...
from quantum.coupling.play_context import PlayContext
from quantum.plugins.connection import ConnectionBase
from quantum.plugins.loader import become_loader
# from quantum.plugins.connection.accelerate import Connection as AccelerateConnection
# from quantum.plugins.connection.chroot import Connection as ChrootConnection
# from quantum.plugins.connection.funcd import Connection as FuncdConnection
# from quantum.plugins.connection.jail import Connection as JailConnection
# from quantum.plugins.connection.libvirt_lxc import Connection as LibvirtLXCConnection
from quantum.plugins.connection.lxc import Connection as LxcConnection
from quantum.plugins.connection.local import Connection as LocalConnection
from quantum.plugins.connection.paramiko_ssh import Connection as ParamikoConnection
from quantum.plugins.connection.ssh import Connection as SSHConnection
from quantum.plugins.connection.docker import Connection as DockerConnection
# from quantum.plugins.connection.winrm import Connection as WinRmConnection
from quantum.plugins.connection.network_cli import Connection as NetworkCliConnection
from quantum.plugins.connection.httpapi import Connection as HttpapiConnection
...
if PY3:
    with patch('builtins.__import__', side_effect=import_mock):
        from quantum.plugins.connection.netconf import Connection as NetconfConnection
else:
...
class TestConnectionBaseClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.play_context.prompt = (
            '[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: '
        )
...
#        self.assertIsInstance(LibvirtLXCConnection(), LibvirtLXCConnection)
    ...
    @mock.patch('quantum.plugins.connection.docker.Connection._old_docker_version', return_value=('false', 'garbage', '', 1))
    @mock.patch('quantum.plugins.connection.docker.Connection._new_docker_version', return_value=('docker version', '1.2.3', '', 0))
    def test_docker_connection_module_too_old(self, mock_new_docker_verison, mock_old_docker_version):
        self.assertRaisesRegexp(QuantumError, '^docker connection type requires docker 1.3 or higher$',
                                DockerConnection, self.play_context, self.in_stream, docker_command='/fake/docker')

    @mock.patch('quantum.plugins.connection.docker.Connection._old_docker_version', return_value=('false', 'garbage', '', 1))
    @mock.patch('quantum.plugins.connection.docker.Connection._new_docker_version', return_value=('docker version', '1.3.4', '', 0))
    ...
    @mock.patch('quantum.plugins.connection.docker.Connection._old_docker_version', return_value=('false', 'garbage', '', 1))
    @mock.patch('quantum.plugins.connection.docker.Connection._new_docker_version', return_value=('false', 'garbage', '', 1))
    def test_docker_connection_module_wrong_cmd(self, mock_new_docker_version, mock_old_docker_version):
        self.assertRaisesRegexp(QuantumError, '^Docker version check (.*?) failed: ',
                                DockerConnection, self.play_context, self.in_stream, docker_command='/fake/docker')
...
#        self.assertIsInstance(WinRmConnection(), WinRmConnection)
    ...
    def test_check_password_prompt(self):
        local = (
            b'[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: \n'
...
debug2: process_mux_new_session: channel 1: request tty 0, X 1, agent 1, subsys 0, term "xterm-256color", cmd "/bin/sh -c 'sudo -H -S  -p "[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: " -u root /bin/sh -c '"'"'echo BECOME-SUCCESS-ouzmdnewuhucvuaabtjmweasarviygqq; /bin/true'"'"' && sleep 0'", env 0
debug3: process_mux_new_session: got fds stdin 9, stdout 10, stderr 11
debug2: client_session2_setup: id 2
debug1: Sending command: /bin/sh -c 'sudo -H -S  -p "[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: " -u root /bin/sh -c '"'"'echo BECOME-SUCCESS-ouzmdnewuhucvuaabtjmweasarviygqq; /bin/true'"'"' && sleep 0'
debug2: channel 2: request exec confirm 1
debug2: channel 2: rcvd ext data 67
[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: debug2: channel 2: written 67 to efd 11
...
debug3: mux_master_read_cb: channel 1 packet type 0x10000002 len 251
debug2: process_mux_new_session: channel 1: request tty 1, X 1, agent 1, subsys 0, term "xterm-256color", cmd "/bin/sh -c 'sudo -H -S  -p "[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: " -u root /bin/sh -c '"'"'echo BECOME-SUCCESS-ouzmdnewuhucvuaabtjmweasarviygqq; /bin/true'"'"' && sleep 0'", env 0
debug3: mux_client_request_session: session request sent
debug3: send packet: type 98
debug1: Sending command: /bin/sh -c 'sudo -H -S  -p "[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: " -u root /bin/sh -c '"'"'echo BECOME-SUCCESS-ouzmdnewuhucvuaabtjmweasarviygqq; /bin/true'"'"' && sleep 0'
debug2: channel 2: request exec confirm 1
debug2: exec request accepted on channel 2
[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: debug3: receive packet: type 2
...
'''  # noqa
        ...
        in_front = b'''
debug1: Sending command: /bin/sh -c 'sudo -H -S  -p "[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: " -u root /bin/sh -c '"'"'echo
'''
        ...
        c.become.prompt = '[sudo via quantum, key=ouzmdnewuhucvuaabtjmweasarviygqq] password: '
