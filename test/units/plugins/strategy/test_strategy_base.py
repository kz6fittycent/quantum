...
class TestStrategyBase(unittest.TestCase):
    ...
    def test_strategy_base_process_pending_results(self):
        ...
        queue_items.append(TaskResult(host=mock_host.name, task=mock_task._uuid, return_data=dict(changed=True, _quantum_notify=['test handler'])))
        strategy_base._blocked_hosts['test01'] = True
        strategy_base._pending_results = 1
        strategy_base._queued_task_cache = deepcopy(mock_queued_task_cache)
        results = strategy_base._wait_on_pending_results(iterator=mock_iterator)
        ...
        # self.assertRaises(QuantumError, strategy_base._process_pending_results, iterator=mock_iterator)
        strategy_base.cleanup()
