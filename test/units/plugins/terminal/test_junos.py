...
from quantum.plugins.terminal import junos
...
def test_on_open_shell_raises_problem_setting_terminal_config(junos_terminal):
    junos_terminal._connection.exec_command.side_effect = QuantumConnectionFailure
    with pytest.raises(QuantumConnectionFailure) as exc:
        junos_terminal.on_open_shell()
