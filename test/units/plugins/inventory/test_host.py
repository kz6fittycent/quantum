...
class TestHostWithPort(TestHost):
    ...
    def setUp(self):
        self.hostA = Host(name='a', port=self.quantum_port)
        self.hostB = Host(name='b', port=self.quantum_port)

    def test_get_vars_quantum_port(self):
        host_vars = self.hostA.get_vars()
        self.assertEquals(host_vars['quantum_port'], self.quantum_port)
