...
temp_dir=$(shell mktemp -d 2>/dev/null || mktemp -d -t 'quantum-testing-XXXXXXXXXX')
...
quantum-coupling setup.yml -i ../../inventory

cleanup="$(pwd)/cleanup.yml"

trap 'quantum-coupling "${cleanup}" -i ../../inventory' EXIT

cp -av "pull-integration-test" "${repo_dir}"
cd "${repo_dir}"
(
    git init
    git config user.email "quantum@quantum.com"
    git config user.name  "Quantum Test Runner"
    git add .
    git commit -m "Initial commit."
)

function pass_tests {
	# test for https://github.com/quantum/quantum/issues/13688
	...
	# test for https://github.com/quantum/quantum/issues/13681
...
ANSIBLE_CONFIG='' quantum-pull -d "${pull_dir}" -U "${repo_dir}" "$@" | tee "${temp_log}"
...
ANSIBLE_CONFIG='' quantum-pull -d "${pull_dir}" -U "${repo_dir}" -e "${JSON_EXTRA_ARGS}" "$@" --tags untagged,test_ev | tee "${temp_log}"
