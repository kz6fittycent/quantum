...
# A recent patch to OpenSSH causes a validation error when running through Quantum. It seems like if the path is quoted
# then it will fail with 'protocol error: filename does not match request'. We currently ignore this by setting
# 'quantum_scp_extra_args=-T' to ignore this check but this should be removed once that bug is fixed and our test
...
INVENTORY="${OUTPUT_DIR}/test_connection.inventory" ./test.sh \
    ...
    -e quantum_scp_extra_args=-T \
    "$@"

cd ../connection_windows_ssh

quantum-coupling -i "${OUTPUT_DIR}/test_connection.inventory" tests_fetch.yml \
    -e quantum_scp_extra_args=-T \
    "$@"
