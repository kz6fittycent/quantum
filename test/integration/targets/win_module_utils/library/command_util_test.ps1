...
$exe = Get-QuantumParam -obj $params -name "exe" -type "path" -failifempty $true
...
$tmp_dir = Join-Path -Path $env:TEMP -ChildPath "quantum .ÅÑŚÌβŁÈ [$!@^&test(;)]"
try {
...
$actual = Run-Command -command "powershell.exe -ExecutionPolicy ByPass -EncodedCommand $encoded_wrapper" -stdin "Quantum"
Assert-Equals -actual $actual.stdout -expected "Quantum`n"
