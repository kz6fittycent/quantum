...
class InventoryModule(BaseFileInventoryPlugin):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not data:
            raise QuantumParserError('Parsed empty YAML file')
        elif not isinstance(data, MutableMapping):
            raise QuantumParserError('YAML inventory has invalid structure, it should be a dictionary, got: %s' % type(data))
        elif data.get('plugin'):
            raise QuantumParserError('Plugin configuration YAML file, not YAML inventory')
    ...
    def _parse_group(self, group, group_data):
        ...
        if isinstance(group_data, (MutableMapping, NoneType)):
            ...
            try:
                ...
                raise QuantumParserError("Unable to add group %s: %s" % (group, to_text(e)))
                ...
                for section in ['vars', 'children', 'hosts']:
                    if section in group_data:
                        ...
                        if not isinstance(group_data[section], (MutableMapping, NoneType)):
                            raise QuantumParserError('Invalid "%s" entry for "%s" group, requires a dictionary, found "%s" instead.' %
