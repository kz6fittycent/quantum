...
namespace Quantum.WinPhonebookEntry
{
...
$exists = [Quantum.WinPhonebookEntry.Phonebook]::IsValidEntry($name)
if ($exists) {
    $entry = [Quantum.WinPhonebookEntry.Phonebook]::GetEntry($name)
...
if ($state -eq "present") {
    ...
    $expected_type = switch ($type) {
        "broadband" { [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryTypes]::RASET_Broadband }
        "direct" { [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryTypes]::RASET_Direct }
        "phone" { [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryTypes]::RASET_Phone }
        "vpn" { [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryTypes]::RASET_Vpn }
    }

    $expected_framing_protocol = switch ($framing_protocol) {
        "ppp" { [Quantum.WinPhonebookEntry.NativeHelpers+RasFramingProtocol]::RASFP_Ppp }
        "ras" { [Quantum.WinPhonebookEntry.NativeHelpers+RasFramingProtocol]::RASFP_Ras }
        "slip" { [Quantum.WinPhonebookEntry.NativeHelpers+RasFramingProtocol]::RASFP_Slip }
    }
    ...
    $expected_options1 = [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryOptions]($expected_options1 -join ", ")
    $expected_options2 = [Quantum.WinPhonebookEntry.NativeHelpers+RasEntryOptions2]($expected_options2 -join ", ")
    ...
    if (-not $exists) {
        $entry = New-Object -TypeName Quantum.WinPhonebookEntry.NativeHelpers+RASENTRYW
        foreach ($kvp in $property_map.GetEnumerator()) {
            $entry."$($kvp.Key)" = $kvp.Value
        }

        [Quantum.WinPhonebookEntry.Phonebook]::CreateEntry($name, $entry)
    ...
    if ($exists) {
        [Quantum.WinPhonebookEntry.Phonebook]::DeleteEntry($name)
        $module.Result.changed = $true
    }
}
