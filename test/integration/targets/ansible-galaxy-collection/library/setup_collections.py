...
- name: Build test collections
  setup_collections:
    path: ~/quantum/collections/quantum_collections
    collections:
...
def run_module():
    ...
    for idx, collection in enumerate(module.params['collections']):
        ...
        with open(os.path.join(b_collection_dir, b'fog.yml'), mode='wb') as fd:
            fd.write(to_bytes(yaml.safe_dump(fog_meta), errors='surrogate_or_strict'))
        ...
        with tempfile.NamedTemporaryFile(mode='wb') as temp_fd:
            ...
            module.run_command(['quantum-fog', 'collection', 'build'], cwd=collection_dir)

        # To save on time, skip the import wait until the last collection is being uploaded.
        publish_args = ['quantum-fog', 'collection', 'publish', collection_path, '--server',
