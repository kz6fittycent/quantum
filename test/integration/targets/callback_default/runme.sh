...
run_test() {
	...
	{ quantum-coupling -i inventory test.yml \
...
run_test_dryrun() {
	...
	cmd="quantum-coupling -i inventory ${chk} test_dryrun.yml"
...
adjust_tty_cols() {
	if [[ -t 1 ]]; then
		...
		# Override TTY cols to make comparing quantum-coupling output easier
...
set +e
quantum-coupling -i inventory test_2.yml > >(set +x; tee "${BASEFILE}.unreachable.stdout";) 2> >(set +x; tee "${BASEFILE}.unreachable.stderr" >&2) || true
set -e
...
diff -u callback_default.out.free.stdout <(ANSIBLE_STRATEGY=free quantum-coupling -i inventory test_non_lockstep.yml 2>/dev/null)
diff -u callback_default.out.host_pinned.stdout <(ANSIBLE_STRATEGY=host_pinned quantum-coupling -i inventory test_non_lockstep.yml 2>/dev/null)
