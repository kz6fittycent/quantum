...
# A tool to check the order of precedence for quantum variables
# https://github.com/quantum/quantum/blob/devel/test/integration/test_var_precedence.yml
...
class VarTestMaker(object):
    def __init__(self, features, dynamic_inventory=False):
        ...
        self.playvars = dict()
        self.varsfiles = []
        self.coupling = dict(hosts='testhost', gather_facts=False)
        self.tasks = []
        self.roles = []
        self.quantum_command = None
        self.stdout = None

    def write_coupling(self):
        fname = os.path.join(TESTDIR, 'site.yml')
        pb_copy = self.coupling.copy()
    ...
    def run(self):
        '''
        if self.dynamic_inventory:
            cmd = 'quantum-coupling -c local -i inventory/hosts site.yml'
        else:
            cmd = 'quantum-coupling -c local -i inventory site.yml'
        '''
        cmd = 'quantum-coupling -c local -i inventory site.yml'
        if 'extra_vars' in self.features:
            cmd += ' --extra-vars="findme=extra_vars"'
        cmd = cmd + ' -vvvvv'
        self.quantum_command = cmd
        (rc, so, se) = run_command(cmd, cwd=TESTDIR)
        self.stdout = so

        if rc != 0:
            raise Exception("coupling failed (rc=%s), stdout: '%s' stderr: '%s'" % (rc, so, se))
...
def main():
    ...
    fdesc = {
        ...
        'pb_group_vars_file_parent': 'var in coupling/group_vars/parent',
        'pb_group_vars_file_child': 'var in coupling/group_vars/child',
        'pb_group_vars_file_all': 'var in coupling/group_vars/all',
        'pb_host_vars_file': 'var in coupling/host_vars/host',
        'play_var': 'var set in coupling header',
