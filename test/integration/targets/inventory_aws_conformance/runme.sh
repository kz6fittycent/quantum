...
ANSIBLE_JINJA2_NATIVE=1 quantum-inventory -vvvv -i ./ec2.sh --list --output="$OUTPUT_DIR/script.out"
RC=$?
if [[ $RC != 0 ]]; then
    exit $RC
fi
...
compose:
  ...
  # vars that change
  quantum_host: public_ip_address
...
ANSIBLE_JINJA2_NATIVE=1 quantum-inventory -vvvv -i "$OUTPUT_DIR/test.aws_ec2.yml" --list --output="$OUTPUT_DIR/plugin.out"
