...
#
ANSIBLE_CONFIG=no_filters.ini quantum-coupling copy.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
	echo "### Failed to run copy with no filters applied"
	exit 1
fi
ANSIBLE_CONFIG=no_filters.ini quantum-coupling pause.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
	echo "### Failed to run pause with no filters applied"
	exit 1
fi
ANSIBLE_CONFIG=no_filters.ini quantum-coupling tempfile.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
...
#
# Check that if no modules are blacklisted then Quantum should not through traceback
#
ANSIBLE_CONFIG=no_blacklist_module.ini quantum-coupling tempfile.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
...
#
ANSIBLE_CONFIG=filter_modules.ini quantum-coupling copy.yml -i ../../inventory -v "$@"
if test $? = 0 ; then
...
ANSIBLE_CONFIG=filter_modules.ini quantum-coupling pause.yml -i ../../inventory -v "$@"
if test $? = 0 ; then
...
ANSIBLE_CONFIG=filter_modules.ini quantum-coupling tempfile.yml -i ../../inventory -v "$@"
if test $? = 0 ; then
...
ANSIBLE_CONFIG=no_filters.ini quantum-coupling ping.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
...
ANSIBLE_CONFIG=filter_ping.ini quantum-coupling copy.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
	echo "### Failed to run copy when a filter was applied to ping"
	exit 1
fi
# Check that ping fails to run when it is filtered
ANSIBLE_CONFIG=filter_ping.ini quantum-coupling ping.yml -i ../../inventory -v "$@"
if test $? = 0 ; then
...
ANSIBLE_CONFIG=filter_lookup.ini quantum-coupling lookup.yml -i ../../inventory -vvv "$@"
if test $? != 0 ; then
...
ANSIBLE_CONFIG=no_filters.ini quantum-coupling stat.yml  -i ../../inventory -vvv "$@"
if test $? != 0 ; then
...
CAPTURE=$(quantum-coupling copy.yml  -i ../../inventory -vvv "$@" 2>&1)
if test $? = 0 ; then
	echo "### Copy ran even though stat is in the module blacklist"
	exit 1
else
	echo "$CAPTURE" | grep 'The stat module was specified in the module blacklist file,.*, but Quantum will not function without the stat module.  Please remove stat from the blacklist.'
	if test $? != 0 ; then
...
ANSIBLE_CONFIG=filter_stat.ini
export ANSIBLE_CONFIG
CAPTURE=$(quantum-coupling stat.yml  -i ../../inventory -vvv "$@" 2>&1)
if test $? = 0 ; then
