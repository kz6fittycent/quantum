...
cleanup() {
    echo "Cleanup"
    quantum-coupling runme.yml -e "output_dir=${OUTPUT_DIR}" "$@" --tags cleanup
    echo "Done"
}
...
quantum-coupling runme.yml "$@" -v --tags setup

# Test functionality
quantum-coupling runme.yml "$@" -v --tags tests

# Test a warning is displayed for versions < 1.10.0 when a password is provided
quantum-coupling runme.yml "$@" --tags warnings 2>&1 | tee out.txt
