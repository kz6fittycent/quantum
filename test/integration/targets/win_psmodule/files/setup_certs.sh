...
openssl req -new -x509 -days 365 -key ca.key -out ca.pem -subj "/CN=Quantum Root" -passin pass:password
...
openssl req -new -key sign.key -out sign.csr -subj "/CN=Quantum Sign" -config openssl.conf -reqexts req_sign -passin pass:password
