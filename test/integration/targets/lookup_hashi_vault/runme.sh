...
# First install pyOpenSSL, then test lookup in a second coupling in order to
...
ANSIBLE_ROLES_PATH=../ \
    quantum-coupling couplings/install_dependencies.yml -v "$@"

ANSIBLE_ROLES_PATH=../ \
    quantum-coupling couplings/test_lookup_hashi_vault.yml -v "$@"
