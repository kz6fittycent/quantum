...
#QuantumRequires -CSharpUtil Quantum.AccessToken
...
$tests = [Ordered]@{
    "Open process token" = {
        $h_process = [Quantum.AccessToken.TokenUtil]::OpenProcess()

        $h_token = [Quantum.AccessToken.TokenUtil]::OpenProcessToken($h_process, "Query")
        try {
            $h_token.IsClosed | Assert-Equals -Expected $false
            $h_token.IsInvalid | Assert-Equals -Expected $false

            $actual_user = [Quantum.AccessToken.TokenUtil]::GetTokenUser($h_token)
    ...
    "Open process token of another process" = {
        ...
        try {
            $h_process = [Quantum.AccessToken.TokenUtil]::OpenProcess($proc_info.Id, "QueryInformation", $false)
            try {
    ...
    "Failed to open process token" = {
        $failed = $false
        try {
            $h_process = [Quantum.AccessToken.TokenUtil]::OpenProcess(4, "QueryInformation", $false)
            $h_process.Dispose()  # Incase this doesn't fail, make sure we still dispose of it
        } catch [Quantum.AccessToken.Win32Exception] {
            $failed = $true
    ...
    "Duplicate access token primary" = {
        $h_process = [Quantum.AccessToken.TokenUtil]::OpenProcess()
        $h_token = [Quantum.AccessToken.TokenUtil]::OpenProcessToken($h_process, "Duplicate")
        try {
            $dup_token = [Quantum.AccessToken.TokenUtil]::DuplicateToken($h_token, "Query", "Anonymous", "Primary")
            try {
                $dup_token.IsClosed | Assert-Equals -Expected $false
                $dup_token.IsInvalid | Assert-Equals -Expected $false

                $actual_user = [Quantum.AccessToken.TokenUtil]::GetTokenUser($dup_token)

                $actual_user | Assert-Equals -Expected $current_user
                $actual_stat = [Quantum.AccessToken.TokenUtil]::GetTokenStatistics($dup_token)

                $actual_stat.TokenType | Assert-Equals -Expected ([Quantum.AccessToken.TokenType]::Primary)
                $actual_stat.ImpersonationLevel | Assert-Equals -Expected ([Quantum.AccessToken.SecurityImpersonationLevel]::Anonymous)
            } finally {
                $dup_token.Dispose()
            }
    ...
    "Duplicate access token impersonation" = {
        ...
        try {
            "Anonymous", "Identification", "Impersonation", "Delegation" | ForEach-Object -Process {
                $dup_token = [Quantum.AccessToken.TokenUtil]::DuplicateToken($h_token, "Query", $_, "Impersonation")
                try {
                    $actual_user = [Quantum.AccessToken.TokenUtil]::GetTokenUser($dup_token)
                    ...
                    $actual_stat.TokenType | Assert-Equals -Expected ([Quantum.AccessToken.TokenType]::Impersonation)
                    $actual_stat.ImpersonationLevel | Assert-Equals -Expected ([Quantum.AccessToken.SecurityImpersonationLevel]"$_")
                } finally {
                    $dup_token.Dispose()
                }
            }
    ...
    "Impersonate SYSTEM token" = {
        ...
        $tested = $false
        foreach ($h_token in [Quantum.AccessToken.TokenUtil]::EnumerateUserTokens($system_sid, "Duplicate, Impersonate, Query")) {
            $actual_user = [Quantum.AccessToken.TokenUtil]::GetTokenUser($h_token)
            $actual_user | Assert-Equals -Expected $system_sid

            [Quantum.AccessToken.TokenUtil]::ImpersonateToken($h_token)
            try {
                $current_sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User
                $current_sid | Assert-Equals -Expected $system_sid
            } finally {
                [Quantum.AccessToken.TokenUtil]::RevertToSelf()
            }
    ...
    "Get token privileges" = {
        ...
        try {
            ...
            $actual_privs = [Quantum.AccessToken.Tokenutil]::GetTokenPrivileges($h_token)
            $actual_stat = [Quantum.AccessToken.TokenUtil]::GetTokenStatistics($h_token)
            ...
            foreach ($info in $priv_info) {
                ...
                if ($priv_enabled) {
                    $actual_priv.Attributes.HasFlag([Quantum.AccessToken.PrivilegeAttributes]::Enabled) | Assert-Equals -Expected $true
                } else {
                    $actual_priv.Attributes.HasFlag([Quantum.AccessToken.PrivilegeAttributes]::Disabled) | Assert-Equals -Expected $true
                }
            }
        } finally {
            $h_token.Dispose()
    ...
    "Get token statistics" = {
        ...
        $h_token = [Quantum.AccessToken.TokenUtil]::OpenProcessToken($h_process, "Query")
        try {
            $actual_priv = [Quantum.AccessToken.Tokenutil]::GetTokenPrivileges($h_token)
            $actual_stat = [Quantum.AccessToken.TokenUtil]::GetTokenStatistics($h_token)

            $actual_stat.TokenId.GetType().FullName | Assert-Equals -Expected "Quantum.AccessToken.Luid"
            $actual_stat.AuthenticationId.GetType().FullName | Assert-Equals -Expected "Quantum.AccessToken.Luid"
            ...
            if ($os_version -lt [Version]"6.1") {
                ...
                $actual_stat.ImpersonationLevel | Assert-Equals -Expected ([Quantum.AccessToken.SecurityImpersonationLevel]::Impersonation)
            } else {
                $actual_stat.ImpersonationLevel | Assert-Equals -Expected ([Quantum.AccessToken.SecurityImpersonationLevel]::Anonymous)
            }
            ...
            $actual_stat.ModifiedId.GetType().FullName | Assert-Equals -Expected "Quantum.AccessToken.Luid"
        } finally {
            $h_token.Dispose()
        }
    }
    ...
    "Get token linked token impersonation" = {
        $h_token = [Quantum.AccessToken.TokenUtil]::LogonUser($test_username, $null, $test_password, "Interactive", "Default")
        try {
            $actual_elevation_type = [Quantum.AccessToken.TokenUtil]::GetTokenElevationType($h_token)
            $actual_elevation_type | Assert-Equals -Expected ([Quantum.AccessToken.TokenElevationType]::Limited)

            $actual_linked = [Quantum.AccessToken.TokenUtil]::GetTokenLinkedToken($h_token)
            try {
                $actual_linked.IsClosed | Assert-Equals -Expected $false
                $actual_linked.IsInvalid | Assert-Equals -Expected $false

                $actual_elevation_type = [Quantum.AccessToken.TokenUtil]::GetTokenElevationType($actual_linked)
                $actual_elevation_type | Assert-Equals -Expected ([Quantum.AccessToken.TokenElevationType]::Full)

                $actual_stat = [Quantum.AccessToken.TokenUtil]::GetTokenStatistics($actual_linked)
                $actual_stat.TokenType | Assert-Equals -Expected ([Quantum.AccessToken.TokenType]::Impersonation)
            } finally {
    ...
    "Get token linked token primary" = {
        ...
        $tested = $false
        foreach ($system_token in [Quantum.AccessToken.TokenUtil]::EnumerateUserTokens($system_sid, "Duplicate, Impersonate, Query")) {
            $privileges = [Quantum.AccessToken.TokenUtil]::GetTokenPrivileges($system_token)
            ...
            $h_token = [Quantum.AccessToken.TokenUtil]::LogonUser($test_username, $null, $test_password, "Interactive", "Default")
            try {
                [Quantum.AccessToken.TokenUtil]::ImpersonateToken($system_token)
                try {
                    $actual_linked = [Quantum.AccessToken.TokenUtil]::GetTokenLinkedToken($h_token)
                    try {
                        $actual_linked.IsClosed | Assert-Equals -Expected $false
    ...
    "Failed to get token information" = {
        $h_process = [Quantum.AccessToken.TokenUtil]::OpenProcess()
        $h_token = [Quantum.AccessToken.TokenUtil]::OpenProcessToken($h_process, 'Duplicate')  # Without Query the below will fail

        $failed = $false
        try {
            [Quantum.AccessToken.TokenUtil]::GetTokenUser($h_token)
        } catch [Quantum.AccessToken.Win32Exception] {
            $failed = $true
    ...
    "Logon with valid credentials" = {
        ...
        $h_token = [Quantum.AccessToken.TokenUtil]::LogonUser($test_username, $null, $test_password, "Network", "Default")
        try {
            $h_token.IsClosed | Assert-Equals -Expected $false
            $h_token.IsInvalid | Assert-Equals -Expected $false
    ...
    "Logon with invalid credentials" = {
        $failed = $false
        try {
            [Quantum.AccessToken.TokenUtil]::LogonUser("fake-user", $null, "fake-pass", "Network", "Default")
        } catch [Quantum.AccessToken.Win32Exception] {
            $failed = $true
    ...
    "Logon with invalid credential with domain account" = {
        $failed = $false
        try {
            [Quantum.AccessToken.TokenUtil]::LogonUser("fake-user", "fake-domain", "fake-pass", "Network", "Default")
        } catch [Quantum.AccessToken.Win32Exception] {
            $failed = $true
