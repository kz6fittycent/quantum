...
#QuantumRequires -CSharpUtil Quantum.Process
...
$tests = @{
    "ParseCommandLine empty string" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::ParseCommandLine("")
        Assert-Equals -Actual $actual -Expected $expected
    }

    "ParseCommandLine single argument" = {
        $expected = @("powershell.exe")
        $actual = [Quantum.Process.ProcessUtil]::ParseCommandLine("powershell.exe")
        Assert-Equals -Actual $actual -Expected $expected
    }

    "ParseCommandLine multiple arguments" = {
        $expected = @("powershell.exe", "-File", "C:\temp\script.ps1")
        $actual = [Quantum.Process.ProcessUtil]::ParseCommandLine("powershell.exe -File C:\temp\script.ps1")
        Assert-Equals -Actual $actual -Expected $expected
    }

    "ParseCommandLine comples arguments" = {
        $expected = @('abc', 'd', 'ef gh', 'i\j', 'k"l', 'm\n op', 'ADDLOCAL=qr, s', 'tuv\', 'w''x', 'yz')
        $actual = [Quantum.Process.ProcessUtil]::ParseCommandLine('abc d "ef gh" i\j k\"l m\\"n op" ADDLOCAL="qr, s" tuv\ w''x yz')
        Assert-Equals -Actual $actual -Expected $expected
    }

    "SearchPath normal" = {
        $expected = "$($env:SystemRoot)\System32\WindowsPowerShell\v1.0\powershell.exe"
        $actual = [Quantum.Process.ProcessUtil]::SearchPath("powershell.exe")
    ...
    "SearchPath missing" = {
        $failed = $false
        try {
            [Quantum.Process.ProcessUtil]::SearchPath("fake.exe")
        } catch {
            $failed = $true
    ...
    "CreateProcess basic" = {
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess("whoami.exe")
        $actual.GetType().FullName | Assert-Equals -Expected "Quantum.Process.Result"
    ...
    "CreateProcess stderr" = {
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess("powershell.exe [System.Console]::Error.WriteLine('hi')")
    ...
    "CreateProcess exit code" = {
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess("powershell.exe exit 10")
        $actual.StandardOut | Assert-Equals -Expected ""
        $actual.StandardError | Assert-Equals -Expected ""
    ...
    "CreateProcess bad executable" = {
        $failed = $false
        try {
            [Quantum.Process.ProcessUtil]::CreateProcess("fake.exe")
        } catch {
            $failed = $true
    ...
    "CreateProcess with unicode" = {
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess("cmd.exe /c echo 💩 café")
        $actual.StandardOut | Assert-Equals -Expected "💩 café`r`n"
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0

        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, "cmd.exe /c echo 💩 café", $null, $null)
        $actual.StandardOut | Assert-Equals -Expected "💩 café`r`n"
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0
    }
    ...
    "CreateProcess without working dir" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $pwd.Path', $null, $null)
    ...
    "CreateProcess with working dir" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $pwd.Path', "C:\Windows", $null)
        $actual.StandardOut | Assert-Equals -Expected $expected
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0
    }
    ...
    "CreateProcess without environment" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $env:TEST; $env:USERNAME', $null, $null)
        $actual.StandardOut | Assert-Equals -Expected $expected
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0
    }
    ...
    "CreateProcess with environment" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, 'cmd.exe /c set', $null, $env_vars)
    ...
    "CreateProcess with string stdin" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe [System.Console]::In.ReadToEnd()',
    ...
    "CreateProcess with lpApplicationName" = {
        ...
        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($full_path, "Write-Output 'abc'", $null, $null)
        $actual.StandardOut | Assert-Equals -Expected $expected
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0

        $actual = [Quantum.Process.ProcessUtil]::CreateProcess($full_path, "powershell.exe Write-Output 'abc'", $null, $null)
        $actual.StandardOut | Assert-Equals -Expected $expected
        $actual.StandardError | Assert-Equals -Expected ""
        $actual.ExitCode | Assert-Equals -Expected 0
    }
