...
Function Exit-Module {
    ...
    [Quantum.Basic.QuantumModule]::Exit = { param([Int32]$rc) exit $rc }
...
[Quantum.Basic.QuantumModule]::Exit = {
    param([Int32]$rc)
...
[Quantum.Basic.QuantumModule]::WriteLine = {
    param([String]$line)
...
$tests = @{
    "Empty spec and no options - args file" = {
        ...
        $m = [Quantum.Basic.QuantumModule]::Create(@($args_file), @{})
        ...
        $m.QuantumVersion | Assert-Equals -Expected $null
    }

    "Empty spec and no options - complex_args" = {
        $complex_args = @{}
        $m = [Quantum.Basic.QuantumModule]::Create(@(), @{})
...
{
    "ANSIBLE_MODULE_ARGS": {
        "_quantum_check_mode": true,
        "_quantum_debug": true,
        "_quantum_diff": true,
        "_quantum_keep_remote_files": true,
        "_quantum_module_name": "quantum_basic_tests",
        "_quantum_no_log": true,
        "_quantum_remote_tmp": "%TEMP%",
        "_quantum_selinux_special_fs": "ignored",
        "_quantum_shell_executable": "ignored",
        "_quantum_socket": "ignored",
        "_quantum_syslog_facility": "ignored",
        "_quantum_tmpdir": "$($m_tmpdir -replace "\\", "\\")",
        "_quantum_verbosity": 3,
        "_quantum_version": "2.8.0"
    }
}
"@)
        $m = [Quantum.Basic.QuantumModule]::Create(@($args_file), @{supports_check_mode=$true})
        ...
        $m.ModuleName | Assert-Equals -Expected "quantum_basic_tests"
        $m.NoLog | Assert-Equals -Expected $true
        $m.Verbosity | Assert-Equals -Expected 3
        $m.QuantumVersion | Assert-Equals -Expected "2.8.0"
    ...
    "Internal param changes - complex_args" = {
        ...
        $complex_args = @{
            ...
            _quantum_socket = "ignored"
            _quantum_syslog_facility = "ignored"
            _quantum_tmpdir = $m_tmpdir.ToString()
            _quantum_verbosity = 3
            _quantum_version = "2.8.0"
        }
        $spec = @{
        ...
        $m = [Quantum.Basic.QuantumModule]::Create(@(), $spec)
        $m.CheckMode | Assert-Equals -Expected $true
        $m.DebugMode | Assert-Equals -Expected $true
        $m.DiffMode | Assert-Equals -Expected $true
        $m.KeepRemoteFiles | Assert-Equals -Expected $true
    ...
    "Parse complex module options" = {
        ...
        [Quantum.Basic.QuantumModule]::FromJson($m.Params.json_type_dict) | Assert-DictionaryEquals -Expected ([Quantum.Basic.QuantumModule]::FromJson('{"a":"a","b":1,"c":["a","b"]}'))
        ...
        try {
            ...
            $actual = [Quantum.Basic.QuantumModule]::FromJson($_.Exception.InnerException.Output)
        }
    ...
    "Parse module args with case insensitive input" = {
        ...
        $expected_warnings += "Module options will become case sensitive in a future Quantum release. "
    ...
    "No log values" = {
        ...
        $complex_args = @{
            _quantum_module_name = "test_no_log"
...
test_no_log - Invoked with:
  ...
  password: VALUE_SPECIFIED_IN_NO_LOG_PARAMETER
'@
        $actual_event = (Get-EventLog -LogName Application -Source Quantum -Newest 1).Message
    ...
    "No log value with an empty string" = {
        ...
        try {
            ...
            $failed = $true
            $_.Exception.Message | Assert-Equals -Expected "exit: 0"
            $actual = [Quantum.Basic.QuantumModule]::FromJson($_test_out)
        }
        $failed | Assert-Equals -Expected $true
    ...
    "Deprecate and warn" = {
        ...
        $actual_deprecate_event = Get-EventLog -LogName Application -Source Quantum -Newest 1
        $m.Warn("warning")
        $actual_warn_event = Get-EventLog -LogName Application -Source Quantum -Newest 1
    ...
    "ParseBool tests" = {
        ...
        foreach ($map in $mapping.GetEnumerator()) {
            $expected = $map.Value
            $actual = [Quantum.Basic.QuantumModule]::ParseBool($map.Key)
        ...
        foreach ($fail_bool in $fail_bools) {
            $failed = $false
            try {
                [Quantum.Basic.QuantumModule]::ParseBool($fail_bool)
            } catch {
                $failed = $true
    ...
    "Unknown internal key" = {
        ...
        try {
            ...
            $expected = @{
                ...
                changed = $false
                failed = $true
                msg = "Unsupported parameters for (undefined win module) module: _quantum_invalid. Supported parameters include: "
            }
            $actual = [Quantum.Basic.QuantumModule]::FromJson($_.Exception.InnerException.Output)
            $actual | Assert-DictionaryEquals -Expected $expected
        }
    ...
    "Module tmpdir with present remote tmp" = {
        ...
        $complex_args = @{
            _quantum_remote_tmp = $remote_tmp.ToString()
        }
        $m = [Quantum.Basic.QuantumModule]::Create(@(), @{})
        ...
        $tmpdir_name.StartSwith("quantum-moduletmp-") | Assert-Equals -Expected $true
        ...
        try {
            ...
            $output = [Quantum.Basic.QuantumModule]::FromJson($_.Exception.InnerException.Output)
        }
        ...
        $spec = @{
            options = @{
                sub_options = @{
                    ...
                    # Quantum.Basic automatically sets that to $false and we want it to ignore it for a nested check
                    type = "dict"
                    options = @{
    ...
    "Case insensitive choice" = {
        ...
        $expected_warning += "Checking of choices will be case sensitive in a future Quantum release. "
    ...
    "String json array to object" = {
        $input_json = '["abc", "def"]'
        $actual = [Quantum.Basic.QuantumModule]::FromJson($input_json)
...
try {
    ...
    if ($_.Exception.Message.StartSwith("exit: ")) {
        ...
        $module.Result.msg = "Uncaught QuantumModule exit in tests, see output"
    } else {
        # Unrelated exception
