...
$dev_info_set = [Quantum.Device.NativeMethods]::SetupDiGetClassDevsW(
    [Guid]::Empty,
    [NullString]::Value,
    [System.IntPtr]::Zero,
    [Quantum.Device.NativeHelpers+GetClassFlags]::DIGCF_ALLCLASSES
...
try {
    ...
    if ($null -ne $name) {
        ...
        $idx = 0
        while ($true) {
            $dev_info = New-Object -TypeName Quantum.Device.NativeHelpers+SP_DEVINFO_DATA
            $res = [Quantum.Device.NativeMethods]::SetupDiEnumDeviceInfo(
                $dev_info_set,
                $idx,
                $dev_info
            ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()
            ...
            $device_name = [Quantum.Device.DeviceUtil]::GetDeviceFriendlyName($dev_info_set, $dev_info)
    ...
    if ($state -eq "absent" -and $null -ne $dev_info) {
        if (-not $module.CheckMode) {
            $res = [Quantum.Device.NativeMethods]::SetupDiCallClassInstaller(
                [Quantum.Device.NativeHelpers+DifCodes]::DIF_REMOVE,
                $dev_info_set,
                $dev_info
            ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()
        ...
        if ($path) {
            ...
            $required_size = 0
            $res = [Quantum.Device.NativeMethods]::SetupDiGetINFClassW(
                $path,
                [ref]$class_id,
                $class_name_sb,
        ...
        $dev_info_set = [Quantum.Device.NativeMethods]::SetupDiCreateDeviceInfoList(
            $class_id,
            [System.IntPtr]::Zero
        ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()
        ...
        $dev_info = New-Object -TypeName Quantum.Device.NativeHelpers+SP_DEVINFO_DATA
        $res = [Quantum.Device.NativeMethods]::SetupDiCreateDeviceInfoW(
            $dev_info_set,
            $class_name,
            $class_id,
            $null,
            [System.IntPtr]::Zero,
            [Quantum.Device.NativeHelpers+DeviceInfoCreationFlags]::DICD_GENERATE_ID,
            $dev_info
        ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()
        ...
        [Quantum.Device.DeviceUtil]::SetDeviceHardwareId($dev_info_set, $dev_info, $hardware_id)

        if (-not $module.CheckMode) {
            # Install the device
            $res = [Quantum.Device.NativeMethods]::SetupDiCallClassInstaller(
                [Quantum.Device.NativeHelpers+DifCodes]::DIF_REGISTERDEVICE,
                $dev_info_set,
                $dev_info
            ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()
            ...
            $res = [Quantum.Device.NativeMethods]::UpdateDriverForPlugAndPlayDevicesW(
                [System.IntPtr]::Zero,
                $hardware_id,
                $path,
                [Quantum.Device.NativeHelpers+InstallFlags]'INSTALLFLAG_FORCE, INSTALLFLAG_NONINTERACTIVE',
                [ref]$reboot_required
            ); $err = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if (-not $res) {
                # On a failure make sure we cleanup the "installed" device
                [Quantum.Device.NativeMethods]::SetupDiCallClassInstaller(
                    [Quantum.Device.NativeHelpers+DifCodes]::DIF_REMOVE,
                    $dev_info_set,
                    $dev_info
                ) > $null
            ...
            # Now get the name of the newly created device which we return back to Quantum.
            $name = [Quantum.Device.DeviceUtil]::GetDeviceFriendlyName($dev_info_set, $dev_info)
        } else {
