"""Test runner for all Quantum tests."""
...
def parse_args():
    ...
    try:
        ...
        if '--requirements' not in sys.argv:
            raise
        raw_command(generate_pip_install(generate_pip_command(sys.executable), 'quantum-test'))
        import argparse
    ...
    common.add_argument('--debug',
                        action='store_true',
                        help='run quantum commands in debug mode')
    ...
    integration.add_argument('--debug-strategy',
                             action='store_true',
                             help='run test couplings using the debug strategy')
    ...
    env.add_argument('--timeout',
                     type=int,
                     metavar='MINUTES',
                     help='timeout for future quantum-test commands (0 clears)')
...
def add_environments(parser, tox_version=False, tox_only=False):
    ...
    environments.add_argument('--venv',
                              action='store_true',
                              help='run from quantum-test managed virtual environments')
...
def add_httptester_options(parser, argparse):
    ...
    group.add_argument('--httptester',
                       metavar='IMAGE',
                       default='quay.io/quantum/http-test-container:1.0.0',
