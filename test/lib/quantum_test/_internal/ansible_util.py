"""Miscellaneous utility functions and classes specific to quantum cli tools."""
...
def quantum_environment(args, color=True, quantum_config=None):
    """
    ...
    if not quantum_config:
        ...
        quantum_config = args.get_quantum_config()

    if not args.explain and not os.path.exists(quantum_config):
        raise ApplicationError('Configuration not found: %s' % quantum_config)
    ...
    quantum = dict(
        ...
        ANSIBLE_CONFIG=quantum_config,
        ANSIBLE_LIBRARY='/dev/null',
        PYTHONPATH=get_quantum_python_path(),
        PAGER='/bin/cat',
        PATH=path,
    )
    ...
    if isinstance(args, PosixIntegrationConfig):
        quantum.update(dict(
            ANSIBLE_PYTHON_INTERPRETER='/set/quantum_python_interpreter/in/inventory',  # force tests to set quantum_python_interpreter in inventory
        ))
...
def get_quantum_python_path():  # type: () -> str
    """
    Return a directory usable for PYTHONPATH, containing only the quantum package.
    If a temporary directory is required, it will be cached for the lifetime of the process and cleaned up at exit.
    """
    if ANSIBLE_SOURCE_ROOT:
        # when running from source there is no need for a temporary directory to isolate the quantum package
        return os.path.dirname(ANSIBLE_LIB_ROOT)

    try:
        return get_quantum_python_path.python_path
    except AttributeError:
        pass

    python_path = create_temp_dir(prefix='quantum-test-')
    get_quantum_python_path.python_path = python_path

    os.symlink(ANSIBLE_LIB_ROOT, os.path.join(python_path, 'quantum'))
