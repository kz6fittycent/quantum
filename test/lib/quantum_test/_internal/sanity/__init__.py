...
def collect_code_smell_tests():  # type: () -> t.Tuple[SanityFunc, ...]
    ...
    if data_context().content.is_quantum:
        # include Quantum specific code-smell tests which are not configured to be skipped
        quantum_code_smell_root = os.path.join(data_context().content.root, 'test', 'sanity', 'code-smell')
        skip_tests = read_lines_without_comments(os.path.join(quantum_code_smell_root, 'skip.txt'), remove_blank_lines=True, optional=True)
        paths.extend(path for path in glob.glob(os.path.join(quantum_code_smell_root, '*.py')) if os.path.basename(path) not in skip_tests)
...
class SanityIgnoreParser:
    ...
    def __init__(self, args):  # type: (SanityConfig) -> None
        if data_context().content.collection:
            quantum_version = '%s.%s' % tuple(get_quantum_version().split('.')[:2])

            quantum_label = 'Quantum %s' % quantum_version
            file_name = 'ignore-%s.txt' % quantum_version
        else:
        ...
        display.info('Read %d sanity test ignore line(s) for %s from: %s' % (len(lines), quantum_label, self.relative_path), verbosity=1)
...
class SanityTest(ABC):
    ...
    def error_code(self):  # type: () -> t.Optional[str]
        """Error code for quantum-test matching the format used by the underlying test program, or None if the program does not use error codes."""
        return None
...
class SanityCodeSmellTest(SanityTest):
    ...
    def filter_targets(self, targets):  # type: (t.List[TestTarget]) -> t.List[TestTarget]
        ...
        if self.ignore_self and data_context().content.is_quantum:
...
def sanity_init():
    ...
    sanity_tests = tuple([plugin() for plugin in sanity_plugins.values() if data_context().content.is_quantum or not plugin.quantum_only])
