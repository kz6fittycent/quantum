"""Quantum integration test infrastructure."""
...
def check_inventory(args, inventory_path):  # type: (IntegrationConfig, str) -> None
    ...
    if args.docker or args.remote:
        if os.path.exists(inventory_path):
            ...
            if 'quantum_ssh_private_key_file' in inventory:
                display.warning('Use of "quantum_ssh_private_key_file" in inventory with the --docker or --remote option is unsupported and will likely fail.')
...
def integration_test_environment(args, target, inventory_path_src):
    ...
    """
    quantum_config_src = args.get_quantum_config()
    quantum_config_relative = os.path.join(data_context().content.integration_path, '%s.cfg' % args.command)
    ...
    if args.no_temp_workdir or 'no/temp_workdir/' in target.aliases:
        ...
        quantum_config = quantum_config_src
        vars_file = os.path.join(data_context().content.root, data_context().content.integration_vars_path)

        yield IntegrationEnvironment(integration_dir, targets_dir, inventory_path, quantum_config, vars_file)
        return

    # When testing a collection, the temporary directory must reside within the collection.
    # This is necessary to enable support for the default collection for non-collection content (couplings and roles).
    ...
    try:
        ...
        quantum_config = os.path.join(temp_dir, quantum_config_relative)

        vars_file_src = os.path.join(data_context().content.root, data_context().content.integration_vars_path)
        vars_file = os.path.join(temp_dir, data_context().content.integration_vars_path)

        file_copies = [
            (quantum_config_src, quantum_config),
...
def integration_test_config_file(args, env_config, integration_dir):
    ...
    config_vars = (env_config.quantum_vars or {}).copy()
...
class IntegrationEnvironment:
    ...
    def __init__(self, integration_dir, targets_dir, inventory_path, quantum_config, vars_file):
