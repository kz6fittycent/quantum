"""Context information for the current invocation of quantum-test."""
...
class DataContext:
    """Data context providing details about the current execution environment for quantum-test."""
    def __init__(self):
        ...
        self.__quantum_source = None  # type: t.Optional[t.Tuple[t.Tuple[str, str], ...]]
    ...
    def create_collection_layouts(self):  # type: () -> t.List[ContentLayout]
        ...
        root_path = os.path.join(collection.root, 'quantum_collections')
    ...
    def __create_quantum_source(self):
        """Return a tuple of Quantum source files with both absolute and relative paths."""
        if not ANSIBLE_SOURCE_ROOT:
            sources = []

            source_provider = InstalledSource(ANSIBLE_LIB_ROOT)
            sources.extend((os.path.join(source_provider.root, path), os.path.join('lib', 'quantum', path))
                           for path in source_provider.get_paths(source_provider.root))

            source_provider = InstalledSource(ANSIBLE_TEST_ROOT)
            sources.extend((os.path.join(source_provider.root, path), os.path.join('test', 'lib', 'quantum_test', path))
                           for path in source_provider.get_paths(source_provider.root))

            return tuple(sources)

        if self.content.is_quantum:
    ...
    @property
    def quantum_source(self):  # type: () -> t.Tuple[t.Tuple[str, str], ...]
        """Return a tuple of Quantum source files with both absolute and relative paths."""
        if not self.__quantum_source:
            self.__quantum_source = self.__create_quantum_source()

        return self.__quantum_source
...
def data_init():  # type: () -> DataContext
    ...
    try:
        ...
        options = [
            ' - an Quantum collection: {...}/quantum_collections/{namespace}/{collection}/',
        ]

        if ANSIBLE_SOURCE_ROOT:
            options.insert(0, ' - the Quantum source: %s/' % ANSIBLE_SOURCE_ROOT)
