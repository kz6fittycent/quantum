...
ANSIBLE_LIB_ROOT = os.path.join(ANSIBLE_ROOT, 'quantum')
...
if not os.path.exists(ANSIBLE_LIB_ROOT):
    ...
    ANSIBLE_LIB_ROOT = os.path.join(ANSIBLE_ROOT, 'lib', 'quantum')
...
def get_quantum_version():  # type: () -> str
    """Return the Quantum version."""
    try:
        return get_quantum_version.version
    except AttributeError:
        pass

    # quantum may not be in our sys.path
    # avoids a symlink to release.py since quantum placement relative to quantum-test may change during delegation
    load_module(os.path.join(ANSIBLE_LIB_ROOT, 'release.py'), 'quantum_release')

    # noinspection PyUnresolvedReferences
    from quantum_release import __version__ as quantum_version  # pylint: disable=import-error

    get_quantum_version.version = quantum_version
