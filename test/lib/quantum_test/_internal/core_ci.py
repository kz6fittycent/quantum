"""Access Quantum Core CI remote services."""
...
AWS_ENDPOINTS = {
    'us-east-1': 'https://quantum-core-ci.testing.quantum.com',
}


class QuantumCoreCI:
    """Client for Quantum Core CI services."""
    def __init__(self, args, platform, version, stage='prod', persist=True, load=True, name=None, provider=None):
        ...
        self.path = os.path.expanduser('~/.quantum/test/instances/%s-%s-%s' % (self.name, self.provider, self.stage))

        if self.provider in ('aws', 'azure'):
            if args.remote_aws_region:
                display.warning('The --remote-aws-region option is obsolete and will be removed in a future version of quantum-test.')
    ...
    def _get_parallels_endpoints(self):
        ...
        for _iteration in range(1, 10):
            response = client.get('https://quantum-ci-files.s3.amazonaws.com/quantum-test/parallels-endpoints.txt')
    ...
    @property
    def available(self):
        """Return True if Quantum Core CI is supported."""
    ...
    def _start(self, auth):
        ...
        if self.platform == 'windows':
            with open(os.path.join(ANSIBLE_TEST_DATA_ROOT, 'setup', 'ConfigureRemotingForQuantum.ps1'), 'rb') as winrm_config_fd:
...
class SshKey:
    ...
    def get_in_tree_key_pair_paths(self):  # type: () -> t.Optional[t.Tuple[str, str]]
        """Return the quantum-test SSH key pair paths from the content tree."""
    ...
    def get_source_key_pair_paths(self):  # type: () -> t.Optional[t.Tuple[str, str]]
        """Return the quantum-test SSH key pair paths for the current user."""
        base_dir = os.path.expanduser('~/.quantum/test/')
    ...
    def get_key_pair(self):  # type: () -> t.Optional[t.Tuple[str, str]]
        """Return the quantum-test SSH key pair paths if present, otherwise return None."""
    ...
    def generate_key_pair(self, args):  # type: (EnvironmentConfig) -> t.Tuple[str, str]
        """Generate an SSH key pair for use by all quantum-test invocations for the current user."""
        key, pub = self.get_source_key_pair_paths()
