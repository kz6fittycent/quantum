...
def get_python_module_utils_name(path):  # type: (str) -> str
    ...
    if data_context().content.collection:
        prefix = 'quantum_collections.' + data_context().content.collection.prefix + 'plugins.module_utils'
    else:
        prefix = 'quantum.module_utils'
...
def get_import_path(name, package=False):  # type: (str, bool) -> str
    ...
    if name.startswith('quantum.module_utils.') or name == 'quantum.module_utils':
        path = os.path.join('lib', filename)
    elif data_context().content.collection and (
            name.startswith('quantum_collections.%s.plugins.module_utils.' % data_context().content.collection.full_name) or
            name == 'quantum_collections.%s.plugins.module_utils' % data_context().content.collection.full_name):
...
class ModuleUtilFinder(ast.NodeVisitor):
    ...
    def __init__(self, path, module_utils):
        ...
        if path.startswith('lib/quantum/module_utils/'):
            package = os.path.split(path)[0].replace('/', '.')[4:]

            if package != 'quantum.module_utils' and package not in VIRTUAL_PACKAGES:
                self.add_import(package, 0)

        self.module = None

        if data_context().content.is_quantum:
            # Various parts of the Quantum source tree execute within diffent modules.
            ...
            path_map = (
                ('^hacking/build_library/build_quantum/', 'build_quantum/'),
                ('^lib/quantum/', 'quantum/'),
                ('^test/lib/quantum_test/_data/sanity/validate-modules/', 'validate_modules/'),
                ('^test/units/', 'test/units/'),
                ('^test/lib/quantum_test/_internal/', 'quantum_test/_internal/'),
                ('^test/integration/targets/.*/quantum_collections/(?P<ns>[^/]*)/(?P<col>[^/]*)/', r'quantum_collections/\g<ns>/\g<col>/'),
                ('^test/integration/targets/.*/library/', 'quantum/modules/'),
            )
            ...
            # This assumes that all files within the collection are executed by Quantum as part of the collection.
    ...
    def visit_Import(self, node):
        ...
        # import quantum.module_utils.MODULE[.MODULE]
        # import quantum_collections.{ns}.{col}.plugins.module_utils.module_utils.MODULE[.MODULE]
    ...
    def visit_ImportFrom(self, node):
        ...
        if not module.startswith('quantum'):
            return

        # from quantum.module_utils import MODULE[, MODULE]
        # from quantum.module_utils.MODULE[.MODULE] import MODULE[, MODULE]
        # from quantum_collections.{ns}.{col}.plugins.module_utils import MODULE[, MODULE]
        # from quantum_collections.{ns}.{col}.plugins.module_utils.MODULE[.MODULE] import MODULE[, MODULE]
    ...
    def is_module_util_name(name):  # type: (str) -> bool
        ...
        if data_context().content.is_quantum and name.startswith('quantum.module_utils.'):
            return True

        if data_context().content.collection and name.startswith('quantum_collections.%s.plugins.module_utils.' % data_context().content.collection.full_name):
            return True
