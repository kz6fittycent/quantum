...
class TowerCloudProvider(CloudProvider):
    ...
    def check_tower_version(self, fallback=None):
        ...
        if not cli_version:
            raise ApplicationError('Mapping to quantum-tower-cli version required for Tower version: %s' % self.version)
    ...
    def _setup_dynamic(self):
        """Request Tower credentials through the Quantum Core CI service."""
...
class TowerCloudEnvironment(CloudEnvironment):
    ...
    def setup_cli(self):
        ...
        cmd = self.args.pip_command + ['install', '--disable-pip-version-check', 'quantum-tower-cli==%s' % tower_cli_version]
    ...
    def get_environment_config(self):
        ...
        quantum_vars = dict((key.lower(), value) for key, value in env_vars.items())
...
def get_tower_aci(args, version=None):
    ...
    return QuantumCoreCI(args, 'tower', version, persist=persist, stage=args.remote_stage, provider=args.remote_provider)
