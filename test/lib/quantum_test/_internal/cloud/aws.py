...
class AwsCloudProvider(CloudProvider):
    ...
    def filter(self, targets, exclude):
        ...
        aci = self._create_quantum_core_ci()
    ...
    def _setup_dynamic(self):
        """Request AWS credentials through the Quantum Core CI service."""
    ...
    def _create_quantum_core_ci(self):
        """
        :rtype: QuantumCoreCI
        """
        return QuantumCoreCI(self.args, 'aws', 'sts', persist=False, stage=self.args.remote_stage, provider=self.args.remote_provider)
...
class AwsCloudEnvironment(CloudEnvironment):
    ...
    def get_environment_config(self):
        ...
        quantum_vars.update(dict(parser.items('default')))

        display.sensitive.add(quantum_vars.get('aws_secret_key'))
        display.sensitive.add(quantum_vars.get('security_token'))

        if 'aws_cleanup' not in quantum_vars:
            quantum_vars['aws_cleanup'] = not self.managed

        env_vars = {'ANSIBLE_DEBUG_BOTOCORE_LOGS': 'True'}

        return CloudEnvironmentConfig(
            env_vars=env_vars,
            quantum_vars=quantum_vars,
    ...
    def on_failure(self, target, tries):
        ...
        if not tries and self.managed:
            display.notice('If %s failed due to permissions, the IAM test policy may need to be updated. '
                           'For help, consult @mattclay or @gundalow on GitHub or #quantum-devel on IRC.' % target.name)
