...
def delegate_tox(args, exclude, require, integration_targets):
    ...
    for version in versions:
        ...
        # passing this option will allow the delegated quantum-test to verify it is running under the expected python version
        ...
        # temporary solution to permit quantum-test delegated to tox to provision remote resources
        optional = (
            'SHIPPABLE',
            'SHIPPABLE_BUILD_ID',
...
def delegate_venv(args,  # type: EnvironmentConfig
    ...
    """Delegate quantum-test execution to a virtual environment using venv or virtualenv."""
    if args.python:
        versions = (args.python_version,)
    else:
        versions = SUPPORTED_PYTHON_VERSIONS
    ...
    with tempdir() as inject_path:
        ...
        with tempdir() as library_path:
            # expose quantum and quantum_test to the virtual environment (only required when running from an install)
            os.symlink(ANSIBLE_LIB_ROOT, os.path.join(library_path, 'quantum'))
            os.symlink(ANSIBLE_TEST_ROOT, os.path.join(library_path, 'quantum_test'))
...
def delegate_docker(args, exclude, require, integration_targets):
    ...
    with tempfile.NamedTemporaryFile(prefix='quantum-source-', suffix='.tgz') as local_source_fd:
        try:
            ...
            docker_put(args, test_id, local_source_fd.name, '/root/quantum.tgz')
            docker_exec(args, test_id, ['mkdir', '/root/quantum'])
            docker_exec(args, test_id, ['tar', 'oxzf', '/root/quantum.tgz', '-C', '/root/quantum'])
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(prefix='quantum-result-', suffix='.tgz') as local_result_fd:
...
def delegate_remote(args, exclude, require, integration_targets):
    ...
    try:
        ...
        if platform == 'windows':
            # Windows doesn't need the quantum-test fluff, just run the SSH command
            ...
            install_root = os.path.join(pwd, 'quantum')
...
def generate_command(args, python_interpreter, quantum_bin_path, content_root, options, exclude, require):
    """
    :type args: EnvironmentConfig
    :type python_interpreter: str | None
    :type quantum_bin_path: str
    ...
    cmd = [os.path.join(quantum_bin_path, 'quantum-test')]

    if python_interpreter:
        cmd = [python_interpreter] + cmd

    # Force the encoding used during delegation.
    # This is only needed because quantum-test relies on Python's file system encoding.
