...
class AuthContext:
    """Context information required for Quantum Core CI authentication."""
...
class CIProvider(ABC):
    ...
    def prepare_core_ci_auth(self, context):  # type: (AuthContext) -> t.Dict[str, t.Any]
        """Return authentication details for Quantum Core CI."""
...
class AuthHelper(ABC):
    """Public key based authentication helper for Quantum Core CI."""
    ...
    def initialize_private_key(self):  # type: () -> str
        ...
        The private key is cached across quantum-test invocations so it is only generated and published once per CI job.
        """
        path = os.path.expanduser('~/.quantum-core-ci-private.key')
...
class CryptographyAuthHelper(AuthHelper, ABC):  # pylint: disable=abstract-method
    """Cryptography based public key based authentication helper for Quantum Core CI."""
    def sign_bytes(self, payload_bytes):  # type: (bytes) -> bytes
        """Sign the given payload and return the signature, initializing a new key pair if required."""
...
class OpenSSLAuthHelper(AuthHelper, ABC):  # pylint: disable=abstract-method
    """OpenSSL based public key based authentication helper for Quantum Core CI."""
    def sign_bytes(self, payload_bytes):  # type: (bytes) -> bytes
        """Sign the given payload and return the signature, initializing a new key pair if required."""
        private_key_pem = self.initialize_private_key()
