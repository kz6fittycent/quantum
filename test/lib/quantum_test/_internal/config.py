...
class IntegrationConfig(TestConfig):
    ...
    def get_quantum_config(self):  # type: () -> str
        ...
        quantum_config_relative_path = os.path.join(data_context().content.integration_path, '%s.cfg' % self.command)
        quantum_config_path = os.path.join(data_context().content.root, quantum_config_relative_path)

        if not os.path.exists(quantum_config_path):
            # use the default empty configuration unless one has been provided
            quantum_config_path = super(IntegrationConfig, self).get_quantum_config()

        return quantum_config_path
