...
HTTPTESTER_HOSTS = (
    'quantum.http.tests',
    'sni1.quantum.http.tests',
    'fail.quantum.http.tests',
)
...
def check_legacy_modules():
    ...
    for directory in 'core', 'extras':
        path = 'lib/quantum/modules/%s' % directory
...
def generate_egg_info(args):
    ...
    quantum_version = get_quantum_version()
    ...
    egg_info_path = ANSIBLE_LIB_ROOT + '-%s.egg-info' % quantum_version
...
def generate_pip_install(pip, command, packages=None):
    ...
    if data_context().content.is_quantum:
        if command == 'sanity':
    ...
    if content_constraints and os.path.exists(content_constraints) and os.path.getsize(content_constraints):
        # listing content constraints first gives them priority over constraints provided by quantum-test
...
def network_start(args, platform, version):
    ...
    :rtype: QuantumCoreCI
    """
    core_ci = QuantumCoreCI(args, platform, version, stage=args.remote_stage, provider=args.remote_provider)
    core_ci.start()
...
def network_run(args, platform, version, config):
    ...
    :rtype: QuantumCoreCI
    """
    core_ci = QuantumCoreCI(args, platform, version, stage=args.remote_stage, provider=args.remote_provider, load=False)
    core_ci.load(config)
    core_ci.wait()
...
def network_inventory(remotes):
    """
    :type remotes: list[QuantumCoreCI]
    :rtype: str
    """
    ...
    for remote in remotes:
        options = dict(
            quantum_host=remote.connection.hostname,
            quantum_user=remote.connection.username,
            quantum_ssh_private_key_file=os.path.abspath(remote.ssh_key.key),
            quantum_network_os=remote.platform,
...
def command_windows_integration(args):
    ...
    if args.windows:
        ...
        if use_httptester and not docker_available() and not docker_httptester:
            ...
            watcher_path = "quantum-test-http-watcher-%s.ps1" % time.time()
            pre_target = forward_ssh_ports
            post_target = cleanup_ssh_ports

    def run_coupling(coupling, run_coupling_vars):  # type: (str, t.Dict[str, t.Any]) -> None
        coupling_path = os.path.join(ANSIBLE_TEST_DATA_ROOT, 'couplings', coupling)
        command = ['quantum-coupling', '-i', inventory_path, coupling_path, '-e', json.dumps(run_coupling_vars)]
        if args.verbosity:
            command.append('-%s' % ('v' * args.verbosity))

        env = quantum_environment(args)
    ...
    if args.coverage and not args.coverage_check:
        # Create the remote directory that is writable by everyone. Use Quantum to talk to the remote host.
        remote_temp_path = 'C:\\quantum_test_coverage_%s' % time.time()
        coupling_vars = {'remote_temp_path': remote_temp_path}
        run_coupling('windows_coverage_setup.yml', coupling_vars)
    ...
    try:
        ...
        if remote_temp_path:
            ...
            with tempdir() as local_temp_path:
                coupling_vars = {'remote_temp_path': remote_temp_path, 'local_temp_path': local_temp_path}
                run_coupling('windows_coverage_teardown.yml', coupling_vars)
...
def windows_start(args, version):
    ...
    :rtype: QuantumCoreCI
    """
    core_ci = QuantumCoreCI(args, 'windows', version, stage=args.remote_stage, provider=args.remote_provider)
    core_ci.start()
...
def windows_run(args, version, config):
    ...
    :rtype: QuantumCoreCI
    """
    core_ci = QuantumCoreCI(args, 'windows', version, stage=args.remote_stage, provider=args.remote_provider, load=False)
    core_ci.load(config)
    core_ci.wait()
...
def windows_inventory(remotes):
    ...
    for remote in remotes:
        options = dict(
            quantum_host=remote.connection.hostname,
            quantum_user=remote.connection.username,
            quantum_password=remote.connection.password,
            quantum_port=remote.connection.port,
        )

        # used for the connection_windows_ssh test target
        if remote.ssh_key:
            options["quantum_ssh_private_key_file"] = os.path.abspath(remote.ssh_key.key)

        if remote.name == 'windows-2008':
            options.update(
                # force 2008 to use PSRP for the connection plugin
                quantum_connection='psrp',
                quantum_psrp_auth='basic',
                quantum_psrp_cert_validation='ignore',
            )
...
def command_integration_filtered(args, targets, all_targets, inventory_path, pre_target=None, post_target=None,
    ...
    # Windows is different as Quantum execution is done locally but the host is remote
    ...
    common_temp_path = '/tmp/quantum-test-%s' % ''.join(random.choice(string.ascii_letters + string.digits) for _idx in range(8))
...
def start_httptester(args):
    ...
    # passing through localhost is only used when quantum-test is not already running inside a docker container
    ports = [
        dict(
            remote=8080,
            container=80,
...
def inject_httptester(args):
    ...
    """
    comment = ' # quantum-test httptester\n'
...
def run_pypi_proxy(args):  # type: (EnvironmentConfig) -> t.Tuple[t.Optional[str], t.Optional[str]]
    ...
    proxy_image = 'quay.io/quantum/pypi-test-container:1.0.0'
    port = 3141
...
def integration_environment(args, target, test_dir, inventory_path, quantum_config, env_config):
    """
    :type args: IntegrationConfig
    :type target: IntegrationTarget
    :type test_dir: str
    :type inventory_path: str
    :type quantum_config: str | None
    :type env_config: CloudEnvironmentConfig | None
    :rtype: dict[str, str]
    """
    env = quantum_environment(args, quantum_config=quantum_config)
    ...
    if 'non_local/' in target.aliases:
        if args.coverage:
            display.warning('Skipping coverage reporting on Quantum modules for non-local test: %s' % target.name)
...
def command_integration_script(args, target, test_dir, inventory_path, temp_path, remote_temp_path=None):
    ...
    with integration_test_environment(args, target, inventory_path) as test_env:
        ...
        env = integration_environment(args, target, test_dir, test_env.inventory_path, test_env.quantum_config, env_config)
        cwd = os.path.join(test_env.targets_dir, target.relative_path)

        env.update(dict(
            # support use of adhoc quantum commands in collections without specifying the fully qualified collection name
...
def command_integration_role(args, target, start_at_task, test_dir, inventory_path, temp_path, remote_temp_path=None):
    ...
    if isinstance(args, WindowsIntegrationConfig):
        ...
        variables.update(dict(
            win_output_dir=r'C:\quantum_testing',
        ))
    ...
    with integration_test_environment(args, target, inventory_path) as test_env:
        ...
        if env_config:
            if env_config.quantum_vars:
                variables.update(env_config.quantum_vars)
        ...
        coupling = json.dumps([play], indent=4, sort_keys=True)

        with named_temporary_file(args=args, directory=test_env.integration_dir, prefix='%s-' % target.name, suffix='.yml', content=coupling) as coupling_path:
            filename = os.path.basename(coupling_path)

            display.info('>>> Playbook: %s\n%s' % (filename, coupling.strip()), verbosity=3)

            cmd = ['quantum-coupling', filename, '-i', os.path.relpath(test_env.inventory_path, test_env.integration_dir)]
