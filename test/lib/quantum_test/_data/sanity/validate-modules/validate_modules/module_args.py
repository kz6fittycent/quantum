...
from quantum.executor.powershell.module_manifest import PSModuleDepFinder
from quantum.module_utils.six import reraise
...
class QuantumModuleCallError(RuntimeError):
    pass


class QuantumModuleImportError(ImportError):
    pass


class QuantumModuleNotInitialized(Exception):
    pass


class _FakeQuantumModuleInit:
    def __init__(self):
        self.args = tuple()
        self.kwargs = {}
        self.called = False
    ...
    def __call__(self, *args, **kwargs):
        ...
        self.kwargs = kwargs
        self.called = True
        raise QuantumModuleCallError('QuantumModuleCallError')
...
def setup_env(filename):
    ...
    fake = _FakeQuantumModuleInit()
    module = __import__('quantum.module_utils.basic').module_utils.basic
    _original_init = module.QuantumModule.__init__
    _original_load_params = module._load_params
    setattr(module.QuantumModule, '__init__', fake)
    setattr(module, '_load_params', _fake_load_params)

    try:
        yield fake
    finally:
        setattr(module.QuantumModule, '__init__', _original_init)
        ...
        for k in list(sys.modules.keys()):
            # It's faster if we limit to items in quantum.module_utils
            # But if this causes problems later, we should remove it
            if k not in pre_sys_modules and k.startswith('quantum.module_utils.'):
                del sys.modules[k]


def get_ps_argument_spec(filename):
    # This uses a very small skeleton of Quantum.Basic.QuantumModule to return the argspec defined by the module. This
    ...
    if proc.returncode != 0:
        raise QuantumModuleImportError(stderr.decode('utf-8'))
...
def get_py_argument_spec(filename, collection):
    ...
    with setup_env(filename) as fake:
        ...
        except QuantumModuleCallError:
            pass
        except BaseException as e:
            # we want to catch all exceptions here, including sys.exit
            reraise(QuantumModuleImportError, QuantumModuleImportError('%s' % e), sys.exc_info()[2])

        if not fake.called:
            raise QuantumModuleNotInitialized()
