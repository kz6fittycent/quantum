...
class QuantumTextIOWrapper(TextIOWrapper):
    def write(self, s):
        super(QuantumTextIOWrapper, self).write(to_text(s, self.encoding, errors='replace'))
...
class CaptureStd():
    ...
    def __enter__(self):
        ...
        sys.stdout = self.stdout = QuantumTextIOWrapper(BytesIO(), encoding=self.sys_stdout.encoding)
        sys.stderr = self.stderr = QuantumTextIOWrapper(BytesIO(), encoding=self.sys_stderr.encoding)
        return self
...
def get_module_name_from_filename(filename, collection):
    ...
    if collection:
        # collection is a relative path, example: quantum_collections/my_namespace/my_collection
        # filename is a relative path, example: plugins/modules/my_module.py
        path = os.path.join(collection, filename)
    else:
        # filename is a relative path, example: lib/quantum/modules/system/ping.py
...
class NoArgsQuantumModule(QuantumModule):
    """QuantumModule that does not actually load params. This is used to get access to the
    methods within QuantumModule without having to fake a bunch of data
    """
    def _load_params(self):
        self.params = {'_quantum_selinux_special_fs': [], '_quantum_remote_tmp': '/tmp', '_quantum_keep_remote_files': False, '_quantum_check_mode': False}
