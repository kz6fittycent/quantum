...
class QuantumBlacklistChecker(BaseChecker):
    """Checker for blacklisted imports and functions."""
    __implements__ = (IAstroidChecker,)

    name = 'blacklist'

    BAD_IMPORT = 'quantum-bad-import'
    BAD_IMPORT_FROM = 'quantum-bad-import-from'
    BAD_FUNCTION = 'quantum-bad-function'
    BAD_MODULE_IMPORT = 'quantum-bad-module-import'
    ...
    msgs = dict(
        ...
        E5104=('Import external package or quantum.module_utils not %s',
               BAD_MODULE_IMPORT,
               'Identifies imports which should not be used.'),
    )
    ...
    blacklist_imports = dict(
        ...
        # requests=BlacklistEntry('quantum.module_utils.urls', modules_only=True),
        # urllib=BlacklistEntry('quantum.module_utils.urls', modules_only=True),

        # see https://docs.python.org/2/library/urllib2.html
        urllib2=BlacklistEntry('quantum.module_utils.urls',
                               ignore_paths=(
                                   '/lib/quantum/module_utils/urls.py',
                               )),

        # see https://docs.python.org/3.7/library/collections.abc.html
        collections=BlacklistEntry('quantum.module_utils.common._collections_compat',
                                   ignore_paths=(
                                       '/lib/quantum/module_utils/common/_collections_compat.py',
                                   ),
                                   names=(
                                       'MappingView',
                                       'ItemsView',
    ...
    blacklist_functions = {
        ...
        'sys.exit': BlacklistEntry('exit_json or fail_json',
                                   ignore_paths=(
                                       '/lib/quantum/module_utils/basic.py',
                                       '/lib/quantum/modules/utilities/logic/async_wrapper.py',
                                       '/lib/quantum/module_utils/common/removed.py',
                                   ),
                                   modules_only=True),
    }
    ...
    def _check_module_import(self, node, modname):
        ...
        if modname == 'quantum.module_utils' or modname.startswith('quantum.module_utils.'):
            return

        if modname == 'quantum' or modname.startswith('quantum.'):
            self.add_message(self.BAD_MODULE_IMPORT, args=(modname,), node=node)


def register(linter):
    """required method to auto register this checker """
    linter.register_checker(QuantumBlacklistChecker(linter))
