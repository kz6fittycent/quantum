...
on the Quantum host. This will setup the Windows host file and forward the
...
httptester container, e.g. 'quantum.host.com|secondary.host.test'.
#>
[CmdletBinding()]
param(
...
foreach ($httptester_host in $Hosts) {
    $host_line = "127.0.0.1 $httptester_host # quantum-test httptester"
...
if ($os_version -ge [Version]"6.2") {
    ...
    $s3_url = "https://quantum-ci-files.s3.amazonaws.com/quantum-test/pmapper-1.04.exe"
...
foreach ($host_line in $hosts_file_lines) {
    if ($host_line.EndsWith("# quantum-test httptester")) {
        $changed = $true
        continue
    }
