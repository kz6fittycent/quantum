...
def main():
    ...
    quantum_path = os.environ['PYTHONPATH']
    ...
    # pre-load an empty quantum package to prevent unwanted code in __init__.py from loading
    # without this the quantum.release import there would pull in many Python modules which Quantum modules should not have access to
    quantum_module = types.ModuleType('quantum')
    quantum_module.__file__ = os.path.join(os.environ['PYTHONPATH'], 'quantum', '__init__.py')
    quantum_module.__path__ = [os.path.dirname(quantum_module.__file__)]
    quantum_module.__package__ = 'quantum'

    sys.modules['quantum'] = quantum_module
    ...
    class ImporterQuantumModuleException(Exception):
        """Exception thrown during initialization of ImporterQuantumModule."""

    class ImporterQuantumModule:
        """Replacement for QuantumModule to support import testing."""
        def __init__(self, *args, **kwargs):
            raise ImporterQuantumModuleException()
    ...
    class ImportBlacklist:
        ...
        def find_module(self, fullname, path=None):
            ...
            if is_name_in_namepace(fullname, ['quantum']):
                if fullname in ('quantum.module_utils.basic', 'quantum.module_utils.common.removed'):
                    return self  # intercept loading so we can modify the result

                if is_name_in_namepace(fullname, ['quantum.module_utils', self.name]):
                    return None  # module_utils and module under test are always allowed

                if os.path.exists(convert_quantum_name_to_absolute_path(fullname)):
                    return self  # blacklist quantum files that exist

                return None  # quantum file does not exist, do not blacklist

            if is_name_in_namepace(fullname, ['quantum_collections']):
                if not collection_loader:
                    return self  # blacklist collections when we are not testing a collection

                if is_name_in_namepace(fullname, ['quantum_collections...plugins.module_utils', self.name]):
                    return None  # module_utils and module under test are always allowed
        ...
        def load_module(self, fullname):
            ...
            :type fullname: str
            """
            if fullname == 'quantum.module_utils.basic':
                module = self.__load_module(fullname)

                # stop Quantum module execution during QuantumModule instantiation
                module.QuantumModule = ImporterQuantumModule
                # no-op for _load_params since it may be called before instantiating QuantumModule
                module._load_params = lambda *args, **kwargs: {}  # pylint: disable=protected-access

                return module

            if fullname == 'quantum.module_utils.common.removed':
                module = self.__load_module(fullname)

                # no-op for removed_module since it is called in place of QuantumModule instantiation
    ...
    def test_python_module(path, name, base_dir, messages):
        ...
        is_quantum_module = (path.startswith('lib/quantum/modules/') or path.startswith('plugins/modules/')) and os.path.basename(path) != '__init__.py'
        run_main = is_quantum_module

        if path == 'lib/quantum/modules/utilities/logic/async_wrapper.py':
            # async_wrapper is a non-standard Quantum module (does not use QuantumModule) so we cannot test the main function
            run_main = False
        ...
        except ImporterQuantumModuleException:
            # module instantiated QuantumModule without raising an exception
            pass
    ...
    def is_name_in_namepace(name, namespaces):
        ...
        for namespace in namespaces:
            ...
            # example: name=quantum, allowed_name=quantum.module_utils
            # example: name=quantum.module_utils.system.ping, allowed_name=quantum.module_utils
    ...
    def convert_quantum_name_to_absolute_path(name):
        """Calculate the module path from the given name.
        :type name: str
        :rtype: str
        """
        return os.path.join(quantum_path, name.replace('.', os.path.sep))
    ...
    def convert_relative_path_to_name(path):
        ...
        if collection_loader:
            ...
            name = 'quantum_collections.%s.%s' % (collection_full_name, name)
        else:
            # when testing quantum all files being imported reside under the lib directory
            name = name[len('lib/'):]
    ...
    def monitor_sys_modules(path, messages):
        ...
        try:
            ...
            for key in set(sys.modules.keys()) - set(snapshot.keys()):
                if is_name_in_namepace(key, ('quantum', 'quantum_collections')):
