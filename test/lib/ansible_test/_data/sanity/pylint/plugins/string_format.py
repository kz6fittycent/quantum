...
class QuantumStringFormatChecker(BaseChecker):
    ...
    def _check_new_format(self, node, func):
        ...
        if _PY3K and isinstance(strnode.value, six.binary_type):
            self.add_message('quantum-no-format-on-bytestring', node=node)
            return
        ...
        if num_args:
            self.add_message('quantum-format-automatic-specification',
                             node=node)
            return
...
def register(linter):
    ...
    linter.register_checker(QuantumStringFormatChecker(linter))
