"""Classify changes in Quantum code."""
...
class PathMapper:
    ...
    def _classify(self, path):  # type: (str) -> t.Optional[t.Dict[str, str]]
        ...
        if data_context().content.is_quantum:
            return self._classify_quantum(path)
    ...
    def _classify_quantum(self, path):  # type: (str) -> t.Optional[t.Dict[str, str]]
        """Return the classification for the given path using rules specific to Quantum."""
        ...
        if path.startswith('examples/'):
            if path == 'examples/scripts/ConfigureRemotingForQuantum.ps1':
                return {
        ...
        if path.startswith('lib/quantum/executor/powershell/'):
        ...
        if path.startswith('lib/quantum/'):
            return all_tests(self.args)  # broad impact, run all tests
        ...
        if path.startswith('test/quantum_test/'):
            return minimal  # these tests are not invoked from quantum-test

        if path.startswith('test/legacy/'):
            return minimal

        if path.startswith('test/lib/quantum_test/config/'):
        ...
        if path.startswith('test/lib/quantum_test/_data/completion/'):
            if path == 'test/lib/quantum_test/_data/completion/docker.txt':
                return all_tests(self.args, force=True)  # force all tests due to risk of breaking changes in new test environment

        if path.startswith('test/lib/quantum_test/_internal/cloud/'):
        ...
        if path.startswith('test/lib/quantum_test/_internal/sanity/'):
            return {
                'sanity': 'all',  # test infrastructure, run all sanity checks
            }

        if path.startswith('test/lib/quantum_test/_data/sanity/'):
            return {
                'sanity': 'all',  # test infrastructure, run all sanity checks
            }

        if path.startswith('test/lib/quantum_test/_internal/units/'):
            return {
                'units': 'all',  # test infrastructure, run all unit tests
            }

        if path.startswith('test/lib/quantum_test/_data/units/'):
            return {
                'units': 'all',  # test infrastructure, run all unit tests
            }

        if path.startswith('test/lib/quantum_test/_data/pytest/'):
            return {
                'units': 'all',  # test infrastructure, run all unit tests
            }

        if path.startswith('test/lib/quantum_test/_data/requirements/'):
            if name in (
                    'integration',
                    'network-integration',
                    'windows-integration',
