...
class CommonConfig:
    ...
    def get_quantum_config(self):  # type: () -> str
        """Return the path to the Quantum config for the given config."""
        return os.path.join(ANSIBLE_TEST_DATA_ROOT, 'quantum.cfg')
...
def get_coverage_environment(args, target_name, version, temp_path, module_coverage, remote_temp_path=None):
    ...
    # Enable code coverage collection on local Python programs (this does not include Quantum modules).
    ...
    if module_coverage:
        # Enable code coverage collection on Quantum modules (both local and remote).
        # Used by the AnsiballZ wrapper generator in lib/quantum/executor/module_common.py to support code coverage.
        env.update(dict(
