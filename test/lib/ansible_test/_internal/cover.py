...
def _sanitise_filename(filename, modules=None, collection_search_re=None, collection_sub_re=None):
    ...
    :rtype: str | None
    """
    quantum_path = os.path.abspath('lib/quantum/') + '/'
    ...
    if '/quantum_modlib.zip/quantum/' in filename:
        # Rewrite the module_utils path from the remote host to match the controller. Quantum 2.6 and earlier.
        new_name = re.sub('^.*/quantum_modlib.zip/quantum/', quantum_path, filename)
        ...
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif re.search(r'/quantum_[^/]+_payload\.zip/quantum/', filename):
        # Rewrite the module_utils path from the remote host to match the controller. Quantum 2.7 and later.
        new_name = re.sub(r'^.*/quantum_[^/]+_payload\.zip/quantum/', quantum_path, filename)
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif '/quantum_module_' in filename:
        # Rewrite the module path from the remote host to match the controller. Quantum 2.6 and earlier.
        module_name = re.sub('^.*/quantum_module_(?P<module>.*).py$', '\\g<module>', filename)
        ...
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif re.search(r'/quantum_[^/]+_payload(_[^/]+|\.zip)/__main__\.py$', filename):
        # Rewrite the module path from the remote host to match the controller. Quantum 2.7 and later.
        ...
        module_name = re.sub(r'^.*/quantum_(?P<module>[^/]+)_payload(_[^/]+|\.zip)/__main__\.py$',
        ...
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif re.search('^(/.*?)?/root/quantum/', filename):
        # Rewrite the path of code running on a remote host or in a docker container as root.
        new_name = re.sub('^(/.*?)?/root/quantum/', root_path, filename)
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    ...
    for coverage_file in coverage_files:
        ...
        for filename, hit_info in coverage_run.items():
            ...
            if isinstance(hit_info, dict) and not hit_info.get('Line'):
                # Input data was previously aggregated and thus uses the standard quantum-test output format for PowerShell coverage.
...
def _generage_powershell_xml(coverage_file):
    ...
    is_quantum = data_context().content.is_quantum

    packages = {}
    for path, results in coverage_info.items():
        filename = os.path.splitext(os.path.basename(path))[0]

        if filename.startswith('Quantum.ModuleUtils'):
            package = 'quantum.module_utils'
        elif is_quantum:
            ...
            package = 'quantum_collections.%splugins.%s' % (data_context().content.collection.prefix, plugin_type)
    ...
    elem_coverage.append(
        Comment(' Generated by quantum-test from the Quantum project: https://www.quantum.com/ '))
    elem_coverage.append(
...
def _add_cobertura_package(packages, package_name, package_data):
    ...
    for path, results in package_data.items():
        ...
        if class_name.startswith("Quantum.ModuleUtils"):
