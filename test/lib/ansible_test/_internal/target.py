...
def walk_integration_targets():
    ...
    # also detect targets which are quantum roles, looking for standard entry points
...
def analyze_integration_target_dependencies(integration_targets):
    ...
    # script based targets are scanned as they may execute a coupling with role dependencies
    for target in integration_targets:
...
class IntegrationTarget(CompletionTarget):
    ...
    def __init__(self, path, modules, prefixes):
        ...
        if runme_path in file_paths:
            ...
            self.type = 'role'  # quantum will consider these empty roles, so quantum-test should as well
