...
class ManageWindowsCI:
    """Manage access to a Windows instance provided by Quantum Core CI."""
    def __init__(self, core_ci):
        """
        :type core_ci: QuantumCoreCI
        """
    ...
    def wait(self):
        """Wait for instance to respond to quantum ping."""
        extra_vars = [
            'quantum_connection=winrm',
            'quantum_host=%s' % self.core_ci.connection.hostname,
            'quantum_user=%s' % self.core_ci.connection.username,
            'quantum_password=%s' % self.core_ci.connection.password,
            'quantum_port=%s' % self.core_ci.connection.port,
            'quantum_winrm_server_cert_validation=ignore',
        ]

        name = 'windows_%s' % self.core_ci.version

        env = quantum_environment(self.core_ci.args)
        cmd = ['quantum', '-m', 'win_ping', '-i', '%s,' % name, name, '-e', ' '.join(extra_vars)]
...
class ManageNetworkCI:
    """Manage access to a network instance provided by Quantum Core CI."""
    def __init__(self, core_ci):
        """
        :type core_ci: QuantumCoreCI
        """
    ...
    def wait(self):
        ...
        extra_vars = [
            ...
            'quantum_ssh_private_key_file=%s' % self.core_ci.ssh_key.key,
        ]
...
class ManagePosixCI:
    """Manage access to a POSIX instance provided by Quantum Core CI."""
    def __init__(self, core_ci):
        """
        :type core_ci: QuantumCoreCI
        """
    ...
    def setup(self, python_version):
        """Start instance and wait for it to become ready and respond to an quantum ping.
        :type python_version: str
        :rtype: str
        """
        pwd = self.wait()
    ...
    def upload_source(self):
        ...
        with tempfile.NamedTemporaryFile(prefix='quantum-source-', suffix='.tgz') as local_source_fd:
            ...
            self.ssh('rm -rf ~/quantum && mkdir ~/quantum && cd ~/quantum && tar oxzf %s' % remote_source_path)
