...
def main():
    ...
    original = 'examples/scripts/ConfigureRemotingForQuantum.ps1'
    # required to be packaged with quantum-test and must match the original file, but cannot be a symbolic link
    # the packaged version is needed to run tests when quantum-test has been installed
    # keeping the packaged version identical to the original makes sure tests cover both files
    packaged = 'test/lib/quantum_test/_data/setup/ConfigureRemotingForQuantum.ps1'
