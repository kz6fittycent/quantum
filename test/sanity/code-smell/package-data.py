...
def assemble_files_to_ship(complete_file_list):
    ...
    # Manpages
    manpages = ['docs/man/man1/quantum.1']
    for dirname, dummy, files in os.walk('bin'):
        for filename in files:
            path = os.path.join(dirname, filename)
            if os.path.islink(path):
                if os.readlink(path) == 'quantum':
...
def assemble_files_to_install(complete_file_list):
    """
    This looks for all of the files which should show up in an installation of quantum
    """
    ...
    for path in complete_file_list:
        ...
        if path.startswith("lib/quantum"):
            prefix = 'lib'
        elif path.startswith("test/lib/quantum_test"):
            prefix = 'test/lib'
        else:
            continue
...
def install_sdist(tmp_dir, sdist_dir):
    ...
    match = re.search('^creating (%s/.*?/(?:site|dist)-packages)/quantum$' %
    ...
    for dirname, dummy, files in os.walk(sdist_dir):
        ...
        for filename in files:
            ...
            if path not in to_ship_files:
                ...
                # FIXME: quantum-test doesn't pass the paths of symlinks to us so we aren't
                # checking those
...
EGG_RE = re.compile('quantum[^/]+\\.egg-info/(PKG-INFO|SOURCES.txt|'
    ...
    for dirname, dummy, files in os.walk(install_dir):
        ...
        for filename in files:
            ...
            if path not in to_install_files:
                # FIXME: quantum-test doesn't pass the paths of symlinks to us so we
...
def main():
    ...
    # quantum-test isn't currently passing symlinks to us so construct those ourselves for now
    for filename in _find_symlinks():
        if filename not in complete_file_list:
            # For some reason quantum-test is passing us lib/quantum/module_utils/quantum_release.py
