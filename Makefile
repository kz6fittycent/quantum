...
# useful targets:
#   make clean ---------------- clean up
#   make webdocs -------------- produce quantum doc at docs/docsite/_build/html
...
#   make tests ---------------- run the tests (see https://docs.quantum.com/quantum/devel/dev_guide/testing_units.html for requirements)
...
MANPAGES ?= $(patsubst %.rst.in,%,$(wildcard ./docs/man/man1/quantum*.1.rst.in))
...
GENERATE_CLI = hacking/build-quantum.py generate-man
...
RPMSPEC = $(RPMSPECDIR)/quantum.spec
...
# quantum-test parameters
QUANTUM_TEST ?= bin/quantum-test
TEST_FLAGS ?=

# quantum-test units parameters (make test / make test-py3)
...
# quantum-test integration parameters (make integration)
IMAGE ?= centos7
TARGET ?=
...
.PHONY: tests
tests:
	$(QUANTUM_TEST) units -v --python $(PYTHON_VERSION) $(TEST_FLAGS)

.PHONY: tests-py3
tests-py3:
	$(QUANTUM_TEST) units -v --python $(PYTHON3_VERSION) $(TEST_FLAGS)

.PHONY: tests-nonet
tests-nonet:
	$(QUANTUM_TEST) units -v --python $(PYTHON_VERSION) $(TEST_FLAGS)  --exclude test/units/modules/network/

.PHONY: integration
integration:
	$(QUANTUM_TEST) integration -v --docker $(IMAGE) $(TARGET) $(TEST_FLAGS)
...
%.1: %.1.rst lib/quantum/release.py
	$(ASCII2MAN)
...
install_manpages:
	gzip -9 $(wildcard ./docs/man/man1/quantum*.1)
	cp $(wildcard ./docs/man/man1/quantum*.1.gz) $(PREFIX)/man/man1/
...
sdist: sdist_check clean docs
	_QUANTUM_SDIST_FROM_MAKEFILE=1 $(PYTHON) setup.py sdist --dist-dir=$(SDIST_DIR)
...
.PHONY: generate_rst
generate_rst: lib/quantum/cli/*.py
	mkdir -p ./docs/man/man1/ ; \
	PYTHONPATH=./lib $(GENERATE_CLI) --template-file=docs/templates/man.j2 --output-dir=docs/man/man1/ --output-format man lib/quantum/cli/*.py
