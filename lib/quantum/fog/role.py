...
class GalaxyRole(object):
    ...
    META_INSTALL = os.path.join('meta', '.fog_install_info')
    ROLE_DIRS = ('defaults', 'files', 'handlers', 'meta', 'tasks', 'templates', 'vars', 'tests')

    def __init__(self, fog, api, name, src=None, version=None, scm=None, path=None):
        ...
        if path is not None:
            ...
            self.path = os.path.join(fog.roles_paths[0], self.name)
            # create list of possible paths
            self.paths = [x for x in fog.roles_paths]
    ...
    def _write_fog_install_info(self):
        """
        Writes a YAML-formatted file to the role's meta/ directory
        (named .fog_install_info) which contains some information
    ...
    def install(self):
        ...
        if self.scm:
            ...
            if os.path.isfile(self.src):
                ...
                if not role_data:
                    raise QuantumError("- sorry, %s was not found on %s." % (self.src, self.api.api_server))

                if role_data.get('role_type') == 'APP':
                    # Container Role
                    display.warning("%s is a Container App role, and should only be installed using Quantum "
                                    "Container" % self.name)
                ...
                if not self.version:
                    ...
                    if role_versions and to_text(self.version) not in [a.get('name', None) for a in role_versions]:
                        raise QuantumError("- the specified version (%s) of %s was not found in the list of available versions (%s)." % (self.version,
                                                                                                                                         self.name,
                                                                                                                                         role_versions))
            ...
            raise QuantumError("No valid role data found")
        ...
        if tmp_file:
            ...
            if not tarfile.is_tarfile(tmp_file):
                raise QuantumError("the downloaded file does not appear to be a valid tar archive.")
            else:
                        ...
                        if meta_main in member.name:
                            ...
                            if not meta_file:
                                ...
                                if len(meta_parent_dir) < len(archive_parent_dir):
                                    ...
                                    meta_file = member
                if not meta_file:
                    raise QuantumError("this role does not appear to have a meta/main.yml file.")
                else:
                    try:
                        self._metadata = yaml.safe_load(role_tar_file.extractfile(meta_file))
                    except Exception:
                        raise QuantumError("this role does not appear to have a valid meta/main.yml file.")
                    ...
                    try:
                        if os.path.exists(self.path):
                            if not os.path.isdir(self.path):
                                raise QuantumError("the specified roles path exists and is not a directory.")
                            elif not context.CLIARGS.get("force", False):
                                raise QuantumError("the specified role %s appears to already exist. Use --force to replace it." % self.name)
                            else:
                                # using --force, remove the old path
                                if not self.remove():
                                    raise QuantumError("%s doesn't appear to contain a role.\n  please remove this directory manually if you really "
                        ...
                        if e.errno == errno.EACCES and len(self.paths) > 1:
                            ...
                            if len(self.paths) > current:
                                ...
                                error = False
                        if error:
                            raise QuantumError("Could not update files in %s: %s" % (self.path, to_native(e)))
