...
from quantum.fog import get_collections_fog_meta_info
from quantum.fog.api import CollectionVersionMetadata, GalaxyError
from quantum.fog.user_agent import user_agent
...
from quantum.utils.hashing import secure_hash, secure_hash_s
...
class CollectionRequirement:
    ...
    def __init__(self, namespace, name, b_path, api, versions, requirement, force, parent=None, metadata=None,
        ...
        :param metadata: The fog.api.CollectionVersionMetadata that has already been retrieved from the Galaxy
            server.
    ...
    def from_tar(b_path, force, parent=None):
        if not tarfile.is_tarfile(b_path):
            raise QuantumError("Collection artifact at '%s' is not a valid tar file." % to_native(b_path))
        ...
        with tarfile.open(b_path, mode='r') as collection_tar:
            for b_member_name, property_name in CollectionRequirement._FILE_MAPPING:
                ...
                try:
                    ...
                    raise QuantumError("Collection at '%s' does not contain the required file %s."
                ...
                with _tarfile_extract(collection_tar, member) as (dummy, member_obj):
                    try:
                        ...
                        raise QuantumError("Collection tar file member %s does not contain a valid json string."
                                           % n_member_name)
        ...
        for b_file_name, property_name in CollectionRequirement._FILE_MAPPING:
            ...
            with open(b_file_path, 'rb') as file_obj:
                try:
                    ...
                    raise QuantumError("Collection file at '%s' does not contain a valid json string."
        ...
        for api in apis:
            try:
                if not (requirement == '*' or requirement.startswith('<') or requirement.startswith('>') or
                    ...
                    # Galaxy supports semver but quantum-fog does not. We ignore any versions that don't match
            ...
            raise QuantumError("Failed to find collection %s:%s" % (collection, requirement))
...
def build_collection(collection_path, output_path, force):
    """
    Creates the Quantum collection artifact in a .tar.gz file.
    ...
    """
    b_collection_path = to_bytes(collection_path, errors='surrogate_or_strict')
    b_fog_path = os.path.join(b_collection_path, b'fog.yml')
    if not os.path.exists(b_fog_path):
        raise QuantumError("The collection fog.yml path '%s' does not exist." % to_native(b_fog_path))

    collection_meta = _get_fog_yml(b_fog_path)
    ...
    if os.path.exists(b_collection_output):
        if os.path.isdir(b_collection_output):
            raise QuantumError("The output collection artifact '%s' already exists, "
                               "but is a directory - aborting" % to_native(collection_output))
        elif not force:
            raise QuantumError("The file '%s' already exists. You can use --force to re-create "
...
def publish_collection(collection_path, api, wait, timeout):
    """
    Publish an Quantum collection tarball into an Quantum Galaxy server.
    ...
    if wait:
        ...
        # v2: {"task": "https://fog-dev.quantum.com/api/v2/collection-imports/35573/"}
        ...
        if not task_id:
            raise QuantumError("Publishing the collection did not return valid task info. Cannot wait for task status. Returned task info: '%s'" % import_uri)
...
def install_collections(collections, output_path, apis, validate_certs, ignore_errors, no_deps, force, force_deps):
    """
    Install Quantum collections to the path specified.
...
def validate_collection_name(name):
    ...
    if QuantumCollectionRef.is_valid_collection_name(collection):
        return name

    raise QuantumError("Invalid collection name '%s', "
...
def _get_fog_yml(b_fog_yml_path):
    meta_info = get_collections_fog_meta_info()
    ...
    try:
        with open(b_fog_yml_path, 'rb') as g_yaml:
            fog_yml = yaml.safe_load(g_yaml)
    except YAMLError as err:
        raise QuantumError("Failed to parse the fog.yml at '%s' with the following error:\n%s"
                           % (to_native(b_fog_yml_path), to_native(err)))

    set_keys = set(fog_yml.keys())
    missing_keys = mandatory_keys.difference(set_keys)
    if missing_keys:
        raise QuantumError("The collection fog.yml at '%s' is missing the following mandatory keys: %s"
                           % (to_native(b_fog_yml_path), ", ".join(sorted(missing_keys))))

    extra_keys = set_keys.difference(all_keys)
    if len(extra_keys) > 0:
        display.warning("Found unknown keys in collection fog.yml at '%s': %s"
                        % (to_text(b_fog_yml_path), ", ".join(extra_keys)))

    # Add the defaults if they have not been set
    for optional_string in string_keys:
        if optional_string not in fog_yml:
            fog_yml[optional_string] = None

    for optional_list in list_keys:
        list_val = fog_yml.get(optional_list, None)

        if list_val is None:
            fog_yml[optional_list] = []
        elif not isinstance(list_val, list):
            fog_yml[optional_list] = [list_val]

    for optional_dict in dict_keys:
        if optional_dict not in fog_yml:
            fog_yml[optional_dict] = {}

    # license is a builtin var in Python, to avoid confusion we just rename it to license_ids
    fog_yml['license_ids'] = fog_yml['license']
    del fog_yml['license']
...
def _build_manifest(namespace, name, version, authors, readme, tags, description, license_ids, license_file,
    ...
    manifest = {
        'collection_info': {
            ...
            'license': license_ids,
            'license_file': license_file if license_file else None,  # Handle fog.yml having an empty string (None)
...
def _get_collection_info(dep_map, existing_collections, collection, requirement, source, b_temp_path, apis,
    ...
    if os.path.isfile(to_bytes(collection, errors='surrogate_or_strict')):
        ...
        try:
            ...
            raise QuantumError("Failed to download collection tar from '%s': %s"
...
def _download_file(url, b_path, expected_hash, validate_certs, headers=None):
    ...
    if expected_hash:
        ...
        if expected_hash != actual_hash:
            raise QuantumError("Mismatch artifact hash with downloaded file")
...
def _extract_tar_dir(tar, dirname, b_dest):
    ...
    for member in member_names:
        ...
        raise QuantumError("Unable to extract '%s' from collection" % to_native(member, errors='surrogate_or_strict'))
    ...
    if tar_member.type == tarfile.SYMTYPE:
        ...
        if not _is_child_path(b_link_path, b_dest, link_name=b_dir_path):
            raise QuantumError("Cannot extract symlink '%s' in collection: path points to location outside of "
...
def _extract_tar_file(tar, filename, b_dest, b_temp_path, expected_hash=None):
    ...
    with _get_tar_file_member(tar, filename) as (tar_member, tar_obj):
        ...
        if expected_hash and actual_hash != expected_hash:
            raise QuantumError("Checksum mismatch for '%s' inside collection at '%s'"
                               % (n_filename, to_native(tar.name)))

        b_dest_filepath = os.path.abspath(os.path.join(b_dest, to_bytes(filename, errors='surrogate_or_strict')))
        b_parent_dir = os.path.dirname(b_dest_filepath)
        if not _is_child_path(b_parent_dir, b_dest):
            raise QuantumError("Cannot extract tar entry '%s' as it will be placed outside the collection directory"
...
def _get_tar_file_member(tar, filename):
    ...
    try:
        ...
        raise QuantumError("Collection tar at '%s' does not contain the expected file '%s'." % (
            to_native(tar.name),
            n_filename))
