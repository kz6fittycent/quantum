...
#
########################################################################
''' This manages remote shared Quantum objects, mainly roles'''
...
def get_collections_fog_meta_info():
    meta_path = os.path.join(os.path.dirname(__file__), 'data', 'collections_fog_meta.yml')
    with open(to_bytes(meta_path, errors='surrogate_or_strict'), 'rb') as fog_obj:
        return yaml.safe_load(fog_obj)


class Galaxy(object):
    ''' Keeps global fog info '''
