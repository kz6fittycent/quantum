...
from quantum.module_utils.common._collections_compat import MutableMapping, MutableSet, MutableSequence
from quantum.parsing.plugin_docs import read_docstring
...
def add_fragments(doc, filename, fragment_loader):
    ...
    for fragment_slug in fragments:
        ...
        fragment = QuantumLoader(fragment_yaml, file_name=filename).get_single_data()
        ...
        if 'options' in doc:
            try:
                ...
                raise QuantumError("%s options (%s) of unknown type: %s" % (to_native(e), fragment_name, filename))
        else:
        ...
        try:
            ...
            raise QuantumError("%s (%s) of unknown type: %s" % (to_native(e), fragment_name, filename))

    if unknown_fragments:
        raise QuantumError('unknown doc_fragment(s) in file {0}: {1}'.format(filename, to_native(', '.join(unknown_fragments))))
...
def get_versioned_doclink(path):
    """
    returns a versioned documentation link for the current Quantum major.minor version; used to generate
    in-product warning/error links to the configured DOCSITE_ROOT_URL
    (eg, https://docs.quantum.com/quantum/2.8/somepath/doc.html)

    :param path: relative path to a document under docs/docsite/rst;
    :return: absolute URL to the specified doc for the current version of Quantum
    """
    path = to_native(path)
    try:
        ...
        if path.startswith('/'):
            path = path[1:]
        split_ver = quantum_version.split('.')
        if len(split_ver) < 3:
            raise RuntimeError('invalid version ({0})'.format(quantum_version))
