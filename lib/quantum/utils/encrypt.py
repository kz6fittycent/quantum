...
def random_password(length=DEFAULT_PASSWORD_LENGTH, chars=C.DEFAULT_PASSWORD_CHARS):
    ...
    if not isinstance(chars, text_type):
        raise QuantumAssertionError('%s (%s) is not a text_type' % (chars, type(chars)))
...
class CryptHash(BaseHash):
    def __init__(self, algorithm):
        ...
        if sys.platform.startswith('darwin'):
            raise QuantumError("crypt.crypt not supported on Mac OS X/Darwin, install passlib python module")

        if algorithm not in self.algorithms:
            raise QuantumError("crypt.crypt does not support '%s' algorithm" % self.algorithm)
...
class PasslibHash(BaseHash):
    def __init__(self, algorithm):
        ...
        if not PASSLIB_AVAILABLE:
            raise QuantumError("passlib must be installed to hash with '%s'" % algorithm)

        try:
            self.crypt_algo = getattr(passlib.hash, algorithm)
        except Exception:
            raise QuantumError("passlib does not support '%s' algorithm" % algorithm)
    ...
    def _hash(self, secret, salt, salt_size, rounds):
        ...
        if hasattr(self.crypt_algo, 'hash'):
            ...
            raise QuantumError("installed passlib version %s not supported" % passlib.__version__)
        ...
        if not result:
            raise QuantumError("failed to hash with algorithm '%s'" % self.algorithm)
