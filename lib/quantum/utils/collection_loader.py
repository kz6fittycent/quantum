...
from quantum.module_utils.six import iteritems, string_types, with_metaclass
from quantum.utils.path import cs_open
from quantum.utils.singleton import Singleton

_SYNTHETIC_PACKAGES = {
    # these provide fallback package definitions when there are no on-disk paths
    'quantum_collections': dict(type='pkg_only', allow_external_subpackages=True),
    'quantum_collections.quantum': dict(type='pkg_only', allow_external_subpackages=True),
    # these implement the quantum.builtin synthetic collection mapped to the packages inside the quantum distribution
    'quantum_collections.quantum.builtin': dict(type='pkg_only'),
    'quantum_collections.quantum.builtin.plugins': dict(type='map', map='quantum.plugins'),
    'quantum_collections.quantum.builtin.plugins.module_utils': dict(type='map', map='quantum.module_utils', graft=True),
    'quantum_collections.quantum.builtin.plugins.modules': dict(type='flatmap', flatmap='quantum.modules', graft=True),
}


# FIXME: exception handling/error logging
class QuantumCollectionLoader(with_metaclass(Singleton, object)):
    def __init__(self, config=None):
        ...
        for syn_pkg_def in (p for p in iteritems(_SYNTHETIC_PACKAGES) if p[1].get('graft')):
            ...
            newmod.__file__ = '<quantum_synthetic_collection_package>'
            pkg_type = pkg_def.get('type')

            # TODO: need to rethink map style so we can just delegate all the loading

            if pkg_type == 'flatmap':
                newmod.__loader__ = QuantumFlatMapLoader(import_module(pkg_def['flatmap']))
            newmod.__path__ = []
    ...
    def n_collection_paths(self):
        return self._n_coupling_paths + self._n_configured_paths

    def get_collection_path(self, collection_name):
        if not QuantumCollectionRef.is_valid_collection_name(collection_name):
            raise ValueError('{0} is not a valid collection name'.format(to_native(collection_name)))

        m = import_module('quantum_collections.{0}'.format(collection_name))

        return m.__file__

    def set_coupling_paths(self, b_coupling_paths):
        if isinstance(b_coupling_paths, string_types):
            b_coupling_paths = [b_coupling_paths]
        ...
        self._n_coupling_paths = [os.path.join(to_native(p), 'collections') for p in b_coupling_paths if not (p in added_paths or added_paths.add(p))]
    ...
    def _find_module(self, fullname, path, load):
        # this loader is only concerned with items under the Quantum Collections namespace hierarchy, ignore others
        if not fullname.startswith('quantum_collections.') and fullname != 'quantum_collections':
            return False, None
        ...
        # this loader implements key functionality for Quantum collections
        # * implicit distributed namespace packages for the root Quantum namespace (no pkgutil.extend_path hackery reqd)
        ...
        # FIXME: need to handle the "no dirs present" case for at least the root and synthetic internal collections like quantum.builtin
...
class QuantumFlatMapLoader(object):
...
class QuantumCollectionRef:
    ...
    def __init__(self, collection_name, subdirs, resource, ref_type):
        """
        Create an QuantumCollectionRef from components
        ...
        package_components = [u'quantum_collections', self.collection]
    ...
    def from_fqcr(ref, ref_type):
        ...
        :param ref_type: the type of the reference, eg 'module', 'role', 'doc_fragment'
        :return: a populated QuantumCollectionRef object
        """
        ...
        # ns.coll.resource -> quantum_collections.ns.coll.plugins.(plugintype).resource
        # ns.coll.subdir1.resource -> quantum_collections.ns.coll.plugins.subdir1.(plugintype).resource
        # ns.coll.rolename -> quantum_collections.ns.coll.roles.rolename
        if not QuantumCollectionRef.is_valid_fqcr(ref):
        ...
        return QuantumCollectionRef(collection_name, subdirs, resource, ref_type)
    ...
    def try_parse_fqcr(ref, ref_type):
        ...
        :param ref: collection reference to parse (a valid ref is of the form 'ns.coll.resource' or 'ns.coll.subdir1.subdir2.resource')
        :param ref_type: the type of the reference, eg 'module', 'role', 'doc_fragment'
        :return: a populated QuantumCollectionRef object on successful parsing, else None
        """
        try:
            return QuantumCollectionRef.from_fqcr(ref, ref_type)
        except ValueError:
            pass
    ...
    def legacy_plugin_dir_to_plugin_type(legacy_plugin_dir_name):
        ...
        if plugin_type not in QuantumCollectionRef.VALID_REF_TYPES:
    ...
    def is_valid_fqcr(ref, ref_type=None):
        ...
        if not ref_type:
            return bool(re.match(QuantumCollectionRef.VALID_FQCR_RE, ref))

        return bool(QuantumCollectionRef.try_parse_fqcr(ref, ref_type))
    ...
    def is_valid_collection_name(collection_name):
        ...
        return bool(re.match(QuantumCollectionRef.VALID_COLLECTION_NAME_RE, collection_name))


def get_collection_role_path(role_name, collection_list=None):
    acr = QuantumCollectionRef.try_parse_fqcr(role_name, 'role')
    ...
    for collection_name in collection_list:
        try:
            acr = QuantumCollectionRef(collection_name=collection_name, subdirs=subdirs, resource=resource, ref_type='role')
...
_N_COLLECTION_PATH_RE = re.compile(r'/quantum_collections/([^/]+)/([^/]+)')
...
def get_collection_name_from_path(path):
    ...
    """
    n_collection_paths = [to_native(os.path.realpath(to_bytes(p))) for p in QuantumCollectionLoader().n_collection_paths]
    ...
    for coll_path in n_collection_paths:
        ...
        if common_prefix == coll_path:
            ...
            loaded_collection_path = QuantumCollectionLoader().get_collection_path(n_collection_name)
...
def set_collection_coupling_paths(b_coupling_paths):
    QuantumCollectionLoader().set_coupling_paths(b_coupling_paths)
