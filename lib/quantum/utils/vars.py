...
def _validate_mutable_mappings(a, b):
    ...
    :raises QuantumError: if one of the arguments is not a MutableMapping
    """
    ...
    if not (isinstance(a, MutableMapping) and isinstance(b, MutableMapping)):
        ...
        raise QuantumError("failed to combine variables, expected dicts but got a '{0}' and a '{1}': \n{2}\n{3}".format(
...
def load_extra_vars(loader):
    ...
    for extra_vars_opt in context.CLIARGS.get('extra_vars', tuple()):
        ...
        if isinstance(data, MutableMapping):
            ...
            raise QuantumOptionsError("Invalid extra vars data supplied. '%s' could not be made into a dictionary" % extra_vars_opt)
...
def load_options_vars(version):
    ...
    if version is None:
        version = 'Unknown'
    options_vars = {'quantum_version': version}
    ...
    for attr, alias in attrs.items():
        ...
        if opt is not None:
            options_vars['quantum_%s' % alias] = opt
