...
def get_common_arg_spec(supports_create=False, supports_wait=False):
    ...
    """
    # Note: This method is used by most OCI quantum resource modules during initialization. When making changes to this
...
def get_facts_module_arg_spec(filter_by_name=False):
    # Note: This method is used by most OCI quantum fact modules during initialization. When making changes to this
    # method, ensure that no `oci` python sdk dependencies are introduced in this method. This ensures that the modules
    # can check for absence of OCI Python SDK and fail with an appropriate message. Introducing an OCI dependency in
    # this method would break that error handling logic.
...
def get_oci_config(module, service_client_class=None):
    ...
    any overrides specified for configuration attributes through Quantum module options or environment variables. The
    ...
    config["additional_user_agent"] = "Oracle-Quantum/{0}".format(__version__)
...
def create_service_client(module, service_client_class):
    ...
    :param module: An QuantumModule that represents user provided options for a Task
...
def _merge_auth_option(
    ...
    """Merge the values for an authentication attribute from quantum module options and
    ...
    _debug(
        "\t Quantum module option {0} = {1}".format(module_option_name, auth_attribute)
    )
    ...
    # An authentication attribute has been provided through an env-variable or an quantum
...
def setup_logging(
    ...
    log_file_path = os.path.join(log_path, "oci_quantum_module.log")
...
def check_and_update_resource(
    ...
    :param module: Instance of QuantumModule
...
def get_taggable_arg_spec(supports_create=False, supports_wait=False):
    ...
    :return: A dict that represents an quantum arg spec that builds over the common_arg_spec and adds free-form and
    defined tags.
    """
...
def add_tags_to_model_from_module(model, module):
    """
    Adds free-form and defined tags from an quantum module to a resource model
    :param model: A resource model instance that supports 'freeform_tags' and 'defined_tags' as attributes
    :param module: An QuantumModule representing the options provided by the user
...
def _get_attributes_to_consider(exclude_attributes, model, module):
    ...
    :param module: An instance of QuantumModule that contains user's desires around a resource's state
...
def does_existing_resource_match_user_inputs(
    ...
    :param module: The QuantumModule representing the options provided by the user.
...
def are_attrs_equal(current_resource, module, attributes):
    ...
    Model instance already has the values specified by an Quantum user while invoking an OCI Quantum module and if a
...
def _get_user_provided_value(module, attribute_name):
    ...
    if user_provided_value is None:
        # If the attribute_name is set as an alias for some option X and user has provided value in the coupling using
...
def get_existing_resource(target_fn, module, **kwargs):
    ...
    :param module Instance of QuantumModule attribute value
