...
Function Get-QuantumParam($obj, $name, $default = $null, $resultobj = @{}, $failifempty = $false, $emptyattributefailmessage, $ValidateSet, $ValidateSetErrorMessage, $type = $null, $aliases = @())
{
<#
    .SYNOPSIS
    ...
    slightly more pythonic
    .EXAMPLE
    $attr = Get-QuantumParam $response "code" -default "1"
    .EXAMPLE
    Get-QuantumParam -obj $params -name "State" -default "Present" -ValidateSet "Present","Absent" -resultobj $resultobj -failifempty $true
    Get-QuantumParam also supports Parameter validation to save you from coding that manually
...
#>
    ...
    try {
        ...
        if ($ValidateSet) {
            ...
            if ($ValidateSet -contains ($obj.$name)) {
                ...
                if ($null -eq $ValidateSetErrorMessage) {
                    ...
                    $ValidateSetErrorMessage = "Get-QuantumParam: Argument $name needs to be one of $($ValidateSet -join ",") but was $($obj.$name)."
                }
        ...
        if ($failifempty -eq $false) {
            ...
            if (-not $emptyattributefailmessage) {
                $emptyattributefailmessage = "Get-QuantumParam: Missing required argument: $name"
            }
    ...
    if ($type -eq "path") {
        ...
        if (-not (Test-Path -IsValid $value)) {
            ...
            if ($path_invalid) {
                Fail-Json -obj $resultobj -message "Get-QuantumParam: Parameter '$name' has an invalid path '$value' specified."
            }
        }
    } elseif ($type -eq "str") {
        ...
        if ($value -is [array]) {
            ...
            Fail-Json -obj $resultobj -message "Get-QuantumParam: Parameter '$name' is not a YAML list."
        }
...
#Alias Get-attr-->Get-QuantumParam for backwards compat. Only add when needed to ease debugging of scripts
If (-not(Get-Alias -Name "Get-attr" -ErrorAction SilentlyContinue))
{
    New-Alias -Name Get-attr -Value Get-QuantumParam
}
...
<#
    .SYNOPSIS
    Helper function to parse Quantum JSON arguments from a "file" passed as
