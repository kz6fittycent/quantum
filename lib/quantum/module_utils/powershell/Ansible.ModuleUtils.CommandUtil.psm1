...
Function Get-ExecutablePath {
    ...
    if ($null -ne $file) {
        ...
        $executable_path = [Quantum.Process.ProcessUtil]::SearchPath($executable)
    }
...
Function Run-Command {
    ...
    $arguments = [Quantum.Process.ProcessUtil]::ParseCommandLine($command)
    $executable = Get-ExecutablePath -executable $arguments[0] -directory $working_directory

    # run the command and get the results
    $command_result = [Quantum.Process.ProcessUtil]::CreateProcess($executable, $command, $working_directory, $environment, $stdin)
