...
Function Get-QuantumWebRequest {
    <#
    .SYNOPSIS
    Creates a System.Net.WebRequest object based on common URL module options in Quantum.

    .DESCRIPTION
    Will create a WebRequest based on common input options within Quantum. This can be used manually or with
    ...
    .PARAMETER Module
    The QuantumBasic module that can be used as a backup parameter source or a way to return warnings back to the
    Quantum controller.
    ...
    $spec.options += $quantum_web_request_options
    $module = Quantum.Basic.QuantumModule]::Create($args, $spec)

    $web_request = Get-QuantumWebRequest -Module $module
    #>
    [CmdletBinding()]
    ...
    Param (
        ...
        [ValidateScript({ $_.GetType().FullName -eq 'Quantum.Basic.QuantumModule' })]
        [System.Object]
        $Module
    )
    ...
    if ($web_request -is [System.Net.HttpWebRequest]) {
        ...
        if ($Headers -and $Headers.ContainsKey("User-Agent")) {
            if ($HttpAgent -eq $quantum_web_request_options.http_agent.default) {
...
Function Invoke-WithWebRequest {
    ...
    .PARAMETER Module
    The Quantum.Basic module to set the return values for. This will set the following return values;
    ...
    .PARAMETER Request
    The System.Net.WebRequest to call. This can either be manually crafted or created with Get-QuantumWebRequest.
...
$quantum_web_request_options = @{
    url = @{ type="str"; required=$true }
    method = @{ type="str" }
    follow_redirects = @{ type="str"; choices=@("all","none","safe"); default="safe" }
    headers = @{ type="dict" }
    http_agent = @{ type="str"; default="quantum-httpget" }
...
$export_members = @{
    Function = "Get-QuantumWebRequest", "Invoke-WithWebRequest"
    Variable = "quantum_web_request_options"
}
