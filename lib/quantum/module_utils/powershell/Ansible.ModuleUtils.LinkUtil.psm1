...
'@
    ...
    # FUTURE: find a better way to get the _quantum_remote_tmp variable
    $original_tmp = $env:TMP

    $remote_tmp = $original_tmp
    $module_params = Get-Variable -Name complex_args -ErrorAction SilentlyContinue
    if ($module_params) {
        if ($module_params.Value.ContainsKey("_quantum_remote_tmp") ) {
            $remote_tmp = $module_params.Value["_quantum_remote_tmp"]
    ...
    $state = Get-QuantumPrivilege -Name SeBackupPrivilege
    if ($state -eq $false) {
...
Function Get-Link($link_path) {
    $link_info = [Quantum.LinkUtil]::GetLinkInfo($link_path)
    return $link_info
}

Function Remove-Link($link_path) {
    [Quantum.LinkUtil]::DeleteLink($link_path)
}
