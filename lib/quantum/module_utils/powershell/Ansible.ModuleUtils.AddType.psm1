...
Function Add-CSharpType {
    ...
    more configuration options that are useable within Quantum and it
    ...
    .PARAMETER QuantumModule
    [Quantum.Basic.QuantumModule] used to derive the TempPath and Debug values.
        TempPath is set to the Tmpdir property of the class
        IncludeDebugInfo is set when the Quantum verbosity is >= 3
    ...
    Cannot be used when QuantumModule is set. This is a no-op when
    running on PSCore.

    .PARAMETER IncludeDebugInfo
    [Switch] Whether to include debug information in the compiled
    assembly. Cannot be used when QuantumModule is set. This is a no-op
    ...
    param(
        ...
        [Switch]$PassThru,
        [Parameter(Mandatory=$true, ParameterSetName="Module")][Object]$QuantumModule,
    ...
    if ($is_coreclr) {
        ...
        if ($PSCmdlet.ParameterSetName -eq "Module") {
            $temp_path = $QuantumModule.Tmpdir
            $include_debug = $QuantumModule.Verbosity -ge 3
        } else {
            $temp_path = $TempPath
