...
from quantum.module_utils.facts.collector import BaseFactCollector

# A list of dicts.  If there is a platform with more than one
# package manager, put the preferred one last.  If there is an
# quantum module, use that as the value for the 'name' key.
...
class PkgMgrFactCollector(BaseFactCollector):
    ...
    def _check_rh_versions(self, pkg_mgr_name, collected_facts):
        if collected_facts['quantum_distribution'] == 'Fedora':
            if os.path.exists('/run/ostree-booted'):
                return "atomic_container"
            try:
                if int(collected_facts['quantum_distribution_major_version']) < 23:
                ...
                # just default to dnf
                pkg_mgr_name = 'dnf'
        elif collected_facts['quantum_distribution'] == 'Amazon':
            pkg_mgr_name = 'yum'
        else:
            # If it's not one of the above and it's Red Hat family of distros, assume
            # RHEL or a clone. For versions of RHEL < 8 that Quantum supports, the
            ...
            if int(collected_facts['quantum_distribution_major_version']) < 8:
                pkg_mgr_name = 'yum'
            else:
                pkg_mgr_name = 'dnf'
        return pkg_mgr_name
