...
class QuantumDockerSwarmClient(QuantumDockerClient):

    def __init__(self, **kwargs):
        super(QuantumDockerSwarmClient, self).__init__(**kwargs)
    ...
    def fail_task_if_not_swarm_manager(self):
        """
        If host is not a swarm manager then Quantum task on this host should end with 'failed' state
        """
    ...
    def check_if_swarm_node_is_down(self, node_id=None, repeat_check=1):
        ...
        host that is not part of Swarm it will fail the coupling
    ...
    def get_unlock_key(self):
        if self.docker_py_version < LooseVersion('2.7.0'):
            return None
        return super(QuantumDockerSwarmClient, self).get_unlock_key()
