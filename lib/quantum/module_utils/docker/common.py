...
from quantum.module_utils.basic import QuantumModule, env_fallback, missing_required_lib
from quantum.module_utils.common._collections_compat import Mapping, Sequence
...
if not HAS_DOCKER_PY:
    ...
    # instantiation of QuantumModule and proper error handing
...
def sanitize_result(data):
    """Sanitize data object for return to Quantum.

    When the data object contains types such as docker.types.containers.HostConfig,
    Quantum will fail when these are returned via exit_json or fail_json.
...
class QuantumDockerClient(Client):
    ...
    def inspect_distribution(self, image, **kwargs):
        ...
        return super(QuantumDockerClient, self).inspect_distribution(image, **kwargs)
...
def clean_dict_booleans_for_docker_api(data):
    '''
    Go doesn't like Python booleans 'True' or 'False', while Quantum is just
