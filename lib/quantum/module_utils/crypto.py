# -*- coding: utf-8 -*-
#
# (c) 2016, Yanis Guenane <yanis+quantum@guenane.org>
#
...
def get_fingerprint_of_bytes(source):
    ...
    for algo in algorithms:
        ...
        try:
            ...
            # (https://github.com/quantum/quantum/issues/67213)
            continue
        try:
...
def write_file(module, content, default_mode=None, path=None):
    ...
    tmp_fd, tmp_name = tempfile.mkstemp(prefix=b'.quantum_tmp')
    try:
        os.close(tmp_fd)
    except Exception as dummy:
        pass
    module.add_cleanup_file(tmp_name)  # if we fail, let Quantum try to remove the file
    try:
        try:
            # Create tempfile
