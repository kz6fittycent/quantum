...
from quantum.module_utils.common import dict_transformations
from quantum.module_utils.common._collections_compat import Sequence
from quantum.module_utils.k8s.common import list_dict_str
...
class KubeVirtRawModule(KubernetesRawModule):
    ...
    def _define_datavolumes(self, datavolumes, spec):
        """
        Takes datavoulmes parameter of Quantum and create kubevirt API datavolumesTemplateSpec
        structure from it
        """
        if not datavolumes:
            return
    ...
    def _define_cloud_init(self, cloud_init_nocloud, template_spec):
        ...
        API strucuture. The name for disk is hardcoded to quantumcloudinitdisk.
        """
        if cloud_init_nocloud:
            ...
            template_spec['volumes'].append({'name': 'quantumcloudinitdisk', 'cloudInitNoCloud': cloud_init_nocloud})
            template_spec['domain']['devices']['disks'].append({
                'name': 'quantumcloudinitdisk',
                'disk': {'bus': 'virtio'},
            })

    def _define_interfaces(self, interfaces, template_spec, defaults):
        """
        Takes interfaces parameter of Quantum and create kubevirt API interfaces
        ...
        if interfaces:
            # Extract interfaces k8s specification from interfaces list passed to Quantum:
            spec_interfaces = []
            for i in interfaces:
            ...
            # Extract networks k8s specification from interfaces list passed to Quantum:
            spec_networks = []
            for i in interfaces:
                net = i['network']
                net['name'] = i['name']
    ...
    def _define_disks(self, disks, template_spec, defaults):
        """
        Takes disks parameter of Quantum and create kubevirt API disks and
        ...
        if disks:
            # Extract k8s specification from disks list passed to Quantum:
            spec_disks = []
            for d in disks:
                spec_disks.append(
            ...
            # Extract volumes k8s specification from disks list passed to Quantum:
            spec_volumes = []
            for d in disks:
                volume = d['volume']
                volume['name'] = d['name']
