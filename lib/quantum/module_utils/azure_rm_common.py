...
ANSIBLE_USER_AGENT = 'Quantum/{0}'.format(ANSIBLE_VERSION)
...
class AzureRMModuleBase(object):
    def __init__(self, derived_arg_spec, bypass_checks=False, no_log=False,
        ...
        self.module = QuantumModule(argument_spec=merged_arg_spec,
        ...
        if not HAS_AZURE:
            self.fail(msg=missing_required_lib('quantum[azure] (azure >= {0})'.format(AZURE_MIN_RELEASE)),
        ...
        if package_version is not None:
            ...
            if Version(client_version) < Version(expected_version):
                self.fail("Installed azure-mgmt-{0} client version is {1}. The minimum supported version is {2}. Try "
                          "`pip install quantum[azure]`".format(client_name, client_version, expected_version))
            if Version(client_version) != Version(expected_version):
    ...
    def get_mgmt_svc_client(self, client_type, base_url=None, api_version=None):
        ...
        try:
            ...
            def _quantum_get_models(self, *arg, **kwarg):
                return self._quantum_models

            setattr(client, '_quantum_models', importlib.import_module(client_type.__module__).models)
            client.models = types.MethodType(_quantum_get_models, client)
