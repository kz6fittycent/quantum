...
# Copyright (c), Toshio Kuratomi <tkuratomi@quantum.com> 2016
...
# Quantum modules can be written in any language.
...
from quantum.module_utils.common.text.converters import (
    jsonify,
    container_to_bytes as json_dict_unicode_to_bytes,
    container_to_text as json_dict_bytes_to_unicode,
)

from quantum.module_utils.common.text.formatters import (
    lenient_lowercase,
    bytes_to_human,
    human_to_bytes,
    SIZE_RANGES,
...
try:
    from quantum.module_utils.common._json_compat import json
except ImportError as e:
    print('\n{{"msg": "Error: quantum requires the stdlib json: {0}", "failed": true}}'.format(to_native(e)))
    sys.exit(1)
...
from quantum.module_utils.common._collections_compat import (
    KeysView,
...
from quantum.module_utils.common.file import (
...
from quantum.module_utils.common.sys_info import (
    get_distribution,
    get_distribution_version,
    get_platform_subclass,
)
from quantum.module_utils.pycompat24 import get_exception, literal_eval
from quantum.module_utils.common.parameters import (
    handle_aliases,
    list_deprecations,
    list_no_log_values,
    PASS_VARS,
...
from quantum.module_utils.six.moves import map, reduce, shlex_quote
from quantum.module_utils.common.validation import (
...
from quantum.module_utils.common._utils import get_all_subclasses as _get_all_subclasses
from quantum.module_utils.parsing.convert_bool import BOOLEANS, BOOLEANS_FALSE, BOOLEANS_TRUE, boolean
...
# multiple QuantumModules are created.  Otherwise each QuantumModule would
...
if not _PY_MIN:
    print(
        '\n{"failed": true, '
        '"msg": "Quantum requires a minimum of Python2 version 2.6 or Python3 version 3.5. Current version: %s"}' % ''.join(sys.version.splitlines())
    )
    sys.exit(1)
...
def load_platform_subclass(cls, *args, **kwargs):
    """**Deprecated**: Use quantum.module_utils.common.sys_info.get_platform_subclass instead"""
    platform_cls = get_platform_subclass(cls)
    return super(cls, platform_cls).__new__(platform_cls)


def get_all_subclasses(cls):
    """**Deprecated**: Use quantum.module_utils.common._utils.get_all_subclasses instead"""
...
def sanitize_keys(obj, no_log_strings, ignore_keys=frozenset()):
    ...
    while deferred_removals:
        ...
        if isinstance(new_data, Mapping):
            for old_key, old_elem in old_data.items():
                if old_key in ignore_keys or old_key.startswith('_quantum'):
...
def missing_required_lib(library, reason=None, url=None):
    ...
    msg += (" Please read module documentation and install in the appropriate location."
            " If the required library is installed, but Quantum is using the wrong Python interpreter,"
            " please consult the documentation on quantum_python_interpreter")
    return msg


class QuantumFallbackNotFound(Exception):
    pass


class QuantumModule(object):
    def __init__(self, argument_spec, bypass_checks=False, no_log=False,
        ...
        '''
        Common code for quickly building an quantum module in Python
        ...
        if module_set_check_invalid_arguments and self._name not in ('uri', 'zfs'):
            self.deprecate('Setting check_invalid_arguments is deprecated and will be removed.'
                           ' Update the code for this module  In the future, QuantumModule will'
                           ' always check for invalid arguments.', version='2.9')

    @property
    def tmpdir(self):
        # if _quantum_tmpdir was not set and we have a remote_tmp,
        ...
        if self._tmpdir is None:
            ...
            basefile = "quantum-moduletmp-%s-" % time.time()
            try:
    ...
    def deprecate(self, msg, version=None, date=None, collection_name=None):
        # `date` and `collection_name` are Quantum 2.10 parameters. We accept and ignore them,
        # to avoid modules/plugins from 2.10 conformant collections to break with new enough
        # versions of Quantum 2.9.
    ...
    def _check_arguments(self, check_invalid_arguments, spec=None, param=None, legal_inputs=None):
        ...
        for k in PASS_VARS:
            # handle setting internal properties from internal quantum vars
            param_key = '_quantum_%s' % k
    ...
    def _set_fallbacks(self, spec=None, param=None):
        ...
        for (k, v) in spec.items():
            ...
            if k not in param and fallback_strategy is not None:
                ...
                except QuantumFallbackNotFound:
                    continue
                else:
    ...
    def _log_invocation(self):
        ''' log that quantum ran the module '''
    ...
    def atomic_move(self, src, dest, unsafe_writes=False):
        ...
        try:
            ...
            if e.errno not in [errno.EPERM, errno.EXDEV, errno.EACCES, errno.ETXTBSY, errno.EBUSY]:
                ...
                tmp_dest_name = None
                try:
                    tmp_dest_fd, tmp_dest_name = tempfile.mkstemp(prefix=b'.quantum_tmp', dir=b_dest_dir, suffix=b_suffix)
                except (OSError, IOError) as e:
    ...
    def run_command(self, args, check_rc=False, close_fds=True, executable=None, data=None, binary_data=False, path_prefix=None, cwd=None,
        ...
        #   /tmp/test_module_scratch/debug_dir/quantum/module_utils/basic.py
        # If using quantum or quantum-coupling with a remote system:
        #   /tmp/quantum_vmweLQ/quantum_modlib.zip/quantum/module_utils/basic.py

        # Clean out python paths set by ansiballz
        if 'PYTHONPATH' in os.environ:
            pypaths = os.environ['PYTHONPATH'].split(':')
            pypaths = [x for x in pypaths
                       if not x.endswith('/quantum_modlib.zip') and
