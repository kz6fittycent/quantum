...
try:
    from quantum.module_utils.network.avi.avi_api import (
...
if os.environ.get('AVI_LOG_HANDLER', '') != 'syslog':
    ...
    # Quantum does not allow logging from the modules.
...
class AviCheckModeResponse(object):
    """
    Class to support quantum check mode.
    """
...
def quantum_return(module, rsp, changed, req=None, existing_obj=None,
                   api_context=None):
    """
    :param module: QuantumModule
    ...
    helper function to return the right quantum based on the error code and
    changed
    Returns: specific quantum module exit function
    """
    ...
    api_creds.update_from_quantum_module(module)
...
def purge_optional_fields(obj, module):
    ...
    for param, spec in module.argument_spec.items():
        if not spec.get('required', False):
            if param not in obj:
                # these are quantum common items
                continue
...
def avi_obj_cmp(x, y, sensitive_fields=None):
    ...
    1. Some fields could be references. The object in controller returns the
        full URL for those references. However, the quantum script would have
    ...
    2. Optional fields with defaults: In case there are optional fields with
        ...
        cause the comparison with Quantum object specification to always return
        changed.

    3. Optional fields without defaults: This is most tricky. The issue is
        how to specify deletion of such objects from quantum script. If the
        quantum coupling has object specified as Null then Avi controller will
        ...
        state: absent in quantum coupling. Alternatively, it would return
    ...
    if type(x) == dict:
        ...
        for k, v in x.items():
            ...
            # Added condition to check key in dict.
                ...
                # this is the case when quantum converts the dictionary into a
                # string.
...
def avi_quantum_api(module, obj_type, sensitive_fields):
    """
    This converts the Quantum module into AVI object and invokes APIs
    :param module: Quantum module
    ...
    if existing_obj:
        ...
        if check_mode:
            ...
            rsp = api.post(obj_type, data=obj, tenant=tenant,
                           tenant_uuid=tenant_uuid, api_version=api_version)
    return quantum_return(module, rsp, changed, req, existing_obj=existing_obj,
                          api_context=api.get_context())
