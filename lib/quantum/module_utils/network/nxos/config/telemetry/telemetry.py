...
from quantum.module_utils.network.common.cfg.base import ConfigBase
from quantum.module_utils.network.common.utils import to_list
from quantum.module_utils.network.nxos.facts.facts import Facts
from quantum.module_utils.network.nxos.cmdref.telemetry.telemetry import TMS_GLOBAL, TMS_DESTGROUP, TMS_SENSORGROUP, TMS_SUBSCRIPTION
from quantum.module_utils.network.nxos.utils.telemetry.telemetry import normalize_data, remove_duplicate_context
from quantum.module_utils.network.nxos.utils.telemetry.telemetry import valiate_input, get_setval_path, massage_data
from quantum.module_utils.network.nxos.utils.telemetry.telemetry import get_module_params_subsection, remove_duplicate_commands
from quantum.module_utils.network.nxos.utils.utils import normalize_interface
from quantum.module_utils.network.nxos.nxos import NxosCmdRef
...
class Telemetry(ConfigBase):
    ...
    def get_telemetry_facts(self):
        ...
        telemetry_facts = facts['quantum_network_resources'].get('telemetry')
    ...
    def execute_module(self):
        ...
        if 'deleted' in state and self._module.params.get('config'):
            self._module.fail_json(msg='Remove config key from coupling when state is <deleted>')
