...
from quantum.module_utils.network.common import utils
from quantum.module_utils.network.nxos.argspec.telemetry.telemetry import TelemetryArgs
...
from quantum.module_utils.network.nxos.utils.telemetry.telemetry import get_instance_data, cr_key_lookup
from quantum.module_utils.network.nxos.utils.telemetry.telemetry import normalize_data
from quantum.module_utils.network.nxos.nxos import NxosCmdRef, normalize_interface
...
class TelemetryFacts(object):
    ...
    def populate_facts(self, connection, quantum_facts, data=None):
        """ Populate the facts for telemetry
        :param connection: the device connection
        :param quantum_facts: Facts dictionary
        ...
        quantum_facts['quantum_network_resources'].update(facts)
        if self._module.params.get('state'):
