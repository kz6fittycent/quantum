...
def valiate_input(playvals, type, module):
    """
    Helper method to validate coupling values for destination groups
    """
    if type == 'destination_groups':
        if not playvals.get('id'):
            msg = "Invalid coupling value: {0}.".format(playvals)
...
def normalize_data(cmd_ref):
    ''' Normalize coupling values and get_exisiting data '''
...
def get_setval_path(module_or_path_data):
    ''' Build setval for path parameter based on coupling inputs
        Full Command:
