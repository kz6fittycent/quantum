#
#  Copyright 2017 Michael De La Rue | Quantum
#
...
**aggressively as the exact correct interface for quantum AWS modules**
...
**publish modules using this except directly to the main Quantum**
**development repository.**

In order to use this module, include it as part of a custom
module as shown below.

  from quantum.module_utils.aws import QuantumAWSModule
  module = QuantumAWSModule(argument_spec=dictionary, supports_check_mode=boolean
                            mutually_exclusive=list1, required_together=list2)

The 'QuantumAWSModule' module provides similar, but more restricted,
interfaces to the normal Quantum module.  It also includes the
...
from quantum.module_utils.ec2 import HAS_BOTO3, camel_dict_to_snake_dict, ec2_argument_spec, boto3_conn, get_aws_connection_info

# We will also export HAS_BOTO3 so end user modules can use it.
__all__ = ('QuantumAWSModule', 'HAS_BOTO3', 'is_boto3_error_code')


class QuantumAWSModule(object):
    """An quantum module class for AWS modules

    QuantumAWSModule provides an a class for building modules which
    ...
    feature is missing please contact the author/Quantum AWS team
    (available on #quantum-aws on IRC) to request the additional
    features needed.
    """
    default_settings = {
        "default_args": True,
        "check_boto3": True,
        "auto_retry": True,
        "module_class": QuantumModule
    }

    def __init__(self, **kwargs):
        local_settings = {}
        for key in QuantumAWSModule.default_settings:
            try:
                local_settings[key] = kwargs.pop(key)
            except KeyError:
                local_settings[key] = QuantumAWSModule.default_settings[key]
        ...
        self._module = QuantumAWSModule.default_settings["module_class"](**kwargs)
