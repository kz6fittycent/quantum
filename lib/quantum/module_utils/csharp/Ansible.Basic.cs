...
namespace Quantum.Basic
{
    public class QuantumModule
    {
        ...
        {
            // null values means no mapping, not used in Quantum.Basic.QuantumModule
        ...
        private Dictionary<string, List<object>> specDefaults = new Dictionary<string, List<object>>()
        {
            ...
            { "removed_at_date", new List<object>() { null, typeof(DateTime) } },  // Quantum 2.10 compatibility
            { "removed_from_collection", new List<object>() { null, typeof(string) } },  // Quantum 2.10 compatibility
        ...
        public string QuantumVersion { get; private set; }
        ...
        public string Tmpdir
        {
            get
            {
                if (tmpdir == null)
                {
                    ...
                    string dirName = String.Format("quantum-moduletmp-{0}-{1}", dateTime, new Random().Next(0, int.MaxValue));
        ...
        public QuantumModule(string[] args, IDictionary argumentSpec)
        {
            ...
            List<string> legalInputs = passVars.Keys.Select(v => "_quantum_" + v).ToList();
            ...
            // Set a Quantum friendly invocation value in the result object
        ...
        public static QuantumModule Create(string[] args, IDictionary argumentSpec)
        {
            return new QuantumModule(args, argumentSpec);
        }
        ...
        {
            // `collectionName` is a Quantum 2.10 parameter. We accept and ignore it,
            // to avoid modules/plugins from 2.10 conformant collections to break with
            // new enough versions of Quantum 2.9.
        ...
        {
            // This function is only available for Quantum 2.10. We still accept and ignore it,
                    ...
                    if (choices.Count > 0)
                    {
                        ...
                        else if (caseDiffList.Count > 0)
                        {
                            ...
                            string msg = String.Format(
                                "value of {0} was a case insensitive match of {1}: {2}. Checking of choices will be case sensitive in a future Quantum release. Case insensitive matches were: {3}",
        ...
        private void CheckUnsupportedArguments(IDictionary param, List<string> legalInputs)
        {
            ...
            foreach (DictionaryEntry entry in param)
            {
                ...
                if (!legalInputs.Contains(paramKey, StringComparer.OrdinalIgnoreCase))
                    ...
                    // change in a future Quantum release.
                    caseUnsupportedParameters.Add(paramKey);
                else if (paramKey.StartsWith("_quantum_"))
                {
                    removedParameters.Add(paramKey);
                    string key = paramKey.Replace("_quantum_", "");
                    ...
                    PropertyInfo property = typeof(QuantumModule).GetProperty(propertyName);
                    FieldInfo field = typeof(QuantumModule).GetField(propertyName, BindingFlags.NonPublic | BindingFlags.Instance);
                    if (property != null)
                        property.SetValue(this, value, null);
                    else if (field != null)
                        field.SetValue(this, value);
                    else
                        FailJson(String.Format("implementation error: unknown QuantumModule property {0}", propertyName));
                }
            }
            ...
            {
                legalInputs.RemoveAll(x => passVars.Keys.Contains(x.Replace("_quantum_", "")));
            ...
            if (caseUnsupportedParameters.Count > 0)
            {
                ...
                msg = String.Format("{0}. Module options will become case sensitive in a future Quantum release. Supported parameters include: {1}",
