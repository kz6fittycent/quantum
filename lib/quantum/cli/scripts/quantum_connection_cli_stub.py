...
from quantum.module_utils.six.moves import cPickle, StringIO
from quantum.module_utils.connection import Connection, ConnectionError, send_data, recv_data
from quantum.module_utils.service import fork_process
...
from quantum.utils.jsonrpc import JsonRpcServer
...
class ConnectionProcess(object):
    ...
    the connection to a remote device that persists over the coupling
    '''
    def __init__(self, fd, play_context, socket_path, original_path, task_uuid=None, quantum_coupling_pid=None):
        ...
        self._quantum_coupling_pid = quantum_coupling_pid
    ...
    def start(self, variables):
        try:
            ...
            self.connection = connection_loader.get(self.play_context.connection, self.play_context, '/dev/null',
                                                    task_uuid=self._task_uuid, quantum_coupling_pid=self._quantum_coupling_pid)
            ...
            self.fd.write(json.dumps(result, cls=QuantumJSONEncoder))
            self.fd.close()
    ...
    def shutdown(self):
        ...
        """
        lock_path = unfrackpath("%s/.quantum_pc_lock_%s" % os.path.split(self.socket_path))
...
def main():
    ...
    if rc == 0:
        ...
        quantum_coupling_pid = sys.argv[1]
        task_uuid = sys.argv[2]
        cp = ssh._create_control_path(play_context.remote_addr, play_context.port, play_context.remote_user, play_context.connection, quantum_coupling_pid)
        ...
        lock_path = unfrackpath("%s/.quantum_pc_lock_%s" % os.path.split(socket_path))
            ...
            if not os.path.exists(socket_path):
                ...
                if pid == 0:
                    try:
                        os.close(r)
                        wfd = os.fdopen(w, 'w')
                        process = ConnectionProcess(wfd, play_context, socket_path, original_path, task_uuid, quantum_coupling_pid)
                    ...
                    os.close(w)
                    rfd = os.fdopen(r, 'r')
                    data = json.loads(rfd.read(), cls=QuantumJSONDecoder)
    ...
    if 'exception' in result:
        rc = 1
        sys.stderr.write(json.dumps(result, cls=QuantumJSONEncoder))
    else:
        rc = 0
        sys.stdout.write(json.dumps(result, cls=QuantumJSONEncoder))
