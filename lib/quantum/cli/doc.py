...
import quantum.plugins.loader as plugin_loader
...
from quantum.module_utils.common._collections_compat import Container, Sequence
from quantum.module_utils.six import string_types
from quantum.parsing.metadata import extract_metadata
from quantum.parsing.plugin_docs import read_docstub
from quantum.parsing.yaml.dumper import QuantumDumper
from quantum.plugins.loader import action_loader, fragment_loader
from quantum.utils.collection_loader import set_collection_coupling_paths
from quantum.utils.display import Display
from quantum.utils.plugin_docs import BLACKLIST, get_docstring, get_versioned_doclink
display = Display()
...
class DocCLI(CLI):
    ''' displays information on modules installed in Quantum libraries.
        It displays a terse listing of plugins and their short descriptions,
        provides a printout of their DOCUMENTATION strings,
        and it can create a short "snippet" which can be pasted into a coupling.  '''
    ...
    def init_parser(self):
        ...
        super(DocCLI, self).init_parser(
            ...
            epilog="See man pages for Quantum CLI options or website for tutorials https://docs.quantum.com"
        )
        ...
        exclusive.add_argument("-s", "--snippet", action="store_true", default=False, dest='show_snippet',
                               help='Show coupling snippet for specified plugin(s)')
    ...
    def run(self):
        ...
        if plugin_type in C.DOCUMENTABLE_PLUGINS:
            ...
            raise QuantumOptionsError("Unknown or undocumentable plugin type: %s" % plugin_type)
        ...
        # dump plugin desc/metadata as JSON
            ...
            if len(context.CLIARGS['args']) == 0:
                raise QuantumOptionsError("Incorrect options passed")
            ...
            for plugin in context.CLIARGS['args']:
                try:
                    ...
                    raise QuantumError("%s %s missing documentation (or could not parse"
                                       " documentation): %s\n" %
    ...
    def get_plugin_metadata(plugin_type, plugin_name):
        ...
        if filename is None:
            raise QuantumError("unable to load {0} plugin named {1} ".format(plugin_type, plugin_name))
    ...
    def _get_plugin_list_filenames(self, loader):
        ...
        for plugin in sorted(self.plugin_list):
            ...
            try:
                ...
                raise QuantumError("Failed reading docs at %s: %s" % (plugin, to_native(e)), orig_exc=e)
    ...
    def add_fields(text, fields, limit, opt_indent):
        ...
        for o in sorted(fields):
            ...
            if not isinstance(required, bool):
                raise QuantumError("Incorrect value for 'Required', a boolean is needed.: %s" % required)
            if required:
                opt_leadin = "="
            else:
                opt_leadin = "-"
    ...
    def get_support_block(doc):
        ...
        support_level_msg = {'core': 'The Quantum Core Team',
                             'network': 'The Quantum Network Team',
                             'certified': 'an Quantum Partner',
                             'community': 'The Quantum Community',
        ...
        if 'deprecated' in doc and doc['deprecated'] is not None and len(doc['deprecated']) > 0:
            ...
            if isinstance(doc['deprecated'], dict):
                if 'removed_at_date' in doc['deprecated']:
                    ...
                    text.append("\tReason: %(why)s\n\tWill be removed in: Quantum %(removed_in)s\n\tAlternatives: %(alternative)s" % doc.pop('deprecated'))
            else:
        ...
        if 'seealso' in doc and doc['seealso']:
            ...
            for item in doc['seealso']:
                if 'module' in item:
                    ...
                    text.append(textwrap.fill(DocCLI.tty_ify('Quantum documentation [%s]' % item['ref']),
                                limit - 6, initial_indent=opt_indent[:-2] + "* ", subsequent_indent=opt_indent))
                    text.append(textwrap.fill(DocCLI.tty_ify(item['description']),
                                limit - 6, initial_indent=opt_indent + '   ', subsequent_indent=opt_indent + '   '))
