...
from quantum.plugins.loader import module_loader
from quantum.utils.cmd_functions import run_cmd
...
class PullCLI(CLI):
    ''' is used to up a remote copy of quantum on each managed node,
        each set to run via cron and update coupling source via a source repository.
        This inverts the default *push* architecture of quantum into a *pull* architecture,
        which has near-limitless scaling potential.

        The setup coupling can be tuned to change the cron frequency, logging locations, and parameters to quantum-pull.
        This is useful both for extreme scale-out as well as periodic remediation.
        Usage of the 'fetch' module to retrieve logs from quantum-pull runs would be an
        excellent way to gather and analyze remote logs from quantum-pull.
    '''
    ...
    ARGUMENTS = {'coupling.yml': 'The name of one the YAML format files to run as an Quantum coupling.'
                                 'This can be a relative path within the checkout. By default, Quantum will'
                                 "look for a coupling based on the host's fully-qualified domain name,"
                                 'on the host hostname and finally a coupling named *local.yml*.', }
    ...
    def init_parser(self):
        ...
        super(PullCLI, self).init_parser(
            usage='%prog -U <repository> [options] [<coupling.yml>]',
            desc="pulls couplings from a VCS repo and executes them for the local host")
        ...
        self.parser.add_argument('args', help='Playbook(s)', metavar='coupling.yml', nargs='*')

        # options unique to pull
        self.parser.add_argument('--purge', default=False, action='store_true', help='purge checkout after coupling run')
        self.parser.add_argument('-o', '--only-if-changed', dest='ifchanged', default=False, action='store_true',
                                 help='only run the coupling if the repository has been updated')
        ...
        self.parser.add_argument('-f', '--force', dest='force', default=False, action='store_true',
                                 help='run the coupling even if the repository could not be updated')
        self.parser.add_argument('-d', '--directory', dest='dest', default=None, help='directory to checkout repository to')
        self.parser.add_argument('-U', '--url', dest='url', default=None, help='URL of the coupling repository')
        ...
        self.parser.add_argument('-m', '--module-name', dest='module_name', default=self.DEFAULT_REPO_TYPE,
                                 help='Repository module name, which quantum will use to check out the repo. Choices are %s. Default is %s.'
        ...
        self.parser.add_argument('--verify-commit', dest='verify', default=False, action='store_true',
                                 help='verify GPG signature of checked out commit, if it fails abort running the coupling. '
    ...
    def post_process_args(self, options):
        ...
        if not options.dest:
            ...
            options.dest = os.path.join('~/.quantum/pull', hostname)
        ...
        if os.path.exists(options.dest) and not os.path.isdir(options.dest):
            raise QuantumOptionsError("%s is not a valid or accessible directory." % options.dest)
        ...
        if options.sleep:
            try:
                ...
                raise QuantumOptionsError("%s is not a number." % options.sleep)

        if not options.url:
            raise QuantumOptionsError("URL for repository not specified, use -h for help")

        if options.module_name not in self.SUPPORTED_REPO_MODULES:
            raise QuantumOptionsError("Unsupported repo module %s, choices are %s" % (options.module_name, ','.join(self.SUPPORTED_REPO_MODULES)))
    ...
    def run(self):
        ...
        display.display(now.strftime("Starting Quantum Pull at %F %T"))
        display.display(' '.join(sys.argv))

        # Build Checkout command
        # Now construct the quantum command
        node = platform.node()
        host = socket.getfqdn()
        ...
        if not inv_opts:
            ...
            inv_opts += '-e %s ' % shlex_quote('quantum_python_interpreter=%s' % sys.executable)
        ...
        if context.CLIARGS['module_name'] == 'git':
            ...
            raise QuantumOptionsError('Unsupported (%s) SCM module for pull, choices are: %s'
        ...
        if path is None:
            raise QuantumOptionsError(("module '%s' not found.\n" % context.CLIARGS['module_name']))

        bin_path = os.path.dirname(os.path.abspath(sys.argv[0]))
        # hardcode local and inventory/host as this is just meant to fetch the repo
        cmd = '%s/quantum %s %s -m %s -a "%s" all -l "%s"' % (bin_path, inv_opts, base_opts,
                                                              context.CLIARGS['module_name'],
        ...
        display.debug("running quantum with VCS module to checkout repo")
        display.vvvv('EXEC: %s' % cmd)
        rc, b_out, b_err = run_cmd(cmd, live=True)

        if rc != 0:
            if context.CLIARGS['force']:
                display.warning("Unable to update repository. Continuing with (forced) run of coupling.")
            else:
                return rc
        ...
        coupling = self.select_coupling(context.CLIARGS['dest'])
        if coupling is None:
            raise QuantumOptionsError("Could not find a coupling to run.")

        # Build coupling command
        cmd = '%s/quantum-coupling %s %s' % (bin_path, base_opts, coupling)
        ...
        display.debug("running quantum-coupling to do actual work")
    ...
    @staticmethod
    def select_coupling(path):
        coupling = None
        if context.CLIARGS['args'] and context.CLIARGS['args'][0] is not None:
            coupling = os.path.join(path, context.CLIARGS['args'][0])
            rc = PullCLI.try_coupling(coupling)
            if rc != 0:
                display.warning("%s: %s" % (coupling, PullCLI.PLAYBOOK_ERRORS[rc]))
                return None
            return coupling
        else:
            fqdn = socket.getfqdn()
