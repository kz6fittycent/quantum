...
class AdHocCLI(CLI):
    ''' is an extra-simple tool/framework/API for doing 'remote things'.
        this command allows you to define and run a single task 'coupling' against a set of hosts
    '''

    def init_parser(self):
        ''' create an options parser for bin/quantum '''
        super(AdHocCLI, self).init_parser(usage='%prog <host-pattern> [options]',
                                          desc="Define and run a single task 'coupling' against"
                                          " a set of hosts",
        ...
        # options unique to quantum ad-hoc
    ...
    def post_process_args(self, options):
        '''Post process and validate options for bin/quantum '''
    ...
    def run(self):
        ''' create and execute the single task coupling '''
        ...
        if context.CLIARGS['module_name'] in C.MODULE_REQUIRE_ARGS and not context.CLIARGS['module_args']:
            ...
            if pattern.endswith(".yml"):
                err = err + ' (did you mean to run quantum-coupling?)'
            raise QuantumOptionsError(err)

        # Avoid modules that don't work with ad-hoc
        if context.CLIARGS['module_name'] in C._ACTION_IMPORT_PLAYBOOK:
            raise QuantumOptionsError("'%s' is not a valid action for ad-hoc commands"
        ...
        coupling = Playbook(loader)
        coupling._entries.append(play)
        coupling._file_name = '__adhoc_coupling__'

        if self.callback:
            cb = self.callback
        elif context.CLIARGS['one_line']:
            cb = 'oneline'
        # Respect custom 'stdout_callback' only with enabled 'bin_quantum_callbacks'
        ...
        try:
            ...
            self._tqm.send_callback('v2_coupling_on_start', coupling)
