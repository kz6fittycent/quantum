...
class QuantumVersion(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        quantum_version = to_native(version(getattr(parser, 'prog')))
        print(quantum_version)
        parser.exit()
...
def unfrack_path(pathsep=False):
    """Turn an Option's data into a single path in Quantum locations"""
    def inner(value):
        if pathsep:
...
def version(prog=None):
    ...
    result = result + "\n  quantum python module location = %s" % ':'.join(quantum.__path__)
...
def create_base_parser(prog, usage="", desc=None, epilog=None):
    """
    Create an options parser for all quantum scripts
    """
    # base opts
    ...
    parser.add_argument('--version', action=QuantumVersion, nargs=0, help=version_help)
...
def add_basedir_options(parser):
    """Add options for commands which can set a coupling basedir"""
    parser.add_argument('--coupling-dir', default=C.config.get_config_value('PLAYBOOK_DIR'), dest='basedir', action='store',
                        help="Since this tool does not use couplings, use this as a substitute coupling directory."
...
def add_check_options(parser):
    ...
    parser.add_argument('--syntax-check', dest='syntax', action='store_true',
                        help="perform a syntax check on the coupling, but do not execute it")
...
def add_runas_options(parser):
    ...
    runas_group.add_argument('--become-method', dest='become_method', default=C.DEFAULT_BECOME_METHOD,
                             help="privilege escalation method to use (default=%(default)s), use "
                                  "`quantum-doc -t become -l` to list valid choices.")
