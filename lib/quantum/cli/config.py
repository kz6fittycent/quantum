...
from quantum.config.manager import ConfigManager, Setting, find_ini_config_file
...
class ConfigCLI(CLI):
    ...
    def init_parser(self):
        ...
        super(ConfigCLI, self).init_parser(
            desc="View quantum configuration.",
        )
    ...
    def run(self):
        ...
        if context.CLIARGS['config_file']:
            ...
            if os.path.exists(b_config) and os.access(b_config, os.R_OK):
                ...
                raise QuantumOptionsError('The provided configuration file is missing or not accessible: %s' % to_native(self.config_file))
        else:
        ...
        if self.config_file:
            try:
                if not os.path.exists(self.config_file):
                    raise QuantumOptionsError("%s does not exist or is not accessible" % (self.config_file))
                elif not os.path.isfile(self.config_file):
                    raise QuantumOptionsError("%s is not a valid file" % (self.config_file))
            ...
            raise QuantumError('Invalid or no config file was supplied')

        context.CLIARGS['func']()

    def execute_update(self):
        '''
        Updates a single setting in the specified quantum.cfg
        '''
        raise QuantumError("Option not implemented yet")

        # pylint: disable=unreachable
        if context.CLIARGS['setting'] is None:
            raise QuantumOptionsError("update option requires a setting to update")
    ...
    def execute_view(self):
        ...
        try:
            ...
            raise QuantumError("Failed to open config file: %s" % to_native(e))

    def execute_edit(self):
        '''
        Opens quantum.cfg in the default EDITOR
        '''
        raise QuantumError("Option not implemented yet")
        ...
        try:
            ...
            raise QuantumError("Failed to open editor: %s" % to_native(e))

    def execute_list(self):
        '''
        list all current configs reading lib/constants.py and shows env and config file setting names
        '''
        self.pager(to_text(yaml.dump(self.config.get_configuration_definitions(), Dumper=QuantumDumper), errors='surrogate_or_strict'))

    def execute_dump(self):
        '''
        Shows the current settings, merges quantum.cfg if specified
        '''
