...
from quantum.parsing.vault import VaultEditor, VaultLib, match_encrypt_secret
...
class VaultCLI(CLI):
    ''' can encrypt any structured data file used by Quantum.
    This can include *group_vars/* or *host_vars/* inventory variables,
    variables loaded by *include_vars* or *vars_files*, or variable files
    passed on the quantum-coupling command line with *-e @file.yml* or *-e @file.json*.
    Role variables and defaults are also included!

    Because Quantum tasks, handlers, and other objects are data, these can also be encrypted with vault.
    ...
    def init_parser(self):
        super(VaultCLI, self).init_parser(
            desc="encryption/decryption utility for Quantum data files",
    ...
    def post_process_args(self, options):
        ...
        if options.vault_ids:
            for vault_id in options.vault_ids:
                if u';' in vault_id:
                    raise QuantumOptionsError("'%s' is not a valid vault id. The character ';' is not allowed in vault ids" % vault_id)

        if getattr(options, 'output_file', None) and len(options.args) > 1:
            raise QuantumOptionsError("At most one input file may be used with the --output option")
        ...
        if options.action == 'encrypt_string':
            ...
            if options.encrypt_string_prompt and self.encrypt_string_read_stdin:
                raise QuantumOptionsError('The --prompt option is not supported if also reading input from stdin')
    ...
    def run(self):
        ...
        if action in ['decrypt', 'view', 'rekey', 'edit']:
            ...
            if not vault_secrets:
                raise QuantumOptionsError("A vault password is required to use Quantum's Vault")
        ...
        if action in ['encrypt', 'encrypt_string', 'create']:
            ...
            if len(vault_secrets) > 1 and not encrypt_vault_id:
                raise QuantumOptionsError("The vault-ids %s are available to encrypt. Specify the vault-id to encrypt with --encrypt-vault-id" %
        ...
        if action in ['rekey']:
            ...
            if not new_vault_secrets:
                raise QuantumOptionsError("A new vault password is required to use Quantum's Vault rekey")
    ...
    def execute_encrypt_string(self):
        ...
        if context.CLIARGS['encrypt_string_prompt']:
            ...
            if prompt_response == '':
                raise QuantumOptionsError('The plaintext provided from the prompt was empty, not encrypting')
        ...
        if self.encrypt_string_read_stdin:
            ...
            if stdin_text == '':
                raise QuantumOptionsError('stdin was empty, not encrypting')
        ...
        for name_and_text in name_and_text_list:
            ...
            if plaintext == '':
                raise QuantumOptionsError('The plaintext provided from the command line args was empty, not encrypting')
    ...
    def _format_output_vault_strings(self, b_plaintext_list, vault_id=None):
        ...
        # Encrypt the plaintext, and format it into a yaml block that can be pasted into a coupling.
    ...
    def execute_create(self):
        ...
        if len(context.CLIARGS['args']) > 1:
            raise QuantumOptionsError("quantum-vault create can take only one filename argument")
