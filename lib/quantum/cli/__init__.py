...
# Copyright: (c) 2016, Toshio Kuratomi <tkuratomi@quantum.com>
...
from quantum.inventory.manager import InventoryManager
from quantum.module_utils.six import with_metaclass, string_types
from quantum.module_utils._text import to_bytes, to_text
from quantum.parsing.dataloader import DataLoader
from quantum.parsing.vault import PromptVaultSecret, get_file_vault_secret
...
from quantum.utils.path import unfrackpath
...
class CLI(with_metaclass(ABCMeta, object)):
    ''' code behind bin/quantum* programs '''
    ...
    def ask_passwords():
        ...
        if sshpass:
            sshpass = QuantumUnsafeText(to_text(sshpass))
        if becomepass:
            becomepass = QuantumUnsafeText(to_text(becomepass))
    ...
    def init_parser(self, usage="", desc=None, epilog=None):
        """
        Create an options parser for most quantum scripts
        ...
        An implementation will look something like this::
            ...
            def init_parser(self):
                super(MyCLI, self).init_parser(usage="My Quantum CLI", inventory_opts=True)
                quantum.arguments.option_helpers.add_runas_options(self.parser)
    ...
    def post_process_args(self, options):
        ...
        An implementation will look something like this::
            ...
            def post_process_args(self, options):
                ...
                if options.addition and options.subtraction:
                    raise QuantumOptionsError('Only one of --addition and --subtraction can be specified')
        ...
        # 'quantum-fog -vvv init' has no verbosity set but 'quantum-fog init -vvv' sets a level of 3. To preserve
        # back compat with pre-argparse changes we manually scan and set verbosity based on the argv values.
        if self.parser.prog in ['quantum-fog', 'quantum-vault'] and not options.verbosity:
    ...
    def version_info(gitinfo=False):
        ''' return full quantum version info '''
        if gitinfo:
            # expensive call, user with care
            quantum_version_string = opt_help.version()
        else:
            quantum_version_string = __version__
        quantum_version = quantum_version_string.split()[0]
        quantum_versions = quantum_version.split('.')
        for counter in range(len(quantum_versions)):
            if quantum_versions[counter] == "":
                quantum_versions[counter] = 0
            try:
                quantum_versions[counter] = int(quantum_versions[counter])
            except Exception:
                pass
        if len(quantum_versions) < 3:
            for counter in range(len(quantum_versions), 3):
                quantum_versions.append(0)
        return {'string': quantum_version_string.strip(),
                'full': quantum_version,
                'major': quantum_versions[0],
                'minor': quantum_versions[1],
                'revision': quantum_versions[2]}
    ...
    def _play_prereqs():
        ...
        if basedir:
            ...
            set_collection_coupling_paths(basedir)
            ...
            if default_collection:
                ...
                QuantumCollectionLoader().set_default_collection(default_collection)
    ...
    def get_host_list(inventory, subset, pattern='all'):
        ...
        if not hosts and no_hosts is False:
            raise QuantumError("Specified hosts and/or --limit does not match any hosts")
