...
from quantum.coupling.helpers import load_list_of_roles
...
class RoleMetadata(Base, CollectionSearch):
    ...
    _fog_info = FieldAttribute(isa='GalaxyInfo')
    ...
    def load(data, owner, variable_manager=None, loader=None):
        ...
        if not isinstance(data, dict):
            raise QuantumParserError("the 'meta/main.yml' for role %s is not a dictionary" % owner.get_name())
    ...
    def _load_dependencies(self, attr, ds):
        ...
        if ds:
            if not isinstance(ds, list):
                raise QuantumParserError("Expected role dependencies to be a list.", obj=self._ds)
            ...
            for role_def in ds:
                if isinstance(role_def, string_types) or 'role' in role_def or 'name' in role_def:
                    ...
                    continue
                try:
                    # role_def is new style: { src: 'fog.role,version,name', other_vars: "here" }
                    ...
                    raise QuantumParserError(to_native(exc), obj=role_def, orig_exc=exc)
        ...
        if self._owner:
            ...
            if 'quantum.legacy' not in collection_search_list:
                collection_search_list.append('quantum.legacy')
        ...
        try:
            ...
            raise QuantumParserError("A malformed list of role dependencies was encountered.", obj=self._ds, orig_exc=e)

    def _load_fog_info(self, attr, ds):
        '''
        This is a helper loading function for the fog info entry
