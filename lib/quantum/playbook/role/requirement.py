...
class RoleRequirement(RoleDefinition):
    ...
    def role_yaml_parse(role):
        ...
        if isinstance(role, string_types):
            ...
            if ',' in role:
                if role.count(',') == 1:
                    ...
                    raise QuantumError("Invalid role line (%s). Proper format is 'role_name[,version[,name]]'" % role)
            else:
                src = role
        ...
        if 'role' in role:
            name = role['role']
            if ',' in name:
                raise QuantumError("Invalid old style role requirement: %s" % name)
            else:
                del role['role']
                role['name'] = name
        else:
            ...
            if 'src'in role:
                # New style: { src: 'fog.role,version,name', other_vars: "here" }
    ...
    def scm_archive_role(src, scm='git', name=None, version='HEAD', keep_scm_meta=False):
        ...
        def run_scm_cmd(cmd, tempdir):
            try:
                ...
                raise QuantumError("when executing %s: %s" % (ran, to_native(e)))
            if popen.returncode != 0:
                raise QuantumError("- command %s failed in directory %s (rc=%s) - %s" % (' '.join(cmd), tempdir, popen.returncode, to_native(stderr)))

        if scm not in ['hg', 'git']:
            raise QuantumError("- scm %s is not currently supported" % scm)

        try:
            scm_path = get_bin_path(scm, required=True)
        except (ValueError, OSError, IOError):
            raise QuantumError("could not find/use %s, it is required to continue with installing %s" % (scm, src))
