...
from quantum.utils.collection_loader import get_collection_role_path, QuantumCollectionRef
...
class RoleDefinition(Base, Conditional, Taggable, CollectionSearch):
    ...
    def load(data, variable_manager=None, loader=None):
        raise QuantumError("not implemented")
    ...
    def preprocess_data(self, ds):
        ...
        if not isinstance(ds, dict) and not isinstance(ds, string_types) and not isinstance(ds, QuantumBaseYAMLObject):
    ...
    def _load_role_name(self, ds):
        ...
        if not role_name or not isinstance(role_name, string_types):
            raise QuantumError('role definitions must contain a role name', obj=ds)
    ...
    def _load_role_path(self, role_name):
        ...
        if self._collection_list or QuantumCollectionRef.is_valid_fqcr(role_name):
        ...
        # quantum.legacy searches with the collections keyword

        # we always start the search for roles in the base directory of the coupling
        role_search_paths = [
        ...
        # in the loader (which should be the coupling dir itself) but without
        ...
        raise QuantumError("the role '%s' was not found in %s" % (role_name, ":".join(searches)), obj=self._ds)
