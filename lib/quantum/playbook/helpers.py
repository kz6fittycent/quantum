...
from quantum.errors import QuantumParserError, QuantumUndefinedVariable, QuantumFileNotFound, QuantumAssertionError
from quantum.module_utils._text import to_native
from quantum.module_utils.six import string_types
from quantum.parsing.mod_args import ModuleArgsParser
...
def load_list_of_blocks(ds, play, parent_block=None, role=None, task_include=None, use_handlers=False, variable_manager=None, loader=None):
    ...
    if not isinstance(ds, (list, type(None))):
        raise QuantumAssertionError('%s should be a list or None but is %s' % (ds, type(ds)))
...
def load_list_of_tasks(ds, play, block=None, role=None, task_include=None, use_handlers=False, variable_manager=None, loader=None):
    ...
    from quantum.coupling.role_include import IncludeRole
    from quantum.coupling.handler_task_include import HandlerTaskInclude
    from quantum.template import Templar

    if not isinstance(ds, list):
        raise QuantumAssertionError('The ds (%s) should be a list but was a %s' % (ds, type(ds)))

    task_list = []
    for task_ds in ds:
        if not isinstance(task_ds, dict):
            raise QuantumAssertionError('The ds (%s) should be a dict but was a %s' % (ds, type(ds)))
        ...
        if 'block' in task_ds:
            ...
            try:
                ...
                raise QuantumParserError(to_native(e), obj=task_ds, orig_exc=e)
            ...
            if action in C._ACTION_ALL_INCLUDE_IMPORT_TASKS:
                ...
                if is_static:
                    if t.loop is not None:
                        if action in C._ACTION_IMPORT_TASKS:
                            raise QuantumParserError("You cannot use loops on 'import_tasks' statements. You should use 'include_tasks' instead.", obj=task_ds)
                        else:
                            raise QuantumParserError("You cannot use 'static' on an include with a loop", obj=task_ds)
                    ...
                    try:
                        ...
                        if data is None:
                            ...
                            raise QuantumParserError("included task files must contain a list of tasks", obj=data)
                        ...
                        display.deprecated(
                            ...
                            "dynamic, use 'static: yes' on the include or set the global quantum.cfg "
                    ...
                    if len(tags) > 0:
                        if action in C._ACTION_ALL_PROPER_INCLUDE_IMPORT_TASKS:
                            raise QuantumParserError('You cannot specify "tags" inline to the task, it is a task keyword')
                ...
                if is_static:
                    if ir.loop is not None:
                        if action in C._ACTION_IMPORT_ROLE:
                            raise QuantumParserError("You cannot use loops on 'import_role' statements. You should use 'include_role' instead.", obj=task_ds)
                        else:
                            raise QuantumParserError("You cannot use 'static' on an include_role with a loop", obj=task_ds)
...
def load_list_of_roles(ds, play, current_role_path=None, variable_manager=None, loader=None, collection_search_list=None):
    ...
    if not isinstance(ds, list):
        raise QuantumAssertionError('ds (%s) should be a list but was a %s' % (ds, type(ds)))
