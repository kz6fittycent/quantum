...
class IncludeRole(TaskInclude):
    ...
    def load(data, block=None, role=None, task_include=None, variable_manager=None, loader=None):
        ...
        if ir._role_name is None:
            raise QuantumParserError("'name' is a required field for %s." % ir.action, obj=data)

        if 'public' in ir.args and ir.action not in C._ACTION_INCLUDE_ROLE:
            raise QuantumParserError('Invalid options for %s: public' % ir.action, obj=data)

        # validate bad args, otherwise we silently ignore
        bad_opts = my_arg_names.difference(IncludeRole.VALID_ARGS)
        if bad_opts:
            raise QuantumParserError('Invalid options for %s: %s' % (ir.action, ','.join(list(bad_opts))), obj=data)
        ...
        if apply_attrs and ir.action not in C._ACTION_INCLUDE_ROLE:
            raise QuantumParserError('Invalid options for %s: apply' % ir.action, obj=data)
        elif not isinstance(apply_attrs, dict):
            raise QuantumParserError('Expected a dict for apply but got %s instead' % type(apply_attrs), obj=data)
    ...
    def get_include_params(self):
        ...
        if self._parent_role:
            ...
            v.setdefault('quantum_parent_role_names', []).insert(0, self._parent_role.get_name())
            v.setdefault('quantum_parent_role_paths', []).insert(0, self._parent_role._role_path)
        return v
