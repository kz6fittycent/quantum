...
from quantum.errors import QuantumError, QuantumParserError, QuantumUndefinedVariable, QuantumAssertionError
...
from quantum.plugins.loader import lookup_loader
...
from quantum.coupling.loop_control import LoopControl
...
class Task(Base, Conditional, Taggable, CollectionSearch):
    ...
    def _preprocess_with_loop(self, ds, new_ds, k, v):
        ...
        if new_ds.get('loop') is not None or new_ds.get('loop_with') is not None:
            raise QuantumError("duplicate loop in task: %s" % loop_name, obj=ds)
        if v is None:
            raise QuantumError("you must specify a value when using %s" % k, obj=ds)
    ...
    def preprocess_data(self, ds):
        ...
        if collections_list and 'quantum.builtin' not in collections_list and 'quantum.legacy' not in collections_list:
            collections_list.append('quantum.legacy')
        ...
        try:
            ...
            raise QuantumParserError(to_native(e), obj=ds, orig_exc=e)
        ...
        if action in C._ACTION_HAS_CMD:
            if 'cmd' in args:
                if args.get('_raw_params', '') != '':
                    raise QuantumError("The 'cmd' argument cannot be used when other raw parameters are specified."
            ...
            if k in ('action', 'local_action', 'args', 'delegate_to') or k == action or k == 'shell':
                ...
                if action in C._ACTION_INCLUDE and k not in self._valid_attrs and k not in self.DEPRECATED_ATTRIBUTES:
                    display.deprecated("Specifying include variables at the top-level of the task is deprecated."
                                       " Please see:\nhttps://docs.quantum.com/quantum/couplings_roles.html#task-include-files-and-encouraging-reuse\n\n"
    ...
    def post_validate(self, templar):
        ...
        if QuantumCollectionLoader().default_collection:
            pass
        ...
        if value is not None:
            ...
            def _parse_env_kv(k, v):
                try:
                    ...
                    error = to_native(e)
                    if self.action in C._ACTION_FACT_GATHERING and 'quantum_facts.env' in error or 'quantum_env' in error:
    ...
    def get_search_path(self):
        ...
        This follows role/coupling dependency chain.
        '''
        path_stack = []
