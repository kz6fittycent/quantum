...
from quantum.plugins import get_plugin_class
from quantum.utils.display import Display
from quantum.plugins.loader import get_shell_plugin
...
class PlayContext(Base):
    ...
    def set_task_and_variable_override(self, task, variables, templar):
        ...
        # If the value 'quantum_delegated_vars' is in the variables, it means
        ...
        if task.delegate_to is not None:
            ...
            delegated_vars = variables.get('quantum_delegated_vars', dict()).get(delegated_host_name, dict())
            ...
            for address_var in ('quantum_%s_host' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('remote_addr'):
                if address_var in delegated_vars:
                    break
            else:
                display.debug("no remote address found for delegated host %s\nusing its name, so success depends on DNS resolution" % delegated_host_name)
                delegated_vars['quantum_host'] = delegated_host_name

            # reset the port back to the default if none was specified, to prevent
            # the delegated host from inheriting the original host's setting
            for port_var in ('quantum_%s_port' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('port'):
                if port_var in delegated_vars:
                    break
            else:
                if delegated_transport == 'winrm':
                    delegated_vars['quantum_port'] = 5986
                else:
                    delegated_vars['quantum_port'] = C.DEFAULT_REMOTE_PORT

            # and likewise for the remote user
            for user_var in ('quantum_%s_user' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('remote_user'):
                if user_var in delegated_vars and delegated_vars[user_var]:
                    break
            else:
                delegated_vars['quantum_user'] = task.remote_user or self.remote_user
        else:
            delegated_vars = dict()
    ...
    def make_become_cmd(self, cmd, executable=None):
        ...
        if plugin:
            ...
            raise QuantumError("Privilege escalation method not found: %s" % become_method)
