...
from quantum.errors import QuantumParserError, QuantumUndefinedVariable, QuantumAssertionError
...
from quantum.utils.sentinel import Sentinel
from quantum.utils.vars import combine_vars, isidentifier, get_unique_id
...
class FieldAttributeBase(with_metaclass(BaseMeta, object)):
    ...
    def load_data(self, ds, variable_manager=None, loader=None):
        ...
        if ds is None:
            raise QuantumAssertionError('ds (%s) should not be None but it is.' % ds)
    ...
    def _post_validate_debugger(self, attr, value, templar):
        ...
        if value and isinstance(value, string_types) and value not in valid_values:
            raise QuantumParserError("'%s' is not a valid value for debugger. Must be one of %s" % (value, ', '.join(valid_values)), obj=self.get_ds())
        return value
    ...
    def _validate_attributes(self, ds):
        ...
        for key in ds:
            if key not in valid_attrs:
                raise QuantumParserError("'%s' is not a valid attribute for a %s" % (key, self.__class__.__name__), obj=ds)
            ...
            if attribute.listof is not None:
                for item in value:
                    if not isinstance(item, attribute.listof):
                        raise QuantumParserError("the field '%s' should be a list of %s, "
                                                 "but the item '%s' is a %s" % (name, attribute.listof, item, type(item)), obj=self.get_ds())
                    elif attribute.required and attribute.listof == string_types:
                        if item is None or item.strip() == "":
                            raise QuantumParserError("the field '%s' is required, and cannot have empty values" % (name,), obj=self.get_ds())
        ...
        for (name, attribute) in iteritems(self._valid_attrs):
            ...
            if getattr(self, name) is None:
                if not attribute.required:
                    ...
                    raise QuantumParserError("the field '%s' is required but was not set" % name)
            ...
            try:
                ...
                value = getattr(self, name)
                raise QuantumParserError("the field '%s' has an invalid value (%s), and could not be converted to an %s."
                ...
                if templar._fail_on_undefined_errors and name != 'name':
                    ...
                    raise QuantumParserError(msg, obj=self.get_ds(), orig_exc=e)
    ...
    def _load_vars(self, attr, ds):
        ...
        try:
            ...
            raise QuantumParserError("Vars in a %s must be specified as a dictionary, or a list of dictionaries" % self.__class__.__name__,
                                     obj=ds, orig_exc=e)
        except TypeError as e:
            raise QuantumParserError("Invalid variable name in vars specified for %s: %s" % (self.__class__.__name__, e), obj=ds, orig_exc=e)
    ...
    def deserialize(self, data):
        ...
        if not isinstance(data, dict):
            raise QuantumAssertionError('data (%s) should be a dict but is a %s' % (data, type(data)))
