...
- Runs a remote command from a Linux host to a Windows host without WinRM being
  set up.
- Can be run on the Quantum controller to bootstrap Windows hosts to get them
  ready for WinRM.
options:
  hostname:
    description:
...
- name: Download and run ConfigureRemotingForQuantum.ps1 to setup WinRM
  psexec:
    hostname: '{{ hostvars[inventory_hostname]["quantum_host"] | default(inventory_hostname) }}'
    connection_username: '{{ quantum_user }}'
    connection_password: '{{ quantum_password }}'
    encrypt: yes
    executable: powershell.exe
    arguments: '-'
    stdin: |
      ...
      $url = "https://github.com/quantum/quantum/raw/devel/examples/scripts/ConfigureRemotingForQuantum.ps1"
