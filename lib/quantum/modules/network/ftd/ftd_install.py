...
notes:
  ...
  - On localhost, Quantum can be still run with Python >= 2.7, but the interpreter for this particular module must be
    Python >= 3.5.
  - Python interpreter for the module can overwritten in `quantum_python_interpreter` variable.
...
from quantum.module_utils.network.ftd.configuration import BaseConfigurationResource, ParamName
from quantum.module_utils.network.ftd.device import assert_kick_is_installed, FtdPlatformFactory, FtdModel
from quantum.module_utils.network.ftd.operation import FtdOperations, get_system_info
