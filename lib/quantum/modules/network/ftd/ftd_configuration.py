...
options:
  ...
  register_as:
    description:
      - Specifies Quantum fact name that is used to register received response from the FTD device.
    type: str
  filters:
    description:
...
- name: Create a network object
  ftd_configuration:
    ...
    data:
      name: "Quantum-network-host"
      description: "From Quantum with love"
      subType: "HOST"
      value: "192.168.2.0"
...
from quantum.module_utils.network.ftd.configuration import BaseConfigurationResource, CheckModeException, \
    FtdInvalidOperationNameError
from quantum.module_utils.network.ftd.fdm_swagger_client import ValidationError
from quantum.module_utils.network.ftd.common import construct_quantum_facts, FtdConfigurationError, \
...
def main():
    ...
    try:
        ...
        module.exit_json(changed=resource.config_changed, response=resp,
                         quantum_facts=construct_quantum_facts(resp, module.params))
