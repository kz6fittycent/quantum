...
- name: create new user account
  junos_user:
    name: quantum
    role: super-user
    sshkey: "{{ lookup('file', '~/.ssh/quantum.pub') }}"
    state: present
...
- name: remove all user accounts except quantum
  junos_user:
    aggregate:
    - name: quantum
    purge: yes
...
from quantum.module_utils.network.junos.junos import junos_argument_spec, get_connection, tostring
from quantum.module_utils.network.junos.junos import commit_configuration, discard_changes
from quantum.module_utils.network.junos.junos import load_config, locked_config
