...
description:
  - This module supports two different workflows for copying a file
    ...
    from the Quantum controller to the device or (2) pulled from a remote SCP
...
options:
  local_file:
    description:
      - When (file_pull is False) this is the path to the local file on the Quantum controller.
  ...
  file_pull:
    description:
      - When (False) file is copied from the Quantum controller to the NXOS device.
...
EXAMPLES = '''
# File copy from quantum controller to nxos device
