...
description:
  - Collects facts from Cisco Nexus devices running the NX-OS operating
    ...
    with C(quantum_net_<fact>).  The facts module will always collect a
...
RETURN = """
quantum_net_gather_subset:
  description: The list of fact subsets collected from the device
  returned: always
  type: list
quantum_net_gather_network_resources:
...
quantum_net_api:
  ...
  returned: always
  type: str
quantum_net_license_hostid:
  description: The License host id of the device
  returned: always
  type: str
quantum_net_python_version:
  description: The Python version Quantum controller is using
  returned: always
  type: str
# hardware
quantum_net_filesystems:
  description: All file system names available on the device
  returned: when hardware is configured
  type: list
quantum_net_memfree_mb:
  description: The available free memory on the remote device in Mb
  returned: when hardware is configured
  type: int
quantum_net_memtotal_mb:
...
quantum_net_config:
  ...
  type: str
# interfaces
quantum_net_all_ipv4_addresses:
  description: All IPv4 addresses configured on the device
  returned: when interfaces is configured
  type: list
quantum_net_all_ipv6_addresses:
  description: All IPv6 addresses configured on the device
  returned: when interfaces is configured
  type: list
quantum_net_interfaces:
  description: A hash of all interfaces running on the system
  returned: when interfaces is configured
  type: dict
quantum_net_neighbors:
  description:
    - The list of LLDP and CDP neighbors from the device. If both,
      CDP and LLDP neighbor data is present on one port, CDP is preferred.
  returned: when interfaces is configured
  type: dict
# legacy (pre Quantum 2.2)
fan_info:
...
def main():
    """ Main entry point for QuantumModule
    """
    ...
    quantum_facts, additional_warnings = result
    warnings.extend(additional_warnings)

    module.exit_json(quantum_facts=quantum_facts, warnings=warnings)
