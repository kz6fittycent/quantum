  ...
  purge:
    description:
      - Remove any switch logging configuration that does not match what has been configured
        Not supported for quantum_connection local.
        All nxos_logging tasks must use the same quantum_connection type.
...
- name: Configure Source Interface for Logging
  nxos_logging:
    interface: mgmt0
    state: present
- name: Purge nxos_logging configuration not managed by this coupling
  nxos_logging:
    purge: true
...
from quantum.module_utils.network.nxos.nxos import get_config, load_config, run_commands, save_module_context, read_module_context
from quantum.module_utils.network.nxos.nxos import nxos_argument_spec, check_args, normalize_interface
