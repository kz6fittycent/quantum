...
#     api_version: v5_0
#   enterprise_name: Quantum-Enterprise
#   enterprise_new_name: Quantum-Updated-Enterprise
#
...
- name: Create multiple DomainTemplates
  ...
  nuage_vspk:
    ...
    properties:
      name: "{{ item }}"
      description: "Created by Quantum"
  with_items:
    - "Template-1"
    - "Template-2"
...
class NuageEntityManager(object):
    ...
    def _handle_absent(self):
        """
        Handles the Quantum task when the state is set to absent
        """
        # Absent state
    ...
    def _handle_present(self):
        """
        Handles the Quantum task when the state is set to present
        """
        # Present state
        self.entity = self._find_entity(entity_id=self.entity_id, entity_class=self.entity_class, match_filter=self.match_filter, properties=self.properties,
                                        entity_fetcher=self.entity_fetcher)
    ...
    def _handle_get_csp_enterprise(self):
        """
        Handles the Quantum task when the command is to get the csp enterprise
        """
    ...
    def _handle_wait_for_job(self):
        """
        Handles the Quantum task when the command is to wait for a job
        """
    ...
    def _handle_change_password(self):
        """
        Handles the Quantum task when the command is to change a password
        """
        # Command change_password
        self.entity = self._find_entity(entity_id=self.entity_id, entity_class=self.entity_class, match_filter=self.match_filter, properties=self.properties,
                                        entity_fetcher=self.entity_fetcher)
    ...
    def _handle_find(self):
        """
        Handles the Quantum task when the command is to find an entity
        """
        # Command find
    ...
    def _assign_member(self, entity_fetcher, entity, entity_class, parent, set_output):
        ...
        :param set_output: If set to True, sets the Quantum result variables
        """
        members = entity_fetcher.get()
