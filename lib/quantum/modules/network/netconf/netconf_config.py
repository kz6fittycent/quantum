...
options:
  ...
  src:
    ...
    - Specifies the source path to the xml file that contains the configuration or configuration template
      to load. The path to the source file can either be the full path on the Quantum control host or
      a relative path from the coupling or role root directory. This argument is mutually exclusive with I(xml).
    version_added: "2.4"
  backup_options:
    description:
...
from quantum.module_utils.network.netconf.netconf import get_capabilities, get_config, sanitize_xml
