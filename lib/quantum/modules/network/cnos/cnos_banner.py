...
description:
  - This will configure both login and motd banners on remote devices
    running Lenovo CNOS.  It allows couplings to add or remote
  ...
  banner:
    description:
      - Specifies which banner should be configured on the remote device.
        In Quantum 2.8 and earlier only I(login) and I(motd) were supported.
    required: true
    choices: ['login', 'motd']
  text:
    description:
...
from quantum.module_utils.network.cnos.cnos import load_config, run_commands
