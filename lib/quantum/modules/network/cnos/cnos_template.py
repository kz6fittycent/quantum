...
description:
    - This module allows you to work with the running configuration of a switch. It provides a way
     ...
     that must be created by the user in their local directory to where the coupling is run.
version_added: "2.3"
    ...
    commandfile:
        description:
           - This specifies the path to the CNOS command file which needs to be applied. This usually
             ...
             Note The command file must contain the Quantum keyword {{ inventory_hostname }} in its
...
import os
try:
    from quantum.module_utils.network.cnos import cnos
    HAS_LIB = True
except Exception:
    HAS_LIB = False
