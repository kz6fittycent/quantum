  ...
  backup:
    description:
      - The C(backup) argument will backup the current devices active
        configuration to the Quantum control host prior to making any
        changes. If the C(backup_options) value is not given, the
        backup file will be located in the backup folder in the coupling
...
backup_path:
  ...
  type: str
  sample: /couplings/quantum/backup/vyos_config.2016-07-16@22:28:34
filename:
...
shortname:
  ...
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: /couplings/quantum/backup/vyos_config
date:
...
from quantum.module_utils.network.vyos.vyos import load_config, get_config, run_commands
from quantum.module_utils.network.vyos.vyos import vyos_argument_spec, get_connection
