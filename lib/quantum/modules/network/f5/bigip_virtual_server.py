  ...
  metadata:
    description:
      - Arbitrary key/value pairs that you can attach to a virtual server. This is useful in
        situations where you might want to annotate a virtual to be managed by Quantum.
...
from quantum.module_utils.basic import env_fallback
...
try:
    ...
    from library.module_utils.network.f5.common import QuantumF5Parameters
    ...
    from quantum.module_utils.network.f5.bigip import F5RestClient
    from quantum.module_utils.network.f5.common import MANAGED_BY_ANNOTATION_VERSION
    from quantum.module_utils.network.f5.common import MANAGED_BY_ANNOTATION_MODIFIED
    from quantum.module_utils.network.f5.common import F5ModuleError
    from quantum.module_utils.network.f5.common import QuantumF5Parameters
    from quantum.module_utils.network.f5.common import fq_name
    from quantum.module_utils.network.f5.common import f5_argument_spec
    from quantum.module_utils.network.f5.common import transform_name
    from quantum.module_utils.network.f5.common import mark_managed_by
    from quantum.module_utils.network.f5.common import only_has_managed_metadata
    from quantum.module_utils.network.f5.common import flatten_boolean
    from quantum.module_utils.network.f5.compare import cmp_simple_list
    from quantum.module_utils.network.f5.ipaddress import is_valid_ip
    from quantum.module_utils.network.f5.ipaddress import is_valid_ip_interface
    from quantum.module_utils.network.f5.ipaddress import ip_interface
    from quantum.module_utils.network.f5.ipaddress import validate_ip_v6_address
    from quantum.module_utils.network.f5.ipaddress import get_netmask
    from quantum.module_utils.network.f5.ipaddress import compress_address
    from quantum.module_utils.network.f5.icontrol import modules_provisioned


class Parameters(QuantumF5Parameters):
    api_map = {
...
class VirtualServerValidator(object):
    ...
    def _verify_virtual_has_required_parameters(self):
        ...
        Quantum supports ``default`` variables in an Argument Spec, but those defaults
        apply to all operations; including create, update, and delete. Since users are not
        required to always specify these parameters, we cannot use Quantum's facility.
...
class ModuleManager(object):
    ...
    def update_on_device(self):
        ...
        if self.want.insert_metadata:
            # Mark the resource as managed by Quantum, this is default behavior
            params = mark_managed_by(self.module.quantum_version, params)
