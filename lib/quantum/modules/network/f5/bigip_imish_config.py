...
options:
  ...
  before:
    description:
      ...
      - This allows the coupling designer the opportunity to perform
  ...
  after:
    description:
      ...
      - Just like with I(before) this allows the coupling designer to
  ...
  backup:
    description:
      - This argument will cause the module to create a full backup of
        ...
        changes are made.
      - The backup file is written to the C(backup) folder in the coupling
        root directory or role root directory, if coupling is part of an
        quantum role. If the directory does not exist, it is created.
    type: bool
    default: 'no'
  running_config:
    description:
      ...
      - There are times when it is not desirable to have the task get the
        current running-config for every task in a coupling.
  ...
  diff_against:
    description:
      - When using the C(quantum-coupling --diff) command line argument
...
backup_path:
  ...
  type: str
  sample: /couplings/quantum/backup/bigip_imish_config.2016-07-16@22:28:34
'''
...
from quantum.module_utils.network.common.config import NetworkConfig, dumps
...
try:
    ...
    from quantum.module_utils.network.f5.icontrol import upload_file
