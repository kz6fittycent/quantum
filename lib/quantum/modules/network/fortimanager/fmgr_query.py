...
short_description: Query FortiManager data objects for use in Quantum workflows.
description:
  - Provides information on data objects within FortiManager so that couplings can perform conditionals.
...
- name: QUERY FORTIGATE DEVICE BY FRIENDLY NAME
  fmgr_query:
    adom: "quantum"
    object: "device"
    device_unique_name: "quantum-fgt01"
...
- name: VERIFY CLUSTER MEMBERS AND STATUS
  fmgr_query:
    ...
    nodes: ["quantum-fgt01", "quantum-fgt02", "quantum-fgt03"]
...
- name: USE CUSTOM TYPE TO QUERY AVAILABLE SCRIPTS
  fmgr_query:
    adom: "quantum"
    object: "custom"
    custom_endpoint: "/dvmdb/adom/quantum/script"
    ...
    try:
        ...
        if paramgram["object"] == "cluster_nodes" and paramgram["nodes"] is not None:
            ...
            if results["cluster_status"] == "MISSING":
                ...
                module.fail_json(msg="Query FAILED -- Check module or coupling syntax")
    except Exception as err:
        raise FMGBaseException(err)
