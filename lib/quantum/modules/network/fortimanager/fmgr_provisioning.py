...
- name: Create FGT1 Model Device
  fmgr_provisioning:
    ...
    description: "Provisioned by Quantum"
    os_version: '6.0'
    minor_release: 0
    patch_release: 0
    os_type: 'fos'
...
from quantum.module_utils.network.fortimanager.fortimanager import QuantumFortiManager
...
def set_devprof_scope(self, provisioning_template, adom, provision_targets):
    ...
    fields["type"] = "devprof"
    fields["description"] = "CreatedByQuantum"
...
def main():
    ...
    module = QuantumModule(argument_spec, supports_check_mode=True, )
    ...
    fmg = QuantumFortiManager(module, module.params["host"], module.params["username"], module.params["password"])
    response = fmg.login()
    ...
    if "FortiManager instance connnected" not in str(response):
        ...
        # results = assign_dev_grp(fmg, 'Quantum', 'FGVM000000117992', 'root', 'root')
        # if not results[0] == 0:
