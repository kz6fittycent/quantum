...
try:
    from quantum.module_utils.network.avi.avi import (
        avi_common_argument_spec, quantum_return, HAS_AVI)
    from quantum.module_utils.network.avi.quantum_utils import (
        avi_quantum_api)
except ImportError:
    HAS_AVI = False
...
def main():
    ...
    module = QuantumModule(argument_spec=argument_specs, supports_check_mode=True)
    if not HAS_AVI:
        return module.fail_json(msg=(
            'Avi python API SDK (avisdk>=17.1) or requests is not installed. '
            'For more details visit https://github.com/avinetworks/sdk.'))
    return avi_quantum_api(module, 'user',
                           set([]))
