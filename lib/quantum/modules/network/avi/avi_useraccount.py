...
try:
    from quantum.module_utils.network.avi.avi import (
        avi_common_argument_spec, quantum_return, avi_obj_cmp,
...
def main():
    argument_specs = dict(
        ...
        # To handle both Saas and conventional (Entire state in coupling) scenario.
        force_change=dict(type='bool', default=False)
    )
    argument_specs.update(avi_common_argument_spec())
    module = QuantumModule(argument_spec=argument_specs)
    if not HAS_AVI:
    ...
    # First try new password if 'force_change' is set to false or not specified in coupling.
    else:
    ...
    if not password_changed:
        ...
        if not force_change:
            ...
            if rsp:
                password_changed = True
    if password_changed:
        return quantum_return(module, rsp, True, req=data)
    else:
        return quantum_return(module, rsp, False, req=data)
