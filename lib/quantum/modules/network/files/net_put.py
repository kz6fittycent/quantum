...
short_description: Copy a file from Quantum Controller to a network device
description:
  - This module provides functionality to copy file from Quantum controller to
    network devices.
  ...
  src:
    description:
      - Specifies the source file. The path to the source file can either be
        the full path on the Quantum control host or a relative path from the
        coupling or role root directory.
    required: true
  protocol:
    description:
...
EXAMPLES = """
- name: copy file from quantum controller to a network device
  net_put:
