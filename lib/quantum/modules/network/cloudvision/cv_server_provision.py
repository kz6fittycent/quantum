...
version_added: "2.4"
author: "EOS+ CS (quantum-dev@arista.com) (@mharista)"
short_description:
...
portConfigurable:
  description: Signifies if the user specified port has an entry in the
               configlet that Quantum has access to
  returned: success
  type: bool
  sample: true
switchConfigurable:
...
def connect(module):
    ''' Connects to CVP device using user provided credentials from coupling.

    :param module: Quantum module with parameters and client connection.
...
def server_configurable_configlet(module, sw_info):
    ''' Check CVP that the user specified switch has a configlet assigned to
        it that Quantum is allowed to edit.
...
def port_configurable(module, configlet):
    ''' Check configlet if the user specified port has a configuration entry
        in the configlet to determine if Quantum is allowed to configure the
        port on this switch.
    ...
    try:
        ...
        if module.params['auto_run'] and module.params['action'] != 'show':
            ...
            if task_id:
                ...
                note = ('Update config on %s with %s action from Quantum.'
                        % (module.params['switch_name'],
