...
description:
     ...
     - By default, if an environment variable C(<protocol>_proxy) is set on
       ...
       <https://docs.quantum.com/couplings_environment.html>`_),
       or by using the use_proxy option.
     - HTTP redirects can redirect from HTTP to HTTPS so you should be sure that
       your proxy environment for both protocols is correct.
     - From Quantum 2.4 when run with C(--check), it will do a HEAD request to validate the URL but
...
options:
  ...
  tmp_dest:
    description:
      ...
      - When run on Quantum 2.5 or greater, path defaults to quantum's remote_tmp setting
      - When run on Quantum prior to 2.5, it defaults to C(TMPDIR), C(TEMP) or C(TMP) env variables or a platform specific value.
  ...
  headers:
    description:
        ...
        - The hash/dict format was added in Quantum 2.6.
