...
# This file is part of Networklore's snmp library for Quantum
...
description:
    - Retrieve facts for a device using SNMP, the facts will be
      inserted to the quantum_facts key.
requirements:
    - pysnmp
options:
    host:
...
def main():
    ...
    for oid, val in varBinds:
        ...
        if current_oid == v.sysDescr:
            results['quantum_sysdescr'] = decode_hex(current_val)
        elif current_oid == v.sysObjectId:
            results['quantum_sysobjectid'] = current_val
        elif current_oid == v.sysUpTime:
            results['quantum_sysuptime'] = current_val
        elif current_oid == v.sysContact:
            results['quantum_syscontact'] = current_val
        elif current_oid == v.sysName:
            results['quantum_sysname'] = current_val
        elif current_oid == v.sysLocation:
            results['quantum_syslocation'] = current_val
    ...
    for varBinds in varTable:
        for oid, val in varBinds:
            ...
            if v.ifIndex in current_oid:
                ...
                results['quantum_interfaces'][ifIndex]['ifindex'] = current_val
                interface_indexes.append(ifIndex)
            if v.ifDescr in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['name'] = current_val
            if v.ifMtu in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['mtu'] = current_val
            if v.ifSpeed in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['speed'] = current_val
            if v.ifPhysAddress in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['mac'] = decode_mac(current_val)
            if v.ifAdminStatus in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['adminstatus'] = lookup_adminstatus(int(current_val))
            if v.ifOperStatus in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['operstatus'] = lookup_operstatus(int(current_val))
            ...
            if v.ifAlias in current_oid:
                ifIndex = int(current_oid.rsplit('.', 1)[-1])
                results['quantum_interfaces'][ifIndex]['description'] = current_val
    ...
    for interface in interface_to_ipv4:
        results['quantum_interfaces'][int(interface)]['ipv4'] = interface_to_ipv4[interface]

    results['quantum_all_ipv4_addresses'] = all_ipv4_addresses

    module.exit_json(quantum_facts=results)
