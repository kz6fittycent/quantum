...
- name: update an ipv4 host record
  nios_host_record:
    name: {new_name: host-new.quantum.com, old_name: host.quantum.com}
    ipv4:
      - address: 192.168.10.1
    state: present
    provider:
...
from quantum.module_utils.net_tools.nios.api import NIOS_HOST_RECORD


def ipaddr(module, key, filtered_keys=None):
    ''' Transforms the input value into a struct supported by WAPI
    This function will transform the input from the coupling into a struct
