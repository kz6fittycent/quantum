...
options:
  ...
  host_all:
    description:
      - Override the host option, making quantum apply changes to all hostnames for a given user.
...
notes:
   - "MySQL server installs with default login_user of 'root' and no password. To secure this user
     as part of an idempotent coupling, you must create at least two tasks: the first must change the root user's password,
     without providing any login_user/login_password details. The second must drop a ~/.my.cnf file containing
     the new root credentials. Subsequent runs of the coupling will then succeed by reading the new credentials from
     the file."
...
from quantum.module_utils.database import SQLParseError
from quantum.module_utils.mysql import mysql_connect, mysql_driver, mysql_driver_fail_msg
...
def privileges_unpack(priv, mode):
    ...
    custom format to avoid using YAML/JSON strings inside YAML couplings. Example
