...
options:
  ...
  state:
    ...
    - C(dump) requires a target definition to which the database will be backed up. (Added in Quantum 2.4)
    ...
    - C(restore) also requires a target definition from which the database will be restored. (Added in Quantum 2.4)
  ...
  maintenance_db:
    description:
      - The value specifies the initial database (which is also called as maintenance DB) that Quantum connects to.
    type: str
    default: postgres
    version_added: '2.5'
  conn_limit:
...
import quantum.module_utils.postgres as pgutils
from quantum.module_utils.basic import QuantumModule
from quantum.module_utils.database import SQLParseError, pg_quote_identifier
