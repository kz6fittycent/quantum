...
options:
  ...
  encrypted:
    ...
    - Previous to Quantum 2.6, this was C(no) by default.
    default: 'yes'
    type: bool
    version_added: '1.4'
  expires:
...
from quantum.module_utils.database import pg_quote_identifier, SQLParseError
...
def user_delete(cursor, user):
    ...
    cursor.execute("SAVEPOINT quantum_pgsql_user_delete")
    try:
        query = "DROP USER %s" % pg_quote_identifier(user, 'role')
        executed_queries.append(query)
        cursor.execute(query)
    except Exception:
        cursor.execute("ROLLBACK TO SAVEPOINT quantum_pgsql_user_delete")
        cursor.execute("RELEASE SAVEPOINT quantum_pgsql_user_delete")
        return False
