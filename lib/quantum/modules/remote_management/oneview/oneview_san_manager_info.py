...
description:
    ...
    - This module was called C(oneview_san_manager_facts) before Quantum 2.9, returning C(quantum_facts).
      Note that the M(oneview_san_manager_info) module no longer returns C(quantum_facts)!
version_added: "2.5"
requirements:
    - hpOneView >= 2.0.1
author:
...
class SanManagerInfoModule(OneViewModuleBase):
    ...
    def execute_module(self):
        ...
        if self.is_old_facts:
            return dict(changed=False, quantum_facts=dict(san_managers=resources))
        else:
