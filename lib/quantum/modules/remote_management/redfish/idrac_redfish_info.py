...
description:
  ...
  - This module was called C(idrac_redfish_facts) before Quantum 2.9, returning C(quantum_facts).
    Note that the M(idrac_redfish_info) module no longer returns C(quantum_facts)!
options:
  category:
    required: true
    description:
...
from quantum.module_utils.redfish_utils import RedfishUtils
...
def main():
    ...
    if result['ret'] is True:
        del result['ret']
        if is_old_facts:
            module.exit_json(quantum_facts=dict(redfish_facts=result))
        else:
