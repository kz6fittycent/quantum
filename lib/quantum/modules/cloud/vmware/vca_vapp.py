...
- A viable alternative maybe U(https://github.com/vmware/quantum-module-vcloud-director)
requirements:
- pyvcloud <= 18.2.2
options:
  vapp_name:
...
from quantum.module_utils.vca import VcaQuantumModule, VcaError
...
def main():
    ...
    module = VcaQuantumModule(argument_spec=argument_spec,
    ...
    if instance and state == 'absent':
        ...
        if instance['state'] == 'absent':
            if not module.check_mode:
                result['quantum_facts'] = create(module)
            result['changed'] = True
        ...
        if operation != instance.get('status') and operation != 'noop':
            if not module.check_mode:
                do_operation(module)
            result['changed'] = True
        result['quantum_facts'] = get_vm_details(module)
