...
from quantum.module_utils.compat import ipaddress
    ...
    def ensure(self):
        ...
        for host in self.hosts:
            ...
            for rule_option in self.rule_options:
                ...
                allowed_hosts = rule_option.get('allowed_hosts', {})
                coupling_allows_all = allowed_hosts.get('all_ip', False)
                coupling_allowed_ips = set(allowed_hosts.get('ip_address', []))
                coupling_allowed_networks = set(allowed_hosts.get('ip_network', []))

                # compare what is configured on the firewall rule with what the coupling provides
                allowed_all_ips_different = bool(rule_allows_all != coupling_allows_all)
                ip_list_different = bool(rule_allowed_ips != coupling_allowed_ips)
                ip_network_different = bool(rule_allowed_networks != coupling_allowed_networks)
                ...
                if allowed_all_ips_different is True or ip_list_different is True or ip_network_different is True:
                    try:
                        ...
                        if not self.module.check_mode:
                            ...
                            firewall_spec.allowedHosts.allIp = coupling_allows_all
                            firewall_spec.allowedHosts.ipAddress = list(coupling_allowed_ips)
                            firewall_spec.allowedHosts.ipNetwork = []

                            for i in coupling_allowed_networks:
                ...
                results['rule_set_state'][host.name][rule_name] = {
                    ...
                    'allowed_hosts': {
                        'current_allowed_all': coupling_allows_all,
                        'previous_allowed_all': permitted_networking['allowed_hosts']['all_ip'],
                        'desired_allowed_all': coupling_allows_all,
                        'current_allowed_ip': coupling_allowed_ips,
                        'previous_allowed_ip': set(permitted_networking['allowed_hosts']['ip_address']),
                        'desired_allowed_ip': coupling_allowed_ips,
                        'current_allowed_networks': coupling_allowed_networks,
                        'previous_allowed_networks': set(permitted_networking['allowed_hosts']['ip_network']),
                        'desired_allowed_networks': coupling_allowed_networks,
                    }
                }
