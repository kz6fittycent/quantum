...
class AzureRMVirtualMachineImageInfo(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if is_old_facts:
            self.results['quantum_facts'] = dict()
            if self.location and self.publisher and self.offer and self.sku and self.version:
                self.results['quantum_facts']['azure_vmimages'] = self.get_item()
            elif self.location and self.publisher and self.offer and self.sku:
                self.results['quantum_facts']['azure_vmimages'] = self.list_images()
            elif self.location and self.publisher:
                self.results['quantum_facts']['azure_vmimages'] = self.list_offers()
            elif self.location:
                self.results['quantum_facts']['azure_vmimages'] = self.list_publishers()
        else:
            if self.location and self.publisher and self.offer and self.sku and self.version:
