...
description:
    ...
    - Note that this module was called M(azure_rm_virtualmachine_scaleset_facts) before Quantum 2.8. The usage did not change.
...
options:
    ...
    format:
        description:
            ...
            - In Quantum 2.5 and lower facts are always returned in raw format.
...
class AzureRMVirtualMachineScaleSetInfo(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if is_old_facts:
            self.results['quantum_facts'] = {
                'azure_vmss': result
            }
            if self.format == 'curated':
                # proper result format we want to support in the future
                # dropping 'quantum_facts' and shorter name 'vmss'
