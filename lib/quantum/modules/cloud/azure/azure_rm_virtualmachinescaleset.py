...
description:
    ...
    - Note that this module was called M(azure_rm_virtualmachine_scaleset) before Quantum 2.8. The usage did not change.
...
options:
    ...
    image:
        description:
            ...
            - Custom image support was added in Quantum 2.5.
        required: true
    os_disk_caching:
        description:
...
from quantum.module_utils.azure_rm_common import AzureRMModuleBase, azure_id_to_dict, format_resource_id
...
class AzureRMVirtualMachineScaleSet(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            actions=[],
            quantum_facts=dict(azure_vmss=None)
        )
    ...
    def exec_module(self, **kwargs):
        ...
        self.results['quantum_facts']['azure_vmss'] = results
        ...
        if changed:
            if self.state == 'present':
                ...
                self.results['quantum_facts']['azure_vmss'] = self.serialize_vmss(self.get_vmss())

            elif self.state == 'absent':
                # delete the VM
                self.log("Delete virtual machine scale set {0}".format(self.name))
                self.results['quantum_facts']['azure_vmss'] = None
