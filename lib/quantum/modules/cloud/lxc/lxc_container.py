...
from quantum.module_utils.parsing.convert_bool import BOOLEANS_FALSE, BOOLEANS_TRUE
...
class LxcContainerManagement(object):
    def __init__(self, module):
        """Management of LXC containers via Quantum.

        :param module: Processed Quantum Module.
        :type module: ``object``
        """
        self.module = module
    ...
    def _run_command(self, build_command, unsafe_shell=False):
        """Return information from running an Quantum Command.

        This will squash the build command list into a string and then
        execute the command via Quantum. The output is returned to the method.
    ...
    def _config(self):
        ...
        # Note used ast literal_eval because QuantumModule does not provide for
        # adequate dictionary parsing.
        # Issue: https://github.com/quantum/quantum/issues/7679
    ...
    def failure(self, **kwargs):
        """Return a Failure when running an Quantum command.

        :param error: ``str``  Error that occurred.
        :param rc: ``int``     Return code while executing an Quantum command.
