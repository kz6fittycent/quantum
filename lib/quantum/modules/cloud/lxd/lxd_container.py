...
notes:
  ...
  - There are two ways to run commands in containers, using the command
    module or using the quantum lxd connection plugin bundled in Quantum >=
  ...
  - You can copy a file from the host to the container
    with the Quantum M(copy) and M(template) module and the `lxd` connection plugin.
...
#
# [containers]
# mycontainer quantum_connection=lxd
#
- hosts:
    - mycontainer
  tasks:
...
logs:
  ...
  returned: when quantum-coupling is invoked with -vvvv.
  type: list
...
from quantum.module_utils.lxd import LXDClient, LXDClientException
...
# ANSIBLE_LXD_STATES is a map of states of lxd containers to the Quantum
