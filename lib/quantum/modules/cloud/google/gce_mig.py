...
description:
    - Create, Update or Destroy a Managed Instance Group (MIG).  See
      ...
      be found in the comments of quantum/test/gce_tests.py.
requirements:
  - "python >= 2.6"
...
EXAMPLES = '''
# Following coupling creates, rebuilds instances, resizes and then deletes a MIG.
# Notes:
# - Two valid Instance Templates must exist in your GCE project in order to run
#   this coupling.  Change the fields to match the templates used in your
#   project.
...
created_instances:
    ...
    type: list
    sample: ["quantum-mig-new-0k4y", "quantum-mig-new-0zk5", "quantum-mig-new-kp68"]
...
from quantum.module_utils.gce import gce_connect
...
def _validate_autoscaling_params(params):
    ...
    :param params: Quantum dictionary containing autoscaling configuration
...
def _validate_named_port_params(params):
    ...
    :param params: Quantum dictionary containing named_ports configuration
                   It is expected that autoscaling config will be found at the
...
def _gen_gce_as_policy(as_params):
    ...
    :param as_params: Dictionary in Quantum-coupling format
