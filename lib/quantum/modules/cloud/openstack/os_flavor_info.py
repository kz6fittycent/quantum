...
description:
    - Retrieve information about available OpenStack instance flavors. By default,
      ...
      *ALL* filters must match on a flavor before that flavor is returned as
      a fact.
    - This module was called C(os_flavor_facts) before Quantum 2.9, returning C(quantum_facts).
      Note that the M(os_flavor_info) module no longer returns C(quantum_facts)!
notes:
    ...
    try:
        ...
        if is_old_facts:
            module.exit_json(changed=False,
                             quantum_facts=dict(openstack_flavors=flavors))
        else:
            module.exit_json(changed=False,
