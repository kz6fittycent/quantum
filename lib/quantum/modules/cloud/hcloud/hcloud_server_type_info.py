...
description:
    ...
    - This module was called C(hcloud_server_type_facts) before Quantum 2.9, returning C(quantum_facts) and C(hcloud_server_type_facts).
      Note that the M(hcloud_server_type_info) module no longer returns C(quantum_facts) and the value was renamed to C(hcloud_server_type_info)!
...
from quantum.module_utils.hcloud import Hcloud
...
class QuantumHcloudServerTypeInfo(Hcloud):
...
def main():
    module = QuantumHcloudServerTypeInfo.define_module()
    ...
    hcloud = QuantumHcloudServerTypeInfo(module)
    ...
    if is_old_facts:
        ...
        module.exit_json(quantum_facts=quantum_info)
    else:
        quantum_info = {
            'hcloud_server_type_info': result['hcloud_server_type_info']
        }
        module.exit_json(**quantum_info)
