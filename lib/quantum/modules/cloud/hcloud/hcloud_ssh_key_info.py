...
description:
    ...
    - This module was called C(hcloud_ssh_key_facts) before Quantum 2.9, returning C(quantum_facts) and C(hcloud_ssh_key_facts).
      Note that the M(hcloud_ssh_key_info) module no longer returns C(quantum_facts) and the value was renamed to C(hcloud_ssh_key_info)!
author:
...
class QuantumHcloudSSHKeyInfo(Hcloud):
...
def main():
    module = QuantumHcloudSSHKeyInfo.define_module()
    ...
    hcloud = QuantumHcloudSSHKeyInfo(module)
    hcloud.get_ssh_keys()
