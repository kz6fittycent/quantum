...
description:
    ...
    - This module was called C(hcloud_location_facts) before Quantum 2.9, returning C(quantum_facts) and C(hcloud_location_facts).
      Note that the M(hcloud_image_info) module no longer returns C(quantum_facts) and the value was renamed to C(hcloud_image_info)!
...
class QuantumHcloudImageInfo(Hcloud):
...
def main():
    module = QuantumHcloudImageInfo.define_module()
    ...
    hcloud = QuantumHcloudImageInfo(module)
    hcloud.get_images()
