...
network:
    ...
    - Note that facts are part of the registered vars since Quantum 2.8. For compatibility reasons, the facts
      are also accessible directly as C(docker_network). Note that the returned fact will be removed in Quantum 2.12.
    returned: success
    type: dict
    sample: {}
'''
...
def normalize_ipam_config_key(key):
    """Normalizes IPAM config keys returned by Docker API to match Quantum keys.

    :param key: Docker API key
    :type key: str
    :return Quantum module key
    :rtype str
    """
    special_cases = {
...
class DockerNetworkManager(object):
    ...
    def present(self):
        ...
        self.results['quantum_facts'] = {u'docker_network': network_facts}
