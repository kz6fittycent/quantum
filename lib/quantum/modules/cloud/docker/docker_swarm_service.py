...
options:
  ...
  mounts:
    ...
    suboptions:
      ...
      type:
        description:
          - The mount type.
          - Note that C(npipe) is only supported by Docker for Windows. Also note that C(npipe) was added in Quantum 2.9.
        type: str
        default: bind
        choices:
          - bind
  ...
  update_delay:
    description:
      ...
      - "Accepts a duration as an integer in nanoseconds or as a string in a format that look like:
        C(5h34m56s), C(1m30s) etc. The supported units are C(us), C(ms), C(s), C(m) and C(h)."
      - Corresponds to the C(--update-delay) option of C(docker service create).
      - Before Quantum 2.8, the default value for this option was C(10).
  ...
  update_parallelism:
    description:
      - Rolling update parallelism.
      - Corresponds to the C(--update-parallelism) option of C(docker service create).
      - Before Quantum 2.8, the default value for this option was C(1).
  ...
  user:
    description:
      ...
      - Before Quantum 2.8, the default value for this option was C(root).
...
swarm_service:
  ...
  description:
    ...
    - Note that before Quantum 2.7.9, the return variable was documented as C(quantum_swarm_service),
      while the module actually returned a variable called C(quantum_docker_service). The variable
      was renamed to C(swarm_service) in both code and documentation for Quantum 2.7.9 and Quantum 2.8.0.
      In Quantum 2.7.x, the old name C(quantum_docker_service) can still be used.
  sample: '{
    "args": [
      "3600"
    ],
...
from quantum.module_utils.basic import human_to_bytes
...
try:
    ...
    # missing Docker SDK for Python handled in quantum.module_utils.docker.common
    pass
...
class DockerService(DockerBaseClass):
    ...
    @staticmethod
    def get_restart_config_from_quantum_params(params):
    ...
    @staticmethod
    def get_update_config_from_quantum_params(params):
    ...
    @staticmethod
    def get_rollback_config_from_quantum_params(params):
    ...
    @staticmethod
    def get_logging_from_quantum_params(params):
    ...
    @staticmethod
    def get_limits_from_quantum_params(params):
    ...
    @staticmethod
    def get_reservations_from_quantum_params(params):
    ...
    @staticmethod
    def get_placement_from_quantum_params(params):
    ...
    def from_quantum_params(
        ...
        s.rollback_config = cls.get_rollback_config_from_quantum_params(ap)

        update_config = cls.get_update_config_from_quantum_params(ap)
        for key, value in update_config.items():
            setattr(s, key, value)

        restart_config = cls.get_restart_config_from_quantum_params(ap)
        for key, value in restart_config.items():
            setattr(s, key, value)

        logging_config = cls.get_logging_from_quantum_params(ap)
        for key, value in logging_config.items():
            setattr(s, key, value)

        limits = cls.get_limits_from_quantum_params(ap)
        for key, value in limits.items():
            setattr(s, key, value)

        reservations = cls.get_reservations_from_quantum_params(ap)
        for key, value in reservations.items():
            setattr(s, key, value)

        placement = cls.get_placement_from_quantum_params(ap)
...
class DockerServiceManager(object):
    ...
    def run(self):
        ...
        try:
            ...
            new_service = DockerService.from_quantum_params(
                module.params,
                current_service,
                image_digest,
                secret_ids,
