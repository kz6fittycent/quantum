...
description:
  ...
  - Supports check mode.
  - This module was called C(docker_service) before Quantum 2.8. The usage did not change.
        ...
        definition:
          ...
          services:
            db:
              image: postgres
            web:
              build: "{{ coupling_dir }}/flask"
              command: "python manage.py runserver 0.0.0.0:8000"
              volumes:
                - "{{ coupling_dir }}/flask:/code"
              ports:
                - "8000:8000"
              depends_on:
                - db
...
services:
  ...
  - Note that facts are part of the registered vars since Quantum 2.8. For compatibility reasons, the facts
    ...
    can be accessed. Note that the returned facts will be removed in Quantum 2.12.
  returned: success
  type: complex
  contains:
      container_name:
...
def make_redirection_tempfiles():
    dummy, out_redir_name = tempfile.mkstemp(prefix="quantum")
    dummy, err_redir_name = tempfile.mkstemp(prefix="quantum")
...
class ContainerManager(DockerBaseClass):
    ...
    def __init__(self, client):
        ...
        if self.definition:
            ...
            self.project_src = tempfile.mkdtemp(prefix="quantum")
    ...
    def cmd_up(self):
        ...
        detached = True
        result = dict(changed=False, actions=[], quantum_facts=dict(), services=dict())
        ...
        for service in self.project.services:
            service_facts = dict()
            result['quantum_facts'][service.name] = service_facts
