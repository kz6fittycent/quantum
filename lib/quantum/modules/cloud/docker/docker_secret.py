...
description:
     ...
     - Adds to the metadata of new secrets 'quantum_key', an encrypted hash representation of the data, which is then used
       in future runs to test if a secret has changed. If 'quantum_key is not present, then a secret will not be updated
...
- name: Create secret foo (from a file on the control machine)
  docker_secret:
    name: foo
    # If the file is JSON or binary, Quantum might modify it (because
...
class SecretManager(DockerBaseClass):
    ...
    def present(self):
        ...
        if secret:
            ...
            if attrs.get('Labels', {}).get('quantum_key'):
                if attrs['Labels']['quantum_key'] != self.data_key:
                    data_changed = True
