...
options:
  ...
  networks:
    description:
      ...
      - Note that as opposed to C(docker run ...), M(docker_container) does not remove the default
        ...
        Alternatively, use the I(networks_cli_compatible) option, which will be enabled by default from Quantum 2.12 on.
    type: list
    elements: dict
    suboptions:
      name:
  ...
  networks_cli_compatible:
    description:
      ...
      - "Note that docker CLI also sets I(network_mode) to the name of the first network
         ...
         adding."
      - Current value is C(no). A new default of C(yes) will be set in Quantum 2.12.
    type: bool
    version_added: "2.8"
  oom_killer:
    description:
  ...
  volumes:
    description:
      ...
      - "Note that Quantum 2.7 and earlier only supported one mode, which had to be one of C(ro), C(rw),
        C(z), and C(Z)."
    type: list
    elements: str
  volume_driver:
...
container:
    description:
      ...
      - Note that facts are part of the registered vars since Quantum 2.8. For compatibility reasons, the facts
        are also accessible directly as C(docker_container). Note that the returned fact will be removed in Quantum 2.12.
      - Before 2.3 this was C(quantum_docker_container) but was renamed in 2.3 to C(docker_container) due to
...
from quantum.module_utils.common.text.formatters import human_to_bytes
...
try:
    ...
    from quantum.module_utils.docker.common import docker_version
...
class ContainerManager(DockerBaseClass):
    ...
    def __init__(self, client):
        ...
        if self.facts:
            self.results['quantum_facts'] = {'docker_container': self.facts}
...
class QuantumDockerClientContainer(QuantumDockerClient):
    ...
    def __init__(self, **kwargs):
        ...
        super(QuantumDockerClientContainer, self).__init__(
...
def main():
    ...
    client = QuantumDockerClientContainer(
    ...
    if client.module.params['networks_cli_compatible'] is None and client.module.params['networks']:
        client.module.deprecate(
            ...
            'This behavior will change in Quantum 2.12. You can change the behavior now by setting '
