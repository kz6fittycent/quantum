    ...
    objects:
        description:
            ...
            - As part of the the list user can pass also I(VirtualMachine) kind. When passing I(VirtualMachine)
              user must use Quantum structure of the parameters not the Kubernetes API structure. For more information
...
from quantum.module_utils.k8s.common import AUTH_ARG_SPEC

from quantum.module_utils.kubevirt import (
    virtdict,
    KubeVirtRawModule,
    API_GROUP,
...
class KubeVirtVMTemplate(KubeVirtRawModule):
    ...
    def execute_module(self):
        ...
        # TODO: Make it more Ansiblish, so user don't have to specify API JSON path, but rather Quantum params:
