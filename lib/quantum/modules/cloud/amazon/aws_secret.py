...
from quantum.module_utils.ec2 import snake_dict_to_camel_dict, camel_dict_to_snake_dict
from quantum.module_utils.ec2 import boto3_tag_list_to_quantum_dict, compare_aws_tags, quantum_dict_to_boto3_tag_list
...
class Secret(object):
    ...
    def create_args(self):
        ...
        if self.tags:
            args["Tags"] = quantum_dict_to_boto3_tag_list(self.tags)
    ...
    @property
    def boto3_tags(self):
        return quantum_dict_to_boto3_tag_list(self.Tags)
    ...
    if state == 'present':
        if current_secret is None:
            ...
            if not rotation_match(secret, current_secret):
                ...
                changed = True
            current_tags = boto3_tag_list_to_quantum_dict(current_secret.get('Tags', []))
            tags_to_add, tags_to_remove = compare_aws_tags(current_tags, secret.tags)
            if tags_to_add:
                secrets_mgr.tag_secret(secret.name, quantum_dict_to_boto3_tag_list(tags_to_add))
                changed = True
            if tags_to_remove:
