...
class QuantumEc2Igw(object):
    ...
    def ensure_tags(self, igw_id, tags, add_only):
        ...
        filters = quantum_dict_to_boto3_filter_list({'resource-id': igw_id, 'resource-type': 'internet-gateway'})
        cur_tags = None
        try:
        ...
        final_tags = boto3_tag_list_to_quantum_dict(cur_tags.get('Tags'))
        ...
        if not self._check_mode and (to_update or to_delete):
            try:
                ...
                final_tags = boto3_tag_list_to_quantum_dict(response.get('Tags'))
            except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
                self._module.fail_json_aws(e, msg="Couldn't describe tags")
...
def main():
    ...
    igw_manager = QuantumEc2Igw(module=module, results=results)
    igw_manager.process()
