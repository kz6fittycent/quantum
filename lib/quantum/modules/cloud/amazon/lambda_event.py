...
description:
    - This module allows the management of AWS Lambda function event source mappings such as DynamoDB and Kinesis stream
      events via the Quantum framework. These event source mappings are relevant only in the AWS Lambda pull model, where
...
class AWSConnection:
    ...
    def __init__(self, quantum_obj, resources, use_boto3=True):

        try:
            self.region, self.endpoint, aws_connect_kwargs = get_aws_connection_info(quantum_obj, boto3=use_boto3)
