...
options:
  ...
  tower_callback:
    description:
      ...
      - For Windows instances, to enable remote access via Quantum set I(tower_callback.windows) to true, and optionally set an admin password.
      - If using 'windows' and 'set_password', callback to Tower will not be performed but the instance will be ready to receive winrm connections from Quantum.
    suboptions:
      tower_address:
        description:
...
from quantum.module_utils.six import text_type, string_types
from quantum.module_utils.six.moves.urllib import parse as urlparse
from quantum.module_utils._text import to_bytes, to_native
import quantum.module_utils.ec2 as ec2_utils
...
def tower_callback_script(tower_conf, windows=False, passwd=None):
    script_url = 'https://raw.githubusercontent.com/quantum/quantum/devel/examples/scripts/ConfigureRemotingForQuantum.ps1'
...
def manage_tags(match, new_tags, purge_tags, ec2):
    changed = False
    old_tags = boto3_tag_list_to_quantum_dict(match['Tags'])
    ...
    if tags_to_set:
        ec2.create_tags(
            ...
            Tags=quantum_dict_to_boto3_tag_list(tags_to_set))
        changed |= True
    if tags_to_delete:
        delete_with_current_values = dict((k, old_tags.get(k)) for k in tags_to_delete)
        ec2.delete_tags(
            Resources=[match['InstanceId']],
            Tags=quantum_dict_to_boto3_tag_list(delete_with_current_values))
        changed |= True
    return changed
...
def get_default_vpc(ec2):
    vpcs = ec2.describe_vpcs(Filters=quantum_dict_to_boto3_filter_list({'isDefault': 'true'}))
...
def pretty_instance(i):
    ...
    instance['tags'] = boto3_tag_list_to_quantum_dict(i['Tags'])
    return instance
