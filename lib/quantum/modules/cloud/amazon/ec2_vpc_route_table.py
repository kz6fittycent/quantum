...
#
# This Quantum library is distributed in the hope that it will be useful,
...
from quantum.module_utils.aws.waiters import get_waiter
from quantum.module_utils.ec2 import ec2_argument_spec, boto3_conn, get_aws_connection_info
from quantum.module_utils.ec2 import quantum_dict_to_boto3_filter_list
from quantum.module_utils.ec2 import camel_dict_to_snake_dict, snake_dict_to_camel_dict
from quantum.module_utils.ec2 import quantum_dict_to_boto3_tag_list, boto3_tag_list_to_quantum_dict
from quantum.module_utils.ec2 import compare_aws_tags, AWSRetry
...
def find_subnets(connection, module, vpc_id, identified_subnets):
    ...
    subnets_by_id = []
    if subnet_ids:
        filters = quantum_dict_to_boto3_filter_list({'vpc-id': vpc_id})
        try:
    ...
    subnets_by_cidr = []
    if subnet_cidrs:
        filters = quantum_dict_to_boto3_filter_list({'vpc-id': vpc_id, 'cidr': subnet_cidrs})
        try:
    ...
    subnets_by_name = []
    if subnet_names:
        filters = quantum_dict_to_boto3_filter_list({'vpc-id': vpc_id, 'tag:Name': subnet_names})
        try:
...
def find_igw(connection, module, vpc_id):
    ...
    """
    filters = quantum_dict_to_boto3_filter_list({'attachment.vpc-id': vpc_id})
    try:
...
def ensure_tags(connection=None, module=None, resource_id=None, tags=None, purge_tags=None, check_mode=None):
    ...
    if to_add:
        try:
            connection.create_tags(Resources=[resource_id], Tags=quantum_dict_to_boto3_tag_list(to_add))
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
...
def ensure_subnet_association(connection=None, module=None, vpc_id=None, route_table_id=None, subnet_id=None,
                              check_mode=None):
    filters = quantum_dict_to_boto3_filter_list({'association.subnet-id': subnet_id, 'vpc-id': vpc_id})
    try:
        route_tables = describe_route_tables_with_backoff(connection, Filters=filters)
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
...
def main():
    ...
    module = QuantumAWSModule(argument_spec=argument_spec,
