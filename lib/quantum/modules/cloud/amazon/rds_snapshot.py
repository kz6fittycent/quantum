#!/usr/bin/python
# Copyright (c) 2014 Quantum Project
...
db_instance_identifier:
  ...
  returned: always
  type: str
  sample: quantum-test-16638696
db_snapshot_arn:
  description: Amazon Resource Name for the snapshot.
  returned: always
  type: str
  sample: arn:aws:rds:us-west-2:123456789012:snapshot:quantum-test-16638696-test-snapshot
db_snapshot_identifier:
  description: Name of the snapshot.
  returned: always
  type: str
  sample: quantum-test-16638696-test-snapshot
dbi_resource_id:
...
try:
    ...
    pass  # protected by QuantumAWSModule

# import module snippets
from quantum.module_utils.aws.core import QuantumAWSModule
from quantum.module_utils.ec2 import camel_dict_to_snake_dict, AWSRetry, compare_aws_tags
from quantum.module_utils.ec2 import boto3_tag_list_to_quantum_dict, quantum_dict_to_boto3_tag_list
...
def snapshot_to_facts(client, module, snapshot):
    try:
        snapshot['Tags'] = boto3_tag_list_to_quantum_dict(client.list_tags_for_resource(ResourceName=snapshot['DBSnapshotArn'],
...
def ensure_tags(client, module, resource_arn, existing_tags, tags, purge_tags):
    ...
    if tags_to_add:
        try:
            client.add_tags_to_resource(ResourceName=resource_arn, Tags=quantum_dict_to_boto3_tag_list(tags_to_add))
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
...
def ensure_snapshot_present(client, module):
    ...
    existing_tags = boto3_tag_list_to_quantum_dict(client.list_tags_for_resource(ResourceName=snapshot['DBSnapshotArn'],
                                                                                 aws_retry=True)['TagList'])
