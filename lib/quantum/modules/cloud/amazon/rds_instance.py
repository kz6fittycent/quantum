    ...
    wait:
        description:
          - Whether to wait for the cluster to be available, stopped, or deleted. At a later time a wait_timeout option may be added.
            ...
            If setting this to False task retries and delays may make your coupling execution better handle timeouts for major modifications.
        type: bool
        default: True
...
- name: create minimal aurora instance in default VPC and default subnet group
  rds_instance:
    engine: aurora
    db_instance_identifier: quantum-test-aurora-db-instance
    instance_type: db.t2.small
    password: "{{ password }}"
    username: "{{ username }}"
    cluster_id: quantum-test-cluster  # This cluster must exist - see rds_cluster to manage it
...
db_instance_arn:
  ...
  returned: always
  type: str
  sample: arn:aws:rds:us-east-1:123456789012:db:quantum-test
db_instance_class:
...
endpoint:
  ...
  contains:
    address:
      ...
      returned: always
      type: str
      sample: quantum-test.cvlrtwiennww.us-east-1.rds.amazonaws.com
    hosted_zone_id:
...
from quantum.module_utils.aws.core import QuantumAWSModule, is_boto3_error_code, get_boto3_client_method_parameters
from quantum.module_utils.aws.rds import ensure_tags, arg_spec_to_rds_params, call_method, get_rds_method_attribute, get_tags, get_final_identifier
...
from quantum.module_utils.ec2 import quantum_dict_to_boto3_tag_list, AWSRetry
...
def get_parameters(client, module, parameters, method_name):
    ...
    if method_name == 'create_db_instance' and parameters.get('Tags'):
        parameters['Tags'] = quantum_dict_to_boto3_tag_list(parameters['Tags'])
