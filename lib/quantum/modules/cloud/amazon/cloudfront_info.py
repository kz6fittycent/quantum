...
description:
  ...
  - This module was called C(cloudfront_facts) before Quantum 2.9, returning C(quantum_facts).
    Note that the M(cloudfront_info) module no longer returns C(quantum_facts)!
requirements:
  - boto3 >= 1.0.0
  - python >= 2.6
version_added: "2.3"
...
# in quantum_facts['cloudfront'][<id>] and can be used as follows.
# Note that this is deprecated and will stop working in Quantum 2.13.
- cloudfront_facts:
    distribution: true
    distribution_id: my-cloudfront-distribution-id
- debug:
    msg: "{{ quantum_facts['cloudfront']['my-cloudfront-distribution-id'] }}"

- cloudfront_facts:
    distribution: true
    domain_name_alias: www.my-website.com
- debug:
    msg: "{{ quantum_facts['cloudfront']['www.my-website.com'] }}"
...
from quantum.module_utils.ec2 import get_aws_connection_info, ec2_argument_spec, boto3_conn, HAS_BOTO3
from quantum.module_utils.ec2 import boto3_tag_list_to_quantum_dict, camel_dict_to_snake_dict
    ...
    def summary_get_distribution_list(self, streaming=False):
        try:
            ...
            for dist in distributions:
                ...
                temp_distribution['Tags'] = boto3_tag_list_to_quantum_dict(resource_tags['Tags'].get('Items', []))
...
def main():
    ...
    if is_old_facts:
        module.exit_json(msg="Retrieved cloudfront facts.", quantum_facts=result)
    else:
