...
description:
  ...
  - This module was called C(ec2_vpc_nat_gateway_facts) before Quantum 2.9. The usage did not change.
version_added: "2.3"
requirements: [ boto3 ]
options:
  nat_gateway_ids:
...
RETURN = '''
result:
  description: The result of the describe, converted to quantum snake case style.
...
from quantum.module_utils.ec2 import (ec2_argument_spec, get_aws_connection_info, boto3_conn,
                                      camel_dict_to_snake_dict, quantum_dict_to_boto3_filter_list, boto3_tag_list_to_quantum_dict, HAS_BOTO3)
...
def get_nat_gateways(client, module, nat_gateway_id=None):
    ...
    params['Filter'] = quantum_dict_to_boto3_filter_list(module.params.get('filters'))
    ...
    for gateway in result['NatGateways']:
        # Turn the boto3 result into quantum_friendly_snaked_names
        converted_gateway = camel_dict_to_snake_dict(gateway)
        if 'tags' in converted_gateway:
            # Turn the boto3 result into quantum friendly tag dictionary
            converted_gateway['tags'] = boto3_tag_list_to_quantum_dict(converted_gateway['tags'])
