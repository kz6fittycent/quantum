...
import time
from quantum.module_utils.ec2 import get_aws_connection_info, ec2_argument_spec, boto3_conn, camel_dict_to_snake_dict
from quantum.module_utils.ec2 import quantum_dict_to_boto3_tag_list, boto3_tag_list_to_quantum_dict, compare_aws_tags
...
def get_ami_info(camel_image):
    ...
    return dict(
        ...
        name=image.get("name"),
        tags=boto3_tag_list_to_quantum_dict(image.get('tags')),
...
def create_image(module, connection):
    ...
    if tags:
        try:
            connection.create_tags(Resources=[image_id], Tags=quantum_dict_to_boto3_tag_list(tags))
        except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:
...
def update_image(module, connection, image_id):
    ...
    if desired_tags is not None:
        current_tags = boto3_tag_list_to_quantum_dict(image.get('Tags'))
        ...
        if tags_to_add:
            try:
                connection.create_tags(Resources=[image_id], Tags=quantum_dict_to_boto3_tag_list(tags_to_add))
                changed = True
            except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:
                module.fail_json_aws(e, msg="Error updating tags")
