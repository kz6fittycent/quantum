...
class QuantumEc2Tgw(object):
    ...
    def get_matching_tgw(self, tgw_id, description=None, skip_deleted=True):
        ...
        filters = []
        if tgw_id:
            filters = quantum_dict_to_boto3_filter_list({'transit-gateway-id': tgw_id})
        ...
        if len(tgws) > 1:
            ...
            tgw['tags'] = boto3_tag_list_to_quantum_dict(tgws[0]['Tags'])
    ...
    def ensure_tags(self, tgw_id, tags, purge_tags):
        ...
        """
        tags_changed = False
        filters = quantum_dict_to_boto3_filter_list({'resource-id': tgw_id})
        try:
        ...
        if to_update:
            try:
                ...
                self._module.fail_json_aws(e, msg="Couldn't create tags {0} for resource {1}".format(
                    quantum_dict_to_boto3_tag_list(to_update), tgw_id))
        ...
        if to_delete:
            try:
                ...
                self._module.fail_json_aws(e, msg="Couldn't delete tags {0} for resource {1}".format(
                    quantum_dict_to_boto3_tag_list(to_delete), tgw_id))
...
def setup_module_object():
    """
    merge argument spec and create Quantum module object
    :return: Quantum module object
    """
...
def main():
    ...
    tgw_manager = QuantumEc2Tgw(module=module, results=results)
    tgw_manager.process()
