...
operations_log:
  ...
  - action: CREATE
    ...
    operation_id: Quantum-StackInstance-Create-0ff2af5b-251d-4fdb-8b89-1ee444eba8b8
    status: FAILED
    stack_instances:
    - account: '1234567890'
      region: us-east-1
...
try:
    ...
    # handled by QuantumAWSModule
    pass

from quantum.module_utils.ec2 import AWSRetry, boto3_tag_list_to_quantum_dict, quantum_dict_to_boto3_tag_list, camel_dict_to_snake_dict
...
def stack_set_facts(cfn, stack_set_name):
    try:
        ...
        ss['Tags'] = boto3_tag_list_to_quantum_dict(ss['Tags'])
        return ss
...
def describe_stack_tree(module, stack_set_name, operation_ids=None):
    ...
    result['stack_set']['tags'] = boto3_tag_list_to_quantum_dict(result['stack_set']['tags'])
...
def main():
    ...
    if module.params.get('tags') and isinstance(module.params.get('tags'), dict):
        stack_params['Tags'] = quantum_dict_to_boto3_tag_list(module.params['tags'])
    ...
    if state == 'present':
        if not existing_stack_set:
            ...
            stack_params['ClientRequestToken'] = 'Quantum-StackSet-Create-{0}'.format(operation_uuid)
            changed = True
            create_stack_set(module, stack_params, cfn)
        else:
            stack_params['OperationId'] = 'Quantum-StackSet-Update-{0}'.format(operation_uuid)
        ...
        if new_stack_instances:
            operation_ids.append('Quantum-StackInstance-Create-{0}'.format(operation_uuid))
            changed = True
            ...
            operation_ids.append('Quantum-StackInstance-Update-{0}'.format(operation_uuid))
        ...
        try:
            ...
            delete_instances_op = 'Quantum-StackInstance-Delete-{0}'.format(operation_uuid)
