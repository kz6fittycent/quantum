...
description:
  ...
  - This module was called C(cloudformation_facts) before Quantum 2.9, returning C(quantum_facts).
    Note that the M(cloudformation_info) module no longer returns C(quantum_facts)!
requirements:
  - boto3 >= 1.0.0
  - python >= 2.6
version_added: "2.2"
...
# in quantum_facts['cloudformation'][<stack_name>] and can be used as follows.
...
- debug:
    msg: "{{ quantum_facts['cloudformation']['my-cloudformation-stack'] }}"
...
from quantum.module_utils.ec2 import (get_aws_connection_info, ec2_argument_spec, boto3_conn,
                                      camel_dict_to_snake_dict, AWSRetry, boto3_tag_list_to_quantum_dict)
...
def main():
    ...
    if is_old_facts:
        result = {'quantum_facts': {'cloudformation': {}}}
    else:
    ...
    for stack_description in service_mgr.describe_stacks(module.params.get('stack_name')):
        ...
        if facts['stack_description']:
            ...
            facts['stack_tags'] = boto3_tag_list_to_quantum_dict(facts['stack_description'].get('Tags'))
        ...
        if is_old_facts:
            result['quantum_facts']['cloudformation'][stack_name] = facts
        else:
