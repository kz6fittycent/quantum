...
options:
  ...
  wait:
    description:
      - When specified, Quantum will check the status of the load balancer to ensure it has been successfully
        removed from AWS.
    type: bool
    default: 'no'
    version_added: "2.1"
...
from quantum.module_utils.ec2 import ec2_argument_spec, connect_to_aws, QuantumAWSError, get_aws_connection_info
...
def main():
    ...
    quantum_facts = {'ec2_elb': 'info'}
    ec2_facts_result = dict(changed=elb_man.changed,
                            elb=elb_man.get_info(),
                            quantum_facts=quantum_facts)
