...
from quantum.module_utils.ec2 import (camel_dict_to_snake_dict, boto3_tag_list_to_quantum_dict, ec2_argument_spec,
                                      compare_aws_tags, quantum_dict_to_boto3_tag_list)
from distutils.version import LooseVersion


def get_tg_attributes(connection, module, tg_arn):
    try:
        tg_attributes = boto3_tag_list_to_quantum_dict(connection.describe_target_group_attributes(TargetGroupArn=tg_arn)['Attributes'])
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_aws(e, msg="Couldn't get target group attributes")

    # Replace '.' with '_' in attribute key names to make it more Quantumy
...
def create_or_update_target_group(connection, module):
    ...
    if tags:
        ...
        tags_need_modify, tags_to_delete = compare_aws_tags(boto3_tag_list_to_quantum_dict(current_tags), tags, purge_tags)
        if tags_to_delete:
            try:
        ...
        if tags_need_modify:
            try:
                connection.add_tags(ResourceArns=[tg['TargetGroupArn']], Tags=quantum_dict_to_boto3_tag_list(tags_need_modify))
            except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
    ...
    snaked_tg['tags'] = boto3_tag_list_to_quantum_dict(get_target_group_tags(connection, module, tg['TargetGroupArn']))
