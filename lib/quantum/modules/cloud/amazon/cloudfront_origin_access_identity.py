...
from quantum.module_utils.ec2 import get_aws_connection_info, ec2_argument_spec
from quantum.module_utils.ec2 import boto3_conn
...
try:
    ...
    pass  # caught by imported QuantumAWSModule
...
class CloudFrontOriginAccessIdentityValidationManager(object):
    ...
    def validate_comment(self, comment):
        if comment is None:
            return "origin access identity created by Quantum with datetime " + datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')
        return comment
...
def main():
    ...
    module = QuantumAWSModule(argument_spec=argument_spec, supports_check_mode=False)
