...
description:
    ...
    - This module was called C(elb_target_group_facts) before Quantum 2.9. The usage did not change.
version_added: "2.4"
requirements: [ boto3 ]
...
def get_target_group_attributes(connection, module, target_group_arn):
    ...
    try:
        target_group_attributes = boto3_tag_list_to_quantum_dict(connection.describe_target_group_attributes(TargetGroupArn=target_group_arn)['Attributes'])
...
def get_target_group_tags(connection, module, target_group_arn):
    ...
    try:
        return boto3_tag_list_to_quantum_dict(connection.describe_tags(ResourceArns=[target_group_arn])['TagDescriptions'][0]['Tags'])
    except ClientError as e:
        module.fail_json(msg=e.message, exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.response))
