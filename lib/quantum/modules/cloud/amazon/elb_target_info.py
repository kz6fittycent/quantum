...
description:
  ...
  - This module was called C(elb_target_facts) before Quantum 2.9. The usage did not change.
...
# practical use case - dynamically deregistering and reregistering nodes
  ...
  - name: Get initial list of target groups
    ...
    elb_target_info:
      instance_id: "{{ quantum_ec2_instance_id }}"
      region: "{{ quantum_ec2_placement_region }}"
...
# instance - useful in case the coupling fails mid-run and manual
#            rollback is required
  - name: "reregistration commands: ELBv2s"
    debug:
      msg: >
             aws --region {{quantum_ec2_placement_region}} elbv2
...
from quantum.module_utils.ec2 import (HAS_BOTO3, camel_dict_to_snake_dict,
                                      AWSRetry)
