...
description:
     ...
     - swagger_file and swagger_text are passed directly on to AWS
       transparently whilst swagger_dict is an quantum dict which is
...
options:
  ...
  swagger_dict:
    description:
      - Swagger definitions API quantum dictionary which will be
  ...
  deploy_desc:
    description:
      - Description of the deployment - recorded and visible in the
        AWS console.
    default: Automatic deployment by Quantum.
author:
...
try:
    ...
    # HAS_BOTOCORE taken care of in QuantumAWSModule
    pass
...
def main():
    argument_spec = dict(
        ...
        deploy_desc=dict(type='str', default="Automatic deployment by Quantum."),
    )
...
def create_empty_api(module, client):
    ...
    """
    desc = "Incomplete API creation by quantum aws_api_gateway module"
    try:
        awsret = create_api(client, name="quantum-temp-api", description=desc)
...
def create_api(client, name=None, description=None):
    return client.create_rest_api(name="quantum-temp-api", description=description)
