...
description:
    ...
    - This module was called C(efs_facts) before Quantum 2.9, returning C(quantum_facts).
      Note that the M(efs_info) module no longer returns C(quantum_facts)!
version_added: "2.2"
requirements: [ boto3 ]
author:
...
from quantum.module_utils.ec2 import boto3_conn, get_aws_connection_info, ec2_argument_spec, AWSRetry
from quantum.module_utils.ec2 import camel_dict_to_snake_dict, boto3_tag_list_to_quantum_dict
...
class EFSConnection(object):
    ...
    def get_tags(self, file_system_id):
        ...
        return boto3_tag_list_to_quantum_dict(paginator.paginate(FileSystemId=file_system_id).build_full_result()['Tags'])
...
def main():
    ...
    if is_old_facts:
        module.exit_json(changed=False, quantum_facts={'efs': file_systems_info})
    else:
