...
from quantum.module_utils.ec2 import (HAS_BOTO3, boto3_conn, camel_dict_to_snake_dict,
                                      ec2_argument_spec, get_aws_connection_info, quantum_dict_to_boto3_tag_list,
                                      compare_aws_tags, boto3_tag_list_to_quantum_dict)
...
class EFSConnection(object):
    ...
    def converge_file_system(self, name, tags, purge_tags, targets, throughput_mode, provisioned_throughput_in_mibps):
        ...
        if tags is not None:
            tags_need_modify, tags_to_delete = compare_aws_tags(boto3_tag_list_to_quantum_dict(self.get_tags(FileSystemId=fs_id)), tags, purge_tags)
            ...
            if tags_need_modify:
                try:
                    self.connection.create_tags(
                        FileSystemId=fs_id,
                        Tags=quantum_dict_to_boto3_tag_list(tags_need_modify)
                    )
                except ClientError as e:
