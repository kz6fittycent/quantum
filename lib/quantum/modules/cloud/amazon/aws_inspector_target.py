...
from quantum.module_utils.ec2 import AWSRetry
...
def main():
    ...
    if tags:
        tags = quantum_dict_to_boto3_tag_list(tags, 'key', 'value')
    ...
    if state == 'present' and target_exists:
        quantum_dict_tags = boto3_tag_list_to_quantum_dict(tags)
        quantum_dict_existing_tags = boto3_tag_list_to_quantum_dict(
            existing_resource_group_tags
        )
        tags_to_add, tags_to_remove = compare_aws_tags(
            quantum_dict_tags,
            quantum_dict_existing_tags
        )
        if not (tags_to_add or tags_to_remove):
            existing_target.update({'tags': quantum_dict_existing_tags})
            ...
            try:
                ...
                updated_target.update({'tags': quantum_dict_tags})
        ...
        try:
            ...
            new_target.update({'tags': boto3_tag_list_to_quantum_dict(tags)})
