...
description:
    ...
    - This module was called C(elb_application_lb_facts) before Quantum 2.9. The usage did not change.
version_added: "2.4"
requirements: [ boto3 ]
...
from quantum.module_utils.ec2 import (boto3_conn, boto3_tag_list_to_quantum_dict, camel_dict_to_snake_dict,
...
def get_load_balancer_attributes(connection, module, load_balancer_arn):
    ...
    try:
        load_balancer_attributes = boto3_tag_list_to_quantum_dict(connection.describe_load_balancer_attributes(LoadBalancerArn=load_balancer_arn)['Attributes'])
    except ClientError as e:
        module.fail_json(msg=e.message, exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.response))
...
def get_load_balancer_tags(connection, module, load_balancer_arn):
    ...
    try:
        return boto3_tag_list_to_quantum_dict(connection.describe_tags(ResourceArns=[load_balancer_arn])['TagDescriptions'][0]['Tags'])
    except ClientError as e:
        module.fail_json(msg=e.message, exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.response))
...
def list_load_balancers(connection, module):
    ...
    # Turn the boto3 result in to quantum_friendly_snaked_names
