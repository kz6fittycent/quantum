...
options:
  ...
  mode:
    description:
      - Switches the module behaviour between put (upload), get (download), geturl (return download url, Quantum 1.3+),
        getstr (download object as string (1.3+)), list (list keys, Quantum 2.0+), create (bucket), delete (bucket),
        and delobj (delete object, Quantum 2.0+).
    required: true
  ...
  region:
    description:
     - "AWS region to create the bucket in. If not set then the value of the AWS_REGION and EC2_REGION environment variables
       ...
       region defaults to the S3 Location: US Standard. Prior to quantum 1.8 this parameter could be specified but had no effect."
    version_added: "1.8"
  retries:
    description:
...
from quantum.module_utils.basic import to_text, to_native
from quantum.module_utils.aws.core import QuantumAWSModule
from quantum.module_utils.aws.s3 import calculate_etag, HAS_MD5
from quantum.module_utils.ec2 import get_aws_connection_info, boto3_conn

try:
    import botocore
except ImportError:
    pass  # will be detected by imported QuantumAWSModule
...
def main():
    ...
    if module.params.get('object'):
        ...
        # remove the leading character to maintain compatibility with Quantum versions < 2.4
        if obj.startswith('/'):
            obj = obj[1:]
    ...
    # allow eucarc environment variables to be used if quantum vars aren't set
