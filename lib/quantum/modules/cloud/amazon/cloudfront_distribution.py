    ...
    comment:
      description:
        - A comment that describes the cloudfront distribution. If not specified, it defaults to a
          generic message that it has been created with Quantum, and a datetime stamp.
...
- cloudfront_distribution:
    ...
    comment: modified by quantum cloudfront.py
...
from quantum.module_utils.aws.cloudfront_facts import CloudFrontFactsServiceManager
from quantum.module_utils.common.dict_transformations import recursive_diff
from quantum.module_utils.ec2 import get_aws_connection_info
from quantum.module_utils.ec2 import ec2_argument_spec, boto3_conn, compare_aws_tags
from quantum.module_utils.ec2 import camel_dict_to_snake_dict, quantum_dict_to_boto3_tag_list
from quantum.module_utils.ec2 import snake_dict_to_camel_dict, boto3_tag_list_to_quantum_dict
import datetime
...
try:
    ...
    try:
        ...
        pass  # caught by QuantumAWSModule (as python 2.6 + boto3 => ordereddict is installed)
...
def quantum_list_to_cloudfront_list(list_items=None, include_quantity=True):
...
def list_tags_for_resource(client, module, arn):
    try:
        ...
        return boto3_tag_list_to_quantum_dict(response.get('Tags').get('Items'))
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
...
def update_tags(client, module, existing_tags, valid_tags, purge_tags, arn):
    ...
    if to_remove:
        ...
        changed = True
    if to_add:
        tag_resource(client, module, arn, quantum_dict_to_boto3_tag_list(to_add))
        changed = True
    return changed
...
class CloudFrontValidationManager(object):
    ...
    def add_key_else_validate(self, dict_object, key_name, attribute_name, value_to_set, valid_values, to_aws_list=False):
        if key_name in dict_object:
            ...
            if to_aws_list:
                dict_object[key_name] = quantum_list_to_cloudfront_list(value_to_set)
    ...
    def validate_origins(self, client, config, origins, default_origin_domain_name,
        ...
        try:
            if origins is None:
                if default_origin_domain_name is None and not create_distribution:
                    if purge_origins:
                        ...
                        return quantum_list_to_cloudfront_list(config)
            ...
            return quantum_list_to_cloudfront_list(list(all_origins.values()))
        except Exception as e:
    ...
    def validate_s3_origin_configuration(self, client, existing_config, origin):
        ...
        if not origin['s3_origin_access_identity_enabled']:
            return None
        try:
            comment = "access-identity-by-quantum-%s-%s" % (origin.get('domain_name'), self.__default_datetime_string)
    ...
    def validate_origin(self, client, existing_config, origin, default_origin_path):
        try:
            ...
            if 'custom_headers' in origin and len(origin.get('custom_headers')) > 0:
                ...
                origin['custom_headers'] = quantum_list_to_cloudfront_list(origin.get('custom_headers'))
            else:
                origin['custom_headers'] = quantum_list_to_cloudfront_list()
            if self.__s3_bucket_domain_identifier in origin.get('domain_name').lower():
                ...
                custom_origin_config['origin_ssl_protocols'] = quantum_list_to_cloudfront_list(custom_origin_config['origin_ssl_protocols'])
            return origin
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
    ...
    def validate_cache_behaviors(self, config, cache_behaviors, valid_origins, purge_cache_behaviors=False):
        try:
            ...
            return quantum_list_to_cloudfront_list(list(all_cache_behaviors.values()))
        except Exception as e:
    ...
    def validate_forwarded_values(self, config, forwarded_values, cache_behavior):
        try:
            ...
            if headers:
                headers.sort()
            forwarded_values['headers'] = quantum_list_to_cloudfront_list(headers)
            if 'cookies' not in forwarded_values:
                ...
                if whitelist:
                    ...
                    forwarded_values['cookies']['whitelisted_names'] = quantum_list_to_cloudfront_list(whitelist)
            ...
            forwarded_values['query_string_cache_keys'] = quantum_list_to_cloudfront_list(query_string_cache_keys)
    ...
    def validate_lambda_function_associations(self, config, lambda_function_associations, cache_behavior):
        try:
            if lambda_function_associations is not None:
                ...
                cache_behavior['lambda_function_associations'] = quantum_list_to_cloudfront_list(lambda_function_associations)
            else:
                if 'lambda_function_associations' in config:
                    ...
                    cache_behavior['lambda_function_associations'] = quantum_list_to_cloudfront_list([])
            return cache_behavior
        except Exception as e:
        ...
        try:
            if allowed_methods is not None:
                ...
                if 'allowed_methods' in config and set(config['allowed_methods']['items']) == set(temp_allowed_items):
                    ...
                    cache_behavior['allowed_methods'] = quantum_list_to_cloudfront_list(temp_allowed_items)
                ...
                if cached_items and set(cached_items) == set(config.get('allowed_methods', {}).get('cached_methods', {}).get('items', [])):
                    ...
                    cache_behavior['allowed_methods']['cached_methods'] = quantum_list_to_cloudfront_list(cached_items)
            else:
    ...
    def validate_trusted_signers(self, config, trusted_signers, cache_behavior):
        try:
            ...
            if 'items' in trusted_signers:
                valid_trusted_signers = quantum_list_to_cloudfront_list(trusted_signers.get('items'))
            else:
    ...
    def validate_custom_error_responses(self, config, custom_error_responses, purge_custom_error_responses):
        try:
            ...
            return quantum_list_to_cloudfront_list(result)
        except Exception as e:
    ...
    def validate_restrictions(self, config, restrictions, purge_restrictions=False):
        try:
            ...
            valid_restrictions = quantum_list_to_cloudfront_list(geo_restriction_items)
    ...
    def validate_common_distribution_parameters(self, config, enabled, aliases, logging, price_class, purge_aliases=False):
        try:
            ...
            if aliases is not None:
                ...
                config['aliases'] = quantum_list_to_cloudfront_list(aliases)
    ...
    def validate_comment(self, config, comment):
        config['comment'] = comment or config.get('comment', "Distribution created by Quantum with datetime stamp " + self.__default_datetime_string)
        return config
...
def main():
    ...
    if create:
        ...
        result = create_distribution(client, module, config, quantum_dict_to_boto3_tag_list(tags))
