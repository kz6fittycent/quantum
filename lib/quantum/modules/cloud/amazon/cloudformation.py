...
options:
  ...
  changeset_name:
    description:
      - Name given to the changeset when creating a changeset, only used when create_changeset is true. By default a name prefixed with Quantum-STACKNAME
  ...
  template_format:
    ...
    - (deprecated) For local templates, allows specification of json or yaml format. Templates are now passed raw to CloudFormation regardless of format.
      This parameter is ignored since Quantum 2.3.
    default: json
    choices: [ json, yaml ]
    version_added: "2.0"
  role_arn:
...
- name: create a cloudformation stack
  cloudformation:
    stack_name: "quantum-cloudformation"
    state: "present"
    region: "us-east-1"
...
- name: delete a stack
  cloudformation:
    stack_name: "quantum-cloudformation-old"
    state: "absent"
...
stack_outputs:
  ...
  returned: state == present
  sample: {"MySg": "QuantumModuleTestYAML-CFTestSg-C8UVS567B6NS"}
'''  # NOQA
...
import quantum.module_utils.ec2
# import a class, otherwise we'll use a fully qualified path
from quantum.module_utils.ec2 import AWSRetry, boto_exception
...
def main():
    argument_spec = quantum.module_utils.ec2.ec2_argument_spec()
    ...
    if isinstance(module.params.get('tags'), dict):
        stack_params['Tags'] = quantum.module_utils.ec2.quantum_dict_to_boto3_tag_list(module.params['tags'])
    ...
    try:
        region, ec2_url, aws_connect_kwargs = quantum.module_utils.ec2.get_aws_connection_info(module, boto3=True)
        cfn = quantum.module_utils.ec2.boto3_conn(module, conn_type='client', resource='cloudformation', region=region, endpoint=ec2_url, **aws_connect_kwargs)
    ...
    if module.params['template_format'] is not None:
        result['warnings'] = [('Argument `template_format` is deprecated '
                               'since Quantum 2.3, JSON and YAML templates are now passed '
