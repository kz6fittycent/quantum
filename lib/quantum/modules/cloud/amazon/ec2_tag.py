...
from quantum.module_utils.ec2 import boto3_tag_list_to_quantum_dict, quantum_dict_to_boto3_tag_list, compare_aws_tags
...
def get_tags(ec2, module, resource):
    ...
    try:
        return boto3_tag_list_to_quantum_dict(ec2.describe_tags(Filters=filters)['Tags'])
...
def main():
    ...
    module = QuantumAWSModule(argument_spec=argument_spec, required_if=required_if, supports_check_mode=True)
    ...
    if remove_tags:
        ...
        if not module.check_mode:
            try:
                ec2.delete_tags(Resources=[resource], Tags=quantum_dict_to_boto3_tag_list(remove_tags))
            except (BotoCoreError, ClientError) as e:
    ...
    if state == 'present' and add_tags:
        ...
        if not module.check_mode:
            try:
                ec2.create_tags(Resources=[resource], Tags=quantum_dict_to_boto3_tag_list(add_tags))
            except (BotoCoreError, ClientError) as e:
