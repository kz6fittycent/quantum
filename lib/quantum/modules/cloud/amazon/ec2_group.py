...
from quantum.module_utils.aws.core import QuantumAWSModule, is_boto3_error_code
from quantum.module_utils.aws.iam import get_aws_account_id
from quantum.module_utils.aws.waiters import get_waiter
from quantum.module_utils.ec2 import AWSRetry, camel_dict_to_snake_dict, compare_aws_tags
from quantum.module_utils.ec2 import quantum_dict_to_boto3_filter_list, boto3_tag_list_to_quantum_dict, quantum_dict_to_boto3_tag_list
from quantum.module_utils.common.network import to_ipv6_subnet, to_subnet
from quantum.module_utils.compat.ipaddress import ip_network, IPv6Network
...
try:
    ...
    pass  # caught by QuantumAWSModule
    ...
    if rule.get('group_id') and re.match(FOREIGN_SECURITY_GROUP_REGEX, rule['group_id']):
        ...
        if group_name == name:
            ...
            if not rule.get('group_desc', '').strip():
                ...
                try:
                    auto_group = get_security_groups_with_backoff(client, Filters=quantum_dict_to_boto3_filter_list(filters)).get('SecurityGroups', [])[0]
...
def update_tags(client, module, group_id, current_tags, tags, purge_tags):
    ...
    if not module.check_mode:
        ...
        if tags_need_modify:
            try:
                client.create_tags(Resources=[group_id], Tags=quantum_dict_to_boto3_tag_list(tags_need_modify))
            except (BotoCoreError, ClientError) as e:
...
def main():
    ...
    if state == 'absent':
        if group:
            ...
            before['tags'] = boto3_tag_list_to_quantum_dict(before.get('tags', []))
            try:
                if not module.check_mode:
    ...
    # Ensure requested group is present
        ...
        if tags is not None and group is not None:
            current_tags = boto3_tag_list_to_quantum_dict(group.get('Tags', []))
    ...
    if group:
        ...
        security_group['tags'] = boto3_tag_list_to_quantum_dict(security_group.get('tags', []))
