...
from quantum.module_utils.ec2 import camel_dict_to_snake_dict, HAS_BOTO3, compare_aws_tags
...
def update_tags(module, connection, group, tags):
    ...
    to_update, to_delete = compare_aws_tags(boto3_tag_list_to_quantum_dict(existing_tags),
...
def ensure_present(module, connection):
    ...
    if not response:
        ...
        if tags:
            params['Tags'] = quantum_dict_to_boto3_tag_list(tags)
        try:
    ...
    try:
        ...
        module.fail_json(msg="Couldn't obtain parameter group tags: %s" % str(e),
                         exception=traceback.format_exc(),
                         **camel_dict_to_snake_dict(e.response))
    group['tags'] = boto3_tag_list_to_quantum_dict(tags)
