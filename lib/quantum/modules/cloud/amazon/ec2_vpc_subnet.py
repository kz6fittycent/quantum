...
from quantum.module_utils.ec2 import (quantum_dict_to_boto3_filter_list, quantum_dict_to_boto3_tag_list,
                                      ec2_argument_spec, camel_dict_to_snake_dict, get_aws_connection_info,
                                      boto3_conn, boto3_tag_list_to_quantum_dict, compare_aws_tags, AWSRetry)
...
def get_subnet_info(subnet):
    ...
    if 'tags' in subnet:
        subnet['tags'] = boto3_tag_list_to_quantum_dict(subnet['tags'])
    else:
        subnet['tags'] = dict()
...
def ensure_tags(conn, module, subnet, tags, purge_tags, start_time):
    ...
    filters = quantum_dict_to_boto3_filter_list({'resource-id': subnet['id'], 'resource-type': 'subnet'})
    try:
        cur_tags = conn.describe_tags(Filters=filters)
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_aws(e, msg="Couldn't describe tags")

    to_update, to_delete = compare_aws_tags(boto3_tag_list_to_quantum_dict(cur_tags.get('Tags')), tags, purge_tags)
        ...
        try:
            if not module.check_mode:
                AWSRetry.exponential_backoff(
                    ...
                    Resources=[subnet['id']],
                    Tags=quantum_dict_to_boto3_tag_list(to_update)
                )
...
def disassociate_ipv6_cidr(conn, module, subnet, start_time):
    ...
    if module.params['wait']:
        filters = quantum_dict_to_boto3_filter_list(
...
def ensure_ipv6_cidr_block(conn, module, subnet, ipv6_cidr, check_mode, start_time):
    ...
    if ipv6_cidr:
        filters = quantum_dict_to_boto3_filter_list({'ipv6-cidr-block-association.ipv6-cidr-block': ipv6_cidr,
                                                     'vpc-id': subnet['vpc_id']})
...
def get_matching_subnet(conn, module, vpc_id, cidr):
    filters = quantum_dict_to_boto3_filter_list({'vpc-id': vpc_id, 'cidr-block': cidr})
    try:
...
def main():
    ...
    module = QuantumAWSModule(argument_spec=argument_spec, supports_check_mode=True, required_if=required_if)
