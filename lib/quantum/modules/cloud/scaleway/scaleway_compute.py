...
from quantum.module_utils.six.moves.urllib.parse import quote as urlquote
from quantum.module_utils.scaleway import SCALEWAY_LOCATION, scaleway_argument_spec, Scaleway
    ...
    try:
        for key in PATCH_MUTABLE_SERVER_ATTRIBUTES:
            if key in target_server and key in wished_server:
                # When you are working with dict, only ID matter as we ask user to put only the resource ID in the coupling
