...
class QuantumCloudStackLBRuleMember(QuantumCloudStack):

    def __init__(self, module):
        super(QuantumCloudStackLBRuleMember, self).__init__(module)
        self.returns = {
            'publicip': 'public_ip',
            'algorithm': 'algorithm',
            'cidrlist': 'cidr',
    ...
    def get_result(self, rule):
        super(QuantumCloudStackLBRuleMember, self).get_result(rule)
        if rule:
            self.result['vms'] = []
            for vm in self._get_members_of_rule(rule=rule):
...
def main():
    ...
    acs_lb_rule_member = QuantumCloudStackLBRuleMember(module)
