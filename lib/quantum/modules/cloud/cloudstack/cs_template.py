...
- name: Create a template from a stopped virtual machine's volume
  cs_template:
    name: Debian 9 (64-bit) 20GB ({{ quantum_date_time.date }})
    vm: debian-9-base-vm
...
- name: Create a template from a stopped virtual machine's volume
  cs_template:
    ...
    display_text: Debian 9 (64-bit) 20GB ({{ quantum_date_time.date }})
...
class QuantumCloudStackTemplate(QuantumCloudStack):

    def __init__(self, module):
        super(QuantumCloudStackTemplate, self).__init__(module)
        self.returns = {
            'checksum': 'checksum',
            'status': 'status',
            'isready': 'is_ready',
    ...
    def get_result(self, template):
        super(QuantumCloudStackTemplate, self).get_result(template)
        if template:
...
def main():
    ...
    acs_tpl = QuantumCloudStackTemplate(module)
