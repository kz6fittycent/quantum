...
description:
  ...
  - Sets Quantum facts accessible by the key C(cloudstack_zone) and since version 2.6 also returns results.
deprecated:
  removed_in: "2.13"
...
- name: Show the facts by the quantum_facts key cloudstack_zone
  debug:
    var: cloudstack_zone
'''
...
from quantum.module_utils.cloudstack import (
    QuantumCloudStack,
    cs_argument_spec,
)


class QuantumCloudStackZoneFacts(QuantumCloudStack):

    def __init__(self, module):
        super(QuantumCloudStackZoneFacts, self).__init__(module)
        self.returns = {
            'dns1': 'dns1',
            'dns2': 'dns2',
    ...
    def get_zone(self):
        return super(QuantumCloudStackZoneFacts, self).get_zone()
...
def main():
    ...
    acs_zone_facts = QuantumCloudStackZoneFacts(module=module)
