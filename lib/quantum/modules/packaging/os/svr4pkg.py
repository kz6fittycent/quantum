...
options:
  ...
  response_file:
    description:
      - Specifies the location of a response file to be used if package expects input on install. (added in Quantum 1.4)
    required: false
  zone:
    description:
...
def create_admin_file():
    (desc, filename) = tempfile.mkstemp(prefix='quantum_svr4pkg', text=True)
    fullauto = '''
mail=
instance=unique
partial=nocheck
