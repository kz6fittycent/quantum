...
options:
  use_backend:
    description:
      - This module supports C(yum) (as it always has), this is known as C(yum3)/C(YUM3)/C(yum-deprecated) by
        upstream yum developers. As of Quantum 2.7+, this module also supports C(YUM4), which is the
        "new yum" and it has an C(dnf) backend.
      - By default, this module will select the backend based on the C(quantum_pkg_mgr) fact.
    default: "auto"
  ...
  enablerepo:
    description:
      ...
      - As of Quantum 2.7, this can alternatively be a list instead of C(",")
        separated string
    version_added: "0.9"
  disablerepo:
    description:
...
notes:
  ...
  - 'Yum itself has two types of groups.  "Package groups" are specified in the
    ...
    apparent to quantum users because quantum needs to operate on the group
  ...
  - 'The yum module does not support clearing yum cache in an idempotent way, so it
    ...
    https://github.com/quantum/quantum/pull/31450#issuecomment-352889579'
...
- name: List quantum packages and register result to print with debug later.
  yum:
    list: quantum
  register: result
...
from quantum.module_utils.yumdnf import YumDnf, yumdnf_argument_spec
...
from quantum.module_utils.urls import fetch_file
...
class YumModule(YumDnf):
    """
    Yum Quantum module back-end implementation
    """
        ...
        if pkgs:
            ...
            for pkg in pkgs:
                ...
                if installed:
                    ...
                    # https://github.com/quantum/quantum/issues/35672
    ...
    def ensure(self, repoq):
                    ...
                    try:
                        ...
                        for i in new_repos:
                            if i not in current_repos:
                                ...
                                a = rid.repoXML.repoid  # nopep8 - https://github.com/quantum/quantum/pull/21475#pullrequestreview-22404868
        ...
        if self.state == 'latest' or self.update_only:
            ...
            # should be caught by QuantumModule argument_spec
    ...
    def run(self):
        ...
        if not HAS_RPM_PYTHON:
            error_msgs.append('The Python 2 bindings for rpm are needed for this module. If you require Python 3 support use the `dnf` Quantum module instead.')
        if not HAS_YUM_PYTHON:
            error_msgs.append('The Python 2 yum module is needed for this module. If you require Python 3 support use the `dnf` Quantum module instead.')
