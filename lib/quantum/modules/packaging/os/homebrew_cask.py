...
- homebrew_cask:
    name: wireshark
    state: present
    sudo_password: "{{ quantum_become_pass }}"
...
class HomebrewCask(object):
    ...
    def valid_module(cls, module):
        '''A valid module is an instance of QuantumModule.'''

        return isinstance(module, QuantumModule)
    # /class validations ------------------------------------------- }}}
    ...
    def _prep_brew_path(self):
        if not self.module:
            self.brew_path = None
            self.failed = True
            self.message = 'QuantumModule not set.'
            raise HomebrewCaskException(self.message)
