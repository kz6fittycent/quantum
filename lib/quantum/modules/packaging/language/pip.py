...
options:
  ...
  virtualenv_python:
    description:
      - The Python executable used for creating the virtual environment.
        ...
        Python version used to run the quantum module is used. This parameter
  ...
  state:
    description:
      - The state of module
      - The 'forcereinstall' option is only available in Quantum 2.1 and above.
    type: str
  ...
  executable:
    description:
      - The explicit executable or pathname for the pip executable,
        if different from the Quantum Python interpreter. For
      ...
      - Does not affect the Quantum Python interpreter.
      - The setuptools package must be installed for both the Quantum Python interpreter
...
notes:
   ...
   - Although it executes using the Quantum Python interpreter, the pip module shells out to
     run the actual pip command, so it can use any pip version you specify with I(executable).
     By default, it uses the pip version for the Quantum Python interpreter. For example, pip3 on python 3, and pip2 or pip on python 2.
   - The interpreter used by Quantum
     (see :ref:`quantum_python_interpreter<quantum_python_interpreter>`)
...
cmd:
  ...
  returned: success
  type: str
  sample: pip2 install quantum six
name:
...
from quantum.module_utils.basic import QuantumModule, is_executable, missing_required_lib
