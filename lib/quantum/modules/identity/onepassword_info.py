...
notes:
    ...
    - When this module is called with the deprecated C(onepassword_facts) name, potentially sensitive data
      from 1Password is returned as Quantum facts. Facts are subject to caching if enabled, which means this
...
description:
    ...
    - This module was called C(onepassword_facts) before Quantum 2.9, returning C(quantum_facts).
      Note that the M(onepassword_info) module no longer returns C(quantum_facts)!
...
options:
    ...
    auto_login:
        ...
        description:
            ...
            - Without this option, you must have already logged in via the 1Password CLI before running Quantum.
            - It is B(highly) recommended to store 1Password credentials in an Quantum Vault. Ensure that the key used to encrypt
              the Quantum Vault is equal to or greater in strength than the 1Password master password.
        suboptions:
            subdomain:
                type: str
                description:
...
class OnePasswordInfo(object):
    ...
    def _run(self, args, expected_rc=0, command_input=None, ignore_errors=False):
        ...
        if not ignore_errors and rc != expected_rc:
            raise QuantumModuleError(to_native(err))
        return rc, out, err
...
def main():
    ...
    if module._name == 'onepassword_facts':
        module.deprecate("The 'onepassword_facts' module has been renamed to 'onepassword_info'. "
                         "When called with the new name it no longer returns 'quantum_facts'", version='2.13')
        module.exit_json(changed=False, quantum_facts=results)
    else:
