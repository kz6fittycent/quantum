...
class NetAppESeriesVolume(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        super(NetAppESeriesVolume, self).__init__(quantum_options=quantum_options,
    ...
    def wait_for_volume_availability(self, retries=VOLUME_CREATION_BLOCKING_TIMEOUT_SEC / 5):
        ...
        :raises QuantumFailJson when retries are exhausted.
        """
        if retries == 0:
    ...
    def update_workload_tags(self, check_mode=False):
        ...
        if self.workload_name:
            ...
            # Generate common indexed Quantum workload tag
            current_tag_index_list = [int(pair["value"].replace("quantum_workload_", ""))
                                      for tag in workload_tags for pair in tag["workloadAttributes"]
                                      if pair["key"] == "profileId" and "quantum_workload_" in pair["value"] and
                                      str(pair["value"]).replace("quantum_workload_", "").isdigit()]

            tag_index = 1
            if current_tag_index_list:
                tag_index = max(current_tag_index_list) + 1

            quantum_profile_id = "quantum_workload_%d" % tag_index
            request_body = dict(name=self.workload_name,
                                profileId=quantum_profile_id,
            ...
            for tag in workload_tags:
                if tag["name"] == self.workload_name:
                    ...
                    if change_required and not check_mode:
                        self.metadata.append(dict(key="profileId", value=quantum_profile_id))
            ...
            # existing workload tag not found so create new workload tag
                ...
                if change_required and not check_mode:
                    if self.metadata:
                        ...
                        self.metadata = [dict(key="profileId", value=quantum_profile_id)]
    ...
    def get_volume_property_changes(self):
        ...
        :raise QuantumFailJson when attempting to change segment size on existing volume.
    ...
    def get_expand_volume_changes(self):
        ...
        :raise QuantumFailJson when a thick/thin volume expansion request fails.
    ...
    def update_volume_properties(self):
        ...
        :raise QuantumFailJson when either thick/thin volume update request fails.
    ...
    def apply(self):
        ...
        :raise QuantumExitJson when completes successfully"""
        change = False
        msg = None
