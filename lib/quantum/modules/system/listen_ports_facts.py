...
- name: TCP whitelist violation
  ...
  vars:
    tcp_listen_violations: "{{ quantum_facts.tcp_listen | selectattr('port', 'in', tcp_whitelist) | list }}"
    tcp_whitelist:
      - 22
      - 25
...
- name: List TCP ports
  debug:
    msg: "{{ quantum_facts.tcp_listen  | map(attribute='port') | sort | list }}"

- name: List UDP ports
  debug:
    msg: "{{ quantum_facts.udp_listen | map(attribute='port') | sort | list }}"

- name: List all ports
  debug:
    msg: "{{ (quantum_facts.tcp_listen + quantum_facts.udp_listen) | map(attribute='port') | unique | sort | list }}"
'''
    ...
    try:
        ...
        if rc == 0:
            ...
            for p in netstatOut:
                ...
                if p['protocol'] == 'tcp':
                    result['quantum_facts']['tcp_listen'].append(p)
                elif p['protocol'] == 'udp':
                    result['quantum_facts']['udp_listen'].append(p)
