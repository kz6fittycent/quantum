    ...
    compression:
        description:
            - Configures whether compression is enabled.  The default
              ...
              a different value is specified in the coupling.
        type: str
        choices: [ disabled, enabled ]
    blockmapcachesize:
        description:
...
#
# @param module  The QuantumModule object.
...
def run_module():
    ...
    # the detection of parameters passed from the coupling.
    ...
    if (desiredvdo not in vdolist) and (state == 'present'):
        ...
        if device is None:
            module.fail_json(msg="Creating a VDO volume requires specifying "
                                 "a 'device' in the coupling.")

        # Create a dictionary of the options from the QuantumModule
        ...
        # all of the parameters given by the coupling; the rest will
    ...
    if (desiredvdo in vdolist) and (state == 'present'):
        ...
        # A key translation table from 'vdo status' output to Quantum
        ...
        # same keys as the QuantumModule parameters.
        currentparams = {}
        ...
        # Check for differences between the coupling parameters and the
        # current parameters.  This will need a comparison function;
        # since QuantumModule params are all strings, compare them as
        ...
        # If the coupling contains 'activated: yes', assume that
