...
options:
  ...
  permanent:
    description:
      ...
      - As of Quantum 2.3, permanent operations can operate on firewalld configs when it is not running (requires firewalld >= 3.0.9).
...
notes:
  ...
  - For distributions where the python2 firewalld bindings are unavailable (e.g Fedora 28 and later) you will have to set the
    quantum_python_interpreter for these hosts to the python3 interpreter path and install the python3 bindings.
...
from quantum.module_utils.firewalld import FirewallTransaction, fw_offline
