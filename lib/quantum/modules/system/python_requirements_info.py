...
description:
    ...
    - This module was called C(python_requirements_facts) before Quantum 2.9. The usage did not change.
version_added: "2.7"
options:
  dependencies:
    description: >
      ...
      I(quantum), the module with a specific version like I(boto3==1.6.1), or a
...
def main():
    ...
    for dep in (module.params.get('dependencies') or []):
        ...
        if match is None:
            module.fail_json(msg="Failed to parse version requirement '{0}'. Must be formatted like 'quantum>2.6'".format(dep))
