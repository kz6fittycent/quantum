...
options:
  ...
  backup:
    ...
    version_added: '2.5'
notes:
  - As of Quantum 2.3, the I(name) option has been changed to I(path) as
...
from quantum.module_utils.ismount import ismount
...
def remount(module, args):
    ...
    try:
        if get_platform().lower().endswith('bsd'):
            ...
            # https://github.com/quantum/quantum-modules-core/issues/5591
            rc = 1
        else:
            rc, out, err = module.run_command(cmd)
    except Exception:
...
# from @jupeter -- https://github.com/quantum/quantum-modules-core/pull/2923
# @jtyr -- https://github.com/quantum/quantum-modules-core/issues/4439
...
def is_bind_mounted(module, linux_mounts, dest, src=None, fstype=None):
    ...
    :arg module: The QuantumModule (used for helper functions)
