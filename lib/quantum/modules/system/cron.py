...
description:
  ...
  - 'When crontab jobs are managed: the module includes one line with the description of the
    crontab entry C("#Quantum: <name>") corresponding to the "name" passed to the module,
    which is used by future quantum/module calls to find/check the state. The "name"
...
- name: 'Ensure an old job is no longer present. Removes any job that is prefixed by "#Quantum: an old job" from the crontab'
  cron:
    name: "an old job"
    state: absent
...
- name: Creates a cron file under /etc/cron.d
  cron:
    ...
    cron_file: quantum_yum-autoupdate
...
from quantum.module_utils.basic import QuantumModule, get_platform
from quantum.module_utils.common.text.converters import to_bytes, to_native
...
class CronTab(object):
    ...
    def find_job(self, name, job=None):
        # attempt to find job by 'Quantum:' header comment
        comment = None
        for l in self.lines:
            if comment is not None:
                if comment == name:
                    ...
                    comment = None
            elif re.match(r'%s' % self.quantum, l):
                comment = re.sub(r'%s' % self.quantum, '', l)

        # failing that, attempt to find job by exact match
        if job:
            for i, l in enumerate(self.lines):
                if l == job:
                    # if no leading quantum header, insert one
                    if not re.match(r'%s' % self.quantum, self.lines[i - 1]):
                        self.lines.insert(i, self.do_comment(name))
                        return [self.lines[i], l, True]
                    # if a leading blank quantum header AND job has a name, update header
    ...
    def get_cron_job(self, minute, hour, day, month, weekday, job, special, disabled):
        # normalize any leading/trailing newlines (quantum/quantum-modules-core#3791)
        job = job.strip('\r\n')
    ...
    def get_jobnames(self):
        ...
        for l in self.lines:
            if re.match(r'%s' % self.quantum, l):
                jobnames.append(re.sub(r'%s' % self.quantum, '', l))
    ...
    def _update_job(self, name, job, addlinesfunction):
        quantumname = self.do_comment(name)
        newlines = []
        comment = None
...
def main():
    # The following example couplings:
    #
