...
notes:
  - When accessing the C(quantum_facts.services) facts collected by this module,
    ...
    C(quantum_facts.services.zuul-gateway). It is instead recommended to
    using the string value of the service name as the key in order to obtain
    the fact data value like C(quantum_facts.services['zuul-gateway'])
...
- debug:
    var: quantum_facts.services

'''

RETURN = '''
quantum_facts:
  description: Facts to add to quantum_facts about the services on the system
  returned: always
  type: complex
  contains:
    services:
...
def main():
    module = QuantumModule(argument_spec=dict(), supports_check_mode=True)
    ...
    if len(all_services) == 0:
        ...
        results = dict(quantum_facts=dict(services=all_services))
