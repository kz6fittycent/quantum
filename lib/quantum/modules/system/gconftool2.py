...
class GConf2Preference(object):
    def __init__(self, quantum, key, value_type, value,
    ...
    def call(self, call_type, fail_onerr=True):
        ...
        try:
            ...
            rc, out, err = self.quantum.run_command(cmd, use_unsafe_shell=True)

            if len(err) > 0:
                if fail_onerr:
                    self.quantum.fail_json(msg='gconftool-2 failed with '
                                               'error: %s' % (str(err)))
            else:
                changed = True

        except OSError as exception:
            self.quantum.fail_json(msg='gconftool-2 failed with exception: '
                                       '%s' % exception)
...
def main():
    ...
    facts = dict(gconftool2={'changed': change,
                             ...
                             'coupling_value': module.params['value']})

    module.exit_json(changed=change, quantum_facts=facts)
