...
Add-CSharpType -QuantumModule $module -References @'
...
namespace Quantum.CredentialManager
{
...
Function ConvertTo-CredentialAttribute {
    ...
    $converted_attributes = [System.Collections.Generic.List`1[Quantum.CredentialManager.CredentialAttribute]]@()
    foreach ($attribute in $Attributes) {
        $new_attribute = New-Object -TypeName Quantum.CredentialManager.CredentialAttribute
...
Function Get-DiffInfo {
    param($QuantumCredential)

    $diff = @{
        alias = $QuantumCredential.TargetAlias
        attributes = [System.Collections.ArrayList]@()
        comment = $QuantumCredential.Comment
        name = $QuantumCredential.TargetName
        persistence = $QuantumCredential.Persist.ToString()
        type = $QuantumCredential.Type.ToString()
        username = $QuantumCredential.UserName
    }

    foreach ($attribute in $QuantumCredential.Attributes) {
        $attribute_info = @{
...
$persistence = switch ($persistence) {
    "local" { [Quantum.CredentialManager.CredentialPersist]::LocalMachine }
    "enterprise" { [Quantum.CredentialManager.CredentialPersist]::Enterprise }
}

$type = switch ($type) {
    "domain_password" { [Quantum.CredentialManager.CredentialType]::DomainPassword }
    "domain_certificate" { [Quantum.CredentialManager.CredentialType]::DomainCertificate }
    "generic_password" { [Quantum.CredentialManager.CredentialType]::Generic }
    "generic_certificate" { [Quantum.CredentialManager.CredentialType]::GenericCertificate }
}

$existing_credential = [Quantum.CredentialManager.Credential]::GetCredential($name, $type)
if ($null -ne $existing_credential) {
    $module.Diff.before = Get-DiffInfo -QuantumCredential $existing_credential
}
...
if ($state -eq "absent") {
    ...
    if ($null -eq $existing_credential) {
        $new_credential = New-Object -TypeName Quantum.CredentialManager.Credential
    ...
    if ($module.CheckMode) {
        ...
        $new_credential = [Quantum.CredentialManager.Credential]::GetCredential($name, $type)
        $module.Diff.after = Get-DiffInfo -QuantumCredential $new_credential
    }
}
