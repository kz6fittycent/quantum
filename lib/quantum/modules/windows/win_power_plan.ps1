...
namespace Quantum.WinPowerPlan
{
...
Function Get-PlanName {
    ...
    $buffer_size = 0
    $buffer = [IntPtr]::Zero
    [Quantum.WinPowerPlan.NativeMethods]::PowerReadFriendlyName([IntPtr]::Zero, $Plan, [IntPtr]::Zero, [IntPtr]::Zero,
        $buffer, [ref]$buffer_size) > $null

    $buffer = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($buffer_size)
    try {
        $res = [Quantum.WinPowerPlan.NativeMethods]::PowerReadFriendlyName([IntPtr]::Zero, $Plan, [IntPtr]::Zero,
...
Function Get-PowerPlans {
    ...
    while ($true) {
        $buffer_size = 0
        $buffer = [IntPtr]::Zero
        $res = [Quantum.WinPowerPlan.NativeMethods]::PowerEnumerate([IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero,
            [Quantum.WinPowerPlan.AccessFlags]::AccessScheme, $i, $buffer, [ref]$buffer_size)
...
Function Get-ActivePowerPlan {
    $buffer = [IntPtr]::Zero
    $res = [Quantum.WinPowerPlan.NativeMethods]::PowerGetActiveScheme([IntPtr]::Zero, [ref]$buffer)
    if ($res -ne 0) {
        $err_msg = Get-LastWin32ErrorMessage -ErrorCode $res
    ...
    try {
        ...
        [Quantum.WinPowerPlan.NativeMethods]::LocalFree($buffer) > $null
    }
...
Function Set-ActivePowerPlan {
    ...
    if ($PSCmdlet.ShouldProcess($Plan, "Set Power Plan")) {
        $res = [Quantum.WinPowerPlan.NativeMethods]::PowerSetActiveScheme([IntPtr]::Zero, $Plan)
    }
