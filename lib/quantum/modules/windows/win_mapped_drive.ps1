...
Function Get-LimitedToken {
    ...
    $h_token = [Quantum.AccessToken.TokenUtil]::OpenProcessToken($h_process, "Duplicate, Query")

    try {
        # If we don't have a Full token, we don't need to get the limited one to set a mapped drive
        $tet = [Quantum.AccessToken.TokenUtil]::GetTokenElevationType($h_token)
        if ($tet -ne [Quantum.AccessToken.TokenElevationType]::Full) {
            return
        }

        foreach ($system_token in [Quantum.AccessToken.TokenUtil]::EnumerateUserTokens("S-1-5-18", "Duplicate")) {
            ...
            $token_privileges = [Quantum.AccessToken.TokenUtil]::GetTokenPrivileges($system_token)
            ...
            try {
                return [Quantum.AccessToken.TokenUtil]::GetTokenLinkedToken($h_token)
            } finally {
...
try {
    ...
    $existing_targets = [Quantum.MappedDrive.Utils]::GetMappedDrives($i_token_ptr)
    ...
    if ($state -eq "absent") {
        if ($null -ne $existing_target) {
            ...
            if (-not $module.CheckMode) {
                [Quantum.MappedDrive.Utils]::RemoveMappedDrive($letter_root, $i_token_ptr)
            }
        ...
        $add_method = [Quantum.MappedDrive.Utils].GetMethod("AddMappedDrive")
