...
$var_name = Get-QuantumParam $parsed_args "name" -Default "PATH"
$elements = Get-QuantumParam $parsed_args "elements" -FailIfEmpty $result
$state = Get-QuantumParam $parsed_args "state" -Default "present" -ValidateSet "present","absent"
$scope = Get-QuantumParam $parsed_args "scope" -Default "machine" -ValidateSet "machine","user"

$check_mode = Get-QuantumParam $parsed_args "_quantum_check_mode" -Default $false
