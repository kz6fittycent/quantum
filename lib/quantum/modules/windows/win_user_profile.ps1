...
namespace Quantum.WinUserProfile
{
...
Function Get-ExpectedProfilePath {
    ...
    [Quantum.WinUserProfile.NativeMethods]::GetProfilesDirectoryW($null,
    ...
    $res = [Quantum.WinUserProfile.NativeMethods]::GetProfilesDirectoryW($raw_profile_path,
...
if ($state -eq "absent") {
    ...
    foreach ($user_profile in $user_profiles) {
        ...
        if (-not $module.CheckMode) {
            $res = [Quantum.WinUserProfile.NativeMethods]::DeleteProfileW($user_profile.PSChildName, [IntPtr]::Zero,
                [IntPtr]::Zero)
            if ($res -eq $false) {
                $msg = Get-LastWin32ExceptionMessage -Error ([System.Runtime.InteropServices.Marshal]::GetLastWin32Error())
    ...
    if ($null -eq $user_profile) {
        ...
        if ($module.CheckMode) {
            ...
            $res = [Quantum.WinUserProfile.NativeMethods]::CreateProfile($username.Value, $name, $raw_profile_path,
