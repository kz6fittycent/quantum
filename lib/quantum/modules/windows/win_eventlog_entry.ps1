...
$log = Get-QuantumParam -obj $params -name "log" -type "str" -failifempty $true
$source = Get-QuantumParam -obj $params -name "source" -type "str" -failifempty $true
$event_id = Get-QuantumParam -obj $params -name "event_id" -type "int" -failifempty $true
$message = Get-QuantumParam -obj $params -name "message" -type "str" -failifempty $true
$entry_type = Get-QuantumParam -obj $params -name "entry_type" -type "str" -validateset "Error","FailureAudit","Information","SuccessAudit","Warning"
$category = Get-QuantumParam -obj $params -name "category" -type "int"
$raw_data = Get-QuantumParam -obj $params -name "raw_data" -type "str"
