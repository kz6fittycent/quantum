...
$name = Get-QuantumParam $params -name "name" -type str -failifempty $true -aliases 'website'
$state = Get-QuantumParam $params "state" -default "present" -validateSet "present","absent"
$host_header = Get-QuantumParam $params -name "host_header" -type str
$protocol = Get-QuantumParam $params -name "protocol" -type str -default 'http'
$port = Get-QuantumParam $params -name "port" -default '80'
$ip = Get-QuantumParam $params -name "ip" -default '*'
$certificateHash = Get-QuantumParam $params -name "certificate_hash" -type str -default ([string]::Empty)
$certificateStoreName = Get-QuantumParam $params -name "certificate_store_name" -type str -default ([string]::Empty)
$sslFlags = Get-QuantumParam $params -name "ssl_flags" -default '0' -ValidateSet '0','1','2','3'
