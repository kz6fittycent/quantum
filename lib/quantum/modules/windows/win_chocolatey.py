...
options:
  ...
  force:
    ...
    - Using I(force) will cause Quantum to always report that a change was
      made.
    type: bool
    default: no
  install_args:
  ...
  package_params:
    ...
    - Before Quantum 2.7, this option was just I(params).
    type: str
    version_added: '2.1'
    aliases: [ params ]
  pinned:
  ...
  proxy_username:
    ...
    - Before Quantum 2.7, users with double quote characters C(") would need to
...
stdout:
  description: The stdout from the chocolatey task. The verbosity level of the
    messages are affected by Quantum verbosity setting, see notes for more
    details.
  returned: changed
  type: str
