...
Function Get-CustomFacts {
  ...
  foreach ($FactsFile in $FactsFiles) {
      ...
      $result.quantum_facts.Add("quantum_$(($FactsFile.Name).Split('.')[0])", $out)
  }
}
...
$factpath = Get-QuantumParam -obj $params -name "fact_path" -type "path"
$gather_subset_source = Get-QuantumParam -obj $params -name "gather_subset" -type "list" -default "all"
...
if($gather_subset.Contains('bios')) {
    ...
    $quantum_facts += @{
        quantum_bios_date = $null
        quantum_bios_version = $win32_bios.SMBIOSBIOSVersion
        quantum_product_name = $win32_cs.Model.Trim()
        quantum_product_serial = $win32_bios.SerialNumber
        # quantum_product_version = ([string] $win32_cs.SystemFamily)
    }

    if ($win32_bios.ReleaseDate) {
        $quantum_facts.quantum_bios_date = $win32_bios.ReleaseDate.ToString("MM/dd/yyyy")
    }
}
...
if($gather_subset.Contains('distribution')) {
    ...
    $quantum_facts += @{
        quantum_distribution = $win32_os.Caption
        quantum_distribution_version = $osversion.Version.ToString()
        quantum_distribution_major_version = $osversion.Version.Major.ToString()
        quantum_os_family = "Windows"
        quantum_os_name = ($win32_os.Name.Split('|')[0]).Trim()
        quantum_os_product_type = $product_type
    }
}
...
if($gather_subset.Contains('facter')) {
    ...
    if ($facter_installed) {
        ...
        ForEach($fact in $facts.PSObject.Properties) {
            $fact_name = $fact.Name
            $quantum_facts.Add("facter_$fact_name", $fact.Value)
        }
    }
}
...
if($gather_subset.Contains('interfaces')) {
    ...
    $quantum_facts += @{
        quantum_interfaces = $formattednetcfg
    }
}
...
if($gather_subset.Contains('memory')) {
    ...
    $quantum_facts += @{
        ...
        quantum_memtotal_mb = ([math]::ceiling($win32_cs.TotalPhysicalMemory / 1024 / 1024))
        quantum_swaptotal_mb = ([math]::round($win32_os.TotalSwapSpaceSize / 1024))
    }
}
...
if($gather_subset.Contains('platform')) {
    ...
    try {
        $quantum_reboot_pending = Get-PendingRebootStatus
    } catch {
        # fails for non-admin users, set to null in this case
        $quantum_reboot_pending = $null
    }

    $quantum_facts += @{
        quantum_architecture = $win32_os.OSArchitecture
        quantum_domain = $domain_suffix
        quantum_fqdn = $fqdn
        quantum_hostname = $ip_props.HostName
        quantum_netbios_name = $win32_cs.Name
        quantum_kernel = $osversion.Version.ToString()
        quantum_nodename = $fqdn
        quantum_machine_id = Get-MachineSid
        quantum_owner_contact = ([string] $win32_cs.PrimaryOwnerContact)
        quantum_owner_name = ([string] $win32_cs.PrimaryOwnerName)
        # FUTURE: should this live in its own subset?
        quantum_reboot_pending = $quantum_reboot_pending
        quantum_system = $osversion.Platform.ToString()
        quantum_system_description = ([string] $win32_os.Description)
        quantum_system_vendor = $win32_cs.Manufacturer
    }
}

if($gather_subset.Contains('powershell_version')) {
    $quantum_facts += @{
        quantum_powershell_version = ($PSVersionTable.PSVersion.Major)
    }
}
...
if($gather_subset.Contains('processor')) {
    ...
    $quantum_facts += @{
        quantum_processor = $cpu_list
        quantum_processor_cores = $win32_cpu.NumberOfCores
        quantum_processor_count = $win32_cs.NumberOfProcessors
        quantum_processor_threads_per_core = ($win32_cpu.NumberOfLogicalProcessors / $win32_cpu.NumberofCores)
        quantum_processor_vcpus = $win32_cs.NumberOfLogicalProcessors
    }
}
...
if($gather_subset.Contains('uptime')) {
    $win32_os = Get-LazyCimInstance Win32_OperatingSystem
    $quantum_facts += @{
        quantum_lastboot = $win32_os.lastbootuptime.ToString("u")
        quantum_uptime_seconds = $([System.Convert]::ToInt64($(Get-Date).Subtract($win32_os.lastbootuptime).TotalSeconds))
    }
}
...
if($gather_subset.Contains('windows_domain')) {
    ...
    $quantum_facts += @{
        quantum_windows_domain = $win32_cs.Domain
        quantum_windows_domain_member = $win32_cs.PartOfDomain
        quantum_windows_domain_role = $domain_role
    }
}
...
if($gather_subset.Contains('winrm')) {
    ...
    if ($winrm_cert_expirations) {
        # this fact was renamed from quantum_winrm_certificate_expires due to collision with quantum_winrm_X connection var pattern
        $quantum_facts.Add("quantum_win_rm_certificate_expires", $winrm_cert_expirations[0].NotAfter.ToString("yyyy-MM-dd HH:mm:ss"))
    }
}
...
if($gather_subset.Contains('virtual')) {
    ...
    $quantum_facts += @{
        quantum_virtualization_role = $machine_role
        quantum_virtualization_type = $machine_type
    }
}

$result.quantum_facts += $quantum_facts
