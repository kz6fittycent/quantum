...
If ($state -eq "absent") {
    ...
    If (Get-Member -InputObject $ShortCut -Name Arguments) {
        ...
        If (($null -ne $src) -and ($ShortCut.TargetPath -ne $src)) {
            if ((Test-Path -Path $dest) -and (-not $ShortCut.TargetPath)) {
                ...
                $expanded_src = [Quantum.Shortcut.ShellLink]::GetDisplayNameFromPath($src)
                $actual_src = [Quantum.Shortcut.ShellLink]::GetTargetPath($dest)
    ...
    if ((Test-Path -Path $dest) -and $file_shortcut) {
        ...
        $flags = [Quantum.Shortcut.ShellLink]::GetFlags($dest)
        if ($run_as_admin -and (-not $flags.HasFlag([Quantum.Shortcut.ShellLinkFlags]::RunAsUser))) {
            [Quantum.Shortcut.ShellLink]::SetFlags($dest, ($flags -bor [Quantum.Shortcut.ShellLinkFlags]::RunAsUser))
            $module.Result.changed = $true
        } elseif (-not $run_as_admin -and ($flags.HasFlag([Quantum.Shortcut.ShellLinkFlags]::RunAsUser))) {
            [Quantum.Shortcut.ShellLink]::SetFlags($dest, ($flags -bxor [Quantum.Shortcut.ShellLinkFlags]::RunAsUser))
            $module.Result.changed = $true
        }
    }
}
