...
# Copyright: (c) 2016, Quantum Project
...
$paths = Get-QuantumParam -obj $params -name 'paths' -failifempty $true

$age = Get-QuantumParam -obj $params -name 'age'
$age_stamp = Get-QuantumParam -obj $params -name 'age_stamp' -default 'mtime' -ValidateSet 'mtime','ctime','atime'
$file_type = Get-QuantumParam -obj $params -name 'file_type' -default 'file' -ValidateSet 'file','directory'
$follow = Get-QuantumParam -obj $params -name 'follow' -type "bool" -default $false
$hidden = Get-QuantumParam -obj $params -name 'hidden' -type "bool" -default $false
$patterns = Get-QuantumParam -obj $params -name 'patterns' -aliases "regex","regexp"
$recurse = Get-QuantumParam -obj $params -name 'recurse' -type "bool" -default $false
$size = Get-QuantumParam -obj $params -name 'size'
$use_regex = Get-QuantumParam -obj $params -name 'use_regex' -type "bool" -default $false
$get_checksum = Get-QuantumParam -obj $params -name 'get_checksum' -type "bool" -default $true
$checksum_algorithm = Get-QuantumParam -obj $params -name 'checksum_algorithm' -default 'sha1' -ValidateSet 'md5', 'sha1', 'sha256', 'sha384', 'sha512'
...
Function Get-FileStat($file) {
    ...
    if ($attributes -contains 'ReparsePoint') {
        ...
        try {
            $lnk_source = [Quantum.Command.SymLinkHelper]::GetSymbolicLinkTarget($file)
