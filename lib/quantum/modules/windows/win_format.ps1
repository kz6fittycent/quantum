...
$quantum_volume = Get-QuantumVolume -DriveLetter $drive_letter -Path $path -Label $label
$quantum_file_system = $quantum_volume.FileSystem
$quantum_volume_size = $quantum_volume.Size

$quantum_partition = Get-Partition -Volume $quantum_volume

foreach ($access_path in $quantum_partition.AccessPaths) {
    if ($access_path -ne $Path) {
        ...
        if (-not $force_format -and $files_in_volume -gt 0) {
            ...
            if (-not $force_format -and
                ...
                -not [string]::IsNullOrEmpty($quantum_file_system) -and
                $file_system -ne $quantum_file_system) {
                $module.FailJson("Force format must be specified since target file system: $($file_system) is different from the current file system of the volume: $($quantum_file_system.ToLower())")
            } else {
                $pristine = $true
            }
        }
...
if ($force_format) {
    if (-not $module.CheckMode) {
        Format-QuantumVolume -Path $quantum_volume.Path -Full $full_format -Label $new_label -FileSystem $file_system -SetIntegrityStreams $integrity_streams -UseLargeFRS $large_frs -Compress $compress_volume
    }
    ...
    if ($pristine) {
        if ($null -eq $new_label) {
            $new_label = $quantum_volume.FileSystemLabel
        }
        # Conditions for formatting
        if ($quantum_volume_size -eq 0 -or
            $quantum_volume.FileSystemLabel -ne $new_label) {
            if (-not $module.CheckMode) {
                Format-QuantumVolume -Path $quantum_volume.Path -Full $full_format -Label $new_label -FileSystem $file_system -SetIntegrityStreams $integrity_streams -UseLargeFRS $large_frs -Compress $compress_volume
            }
            $module.Result.changed = $true
