...
# Copyright: (c) 2015, Matt Davis <mdavis_quantum@rolpdog.com>
...
options:
    ...
    reboot:
        description:
        - Quantum will automatically reboot the remote host if it is required
    ...
    state:
        ...
        - This module also supports Quantum check mode, which has the same effect as setting state=searched
        type: str
...
- By default C(win_updates) does not manage reboots, but will signal when a
  reboot is required with the I(reboot_required) return value, as of Quantum v2.5
...
- name: Search-only, return list of found updates (if any), log to C:\quantum_wu.txt
  win_updates:
    category_names: SecurityUpdates
    state: searched
    log_path: C:\quantum_wu.txt
