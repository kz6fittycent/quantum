...
$state = Get-QuantumParam $params "state" -validateset @("domain","workgroup") -failifempty $result

$dns_domain_name = Get-QuantumParam $params "dns_domain_name"
$hostname = Get-QuantumParam $params "hostname"
$workgroup_name = Get-QuantumParam $params "workgroup_name"
$domain_admin_user = Get-QuantumParam $params "domain_admin_user" -failifempty $result
$domain_admin_password = Get-QuantumParam $params "domain_admin_password" -failifempty $result
$domain_ou_path = Get-QuantumParam $params "domain_ou_path"

$log_path = Get-QuantumParam $params "log_path"
$_quantum_check_mode = Get-QuantumParam $params "_quantum_check_mode" -default $false
...
Try {
    ...
    Switch($state) {
        domain {
            ...
            If($result.changed -and -not $_quantum_check_mode) {
                If(-not $domain_match) {
                    If(Is-DomainJoined) {
