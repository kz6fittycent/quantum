...
$path = Get-QuantumParam -obj $params -name "path" -type "str" -failifempty $true -aliases "key"
$name = Get-QuantumParam -obj $params -name "name" -type "str" -aliases "entry","value"
$data = Get-QuantumParam -obj $params -name "data"
$type = Get-QuantumParam -obj $params -name "type" -type "str" -default "string" -validateset "none","binary","dword","expandstring","multistring","string","qword" -aliases "datatype"
$state = Get-QuantumParam -obj $params -name "state" -type "str" -default "present" -validateset "present","absent"
$delete_key = Get-QuantumParam -obj $params -name "delete_key" -type "bool" -default $true
$hive = Get-QuantumParam -obj $params -name "hive" -type "path"
...
if ($registry_path -ne $registry_leaf -and -not $registry_path.Contains('\')) {
    $msg = "path is not using '\' as a separator, support for '/' as a separator will be removed in a future Quantum version"
...
Function Set-StateAbsent {
    param(
        # Used for diffs and exception messages to match up against Quantum input
...
if ($type -in @("binary", "none")) {
    ...
    if ($data -is [String]) {
        ...
        # it needs to be unsigned as Quantum passes in an unsigned value while
...
try {
    if ($hive) {
        ...
        try {
            Set-QuantumPrivilege -Name SeBackupPrivilege -Value $true
            Set-QuantumPrivilege -Name SeRestorePrivilege -Value $true
        ...
        if (Test-Path -Path HKLM:\ANSIBLE) {
            ...
            try {
                [Quantum.WinRegedit.Hive]::UnloadHive("ANSIBLE")
            } catch [System.ComponentModel.Win32Exception] {
        ...
        try {
            $loaded_hive = New-Object -TypeName Quantum.WinRegedit.Hive -ArgumentList "ANSIBLE", $hive
        } catch [System.ComponentModel.Win32Exception] {
