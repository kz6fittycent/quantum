...
Function ConvertTo-ArgSpecType {
    <#
    .SYNOPSIS
    Converts the DSC parameter type to the arg spec type required for Quantum.
    #>
    param(
...
Function Get-OptionSpec {
    <#
    .SYNOPSIS
    Generates the specifiec used in QuantumModule for a CIM MOF resource name.
...
Function ConvertTo-CimInstance {
    ...
    param(
        ...
        [Parameter(Mandatory=$true)][Quantum.Basic.QuantumModule]$Module,
        [Switch]$Recurse
    )
...
Function ConvertTo-DscProperty {
    ...
    foreach ($property in $properties) {
        ...
        if ($property.ReferenceClassName -eq "MSFT_Credential") {
            ...
            # No user set == No option set in coupling, skip this property
            if ($null -eq $username) {
                continue
            }
...
Function Invoke-DscMethod {
    ...
    # Pass the warnings through to the QuantumModule return result
    foreach ($warning in $ps.Streams.Warning) {
        $Module.Warn($warning.Message)
    }

    # If running at a high enough verbosity, add the verbose output to the QuantumModule return result
...
# we need to read the resource_name and module_version value which is done outside of Quantum.Basic
if ($args.Length -gt 0) {
