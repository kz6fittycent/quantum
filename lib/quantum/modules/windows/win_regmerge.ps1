...
$path = Get-QuantumParam -obj $params -name "path" -type "path" -failifempty $true -resultobj $result
$compare_to = Get-QuantumParam -obj $params -name "compare_to" -type "str" -resultobj $result
...
If ( $do_comparison -eq $True ) {
  $guid = [guid]::NewGuid()
  $exported_path = $env:TEMP + "\" + $guid.ToString() + 'quantum_win_regmerge.reg'
