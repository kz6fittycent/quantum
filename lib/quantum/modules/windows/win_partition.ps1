...
$quantum_partition = $false
$quantum_partition_size = $null
...
if ($null -ne $partition_size) {
    ...
    elseif ($partition_size -match '^(?<Size>[0-9]+)[ ]*(?<Units>b|kb|kib|mb|mib|gb|gib|tb|tib)$') {
        $quantum_partition_size = Convert-SizeToBytes -Size $Matches.Size -Units $Matches.Units
    }
    else {
...
if ($null -ne $disk_number -and $null -ne $partition_number) {
    $quantum_partition = Get-Partition -DiskNumber $disk_number -PartitionNumber $partition_number -ErrorAction SilentlyContinue
}
...
elseif ($drive_letter -and -not ($disk_number -and $partition_number)) {
    if ($drive_letter -eq "auto" -or $drive_letter -match "^[a-zA-Z]$") {
        $quantum_partition = Get-Partition -DriveLetter $drive_letter -ErrorAction SilentlyContinue
    }
    else {
...
function Set-QuantumPartitionState {
    param(
        $hidden,
        $read_only,
        $active,
...
if ($quantum_partition) {
    if ($state -eq "absent") {
        try {
            Remove-Partition -DiskNumber $quantum_partition.DiskNumber -PartitionNumber $quantum_partition.PartitionNumber -Confirm:$false -WhatIf:$module.CheckMode
        } catch {
        ...
        if ($null -ne $gpt_type -and $gpt_styles.$gpt_type -ne $quantum_partition.GptType) {
            $module.FailJson("gpt_type is not a valid parameter for existing partitions")
        }
        if ($null -ne $mbr_type -and $mbr_styles.$mbr_type -ne $quantum_partition.MbrType) {
            $module.FailJson("mbr_type is not a valid parameter for existing partitions")
        }

        if ($partition_size) {
            try {
                $max_supported_size = (Get-PartitionSupportedSize -DiskNumber $quantum_partition.DiskNumber -PartitionNumber $quantum_partition.PartitionNumber).SizeMax
            } catch {
                $module.FailJson("Unable to get maximum supported partition size: $($_.Exception.Message)", $_)
            }
            if ($size_is_maximum) {
                $quantum_partition_size = $max_supported_size
            }
            if ($quantum_partition_size -ne $quantum_partition.Size -and $quantum_partition_size -le $max_supported_size) {
                if ($quantum_partition.IsReadOnly) {
                    $module.FailJson("Unable to resize partition: Partition is read only")
                } else {
                    try {
                        Resize-Partition -DiskNumber $quantum_partition.DiskNumber -PartitionNumber $quantum_partition.PartitionNumber -Size $quantum_partition_size -WhatIf:$module.CheckMode
                    } catch {
                        $module.FailJson("Unable to change partition size: $($_.Exception.Message)", $_)
                    }
                    $module.Result.changed = $true
                }
            } elseif ($quantum_partition_size -gt $max_supported_size) {
                $module.FailJson("Specified partition size exceeds size supported by the partition")
            }
        }

        if ($drive_letter -NotIn ("auto", $null, $quantum_partition.DriveLetter)) {
            if (-not $module.CheckMode) {
                try {
                    Set-Partition -DiskNumber $quantum_partition.DiskNumber -PartitionNumber $quantum_partition.PartitionNumber -NewDriveLetter $drive_letter
                } catch {
    ...
    if ($state -eq "present") {
        ...
        if ($null -eq $quantum_partition_size -and -not $size_is_maximum){
        ...
        if (-not $size_is_maximum) {
            ...
            if ($quantum_partition_size -gt $max_supported_size) {
        ...
        if (-not $module.CheckMode) {
            $quantum_partition = New-QuantumPartition -DiskNumber $disk_number -Letter $drive_letter -SizeMax $size_is_maximum -Size $quantum_partition_size -MbrType $mbr_type -GptType $gpt_type -Style $partition_style
        }
        $module.Result.changed = $true
    }
}

if ($state -eq "present" -and $quantum_partition) {
    if ($offline -NotIn ($null, $quantum_partition.IsOffline)) {
        if (-not $module.CheckMode) {
            try {
                Set-Partition -DiskNumber $quantum_partition.DiskNumber -PartitionNumber $quantum_partition.PartitionNumber -IsOffline $offline
            } catch {
    ...
    if ($hidden -NotIn ($null, $quantum_partition.IsHidden) -or $read_only -NotIn ($null, $quantum_partition.IsReadOnly) -or $active -NotIn ($null, $quantum_partition.IsActive)) {
        if (-not $module.CheckMode) {
            Set-QuantumPartitionState -hidden $hidden -read_only $read_only -active $active -partition $quantum_partition
        }
        $module.Result.changed = $true
    }
}
