...
If ($state -eq "absent") {
    ...
    $path = Get-QuantumParam -obj $params -name "path" -type "path" -failifempty $true
    $description = Get-QuantumParam -obj $params -name "description" -type "str" -default ""

    $permissionList = Get-QuantumParam -obj $params -name "list" -type "bool" -default $false
    $folderEnum = if ($permissionList) { "Unrestricted" } else { "AccessBased" }

    $permissionRead = Get-QuantumParam -obj $params -name "read" -type "str" -default "" | NormalizeAccounts
    $permissionChange = Get-QuantumParam -obj $params -name "change" -type "str" -default "" | NormalizeAccounts
    $permissionFull = Get-QuantumParam -obj $params -name "full" -type "str" -default "" | NormalizeAccounts
    $permissionDeny = Get-QuantumParam -obj $params -name "deny" -type "str" -default "" | NormalizeAccounts

    $cachingMode = Get-QuantumParam -obj $params -name "caching_mode" -type "str" -default "Manual" -validateSet "BranchCache","Documents","Manual","None","Programs","Unknown"
    $encrypt = Get-QuantumParam -obj $params -name "encrypt" -type "bool" -default $false
