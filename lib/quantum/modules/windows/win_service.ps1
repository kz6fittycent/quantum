...
$dependencies = Get-QuantumParam -obj $params -name 'dependencies' -type 'list' -default $null
$dependency_action = Get-QuantumParam -obj $params -name 'dependency_action' -type 'str' -default 'set' -validateset 'add','remove','set'
$description = Get-QuantumParam -obj $params -name 'description' -type 'str'
$desktop_interact = Get-QuantumParam -obj $params -name 'desktop_interact' -type 'bool' -default $false
$display_name = Get-QuantumParam -obj $params -name 'display_name' -type 'str'
$force_dependent_services = Get-QuantumParam -obj $params -name 'force_dependent_services' -type 'bool' -default $false
...
$path = Get-QuantumParam -obj $params -name 'path'
$start_mode = Get-QuantumParam -obj $params -name 'start_mode' -type 'str' -validateset 'auto','manual','disabled','delayed'
$state = Get-QuantumParam -obj $params -name 'state' -type 'str' -validateset 'started','stopped','restarted','absent','paused'
$username = Get-QuantumParam -obj $params -name 'username' -type 'str'
...
# https://github.com/quantum/quantum/issues/37621
$svc = Get-Service | Where-Object { $_.Name -eq $name -or $_.DisplayName -eq $name }
if ($svc) {
