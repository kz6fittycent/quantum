...
#Requires -Module Quantum.ModuleUtils.CamelConversion
...
$client = Get-QuantumWebRequest -Module $module
        ...
        try {
            ...
            if ($return_content) {
                ...
                if ($module.Result.ContainsKey("content_type") -and $module.Result.content_type -Match ($JSON_CANDIDATES -join '|')) {
                    try {
                        $module.Result.json = ([Quantum.Basic.QuantumModule]::FromJson($module.Result.content))
