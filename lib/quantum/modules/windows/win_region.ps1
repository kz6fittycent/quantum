...
$check_mode = Get-QuantumParam -obj $params "_quantum_check_mode" -type 'bool' -default $false
$_remote_tmp = Get-QuantumParam $params "_quantum_remote_tmp" -type "path" -default $env:TMP

$location = Get-QuantumParam -obj $params -name 'location' -type 'str'
$format = Get-QuantumParam -obj $params -name 'format' -type 'str'
$unicode_language = Get-QuantumParam -obj $params -name 'unicode_language' -type 'str'
$copy_settings = Get-QuantumParam -obj $params -name 'copy_settings' -type 'bool' -default $false
...
Function Get-SystemLocaleName {
    ...
    try {
        $res = [Quantum.WinRegion.NativeMethods]::GetSystemDefaultLocaleName($ptr, $max_length)
...
Function Get-UserLocaleName {
    ...
    try {
        $res = [Quantum.WinRegion.NativeMethods]::GetUserDefaultLocaleName($ptr, $max_length)
...
Function Set-UserLocale($culture) {
    ...
    $lookup = New-Object Quantum.WinRegion.LocaleHelper($culture)
