...
#Requires -Module Quantum.ModuleUtils.Backup
...
function Present($path, $regex, $line, $insertafter, $insertbefore, $create, $backup, $backrefs, $validate, $encodingobj, $linesep, $check_mode, $diff_support) {
	...
	# Note that we have to clean up the path because quantum wants to treat / and \ as
...
function Absent($path, $regex, $line, $backup, $validate, $encodingobj, $linesep, $check_mode, $diff_support) {
	...
	# that we have to clean up the path because quantum wants to treat / and \ as
	# interchangeable in windows pathnames, but .NET framework internals do not support that.
	$cleanpath = $path.Replace("/", "\");
	$before = [System.IO.File]::ReadAllLines($cleanpath, $encodingobj);
	If ($null -eq $before) {
...
# Parse the parameters file dropped by the Quantum machinery
...
$path = Get-QuantumParam -obj $params -name "path" -type "path" -failifempty $true -aliases "dest","destfile","name";
$regex = Get-QuantumParam -obj $params -name "regex" -type "str" -aliases "regexp";
$state = Get-QuantumParam -obj $params -name "state" -type "str" -default "present" -validateset "present","absent";
$line = Get-QuantumParam -obj $params -name "line" -type "str";
$backrefs = Get-QuantumParam -obj $params -name "backrefs" -type "bool" -default $false;
$insertafter = Get-QuantumParam -obj $params -name "insertafter" -type "str";
$insertbefore = Get-QuantumParam -obj $params -name "insertbefore" -type "str";
$create = Get-QuantumParam -obj $params -name "create" -type "bool" -default $false;
$backup = Get-QuantumParam -obj $params -name "backup" -type "bool" -default $false;
$validate = Get-QuantumParam -obj $params -name "validate" -type "str";
$encoding = Get-QuantumParam -obj $params -name "encoding" -type "str" -default "auto";
$newline = Get-QuantumParam -obj $params -name "newline" -type "str" -default "windows" -validateset "unix","windows";
