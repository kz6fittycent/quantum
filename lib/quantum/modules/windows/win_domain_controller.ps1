...
$dns_domain_name = Get-QuantumParam -obj $params -name "dns_domain_name"
$safe_mode_password= Get-QuantumParam -obj $params -name "safe_mode_password"
$domain_admin_user = Get-QuantumParam -obj $params -name "domain_admin_user" -failifempty $result
$domain_admin_password= Get-QuantumParam -obj $params -name "domain_admin_password" -failifempty $result
$local_admin_password= Get-QuantumParam -obj $params -name "local_admin_password"
$database_path = Get-QuantumParam -obj $params -name "database_path" -type "path"
$sysvol_path = Get-QuantumParam -obj $params -name "sysvol_path" -type "path"
$read_only = Get-QuantumParam -obj $params -name "read_only" -type "bool" -default $false
$site_name = Get-QuantumParam -obj $params -name "site_name" -type "str" -failifempty $read_only

$state = Get-QuantumParam -obj $params -name "state" -validateset ("domain_controller", "member_server") -failifempty $result

$log_path = Get-QuantumParam -obj $params -name "log_path"
$_quantum_check_mode = Get-QuantumParam -obj $params -name "_quantum_check_mode" -default $false
        ...
        domain_controller {
            ...
            If(-not $current_dc_domain) {
                ...
                if ($read_only) {
                    ...
                    # https://github.com/quantum/quantum/issues/35858
                ...
                # required for Quantum to connect back to the host and reboot in a
                # later task. Even if this fails Quantum can still connect but only
                # with quantum_winrm_transport=basic so we just display a warning if
                # this fails.
                try {
                    Start-Service -Name Netlogon
                } catch {
                    Write-DebugLog "Failed to start the Netlogon service: $($_.Exception.Message)"
                    Add-Warning -obj $result -message "Failed to start the Netlogon service after promoting the host, Quantum may be unable to connect until the host is manually rebooting: $($_.Exception.Message)"
                }
