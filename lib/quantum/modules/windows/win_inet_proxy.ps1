...
namespace Quantum.WinINetProxy
{
...
'@
Add-CSharpType -References $win_inet_invoke -QuantumModule $module

# We need to validate the connection because WinINet will just silently continue even if the connection does not
# already exist.
if ($null -ne $connection -and -not [Quantum.WinINetProxy.WinINetProxy]::IsValidConnection($connection)) {
    $module.FailJson("The connection '$connection' does not exist.")
}

$actual_proxy = New-Object -TypeName Quantum.WinINetProxy.WinINetProxy -ArgumentList @(,$connection)
$module.Diff.before = @{
