...
$arguments = Get-QuantumParam -obj $params -name "arguments"
$expected_return_code = Get-QuantumParam -obj $params -name "expected_return_code" -type "list" -default @(0, 3010)
$path = Get-QuantumParam -obj $params -name "path" -type "str"
$chdir = Get-QuantumParam -obj $params -name "chdir" -type "path"
$product_id = Get-QuantumParam -obj $params -name "product_id" -type "str" -aliases "productid"
$state = Get-QuantumParam -obj $params -name "state" -type "str" -default "present" -validateset "absent","present" -aliases "ensure"
$username = Get-QuantumParam -obj $params -name "username" -type "str" -aliases "user_name"
$password = Get-QuantumParam -obj $params -name "password" -type "str" -failifempty ($null -ne $username) -aliases "user_password"
$validate_certs = Get-QuantumParam -obj $params -name "validate_certs" -type "bool" -default $true
$creates_path = Get-QuantumParam -obj $params -name "creates_path" -type "path"
$creates_version = Get-QuantumParam -obj $params -name "creates_version" -type "str"
$creates_service = Get-QuantumParam -obj $params -name "creates_service" -type "str"
$log_path = Get-QuantumParam -obj $params -name "log_path" -type "path"
...
Function Get-ProgramMetadata($state, $path, $product_id, [PSCredential]$credential, $creates_path, $creates_version, $creates_service) {
    ...
    if ($null -ne $product_id) {
        ...
        if (($metadata.msi -eq $true) -and (($metadata.location_type -eq [LocationType]::Local) -or ($metadata.location_type -eq [LocationType]::Unc -and $null -eq $credential))) {
            ...
            try {
                $metadata.product_id = [Quantum.MsiTools]::GetPackageProperty($path, "ProductCode")
            } catch {
