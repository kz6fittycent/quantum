...
options:
  ...
  domain_username:
    ...
    - If this is not set then the user Quantum used to log in with will be
      used instead.
    type: str
  domain_password:
    description:
...
- name: Ensure the group Cow exists using sAMAccountName
  win_domain_group:
    name: Cow
    scope: global
    path: OU=groups,DC=quantum,DC=local

- name: Ensure the group Cow doesn't exist using the Distinguished Name
  win_domain_group:
    name: CN=Cow,OU=groups,DC=quantum,DC=local
    state: absent
...
- name: Create group with delete protection enabled and custom attributes
  win_domain_group:
    ...
    category: security
    attributes:
      mail: helpdesk@quantum.com
      wWWHomePage: www.quantum.com
    ignore_protection: yes
...
- name: Change the OU of a group using the SID and ignore the protection flag
  win_domain_group:
    ...
    scope: global
    organizational_unit: OU=groups,DC=quantum,DC=local
    ignore_protection: yes
...
- name: Add group and specify the AD domain services to use for the create
  win_domain_group:
    ...
    domain_server: corp-DC12.corp.quantum.com
    scope: domainlocal
'''
...
canonical_name:
  ...
  type: str
  sample: quantum.local/groups/Cow
category:
...
distinguished_name:
  ...
  returned: group exists
  type: str
  sample: CN=Cow,OU=groups,DC=quantum,DC=local
group_scope:
...
managed_by:
  ...
  returned: group exists
  type: str
  sample: CN=Domain Admins,CN=Users,DC=quantum,DC=local
name:
