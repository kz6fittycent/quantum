...
$check_mode = Get-QuantumParam -obj $params -name "_quantum_check_mode" -type "bool" -default $false
$diff_mode = Get-QuantumParam -obj $params -name "_quantum_diff" -type "bool" -default $false
$_remote_tmp = Get-QuantumParam $params "_quantum_remote_tmp" -type "path" -default $env:TMP

$name = Get-QuantumParam -obj $params -name "name" -type "str" -failifempty $true
$path = Get-QuantumParam -obj $params -name "path" -type "str" -default "\"
...
$actions = Get-QuantumParam -obj $params -name "actions" -type "list"

# task triggers, list of dicts [{ type, ... }]
$triggers = Get-QuantumParam -obj $params -name "triggers" -type "list"

# task Principal properties
$display_name = Get-QuantumParam -obj $params -name "display_name" -type "str"
$group = Get-QuantumParam -obj $params -name "group" -type "str"
$logon_type = Get-QuantumParam -obj $params -name "logon_type" -type "str" -validateset "none","password","s4u","interactive_token","group","service_account","interactive_token_or_password"
$run_level = Get-QuantumParam -obj $params -name "run_level" -type "str" -validateset "limited", "highest" -aliases "runlevel"
$username = Get-QuantumParam -obj $params -name "username" -type "str" -aliases "user"
$password = Get-QuantumParam -obj $params -name "password" -type "str"
$update_password = Get-QuantumParam -obj $params -name "update_password" -type "bool" -default $true

# task RegistrationInfo properties
$author = Get-QuantumParam -obj $params -name "author" -type "str"
$date = Get-QuantumParam -obj $params -name "date" -type "str"
$description = Get-QuantumParam -obj $params -name "description" -type "str"
$source = Get-QuantumParam -obj $params -name "source" -type "str"
$version = Get-QuantumParam -obj $params -name "version" -type "str"

# task Settings properties
$allow_demand_start = Get-QuantumParam -obj $params -name "allow_demand_start" -type "bool"
$allow_hard_terminate = Get-QuantumParam -obj $params -name "allow_hard_terminate" -type "bool"
$compatibility =  Get-QuantumParam -obj $params -name "compatibility" -type "int" # https://msdn.microsoft.com/en-us/library/windows/desktop/aa383486(v=vs.85).aspx
$delete_expired_task_after = Get-QuantumParam -obj $params -name "delete_expired_task_after" -type "str" # time string PT...
$disallow_start_if_on_batteries = Get-QuantumParam -obj $params -name "disallow_start_if_on_batteries" -type "bool"
$enabled = Get-QuantumParam -obj $params -name "enabled" -type "bool"
$execution_time_limit = Get-QuantumParam -obj $params -name "execution_time_limit" -type "str" # PT72H
$hidden = Get-QuantumParam -obj $params -name "hidden" -type "bool"
# TODO: support for $idle_settings, needs to be created as a COM object
$multiple_instances = Get-QuantumParam -obj $params -name "multiple_instances" -type "int" # https://msdn.microsoft.com/en-us/library/windows/desktop/aa383507(v=vs.85).aspx
# TODO: support for  $network_settings, needs to be created as a COM object
$priority = Get-QuantumParam -obj $params -name "priority" -type "int" # https://msdn.microsoft.com/en-us/library/windows/desktop/aa383512(v=vs.85).aspx
$restart_count = Get-QuantumParam -obj $params -name "restart_count" -type "int"
$restart_interval = Get-QuantumParam -obj $params -name "restart_interval" -type "str" # time string PT..
$run_only_if_idle = Get-QuantumParam -obj $params -name "run_only_if_idle" -type "bool"
$run_only_if_network_available = Get-QuantumParam -obj $params -name "run_only_if_network_available" -type "bool"
$start_when_available = Get-QuantumParam -obj $params -name "start_when_available" -type "bool"
$stop_if_going_on_batteries = Get-QuantumParam -obj $params -name "stop_if_going_on_batteries" -type "bool"
$wake_to_run = Get-QuantumParam -obj $params -name "wake_to_run" -type "bool"
...
Function Compare-PropertyList {
    Param(
        ...
        [Array]$new, # a list of new properties, passed in by Quantum
    ...
    <## map metadata structure
    {
        collection type [TASK_ACTION_TYPE] for Actions or [TASK_TRIGGER_TYPE2] for Triggers {
            mandatory = list of mandatory properties for this type, quantum input name not the COM name
            optional = list of optional properties that could be set for this type
            # maps the quantum input object name to the COM name, e.g. working_directory = WorkingDirectory
            map = {
                quantum input name = COM name
            }
        }
    }##>
