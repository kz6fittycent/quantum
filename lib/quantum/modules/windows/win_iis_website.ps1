...
$application_pool = Get-QuantumParam -obj $params -name "application_pool" -type "str"
$physical_path = Get-QuantumParam -obj $params -name "physical_path" -type "str"
$site_id = Get-QuantumParam -obj $params -name "site_id" -type "str"
$state = Get-QuantumParam -obj $params -name "state" -type "str" -validateset "absent","restarted","started","stopped"

# Binding Parameters
$bind_port = Get-QuantumParam -obj $params -name "port" -type "int"
$bind_ip = Get-QuantumParam -obj $params -name "ip" -type "str"
$bind_hostname = Get-QuantumParam -obj $params -name "hostname" -type "str"

# Custom site Parameters from string where properties
# are separated by a pipe and property name/values by colon.
# Ex. "foo:1|bar:2"
$parameters = Get-QuantumParam -obj $params -name "parameters" -type "str"
