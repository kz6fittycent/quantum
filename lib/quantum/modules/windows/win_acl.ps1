...
Function SetPrivilegeTokens() {
    ...
    if ($myWindowsPrincipal.IsInRole($adminRole)) {
        ...
        foreach ($privilege in $privileges) {
            $state = Get-QuantumPrivilege -Name $privilege
            if ($state -eq $false) {
                Set-QuantumPrivilege -Name $privilege -Value $true
            }
        }
    }
}
...
$path = Get-QuantumParam -obj $params -name "path" -type "str" -failifempty $true
$user = Get-QuantumParam -obj $params -name "user" -type "str" -failifempty $true
$rights = Get-QuantumParam -obj $params -name "rights" -type "str" -failifempty $true

$type = Get-QuantumParam -obj $params -name "type" -type "str" -failifempty $true -validateset "allow","deny"
$state = Get-QuantumParam -obj $params -name "state" -type "str" -default "present" -validateset "absent","present"

$inherit = Get-QuantumParam -obj $params -name "inherit" -type "str"
$propagation = Get-QuantumParam -obj $params -name "propagation" -type "str" -default "None" -validateset "InheritOnly","None","NoPropagateInherit"
