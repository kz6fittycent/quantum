...
$path = Get-QuantumParam -obj $params -name "path" -type "path" -failifempty $true -aliases "dest","name"
$state = Get-QuantumParam -obj $params -name "state" -type "str" -validateset "absent","directory","file","touch"
    ...
    try {
        if ($file.Attributes -band [System.IO.FileAttributes]::ReparsePoint) {
            ...
            if ($file.PSIsContainer) {
                if (-not $checkmode) {
                    [Quantum.Command.SymLinkHelper]::DeleteDirectory($file.FullName)
                }
            } else {
                if (-not $checkmode) {
                    [Quantum.Command.SymlinkHelper]::DeleteFile($file.FullName)
                }
            }
