...
$name = Get-QuantumParam -obj $params -name "name" -type "list" -failifempty $true
$state = Get-QuantumParam -obj $params -name "state" -type "str" -default "present" -validateset "present","absent"

$include_sub_features = Get-QuantumParam -obj $params -name "include_sub_features" -type "bool" -default $false
$include_management_tools = Get-QuantumParam -obj $params -name "include_management_tools" -type "bool" -default $false
$source = Get-QuantumParam -obj $params -name "source" -type "str"

$install_cmdlet = $false
if (Get-Command -Name Install-WindowsFeature -ErrorAction SilentlyContinue) {
    Set-Alias -Name Install-QuantumWindowsFeature -Value Install-WindowsFeature
    Set-Alias -Name Uninstall-QuantumWindowsFeature -Value Uninstall-WindowsFeature
    $install_cmdlet = $true
} elseif (Get-Command -Name Add-WindowsFeature -ErrorAction SilentlyContinue) {
    Set-Alias -Name Install-QuantumWindowsFeature -Value Add-WindowsFeature
    Set-Alias -Name Uninstall-QuantumWindowsFeature -Value Remove-WindowsFeature
} else {
...
if ($state -eq "present") {
    ...
    try {
        $action_results = Install-QuantumWindowsFeature @install_args
    } catch {
    ...
    try {
        $action_results = Uninstall-QuantumWindowsFeature @uninstall_args
    } catch {
...
# controls whether Quantum will fail or not
