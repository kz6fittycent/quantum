...
- name: >-
    Read definition file from the Quantum controller file system.
    If the definition file has been encrypted with Quantum Vault it will automatically be decrypted.
  k8s:
    state: present
    definition: "{{ lookup('file', '/testing/deployment.yml') }}"

- name: Read definition file from the Quantum controller file system after Jinja templating
  k8s:
    state: present
