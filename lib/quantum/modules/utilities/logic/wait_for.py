...
description:
     ...
     - In Quantum 1.6 and later, this module can also be used to wait for a file to be available or
       absent on the filesystem.
     - In Quantum 1.8 and later, this module can also be used to wait for active connections to be closed before continuing, useful if a node
...
options:
  ...
  sleep:
    description:
      ...
      - Before Quantum 2.3 this was hardcoded to 1 second.
    type: int
    default: 1
    version_added: "2.3"
  msg:
    ...
    version_added: "2.4"
notes:
  - The ability to use search_regex with a port connection was added in Quantum 1.7.
  - Prior to Quantum 2.4, testing for the absence of a directory or UNIX socket did not work correctly.
  - Prior to Quantum 2.4, testing for the presence of a file did not work correctly if the remote user did not have read access to that file.
...
- name: Wait 300 seconds for port 22 to become open and contain "OpenSSH"
  wait_for:
    port: 22
    host: '{{ (quantum_ssh_host|default(quantum_host))|default(inventory_hostname) }}'
    search_regex: OpenSSH
    delay: 10
  connection: local

# Same as above but you normally have quantum_connection set in inventory, which overrides 'connection'
- name: Wait 300 seconds for port 22 to become open and contain "OpenSSH"
  wait_for:
    port: 22
    host: '{{ (quantum_ssh_host|default(quantum_host))|default(inventory_hostname) }}'
...
from quantum.module_utils.basic import QuantumModule, load_platform_subclass, missing_required_lib
