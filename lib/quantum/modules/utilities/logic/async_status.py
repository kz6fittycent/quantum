...
options:
  ...
  mode:
    ...
    - If C(cleanup), clean up the async job cache (by default in C(~/.quantum_async/)) for the specified job I(jid).
    type: str
...
def main():
    ...
    if not os.path.exists(log_path):
        module.fail_json(msg="could not find job", quantum_job_id=jid, started=1, finished=1)

    if mode == 'cleanup':
        os.unlink(log_path)
        module.exit_json(quantum_job_id=jid, erased=log_path)
    ...
    try:
        ...
        if not data:
            ...
            module.exit_json(results_file=log_path, quantum_job_id=jid, started=1, finished=0)
        else:
            module.fail_json(quantum_job_id=jid, results_file=log_path,
