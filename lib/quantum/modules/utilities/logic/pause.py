...
---
module: pause
short_description: Pause coupling execution
description:
  - Pauses coupling execution for a set amount of time, or until a prompt is acknowledged.
  ...
  - You can use C(ctrl+c) if you wish to advance a pause earlier than it is set to expire or if you need to abort a coupling run entirely.
    To continue early press C(ctrl+c) and then C(c). To abort a coupling press C(ctrl+c) and then C(a).
  - The pause module integrates into async/parallelized couplings without any special considerations (see Rolling Updates).
    When using pauses with the C(serial) coupling parameter (as in rolling updates) you are only prompted once for the current group of hosts.
