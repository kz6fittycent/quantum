...
description:
  ...
  - May be used only where Quantum tasks are allowed - inside C(pre_tasks), C(tasks), or C(post_tasks) coupling objects, or as a task inside a role.
...
options:
  ...
  public:
    description:
      - This option dictates whether the role's C(vars) and C(defaults) are exposed to the coupling. If set to C(yes)
        ...
        coupling parsing time, and available to earlier roles and tasks as well.
    type: bool
    default: no
    version_added: '2.7'
  handlers_from:
...
notes:
  ...
  - Before Quantum 2.4, as with C(include), this task could be static or dynamic, If static, it implied that it won't
    need templating, loops or conditionals and will show included tasks in the C(--list) options. Quantum would try to
    autodetect what is needed, but you can set C(static) to C(yes) or C(no) at task level to control this.
  - After Quantum 2.4, you can use M(import_role) for C(static) behaviour and this action for C(dynamic) one.
seealso:
