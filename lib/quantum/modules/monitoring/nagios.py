...
description:
  ...
  - All actions require the I(host) parameter to be given explicitly. In couplings you can use the C({{inventory_hostname}}) variable to refer
    to the host the coupling is currently running on.
...
def main():
    ...
    # command = command
    #
    # QuantumModule will verify most stuff, we need to verify
    # 'service' manually.
    ...
    ##################################################################
    quantum_nagios = Nagios(module, **module.params)
...
class Nagios(object):
    ...
    def act(self):
        """
        Figure out what you want to do from quantum, and then do the
