...
options:
  ...
  timeout:
    description:
      - If there are pending actions for the service monitored by monit, then Quantum will check
        for up to this many seconds to verify the requested action has been performed.
        Quantum will sleep for five seconds between each check.
    default: 300
    version_added: "2.1"
...
def main():
    ...
    module = QuantumModule(argument_spec=arg_spec, supports_check_mode=True)
