...
description:
- This module is useful for sending emails from couplings.
...
EXAMPLES = r'''
- name: Example coupling sending mail to root
  mail:
    subject: System {{ quantum_hostname }} has been successfully provisioned.
...
- name: Sending an e-mail using Gmail SMTP servers
  mail:
    ...
    body: System {{ quantum_hostname }} has been successfully provisioned.
  delegate_to: localhost
...
- name: Sending an e-mail using the remote machine, not the Quantum controller node
  mail:
    host: localhost
    port: 25
    to: John Smith <john.smith@example.com>
...
def main():
    ...
    if 'X-Mailer' not in msg:
        msg.add_header('X-Mailer', 'Quantum mail module')
