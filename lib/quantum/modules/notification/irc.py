...
- local_action:
    ...
    server: irc.example.net
    channel: #t1
    msg: 'All finished at {{ quantum_date_time.iso8601 }}'
    color: red
    nick: quantumIRC
...
def send_msg(msg, server='localhost', port='6667', channel=None, nick_to=None, key=None, topic=None,
             nick="quantum", color='none', passwd=False, timeout=30, use_ssl=False, part=True, style=None):
    '''send message to IRC'''
    ...
    irc.send(to_bytes('USER %s %s %s :quantum IRC\r\n' % (nick, nick, nick)))
    motd = ''
    start = time.time()
    while 1:
