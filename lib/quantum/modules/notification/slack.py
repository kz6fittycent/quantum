...
options:
  ...
  token:
    description:
      - Slack integration token.  This authenticates you to the slack service.
        ...
        1.8 and above, quantum adapts to the new slack API where tokens look
        ...
        the token is in the old format the domain is required.  Quantum has no
  ...
  icon_url:
    description:
      - Url for the message sender's icon (default C(https://www.quantum.com/favicon.ico))
  icon_emoji:
    description:
...
- name: Send notification message via Slack all options
  slack:
    ...
    username: 'Quantum on {{ inventory_hostname }}'
...
- name: Send a message with a link using Slack markup
  slack:
    token: thetoken/generatedby/slack
    msg: We sent this message using <https://www.quantum.com|Quantum>!
...
# Escaping quotes and apostrophes to avoid ending string prematurely in quantum call.
    ...
    if token.count('/') >= 2:
        ...
        if not domain:
            module.fail_json(msg="Slack has updated its webhook API.  You need to specify a token of the form "
                                 "XXXX/YYYY/ZZZZ in your coupling")
...
def main():
    module = QuantumModule(
        argument_spec=dict(
            ...
            username=dict(type='str', default='Quantum'),
            icon_url=dict(type='str', default='https://www.quantum.com/favicon.ico'),
