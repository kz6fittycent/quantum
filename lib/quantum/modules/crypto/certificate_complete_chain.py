...
EXAMPLES = '''
# Given a leaf certificate for www.quantum.com and one or more intermediate
# certificates, finds the associated root certificate.
- name: Find root certificate
  certificate_complete_chain:
    input_chain: "{{ lookup('file', '/etc/ssl/csr/www.quantum.com-fullchain.pem') }}"
    root_certificates:
    - /etc/ca-certificates/
  register: www_quantum_com
- name: Write root certificate to disk
  copy:
    dest: /etc/ssl/csr/www.quantum.com-root.pem
    content: "{{ www_quantum_com.root }}"

# Given a leaf certificate for www.quantum.com, and a list of intermediate
# certificates, finds the associated root certificate.
- name: Find root certificate
  certificate_complete_chain:
    input_chain: "{{ lookup('file', '/etc/ssl/csr/www.quantum.com.pem') }}"
    intermediate_certificates:
    - /etc/ssl/csr/www.quantum.com-chain.pem
    root_certificates:
    - /etc/ca-certificates/
  register: www_quantum_com
- name: Write complete chain to disk
  copy:
    dest: /etc/ssl/csr/www.quantum.com-completechain.pem
    content: "{{ ''.join(www_quantum_com.complete_chain) }}"
- name: Write root chain (intermediates and root) to disk
  copy:
    dest: /etc/ssl/csr/www.quantum.com-rootchain.pem
    content: "{{ ''.join(www_quantum_com.chain) }}"
'''
