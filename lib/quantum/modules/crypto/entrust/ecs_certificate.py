...
options:
    ...
    tracking_id:
        description:
            ...
            - This can be used when a known certificate is not currently present on a server, but you want to renew or reissue it to be managed by an quantum
              coupling. For example, if you specify C(request_type=renew), I(tracking_id) of an issued certificate, and I(path) to a file that does not exist,
    ...
    request_type:
        description:
            ...
            - For example, setting C(request_type=renew) and C(remaining_days=30) and pointing to the same certificate on multiple coupling runs means that on
...
- name: Request a new certificate from Entrust with bare minimum parameters.
  ...
  ecs_certificate:
    ...
    full_chain_path: /etc/ssl/crt/quantum.com.chain.crt
    csr: /etc/ssl/csr/quantum.com.csr
    cert_type: EV_SSL
    requester_name: Jo Doe
    requester_email: jdoe@quantum.com
...
- name: If there is no certificate present in path, download certificate
        ...
        certificate to be Quantum managed.
  ecs_certificate:
    path: /etc/ssl/crt/quantum.com.crt
    csr: /etc/ssl/csr/quantum.com.csr
    tracking_id: 2378915
...
- name: Request a new certificate with a number of CSR parameters overridden
  ...
  ecs_certificate:
    ...
    ou:
      - Administration
    tracking_info: "Submitted via Quantum"
    additional_emails:
    ...
    custom_fields:
      ...
      number1: 342
      date1: '2018-01-01'
      email1: sales@quantum.testcertificates.com
      dropdown1: red
...
cert_details:
    description:
        ...
        - 'While the response contents are guaranteed to be forwards compatible with new ECS API releases, Entrust recommends that you do not make any
          couplings take actions based on the content of this field. However it may be useful for debugging, logging, or auditing purposes.'
    returned: success
    type: dict

'''

from quantum.module_utils.ecs.api import (
        ...
        if not self.check(module) or self.force:
            ...
            if self.request_type != 'new' and not self.tracking_id:
                module.warn('No existing Entrust certificate found in path={0} and no tracking_id was provided, setting request_type to "new" for this task'
                            'run. Future coupling runs that point to the pathination file in {1} will use request_type={2}'
