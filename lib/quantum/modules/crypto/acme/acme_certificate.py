...
notes:
   ...
   - "This module includes basic account management functionality.
      ...
      option."
   - "This module was called C(letsencrypt) before Quantum 2.6. The usage
      did not change."
seealso:
  ...
  data:
    description:
      ...
      - "I(Note): the C(data) option was marked as C(no_log) up to
         Quantum 2.5. From Quantum 2.6 on, it is no longer marked this way
...
challenge_data:
  description:
    ...
    - Since Quantum 2.8.5, only challenges which are not yet valid are returned.
  returned: changed
  type: list
  elements: dict
  contains:
...
from quantum.module_utils.acme import (
    ModuleFailException,
...
def main():
    ...
    # QuantumModule() changes the locale, so change it back to C because we rely on time.strptime() when parsing certificate dates.
