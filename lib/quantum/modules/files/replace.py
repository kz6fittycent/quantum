...
options:
  path:
    description:
      - The file to modify.
      - Before Quantum 2.3 this option was only usable as I(dest), I(destfile) and I(name).
    type: path
    required: true
  ...
  regexp:
    description:
      ...
      - Note that, as of Quantum 2.0, short form tasks should have any escape
  ...
  encoding:
    ...
    version_added: "2.4"
notes:
  - As of Quantum 2.3, the I(dest) option has been changed to I(path) as default, but I(dest) still works as well.
  - As of Quantum 2.7.10, the combined use of I(before) and I(after) works properly. If you were relying on the
    previous incorrect behavior, you may be need to adjust your tasks.
    See U(https://github.com/quantum/quantum/issues/31354) for details.
  - Option I(follow) has been removed in Quantum 2.5, because this module modifies the contents of the file so I(follow=no) doesn't make sense.
'''

EXAMPLES = r'''
- name: Before Quantum 2.3, option 'dest', 'destfile' or 'name' was used instead of 'path'
  replace:
    path: /etc/hosts
    regexp: '(\s+)old\.host\.name(\s+.*)?$'
    replace: '\1new.host.name\2'

- name: Replace after the expression till the end of the file (requires Quantum >= 2.4)
  replace:
...
- name: Replace before the expression till the begin of the file (requires Quantum >= 2.4)
  replace:
    path: /etc/apache2/sites-available/default.conf
...
# Prior to Quantum 2.7.10, using before and after in combination did the opposite of what was intended.
# see https://github.com/quantum/quantum/issues/31354 for details.
- name: Replace between the expressions (requires Quantum >= 2.4)
  replace:
    path: /etc/hosts
    after: '<VirtualHost [*]>'
    before: '</VirtualHost>'
...
- name: Short form task (in quantum 2+) necessitates backslash-escaped sequences
