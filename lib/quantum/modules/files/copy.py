...
options:
  ...
  mode:
    ...
    - For those used to C(/usr/bin/chmod) remember that modes are actually octal numbers.
      You must either add a leading zero so that Quantum's YAML parser knows it is an octal number
      (like C(0644) or C(01777))or quote it (like C('644') or C('1777')) so Quantum receives a string
      and can do its own conversion from string into number. Giving Quantum a number without following
      one of these rules will end up with a decimal number which will have unexpected results.
    - As of Quantum 1.8, the mode may be specified as a symbolic mode (for example, C(u+rwx) or C(u=rw,g=r,o=r)).
    - As of Quantum 2.3, the mode may also be the special string C(preserve).
  ...
  checksum:
    ...
    - If this is not provided, quantum will use the local calculated checksum of the src file.
    type: str
    version_added: '2.5'
...
# basic::QuantumModule() until then but if so, make it a private function so that we don't have to
...
def split_pre_existing_dir(dirname):
    ...
    if not os.path.exists(b_head):
        if head == '/':
            raise QuantumModuleError(results={'msg': "The '/' directory doesn't exist on this machine."})
...
def main():
    ...
    if dest.endswith(os.sep):
        ...
        if not os.path.exists(b_dirname):
            ...
            except QuantumModuleError as e:
