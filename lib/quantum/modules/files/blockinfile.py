...
options:
  ...
  marker:
    ...
    - Using a custom marker without the C({mark}) variable may result in the block being repeatedly inserted on subsequent coupling runs.
    type: str
  ...
  marker_begin:
    description:
    - This will be inserted at C({mark}) in the opening quantum block marker.
    type: str
    default: BEGIN
    version_added: '2.5'
  marker_end:
    required: false
    description:
    - This will be inserted at C({mark}) in the closing quantum block marker.
    type: str
    default: END
    version_added: '2.5'
notes:
...
EXAMPLES = r'''
# Before Quantum 2.3, option 'dest' or 'name' was used instead of 'path'
...
- name: Insert/Update HTML surrounded by custom markers after <body> line
  blockinfile:
    ...
    insertafter: "<body>"
    block: |
      <h1>Welcome to {{ quantum_hostname }}</h1>
      <p>Last updated on {{ quantum_date_time.iso8601 }}</p>
...
import os
import tempfile
from quantum.module_utils.six import b
...
def main():
    ...
    if present and block:
        # Escape sequences like '\n' need to be handled in Quantum 1.x
        if module.quantum_version.startswith('1.'):
            block = re.sub('', block, '')
