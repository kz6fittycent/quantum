...
options:
  ...
  state:
    ...
    - If C(directory), all intermediate subdirectories will be created if they
      do not exist. Since Quantum 1.7 they will be created with the supplied permissions.
  ...
  follow:
    ...
    - Previous to Quantum 2.5, this was C(no) by default.
    type: bool
    default: yes
    version_added: '1.8'
  modification_time:
...
# There will only be a single QuantumModule object per module
module = None


class QuantumModuleError(Exception):
    def __init__(self, results):
        self.results = results

    def __repr__(self):
        print('QuantumModuleError(results={0})'.format(self.results))


class ParameterError(QuantumModuleError):
    pass
...
def _quantum_excepthook(exc_type, exc_value, tb):
    # Using an exception allows us to catch it if the calling code knows it can recover
    if issubclass(exc_type, QuantumModuleError):
...
def get_timestamp_for_time(formatted_time, time_format):
    if formatted_time == 'preserve':
        ...
        try:
            ...
            raise QuantumModuleError(results={'msg': 'Error while obtaining timestamp for time %s using format %s: %s'
...
def update_timestamp_for_file(path, mtime, atime, diff=None):
    ...
    try:
        ...
        # https://github.com/quantum/quantum/issues/50943
        ...
        raise QuantumModuleError(results={'msg': 'Error while updating modification or access time: %s'
        ...
        if not module.check_mode:
            if prev_state == 'directory':
                try:
                    ...
                    raise QuantumModuleError(results={'msg': "rmtree failed: %s" % to_native(e)})
            else:
                try:
                    os.unlink(b_path)
                except OSError as e:
                    if e.errno != errno.ENOENT:  # It may already have been removed
                        raise QuantumModuleError(results={'msg': "unlinking failed: %s " % to_native(e),
                                                          'path': path})
    ...
    if not module.check_mode:
        if prev_state == 'absent':
            ...
            try:
                ...
                raise QuantumModuleError(results={'msg': 'Error, could not touch target: %s'
...
def ensure_file_attributes(path, follow, timestamps):
    ...
    if prev_state not in ('file', 'hard'):
        ...
        raise QuantumModuleError(results={'msg': 'file (%s) is %s, cannot continue' % (path, prev_state),
...
def ensure_directory(path, follow, recurse, timestamps):
    ...
    if prev_state == 'absent':
        ...
        try:
            ...
            raise QuantumModuleError(results={'msg': 'There was an issue creating %s as requested:'
        ...
        raise QuantumModuleError(results={'msg': '%s already exists as a %s' % (path, prev_state),
                                          'path': path})
...
def ensure_symlink(path, src, follow, force, timestamps):
    ...
    if not force and not os.path.exists(b_absrc):
        raise QuantumModuleError(results={'msg': 'src file does not exist, use "force=yes" if you'
                                                 ' really want to create the link: %s' % absrc,
                                          'path': path, 'src': src})

    if prev_state == 'directory':
        if not force:
            raise QuantumModuleError(results={'msg': 'refusing to convert from %s to symlink for %s'
                                                     % (prev_state, path),
                                              'path': path})
        elif os.listdir(b_path):
            # refuse to replace a directory that has files in it
            raise QuantumModuleError(results={'msg': 'the directory %s is not empty, refusing to'
                                                     ' convert it' % path,
                                              'path': path})
    ...
    if prev_state == 'absent':
        ...
        changed = True
        if not force:
            raise QuantumModuleError(results={'msg': 'Cannot link because a hard link exists at destination',
                                              'dest': path, 'src': src})
    elif prev_state == 'file':
        changed = True
        if not force:
            raise QuantumModuleError(results={'msg': 'Cannot link because a file exists at destination',
                                              'dest': path, 'src': src})
    elif prev_state == 'directory':
        changed = True
        ...
        raise QuantumModuleError(results={'msg': 'unexpected position reached', 'dest': path, 'src': src})
        ...
        if prev_state != 'absent':
            ...
            try:
                ...
                if os.path.exists(b_tmppath):
                    os.unlink(b_tmppath)
                raise QuantumModuleError(results={'msg': 'Error while replacing: %s'
                                                         % to_native(e, nonstring='simplerepr'),
                                                  'path': path})
        else:
            try:
                ...
                raise QuantumModuleError(results={'msg': 'Error while linking: %s'
                                                         % to_native(e, nonstring='simplerepr'),
                                                  'path': path})
...
def ensure_hardlink(path, src, follow, force, timestamps):
    ...
    if src is None:
        raise QuantumModuleError(results={'msg': 'src is required for creating new hardlinks'})

    if not os.path.exists(b_src):
        raise QuantumModuleError(results={'msg': 'src does not exist', 'dest': path, 'src': src})
    ...
    if prev_state == 'absent':
        ...
        if not os.stat(b_path).st_ino == os.stat(b_src).st_ino:
            changed = True
            if not force:
                raise QuantumModuleError(results={'msg': 'Cannot link, different hard link exists at destination',
                                                  'dest': path, 'src': src})
    elif prev_state == 'file':
        changed = True
        if not force:
            raise QuantumModuleError(results={'msg': 'Cannot link, %s exists at destination' % prev_state,
                                              'dest': path, 'src': src})
    elif prev_state == 'directory':
        changed = True
        if os.path.exists(b_path):
...
def main():
    ...
    # When we rewrite basic.py, we will do something similar to this on instantiating an QuantumModule
    sys.excepthook = _quantum_excepthook
