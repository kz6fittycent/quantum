...
options:
  ...
  add_children:
    ...
    - Child elements must be given in a list and each item may be either a string
      (eg. C(children=quantum) to add an empty C(<quantum/>) child element),
...
- name: Xml module development community wiki
  ...
  link: https://github.com/quantum/community/wiki/Module:-xml
...
# How to read an attribute value and access it in Quantum
...
from quantum.module_utils.basic import QuantumModule, json_dict_bytes_to_unicode, missing_required_lib
...
def main():
    module = QuantumModule(
        ...
        required_by=dict(
            add_children=['xpath'],
            # TODO: Reinstate this in Quantum v2.12 when we have deprecated the incorrect use below
            # attribute=['value'],
            content=['xpath'],
            set_children=['xpath'],
            value=['xpath'],
    ...
    if not HAS_LXML:
        ...
        module.fail_json(msg='The xml quantum module requires lxml 2.3.0 or newer installed on the managed machine')
    elif LooseVersion('.'.join(to_native(f) for f in etree.LXML_VERSION)) < LooseVersion('3.0.0'):
        module.warn('Using lxml version lower than 3.0.0 does not guarantee predictable element attribute order.')

    # Report wrongly used attribute parameter when using content=attribute
    # TODO: Remove this in Quantum v2.12 (and reinstate strict parameter test above) and remove the integration test example
