...
from quantum.errors import QuantumParserError, QuantumError, QuantumAssertionError
from quantum.module_utils.six import iteritems, string_types
from quantum.module_utils._text import to_text
from quantum.parsing.splitter import parse_kv, split_args
from quantum.plugins.loader import module_loader, action_loader
...
class ModuleArgsParser:
    ...
    def __init__(self, task_ds=None, collection_list=None):
        ...
        if not isinstance(task_ds, dict):
            raise QuantumAssertionError("the type of 'task_ds' should be a dict, but is a %s" % type(task_ds))
    ...
    def _normalize_parameters(self, thing, action=None, additional_args=None):
        ...
        if additional_args:
            if isinstance(additional_args, string_types):
                ...
                if templar.is_template(additional_args):
                    ...
                    raise QuantumParserError("Complex args containing variables cannot use bare variables (without Jinja2 delimiters), "
                                             "and must use the full variable style ('{{var_name}}')")
            elif isinstance(additional_args, dict):
                final_args.update(additional_args)
            else:
                raise QuantumParserError('Complex args must be a dictionary or variable string ("{{var}}").')
        ...
        if args and action not in FREEFORM_ACTIONS:
            for arg in args:
                arg = to_text(arg)
                if arg.startswith('_quantum_'):
                    raise QuantumError("invalid parameter specified for action '%s': '%s'" % (action, arg))
    ...
    def _normalize_new_style_args(self, thing, action):
        ...
        if isinstance(thing, dict):
            ...
            raise QuantumParserError("unexpected parameter type in action: %s" % type(thing), obj=self._task_ds)
        return args
    ...
    def parse(self, skip_action_validation=False):
        ...
        if 'local_action' in self._task_ds:
            ...
            if action is not None:
                raise QuantumParserError("action and local_action are mutually exclusive", obj=self._task_ds)
        ...
        for item, value in iteritems(non_task_ds):
            if item in BUILTIN_TASKS or skip_action_validation or action_loader.has_plugin(item, collection_list=self._collection_list) or \
                ...
                if action is not None:
                    raise QuantumParserError("conflicting action statements: %s, %s" % (action, item), obj=self._task_ds)
                action = item
                thing = value
                action, args = self._normalize_parameters(thing, action=action, additional_args=additional_args)
        ...
        if action is None:
            if non_task_ds:  # there was one non-task action, but we couldn't find it
                ...
                raise QuantumParserError("couldn't resolve module/action '{0}'. This often indicates a "
                                         "misspelling, missing collection, or incorrect module path.".format(bad_action),
                                         obj=self._task_ds)
            else:
                raise QuantumParserError("no module/action detected in task.",
                                         obj=self._task_ds)
            ...
            if templar.is_template(raw_params):
                ...
                raise QuantumParserError("this task '%s' has extra params, which is only allowed in the following modules: %s" % (action,
