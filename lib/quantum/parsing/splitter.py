...
def parse_kv(args, check_raw=False):
    ...
    if args is not None:
        try:
            ...
            raise QuantumParserError("Unable to parse argument string", orig_exc=e)
        except ValueError as ve:
            if 'no closing quotation' in str(ve).lower():
                raise QuantumParserError("error parsing argument string, try quoting the entire line.", orig_exc=ve)
            else:
                raise
...
def split_args(args):
    ...
    if print_depth or block_depth or comment_depth or inside_quotes:
        raise QuantumParserError(u"failed at splitting arguments, either an unbalanced jinja2 block or quotes: {0}".format(args))
