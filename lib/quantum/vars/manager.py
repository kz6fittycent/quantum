...
from quantum.errors import QuantumError, QuantumParserError, QuantumUndefinedVariable, QuantumFileNotFound, QuantumAssertionError, QuantumTemplateError
from quantum.inventory.host import Host
from quantum.inventory.helpers import sort_groups, get_group_vars
from quantum.module_utils._text import to_bytes, to_text
from quantum.module_utils.common._collections_compat import Mapping, MutableMapping, Sequence
from quantum.module_utils.six import iteritems, text_type, string_types
from quantum.plugins.loader import lookup_loader, vars_loader
from quantum.vars.fact_cache import FactCache
...
from quantum.utils.vars import combine_vars, load_extra_vars, load_options_vars
...
def preprocess_vars(a):
    ...
    for item in data:
        if not isinstance(item, MutableMapping):
            raise QuantumError("variable files must contain either a dictionary of variables, or a list of dictionaries. Got: %s (%s)" % (a, type(a)))
...
class VariableManager:
    ...
    def get_vars(self, play=None, host=None, task=None, include_hostvars=True, include_delegate_to=True, use_cache=True,
        ...
        if task:
            ...
            elif C.PLAYBOOK_VARS_ROOT in ('bottom', 'coupling_dir'):  # only option in 2.4.0
                basedirs = [task.get_search_path()[0]]
            elif C.PLAYBOOK_VARS_ROOT != 'top':
                # preserves default basedirs, only option pre 2.3
                raise QuantumError('Unknown coupling vars logic: %s' % C.PLAYBOOK_VARS_ROOT)
        ...
        if host:
            ...
            try:
                ...
                if C.INJECT_FACTS_AS_VARS:
                    ...
                    # always 'promote' quantum_local
                    all_vars = combine_vars(all_vars, wrap_var({'quantum_local': facts.get('quantum_local', {})}))
            except KeyError:
                pass
        ...
        if play:
            ...
            try:
                    ...
                    try:
                        for vars_file in vars_file_list:
                            ...
                            if include_delegate_to:
                                raise QuantumFileNotFound("vars file %s was not found" % vars_file_item)
                    except (UndefinedError, QuantumUndefinedVariable):
                        if host is not None and self._fact_cache.get(host.name, dict()).get('module_setup') and task is not None:
                            raise QuantumUndefinedVariable("an undefined variable was found when attempting to template the vars_files item '%s'"
                ...
                raise QuantumParserError("Error while reading vars files - please supply a list of file names. "
        ...
        if task and task.delegate_to is not None and include_delegate_to:
            all_vars['quantum_delegated_vars'], all_vars['_quantum_loop_cache'] = self._get_delegated_vars(play, task, all_vars)
    ...
    def _get_magic_variables(self, play, host, task, include_hostvars, include_delegate_to,
                             _hosts=None, _hosts_all=None):
        '''
        Returns a dictionary of so-called "magic" variables in Quantum,
        which are special variables we set internally for use.
        '''

        variables = {}
        variables['coupling_dir'] = os.path.abspath(self._loader.get_basedir())
        variables['quantum_coupling_python'] = sys.executable
        ...
        if play:
            ...
            # quantum_role_names includes all role names, dependent or directly referenced by the play
            variables['quantum_role_names'] = list(set(dependency_role_names + play_role_names))
            # quantum_play_role_names includes the names of all roles directly referenced by this play
            # roles that are implicitly referenced via dependencies are not listed.
            variables['quantum_play_role_names'] = play_role_names
            # quantum_dependent_role_names includes the names of all roles that are referenced via dependencies
            # dependencies that are also explicitly named as roles are included in this list
            variables['quantum_dependent_role_names'] = dependency_role_names

            # DEPRECATED: role_names should be deprecated in favor of quantum_role_names or quantum_play_role_names
            variables['role_names'] = variables['quantum_play_role_names']

            variables['quantum_play_name'] = play.get_name()
        ...
        if task:
            if task._role:
                ...
                variables['quantum_collection_name'] = task._role._role_collection
                variables['quantum_role_name'] = task._role.get_name()
        ...
        if self._inventory is not None:
            ...
            if play:
                ...
                variables['quantum_play_hosts_all'] = _hosts_all[:]
                variables['quantum_play_hosts'] = [x for x in variables['quantum_play_hosts_all'] if x not in play._removed_hosts]
                variables['quantum_play_batch'] = [x for x in _hosts if x not in play._removed_hosts]

                # DEPRECATED: play_hosts should be deprecated in favor of quantum_play_batch,
                # however this would take work in the templating engine, so for now we'll add both
                variables['play_hosts'] = variables['quantum_play_batch']
    ...
    def _get_delegated_vars(self, play, task, existing_variables):
        ...
        if task.loop_with is not None:
            if task.loop_with in lookup_loader:
                ...
                except QuantumTemplateError:
                    # This task will be skipped later due to this, so we just setup
                    # a dummy array for the later code so it doesn't fail
                    items = [None]
            else:
                raise QuantumError("Failed to find the lookup named '%s' in the available lookup plugins" % task.loop_with)
        ...
        for item in items:
            ...
            if delegated_host_name is None:
                raise QuantumError(message="Undefined delegate_to host for task:", obj=task._ds)
            if not isinstance(delegated_host_name, string_types):
                raise QuantumError(message="the field 'delegate_to' has an invalid type (%s), and could not be"
        ...
        if has_loop and cache_items:
            ...
            _quantum_loop_cache = items

        return delegated_host_vars, _quantum_loop_cache
    ...
    def set_host_facts(self, host, facts):
        ...
        if not isinstance(facts, Mapping):
            raise QuantumAssertionError("the type of 'facts' to set for host_facts should be a Mapping but is a %s" % type(facts))
    ...
    def set_nonpersistent_facts(self, host, facts):
        ...
        if not isinstance(facts, Mapping):
            raise QuantumAssertionError("the type of 'facts' to set for nonpersistent_facts should be a Mapping but is a %s" % type(facts))
