...
def module_response_deepcopy(v):
    ...
    Designed to be used within the Quantum "engine" to improve performance
    ...
    * ``quantum.executor.task_result.TaskResult.clean_copy``
    * ``quantum.vars.clean.clean_facts``
    * ``quantum.vars.namespace_facts``
    """
...
def strip_internal_keys(dirty, exceptions=None):
    # All keys starting with _quantum_ are internal, so change the 'dirty' mapping and remove them.
    ...
    if isinstance(dirty, MutableSequence):
        ...
        for k in list(dirty.keys()):
            if isinstance(k, six.string_types):
                if k.startswith('_quantum_') and k not in exceptions:
                    del dirty[k]
                    continue
        ...
        raise QuantumError("Cannot strip invalid keys from %s" % type(dirty))
...
def remove_internal_keys(data):
    ...
    for key in list(data.keys()):
        if (key.startswith('_quantum_') and key != '_quantum_parsed') or key in C.INTERNAL_RESULT_KEYS:
    ...
    for key in list(data.get('quantum_facts', {}).keys()):
        if key.startswith('discovered_interpreter_') or key.startswith('quantum_discovered_interpreter_'):
            del data['quantum_facts'][key]
...
def clean_facts(facts):
    ...
    for conn_path in connection_loader.all(path_only=True):
        try:
            ...
            re_key = re.compile('^quantum_%s_' % conn_name)
            for fact_key in fact_keys:
                # most lightweight VM or container tech creates devices with this pattern, this avoids filtering them out
                if (re_key.match(fact_key) and not fact_key.endswith(('_bridge', '_gwbridge'))) or re_key.startswith('quantum_become_'):
    ...
    re_interp = re.compile('^quantum_.*_interpreter$')
    for fact_key in fact_keys:
        if re_interp.match(fact_key):
            remove_keys.add(fact_key)
    # then we remove them (except for ssh host keys)
    for r_key in remove_keys:
        if not r_key.startswith('quantum_ssh_host_key_'):
            try:
...
def namespace_facts(facts):
    ''' return all facts inside 'quantum_facts' w/o an quantum_ prefix '''
    deprefixed = {}
    for k in facts:
        if k.startswith('quantum_') and k not in ('quantum_local',):
            deprefixed[k[8:]] = module_response_deepcopy(facts[k])
        else:
            deprefixed[k] = module_response_deepcopy(facts[k])

    return {'quantum_facts': deprefixed}
