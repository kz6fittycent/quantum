...
from quantum.errors import QuantumError, QuantumOptionsError, QuantumParserError
...
from quantum.plugins.loader import inventory_loader
from quantum.utils.helpers import deduplicate_list
...
class InventoryManager(object):
    ...
    def _fetch_inventory_plugins(self):
        ...
        if not plugins:
            raise QuantumError("No inventory plugins available to generate inventory, make sure you have at least one whitelisted.")
    ...
    def parse_sources(self, cache=False):
        ...
        if parsed:
            ...
            if C.INVENTORY_UNPARSED_IS_FAILED:
                raise QuantumError("No inventory was parsed, please check your configuration and options.")
            else:
            ...
            for plugin in self._fetch_inventory_plugins():
                ...
                if plugin_wants:
                    try:
                        ...
                        tb = ''.join(traceback.format_tb(sys.exc_info()[2]))
                        failures.append({'src': source, 'plugin': plugin_name, 'exc': QuantumError(e), 'tb': tb})
                else:
                ...
                if not parsed and failures:
                    ...
                    if C.INVENTORY_ANY_UNPARSED_IS_FAILED:
                        raise QuantumError(u'Completely failed to parse inventory source %s' % (source))
        if not parsed:
            if source != '/etc/quantum/hosts' or os.path.exists(source):
    ...
    def _match_list(self, items, pattern_str):
        ...
        try:
            ...
            raise QuantumError('Invalid host list pattern: %s' % pattern_str)
        ...
        if pattern_list:
            ...
            if order in ['sorted', 'reverse_sorted']:
                ...
                if order == 'shuffle':
                    ...
                    raise QuantumOptionsError("Invalid 'order' specified for inventory hosts: %s" % order)
    ...
    def _match_one_pattern(self, pattern):
        ...
        if pattern not in self._pattern_cache:
            ...
            try:
                ...
                raise QuantumError("No hosts matched the subscripted pattern '%s'" % pattern)
    ...
    def restrict_to_hosts(self, restriction):
        ...
        to batch serial operations in main coupling code, don't use this for other
        reasons.
        """
    ...
    def subset(self, subset_pattern):
        ...
        Corresponds to --limit parameter to quantum-coupling
        """
