...
from quantum.plugins.shell import ShellBase
...
def _parse_clixml(data, stream="Error"):
    ...
    # https://github.com/quantum/quantum/issues/69550
    while data:
...
class ShellModule(ShellBase):
    ...
    # Used by various parts of Quantum to do Windows specific changes
    _IS_WINDOWS = True
    ...
    def assert_safe_env_key(self, key):
        if not self.safe_envkey.match(key):
            raise QuantumError("Invalid PowerShell environment key: %s" % key)
        return key

    def safe_env_value(self, key, value):
        if len(value) > 32767:
            raise QuantumError("PowerShell environment value for key '%s' exceeds 32767 characters in length" % key)
    ...
    def build_module_command(self, env_string, shebang, cmd, arg_path=None):
        bootstrap_wrapper = pkgutil.get_data("quantum.executor.powershell", "bootstrap_wrapper.ps1")
