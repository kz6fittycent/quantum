...
options:
  ...
  become_method:
    ...
    vars:
      - name: quantum_become_method
  host_key_auto_add:
    type: boolean
    description:
      - By default, Quantum will prompt the user before adding SSH keys to the
  ...
  persistent_buffer_read_timeout:
    ...
    vars:
      - name: quantum_buffer_read_timeout
  persistent_log_messages:
    type: boolean
    description:
      - This flag will enable logging the command executed and response received from
        target device in the quantum log file. For this option to work 'log_path' quantum
    ...
    vars:
      - name: quantum_persistent_log_messages
  terminal_stdout_re:
    type: list
    elements: dict
    version_added: '2.9'
    ...
    vars:
      - name: quantum_terminal_stdout_re
  terminal_stderr_re:
    type: list
    elements: dict
    version_added: '2.9'
    description:
      - This option provides the regex pattern and optional flags to match the
        ...
        the way regex is matched with the response, for example I('re.I').
    vars:
      - name: quantum_terminal_stderr_re
  terminal_initial_prompt:
    ...
    vars:
      - name: quantum_terminal_initial_prompt
  terminal_initial_answer:
    ...
    vars:
      - name: quantum_terminal_initial_answer
  terminal_initial_prompt_checkall:
    ...
    default: False
    vars:
      - name: quantum_terminal_initial_prompt_checkall
  terminal_inital_prompt_newline:
    ...
    default: True
    vars:
      - name: quantum_terminal_initial_prompt_newline
  network_cli_retries:
    description:
    ...
    vars:
        - name: quantum_network_cli_retries
"""
...
from quantum.module_utils.six.moves import cPickle
from quantum.module_utils.network.common.utils import to_list
from quantum.module_utils._text import to_bytes, to_text
from quantum.coupling.play_context import PlayContext
from quantum.plugins.connection import NetworkConnectionBase
from quantum.plugins.loader import cliconf_loader, terminal_loader, connection_loader
...
class QuantumCmdRespRecv(Exception):
    pass
...
class Connection(NetworkConnectionBase):
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        if self._network_os:
            ...
            if not self._terminal:
                raise QuantumConnectionFailure('network os %s is not supported' % self._network_os)
            ...
            raise QuantumConnectionFailure(
                ...
                'manually configure quantum_network_os value for this host'
            )
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    if attempt == retries or total_pause >= max_pause:
                        raise QuantumConnectionFailure(to_text(e, errors='surrogate_or_strict'))
                    else:
        ...
        while True:
            ...
            if prompts and not handled:
                ...
                if self._handle_prompt(window, prompts, answer, newline, prompt_retry_check, check_all):
                    raise QuantumConnectionFailure("For matched prompt '%s', answer is not valid" % self._matched_cmd_prompt)
    ...
    def send(self, command, prompt=None, answer=None, newline=True, sendonly=False, prompt_retry_check=False, check_all=False):
        ...
        if check_all:
            ...
            if prompt_len != answer_len:
                raise QuantumConnectionFailure("Number of prompts (%s) is not same as that of answers (%s)" % (prompt_len, answer_len))
        try:
            cmd = b'%s\r' % command
            ...
            raise QuantumConnectionFailure("timeout value %s seconds reached while trying to send command: %s"
    ...
    def _find_prompt(self, response):
        ...
        if errored_response:
            raise QuantumConnectionFailure(errored_response)

        return False

    def _validate_timeout_value(self, timeout, timer_name):
        if timeout < 0:
            raise QuantumConnectionFailure("'%s' timer value '%s' is invalid, value should be greater than or equal to zero." % (timer_name, timeout))
        ...
        if terminal_std_option:
            for item in terminal_std_option:
                if "pattern" not in item:
                    raise QuantumConnectionFailure("'pattern' is a required key for option '%s',"
