...
from quantum.compat import selectors
...
class Connection(ConnectionBase):
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        if 'docker_command' in kwargs:
            ...
            if not self.docker_cmd:
                raise QuantumError("docker command not found in PATH")

        docker_version = self._get_docker_version()
        if docker_version == u'dev':
            display.warning(u'Docker version number is "dev". Will assume latest version.')
        if docker_version != u'dev' and LooseVersion(docker_version) < LooseVersion(u'1.3'):
            raise QuantumError('docker connection type requires docker 1.3 or higher')
    ...
    def _get_docker_version(self):
        ...
        if returncode:
            raise QuantumError('Docker version check (%s) failed: %s' % (to_native(cmd), to_native(err)))
    ...
    def put_file(self, in_path, out_path):
        ...
        with open(to_bytes(in_path, errors='surrogate_or_strict'), 'rb') as in_file:
            ...
            try:
                ...
                raise QuantumError("docker connection requires dd command in the container to put files")
            stdout, stderr = p.communicate()
