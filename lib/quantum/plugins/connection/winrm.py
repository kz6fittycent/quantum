...
DOCUMENTATION = """
    ...
    description:
        ...
        - This plugin allows extra arguments to be passed that are supported by the protocol but not explicitly defined here.
          They should take the form of variables declared with the following pattern `quantum_winrm_<option>`.
    version_added: "2.0"
    requirements:
    ...
    options:
      ...
      scheme:
        ...
        choices: [http, https]
        vars:
          - name: quantum_winrm_scheme
        type: str
      path:
        description: URI path to connect to
        default: '/wsman'
        vars:
          - name: quantum_winrm_path
        type: str
      transport:
        description:
        ...
        type: list
        vars:
          - name: quantum_winrm_transport
      kerberos_command:
        description: kerberos command to use to request a authentication ticket
        default: kinit
        vars:
          - name: quantum_winrm_kinit_cmd
        type: str
      kerberos_mode:
        description:
            - kerberos usage mode.
            - The managed option means Quantum will obtain kerberos ticket.
            - While the manual one means a ticket must already have been obtained by the user.
            - If having issues with Quantum freezing when trying to obtain the
              Kerberos ticket, you can either set this to C(manual) and obtain
              it outside Quantum or install C(pexpect) through pip and try
              again.
        choices: [managed, manual]
        vars:
          - name: quantum_winrm_kinit_mode
        type: str
      connection_timeout:
        description:
            ...
            - The default value is whatever is set in the installed version of
              pywinrm.
        vars:
          - name: quantum_winrm_connection_timeout
        type: int
"""
...
from quantum.errors import QuantumError, QuantumConnectionFailure
...
try:
    ...
    # https://github.com/quantum/quantum/issues/43462
...
class Connection(ConnectionBase):
    ...
    def _build_winrm_kwargs(self):
        ...
        if unsupported_transports:
            raise QuantumError('The installed version of WinRM does not support transport(s) %s' %
        ...
        passed_winrm_args = set([v.replace('quantum_winrm_', '') for v in self.get_option('_extras')])
        unsupported_args = passed_winrm_args.difference(supported_winrm_args)

        # warn for kwargs unsupported by the installed version of pywinrm
        for arg in unsupported_args:
            display.warning("quantum_winrm_{0} unsupported by pywinrm (is an up-to-date version of pywinrm installed?)".format(arg))

        # pass through matching extras, excluding the list we want to treat specially
        for arg in passed_winrm_args.difference(internal_kwarg_mask).intersection(supported_winrm_args):
            self._winrm_kwargs[arg] = self.get_option('_extras')['quantum_winrm_%s' % arg]
    ...
    def _kerb_auth(self, principal, password):
        ...
        kinit_flags = []
        if boolean(self.get_option('_extras').get('quantum_winrm_kerberos_delegation', False)):
        ...
        if HAS_PEXPECT:
            ...
            try:
                ...
                raise QuantumConnectionFailure(err_msg)

            try:
                child.expect(".*:")
                child.sendline(password)
            except OSError as err:
                # child exited before the pass was sent, Quantum will raise
    ...
    def _winrm_connect(self):
        ...
        for transport in self._winrm_transport:
            ...
            try:
                ...
                if re.search(to_text(r'Operation\s+?timed\s+?out'), err_msg, re.I):
                    raise QuantumError('the connection attempt timed out')
        ...
        if errors:
            raise QuantumConnectionFailure(', '.join(map(to_native, errors)))
        else:
            raise QuantumError('No transport found for WinRM connection')
        ...
        try:
            ...
            if stdin_push_failed:
                ...
                try:
                    ...
                    raise QuantumError('winrm send_input failed; \nstdout: %s\nstderr %s'
                                       % (to_native(response.std_out), to_native(stderr)))

            return response
        except requests.exceptions.Timeout as exc:
            raise QuantumConnectionFailure('winrm connection error: %s' % to_native(exc))
        finally:
            if command_id:
    ...
    def _connect(self):
        ...
        if not HAS_WINRM:
            raise QuantumError("winrm or requests is not installed: %s" % to_native(WINRM_IMPORT_ERR))
        elif not HAS_XMLTODICT:
            raise QuantumError("xmltodict is not installed: %s" % to_native(XMLTODICT_IMPORT_ERR))
    ...
    def put_file(self, in_path, out_path):
        ...
        if result.status_code != 0:
            raise QuantumError(to_native(result.std_err))
        ...
        try:
            ...
            if stderr.startswith(b"#< CLIXML"):
                stderr = _parse_clixml(stderr)
            raise QuantumError('winrm put_file failed; \nstdout: %s\nstderr %s' % (to_native(result.std_out), to_native(stderr)))

        remote_sha1 = put_output.get("sha1")
        if not remote_sha1:
            raise QuantumError("Remote sha1 was not returned")

        local_sha1 = secure_hash(in_path)

        if not remote_sha1 == local_sha1:
            raise QuantumError("Remote sha1 hash {0} does not match local hash {1}".format(to_native(remote_sha1), to_native(local_sha1)))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    traceback.print_exc()
                    raise QuantumError('failed to transfer file to "%s"' % to_native(out_path))
        finally:
            if out_file:
                out_file.close()
