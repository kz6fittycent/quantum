...
# Copyright (c) 2015, Toshio Kuratomi <tkuratomi@quantum.com>
...
DOCUMENTATION = """
    ...
    options:
      remote_addr:
        ...
        vars:
            - name: quantum_host
            - name: quantum_jail_host
      remote_user:
        description:
            - User to execute as inside the jail
        vars:
            - name: quantum_user
            - name: quantum_jail_user
"""
...
class Connection(ConnectionBase):
    ...
    # having pipelining=True in their quantum.cfg
    has_pipelining = True
    has_tty = False
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        if os.geteuid() != 0:
            raise QuantumError("jail connection requires running as root")

        self.jls_cmd = self._search_executable('jls')
        self.jexec_cmd = self._search_executable('jexec')

        if self.jail not in self.list_jails():
            raise QuantumError("incorrect jail name %s" % self.jail)

    @staticmethod
    def _search_executable(executable):
        cmd = distutils.spawn.find_executable(executable)
        if not cmd:
            raise QuantumError("%s command not found in PATH" % executable)
        return cmd
        ...
        try:
            with open(to_bytes(in_path, errors='surrogate_or_strict'), 'rb') as in_file:
                ...
                try:
                    ...
                    raise QuantumError("jail connection requires dd command in the jail")
                try:
                    stdout, stderr = p.communicate()
                except Exception:
                    traceback.print_exc()
