...
DOCUMENTATION = """
    ...
    short_description: Allow quantum to piggyback on salt minions
    description:
...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        if not HAVE_SALTSTACK:
            raise errors.QuantumError("saltstack is not installed")
    ...
    def exec_command(self, cmd, sudoable=False, in_data=None):
        ...
        if in_data:
            raise errors.QuantumError("Internal Error: this module does not support optimized module pipelining")

        self._display.vvv("EXEC %s" % (cmd), host=self.host)
        # need to add 'true;' to work around https://github.com/saltstack/salt/issues/28077
        res = self.client.cmd(self.host, 'cmd.exec_code_all', ['bash', 'true;' + cmd])
        if self.host not in res:
            raise errors.QuantumError("Minion %s didn't answer, check if salt-minion is running and the name is correct" % self.host)
