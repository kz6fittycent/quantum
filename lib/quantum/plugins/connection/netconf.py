...
options:
  ...
  network_os:
    description:
      - Configures the device platform network operating system.  This value is
        ...
        configured (or set to C(auto)), then Quantum will attempt to guess the
  ...
  host_key_checking:
    ...
    vars:
      ...
      - name: quantum_netconf_host_key_checking
  ...
  netconf_ssh_config:
    ...
    vars:
      - name: quantum_netconf_ssh_config
        version_added: '2.7'
...
from quantum.module_utils.parsing.convert_bool import BOOLEANS_TRUE, BOOLEANS_FALSE
from quantum.plugins.loader import netconf_loader
from quantum.plugins.connection import NetworkConnectionBase, ensure_connect
...
class Connection(NetworkConnectionBase):
    ...
    def _connect(self):
        if not HAS_NCCLIENT:
            raise QuantumError("%s: %s" % (missing_required_lib("ncclient"), to_native(NCCLIENT_IMP_ERR)))
        ...
        try:
            ...
            raise QuantumConnectionFailure(to_native(exc))
        except ImportError as exc:
            raise QuantumError("connection=netconf is not supported on {0}".format(self._network_os))
