...
from quantum.errors import QuantumError, QuantumFileNotFound, QuantumConnectionFailure
...
DOCUMENTATION = """
    ...
    description:
      ...
      - In case of Windows VMs, set C(quantum_shell_type) to C(powershell).
    ...
    options:
      ...
      validate_certs:
        ...
        vars:
          - name: quantum_vmware_validate_certs
        default: True
        type: bool
      vm_path:
        description:
        ...
        vars:
          - name: quantum_vmware_guest_path
        required: True
      vm_user:
        description:
          - VM username.
        vars:
          - name: quantum_user
          - name: quantum_vmware_tools_user
        required: True
      vm_password:
        description:
        ...
        vars:
          ...
          - name: quantum_vmware_tools_password
        required: True
      exec_command_sleep_interval:
        description:
          - Time in seconds to sleep between execution of command.
        vars:
          - name: quantum_vmware_tools_exec_command_sleep_interval
        default: 0.5
        type: float
      file_chunk_size:
        description:
        ...
        vars:
          - name: quantum_vmware_tools_file_chunk_size
        default: 128
        type: integer
      executable:
        description:
        ...
        vars:
            - name: quantum_executable
            - name: quantum_vmware_tools_executable
"""

EXAMPLES = r'''
# example vars.yml
---
quantum_connection: vmware_tools

quantum_vmware_host: vcenter.example.com
quantum_vmware_user: administrator@vsphere.local
quantum_vmware_password: Secr3tP4ssw0rd!12
quantum_vmware_validate_certs: no  # default is yes

# vCenter Connection VM Path Example
quantum_vmware_guest_path: DATACENTER/vm/FOLDER/{{ inventory_hostname }}
# ESXi Connection VM Path Example
quantum_vmware_guest_path: ha-datacenter/vm/{{ inventory_hostname }}

quantum_vmware_tools_user: root
quantum_vmware_tools_password: MyR00tPassw0rD

# if the target VM guest is Windows set the 'quantum_shell_type' to 'powershell'
quantum_shell_type: powershell


# example coupling_linux.yml
---
...
class Connection(ConnectionBase):
    ...
    def _establish_connection(self):
        ...
        try:
            ...
            raise QuantumError("SSL Error: Certificate verification failed.")
        except (gaierror):
            raise QuantumError("Connection Error: Unable to connect to '%s'." % to_native(connection_kwargs["host"]))
        except vim.fault.InvalidLogin as e:
            raise QuantumError("Connection Login Error: %s" % to_native(e.msg))
    ...
    def _establish_vm(self):
        ...
        if self.vm is None:
            raise QuantumError("Unable to find VM by path '%s'" % to_native(self.get_option("vm_path")))
        ...
        try:
            ...
            raise QuantumError("VM Power State Error: %s" % to_native(e.msg))
        except vim.fault.RestrictedVersion as e:
            raise QuantumError("Restricted Version Error: %s" % to_native(e.msg))
        except vim.fault.GuestOperationsUnavailable as e:
            raise QuantumError("VM Guest Operations (VMware Tools) Error: %s" % to_native(e.msg))
        except vim.fault.InvalidGuestLogin as e:
            raise QuantumError("VM Login Error: %s" % to_native(e.msg))
        except vim.fault.NoPermission as e:
            raise QuantumConnectionFailure("No Permission Error: %s %s" % (to_native(e.msg), to_native(e.privilegeId)))

    def _connect(self):
        if not HAS_REQUESTS:
            raise QuantumError("%s : %s" % (missing_required_lib('requests'), REQUESTS_IMP_ERR))

        if not HAS_PYVMOMI:
            raise QuantumError("%s : %s" % (missing_required_lib('PyVmomi'), PYVMOMI_IMP_ERR))
    ...
    def create_temporary_file_in_guest(self, prefix="", suffix=""):
        ...
        try:
            ...
            raise QuantumError("No Permission Error: %s %s" % (to_native(e.msg), to_native(e.privilegeId)))
    ...
    def _fetch_file_from_vm(self, guestFilePath):
        ...
        if response.status_code != 200:
            raise QuantumError("Failed to fetch file")
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        try:
            ...
            raise QuantumError("StartProgramInGuest Error: %s" % to_native(e.msg))
    ...
    def put_file(self, in_path, out_path):
        ...
        if response.status_code != 200:
            raise QuantumError("File transfer failed")
