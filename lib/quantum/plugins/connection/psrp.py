...
options:
  ...
  remote_user:
    ...
    vars:
    - name: quantum_user
    - name: quantum_psrp_user
  remote_password:
    description: Authentication password for the C(remote_user). Can be supplied as CLI option.
    type: str
    vars:
    - name: quantum_password
    - name: quantum_winrm_pass
    - name: quantum_winrm_password
    aliases:
  ...
  protocol:
    ...
    type: str
    vars:
    - name: quantum_psrp_protocol
  path:
    description:
    - The URI path to connect to.
    type: str
    vars:
    - name: quantum_psrp_path
    default: 'wsman'
  auth:
    description:
  ...
  cert_validation:
    ...
    type: str
    vars:
    - name: quantum_psrp_cert_validation
  ca_cert:
    description:
    ...
    type: path
    vars:
    - name: quantum_psrp_cert_trust_path
    - name: quantum_psrp_ca_cert
  ...
  connection_timeout:
    ...
    type: int
    vars:
    - name: quantum_psrp_connection_timeout
    default: 30
  read_timeout:
    description:
    ...
    type: int
    vars:
    - name: quantum_psrp_read_timeout
    default: 30
    version_added: '2.8'
  reconnection_retries:
    ...
    type: int
    vars:
    - name: quantum_psrp_reconnection_retries
    default: 0
    version_added: '2.8'
  reconnection_backoff:
    ...
    - This is measured in seconds.
    - The C(quantum_psrp_reconnection_backoff) variable was added in Quantum
      2.9.
    type: int
    vars:
    - name: quantum_psrp_connection_backoff
    - name: quantum_psrp_reconnection_backoff
    default: 2
    version_added: '2.8'
  message_encryption:
    description:
    - Controls the message encryption settings, this is different from TLS
      encryption when I(quantum_psrp_protocol) is C(https).
    ...
    type: str
    vars:
    - name: quantum_psrp_message_encryption
    choices:
    - auto
    - always
    - never
  ...
  ignore_proxy:
    ...
    vars:
    - name: quantum_psrp_ignore_proxy
    type: bool
    default: 'no'
  ...
  certificate_key_pem:
    ...
    type: path
    vars:
    - name: quantum_psrp_certificate_key_pem
  certificate_pem:
    description:
    - The local path to an X509 certificate to use with certificate auth.
    type: path
    vars:
    - name: quantum_psrp_certificate_pem
  credssp_auth_mechanism:
    ...
    default: auto
    vars:
    - name: quantum_psrp_credssp_auth_mechanism
  credssp_disable_tlsv1_2:
    ...
    type: bool
    vars:
    - name: quantum_psrp_credssp_disable_tlsv1_2
  credssp_minimum_version:
    ...
    type: int
    vars:
    - name: quantum_psrp_credssp_minimum_version
  negotiate_delegate:
    description:
    ...
    type: bool
    vars:
    - name: quantum_psrp_negotiate_delegate
  negotiate_hostname_override:
    description:
    - Override the remote hostname when searching for the host in the Kerberos
      lookup.
    - This allows Quantum to connect over IP but authenticate with the remote
    ...
    type: str
    vars:
    - name: quantum_psrp_negotiate_hostname_override
  negotiate_send_cbt:
    description:
    ...
    type: bool
    vars:
    - name: quantum_psrp_negotiate_send_cbt
  negotiate_service:
    description:
    ...
    type: str
    vars:
    - name: quantum_psrp_negotiate_service
  ...
  operation_timeout:
    ...
    type: int
    vars:
    - name: quantum_psrp_operation_timeout
    default: 20
  max_envelope_size:
    description:
    ...
    type: int
    vars:
    - name: quantum_psrp_max_envelope_size
    default: 153600
  configuration_name:
    description:
    ...
    type: str
    vars:
    - name: quantum_psrp_configuration_name
...
from quantum.errors import QuantumConnectionFailure, QuantumError
from quantum.errors import QuantumFileNotFound
...
from quantum.plugins.shell.powershell import _common_args
from quantum.utils.display import Display
from quantum.utils.hashing import sha1
...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        if not HAS_PYPSRP:
            raise QuantumError("pypsrp or dependencies are not installed: %s"
        ...
        if not self.runspace:
            ...
            self.host = PSHost(None, None, False, "Quantum PSRP Host", None,
                               host_ui, None)
            ...
            try:
                ...
                raise QuantumConnectionFailure("failed to authenticate with "
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        if cmd.startswith(" ".join(_common_args) + " -EncodedCommand"):
            ...
            if in_data and in_data.startswith(u"#!"):
                ...
                raise QuantumError("cannot run the interpreter '%s' on the psrp "
    ...
    def put_file(self, in_path, out_path):
        ...
        if rc != 0:
            raise QuantumError(to_native(stderr))

        put_output = json.loads(to_text(stdout))
        remote_sha1 = put_output.get("sha1")

        if not remote_sha1:
            raise QuantumError("Remote sha1 was not returned, stdout: '%s', stderr: '%s'"
                               % (to_native(stdout), to_native(stderr)))

        if not remote_sha1 == local_sha1:
            raise QuantumError("Remote sha1 hash %s does not match local hash %s"
    ...
    Write-Output -InputObject "{`"sha1`":`"$hash`"}"
        ...
        if not os.path.exists(b_in_path):
            raise QuantumFileNotFound('file or module does not exist: "%s"'
                                      % to_native(in_path))
...
'''
        ...
        b_in_path = to_bytes(in_path, errors='surrogate_or_strict')
        if not os.path.exists(b_in_path):
            raise QuantumFileNotFound('file or module does not exist: "%s"' % to_native(in_path))
...
if ($bytes_read -gt 0) {
    ...
    Write-Output -InputObject ([System.Convert]::ToBase64String($bytes))
        ...
        rc, stdout, stderr = self._exec_psrp_script(setup_script,
                                                    ...
                                                    force_stop=True)
        if rc != 0:
            raise QuantumError("failed to setup file stream for fetch '%s': %s"
    ...
    def _build_kwargs(self):
        ...
        extra_args = set([v.replace('quantum_psrp_', '') for v in
                          self.get_option('_extras')])
        unsupported_args = extra_args.difference(supported_args)

        for arg in unsupported_args:
            display.warning("quantum_psrp_%s is unsupported by the current "
        ...
        if hasattr(pypsrp, 'FEATURES') and 'wsman_read_timeout' in pypsrp.FEATURES:
            ...
            display.warning("quantum_psrp_read_timeout is unsupported by the current psrp version installed, "
                            "using quantum_psrp_connection_timeout value for read_timeout instead.")
        ...
        if hasattr(pypsrp, 'FEATURES') and 'wsman_reconnections' in pypsrp.FEATURES:
            ...
            if self._psrp_reconnection_retries is not None:
                display.warning("quantum_psrp_reconnection_retries is unsupported by the current psrp version installed.")
            if self._psrp_reconnection_backoff is not None:
                display.warning("quantum_psrp_reconnection_backoff is unsupported by the current psrp version installed.")

        # add in the extra args that were set
        for arg in extra_args.intersection(supported_args):
            option = self.get_option('_extras')['quantum_psrp_%s' % arg]
    ...
    def _parse_pipeline_result(self, pipeline):
        ...
        output into the format that Quantum understands.
