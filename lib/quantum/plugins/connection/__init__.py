...
from quantum.plugins.loader import connection_loader, get_shell_plugin
...
class ConnectionBase(QuantumPlugin):
    '''
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        :ConnectionCommand: This is the command that connects us to the remote
            machine to run the rest of the command.  ``quantum_user``,
            ``quantum_ssh_host`` and so forth are fed to this piece of the
        ...
        :UsersLoginShell: This shell may or may not be created depending on
            ...
            shell that the ``quantum_user`` has configured as their login
            ...
            connects to a remote machine.  ``quantum_shell_type`` may be set
            to inform quantum of differences in how the ``UsersLoginShell``
        ...
        :ANSIBLE_SHELL_EXECUTABLE: This is the shell set via the inventory var
            ``quantum_shell_executable`` or via
        ...
        :Command: Is the command we're actually trying to run remotely.
            (Examples: mkdir -p $HOME/.quantum, python $HOME/.quantum/tmp-script-file)
        """
        pass
...
class NetworkConnectionBase(ConnectionBase):
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        self._quantum_coupling_pid = kwargs.get('quantum_coupling_pid')
