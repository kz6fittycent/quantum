...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        ...
        if self.container.state == "STOPPED":
            raise errors.QuantumError("%s is not running" % self.container_name)
    ...
    def put_file(self, in_path, out_path):
        ...
        if not os.path.exists(in_path):
            ...
            raise errors.QuantumFileNotFound(msg)
        try:
            src_file = open(in_path, "rb")
        except IOError:
            traceback.print_exc()
            raise errors.QuantumError("failed to open input file to %s" % in_path)
        try:
            def write_file(args):
