...
description:
  - This connection plugin provides connectivity to network devices using
    ...
    C(napalm) to be installed locally on the Quantum controller.
version_added: "2.8"
requirements:
  - napalm
options:
...
class Connection(NetworkConnectionBase):
    ...
    def _connect(self):
        if not HAS_NAPALM:
            raise QuantumError('The "napalm" python library is required to use the napalm connection type.\n')
        ...
        if not self.connected:
            ...
            try:
                ...
                raise QuantumConnectionFailure('Failed to import napalm driver for {0}'.format(self._network_os))
