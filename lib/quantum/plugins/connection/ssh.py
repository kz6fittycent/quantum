...
# Copyright 2017 Toshio Kuratomi <tkuratomi@quantum.com>
...
DOCUMENTATION = '''
    ...
    description:
        - This connection plugin allows quantum to communicate to the target machines via normal ssh command line.
        - Quantum does not expose a channel to allow communication between the user and the ssh process to accept
    ...
    options:
      ...
      host_key_checking:
          ...
          env:
              ...
              - name: ANSIBLE_SSH_HOST_KEY_CHECKING
                version_added: '2.5'
          vars:
              - name: quantum_host_key_checking
                version_added: '2.5'
              - name: quantum_ssh_host_key_checking
                version_added: '2.5'
      password:
          description: Authentication password for the C(remote_user). Can be supplied as CLI option.
          vars:
              - name: quantum_password
              - name: quantum_ssh_pass
              - name: quantum_ssh_password
      ssh_args:
      ...
      scp_executable:
          ...
          version_added: "2.6"
          vars:
              - name: quantum_scp_executable
                version_added: '2.7'
      scp_extra_args:
          description: Extra exclusive to the ``scp`` CLI
          vars:
              - name: quantum_scp_extra_args
          env:
      ...
      port:
          ...
          vars:
            - name: quantum_port
            - name: quantum_ssh_port
      remote_user:
          description:
              - User name with which to login to the remote server, normally set by the remote_user keyword.
              - If no user is supplied, Quantum will let the ssh client binary choose the user as it normally
          ini:
            - section: defaults
              key: remote_user
          env:
          ...
          vars:
            - name: quantum_user
            - name: quantum_ssh_user
      pipelining:
          default: ANSIBLE_PIPELINING
          description:
            - Pipelining reduces the number of SSH operations required to execute a module on the remote server,
              by executing many Quantum modules without actual file transfer.
          ...
          type: boolean
          vars:
            - name: quantum_pipelining
            - name: quantum_ssh_pipelining
      private_key_file:
          description:
          ...
          vars:
            - name: quantum_private_key_file
            - name: quantum_ssh_private_key_file

      control_path:
        description:
          - This is the location to save ssh's ControlPath sockets, it uses ssh's variable substitution.
          - Since 2.3, if null, quantum will generate a unique hash. Use `%(directory)s` to indicate where to use the control dir path setting.
        env:
      ...
      sftp_batch_mode:
        ...
        type: bool
        vars:
          - name: quantum_sftp_batch_mode
            version_added: '2.7'
      scp_if_ssh:
        default: smart
        description:
          - "Prefered method to use when transfering files over ssh"
          - When set to smart, Quantum will try them until one succeeds or they all fail
          - If set to True, it will force 'scp', if False it will use 'sftp'
        env: [{name: ANSIBLE_SCP_IF_SSH}]
        ini:
        - {key: scp_if_ssh, section: ssh_connection}
        vars:
          - name: quantum_scp_if_ssh
            version_added: '2.7'
      use_tty:
        version_added: '2.5'
        default: 'yes'
...
from quantum.errors import (
    QuantumAuthenticationFailure,
    QuantumConnectionFailure,
    QuantumError,
    QuantumFileNotFound,
)
from quantum.errors import QuantumOptionsError
from quantum.module_utils.compat import selectors
from quantum.module_utils.six import PY3, text_type, binary_type
from quantum.module_utils.six.moves import shlex_quote
from quantum.module_utils._text import to_bytes, to_native, to_text
from quantum.module_utils.parsing.convert_bool import BOOLEANS, boolean
from quantum.plugins.connection import ConnectionBase, BUFSIZE
from quantum.plugins.shell.powershell import _parse_clixml
from quantum.utils.display import Display
from quantum.utils.path import unfrackpath, makedirs_safe
...
class QuantumControlPersistBrokenPipeError(QuantumError):
...
def _handle_error(remaining_retries, command, return_tuple, no_log, host, display=display):
    ...
    if command == b'sshpass':
        ...
        if return_tuple[0] == 5:
            ...
            raise QuantumAuthenticationFailure(msg)
    ...
    if return_tuple[0] == 255:
        ...
        if SSH_ERROR:
            ...
            if no_log:
                ...
                msg = '{0} {1}'.format(msg, to_native(return_tuple[2]).rstrip())
            raise QuantumConnectionFailure(msg)
    ...
    def wrapped(self, *args, **kwargs):
        ...
        for attempt in range(remaining_tries):
            ...
            try:
                ...
                except (QuantumControlPersistBrokenPipeError):
            ...
            # 5 = Invalid/incorrect password from sshpass
            except QuantumAuthenticationFailure:
                # Raising this exception, which is subclassed from QuantumConnectionFailure, prevents further retries
                raise

            except (QuantumConnectionFailure, Exception) as e:
                ...
                if attempt == remaining_tries - 1:
                    ...
                    if isinstance(e, QuantumConnectionFailure):
...
class Connection(ConnectionBase):
    ...
    def _build_command(self, binary, *other_args):
        ...
        if self._play_context.password:
            if not self._sshpass_available():
                raise QuantumError("to use the 'ssh' connection type with passwords, you must install the sshpass program")
        ...
        #
        # Next, we add [ssh_connection]ssh_args from quantum.cfg.
        #

        if self._play_context.ssh_args:
            b_args = [to_bytes(a, errors='surrogate_or_strict') for a in
                      self._split_ssh_args(self._play_context.ssh_args)]
            self._add_args(b_command, b_args, u"quantum.cfg set ssh_args")

        # Now we add various arguments controlled by configuration file settings
        # (e.g. host_key_checking) or inventory variables (quantum_ssh_port) or
        ...
        if self._play_context.port is not None:
            ...
            self._add_args(b_command, b_args, u"ANSIBLE_REMOTE_PORT/remote_port/quantum_port set")

        key = self._play_context.private_key_file
        if key:
            b_args = (b"-o", b'IdentityFile="' + to_bytes(os.path.expanduser(key), errors='surrogate_or_strict') + b'"')
            self._add_args(b_command, b_args, u"ANSIBLE_PRIVATE_KEY_FILE/private_key_file/quantum_ssh_private_key_file set")
        ...
        if not self._play_context.password:
            self._add_args(
                ...
                u"quantum_password/quantum_ssh_password not set"
            )
        ...
        if user:
            self._add_args(
                ...
                u"ANSIBLE_REMOTE_USER/remote_user/quantum_user/user/-u set"
            )
        ...
        if controlpersist:
            ...
            if not controlpath:
                ...
                if not os.access(b_cpdir, os.W_OK):
                    raise QuantumError("Cannot write to ControlPath %s" % to_native(cpdir))
    ...
    def _send_initial_data(self, fh, in_data, ssh_process):
        ...
        try:
            ...
            # Only raise QuantumConnectionFailure if the ssh process is still alive
            time.sleep(0.001)
            ssh_process.poll()
            if getattr(ssh_process, 'returncode', None) is None:
                raise QuantumConnectionFailure(
    ...
    def _bare_run(self, cmd, in_data, sudoable=True, checkrc=True):
            ...
            while True:
                ...
                if not events:
                    ...
                    if state <= states.index('awaiting_escalation'):
                        ...
                        raise QuantumError('Timeout (%ds) waiting for privilege escalation prompt: %s' % (timeout, to_native(b_stdout)))
                ...
                if states[state] == 'awaiting_escalation':
                    if self._flags['become_success']:
                        ...
                        raise QuantumError('Incorrect %s password' % self.become.name)
                    elif self._flags['become_nopasswd_error']:
                        display.vvv(u'Escalation requires password')
                        self._terminate_process(p)
                        self._flags['become_nopasswd_error'] = False
                        raise QuantumError('Missing %s password' % self.become.name)
        ...
        if C.HOST_KEY_CHECKING:
            if cmd[0] == b"sshpass" and p.returncode == 6:
                raise QuantumError('Using a SSH password instead of a key is not possible because Host Key checking is enabled and sshpass does not support '
                                   'this.  Please add this host\'s fingerprint to your known_hosts file to manage this host.')

        controlpersisterror = b'Bad configuration option: ControlPersist' in b_stderr or b'unknown configuration option: ControlPersist' in b_stderr
        if p.returncode != 0 and controlpersisterror:
            raise QuantumError('using -c ssh on certain older ssh versions may not support ControlPersist, set ANSIBLE_SSH_ARGS="" '
        ...
        if p.returncode == 255:
            ...
            if controlpersist_broken_pipe:
                raise QuantumControlPersistBrokenPipeError('Data could not be sent because of ControlPersist broken pipe: %s' % additional)

            elif in_data and checkrc:
                raise QuantumConnectionFailure('Data could not be sent to remote host "%s". Make sure this host can be reached over ssh: %s'
    ...
    def _file_transport_command(self, in_path, out_path, sftp_action):
        ...
        if ssh_transfer_method is not None:
            if not (ssh_transfer_method in ('smart', 'sftp', 'scp', 'piped')):
                raise QuantumOptionsError('transfer_method needs to be one of [smart|sftp|scp|piped]')
            ...
            if not isinstance(scp_if_ssh, bool):
                ...
                if scp_if_ssh in BOOLEANS:
                    ...
                    raise QuantumOptionsError('scp_if_ssh needs to be one of [smart|True|False]')
            if scp_if_ssh == 'smart':
                methods = smart_methods
        ...
        if returncode == 255:
            raise QuantumConnectionFailure("Failed to connect to the host via %s: %s" % (method, to_native(stderr)))
        else:
            raise QuantumError("failed to transfer file to %s %s:\n%s\n%s" %
    ...
    def put_file(self, in_path, out_path):
        ...
        if not os.path.exists(to_bytes(in_path, errors='surrogate_or_strict')):
            raise QuantumFileNotFound("file or module does not exist: {0}".format(to_native(in_path)))
