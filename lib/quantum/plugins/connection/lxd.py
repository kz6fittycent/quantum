...
class Connection(ConnectionBase):
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        if not self._lxc_cmd:
            raise QuantumError("lxc command not found in PATH")
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        if stderr == "error: Container is not running.\n":
            raise QuantumConnectionFailure("container not running: %s" % self._host)

        if stderr == "error: not found\n":
            raise QuantumConnectionFailure("container not found: %s" % self._host)
    ...
    def put_file(self, in_path, out_path):
        ...
        if not os.path.isfile(to_bytes(in_path, errors='surrogate_or_strict')):
            raise QuantumFileNotFound("input path is not a file: %s" % in_path)
