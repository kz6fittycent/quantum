...
DOCUMENTATION = """
    ...
    description:
        ...
        - This is needed on the Quantum control machine to be reasonably efficient with connections.
    ...
    options:
      ...
      password:
        ...
        vars:
            ...
            - name: quantum_paramiko_pass
            - name: quantum_paramiko_password
              version_added: '2.5'
      host_key_auto_add:
      ...
      record_host_keys:
        ...
        type: boolean
      host_key_checking:
        description: 'Set this to "False" if you want to avoid host key checking by the underlying tools Quantum uses to connect to the host'
        type: boolean
        default: True
        env:
        ...
        vars:
          ...
          - name: quantum_ssh_host_key_checking
            version_added: '2.5'
          - name: quantum_paramiko_host_key_checking
            version_added: '2.5'
...
from quantum.module_utils.compat.paramiko import PARAMIKO_IMPORT_ERR, paramiko
from quantum.module_utils.six import iteritems
from quantum.module_utils.six.moves import input
    ...
    def missing_host_key(self, client, hostname, key):
        ...
        if all((self._options['host_key_checking'], not self._options['host_key_auto_add'])):
            ...
            if self.connection.get_option('use_persistent_connections') or self.connection.force_persistence:
                ...
                raise QuantumError(AUTHENTICITY_MSG[1:92] % (hostname, ktype, fingerprint))
            ...
            if inp not in ['yes', 'y', '']:
                raise QuantumError("host connection rejected by user")

        key._added_by_quantum_this_time = True
...
class Connection(ConnectionBase):
    ...
    def _parse_proxy_command(self, port=22):
        proxy_command = None
        # Parse quantum_ssh_common_args, specifically looking for ProxyCommand
        ssh_args = [
    ...
    def _connect_uncached(self):
        ...
        if paramiko is None:
            raise QuantumError("paramiko is not installed: %s" % to_native(PARAMIKO_IMPORT_ERR))
        ...
        try:
            ...
            raise QuantumConnectionFailure('host key mismatch for %s' % e.hostname)
            ...
            if u"PID check failed" in msg:
                raise QuantumError("paramiko version issue, please upgrade paramiko on the machine running quantum")
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        if in_data:
            raise QuantumError("Internal Error: this module does not support optimized module pipelining")
        ...
        try:
            ...
            if text_e:
                msg += u": %s" % text_e
            raise QuantumConnectionFailure(to_native(msg))

        # sudo usually requires a PTY (cf. requiretty option), therefore
        # we give it one by default (pty=True in quantum.cfg), and we try
            ...
            if self.become and self.become.expect_prompt():
                ...
                while not (become_sucess or passprompt):
                    ...
                    if not chunk:
                        if b'unknown user' in become_output:
                            ...
                            raise QuantumError('user %s does not exist' % n_become_user)
                        else:
                            break
                            # raise QuantumError('ssh connection closed waiting for password prompt')
                    become_output += chunk
                ...
                if passprompt:
                    if self.become:
                        ...
                        raise QuantumError("A password is required but none was supplied")
                else:
                    no_prompt_out += become_output
                    no_prompt_err += become_output
        except socket.timeout:
            raise QuantumError('ssh timed out waiting for privilege escalation.\n' + become_output)
    ...
    def put_file(self, in_path, out_path):
        ...
        if not os.path.exists(to_bytes(in_path, errors='surrogate_or_strict')):
            raise QuantumFileNotFound("file or module does not exist: %s" % in_path)

        try:
            self.sftp = self.ssh.open_sftp()
        except Exception as e:
            raise QuantumError("failed to open a SFTP connection (%s)" % e)

        try:
            self.sftp.put(to_bytes(in_path, errors='surrogate_or_strict'), to_bytes(out_path, errors='surrogate_or_strict'))
        except IOError:
            raise QuantumError("failed to transfer file to %s" % out_path)
    ...
    def fetch_file(self, in_path, out_path):
        ...
        try:
            ...
            raise QuantumError("failed to open a SFTP connection (%s)" % to_native(e))

        try:
            self.sftp.get(to_bytes(in_path, errors='surrogate_or_strict'), to_bytes(out_path, errors='surrogate_or_strict'))
        except IOError:
            raise QuantumError("failed to transfer file from %s" % in_path)

    def _any_keys_added(self):

        for hostname, keys in iteritems(self.ssh._host_keys):
            for keytype, key in iteritems(keys):
                added_this_time = getattr(key, '_added_by_quantum_this_time', False)
                if added_this_time:
                    return True
        return False
