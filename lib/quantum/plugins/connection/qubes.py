...
from quantum.plugins.connection import ConnectionBase, ensure_connect
...
class Connection(ConnectionBase):
    ...
    def put_file(self, in_path, out_path):
        ...
        if retcode != 0:
            raise QuantumConnectionFailure('Failed to put_file to {0}'.format(out_path))
    ...
    def fetch_file(self, in_path, out_path):
        ...
        with open(out_path, "wb") as fobj:
            ...
            p.communicate()
            if p.returncode != 0:
                raise QuantumConnectionFailure('Failed to fetch file to {0}'.format(out_path))
