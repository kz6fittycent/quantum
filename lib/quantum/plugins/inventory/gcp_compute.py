...
compose:
  ...
  quantum_host: networkInterfaces[0].accessConfigs[0].natIP
"""
...
from quantum.module_utils.gcp_utils import (
    GcpSession,
    navigate_hash,
    GcpRequestException,
    ...
    def hostname(self):
        ...
        for order in self.hostname_ordering:
            ...
            if order == "public_ip":
                ...
                raise QuantumParserError("%s is not a valid hostname precedent" % order)

            if name:
                return name

        raise QuantumParserError("No valid name found for host")
