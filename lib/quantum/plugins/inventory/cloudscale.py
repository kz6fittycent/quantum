...
options:
    ...
    quantum_host:
        description: |
            Which IP address to register as the quantum_host. If the
            requested value does not exist or this is set to 'none', no
            quantum_host will be set.
        type: str
        choices:
            - public_v4
            - public_v6
...
# Example command line: quantum-inventory --list -i inventory_cloudscale.yml
...
from quantum.module_utils.cloudscale import API_URL
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not self._token:
            raise QuantumError('Could not find an API token. Set the '
        ...
        if inventory_hostname not in ('name', 'uuid'):
            raise QuantumError('Invalid value for option inventory_hostname: %s'
                               % inventory_hostname)

        quantum_host = self.get_option('quantum_host')
        if quantum_host not in iface_type_map:
            raise QuantumError('Invalid value for option quantum_host: %s'
                               % quantum_host)
        ...
        for name, servers in firstpass.items():
            ...
            # Set variables
            iface_type, iface_version = iface_type_map[quantum_host]
            for server in servers:
                hostname = server.pop('inventory_hostname')
                if quantum_host != 'none':
