...
DOCUMENTATION = r"""
    ...
    short_description: Quantum dynamic inventory plugin for the Hetzner Cloud.
    version_added: "2.8"
    requirements:
        - python >= 2.7
        - hcloud-python >= 1.0.0
...
try:
    ...
    raise QuantumError("The Hetzner Cloud dynamic inventory plugin requires hcloud-python.")
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def _configure_hcloud_client(self):
        ...
        self.client = hcloud.Client(token=self.api_token,
                                    ...
                                    application_name="quantum-inventory",
    ...
    def _test_hcloud_token(self):
        try:
            ...
            raise QuantumError("Invalid Hetzner Cloud API Token.")
    ...
    def _set_server_attributes(self, server):
        ...
        if self.get_option("connect_with") == "public_ipv4":
            self.inventory.set_variable(server.name, "quantum_host", to_native(server.public_net.ipv4.ip))
        elif self.get_option("connect_with") == "hostname":
            self.inventory.set_variable(server.name, "quantum_host", to_native(server.name))
        elif self.get_option("connect_with") == "ipv4_dns_ptr":
            self.inventory.set_variable(server.name, "quantum_host", to_native(server.public_net.ipv4.dns_ptr))
