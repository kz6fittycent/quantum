...
DOCUMENTATION = r'''
    ...
    description:
        ...
        - Uses I(api_config), I(~/.vultr.ini), I(./vultr.ini) or C(VULTR_API_CONFIG) pointing to a Vultr credentials INI file
          (see U(https://docs.quantum.com/quantum/latest/scenario_guides/guide_vultr.html)).
    options:
        plugin:
...
# Example command line: quantum-inventory --list -i inventory_vultr.yml
...
from quantum.module_utils.vultr import Vultr, VULTR_API_ENDPOINT, VULTR_USER_AGENT
...
def _retrieve_servers(api_key, tag_filter=None):
    ...
    try:
        ...
        raise QuantumError("Incorrect JSON payload")
    except Exception as e:
        raise QuantumError("Error while fetching %s: %s" % (api_url, to_native(e)))
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        try:
            ...
            raise QuantumError('Could not find an API key. Check inventory file and Vultr configuration files.')
        ...
        for server in _retrieve_servers(api_key, filter_by_tag):
            ...
            if hostname_preference != 'name':
                self.inventory.set_variable(server['name'], 'quantum_host', server[hostname_preference])
