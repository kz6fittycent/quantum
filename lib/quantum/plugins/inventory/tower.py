...
DOCUMENTATION = '''
    ...
    short_description: Quantum dynamic inventory plugin for Quantum Tower.
    version_added: "2.7"
    description:
        - Reads inventories from Quantum Tower.
    ...
    options:
        plugin:
            ...
            choices: ['tower']
        host:
            description: The network address of your Quantum Tower host.
            type: string
            env:
                - name: TOWER_HOST
            required: True
        username:
            description: The user that you plan to use to access inventories on Quantum Tower.
            type: string
            env:
                - name: TOWER_USERNAME
            required: True
        password:
            description: The password for your Quantum Tower user.
            type: string
            env:
                - name: TOWER_PASSWORD
            required: True
        inventory_id:
            description:
                - The ID of the Quantum Tower inventory that you wish to import.
            ...
            required: True
        validate_certs:
            description: Specify whether Quantum should verify the SSL certificate of Quantum Tower host.
            type: bool
            default: True
            env:
            ...
            aliases: [ verify_ssl ]
        include_metadata:
            description: Make extra requests to provide all group vars with metadata about the source Quantum Tower host.
            type: bool
            default: False
            version_added: "2.8"
'''
...
plugin: tower
host: your_quantum_tower_server_network_address
username: your_quantum_tower_username
password: your_quantum_tower_password
inventory_id: the_ID_of_targeted_quantum_tower_inventory
# Then you can run the following command.
# If some of the arguments are missing, Quantum will attempt to read them from environment variables.
# quantum-inventory -i /path/to/tower_inventory.yml --list
...
# Read the inventory specified in TOWER_INVENTORY from Quantum Tower, and list them.
# The inventory path must always be @tower_inventory if you are reading all settings from environment variables.
# quantum-inventory -i @tower_inventory --list
'''
...
from quantum.module_utils.urls import Request, urllib_error, ConnectionError, socket, httplib
from quantum.module_utils._text import to_text, to_native
from quantum.errors import QuantumParserError, QuantumOptionsError
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def make_request(self, request_handler, tower_url):
        ...
        try:
            ...
            raise QuantumParserError(n_error_msg)
        ...
        try:
            ...
            raise QuantumParserError('Failed to parse json from host: {err}'.format(err=to_native(e)))
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if self.get_option('include_metadata'):
            ...
            for key in ('version', 'quantum_version'):
