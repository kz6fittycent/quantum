...
DOCUMENTATION = '''
    ...
    options:
        ...
        use_contrib_script_compatible_sanitization:
          description:
            - By default this plugin is using a general group name sanitization to create safe and usable group names for use in Quantum.
...
# Note: I(hostnames) sets the inventory_hostname. To modify quantum_host without modifying
...
# Example using constructed features to create groups and set quantum_host
plugin: aws_ec2
regions:
  - us-east-1
  - us-west-1
...
from quantum.module_utils.ec2 import quantum_dict_to_boto3_filter_list, boto3_tag_list_to_quantum_dict
from quantum.module_utils.ec2 import camel_dict_to_snake_dict
...
try:
    ...
    raise QuantumError('The ec2 dynamic inventory plugin requires boto3 and botocore.')
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _get_boto_attr_chain(self, filter_name, instance):
        ...
        if filter_name not in allowed_filters:
            raise QuantumError("Invalid filter '%s' provided; filter must be one of %s." % (filter_name,
                                                                                            allowed_filters))
        ...
        try:
            ...
            if self.boto_profile:
                try:
                    ...
                    raise QuantumError("Insufficient credentials found: %s" % to_native(e))
            else:
                raise QuantumError("Insufficient credentials found: %s" % to_native(e))
        return connection
    ...
    def _boto3_assume_role(self, credentials, region):
        ...
        try:
            ...
            sts_session = sts_connection.assume_role(RoleArn=iam_role_arn, RoleSessionName='quantum_aws_ec2_dynamic_inventory')
            return dict(
            ...
            raise QuantumError("Unable to assume IAM role: %s" % to_native(e))
    ...
    def _boto3_conn(self, regions):
        ...
        if not regions:
            raise QuantumError('Unable to get regions list from available methods, you must specify the "regions" option to continue.')
        ...
        for connection, region in self._boto3_conn(regions):
            try:
                ...
                if e.response['ResponseMetadata']['HTTPStatusCode'] == 403 and not strict_permissions:
                    ...
                    raise QuantumError("Failed to describe instances: %s" % to_native(e))
    ...
    def _get_event_set_and_persistence(self, connection, instance_id, spot_instance):
        ...
        try:
            ...
            if not self.get_option('strict_permissions'):
                ...
                raise QuantumError("Failed to describe instance status: %s" % to_native(e))
        if spot_instance:
            try:
                ...
                if not self.get_option('strict_permissions'):
                    ...
                    raise QuantumError("Failed to describe spot instance requests: %s" % to_native(e))
        return host_vars
    ...
    def _get_tag_hostname(self, preference, instance):
        ...
        tags = boto3_tag_list_to_quantum_dict(instance.get('Tags', []))
        ...
        for preference in hostnames:
            if 'tag' in preference:
                if not preference.startswith('tag:'):
                    raise QuantumError("To name a host by tags name_value, use 'tag:name=value'.")
    ...
    def _add_hosts(self, hosts, group, hostnames):
        ...
        for host in hosts:
            ...
            host['tags'] = boto3_tag_list_to_quantum_dict(host.get('tags', []))
    ...
    def _set_credentials(self):
        ...
        if not self.boto_profile and not (self.aws_access_key_id and self.aws_secret_access_key):
            raise QuantumError("Insufficient boto credentials found. Please provide them in your "
                               "inventory configuration file or set them as environment variables.")

    def verify_file(self, path):
        '''
            :param loader: an quantum.parsing.dataloader.DataLoader object
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        filters = quantum_dict_to_boto3_filter_list(self.get_option('filters'))
