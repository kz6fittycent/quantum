...
DOCUMENTATION = '''
    ...
    description:
        - The source provided must be an executable that returns Quantum inventory JSON
...
from quantum.module_utils.basic import json_dict_bytes_to_unicode
...
class InventoryModule(BaseInventoryPlugin, Cacheable):
    ''' Host inventory parser for quantum using external inventory scripts. '''
        ...
        try:
            ...
            if not cache or cache_key not in self._cache:
                try:
                    ...
                    raise QuantumParserError("problem running %s (%s)" % (' '.join(cmd), to_native(e)))
                ...
                if sp.returncode != 0:
                    raise QuantumError("Inventory script (%s) had an execution error: %s " % (path, err))
                ...
                try:
                    ...
                    raise QuantumError("Inventory {0} contained characters that cannot be interpreted as UTF-8: {1}".format(path, to_native(e)))

                try:
                    self._cache[cache_key] = self.loader.load(data, json_only=True)
                except Exception as e:
                    raise QuantumError("failed to parse executable inventory script results from {0}: {1}\n{2}".format(path, to_native(e), err))
            ...
            if not isinstance(processed, Mapping):
                raise QuantumError("failed to parse executable inventory script results from {0}: needs to be a json dict\n{1}".format(path, err))
            ...
            for host in self._hosts:
                ...
                if data_from_meta is None:
                    ...
                    try:
                        ...
                        raise QuantumError("Improperly formatted host information for %s: %s" % (host, to_native(e)), orig_exc=e)
    ...
    def _parse_group(self, group, data):
        ...
        if 'hosts' in data:
            if not isinstance(data['hosts'], list):
                raise QuantumError("You defined a group '%s' with bad data for the host list:\n %s" % (group, data))
        ...
        if 'vars' in data:
            if not isinstance(data['vars'], dict):
                raise QuantumError("You defined a group '%s' with bad data for variables:\n %s" % (group, data))
    ...
    def get_host_variables(self, path, host):
        ...
        try:
            ...
            raise QuantumError("problem running %s (%s)" % (' '.join(cmd), e))
        ...
        try:
            ...
            raise QuantumError("could not parse post variable response: %s, %s" % (cmd, out))
