...
DOCUMENTATION = '''
    ...
    short_description: Quantum dynamic inventory plugin for Docker swarm nodes.
    requirements:
        - python >= 2.7
    ...
    options:
        ...
        timeout:
            ...
            aliases: [ time_out ]
        include_host_uri:
            description: Toggle to return the additional attribute C(quantum_host_uri) which contains the URI of the
        ...
        include_host_uri_port:
            description: Override the detected port number included in I(quantum_host_uri)
            type: int
'''
...
from quantum.module_utils.docker.common import update_tls_hostname, get_connect_params
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ''' Host inventory parser for quantum using Docker swarm as source. '''
    ...
    def _populate(self):
        ...
        try:
            ...
            for self.node in self.nodes:
                ...
                self.inventory.set_variable(self.node_attrs['ID'], 'quantum_host',
                                            self.node_attrs['Status']['Addr'])
                if self.get_option('include_host_uri'):
                    self.inventory.set_variable(self.node_attrs['ID'], 'quantum_host_uri',
                ...
                if 'ManagerStatus' in self.node_attrs:
                    if self.node_attrs['ManagerStatus'].get('Leader'):
                        ...
                        self.inventory.set_variable(self.node_attrs['ID'], 'quantum_host', swarm_leader_ip)
            ...
            raise QuantumError('Unable to fetch hosts from Docker swarm API, this was the original exception: %s' %
                               to_native(e))
    ...
    def parse(self, inventory, loader, path, cache=True):
        if not HAS_DOCKER:
            raise QuantumError('The Docker swarm dynamic inventory plugin requires the Docker SDK for Python: '
