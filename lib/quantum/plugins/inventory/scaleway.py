...
# Example command line: quantum-inventory --list -i scaleway_inventory.yml
...
regions:
  - par1
variables:
  quantum_host: public_ip.address
'''
...
from quantum.module_utils.scaleway import SCALEWAY_LOCATION, parse_pagination_link
from quantum.module_utils.urls import open_url
from quantum.module_utils._text import to_native

import quantum.module_utils.six.moves.urllib.parse as urllib_parse
...
def _fetch_information(token, url):
    ...
    while True:
        try:
            ...
            raise QuantumError("Error while fetching %s: %s" % (url, to_native(e)))
        try:
        ...
        try:
            ...
            raise QuantumError("Incorrect format from the Scaleway API response")
