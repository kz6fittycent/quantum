...
from quantum.inventory.group import to_safe_group_name as original_safe
from quantum.parsing.utils.addresses import parse_address
from quantum.plugins import QuantumPlugin
from quantum.plugins.cache import CachePluginAdjudicator as CacheObject
...
def expand_hostname_range(line=None):
    ...
    References: https://docs.quantum.com/quantum/latest/user_guide/intro_inventory.html#hosts-and-groups
    '''
    all_hosts = []
    if line:
        ...
        if len(bounds) != 2 and len(bounds) != 3:
            raise QuantumError("host range must be begin:end or begin:end:step")
        beg = bounds[0]
        end = bounds[1]
        if len(bounds) == 2:
            step = 1
        ...
        if not beg:
            beg = "0"
        if not end:
            raise QuantumError("host range must specify end value")
        if beg[0] == '0' and len(beg) > 1:
            rlen = len(beg)  # range length formatting hint
            if rlen != len(end):
                raise QuantumError("host range must specify equal-length begin and end formats")
        ...
        try:
            ...
            if i_beg > i_end:
                raise QuantumError("host range must have begin <= end")
...
def get_cache_plugin(plugin_name, **kwargs):
    try:
        ...
        if 'fact_caching_connection' in to_native(e):
            raise QuantumError("error, '%s' inventory cache plugin requires the one of the following to be set "
    ...
    if plugin_name != 'memory' and kwargs and not getattr(cache._plugin, '_options', None):
        raise QuantumError('Unable to use cache plugin {0} for inventory. Cache options were provided but may not reconcile '
                           'correctly unless set via set_options. Refer to the porting guide if the plugin derives user settings '
                           'from quantum.constants.'.format(plugin_name))
    return cache


class BaseInventoryPlugin(QuantumPlugin):
    ...
    def _populate_host_vars(self, hosts, variables, group=None, port=None):
        if not isinstance(variables, Mapping):
            raise QuantumParserError("Invalid data from file, expected dictionary and got:\n\n%s" % to_native(variables))
    ...
    def _read_config_data(self, path):
        ...
        try:
            ...
            raise QuantumParserError(to_native(e))

        if not config:
            # no data
            raise QuantumParserError("%s is empty" % (to_native(path)))
        elif config.get('plugin') != self.NAME:
            # this is not my config file
            raise QuantumParserError("Incorrect plugin name in file: %s" % config.get('plugin', 'none found'))
        elif not isinstance(config, Mapping):
            # configs are dictionaries
            raise QuantumParserError('inventory source has invalid structure, it should be a dictionary, got: %s' % type(config))
    ...
    def _consume_options(self, data):
        ''' update existing options from alternate configuration sources not normally used by Quantum.
...
class Constructable(object):
    ...
    def _compose(self, template, variables):
        ''' helper method for plugins to compose variables for Quantum based on jinja2 expression and inventory vars'''
        t = self.templar
        ...
        if compose and isinstance(compose, dict):
            for varname in compose:
                try:
                    ...
                    if strict:
                        raise QuantumError("Could not set %s for host %s: %s" % (varname, host, to_native(e)))
                    continue
        ...
        if groups and isinstance(groups, dict):
            ...
            for group_name in groups:
                ...
                try:
                    ...
                    if strict:
                        raise QuantumParserError("Could not add host %s to group %s: %s" % (host, group_name, to_native(e)))
                    continue
            ...
            for keyed in keys:
                if keyed and isinstance(keyed, dict):
                    ...
                    try:
                        ...
                        if strict:
                            raise QuantumParserError("Could not generate group for host %s from %s entry: %s" % (host, keyed.get('key'), to_native(e)))
                        continue
                    ...
                    if key:
                        ...
                        if raw_parent_name:
                            try:
                                ...
                                if strict:
                                    raise QuantumParserError("Could not generate parent group %s for group %s: %s" % (raw_parent_name, key, to_native(e)))
                                continue
                        ...
                        if isinstance(key, string_types):
                            ...
                            raise QuantumParserError("Invalid group name format, expected a string or a list of them or dictionary, got: %s" % type(key))
                        ...
                        if strict and key not in ([], {}):
                            raise QuantumParserError("No key or key resulted empty for %s in host %s, invalid entry" % (keyed.get('key'), host))
                else:
                    raise QuantumParserError("Invalid keyed group entry, it must be a dictionary: %s " % keyed)
