...
DOCUMENTATION = '''
    ...
    options:
        ...
        timeout:
            ...
            default: 60
        compose:
            description: List of custom quantum host vars to create from the device object fetched from NetBox
            default: {}
            type: dict
'''
...
# Example command line: quantum-inventory -v --list -i netbox_inventory.yml
...
from quantum.module_utils.six.moves.urllib.parse import urljoin, urlencode
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def _fetch_information(self, url):
        ...
        try:
            ...
            raise QuantumError("Incorrect encoding of fetched payload from NetBox API.")

        try:
            return json.loads(raw_data)
        except ValueError:
            raise QuantumError("Incorrect JSON payload: %s" % raw_data)
    ...
    def get_resource_list(self, api_url):
        ...
        """
        if not api_url:
            raise QuantumError("Please check API URL in script configuration file.")
    ...
    def extract_name(self, host):
        # An host in an Quantum inventory requires an hostname.
    ...
    def _fill_host_variables(self, host, hostname):
        ...
        if self.extract_primary_ip(host):
            self.inventory.set_variable(hostname, "quantum_host", self.extract_primary_ip(host=host))
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        self.headers = {
            ...
            'User-Agent': "quantum %s Python %s" % (quantum_version, python_version.split(' ')[0]),
