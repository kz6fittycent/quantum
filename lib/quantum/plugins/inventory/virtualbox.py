...
DOCUMENTATION = '''
    ...
    options:
        ...
        network_info_path:
            description: property path to query for network information (quantum_host)
...
simple_config_file:
    ...
    compose:
      quantum_connection: ('indows' in vbox_Guest_OS)|ternary('winrm', 'ssh')

# add hosts (all match with minishift vm) to the group container if any of the vms are in quantum_inventory'
plugin: virtualbox
groups:
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ''' Host inventory parser for quantum using local virtualbox. '''
    ...
    def _populate_from_source(self, source_data, using_current_cache=False):
        ...
        # needed to possibly set quantum_host
        ...
        for line in source_data:
            ...
            if k.startswith('Name') and ',' not in v:  # some setting strings appear in Name
                ...
                if netdata:
                    self.inventory.set_variable(current_host, 'quantum_host', netdata)
