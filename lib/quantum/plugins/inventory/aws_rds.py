...
from quantum.module_utils.aws.core import is_boto3_error_code
...
try:
    ...
    raise QuantumError('The RDS dynamic inventory plugin requires boto3 and botocore.')
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
        ...
        def wrapper(f, *args, **kwargs):
            try:
                ...
                if not strict:
                    ...
                    raise QuantumError("Failed to query RDS: {0}".format(to_native(e)))
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        instance_filters = quantum_dict_to_boto3_filter_list(filters)
        cluster_filters = []
        if 'db-cluster-id' in filters and include_clusters:
            cluster_filters = quantum_dict_to_boto3_filter_list({'db-cluster-id': filters['db-cluster-id']})
