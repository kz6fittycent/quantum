...
from quantum.module_utils.k8s.common import K8sQuantumMixin, HAS_K8S_MODULE_HELPER, k8s_import_exception
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable, K8sQuantumMixin):
    NAME = 'k8s'
    ...
    def get_pods_for_namespace(self, client, name, namespace):
        ...
        for pod in obj.items:
            ...
            for container in pod.status.containerStatuses:
                ...
                self.inventory.set_variable(container_name, 'quantum_remote_tmp', '/tmp/')
                self.inventory.set_variable(container_name, 'quantum_connection', self.transport)
                self.inventory.set_variable(container_name, 'quantum_{0}_pod'.format(self.transport),
                                            pod_name)
                self.inventory.set_variable(container_name, 'quantum_{0}_container'.format(self.transport),
                                            container.name)
                self.inventory.set_variable(container_name, 'quantum_{0}_namespace'.format(self.transport),
                                            namespace)
