...
DOCUMENTATION = r'''
    ...
    description:
        ...
        - By default, sets C(quantum_host) to the first public IP address found (preferring the primary NIC). If no
...
hostvar_expressions:
  ...
  # overrides the default quantum_host value with a custom Jinja2 expression, in this case, the first DNS hostname, or
  # if none are found, the first public IP address.
  quantum_host: (public_dns_hostnames + public_ipv4_addresses) | first
...
from quantum import release
from quantum.plugins.inventory import BaseInventoryPlugin, Constructable
from quantum.module_utils.six import iteritems
from quantum.module_utils.azure_rm_common import AzureRMAuth
from quantum.errors import QuantumParserError, QuantumError
from quantum.module_utils.parsing.convert_bool import boolean
from quantum.module_utils._text import to_native, to_bytes
...
class AzureRMRestConfiguration(AzureConfiguration):
    def __init__(self, credentials, subscription_id, base_url=None):
        ...
        self.add_user_agent('quantum-dynamic-inventory/{0}'.format(release.__version__))
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def _get_hosts(self):
        ...
        for h in self._hosts:
            ...
            self.inventory.set_variable(inventory_hostname, "quantum_host",
        ...
        for condition in self._filters:
            ...
            try:
                ...
                if boolean(self.get_option('fail_on_template_errors')):
                    raise QuantumParserError("Error evaluating filter condition '{0}' for host {1}: {2}".format(condition, inventory_hostname, to_native(e)))
                continue
    ...
    def _process_queue_batch(self):
        while True:
            ...
            if 'responses' in batch_resp:
                ...
                raise QuantumError("didn't find expected key responses/value in batch response")
            ...
            for idx, r in enumerate(batch_resp[key_name]):
                ...
                if status_code != 200:
                    ...
                    raise QuantumError("a batched request failed with status code {0}, url {1}".format(status_code, result.url))
