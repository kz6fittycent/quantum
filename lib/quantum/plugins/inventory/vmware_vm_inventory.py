...
from quantum.plugins.inventory import BaseInventoryPlugin, Cacheable
...
class BaseVMwareInventory:
    ...
    def _login_vapi(self):
        ...
        client = create_vsphere_client(server=self.hostname,
                                       ...
                                       session=session)
        if client is None:
            raise QuantumError("Failed to login to %s using %s" % (self.hostname, self.username))
        return client
    ...
    def _login(self):
        ...
        if self.validate_certs and not hasattr(ssl, 'SSLContext'):
            raise QuantumError('pyVim does not support changing verification mode with python < 2.7.9. Either update '
        ...
        try:
            ...
            raise QuantumParserError("Unable to log on to vCenter or ESXi API at %s:%s as %s: %s" % (self.hostname, self.port, self.username, e.msg))
        except vim.fault.NoPermission as e:
            raise QuantumParserError("User %s does not have required permission"
                                     " to log on to vCenter or ESXi API at %s:%s : %s" % (self.username, self.hostname, self.port, e.msg))
        except (requests.ConnectionError, ssl.SSLError) as e:
            raise QuantumParserError("Unable to connect to vCenter or ESXi API at %s on TCP/%s: %s" % (self.hostname, self.port, e))
        except vmodl.fault.InvalidRequest as e:
            # Request is malformed
            raise QuantumParserError("Failed to get a response from server %s:%s as "
                                     "request is malformed: %s" % (self.hostname, self.port, e.msg))
        except Exception as e:
            raise QuantumParserError("Unknown error while connecting to vCenter or ESXi API at %s:%s : %s" % (self.hostname, self.port, e))

        if service_instance is None:
            raise QuantumParserError("Unknown error while connecting to vCenter or ESXi API at %s:%s" % (self.hostname, self.port))
    ...
    def check_requirements(self):
        ...
        if not HAS_REQUESTS:
            raise QuantumParserError('Please install "requests" Python module as this is required'
                                     ' for VMware Guest dynamic inventory plugin.')
        elif not HAS_PYVMOMI:
            raise QuantumParserError('Please install "PyVmomi" Python module as this is required'
                                     ' for VMware Guest dynamic inventory plugin.')
        if HAS_REQUESTS:
            ...
            try:
                ...
                raise QuantumParserError("Failed to parse 'requests' library version.")

            if requests_major_minor < required_version:
                raise QuantumParserError("'requests' library version should"
                                         " be >= %s, found: %s." % (".".join([str(w) for w in required_version]),
                                                                    requests.__version__))

        if not HAS_VSPHERE and self.with_tags:
            raise QuantumError("Unable to find 'vSphere Automation SDK' Python library which is required."
                               " Please refer this URL for installation steps"
                               " - https://code.vmware.com/web/sdk/65/vsphere-automation-python")

        if not all([self.hostname, self.username, self.password]):
            raise QuantumError("Missing one of the following : hostname, username, password. Please read "
        ...
        for vm_obj in objects:
            for vm_obj_property in vm_obj.propSet:
                ...
                if current_host not in hostvars:
                    ...
                    if host_ip:
                        self.inventory.set_variable(current_host, 'quantum_host', host_ip)
