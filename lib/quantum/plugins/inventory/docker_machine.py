...
DOCUMENTATION = '''
    ...
    description:
        ...
        - The plugin sets standard host variables C(quantum_host), C(quantum_port), C(quantum_user) and C(quantum_ssh_private_key).
...
#   $ quantum-inventory -i ./path/to/docker-machine.yml --host=mymachine
#   {
#     ...
#     "digitalocean": {
#       "hosts": [
...
compose:
  quantum_ssh_common_args: '"-o StrictHostKeyChecking=accept-new"'
'''
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ''' Host inventory parser for quantum using Docker machine as source. '''
    ...
    def _run_command(self, args):
        if not self.DOCKER_MACHINE_PATH:
            try:
                ...
                raise QuantumError('Unable to locate the docker-machine binary.', orig_exc=e)
    ...
    def _get_docker_daemon_variables(self, machine_name):
        ...
        # capture any of the DOCKER_xxx variables that were output and create Quantum host vars
    ...
    def _populate(self):
        ...
        try:
            for self.node in self._get_machine_names():
                ...
                # set standard Quantum remote host connection settings to details captured from `docker-machine`
                # see: https://docs.quantum.com/quantum/latest/user_guide/intro_inventory.html
                self.inventory.set_variable(machine_name, 'quantum_host', ip_addr)
                self.inventory.set_variable(machine_name, 'quantum_port', self.node_attrs['Driver']['SSHPort'])
                self.inventory.set_variable(machine_name, 'quantum_user', self.node_attrs['Driver']['SSHUser'])
                self.inventory.set_variable(machine_name, 'quantum_ssh_private_key_file', self.node_attrs['Driver']['SSHKeyPath'])
            ...
            raise QuantumError('Unable to fetch hosts from Docker Machine, this was the original exception: %s' %
