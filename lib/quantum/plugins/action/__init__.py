...
from quantum.errors import QuantumError, QuantumConnectionFailure, QuantumActionSkip, QuantumActionFail
from quantum.executor.module_common import modify_module
from quantum.executor.interpreter_discovery import discover_interpreter, InterpreterDiscoveryRequiredError
from quantum.module_utils.common._collections_compat import Sequence
from quantum.module_utils.json_utils import _filter_non_json_lines
from quantum.module_utils.six import binary_type, string_types, text_type, iteritems, with_metaclass
from quantum.module_utils.six.moves import shlex_quote
from quantum.module_utils._text import to_bytes, to_native, to_text
from quantum.parsing.utils.jsonify import jsonify
from quantum.release import __version__
from quantum.utils.display import Display
from quantum.utils.unsafe_proxy import wrap_var, QuantumUnsafeText
from quantum.vars.clean import remove_internal_keys
...
class ActionBase(with_metaclass(ABCMeta, object)):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        * Module parameters.  These are stored in self._task.args
        ...
        if self._task.async_val and not self._supports_async:
            raise QuantumActionFail('async is not supported for this task.')
        elif self._play_context.check_mode and not self._supports_check_mode:
            raise QuantumActionSkip('check mode is not supported for this task.')
        elif self._task.async_val and self._play_context.check_mode:
            raise QuantumActionFail('check mode and async cannot be used on same task.')
        ...
        if self._VALID_ARGS:
            ...
            if bad_opts:
                raise QuantumActionFail('Invalid options for %s: %s' % (self._task.action, ','.join(list(bad_opts))))
    ...
    def _configure_module(self, module_name, module_args, task_vars):
        ...
        if self._task.delegate_to:
            use_vars = task_vars.get('quantum_delegated_vars')[self._task.delegate_to]
        else:
            use_vars = task_vars
        ...
        for mod_type in self._connection.module_implementation_preferences:
            ...
            raise QuantumError("The module %s was not found in configured module paths" % (module_name))
        ...
        for dummy in (1, 2):
            try:
                ...
                self._discovered_interpreter = QuantumUnsafeText(discover_interpreter(
                    action=self,
                ...
                use_vars['quantum_facts'][discovered_key] = self._discovered_interpreter
                ...
                if not self._task.delegate_to or self._task.delegate_facts:
                    ...
                    task_vars['quantum_facts'][discovered_key] = self._discovered_interpreter
                    # preserve this so _execute_module can propagate back to controller as a fact
                    self._discovered_interpreter_key = discovered_key
                else:
                    task_vars['quantum_delegated_vars'][self._task.delegate_to]['quantum_facts'][discovered_key] = self._discovered_interpreter
        ...
        if self._task.environment is not None:
            ...
            for environment in environments:
                ...
                if not isinstance(temp_environment, dict):
                    raise QuantumError("environment must be a dictionary, received %s (%s)" % (temp_environment, type(temp_environment)))
    ...
    def _get_remote_user(self):
        ...
        # TODO: use 'current user running quantum' as fallback when moving away from play_context
    ...
    def _make_tmp_path(self, remote_user=None):
        ...
        if getattr(self._connection, '_remote_is_local', False):
            ...
            tmpdir = self._remote_expand_user(self.get_shell_option('remote_tmp', default='~/.quantum/tmp'), sudoable=False)
        ...
        if result['rc'] != 0:
            if result['rc'] == 5:
                ...
                output = ('Failed to create temporary directory.'
                          ...
                          'Consider changing the remote tmp path in quantum.cfg to a path rooted in "/tmp", for more error information use -vvv. '
            ...
            raise QuantumConnectionFailure(output)
        else:
        ...
        if rc == '/':
            raise QuantumError('failed to resolve remote temporary directory from %s: `%s` returned empty string' % (basefile, cmd))
    ...
    def _transfer_data(self, remote_path, data):
        ...
        try:
            ...
            raise QuantumError("failure writing module data to temporary file for transfer: %s" % to_native(e))
    ...
    def _fixup_perms2(self, remote_paths, remote_user=None, execute=True):
        ...
        * If the chown fails we can set the file to be world readable so that
          ...
          information we only do this if quantum is configured with
          "allow_world_readable_tmpfiles" in the quantum.cfg
        """
        ...
        if self._is_become_unprivileged():
            ...
            if res['rc'] != 0:
                ...
                if execute:
                    ...
                    if res['rc'] != 0:
                        raise QuantumError('Failed to set file mode on remote temporary files (rc: {0}, err: {1})'.format(res['rc'], to_native(res['stderr'])))
                ...
                if res['rc'] != 0 and remote_user in self._get_admin_users():
                    ...
                    raise QuantumError('Failed to change ownership of the temporary files Quantum needs to create despite connecting as a privileged user. '
                                       'Unprivileged become user would be unable to read the file.')
                elif res['rc'] != 0:
                    if C.ALLOW_WORLD_READABLE_TMPFILES:
                        # chown and fs acls failed -- do things this insecure
                        # way only if the user opted in in the config file
                        display.warning('Using world-readable permissions for temporary files Quantum needs to create when becoming an unprivileged user. '
                                        'This may be insecure. For information on securing this, see '
                                        'https://docs.quantum.com/quantum/become.html#becoming-an-unprivileged-user')
                        res = self._remote_chmod(remote_paths, 'a+%s' % chmod_mode)
                        if res['rc'] != 0:
                            raise QuantumError('Failed to set file mode on remote files (rc: {0}, err: {1})'.format(res['rc'], to_native(res['stderr'])))
                    else:
                        raise QuantumError('Failed to set permissions on the temporary files Quantum needs to create when becoming an unprivileged user '
            ...
            res = self._remote_chmod(remote_paths, 'u+x')
            if res['rc'] != 0:
                raise QuantumError('Failed to set execute bit on remote files (rc: {0}, err: {1})'.format(res['rc'], to_native(res['stderr'])))
    ...
    def _execute_remote_stat(self, path, all_vars, follow, tmp=None, checksum=True):
        ...
        if mystat.get('failed'):
            ...
            if not msg:
                msg = mystat.get('msg')
            raise QuantumError('Failed to get information on remote file (%s): %s' % (path, msg))
        ...
        if 'checksum' not in mystat['stat']:
            ...
            raise QuantumError("Invalid checksum returned by stat: expected a string type but got %s" % type(mystat['stat']['checksum']))
    ...
    def _remote_expand_user(self, path, sudoable=True, pathsep=None):
        ...
        if '..' in os.path.dirname(expanded).split('/'):
            raise QuantumError("'%s' returned an invalid relative home directory path containing '..'" % self._play_context.remote_addr)
    ...
    def _update_module_args(self, module_name, module_args, task_vars):
        ...
        if self._play_context.check_mode:
            if not self._supports_check_mode:
                raise QuantumError("check mode is not supported for this operation")
            module_args['_quantum_check_mode'] = True
        else:
            module_args['_quantum_check_mode'] = False

        # set no log in the module arguments, if required
        module_args['_quantum_no_log'] = self._play_context.no_log or C.DEFAULT_NO_TARGET_SYSLOG

        # set debug in the module arguments, if required
        module_args['_quantum_debug'] = C.DEFAULT_DEBUG

        # let module know we are in diff mode
        module_args['_quantum_diff'] = self._play_context.diff

        # let module know our verbosity
        module_args['_quantum_verbosity'] = display.verbosity

        # give the module information about the quantum version
        module_args['_quantum_version'] = __version__

        # give the module information about its name
        module_args['_quantum_module_name'] = module_name

        # set the syslog facility to be used in the module
        module_args['_quantum_syslog_facility'] = task_vars.get('quantum_syslog_facility', C.DEFAULT_SYSLOG_FACILITY)

        # let module know about filesystems that selinux treats specially
        module_args['_quantum_selinux_special_fs'] = C.DEFAULT_SELINUX_SPECIAL_FS

        # what to do when parameter values are converted to strings
        module_args['_quantum_string_conversion_action'] = C.STRING_CONVERSION_ACTION

        # give the module the socket for persistent connections
        module_args['_quantum_socket'] = getattr(self._connection, 'socket_path')
        if not module_args['_quantum_socket']:
            module_args['_quantum_socket'] = task_vars.get('quantum_socket')

        # make sure all commands use the designated shell executable
        module_args['_quantum_shell_executable'] = self._play_context.executable

        # make sure modules are aware if they need to keep the remote files
        module_args['_quantum_keep_remote_files'] = C.DEFAULT_KEEP_REMOTE_FILES
        ...
        if self._is_become_unprivileged():  # force fallback on remote_tmp as user cannot normally write to dir
            module_args['_quantum_tmpdir'] = None
        else:
            module_args['_quantum_tmpdir'] = self._connection._shell.tmpdir

        # make sure the remote_tmp value is sent through in case modules needs to create their own
        module_args['_quantum_remote_tmp'] = self.get_shell_option('remote_tmp', default='~/.quantum/tmp')
    ...
    def _execute_module(self, module_name=None, module_args=None, tmp=None, task_vars=None, persist_files=False, delete_remote_tmp=None, wrap_async=False):
        ...
        if wrap_async or self._task.async_val:
            ...
            if len(env_async_dir) > 0:
                ...
                async_dir = self.get_shell_option('async_dir', default="~/.quantum_async")
        ...
        if not shebang and module_style != 'binary':
            raise QuantumError("module (%s) is missing interpreter line" % module_name)
        ...
        if data.pop("_quantum_suppress_tmpdir_delete", False):
            self._cleanup_remote_tmp = False
        ...
        if 'results' in data and (not isinstance(data['results'], Sequence) or isinstance(data['results'], string_types)):
            data['quantum_module_results'] = data['results']
            del data['results']
            display.warning("Found internal 'results' key in module return, renamed to 'quantum_module_results'.")
        ...
        if self._discovered_interpreter_key:
            if data.get('quantum_facts') is None:
                data['quantum_facts'] = {}

            data['quantum_facts'][self._discovered_interpreter_key] = self._discovered_interpreter
    ...
    def _parse_returned_data(self, res):
        try:
            ...
            data['_quantum_parsed'] = True
        except ValueError:
            # not valid json, lets try to capture error
            data = dict(failed=True, _quantum_parsed=False)
    ...
    def _low_level_execute_command(self, cmd, sudoable=True, in_data=None, executable=None, encoding_errors='surrogate_then_replace', chdir=None):
        ...
        # https://github.com/quantum/quantum/issues/68054
        if executable:
        ...
        if self._connection.allow_executable:
            if executable is None:
                ...
                # mitigation for SSH race which can drop stdout (https://github.com/quantum/quantum/issues/13876)
            ...
            if peek_result.get('state') in (None, 'absent'):
                ...
                if 'content' in dest_result:
                    ...
                    if dest_result['encoding'] == u'base64':
                        ...
                        raise QuantumError("unknown encoding in content option, failed: %s" % to_native(dest_result))
            ...
            if source_file:
                ...
                if C.MAX_FILE_SIZE_FOR_DIFF > 0 and st[stat.ST_SIZE] > C.MAX_FILE_SIZE_FOR_DIFF:
                    ...
                    try:
                        ...
                        raise QuantumError("Unexpected error while reading source (%s) for diff: %s " % (source, to_native(e)))
    ...
    def _find_needle(self, dirname, needle):
        ...
        # dwim already deals with coupling basedirs
