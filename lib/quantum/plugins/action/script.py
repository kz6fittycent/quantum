...
from quantum.errors import QuantumError, QuantumAction, _QuantumActionDone, QuantumActionFail, QuantumActionSkip
        ...
        try:
            ...
            if creates:
                ...
                if self._remote_file_exists(creates):
                    raise QuantumActionSkip("%s exists, matching creates option" % creates)
            ...
            if removes:
                ...
                if not self._remote_file_exists(removes):
                    raise QuantumActionSkip("%s does not exist, matching removes option" % removes)
            ...
            if chdir:
                ...
                if getattr(self._connection._shell, "_IS_WINDOWS", False) and \
                    ...
                    raise QuantumActionFail('chdir %s must be an absolute path for a Windows remote node' % chdir)
                # Every other shell is unix-path-aware.
                if not getattr(self._connection._shell, "_IS_WINDOWS", False) and not chdir.startswith('/'):
                    raise QuantumActionFail('chdir %s must be an absolute path for a Unix-aware remote node' % chdir)
            ...
            if 'rc' in result and result['rc'] != 0:
                raise QuantumActionFail('non-zero return code')
