...
from quantum.errors import QuantumError, QuantumAction, QuantumActionFail, QuantumActionSkip
        ...
        try:
            ...
            if 'copy' in self._task.args:
                ...
                if 'remote_src' in self._task.args:
                    raise QuantumActionFail("parameters are mutually exclusive: ('copy', 'remote_src')")
            ...
            if source is None or dest is None:
                raise QuantumActionFail("src (or content) and dest are required")
            ...
            if creates:
                ...
                if self._remote_file_exists(creates):
                    raise QuantumActionSkip("skipped, since %s exists" % creates)
            ...
            if not remote_stat['exists'] or not remote_stat['isdir']:
                raise QuantumActionFail("dest '%s' must be an existing dir" % dest)
