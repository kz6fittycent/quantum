...
from quantum.module_utils.common import validation
...
from quantum.module_utils.compat.paramiko import paramiko
from quantum.module_utils.network.nxos.nxos import run_commands
...
class ActionModule(ActionBase):
    ...
    def process_coupling_values(self):
        ''' Get coupling values and perform input validation '''
        argument_spec = dict(
        ...
        playvals = {}
        # Process key value pairs from coupling task
        for key in argument_spec.keys():
            ...
            try:
                if option_type == 'str':
                    ...
                    raise QuantumError('Unrecognized type <{0}> for coupling parameter <{1}>'.format(option_type, key))

            except (TypeError, ValueError) as e:
                raise QuantumError("argument %s is of type %s and we were unable to convert to %s: %s"
                                   % (key, type(playvals[key]), option_type, to_native(e)))

        # Validate coupling dependencies
        if playvals['file_pull']:
            if playvals.get('remote_file') is None:
                raise QuantumError('Playbook parameter <remote_file> required when <file_pull> is True')
            if playvals.get('remote_scp_server') is None:
                raise QuantumError('Playbook parameter <remote_scp_server> required when <file_pull> is True')
        ...
        if playvals['remote_scp_server'] or \
           playvals['remote_scp_server_user']:
            ...
            if None in (playvals['remote_scp_server'],
                ...
                raise QuantumError('Playbook parameters {0} must be set together'.format(params))
    ...
    def md5sum_check(self, dst, file_system):
        ...
        try:
            ...
            raise QuantumError('Error reading the file: {0}'.format(to_text(exc)))
    ...
    def get_flash_size(self, file_system):
        ...
        if match:
            raise QuantumError('Invalid nxos filesystem {0}'.format(file_system))
        else:
            raise QuantumError('Unable to determine size of filesystem {0}'.format(file_system))
    ...
    def transfer_file_to_device(self, remote_file):
        ...
        if not self.enough_space(local_file, file_system):
            raise QuantumError('Could not transfer file. Not enough space on device.')
    ...
    def file_push(self):
        ...
        if not self.local_file_exists(local_file):
            raise QuantumError('Local file {0} not found'.format(local_file))
    ...
    def copy_file_from_remote(self, local, local_file_directory, file_system):
        ...
        # local_file_directory coupling parameter is set.
        ...
        for copy_attempt in range(6):
            ...
            if outcome['password_prompt_detected']:
                if self.playvals.get('remote_scp_server_password'):
                    ...
                    err_msg += ' Set the <remote_scp_server_password> coupling parameter or configure nxos device for passwordless scp'
                    raise QuantumError(err_msg)
                continue
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        # Get coupling values
        self.playvals = self.process_coupling_values()
        ...
        # - file_pull is False - Push files from the quantum controller to nxos switch.
