...
from quantum.plugins.loader import connection_loader
...
class ActionModule(ActionBase):
    ...
    def _override_module_replaced_vars(self, task_vars):
        ...
        if 'quantum_syslog_facility' in task_vars:
            del task_vars['quantum_syslog_facility']
        for key in list(task_vars.keys()):
            if key.startswith("quantum_") and key.endswith("_interpreter"):
                del task_vars[key]
        ...
        for host in C.LOCALHOST:
            if host in task_vars['hostvars']:
                ...
                break
        if 'quantum_syslog_facility' in localhost:
            task_vars['quantum_syslog_facility'] = localhost['quantum_syslog_facility']
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        #
        # In normal quantum, we connect from controller to inventory_hostname
        # (coupling's hosts: field) or controller to delegate_to host and run
        # a module on one of those hosts.
        #
        # So things that are directly related to the core of quantum are in
        # terms of that sort of connection that always originate on the
        # controller.
        #
        # In synchronize we use quantum to connect to either the controller or
        ...
        # controller to inventory_host/delegate host that quantum typically
        ...
        # remote_transport is the transport quantum thought it would need
        ...
        # Parameter name needed by the quantum module
        _tmp_args['_local_rsync_path'] = task_vars.get('quantum_rsync_path') or 'rsync'
        _tmp_args['_local_rsync_password'] = task_vars.get('quantum_ssh_pass') or task_vars.get('quantum_password')

        # rsync thinks that one end of the connection is localhost and the
        # other is the host we're running the task for  (Note: We use
        # quantum's delegate_to mechanism to determine which host rsync is
        ...
        try:
            dest_host = dest_host_inventory_vars['quantum_host']
        except KeyError:
            dest_host = dest_host_inventory_vars.get('quantum_ssh_host', inventory_hostname)

        dest_host_ids = [hostid for hostid in (dest_host_inventory_vars.get('inventory_hostname'),
                                               dest_host_inventory_vars.get('quantum_host'),
                                               dest_host_inventory_vars.get('quantum_ssh_host'))
        ...
        # not about the quantum connection to run the module.
        dest_is_local = False
        ...
        inv_port = task_vars.get('quantum_ssh_port', None) or C.DEFAULT_REMOTE_PORT
        ...
        if not dest_is_local:
            ...
            if boolean(_tmp_args.get('set_remote_user', 'yes'), strict=False):
                if use_delegate:
                    user = task_vars.get('quantum_delegated_vars', dict()).get('quantum_ssh_user', None)
                    if not user:
                        user = task_vars.get('quantum_ssh_user') or self._play_context.remote_user
                    if not user:
        ...
        if 'SyntaxError' in result.get('exception', result.get('msg', '')):
            ...
            result['msg'] = ('SyntaxError parsing module.  Perhaps invoking "python" on your local (or delegate_to) machine invokes python3. '
                             'You can set quantum_python_interpreter for localhost (or the delegate_to machine) to the location of python2 to fix this')
        return result
