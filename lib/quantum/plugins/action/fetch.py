...
from quantum.errors import QuantumActionFail, QuantumActionSkip
...
from quantum.utils.hashing import checksum, checksum_s, md5, secure_hash
from quantum.utils.path import makedirs_safe, is_subpath
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            if self._play_context.check_mode:
                raise QuantumActionSkip('check mode not (yet) supported for this module')
            ...
            if not is_subpath(dest, original_dest):
                ...
                raise QuantumActionFail("Detected directory traversal, expected to be contained in '%s' but got '%s'" % (original_dest, dest))

            if flat:
                if os.path.isdir(to_bytes(dest, errors='surrogate_or_strict')) and not dest.endswith(os.sep):
                    raise QuantumActionFail("dest is an existing directory, use a trailing slash if you want to fetch src into that directory")
            ...
            if remote_checksum != local_checksum:
                ...
                if remote_data is None:
                    ...
                    try:
                        ...
                        raise QuantumActionFail("Failed to fetch the file: %s" % e)
