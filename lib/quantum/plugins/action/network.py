...
from quantum.plugins.action.normal import ActionModule as _ActionModule
...
class ActionModule(_ActionModule):
    ...
    def _handle_backup_option(self, result, task_vars):
        ...
        try:
            ...
            raise QuantumError('Failed while reading configuration backup')
    ...
    def _handle_src_option(self, convert_data=True):
        ...
        if not os.path.exists(source):
            raise QuantumError('path specified in src not found')

        try:
            with open(source, 'r') as f:
                template_data = to_text(f.read())
        except IOError as e:
            raise QuantumError("unable to load src file {0}, I/O error({1}): {2}".format(source, e.errno, e.strerror))
    ...
    def _get_network_os(self, task_vars):
        ...
        elif 'network_os' in task_vars.get('quantum_facts', {}) and task_vars['quantum_facts']['network_os']:
            display.vvvv('Getting network OS from fact')
            network_os = task_vars['quantum_facts']['network_os']
        else:
            raise QuantumError('quantum_network_os must be specified on this host')
