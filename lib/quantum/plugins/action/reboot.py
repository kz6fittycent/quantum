# Copyright: (c) 2016-2018, Matt Davis <mdavis@quantum.com>
...
from quantum.module_utils.common.collections import is_string
...
class ActionModule(ActionBase):
    ...
    DEFAULT_REBOOT_MESSAGE = 'Reboot initiated by Quantum'
    ...
    def get_distribution(self, task_vars):
        ...
        try:
            if module_output.get('failed', False):
                raise QuantumError('Failed to determine system distribution. {0}, {1}'.format(
                    to_native(module_output['module_stdout']).strip(),
                    to_native(module_output['module_stderr']).strip()))
            distribution['name'] = module_output['quantum_facts']['quantum_distribution'].lower()
            distribution['version'] = to_text(module_output['quantum_facts']['quantum_distribution_version'].split('.')[0])
            distribution['family'] = to_text(module_output['quantum_facts']['quantum_os_family'].lower())
            display.debug("{action}: distribution: {dist}".format(action=self._task.action, dist=distribution))
            return distribution
        except KeyError as ke:
            raise QuantumError('Failed to get distribution information. Missing "{0}" in output.'.format(ke.args[0]))
    ...
    def get_shutdown_command(self, task_vars, distribution):
        ...
        try:
            ...
            raise QuantumError(err_msg.format(search_paths))
        ...
        if not full_path:
            raise QuantumError('Unable to find command "{0}" in search paths: {1}'.format(shutdown_bin, search_paths))
    ...
    def deprecated_args(self):
        for arg, version in self.DEPRECATED_ARGS.items():
            if self._task.args.get(arg) is not None:
                display.warning("Since Quantum {version}, {arg} is no longer a valid option for {action}".format(
                    version=version,
                    arg=arg,
                    action=self._task.action))
    ...
    def get_system_boot_time(self, distribution):
        ...
        if command_result['rc'] != 0:
            ...
            raise QuantumError("{action}: failed to get host boot time info, rc: {rc}, stdout: {out}, stderr: {err}".format(
                               action=self._task.action,
    ...
    def perform_reboot(self, task_vars, distribution):
        ...
        try:
            ...
            display.debug('{action}: QuantumConnectionFailure caught and handled: {error}'.format(action=self._task.action, error=to_text(e)))
            reboot_result['rc'] = 0
            ...
            try:
                ...
                if original_connection_timeout != connect_timeout:
                    try:
                        ...
                        self._connection.reset()
                    except (QuantumError, AttributeError) as e:
