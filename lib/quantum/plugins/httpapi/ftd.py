...
options:
  token_path:
    ...
    vars:
      - name: quantum_httpapi_ftd_token_path
  spec_path:
    type: str
    description:
    ...
    vars:
      - name: quantum_httpapi_ftd_spec_path
"""
...
from quantum.module_utils.network.ftd.fdm_swagger_client import FdmSwaggerParser, SpecProp, FdmSwaggerValidator
...
BASE_HEADERS = {
    ...
    'User-Agent': 'FTD Quantum/%s' % quantum_version
}
...
INVALID_API_TOKEN_PATH_MSG = ('The API token path is incorrect. Please, check correctness of '
                              'the `quantum_httpapi_ftd_token_path` variable in the inventory file.')
MISSING_API_TOKEN_PATH_MSG = ('Quantum could not determine the API token path automatically. Please, '
                              'specify the `quantum_httpapi_ftd_token_path` variable in the inventory file.')
...
class HttpApi(HttpApiBase):
    ...
    def login(self, username, password):
        ...
        if self.refresh_token:
            ...
            raise QuantumConnectionFailure('Username and password are required for login in absence of refresh token')
