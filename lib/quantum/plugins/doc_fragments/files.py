...
options:
  mode:
    ...
    - For those used to I(/usr/bin/chmod) remember that modes are actually octal numbers.
      ...
      (like C(0644) or C(01777)) or quote it (like C('644') or C('1777')) so Quantum receives
      a string and can do its own conversion from string into number.
    - Giving Quantum a number without following one of these rules will end up with a decimal
      number which will have unexpected results.
    - As of Quantum 1.8, the mode may be specified as a symbolic mode (for example, C(u+rwx) or
      C(u=rw,g=r,o=r)).
    type: str
  owner:
    description:
  ...
  unsafe_writes:
    ...
    - This option allows Quantum to fall back to unsafe methods of updating files when atomic operations fail
      (however, it doesn't force Quantum to perform unsafe writes).
