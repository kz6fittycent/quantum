...
- C(quantum_managed) (configurable via the C(defaults) section of C(quantum.cfg)) contains a string which can be used to
...
options:
  src:
    description:
    - Path of a Jinja2 formatted template on the Quantum controller.
...
- Since Quantum 0.9, templates are loaded with C(trim_blocks=True).
- >
...
- Using raw/endraw in Jinja2 will not work as you expect because templates in Quantum are recursively
  evaluated.
