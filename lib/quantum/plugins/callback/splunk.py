...
DOCUMENTATION = '''
    ...
    requirements:
      ...
      - 'Define the url and token in quantum.cfg'
    options:
      url:
...
examples: >
  ...
  Set the quantum.cfg variable in the callback_splunk block
    [callback_splunk]
...
class SplunkHTTPCollectorSource(object):
    ...
    def send_event(self, url, authtoken, state, result, runtime):
        ...
        jsondata = json.dumps(data, cls=QuantumJSONEncoder, sort_keys=True)
...
class CallbackModule(CallbackBase):
    ...
    def v2_coupling_on_start(self, coupling):
        self.splunk.quantum_coupling = basename(coupling._file_name)
