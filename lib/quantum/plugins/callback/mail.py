  ...
  bcc:
    ...
    ini:
        - section: callback_mail
          ...
          version_added: '2.5'
note:
- "TODO: expand configuration options now that plugins can leverage Quantum's configuration"
'''
...
class CallbackModule(CallbackBase):
    ''' This Quantum callback plugin mails errors to interested parties. '''
    ...
    def mail(self, subject='Quantum error mail', body=None):
        if body is None:
            body = subject
    ...
    def mail_result(self, result, failtype):
        ...
        if not self.sender:
            self.sender = '"Quantum: %s" <root>' % host
        ...
        # Make coupling name visible (e.g. in Outlook/Gmail condensed view)
        body = 'Playbook: %s\n' % os.path.basename(self.coupling._file_name)
        ...
        body += self.indent('%s: %s' % (failtype, json.dumps(result._result, cls=QuantumJSONEncoder, indent=4)))
    ...
    def v2_runner_item_on_failed(self, result):
        ...
        self.itembody += self.body_blob(json.dumps(result._result, cls=QuantumJSONEncoder, indent=4), "failed item dump '%(item)s'" % result._result)
