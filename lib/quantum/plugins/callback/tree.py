...
DOCUMENTATION = '''
    ...
    version_added: "2.0"
    description:
        - "This callback is used by the Quantum (adhoc) command line option `-t|--tree`"
...
from quantum.constants import TREE_DIR
...
class CallbackModule(CallbackBase):
    ...
    def __init__(self):
        ...
        self.tree = TREE_DIR
        if not self.tree:
            self.tree = os.path.expanduser("~/.quantum/tree")
            self._display.warning("The tree callback is defaulting to ~/.quantum/tree, as an invalid directory was provided: %s" % self.tree)
