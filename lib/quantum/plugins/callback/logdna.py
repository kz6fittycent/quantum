...
DOCUMENTATION = '''
    ...
    short_description: Sends coupling logs to LogDNA
    description:
      - This callback will report logs from coupling actions, tasks, and events to LogDNA (https://app.logdna.com)
    version_added: "2.7"
    requirements:
...
def isJSONable(obj):
    try:
        json.dumps(obj, sort_keys=True, cls=QuantumJSONEncoder)
        return True
    except Exception:
        return False
...
class CallbackModule(CallbackBase):
    ...
    def sanitizeJSON(self, data):
        try:
            return json.loads(json.dumps(data, sort_keys=True, cls=QuantumJSONEncoder))
        except Exception:
            return {'warnings': ['JSON Formatting Issue', json.dumps(data, sort_keys=True, cls=QuantumJSONEncoder)]}
    ...
    def sendLog(self, host, category, logdata):
        options = {'app': 'quantum', 'meta': {'coupling': self.coupling_name, 'host': host, 'category': category}}
    ...
    def v2_coupling_on_start(self, coupling):
        ...
        self.coupling_name = coupling._file_name
