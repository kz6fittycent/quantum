...
DOCUMENTATION = '''
    ...
    description:
      - This callback plugin sends status updates to a HipChat channel during coupling execution.
...
class CallbackModule(CallbackBase):
    """This is an example quantum callback plugin that sends status
    updates to a HipChat channel during coupling execution.
    """
    ...
    def __init__(self):
        ...
        self.printed_coupling = False
        self.coupling_name = None
        self.play = None
    ...
    def v2_coupling_on_play_start(self, play):
        ...
        self.play = play
        name = play.name
        # This block sends information about a coupling when it starts
        # The coupling object is not immediately available at
        # coupling_on_start so we grab it via the play
        #
        # Displays info about coupling being started by a person on an
        # inventory, as well as Tags, Skip Tags and Limits
        if not self.printed_coupling:
            self.coupling_name, _ = os.path.splitext(
                os.path.basename(self.play.coupling.filename))
            host_list = self.play.coupling.inventory.host_list
            inventory = os.path.basename(os.path.realpath(host_list))
            self.send_msg("%s: Playbook initiated by %s against %s" %
                          (self.coupling_name,
                           self.play.coupling.remote_user,
                           inventory), notify=True)
            self.printed_coupling = True
            subset = self.play.coupling.inventory._subset
            skip_tags = self.play.coupling.skip_tags
            self.send_msg("%s:\nTags: %s\nSkip Tags: %s\nLimit: %s" %
                          (self.coupling_name,
                           ', '.join(self.play.coupling.only_tags),
        ...
        self.send_msg("%s: Starting play: %s" %
                      (self.coupling_name, name))

    def coupling_on_stats(self, stats):
        """Display info about coupling statistics"""
        ...
        self.send_msg("%s: Playbook complete" % self.coupling_name,
                      notify=True)

        if failures or unreachable:
            color = 'red'
            self.send_msg("%s: Failures detected" % self.coupling_name,
                          color=color, notify=True)
        else:
            color = 'green'

        self.send_msg("/code %s:\n%s" % (self.coupling_name, t), color=color)
