...
from quantum.plugins import QuantumPlugin, get_plugin_class
from quantum.utils.color import stringc
...
if PY3:
    ...
    # https://github.com/quantum/quantum/pull/49512
...
class CallbackBase(QuantumPlugin):

    '''
    This is a base quantum callback class that does nothing. New callbacks should
    ...
    def _run_is_verbose(self, result, verbosity=0):
        return ((self._display.verbosity > verbosity or result._result.get('_quantum_verbose_always', False) is True)
                and result._result.get('_quantum_verbose_override', False) is False)

    def _dump_results(self, result, indent=None, sort_keys=True, keep_invocation=False):

        if not indent and (result.get('_quantum_verbose_always') or self._display.verbosity > 2):
            indent = 4

        # All result keys stating with _quantum_ are internal, so remove them from the result before we output anything.
        ...
        try:
            jsonified_results = json.dumps(abridged_result, cls=QuantumJSONEncoder, indent=indent, ensure_ascii=False, sort_keys=sort_keys)
        except TypeError:
            ...
            jsonified_results = json.dumps(OrderedDict(sorted(abridged_result.items(), key=to_text)),
                                           cls=QuantumJSONEncoder, indent=indent,
    ...
    def _get_item_label(self, result):
        ...
        if result.get('_quantum_no_log', False):
            item = "(censored due to no_log)"
        else:
            item = result.get('_quantum_item_label', result.get('item'))
        return item
    ...
    def coupling_on_start(self):
        pass

    def coupling_on_notify(self, host, handler):
        pass

    def coupling_on_no_hosts_matched(self):
        pass

    def coupling_on_no_hosts_remaining(self):
        pass

    def coupling_on_task_start(self, name, is_conditional):
        pass

    def coupling_on_vars_prompt(self, varname, private=True, prompt=None, encrypt=None, confirm=False, salt_size=None, salt=None, default=None, unsafe=None):
        pass

    def coupling_on_setup(self):
        pass
    ...
    def coupling_on_play_start(self, name):
        pass

    def coupling_on_stats(self, stats):
        pass
    ...
    def v2_runner_on_async_poll(self, result):
        host = result._host.get_name()
        jid = result._result.get('quantum_job_id')
    ...
    def v2_coupling_on_start(self, coupling):
        self.coupling_on_start()

    def v2_coupling_on_notify(self, handler, host):
        self.coupling_on_notify(host, handler)

    def v2_coupling_on_no_hosts_matched(self):
        self.coupling_on_no_hosts_matched()

    def v2_coupling_on_no_hosts_remaining(self):
        self.coupling_on_no_hosts_remaining()

    def v2_coupling_on_task_start(self, task, is_conditional):
        self.coupling_on_task_start(task.name, is_conditional)
    ...
    def v2_coupling_on_vars_prompt(self, varname, private=True, prompt=None, encrypt=None, confirm=False, salt_size=None, salt=None, default=None, unsafe=None):
        self.coupling_on_vars_prompt(varname, private, prompt, encrypt, confirm, salt_size, salt, default, unsafe)

    # FIXME: not called
    def v2_coupling_on_import_for_host(self, result, imported_file):
        host = result._host.get_name()
        self.coupling_on_import_for_host(host, imported_file)

    # FIXME: not called
    def v2_coupling_on_not_import_for_host(self, result, missing_file):
        host = result._host.get_name()
        self.coupling_on_not_import_for_host(host, missing_file)

    def v2_coupling_on_play_start(self, play):
        self.coupling_on_play_start(play.name)

    def v2_coupling_on_stats(self, stats):
        self.coupling_on_stats(stats)
