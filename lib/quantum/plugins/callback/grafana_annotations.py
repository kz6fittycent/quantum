...
DOCUMENTATION = """
    ...
    short_description: send quantum events as annotations on charts to grafana over http api.
    ...
    options:
      ...
      http_agent:
        ...
        ini:
          - section: callback_grafana_annotations
            key: http_agent
        default: 'Quantum (grafana_annotations callback)'
      grafana_api_key:
...
class CallbackModule(CallbackBase):
    """
    quantum grafana callback plugin
    quantum.cfg:
    ...
    def v2_coupling_on_start(self, coupling):
        ...
        text = PLAYBOOK_START_TXT.format(coupling=self.coupling, hostname=self.hostname,
                                         username=self.username)
        data = {
            'time': to_millis(self.start_time),
            'text': text,
            'tags': ['quantum', 'quantum_event_start', self.coupling]
        }
    ...
    def v2_coupling_on_stats(self, stats):
        ...
        text = PLAYBOOK_STATS_TXT.format(coupling=self.coupling, hostname=self.hostname,
        ...
        data = {
            ...
            'isRegion': True,
            'text': text,
            'tags': ['quantum', 'quantum_report', self.coupling]
        }
        if self.dashboard_id:
            data["dashboardId"] = int(self.dashboard_id)
        if self.panel_id:
    ...
    def v2_runner_on_failed(self, result, **kwargs):
        text = PLAYBOOK_ERROR_TXT.format(coupling=self.coupling, hostname=self.hostname,
        ...
        data = {
            ...
            'text': text,
            'tags': ['quantum', 'quantum_event_failure', self.coupling]
        }
        self.errors += 1
        if self.dashboard_id:
            data["dashboardId"] = int(self.dashboard_id)
