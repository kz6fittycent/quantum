...
DOCUMENTATION = '''
    ...
    version_added: "2.8"
    description:
        - This is an quantum callback plugin utilizes cgroups to profile system activity of quantum and
          individual tasks, and display a recap at the end of the coupling execution
    notes:
        - Requires quantum to be run from within a cgroup, such as with
          C(cgexec -g cpuacct,memory,pids:quantum_profile quantum-coupling ...)
        - This cgroup should only be used by quantum to get accurate results
        - To create the cgroup, first use a command such as
          C(sudo cgcreate -a ec2-user:ec2-user -t ec2-user:ec2-user -g cpuacct,memory,pids:quantum_profile)
    options:
      control_group:
        required: True
      ...
      output_dir:
        description: Output directory for files containing recorded performance readings. If the value contains a
                     single %s, the start time of the coupling run will be inserted in that space. Only the deepest
                     level directory will be created if it does not exist, parent directories will not be created.
        type: path
        default: /tmp/quantum-perf-%s
        env:
      ...
      file_per_task:
        description: When set as C(True) along with C(write_files), this callback will write 1 file per task
                     instead of 1 file for the entire coupling run
        env:
...
from quantum.parsing.ajson import QuantumJSONEncoder, json
    ...
    def poll(self):
        ...
        if self.writer:
            try:
                ...
                # We may be profiling after the coupling has ended
                self.running = False
...
def json_writer(writer, timestamp, task_name, task_uuid, value):
    ...
    writer.write('%s%s%s' % (RS, json.dumps(data, cls=QuantumJSONEncoder), LF))
