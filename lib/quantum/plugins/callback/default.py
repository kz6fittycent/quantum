...
DOCUMENTATION = '''
    callback: default
    type: stdout
    short_description: default Quantum screen output
    version_added: historical
    description:
        - This is the default output callback for quantum-coupling.
    ...
    options:
      check_mode_markers:
        ...
        at the beggining and ending of coupling execution (when calling C(quantum-coupling --check))
...
# These values use quantum.constants for historical reasons, mostly to allow
# unmodified derivative plugins to work. However, newer options added to the
# plugin are not also added to quantum.constants, so authors of derivative
...
class CallbackModule(CallbackBase):
    ...
    def v2_coupling_on_start(self, coupling):
        if self._display.verbosity > 1:
            ...
            self._display.banner("PLAYBOOK: %s" % basename(coupling._file_name))
