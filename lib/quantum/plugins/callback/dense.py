...
from quantum.plugins.callback.default import CallbackModule as CallbackModule_default
...
#
#  + Use the same color-conventions of Quantum
#
...
#  + Modify Quantum mechanism so we don't need to use sys.stdout directly
...
class CallbackModule(CallbackModule_default):
    ...
    def _add_host(self, result, status):
        ...
        if delegated_vars:
            self.hosts[name]['delegate'] = delegated_vars['quantum_host']
...
#        self.keep = True
    ...
    def _display_results(self, result, status):
        ...
        if result._task.loop and 'results' in result._result:
            ...
            delegated_vars = result._result.get('_quantum_delegated_vars', None)
            if delegated_vars:
                sys.stdout.write(vt100.reset + result._host.get_name() + '>' + colors[status] + delegated_vars['quantum_host'])
            else:
    ...
    def v2_coupling_item_on_ok(self, result):
    ...
    # Old definition in v2.0
    def v2_coupling_item_on_failed(self, result):
    ...
    # Old definition in v2.0
    def v2_coupling_item_on_skipped(self, result):
