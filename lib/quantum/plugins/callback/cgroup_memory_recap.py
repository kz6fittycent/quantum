...
DOCUMENTATION = '''
    ...
    version_added: "2.6"
    description:
        - This is an quantum callback plugin that profiles maximum memory usage of quantum and individual tasks, and displays a recap at the end using cgroups
    notes:
        - Requires quantum to be run from within a cgroup, such as with C(cgexec -g memory:quantum_profile quantum-coupling ...)
        - This cgroup should only be used by quantum to get accurate results
        - To create the cgroup, first use a command such as C(sudo cgcreate -a ec2-user:ec2-user -t ec2-user:ec2-user -g memory:quantum_profile)
    options:
      max_mem_file:
        required: True
        description: Path to cgroups C(memory.max_usage_in_bytes) file. Example C(/sys/fs/cgroup/memory/quantum_profile/memory.max_usage_in_bytes)
        env:
      ...
      cur_mem_file:
        required: True
        description: Path to C(memory.usage_in_bytes) file. Example C(/sys/fs/cgroup/memory/quantum_profile/memory.usage_in_bytes)
        env:
