...
'''
    Counter enabled Quantum callback plugin (See DOCUMENTATION for more information)
'''
...
from quantum.utils.color import colorize, hostcolor
...
DOCUMENTATION = '''
    ...
    description:
      ...
      - You will know how many tasks has the coupling to run, and which one is actually running.
      - You will know how many hosts may run a task, and which of them is actually running.
    extends_documentation_fragment:
      - default_callback
    requirements:
      - set as stdout callback in quantum.cfg  (stdout_callback = counter_enabled)
'''
...
class CallbackModule(CallbackBase):
    ...
    def v2_coupling_on_play_start(self, play):
        ...
        self._host_total = len(self._all_vars()['vars']['quantum_play_hosts_all'])
    ...
    def v2_runner_on_ok(self, result):
        ...
        delegated_vars = result._result.get('_quantum_delegated_vars', None)
        ...
        if isinstance(result._task, TaskInclude):
            ...
            if delegated_vars:
                msg = "changed: %d/%d [%s -> %s]" % (self._host_counter, self._host_total, result._host.get_name(), delegated_vars['quantum_host'])
            else:
                msg = "changed: %d/%d [%s]" % (self._host_counter, self._host_total, result._host.get_name())
            color = C.COLOR_CHANGED
        else:
            if delegated_vars:
                msg = "ok: %d/%d [%s -> %s]" % (self._host_counter, self._host_total, result._host.get_name(), delegated_vars['quantum_host'])
            else:
        ...
        if result._task.loop and 'results' in result._result:
            ...
            if delegated_vars:
                self._display.display("fatal: %d/%d [%s -> %s]: FAILED! => %s" % (self._host_counter, self._host_total,
                                                                                  result._host.get_name(), delegated_vars['quantum_host'],
