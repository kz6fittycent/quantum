...
def _first_usable_query(v, vtype):
    if vtype == 'address':
        ...
        raise errors.QuantumFilterError('Not a network address')
    elif vtype == 'network':
        if v.size == 2:
...
def cidr_merge(value, action='merge'):
    if not hasattr(value, '__iter__'):
        raise errors.QuantumFilterError('cidr_merge: expected iterable, got ' + repr(value))

    if action == 'merge':
        try:
            return [str(ip) for ip in netaddr.cidr_merge(value)]
        except Exception as e:
            raise errors.QuantumFilterError('cidr_merge: error in netaddr:\n%s' % e)
        ...
        raise errors.QuantumFilterError("cidr_merge: invalid action '%s'" % action)
...
def ipaddr(value, query='', version=False, alias='ipaddr'):
    ...
    try:
        ...
        try:
            ...
            raise errors.QuantumFilterError(alias + ': unknown filter type: %s' % query)
...
def ipmath(value, amount):
    try:
        ...
        raise errors.QuantumFilterError(msg)
...
def next_nth_usable(value, offset):
    ...
    if type(offset) != int:
        raise errors.QuantumFilterError('Must pass in an integer')
    if v.size > 1:
        first_usable, last_usable = _first_last(v)
...
def hwaddr(value, query='', alias='hwaddr'):
    ...
    try:
        ...
        if query and query != 'bool':
            raise errors.QuantumFilterError(alias + ': not a hardware address: %s' % value)
...
def _need_netaddr(f_name, *args, **kwargs):
    raise errors.QuantumFilterError("The %s filter requires python's netaddr be "
                                    "installed on the quantum controller" % f_name)
