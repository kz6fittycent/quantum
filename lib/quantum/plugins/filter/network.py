...
from quantum.module_utils.network.common.utils import Template
...
from quantum.errors import QuantumError, QuantumFilterError
from quantum.utils.display import Display
from quantum.utils.encrypt import passlib_or_crypt, random_password
...
def parse_cli(output, tmpl):
    if not isinstance(output, string_types):
        raise QuantumError("parse_cli input should be a string, but was given a input of %s" % (type(output)))

    if not os.path.exists(tmpl):
        raise QuantumError('unable to locate parse_cli template: %s' % tmpl)

    try:
        template = Template()
    except ImportError as exc:
        raise QuantumError(to_native(exc))
...
def parse_cli_textfsm(value, template):
    if not HAS_TEXTFSM:
        raise QuantumError('parse_cli_textfsm filter requires TextFSM library to be installed')

    if not isinstance(value, string_types):
        raise QuantumError("parse_cli_textfsm input should be a string, but was given a input of %s" % (type(value)))

    if not os.path.exists(template):
        raise QuantumError('unable to locate parse_cli_textfsm template: %s' % template)
...
def parse_xml(output, tmpl):
    if not os.path.exists(tmpl):
        raise QuantumError('unable to locate parse_xml template: %s' % tmpl)

    if not isinstance(output, string_types):
        raise QuantumError('parse_xml works on string input, but given input of : %s' % type(output))
...
def type5_pw(password, salt=None):
    if not isinstance(password, string_types):
        raise QuantumFilterError("type5_pw password input should be a string, but was given a input of %s" % (type(password).__name__))
    ...
    if salt is not None and not isinstance(salt, string_types):
        raise QuantumFilterError("type5_pw salt input should be a string, but was given a input of %s" % (type(salt).__name__))
    elif not salt:
        salt = random_password(length=4, chars=salt_chars)
    elif not set(salt) <= set(salt_chars):
        raise QuantumFilterError("type5_pw salt used inproper characters, must be one of %s" % (salt_chars))
...
def hash_salt(password):
    ...
    if len(split_password) != 4:
        raise QuantumFilterError('Could not parse salt out password correctly from {0}'.format(password))
    else:
...
def vlan_parser(vlan_list, first_line_len=48, other_line_len=44):
    ...
    if sorted_list[0] < 1 or sorted_list[-1] > 4094:
        raise QuantumFilterError('Valid VLAN range is 1-4094')
