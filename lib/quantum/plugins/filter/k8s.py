...
def k8s_config_resource_name(resource):
    if not HAS_GENERATE_HASH:
        raise QuantumFilterError("k8s_config_resource_name requires openshift>=0.7.2")
    try:
        return resource['metadata']['name'] + '-' + generate_hash(resource)
    except KeyError:
        raise QuantumFilterError("resource must have a metadata.name key to generate a resource name")
