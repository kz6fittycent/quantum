...
from quantum.errors import QuantumError, QuantumFilterError, QuantumFilterTypeError
from quantum.module_utils.six import iteritems, string_types, integer_types, reraise
from quantum.module_utils.six.moves import reduce, shlex_quote
...
from quantum.module_utils.common._collections_compat import Mapping, MutableMapping
from quantum.parsing.ajson import QuantumJSONEncoder
from quantum.parsing.yaml.dumper import QuantumDumper
from quantum.template import recursive_check_defined
from quantum.utils.display import Display
from quantum.utils.encrypt import passlib_or_crypt
from quantum.utils.hashing import md5s, checksum_s
from quantum.utils.unicode import unicode_wrap
...
def to_yaml(a, *args, **kw):
    ...
    transformed = yaml.dump(a, Dumper=QuantumDumper, allow_unicode=True, default_flow_style=default_flow_style, **kw)
    return to_text(transformed)


def to_nice_yaml(a, indent=4, *args, **kw):
    '''Make verbose, human readable yaml'''
    transformed = yaml.dump(a, Dumper=QuantumDumper, indent=indent, allow_unicode=True, default_flow_style=False, **kw)
    return to_text(transformed)


def to_json(a, *args, **kw):
    ''' Convert the value to JSON '''
    return json.dumps(a, cls=QuantumJSONEncoder, *args, **kw)


def to_nice_json(a, indent=4, sort_keys=True, *args, **kw):
    '''Make verbose, human readable JSON'''
    try:
        return json.dumps(a, indent=indent, sort_keys=sort_keys, separators=(',', ': '), cls=QuantumJSONEncoder, *args, **kw)
...
def strftime(string_format, second=None):
    ...
    if second is not None:
        try:
            ...
            raise QuantumFilterError('Invalid value for epoch value (%s)' % second)
...
def regex_search(value, regex, *args, **kwargs):
    ...
    for arg in args:
        if arg.startswith('\\g'):
            ...
            raise QuantumFilterError('Unknown argument')
...
def regex_escape(string, re_type='python'):
    ...
    # https://remram44.github.io/regex-cheatsheet/regex.html#programs
        ...
        raise QuantumFilterError('Regex type (%s) not yet implemented' % re_type)
    else:
        raise QuantumFilterError('Invalid regex type (%s)' % re_type)
...
def rand(environment, end, start=None, step=None, seed=None):
    ...
    if isinstance(end, integer_types):
        ...
        if start or step:
            raise QuantumFilterError('start and step can only be used with integer values')
        return r.choice(end)
    else:
        raise QuantumFilterError('random can only be used on sequences and integers')
...
def get_encrypted_password(password, hashtype='sha512', salt=None, salt_size=None, rounds=None):
    ...
    try:
        ...
        reraise(QuantumFilterError, QuantumFilterError(to_native(e), orig_exc=e), sys.exc_info()[2])
...
def mandatory(a, msg=None):
    ...
    if isinstance(a, Undefined):
        ...
        if msg is not None:
            raise QuantumFilterError(to_native(msg))
        else:
            raise QuantumFilterError("Mandatory variable %s not defined." % name)
...
def combine(*terms, **kwargs):
    ...
    if len(kwargs) > 1 or (len(kwargs) == 1 and 'recursive' not in kwargs):
        raise QuantumFilterError("'recursive' is the only valid keyword argument")
    ...
    for t in terms:
        if isinstance(t, MutableMapping):
            ...
            raise QuantumFilterError("|combine expects dictionaries, got " + repr(t))
...
def do_groupby(environment, value, attribute):
    ...
    has repr that prevents quantum.template.safe_eval.safe_eval from being
    ...
    See https://github.com/quantum/quantum/issues/20098
...
def subelements(obj, subelements, skip_missing=False):
    ...
    if isinstance(obj, dict):
        ...
        raise QuantumFilterError('obj must be a list of dicts or a nested dict')
    ...
    if isinstance(subelements, list):
        ...
        raise QuantumFilterTypeError('subelements must be a list or a string')
        ...
        for subelement in subelement_list:
            try:
                ...
                if skip_missing:
                    values = []
                    break
                raise QuantumFilterError("could not find %r key in iterated item %r" % (subelement, values))
            except TypeError:
                raise QuantumFilterTypeError("the key %s should point to a dictionary, got '%s'" % (subelement, values))
        if not isinstance(values, list):
            raise QuantumFilterTypeError("the key %r should point to a list, got %r" % (subelement, values))
...
def dict_to_list_of_dict_key_value_elements(mydict, key_name='key', value_name='value'):
    ...
    if not isinstance(mydict, Mapping):
        raise QuantumFilterTypeError("dict2items requires a dictionary, got %s instead." % type(mydict))
...
def list_of_dict_key_value_elements_to_dict(mylist, key_name='key', value_name='value'):
    ...
    if not is_sequence(mylist):
        raise QuantumFilterTypeError("items2dict requires a list, got %s instead." % type(mylist))
...
def random_mac(value, seed=None):
    ...
    if not isinstance(value, string_types):
        raise QuantumFilterError('Invalid value type (%s) for random_mac (%s)' % (type(value), value))

    value = value.lower()
    mac_items = value.split(':')

    if len(mac_items) > 5:
        raise QuantumFilterError('Invalid value (%s) for random_mac: 5 colon(:) separated items max' % value)
    ...
    if len(err):
        raise QuantumFilterError('Invalid value (%s) for random_mac: %s' % (value, err))
...
class FilterModule(object):
    ...
    def filters(self):
        return {
            ...
            'sha1': checksum_s,
            # checksum of string as used by quantum for checksumming files
            'checksum': checksum_s,
            # generic hashing
