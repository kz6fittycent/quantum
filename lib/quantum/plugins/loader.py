...
from quantum.parsing.utils.yaml import from_yaml
from quantum.parsing.yaml.loader import QuantumLoader
from quantum.plugins import get_plugin_class, MODULE_CACHE, PATH_CACHE, PLUGIN_PATH_CACHE
from quantum.utils.collection_loader import QuantumCollectionLoader, QuantumFlatMapLoader, QuantumCollectionRef
from quantum.utils.display import Display
from quantum.utils.plugin_docs import add_fragments
...
def get_shell_plugin(shell_type=None, executable=None):
    ...
    if not shell_type:
        ...
        if executable:
            ...
            raise QuantumError("Either a shell type or a shell executable must be provided ")

    shell = shell_loader.get(shell_type)
    if not shell:
        raise QuantumError("Could not find the shell plugin required (%s)." % shell_type)
...
class PluginLoader:
    ...
    def _get_paths(self, subdirs=True):
        ...
        # In current usage, everything calls this with subdirs=True except for module_utils_loader and quantum-doc
    ...
    def _load_config_defs(self, name, module, path):
        ...
        if self.class_name:
            ...
            if type_name in C.CONFIGURABLE_PLUGINS:
                dstring = QuantumLoader(getattr(module, 'DOCUMENTATION', ''), file_name=path).get_single_data()
                if dstring:
    ...
    def _find_fq_plugin(self, fq_name, extension):
        ...
        plugin_type = QuantumCollectionRef.legacy_plugin_dir_to_plugin_type(self.subdir)

        acr = QuantumCollectionRef.from_fqcr(fq_name, plugin_type)
        ...
        if hasattr(pkg, '__loader__') and isinstance(pkg.__loader__, QuantumFlatMapLoader):
            try:
    ...
    def find_plugin_with_name(self, name, mod_type='', ignore_deprecated=False, check_aliases=False, collection_list=None):
        ...
        if mod_type:
            ...
            # Quantum plugins that run in the controller process (most plugins)
            suffix = '.py'
        else:
            # Only Quantum Modules.  Quantum modules can be any executable so
            # they can have any suffix
            suffix = ''

        # FIXME: need this right now so we can still load shipped PS module_utils- come up with a more robust solution
        if (QuantumCollectionRef.is_valid_fqcr(name) or collection_list) and not name.startswith('Quantum'):
            ...
            for candidate_name in candidates:
                try:
                    ...
                    if candidate_name.startswith('quantum.legacy'):
                        # 'quantum.legacy' refers to the plugin finding behavior used before collections existed.
                        # They need to search 'library' and the various '*_plugins' directories in order to find the file.
                        full_name = name
                        p = self._find_plugin_legacy(name.replace('quantum.legacy.', '', 1), ignore_deprecated, check_aliases, suffix)
                    else:
                        # 'quantum.builtin' should be handled here. This means only internal, or builtin, paths are searched.
    ...
    def _find_plugin_legacy(self, name, ignore_deprecated=False, check_aliases=False, suffix=None):
        ...
        for path in (p for p in self._get_paths() if p not in self._searched_paths and os.path.isdir(p)):
            ...
            for full_path in (f for f in full_paths if os.path.isfile(f) and not f.endswith('__init__.py')):
                ...
                # HACK: We have no way of executing python byte compiled files as quantum modules so specifically exclude them
        ...
        if not name.startswith('_'):
            ...
            if alias_name in pull_cache:
                if not ignore_deprecated and not os.path.islink(pull_cache[alias_name]):
                    ...
                    display.deprecated('%s is kept for backwards compatibility but usage is discouraged. '  # pylint: disable=quantum-deprecated-no-version
    ...
    def _load_module_source(self, name, path):
        ...
        if name.startswith('quantum_collections.'):
            full_name = name
        else:
            full_name = '.'.join([self.package, name])

        if full_name in sys.modules:
            # Avoids double loading, See https://github.com/quantum/quantum/issues/13110
    ...
    def all(self, *args, **kwargs):
        ...
        if path_only and class_only:
            raise QuantumError('Do not set both path_only and class_only when calling PluginLoader.all()')
...
class Jinja2Loader(PluginLoader):
    ...
    def find_plugin(self, name, collection_list=None):
        ...
        raise QuantumError('No code should call find_plugin for Jinja2Loaders (Not implemented)')
    ...
    def all(self, *args, **kwargs):
        ...
        * We do not deduplicate quantum plugin names.  This is because we don't care about our
        ...
        * We reverse the order of the list of plugins compared to other PluginLoaders.  This is
          ...
          Jinja2 plugins from one of our Quantum plugins into a dict.  Then it adds the Jinja2
          plugins from the next Quantum plugin, overwriting any Jinja2 plugins that had the same
        ...
        * We return a list.  We could iterate the list instead but that's extra work for no gain because
        ...
        """
        # We don't deduplicate quantum plugin names.  Instead, calling code deduplicates jinja2
        # plugin names.
...
def _load_plugin_filter():
    ...
    if C.PLUGIN_FILTERS_CFG is None:
        filter_cfg = '/etc/quantum/plugin_filters.yml'
    else:
    ...
    if os.path.exists(filter_cfg):
        ...
        if version == u'1.0':
            ...
            try:
                filters['quantum.modules'] = frozenset(filter_data['module_blacklist'])
            except TypeError:
                display.warning(u'Unable to parse the plugin filter file {0} as'
                                u' module_blacklist is not a list.'
                                u' Skipping.'.format(filter_cfg))
                return filters
            filters['quantum.plugins.action'] = filters['quantum.modules']
        else:
            display.warning(u'The plugin filter file, {0} was a version not recognized by this'
                            u' version of Quantum. Skipping.'.format(filter_cfg))
    else:
        if user_set:
            display.warning(u'The plugin filter file, {0} does not exist.'
                            u' Skipping.'.format(filter_cfg))

    # Specialcase the stat module as Quantum can run very few things if stat is blacklisted.
    if 'stat' in filters['quantum.modules']:
        raise QuantumError('The stat module was specified in the module blacklist file, {0}, but'
                           ' Quantum will not function without the stat module.  Please remove stat'
                           ' from the blacklist.'.format(to_native(filter_cfg)))
    return filters


def _configure_collection_loader():
    if not any((isinstance(l, QuantumCollectionLoader) for l in sys.meta_path)):
        sys.meta_path.insert(0, QuantumCollectionLoader(C.config))


# TODO: All of the following is initialization code   It should be moved inside of an initialization
# function which is called at some point early in the quantum and quantum-coupling CLI startup.
