...
class NetconfBase(QuantumPlugin):
    """
    A base class for implementing Netconf connections

    .. note:: Unlike most of Quantum, nearly all strings in
        :class:`TerminalBase` plugins are byte strings.  This is because of
        how close to the underlying platform these plugins operate.  Remember
        to mark literal strings as byte string (``b"string"``) and to use
        :func:`~quantum.module_utils._text.to_bytes` and
        :func:`~quantum.module_utils._text.to_text` to avoid unexpected
        problems.
