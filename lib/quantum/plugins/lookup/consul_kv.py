...
DOCUMENTATION = """
    ...
    description:
      - Lookup metadata for a coupling from the key value store in a Consul cluster.
        Values can be easily set in the kv store with simple rest commands
      - C(curl -X PUT -d 'some-value' http://localhost:8500/v1/kv/quantum/somedata)
    requirements:
            ...
            for param in params[1:]:
                if param and len(param) > 0:
                    ...
                    if name not in paramvals:
                        raise QuantumAssertionError("%s not a valid consul lookup parameter" % name)
