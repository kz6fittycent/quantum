...
- name: get a host record
  set_fact:
    host: "{{ lookup('nios', 'record:host', filter={'name': 'hostname.quantum.com'}) }}"

- name: get the authoritative zone from a non default dns view
  set_fact:
    host: "{{ lookup('nios', 'zone_auth', filter={'fqdn': 'quantum.com', 'view': 'quantum-dns'}) }}"
"""
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        try:
            ...
            raise QuantumError('the object_type must be specified')
