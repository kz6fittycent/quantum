...
from quantum.module_utils.urls import open_url, ConnectionError, SSLValidationError
...
class ManifoldApiClient(object):
    ...
    http_agent = 'python-manifold-quantum-1.0.0'
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, api_token=None, project=None, team=None):
        ...
        :param variables: quantum variables active at the time of the lookup
        ...
        if not api_token:
            raise QuantumError('API token is required. Please set api_token parameter or MANIFOLD_API_TOKEN env var')
        ...
        try:
            ...
            if team:
                ...
                if len(team_data) == 0:
                    raise QuantumError("Team '{0}' does not exist".format(team))
            ...
            if project:
                ...
                if len(project_data) == 0:
                    raise QuantumError("Project '{0}' does not exist".format(project))
            ...
            if labels and len(resources_data) < len(labels):
                ...
                raise QuantumError("Resource(s) {0} do not exist".format(', '.join(not_found_labels)))
            ...
            raise QuantumError('API Error: {0}'.format(str(e)))
        except QuantumError as e:
            raise e
        except Exception:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            raise QuantumError(format_exception(exc_type, exc_value, exc_traceback))
