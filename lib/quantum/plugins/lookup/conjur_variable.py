...
DOCUMENTATION = """
    ...
    requirements:
      - 'The controlling host running Quantum has a Conjur identity. (More: U(https://developer.conjur.net/key_concepts/machine_identity.html))'
    options:
      _term:
...
from quantum.module_utils.six.moves.urllib.parse import quote_plus
import yaml
...
def _load_conf_from_file(conf_path):
    ...
    if not os.path.exists(conf_path):
        raise QuantumError('Conjur configuration file `{0}` was not found on the controlling host'
                           .format(conf_path))
    ...
    with open(conf_path) as f:
        ...
        if 'account' not in config or 'appliance_url' not in config:
            raise QuantumError('{0} on the controlling host must contain an `account` and `appliance_url` entry'
                               .format(conf_path))
        return config
...
def _load_identity_from_file(identity_path, appliance_url):
    ...
    if not os.path.exists(identity_path):
        raise QuantumError('Conjur identity file `{0}` was not found on the controlling host'
    ...
    if identity.authenticators(conjur_authn_url) is None:
        raise QuantumError('The netrc file on the controlling host does not contain an entry for: {0}'
                           .format(conjur_authn_url))

    id, account, api_key = identity.authenticators(conjur_authn_url)
    if not id or not api_key:
        raise QuantumError('{0} on the controlling host must contain a `login` and `password` entry for {1}'
...
def _fetch_conjur_token(conjur_url, account, username, api_key):
    ...
    if code != 200:
        raise QuantumError('Failed to authenticate as \'{0}\' (got {1} response)'
...
def _fetch_conjur_variable(conjur_variable, token, conjur_url, account):
    ...
    if response.getcode() == 401:
        raise QuantumError('Conjur request has invalid authorization credentials')
    if response.getcode() == 403:
        raise QuantumError('The controlling host\'s Conjur identity does not have authorization to retrieve {0}'
                           .format(conjur_variable))
    if response.getcode() == 404:
        raise QuantumError('The variable {0} does not exist'.format(conjur_variable))
