...
- name: INTERFACES | Create Quantum header for /etc/network/interfaces
  template:
    src: "{{ lookup('first_found', findme)}}"
    dest: "/etc/foo.conf"
  vars:
    findme:
      - "{{ quantum_virtualization_type }}_foo.conf"
      - "default_foo.conf"
...
from quantum.errors import QuantumFileNotFound, QuantumLookupError, QuantumUndefinedVariable
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for fn in total_search:
            ...
            except (QuantumUndefinedVariable, UndefinedError):
                continue
        ...
        if skip:
            return []
        raise QuantumLookupError("No file was found when using first_found. Use errors='ignore' to allow this task to be skipped if no "
                                 "files are found")
