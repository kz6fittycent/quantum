    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        for term in terms:
            ...
            try:
                ...
                raise QuantumError("Received HTTP error for %s : %s" % (term, to_native(e)))
            except URLError as e:
                raise QuantumError("Failed lookup url for %s : %s" % (term, to_native(e)))
            except SSLValidationError as e:
                raise QuantumError("Error validating the server's certificate for %s: %s" % (term, to_native(e)))
            except ConnectionError as e:
                raise QuantumError("Error connecting to %s: %s" % (term, to_native(e)))
