...
from quantum.module_utils.gcp_utils import navigate_hash, GcpSession
...
class GcpFileLookup():
    ...
    def return_if_object(self, module, response, allow_not_found=False):
        ...
        try:
            ...
            raise QuantumError("Invalid JSON response with error: %s" % inst)
        if navigate_hash(result, ['error', 'errors']):
            raise QuantumError(navigate_hash(result, ['error', 'errors']))
        return result
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        if not remote_object:
            raise QuantumError("File does not exist in bucket")
