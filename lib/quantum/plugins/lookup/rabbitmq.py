...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, url=None, queue=None, count=None):
        if not HAS_PIKA:
            raise QuantumError('pika python package is required for rabbitmq lookup.')
        if not url:
            raise QuantumError('URL is required for rabbitmq lookup.')
        if not queue:
            raise QuantumError('Queue is required for rabbitmq lookup.')
        ...
        try:
            ...
            raise QuantumError("URL malformed: %s" % to_native(e))

        try:
            connection = pika.BlockingConnection(parameters)
        except Exception as e:
            raise QuantumError("Connection issue: %s" % to_native(e))
        ...
        try:
            ...
            try:
                ...
                raise QuantumError("Channel and connection closing issues: %s / %s" % to_native(e), to_native(ie))
            raise QuantumError("Channel issue: %s" % to_native(e))
            ...
            if method_frame:
                ...
                if properties.content_type == 'application/json':
                    try:
                        ...
                        raise QuantumError("Unable to decode JSON for message %s: %s" % (method_frame.delivery_tag, to_native(e)))
