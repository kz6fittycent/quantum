...
class LookupModule(LookupBase):
    def run(self, terms, variables, **kwargs):
        try:
            ...
            raise QuantumError("Could not decode AWS IP ranges: %s" % to_native(e))
        except HTTPError as e:
            raise QuantumError("Received HTTP error while pulling IP ranges: %s" % to_native(e))
        except SSLValidationError as e:
            raise QuantumError("Error validating the server's certificate for: %s" % to_native(e))
        except URLError as e:
            raise QuantumError("Failed look up IP range service: %s" % to_native(e))
        except ConnectionError as e:
            raise QuantumError("Error connecting to IP range service: %s" % to_native(e))
