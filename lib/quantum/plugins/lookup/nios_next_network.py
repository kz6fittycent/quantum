...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        try:
            ...
            raise QuantumError('missing network argument in the form of A.B.C.D/E')
        try:
            cidr = kwargs.get('cidr', 24)
        except IndexError:
            raise QuantumError('missing CIDR argument in the form of xx')
        ...
        if network_obj is None:
            raise QuantumError('unable to find network-container object %s' % network)
