...
DOCUMENTATION = """
  lookup: hashi_vault
  author: Jonathan Davila <jdavila(at)quantum.com>
  version_added: "2.0"
...
class HashiVault:
    def __init__(self, **kwargs):
        ...
        s = kwargs.get('secret')
        if s is None:
            raise QuantumError("No secret specified for hashi_vault lookup")
        ...
        if self.auth_method and self.auth_method != 'token':
            try:
                ...
                raise QuantumError("Authentication method '%s' not supported."
            ...
            if self.token is None:
                raise QuantumError("No Vault Token specified")
        ...
        if not self.client.is_authenticated():
            raise QuantumError("Invalid Hashicorp Vault Token Specified for hashi_vault lookup")

    def get(self):
        data = self.client.read(self.secret)

        if data is None:
            raise QuantumError("The secret %s doesn't seem to exist for hashi_vault lookup" % self.secret)

        if self.secret_field == '':
            return data['data']

        if self.secret_field not in data['data']:
            raise QuantumError("The secret %s does not contain the field '%s'. for hashi_vault lookup" % (self.secret, self.secret_field))
    ...
    def check_params(self, **kwargs):
        ...
        if username is None:
            raise QuantumError("Authentication method %s requires a username" % self.auth_method)

        password = kwargs.get('password')
        if password is None:
            raise QuantumError("Authentication method %s requires a password" % self.auth_method)
    ...
    def auth_approle(self, **kwargs):
        ...
        if role_id is None:
            raise QuantumError("Authentication method app role requires a role_id")

        secret_id = kwargs.get('secret_id', os.environ.get('VAULT_SECRET_ID', None))
        if secret_id is None:
            raise QuantumError("Authentication method app role requires a secret_id")
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        if not HAS_HVAC:
            raise QuantumError("Please pip install hvac to use the hashi_vault lookup module.")
        ...
        for param in vault_args:
            try:
                ...
                raise QuantumError("hashi_vault lookup plugin needs key=value pairs, but received %s" % terms)
