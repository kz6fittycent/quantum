...
description:
  ...
  - If the lookup fails due to lack of permissions or due to an AWS client error then the aws_ssm
    will generate an error, normally crashing the current quantum task.  This is normally the right
    ...
    up two quantum tasks, one which sets a variable and ignores failures one which uses the value
...
from quantum.module_utils.ec2 import HAS_BOTO3, boto3_tag_list_to_quantum_dict
    ...
    try:
        ...
        if boto_profile:
            ...
            # FIXME: we should probably do better passing on of the error information
                ...
                raise QuantumError("Insufficient credentials found.")
        else:
            raise QuantumError("Insufficient credentials found.")
    return connection
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, boto_profile=None, aws_profile=None,
        ...
        '''
            ...
            :kwarg variables: quantum variables active at the time of the lookup
        ...
        if not HAS_BOTO3:
            raise QuantumError('botocore and boto3 are required for aws_ssm lookup.')
        ...
        if bypath:
            ...
            for term in terms:
                ...
                try:
                    ...
                    raise QuantumError("SSM lookup exception: {0}".format(to_native(e)))
                paramlist = list()
                ...
                if len(paramlist):
                    ret.append(boto3_tag_list_to_quantum_dict(paramlist,
            ...
            try:
                ...
                raise QuantumError("SSM lookup exception: {0}".format(to_native(e)))
            params = boto3_tag_list_to_quantum_dict(response['Parameters'], tag_name_key_name="Name",
                                                    tag_value_key_name="Value")
            for i in terms:
                if i in params:
                    ret.append(params[i])
                    ...
                    raise QuantumError("Quantum internal error: aws_ssm lookup failed to understand boto3 return value: {0}".format(str(response)))
            return ret
