...
- See U(https://github.com/jborean93/quantum-lookup-laps_password) for more information around installing
...
options:
  ...
  auth:
    ...
    - When using C(gssapi), run C(kinit) before running Quantum to get a valid Kerberos ticket.
  ...
  domain:
    ...
    - This could either be a Windows domain name visible to the Quantum controller from DNS or a specific domain
      controller FQDN.
  ...
  username:
    ...
    - Call C(kinit) outside of Quantum if C(auth=gssapi) is required.
    type: str
  validate_certs:
    description:
...
EXAMPLES = """
# This isn't mandatory but it is a way to call kinit from within Quantum before calling the lookup
...
- name: Get the LAPS password using Kerberos auth, relies on kinit already being called
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'SERVER', domain='dc01.quantum.com') }}"

- name: Specific the domain host using an explicit LDAP URI
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'SERVER', domain='ldap://quantum.com:389') }}"

- name: Use Simple auth over LDAPS
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'server',
                                 domain='dc01.quantum.com',
                                 auth='simple',
                                 scheme='ldaps',
...
- name: Use Simple auth with LDAP and StartTLS
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'app01',
                                 domain='dc01.quantum.com',
                                 auth='simple',
                                 start_tls=True,
                                 username='username@ANSIBLE.COM',
...
- name: Narrow down the search base to a an OU
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'sql10',
                                 domain='dc01.quantum.com',
                                 search_base='OU=Databases,DC=quantum,DC=com') }}"

- name: Set certificate file to use when validating the TLS certificate
  set_fact:
    quantum_password: "{{ lookup('laps_password', 'windows-pc',
                                 domain='dc01.quantum.com',
                                 start_tls=True,
...
from quantum.errors import QuantumLookupError
...
def get_laps_password(conn, cn, search_base):
    ...
    if len(valid_results) == 0:
        raise QuantumLookupError("Failed to find the server '%s' in the base '%s'" % (cn, search_base))
    elif len(valid_results) > 1:
        found_servers = [to_native(attr['distinguishedName'][0]) for attr in valid_results]
        raise QuantumLookupError("Found too many results for the server '%s' in the base '%s'. Specify a more "
    ...
    if not password:
        ...
        raise QuantumLookupError("The server '%s' did not have the LAPS attribute 'ms-Mcs-AdmPwd'" % distinguished_name)
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        if validate_certs_value is None:
            ...
            valid_keys.sort()
            raise QuantumLookupError("Invalid validate_certs value '%s': valid values are '%s'"
                                     % (validate_certs, "', '".join(valid_keys)))

        if auth not in ['gssapi', 'simple']:
            raise QuantumLookupError("Invalid auth value '%s': expecting either 'gssapi', or 'simple'" % auth)
        elif auth == 'gssapi':
            if not ldap.SASL_AVAIL:
                raise QuantumLookupError("Cannot use auth=gssapi when SASL is not configured with the local LDAP "
                                         "install")
            if username or password:
                raise QuantumLookupError("Explicit credentials are not supported when auth='gssapi'. Call kinit "
                                         "outside of Quantum")
        elif auth == 'simple' and not (username and password):
            raise QuantumLookupError("The username and password values are required when auth=simple")
        ...
        if not encrypted and not allow_plaintext:
            raise QuantumLookupError("Current configuration will result in plaintext traffic exposing credentials. "
        ...
        if ldap_url.urlscheme == 'ldaps' or start_tls:
            ...
            if not ldap.TLS_AVAIL:
                raise QuantumLookupError("Cannot use TLS as the local LDAP installed has not been configured to support it")
            ...
            if cacert_file:
                ...
                if not os.path.exists(to_bytes(cacert_path)):
                    raise QuantumLookupError("The cacert_file specified '%s' does not exist" % to_native(cacert_path))
                ...
                try:
                    ...
                    raise QuantumLookupError("Failed to set path to cacert file, this is a known issue with older "
        ...
        if start_tls:
            try:
                ...
                raise QuantumLookupError("Failed to send StartTLS to LDAP host '%s': %s"
        ...
        if auth == 'simple':
            try:
                ...
                raise QuantumLookupError("Failed to simple bind against LDAP host '%s': %s"
                                         % (conn_url, to_native(err)))
        else:
            try:
                ...
                raise QuantumLookupError("Failed to do a sasl bind against LDAP host '%s', the GSSAPI mech is not "
                                         "installed: %s" % (conn_url, to_native(err)))
            except ldap.LDAPError as err:
                raise QuantumLookupError("Failed to do a sasl bind against LDAP host '%s': %s"
                                         % (conn_url, to_native(err)))
