...
DOCUMENTATION = """
    ...
    notes:
      ...
      - Due to the B(very) sensitive nature of these credentials, it is B(highly) recommended that you only pass in the minimal credentials
        needed at any given time. Also, store these credentials in an Quantum Vault using a key that is equal to or greater in strength
        to the 1Password master password.
      - This lookup stores potentially sensitive data from 1Password as Quantum facts.
...
class OnePass(object):
    ...
    def get_token(self):
        ...
        if os.path.isfile(self.config_file_path):
            ...
            if not self.master_password:
                raise QuantumLookupError('Unable to sign in to 1Password. master_password is required.')
    ...
    def assert_logged_in(self):
        try:
            ...
            if e.errno == errno.ENOENT:
                raise QuantumLookupError("1Password CLI tool '%s' not installed in path on control machine" % self.cli_path)
            raise e
    ...
    def full_login(self):
        if None in [self.subdomain, self.username, self.secret_key, self.master_password]:
            raise QuantumLookupError('Unable to perform initial sign in to 1Password. '
    ...
    def _run(self, args, expected_rc=0, command_input=None, ignore_errors=False):
        ...
        if not ignore_errors and rc != expected_rc:
            raise QuantumLookupError(to_text(err))
        return rc, out, err
