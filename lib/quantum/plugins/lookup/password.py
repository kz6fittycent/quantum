...
DOCUMENTATION = """
    ...
    notes:
      - A great alternative to the password lookup plugin,
        ...
        would be to use Vault in couplings.
      ...
      - 'As all lookups, this runs on the Quantum host as the user running the coupling, and "become" does not apply,
        the target file must be readable by the coupling user, or, if it does not exist,
        the coupling user must have sufficient privileges to create it.
        (So, for example, attempts to write into areas such as /etc will fail unless the entire coupling is being run as root).'
"""
...
from quantum.parsing.splitter import parse_kv
from quantum.plugins.lookup import LookupBase
from quantum.utils.encrypt import do_encrypt, random_password, random_salt
...
def _parse_parameters(term):
    ...
    See https://github.com/quantum/quantum-modules-core/issues/1968#issuecomment-136842156
    ...
    if len(first_split) <= 1:
        ...
        if '_raw_params' in params:
            ...
            if not term.startswith(relpath):
                ...
                raise QuantumError('Unrecognized value after key=value parameters given to password lookup')
    ...
    if invalid_params:
        raise QuantumError('Unrecognized parameter(s) given to password lookup: %s' % ', '.join(invalid_params))
...
def _format_content(password, salt, encrypt=None):
    ...
    .. warning:: Passwords are saved in clear.  This is because the couplings
    ...
    if not salt:
        raise QuantumAssertionError('_format_content was called with encryption requested but no salt value')
...
def _get_lock(b_path):
    ...
    first_process = False
    b_pathdir = os.path.dirname(b_path)
    lockfile_name = to_bytes("%s.quantum_lockfile" % hashlib.sha1(b_path).hexdigest())
    ...
    while os.path.exists(lockfile) and not first_process:
        ...
        if counter >= 2:
            raise QuantumError("Password lookup cannot get the lock in 7 seconds, abort..."
