...
class LookupModule(LookupBase):
    def run(self, terms, variables, **kwargs):
        ...
        if not CREDSTASH_INSTALLED:
            raise QuantumError('The credstash lookup plugin requires credstash to be installed.')
        ...
        for term in terms:
            try:
                ...
                raise QuantumError('Key {0} not found'.format(term))
            except Exception as e:
                raise QuantumError('Encountered exception while fetching {0}: {1}'.format(term, e))
            ret.append(val)
