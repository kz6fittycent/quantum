#
# Copyright: (c) 2018, Red Hat | Quantum
...
DOCUMENTATION = r"""
    ...
    options:
      ...
      src:
        ...
        - Reads from the local file system. To read from the Quantum controller's file system, use the file lookup
...
- name: Fetch with label selector
  set_fact:
    service: "{{ lookup('k8s', kind='Service', label_selector='app=fog') }}"

# Use parameters from a YAML config

- name: Load config from the Quantum controller filesystem
  set_fact:
...
from quantum.module_utils.common._collections_compat import KeysView
...
class KubernetesLookup(K8sQuantumMixin):
