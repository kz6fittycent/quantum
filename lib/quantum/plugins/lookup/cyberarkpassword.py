...
DOCUMENTATION = """
    ...
    note:
      - For Quantum on windows, please change the -parameters (-p, -d, and -o) to /parameters (/p, /d, and /o) and change the location of CLIPasswordSDK.exe
"""
...
class CyberarkPassword:
    ...
    def __init__(self, appid=None, query=None, output=None, **kwargs):
        ...
        if self.appid is None:
            raise QuantumError("CyberArk Error: No Application ID specified")
        if self.query is None:
            raise QuantumError("CyberArk Error: No Vault query specified")
    ...
    def get(self):
        ...
        try:
            ...
            if tmp_error:
                raise QuantumError("ERROR => %s " % (tmp_error))
            ...
            raise QuantumError(e.output)
        except OSError as e:
            raise QuantumError("ERROR - AIM not installed or clipasswordsdk not in standard location. ERROR=(%s) => %s " % (to_text(e.errno), e.strerror))
