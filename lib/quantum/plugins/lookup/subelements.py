    ...
    def run(self, terms, variables, **kwargs):
        ...
        for item0 in elementlist:
            if not isinstance(item0, dict):
                raise QuantumError("subelements lookup expects a dictionary, got '%s'" % item0)
            ...
            for subkey in subelements:
                ...
                if subkey not in subvalue:
                    if skip_missing:
                        ...
                        raise QuantumError("could not find '%s' key in iterated item '%s'" % (subkey, subvalue))
                if not lastsubkey:
                    if not isinstance(subvalue[subkey], dict):
                        if skip_missing:
                            continue
                        else:
                            raise QuantumError("the key %s should point to a dictionary, got '%s'" % (subkey, subvalue[subkey]))
                    else:
                        subvalue = subvalue[subkey]
                else:  # lastsubkey
                    if not isinstance(subvalue[subkey], list):
                        raise QuantumError("the key %s should point to a list, got '%s'" % (subkey, subvalue[subkey]))
                    else:
