...
DOCUMENTATION = r"""
    ...
    notes:
      - Like all lookups this runs on the Quantum controller and is unaffected by other keywords, such as become,
        so if you need to different permissions you must change the command or run Quantum as another user.
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for term in terms:
            ...
            if p.returncode == 0:
                ...
                raise QuantumError("lookup_plugin.pipe(%s) returned %d" % (term, p.returncode))
        return ret
