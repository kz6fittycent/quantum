...
from quantum.errors import QuantumError, QuantumFileNotFound, QuantumParserError, QuantumUndefinedVariable
from quantum.executor import action_write_locks
from quantum.executor.process.worker import WorkerProcess
from quantum.executor.task_result import TaskResult
from quantum.module_utils.six.moves import queue as Queue
from quantum.module_utils.six import iteritems, itervalues, string_types
from quantum.module_utils._text import to_text
from quantum.module_utils.connection import Connection, ConnectionError
from quantum.coupling.conditional import Conditional
from quantum.coupling.handler import Handler
from quantum.coupling.helpers import load_list_of_blocks
from quantum.coupling.included_file import IncludedFile
from quantum.coupling.task_include import TaskInclude
from quantum.plugins import loader as plugin_loader
...
from quantum.vars.clean import strip_internal_keys, module_response_deepcopy
...
def SharedPluginLoaderObj():
    ...
    '''
    display.deprecated('SharedPluginLoaderObj is deprecated, please directly use quantum.plugins.loader',
                       version='2.11')
        ...
        for result in results:
            ...
            while result.needs_debugger(globally_enabled=self.debugger_active):
                ...
                if next_action.result == NextAction.REDO:
                    ...
                    # Matches KeyboardInterrupt from bin/quantum
                    sys.exit(99)
            else:
                _processed_results.append(result)
...
class StrategyBase:
    ...
    def add_tqm_variables(self, vars, play):
        ...
        '''
        vars['quantum_current_hosts'] = self.get_hosts_remaining(play)
        vars['quantum_failed_hosts'] = self.get_failed_hosts(play)
    ...
    def _queue_task(self, host, task, task_vars, play_context):
        ...
        try:
            ...
            raise QuantumError("Failed to convert the throttle value to an integer.", obj=task._ds, orig_exc=e)
    ...
    def get_delegated_hosts(self, result, task):
        host_name = result.get('_quantum_delegated_vars', {}).get('quantum_delegated_host', None)
    ...
    def _set_always_delegated_facts(self, result, task):
        ...
        if always_keys:
            _pop = facts.pop
            always_facts = {
                'quantum_facts': dict((k, _pop(k)) for k in list(facts) if k in always_keys)
            }
    ...
    def _process_pending_results(self, iterator, one_pass=False, max_passes=None, do_handlers=False):
        ...
        while True:
            ...
            if '_quantum_retry' in task_result._result:
                self._tqm.send_callback('v2_runner_retry', task_result)
                continue
            elif '_quantum_item_result' in task_result._result:
            ...
            if task_result.is_failed():
                ...
                if not ignore_errors:
                    ...
                    if state and (iterator.get_active_state(state).run_state == iterator.ITERATING_RESCUE or
                        ...
                        self._variable_manager.set_nonpersistent_facts(
                            original_host.name,
                            dict(
                                quantum_failed_task=wrap_var(original_task.serialize()),
                                quantum_failed_result=task_result._result,
                            ),
                        )
                    else:
                ...
                for result_item in result_items:
                    if '_quantum_notify' in result_item:
                        if task_result.is_changed():
                            # The shared dictionary for notified handlers is a proxy, which
                            # does not detect when sub-objects within the proxy are modified.
                            # So, per the docs, we reassign the list so the proxy picks up and
                            # notifies all other threads
                            for handler_name in result_item['_quantum_notify']:
                                found = False
                                ...
                                if target_handler is not None:
                                    ...
                                    if target_handler.notify_host(original_host):
                                        self._tqm.send_callback('v2_coupling_on_notify', target_handler, original_host)
                                ...
                                for listening_handler_block in iterator._play.handlers:
                                    for listening_handler in listening_handler_block.block:
                                        ...
                                        if listening_handler.notify_host(original_host):
                                            self._tqm.send_callback('v2_coupling_on_notify', listening_handler, original_host)
                    ...
                    if 'quantum_facts' in result_item:
                        ...
                        if original_task.action in C._ACTION_INCLUDE_VARS:
                            for (var_name, var_value) in iteritems(result_item['quantum_facts']):
                                # find the host we're actually referring too here, which may
                                # be a host that is not really in inventory at all
                                for target_host in host_list:
                                    self._variable_manager.set_host_variable(target_host, var_name, var_value)
                        else:
                            cacheable = result_item.pop('_quantum_facts_cacheable', False)
                            for target_host in host_list:
                                ...
                                # but for coupling setting it the 'higher' precedence is kept
                                is_set_fact = original_task.action in C._ACTION_SET_FACT
                                if not is_set_fact or cacheable:
                                    self._variable_manager.set_host_facts(target_host, result_item['quantum_facts'].copy())
                                if is_set_fact:
                                    self._variable_manager.set_nonpersistent_facts(target_host, result_item['quantum_facts'].copy())

                    if 'quantum_stats' in result_item and 'data' in result_item['quantum_stats'] and result_item['quantum_stats']['data']:

                        if 'per_host' not in result_item['quantum_stats'] or result_item['quantum_stats']['per_host']:
                            host_list = self.get_task_hosts(iterator, original_host, original_task)
                        else:
                            host_list = [None]

                        data = result_item['quantum_stats']['data']
                        aggregate = 'aggregate' in result_item['quantum_stats'] and result_item['quantum_stats']['aggregate']
    ...
    def _wait_on_handler_results(self, iterator, handler, notified_hosts):
        ...
        while (self._pending_handler_results > 0 and
            ...
            if self._tqm.has_dead_workers():
                raise QuantumError("A worker was found in a dead state")
    ...
    def _add_group(self, host, result_item):
        ...
        if real_host is None:
            if host.name == self._inventory.localhost.name:
                ...
                raise QuantumError('%s cannot be matched in inventory' % host.name)
    ...
    def _load_included_file(self, included_file, iterator, is_handler=False):
        ...
        try:
            ...
            if data is None:
                ...
                raise QuantumError("included task files must contain a list of tasks")
            ...
            if len(tags) > 0:
                if len(included_file._task.tags) > 0:
                    raise QuantumParserError("Include tasks should not specify tags in more than one way (both via args and directly on the task). "
            ...
            if isinstance(e, QuantumFileNotFound):
                reason = "Could not find or access '%s' on the Quantum Controller." % to_text(e.file_name)
            else:
                reason = to_text(e)
        ...
        self._tqm.send_callback('v2_coupling_on_include', included_file)
    ...
    def _do_handler_run(self, handler, handler_name, iterator, play_context, notified_hosts=None):
        ...
        #     self._tqm.send_callback('v2_coupling_on_no_hosts_remaining')
        #     result = False
        #     break
        ...
        if len(notified_hosts) > 0:
            ...
            self._tqm.send_callback('v2_coupling_on_handler_task_start', handler)
    ...
    def _execute_meta(self, task, play_context, iterator, target_host):
        ...
        if meta_action == 'noop':
            ...
            raise QuantumError("invalid meta action requested: %s" % meta_action, obj=task._ds)
    ...
    def update_active_connections(self, results):
        ...
        for r in results:
            if 'args' in r._task_fields:
                socket_path = r._task_fields['args'].get('_quantum_socket')
                if socket_path:
