...
DOCUMENTATION = '''
    ...
    description:
        - Task execution is as fast as possible per batch as defined by C(serial) (default all).
          Quantum will not wait for other hosts to finish the current task before queuing more tasks for other hosts.
...
from quantum.plugins.loader import action_loader
from quantum.plugins.strategy import StrategyBase
                    ...
                    if (host_name not in self._blocked_hosts or not self._blocked_hosts[host_name]):
                        ...
                        if run_once:
                            if action and getattr(action, 'BYPASS_HOST_LOOP', False):
                                raise QuantumError("The '%s' module bypasses the host loop, which is currently not supported in the free strategy "
                        ...
                        if task.action in C._ACTION_META:
                            ...
                            if not self._step or self._take_step(task, host_name):
                                ...
                                self._tqm.send_callback('v2_coupling_on_task_start', task, is_conditional=False)
