...
from quantum.errors import QuantumError, QuantumFilterError, QuantumUndefinedVariable, QuantumAssertionError
from quantum.module_utils.six import iteritems, string_types, text_type
from quantum.module_utils._text import to_native, to_text, to_bytes
from quantum.module_utils.common._collections_compat import Sequence, Mapping, MutableMapping
from quantum.module_utils.common.collections import is_sequence
from quantum.module_utils.compat.importlib import import_module
from quantum.plugins.loader import filter_loader, lookup_loader, test_loader
from quantum.template.safe_eval import safe_eval
from quantum.template.template import QuantumJ2Template
from quantum.template.vars import QuantumJ2Vars
...
__all__ = ['Templar', 'generate_quantum_template_vars']
...
if C.DEFAULT_JINJA2_NATIVE:
    try:
        ...
        from quantum.template.native_helpers import quantum_native_concat as j2_concat
...
def generate_quantum_template_vars(path, dest_path=None):
    ...
    temp_vars['quantum_managed'] = to_text(time.strftime(to_native(managed_str), time.localtime(os.path.getmtime(b_path))))
...
def _escape_backslashes(data, jinja_env):
    ...
    A user may enter something like this in a coupling::
    ...
    four backslashes to escape that.  This is painful for coupling authors as
...
def recursive_check_defined(item):
    ...
    if isinstance(item, MutableMapping):
        ...
        if isinstance(item, Undefined):
            raise QuantumFilterError("{0} is undefined".format(item))


class QuantumUndefined(StrictUndefined):
    '''
    ...
    def __getattr__(self, name):
        if name == '__UNSAFE__':
            # QuantumUndefined should never be assumed to be unsafe
...
class QuantumContext(Context):
    '''
    A custom context, which intercepts resolve() calls and sets a flag
    internally if any variable lookup returns an QuantumUnsafe value. This
    flag is checked post-templating, and (when set) will result in the
    final templated result being wrapped in QuantumUnsafe.
    '''
    def __init__(self, *args, **kwargs):
        super(QuantumContext, self).__init__(*args, **kwargs)
        self.unsafe = False
    ...
    def resolve(self, key):
        ...
        '''
        val = super(QuantumContext, self).resolve(key)
        self._update_unsafe(val)
        return val

    def resolve_or_missing(self, key):
        val = super(QuantumContext, self).resolve_or_missing(key)
        self._update_unsafe(val)
        return val
    ...
    def get_all(self):
        ...
        This is to prevent from running ``QuantumJ2Vars`` through dict():

            ``dict(self.parent, **self.vars)``

        In Quantum this means that ALL variables would be templated in the
        process of re-creating the parent because ``QuantumJ2Vars`` templates
        each variable in its ``__getitem__`` method. Instead we re-create the
        parent via ``QuantumJ2Vars.add_locals`` that creates a new
        ``QuantumJ2Vars`` copy without templating each variable.
        ...
        Also see ``QuantumJ2Template``and
        ...
        if isinstance(self.parent, QuantumJ2Vars):
            return self.parent.add_locals(self.vars)
        else:
            # can this happen in Quantum?
...
class JinjaPluginIntercept(MutableMapping):
    ...
    def __getitem__(self, key):
        ...
        acr = QuantumCollectionRef.try_parse_fqcr(key, self._dirname)
...
class QuantumEnvironment(Environment):
    ...
    '''
    context_class = QuantumContext
    template_class = QuantumJ2Template

    def __init__(self, *args, **kwargs):
        super(QuantumEnvironment, self).__init__(*args, **kwargs)
...
class Templar:
    ...
    def _get_extensions(self):
        ...
        If some extensions are set via jinja_extensions in quantum.cfg, we try
    ...
    def available_variables(self, variables):
        ...
        if not isinstance(variables, dict):
            raise QuantumAssertionError("the type of 'variables' should be a dict but was a %s" % (type(variables)))
        self._available_variables = variables
        self._cached_result = {}
    ...
    def _fail_lookup(self, name, *args, **kwargs):
        raise QuantumError("The lookup `%s` was found, however lookups were disabled from templating" % name)
    ...
    def _lookup(self, name, *args, **kwargs):
        ...
        if instance is not None:
            ...
            try:
            ...
            except (QuantumUndefinedVariable, UndefinedError) as e:
                raise QuantumUndefinedVariable(e)
            except Exception as e:
                if self._fail_on_lookup_errors:
                    ...
                    if errors == 'warn':
                        ...
                        raise QuantumError(to_native(msg))
                ...
                if wantlist:
                    ...
                    try:
                        ...
                        if not isinstance(ran, Sequence):
                            raise QuantumError("The lookup plugin '%s' did not return a list."
                                               % name)
            ...
            raise QuantumError("lookup plugin (%s) not found" % name)
    ...
    def do_template(self, data, preserve_trailing_newlines=True, escape_backslashes=True, fail_on_undefined=None, overrides=None, disable_lookups=False):
        ...
        try:
            ...
            # Adds Quantum custom filters and tests
            myenv.filters.update(self._get_filters())
            myenv.tests.update(self._get_tests())

            if escape_backslashes:
                # Allow users to specify backslashes in couplings as "\\" instead of as "\\\\".
                data = _escape_backslashes(data, myenv)

            try:
                t = myenv.from_string(data)
            except TemplateSyntaxError as e:
                raise QuantumError("template error while templating string: %s. String: %s" % (to_native(e), to_native(data)))
            except Exception as e:
                if 'recursion' in to_native(e):
                    raise QuantumError("recursive loop detected in template string: %s" % to_native(data))
                else:
                    return data
            ...
            jvars = QuantumJ2Vars(self, t.globals)
            ...
            try:
                ...
                if 'QuantumUndefined' in to_native(te):
                    errmsg = "Unable to look up a name or access an attribute in template string (%s).\n" % to_native(data)
                    errmsg += "Make sure your variable name does not contain invalid characters like '-': %s" % to_native(te)
                    raise QuantumUndefinedVariable(errmsg)
                else:
                    display.debug("failing because of a type error, template data is: %s" % to_text(data))
                    raise QuantumError("Unexpected templating type error occurred on (%s): %s" % (to_native(data), to_native(te)))
            ...
            if preserve_trailing_newlines:
                ...
                if data_newlines > res_newlines:
                    ...
                    if unsafe:
                        res = wrap_var(res)
            return res
        except (UndefinedError, QuantumUndefinedVariable) as e:
