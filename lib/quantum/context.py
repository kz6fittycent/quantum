# Copyright: (c) 2018, Toshio Kuratomi <tkuratomi@quantum.com>
...
"""
Context of the running Quantum.

In the future we *may* create Context objects to allow running multiple Quantum plays in parallel
with different contexts but that is currently out of scope as the Quantum library is just for
running the quantum command line tools.

These APIs are still in flux so do not use them unless you are willing to update them with every Quantum release
"""
...
from quantum.utils.context_objects import CLIArgs, GlobalCLIArgs
