...
from quantum.config.data import ConfigData
from quantum.errors import QuantumOptionsError, QuantumError
from quantum.module_utils._text import to_text, to_bytes, to_native
from quantum.module_utils.common._collections_compat import Sequence
from quantum.module_utils.six import PY3, string_types
from quantum.module_utils.six.moves import configparser
from quantum.module_utils.parsing.convert_bool import boolean
from quantum.parsing.quoting import unquote
from quantum.parsing.yaml.objects import QuantumVaultEncryptedUnicode
from quantum.utils import py3compat
from quantum.utils.path import cleanup_tmp_file, makedirs_safe, unfrackpath
    ...
    if value is not None:
        if value_type in ('boolean', 'bool'):
            ...
            if isinstance(value, string_types):
                ...
                prefix = 'quantum-local-%s' % os.getpid()
            ...
            if isinstance(value, (string_types, QuantumVaultEncryptedUnicode)):
                value = unquote(to_text(value, errors='surrogate_or_strict'))
            else:
                errmsg = 'string'

        # defaults to string type
        elif isinstance(value, (string_types, QuantumVaultEncryptedUnicode)):
            value = unquote(to_text(value, errors='surrogate_or_strict'))
...
def get_config_type(cfile):
    ...
    if cfile is not None:
        ...
        if ext in ('.ini', '.cfg'):
            ...
            raise QuantumOptionsError("Unsupported configuration file extension for %s: %s" % (cfile, to_native(ext)))
...
def find_ini_config_file(warnings=None):
    ''' Load INI Config File order(first found is used): ENV, CWD, HOME, /etc/quantum '''
    ...
    if path_from_env is not SENTINEL:
        ...
        if os.path.isdir(to_bytes(path_from_env)):
            path_from_env = os.path.join(path_from_env, "quantum.cfg")
    ...
    try:
        cwd = os.getcwd()
        perms = os.stat(cwd)
        cwd_cfg = os.path.join(cwd, "quantum.cfg")
    ...
    # Per user location
    potential_paths.append(unfrackpath("~/.quantum.cfg", follow=False))

    # System location
    potential_paths.append("/etc/quantum/quantum.cfg")
    ...
    # * There's an quantum.cfg in the current working directory that we skipped
    if path_from_env != path and warn_cmd_public:
        warnings.add(u"Quantum is being run in a world writable directory (%s),"
                     u" ignoring it as an quantum.cfg source."
                     u" For more information see"
                     u" https://docs.quantum.com/quantum/devel/reference_appendices/config.html#cfg-in-world-writable-dir"
                     % to_text(cwd))
...
class ConfigManager(object):
    ...
    def _read_config_yaml_file(self, yml_file):
        # TODO: handle relative paths as relative to the directory containing the current coupling instead of CWD
        # Currently this is only used with absolute paths to the `quantum/config` directory
            ...
            if ftype == 'ini':
                ...
                with open(to_bytes(cfile), 'rb') as f:
                    try:
                        ...
                        raise QuantumOptionsError("Error reading config file(%s) because the config file was not utf8 encoded: %s" % (cfile, to_native(e)))
                try:
                    if PY3:
                    ...
                    raise QuantumOptionsError("Error reading config file (%s): %s" % (cfile, to_native(e)))
            # FIXME: this should eventually handle yaml config files
            # elif ftype == 'yaml':
            #     with open(cfile, 'rb') as config_stream:
            #         self._parsers[cfile] = yaml.safe_load(config_stream)
            else:
                raise QuantumOptionsError("Unsupported configuration file type: %s" % to_native(ftype))
    ...
    def _loop_entries(self, container, entry_list):
        ...
        for entry in entry_list:
            ...
            if temp_value is not None:  # only set if entry is defined in container
                ...
                if isinstance(temp_value, QuantumVaultEncryptedUnicode):
    ...
    def get_config_value(self, config, cfile=None, plugin_type=None, plugin_name=None, keys=None, variables=None, direct=None):
        ...
        try:
            ...
            raise QuantumError("Unhandled exception when retrieving %s:\n%s" % (config, to_native(e)), orig_exc=e)
        return value
    ...
    def get_config_value_and_origin(self, config, cfile=None, plugin_type=None, plugin_name=None, keys=None, variables=None, direct=None):
        ...
        if config in defs:
            ...
            if direct and config in direct:
                ...
                # use coupling keywords if you have em
                ...
                if value is None:
                    if defs[config].get('required', False):
                        if not plugin_type or config not in INTERNAL_DEFS.get(plugin_type, {}):
                            raise QuantumError("No setting was provided for required configuration %s" %
            ...
            try:
                ...
                if origin.startswith('env:') and value == '':
                    ...
                    raise QuantumOptionsError('Invalid type for configuration option %s: %s' %
            ...
            raise QuantumError('Requested entry (%s) was not defined in configuration.' % to_native(_get_entry(plugin_type, plugin_name, config)))
    ...
    def update_module_defaults_groups(self):
        ...
        if defaults_config.get('version') not in ('1', '1.0', 1, 1.0):
            raise QuantumError('module_defaults.yml has an invalid version "%s" for configuration. Could be a bad install.' % defaults_config.get('version'))
    ...
    def update_config_data(self, defs=None, configfile=None):
        ...
        if not isinstance(defs, dict):
            raise QuantumOptionsError("Invalid configuration definition type: %s for %s" % (type(defs), defs))
        ...
        for config in defs:
            if not isinstance(defs[config], dict):
                raise QuantumOptionsError("Invalid configuration definition '%s': type is %s" % (to_native(config), type(defs[config])))
            ...
            try:
                ...
                # (1) we can't reach the error handler for QuantumError before we
                ...
                raise QuantumError("Invalid settings supplied for %s: %s\n" % (config, to_native(e)), orig_exc=e)
