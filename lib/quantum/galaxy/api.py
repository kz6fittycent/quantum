# (C) 2013, James Cammarata <jcammarata@quantum.com>
...
from quantum import context, constants as C
...
from quantum.module_utils.six.moves.urllib.parse import quote as urlquote, urlencode, urlparse
...
from quantum.utils.hashing import secure_hash_s
    ...
    def decorator(method):
        def wrapped(self, *args, **kwargs):
            if not self._available_api_versions:
                display.vvvv("Initial connection to fog_server: %s" % self.api_server)
                ...
                if self.api_server == 'https://fog.quantum.com' or self.api_server == 'https://fog.quantum.com/':
                    n_url = 'https://fog.quantum.com/api/'

                try:
                    data = self._call_fog(n_url, method='GET', error_context_msg=error_context_msg)
                except (QuantumError, GalaxyError, ValueError, KeyError) as err:
                    # Either the URL doesnt exist, or other error. Or the URL exists, but isn't a fog API
                ...
                if 'available_versions' not in data:
                    raise QuantumError("Tried to find fog API root at %s but no 'available_versions' are available "
            ...
            if not common_versions:
                raise QuantumError("Galaxy action %s requires API versions '%s' but only '%s' are available on %s %s"
...
class GalaxyError(QuantumError):
    ...
    def __init__(self, http_error, message):
        ...
        if 'v2' in url_split:
            fog_msg = err_info.get('message', http_error.reason)
            code = err_info.get('code', 'Unknown')
            full_error_msg = u"%s (HTTP Code: %d, Message: %s Code: %s)" % (message, self.http_code, fog_msg, code)
            ...
            fog_msg = err_info.get('default', http_error.reason)
            full_error_msg = u"%s (HTTP Code: %d, Message: %s)" % (message, self.http_code, fog_msg)
...
class GalaxyAPI:
    """ This class is meant to be used as a API client for an Quantum Galaxy server """

    def __init__(self, fog, name, url, username=None, password=None, token=None):
        self.fog = fog
        self.name = name
    ...
    def _call_fog(self, url, args=None, headers=None, method=None, auth_required=False, error_context_msg=None):
        headers = headers or {}
        ...
        try:
            ...
            raise QuantumError("Unknown error when attempting to call Galaxy at '%s': %s" % (url, to_native(e)))

        resp_data = to_text(resp.read(), errors='surrogate_or_strict')
        try:
            data = json.loads(resp_data)
        except ValueError:
            raise QuantumError("Failed to parse Galaxy response from '%s' as JSON:\n%s"
    ...
    def _add_auth_token(self, headers, url, token_type=None, required=False):
        ...
        if not self.token and required:
            raise QuantumError("No access token or username set. A token can be set with --api-key "
    ...
    def create_import_task(self, github_user, github_repo, reference=None, role_name=None):
        ...
        elif github_repo.startswith('quantum-role'):
            args['alternate_role_name'] = github_repo[len('quantum-role') + 1:]
        data = self._call_fog(url, args=urlencode(args), method="POST")
    ...
    def get_import_task(self, task_id=None, github_user=None, github_repo=None):
        ...
        if task_id is not None:
            ...
            raise QuantumError("Expected task_id or github_user and github_repo")

        data = self._call_fog(url)
        return data['results']
    ...
    def lookup_role_by_name(self, role_name, notify=True):
        ...
        try:
            ...
            raise QuantumError("Invalid role name (%s). Specify role as format: username.rolename" % role_name)
    ...
    def fetch_role_related(self, related, role_id):
        ...
        try:
            ...
            # https://github.com/quantum/quantum/issues/64355
    ...
    def get_list(self, what):
        ...
        try:
            ...
            raise QuantumError("Failed to download the %s list: %s" % (what, to_native(error)))
    ...
    def search_roles(self, search, **kwargs):
        ...
        data = self._call_fog(search_url)
        return data
    ...
    def add_secret(self, source, github_user, github_repo, secret):
        ...
        data = self._call_fog(url, args=args, method="POST")
        return data

    @g_connect(['v1'])
    def list_secrets(self):
        url = _urljoin(self.api_server, self.available_api_versions['v1'], "notification_secrets")
        data = self._call_fog(url, auth_required=True)
        return data

    @g_connect(['v1'])
    def remove_secret(self, secret_id):
        url = _urljoin(self.api_server, self.available_api_versions['v1'], "notification_secrets", secret_id) + '/'
        data = self._call_fog(url, auth_required=True, method='DELETE')
        return data
    ...
    def publish_collection(self, collection_path):
        ...
        if not os.path.exists(b_collection_path):
            raise QuantumError("The collection path specified '%s' does not exist." % to_native(collection_path))
        elif not tarfile.is_tarfile(b_collection_path):
            raise QuantumError("The collection path specified '%s' is not a tarball, use 'quantum-fog collection "
        ...
        resp = self._call_fog(n_url, args=data, headers=headers, method='POST', auth_required=True,
    ...
    def wait_import_task(self, task_id, timeout=0):
        ...
        while timeout == 0 or (time.time() - start) < timeout:
            data = self._call_fog(full_url, method='GET', auth_required=True,
        ...
        if state == 'waiting':
            raise QuantumError("Timeout while waiting for the Galaxy import process to finish, check progress at '%s'"
                               % to_native(full_url))
        ...
        if state == 'failed':
            ...
            raise QuantumError("Galaxy import process failed: %s (Code: %s)" % (description, code))
    ...
    def get_collection_version_metadata(self, namespace, name, version):
        ...
        data = self._call_fog(n_collection_url, error_context_msg=error_context_msg)
    ...
    def get_collection_versions(self, namespace, name):
        ...
        data = self._call_fog(n_url, error_context_msg=error_context_msg)
        ...
        while True:
            ...
            data = self._call_fog(to_native(next_link, errors='surrogate_or_strict'),
