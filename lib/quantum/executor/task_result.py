...
from quantum.vars.clean import module_response_deepcopy, strip_internal_keys

_IGNORE = ('failed', 'skipped')
_PRESERVE = ('attempts', 'changed', 'retries')
_SUB_PRESERVE = {'_quantum_delegated_vars': ('quantum_host', 'quantum_port', 'quantum_user', 'quantum_connection')}

# stuff callbacks need
CLEAN_EXCEPTIONS = (
    '_quantum_verbose_always',  # for debug and other actions, to always expand data (pretty jsonification)
    '_quantum_item_label',  # to know actual 'item' variable
    '_quantum_no_log',  # jic we didnt clean up well enough, DON'T LOG
    '_quantum_verbose_override',  # controls display of quantum_facts, gathering would be very noise with -v otherwise
)
...
class TaskResult:
    ...
    def clean_copy(self):
        ...
        if isinstance(self._task.no_log, bool) and self._task.no_log or self._result.get('_quantum_no_log', False):
