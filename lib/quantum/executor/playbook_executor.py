...
from quantum.plugins.loader import become_loader, connection_loader, shell_loader
from quantum.coupling import Playbook
from quantum.template import Templar
from quantum.utils.helpers import pct_to_int
from quantum.utils.path import makedirs_safe
from quantum.utils.ssh_functions import check_for_controlpersist
...
class PlaybookExecutor:
    ...
    '''
    This is the primary class for executing couplings, and thus the
    basis for bin/quantum-coupling operation.
    '''

    def __init__(self, couplings, inventory, variable_manager, loader, passwords):
        ...
        # Note: We run this here to cache whether the default quantum ssh
        # executable supports control persist.  Sometime in the future we may
        # need to enhance this to check that quantum_ssh_executable specified
    ...
    def run(self):
        '''
        Run the given coupling, based on the settings in the play which
        ...
        try:
            ...
            for coupling_path in self._couplings:
                pb = Playbook.load(coupling_path, variable_manager=self._variable_manager, loader=self._loader)
                # FIXME: move out of inventory self._inventory.set_coupling_basedir(os.path.realpath(os.path.dirname(coupling_path)))

                if self._tqm is None:  # we are doing a listing
                    entry = {'coupling': coupling_path}
                    entry['plays'] = []
                else:
                    # make sure the tqm has callbacks loaded
                    self._tqm.load_callbacks()
                    self._tqm.send_callback('v2_coupling_on_start', pb)

                i = 1
                plays = pb.get_plays()
                display.vv(u'%d plays in %s' % (len(plays), to_text(coupling_path)))
                ...
                for play in plays:
                    ...
                    if play.vars_prompt:
                        for var in play.vars_prompt:
                            ...
                            if vname not in self._variable_manager.extra_vars:
                                if self._tqm:
                                    self._tqm.send_callback('v2_coupling_on_vars_prompt', vname, private, prompt, encrypt, confirm, salt_size, salt,
                                                            default, unsafe)
                    ...
                    if self._tqm is None:
                        ...
                        if len(batches) == 0:
                            self._tqm.send_callback('v2_coupling_on_play_start', play)
                            self._tqm.send_callback('v2_coupling_on_no_hosts_matched')
                ...
                if entry:
                    entrylist.append(entry)  # per coupling

                # send the stats callback for this coupling
                    ...
                    if C.RETRY_FILES_ENABLED:
                        ...
                        if len(retries) > 0:
                            if C.RETRY_FILES_SAVE_PATH:
                                ...
                                basedir = os.path.dirname(os.path.abspath(coupling_path))
                            else:
                                basedir = '~/'

                            (retry_name, _) = os.path.splitext(os.path.basename(coupling_path))
                            filename = os.path.join(basedir, "%s.retry" % retry_name)
                            if self._generate_retry_inventory(filename, retries):
                                display.display("\tto retry, use: --limit @%s\n" % filename)

                    self._tqm.send_callback('v2_coupling_on_stats', self._tqm._stats)

                # if the last result wasn't zero, break out of the coupling file name loop
                if result != 0:
                    break
    ...
    def _generate_retry_inventory(self, retry_path, replay_hosts):
        '''
        Called when a coupling run fails. It generates an inventory which allows
