...
from quantum.executor.play_iterator import PlayIterator
from quantum.executor.stats import AggregateStats
...
from quantum.plugins.loader import callback_loader, strategy_loader, module_loader
...
from quantum.vars.reserved import warn_if_reserved
from quantum.utils.display import Display
from quantum.utils.multiprocessing import context as multiprocessing_context
...
class TaskQueueManager:
    ...
    '''
    This class handles the multiprocessing requirements of Quantum by
    ...
    def __init__(self, inventory, variable_manager, loader, passwords, stdout_callback=None, run_additional_callbacks=True, run_tree=False, forks=None):
        ...
        try:
            ...
            raise QuantumError("Unable to use multiprocessing, this is normally caused by lack of access to /dev/shm: %s" % to_native(e))
    ...
    def load_callbacks(self):
        ...
        if isinstance(self._stdout_callback, CallbackBase):
            ...
            if self._stdout_callback not in callback_loader:
                raise QuantumError("Invalid callback for stdout specified: %s" % self._stdout_callback)
            else:
                self._stdout_callback = callback_loader.get(self._stdout_callback)
                self._stdout_callback.set_options()
                stdout_callback_loaded = True
        else:
            raise QuantumError("callback must be an instance of CallbackBase or the name of a callback plugin")
        ...
        for callback_plugin in callback_list:
            ...
            if callback_type == 'stdout':
                ...
                # 2.x plugins shipped with quantum should require whitelisting, older or non shipped should load automatically
                continue
    ...
    def run(self, play):
        ...
        operates like classic Quantum by keeping all hosts in lock-step with
        ...
        self.send_callback('v2_coupling_on_play_start', new_play)
        ...
        if strategy is None:
            raise QuantumError("Invalid play strategy specified: %s" % new_play.strategy, obj=play._ds)
