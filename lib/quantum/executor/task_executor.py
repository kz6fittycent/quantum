...
from quantum.errors import QuantumError, QuantumParserError, QuantumUndefinedVariable, QuantumConnectionFailure, QuantumActionFail, QuantumActionSkip
from quantum.executor.task_result import TaskResult
...
from quantum.module_utils.six import iteritems, string_types, binary_type
from quantum.module_utils.six.moves import xrange
from quantum.module_utils._text import to_text, to_native
from quantum.module_utils.connection import write_to_file_descriptor
from quantum.coupling.conditional import Conditional
from quantum.coupling.task import Task
from quantum.plugins.loader import become_loader, cliconf_loader, connection_loader, httpapi_loader, netconf_loader, terminal_loader
from quantum.template import Templar
from quantum.utils.collection_loader import QuantumCollectionLoader
from quantum.utils.listify import listify_lookup_plugin_terms
from quantum.utils.unsafe_proxy import to_unsafe_text, wrap_var
from quantum.vars.clean import namespace_facts, clean_facts
from quantum.utils.display import Display
from quantum.utils.vars import combine_vars, isidentifier
...
class TaskExecutor:
    ...
    def run(self):
        ...
        try:
                ...
                if len(items) > 0:
                    ...
                    for item in item_results:
                        ...
                        if 'failed' in item and item['failed']:
                            item_ignore = item.pop('_quantum_ignore_errors')
            ...
            return dict(failed=True, msg=wrap_var(to_text(e, nonstring='simplerepr')), _quantum_no_log=self._play_context.no_log)
        except Exception as e:
            return dict(failed=True, msg='Unexpected failure during module execution.', exception=to_text(traceback.format_exc()),
                        stdout='', _quantum_no_log=self._play_context.no_log)
        finally:
            try:
    ...
    def _get_loop_items(self):
        ...
        self._job_vars['quantum_search_path'] = self._task.get_search_path()

        # ensure basedir is always in (dwim already searches here but we need to display it)
        if self._loader.get_basedir() not in self._job_vars['quantum_search_path']:
            self._job_vars['quantum_search_path'].append(self._loader.get_basedir())

        templar = Templar(loader=self._loader, shared_loader_obj=self._shared_loader_obj, variables=self._job_vars)
        items = None
        loop_cache = self._job_vars.get('_quantum_loop_cache')
        if loop_cache is not None:
            # _quantum_loop_cache may be set in `get_vars` when calculating `delegate_to`
            ...
            if self._task.loop_with in self._shared_loader_obj.lookup_loader:
                ...
                raise QuantumError("Unexpected failure in finding the lookup named '%s' in the available lookup plugins" % self._task.loop_with)
    ...
    def _run_loop(self, items):
        ...
        for item_index, item in enumerate(items):
            task_vars['quantum_loop_var'] = loop_var

            task_vars[loop_var] = item
            if index_var:
                task_vars['quantum_index_var'] = index_var
                task_vars[index_var] = item_index

            if extended:
                task_vars['quantum_loop'] = {
                    'allitems': items,
                    'index': item_index + 1,
                    'index0': item_index,
                    'first': item_index == 0,
                ...
                try:
                    task_vars['quantum_loop']['nextitem'] = items[item_index + 1]
                except IndexError:
                    pass
                if item_index - 1 >= 0:
                    task_vars['quantum_loop']['previtem'] = items[item_index - 1]
            ...
            if loop_pause and ran_once:
                try:
                    ...
                    raise QuantumError('Invalid pause value: %s, produced error: %s' % (loop_pause, to_native(e)))
            else:
                ran_once = True
            ...
            except QuantumParserError as e:
            ...
            res[loop_var] = item
            res['quantum_loop_var'] = loop_var
            if index_var:
                res[index_var] = item_index
                res['quantum_index_var'] = index_var
            if extended:
                res['quantum_loop'] = task_vars['quantum_loop']

            res['_quantum_item_result'] = True
            res['_quantum_ignore_errors'] = task_fields.get('ignore_errors')

            # gets templated here unlike rest of loop_control fields, depends on loop_var above
            try:
                res['_quantum_item_label'] = templar.template(label, cache=False)
            except QuantumUndefinedVariable as e:
                res.update({
                    'failed': True,
    ...
    def _execute(self, variables=None):
        ...
        try:
            if not self._task.evaluate_conditional(templar, variables):
                ...
                return dict(changed=False, skipped=True, skip_reason='Conditional result was False', _quantum_no_log=self._play_context.no_log)
        except QuantumError:
        ...
        try:
            ...
            return dict(changed=False, failed=True, _quantum_no_log=self._play_context.no_log, exception=to_text(traceback.format_exc()))
        if '_variable_params' in self._task.args:
            variable_params = self._task.args.pop('_variable_params')
            if isinstance(variable_params, dict):
                if C.INJECT_FACTS_AS_VARS:
                    display.warning("Using a variable for a task's 'args' is unsafe in some situations "
                                    "(see https://docs.quantum.com/quantum/devel/reference_appendices/faq.html#argsplat-unsafe)")
                variable_params.update(self._task.args)
                self._task.args = variable_params

        if self._task.delegate_to:
            # use vars from delegated host (which already include task vars) instead of original host
            cvars = variables.get('quantum_delegated_vars', {}).get(self._task.delegate_to, {})
        ...
        for attempt in xrange(1, retries + 1):
            ...
            except QuantumActionSkip as e:
                return dict(skipped=True, msg=to_text(e))
            except QuantumActionFail as e:
            ...
            # preserve no log
            result["_quantum_no_log"] = self._play_context.no_log

            # update the local copy of vars with the registered value, if specified,
            # or any facts which may have been generated by the module execution
            if self._task.register:
                if not isidentifier(self._task.register):
                    raise QuantumError("Invalid variable name in 'register' specified: '%s'" % self._task.register)
            ...
            if 'quantum_facts' in result:
                if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                    vars_copy.update(result['quantum_facts'])
                else:
                    # TODO: cleaning of facts should eventually become part of taskresults instead of vars
                    af = wrap_var(result['quantum_facts'])
                    vars_copy['quantum_facts'] = combine_vars(vars_copy.get('quantum_facts', {}), namespace_facts(af))
                    if C.INJECT_FACTS_AS_VARS:
        ...
        if 'quantum_facts' in result:
            if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                variables.update(result['quantum_facts'])
            else:
                # TODO: cleaning of facts should eventually become part of taskresults instead of vars
                af = wrap_var(result['quantum_facts'])
                variables['quantum_facts'] = combine_vars(variables.get('quantum_facts', {}), namespace_facts(af))
                if C.INJECT_FACTS_AS_VARS:
        ...
        if self._task.notify is not None:
            result['_quantum_notify'] = self._task.notify

        # add the delegated vars to the result, so we can reference them
        # on the results side without having to do any further templating
        # also now add conneciton vars results when delegating
        if self._task.delegate_to:
            result["_quantum_delegated_vars"] = {'quantum_delegated_host': self._task.delegate_to}
            for k in plugin_vars:
                result["_quantum_delegated_vars"][k] = cvars.get(k)

            for requireshed in ('quantum_host', 'quantum_port', 'quantum_user', 'quantum_connection'):
                if requireshed not in result["_quantum_delegated_vars"] and requireshed in cvars:
                    result["_quantum_delegated_vars"][requireshed] = cvars.get(requireshed)
    ...
    def _poll_async_result(self, result, templar, task_vars=None):
        ...
        async_jid = result.get('quantum_job_id')
        ...
        while time_left > 0:
            ...
            try:
                ...
                if (int(async_result.get('finished', 0)) == 1 or
                        ('failed' in async_result and async_result.get('_quantum_parsed', False)) or
        ...
        if int(async_result.get('finished', 0)) != 1:
            if async_result.get('_quantum_parsed'):
    ...
    def _get_become(self, name):
        ...
        if not become:
            raise QuantumError("Invalid become method specified, could not find matching plugin: '%s'. "
                               "Use `quantum-doc -t become -l` to list available plugins." % name)
        return become
    ...
    def _get_connection(self, cvars, templar):
        ...
        if cvars.get('quantum_connection') is not None:
            self._play_context.connection = templar.template(cvars['quantum_connection'])
        else:
        ...
        connection = self._shared_loader_obj.connection_loader.get(
            ...
            quantum_coupling_pid=to_text(os.getppid())
        )

        if not connection:
            raise QuantumError("the connection plugin '%s' was not found" % conn_type)

        # load become plugin if needed
        if cvars.get('quantum_become') is not None:
            become = boolean(templar.template(cvars['quantum_become']))
        else:
            become = self._task.become

        if become:
            if cvars.get('quantum_become_method'):
                become_plugin = self._get_become(templar.template(cvars['quantum_become_method']))
            else:
    ...
    def _set_connection_options(self, variables, templar):
        ...
        if getattr(self._connection, 'allow_extras', False):
            for k in variables:
                if k.startswith('quantum_%s_' % self._connection._load_name) and k not in options:
    ...
    def _get_action_handler(self, connection, templar):
        ...
        if self._shared_loader_obj.action_loader.has_plugin(self._task.action, collection_list=collections):
            ...
            # FUTURE: once we're comfortable with collections impl, preface this action with quantum.builtin so it can't be hijacked
            handler_name = 'normal'
        ...
        if not handler:
            raise QuantumError("the handler '%s' was not found" % handler_name)
...
def start_connection(play_context, variables, task_uuid=None):
    ...
    for dirname in candidate_paths:
        quantum_connection = os.path.join(dirname, 'quantum-connection')
        if os.path.isfile(quantum_connection):
            break
    else:
        raise QuantumError("Unable to find location of 'quantum-connection'. "
                           "Please set or check the value of ANSIBLE_CONNECTION_PATH")

    env = os.environ.copy()
    env.update({
        # HACK; most of these paths may change during the controller's lifetime
        # (eg, due to late dynamic role includes, multi-coupling execution), without a way
        # to invalidate/update, quantum-connection won't always see the same plugins the controller
        # can.
        'ANSIBLE_BECOME_PLUGINS': become_loader.print_paths(),
        'ANSIBLE_CLICONF_PLUGINS': cliconf_loader.print_paths(),
        'ANSIBLE_COLLECTIONS_PATHS': os.pathsep.join(QuantumCollectionLoader().n_collection_paths),
    ...
    p = subprocess.Popen(
        [python, quantum_connection, to_text(os.getppid()), to_text(task_uuid)],
    ...
    if 'error' in result:
        ...
        raise QuantumError(result['error'])
