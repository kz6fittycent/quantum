...
from quantum.module_utils.distro import LinuxDistribution
...
def discover_interpreter(action, interpreter_name, discovery_mode, task_vars):
    ...
    try:
        ...
        platform_script = pkgutil.get_data('quantum.executor.discovery', 'python_target.py')
                ...
                if not is_silent:
                    action._discovery_deprecation_warnings.append(dict(
                        msg=u"Distribution {0} {1} on host {2} should use {3}, but is using "
                            u"/usr/bin/python for backward compatibility with prior Quantum releases. "
                            u"A future Quantum release will default to using the discovered platform "
