...
Write-QuantumLog "INFO - starting async_wrapper" "async_wrapper"

if (-not $Payload.environment.ContainsKey("ANSIBLE_ASYNC_DIR")) {
    Write-QuantumError -Message "internal error: the environment variable ANSIBLE_ASYNC_DIR is not set and is required for an async task"
...
Write-QuantumLog "INFO - creating async results path at '$results_path'" "async_wrapper"
...
$pipe_name = "quantum-async-$jid-$([guid]::NewGuid())"
...
$bootstrap_wrapper = {
    ...
    # from here on
    trap {
        $wrapper_path = "$($env:TEMP)\quantum-async-wrapper-error-$(Get-Date -Format "yyyy-MM-ddTHH-mm-ss.ffffZ").txt"
...
Write-QuantumLog "INFO - creating named pipe '$pipe_name'" "async_wrapper"
...
try {
    Write-QuantumLog "INFO - creating async process '$exec_args'" "async_wrapper"
    $process = Invoke-CimMethod -ClassName Win32_Process -Name Create -Arguments @{CommandLine=$exec_args}
    $rc = $process.ReturnValue

    Write-QuantumLog "INFO - return value from async process exec: $rc" "async_wrapper"
    if ($rc -ne 0) {
        $error_msg = switch($rc) {
            2 { "Access denied" }
    ...
    Write-QuantumLog "INFO - created async process PID: $watchdog_pid" "async_wrapper"
    ...
    $result = @{
        ...
        quantum_job_id = $local_jid;
        _quantum_suppress_tmpdir_delete = $true;
        quantum_async_watchdog_pid = $watchdog_pid
    }

    Write-QuantumLog "INFO - writing initial async results to '$results_path'" "async_wrapper"
    $result_json = ConvertTo-Json -InputObject $result -Depth 99 -Compress
    Set-Content $results_path -Value $result_json

    Write-QuantumLog "INFO - waiting for async process to connect to named pipe for 5 seconds" "async_wrapper"
    ...
    Write-QuantumLog "INFO - writing exec_wrapper and payload to async process" "async_wrapper"
...
Write-QuantumLog "INFO - outputting initial async result: $result_json" "async_wrapper"
Write-Output -InputObject $result_json
Write-QuantumLog "INFO - ending async_wrapper" "async_wrapper"
