...
from quantum.plugins.loader import ps_module_utils_loader
...
class PSModuleDepFinder(object):
    ...
    def __init__(self):
        ...
        self._re_cs_module = [
            ...
            # 'using quantum_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            re.compile(to_bytes(r'(?i)^using\s((Quantum\..+)|'
                                r'(quantum_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+));\s*$')),
        ]

        self._re_cs_in_ps_module = [
            # Reference C# module_util in a PowerShell module
            # '#QuantumRequires -CSharpUtil Quantum.{name}'
            # '#QuantumRequires -CSharpUtil quantum_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            re.compile(to_bytes(r'(?i)^#\s*quantumrequires\s+-csharputil\s+((Quantum\..+)|'
                                r'(quantum_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+))')),
        ]

        self._re_ps_module = [
            # Original way of referencing a builtin module_util
            # '#Requires -Module Quantum.ModuleUtils.{name}
            re.compile(to_bytes(r'(?i)^#\s*requires\s+\-module(?:s?)\s*(Quantum\.ModuleUtils\..+)')),
            # New way of referencing a builtin and collection module_util
            # '#QuantumRequires -PowerShell quantum_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            # '#QuantumRequires -PowerShell Quantum.ModuleUtils.{name}'
            re.compile(to_bytes(r'(?i)^#\s*quantumrequires\s+-powershell\s+(((Quantum\.ModuleUtils\..+))|'
                                r'(quantum_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+))')),
        ]

        self._re_wrapper = re.compile(to_bytes(r'(?i)^#\s*quantumrequires\s+-wrapper\s+(\w*)'))
        self._re_ps_version = re.compile(to_bytes(r'(?i)^#requires\s+\-version\s+([0-9]+(\.[0-9]+){0,3})$'))
        self._re_os_version = re.compile(to_bytes(r'(?i)^#quantumrequires\s+\-osversion\s+([0-9]+(\.[0-9]+){0,3})$'))
        self._re_become = re.compile(to_bytes(r'(?i)^#quantumrequires\s+\-become$'))
    ...
    def scan_module(self, module_data, wrapper=False, powershell=True):
        ...
        if powershell:
            checks = [
                # PS module contains '#Requires -Module Quantum.ModuleUtils.*'
                # PS module contains '#QuantumRequires -Powershell Quantum.*' (or FQ collections module_utils ref)
                (self._re_ps_module, self.ps_modules, ".psm1"),
                # PS module contains '#QuantumRequires -CSharpUtil Quantum.*'
                (self._re_cs_in_ps_module, cs_utils, ".cs"),
            ]
        else:
            checks = [
                # CS module contains 'using Quantum.*;' or 'using quantum_collections.ns.coll.plugins.module_utils.*;'
    ...
    def scan_exec_script(self, name):
        # scans lib/quantum/executor/powershell for scripts used in the module
        # exec side. It also scans these scripts for any dependencies
        name = to_text(name)
        if name in self.exec_scripts.keys():
            return

        data = pkgutil.get_data("quantum.executor.powershell", name + ".ps1")
        if data is None:
            raise QuantumError("Could not find executor powershell script "
                               "for '%s'" % name)
    ...
    def _add_module(self, name, wrapper=False):
        ...
        if m.startswith("Quantum."):
            ...
            if not mu_path:
                raise QuantumError('Could not find imported module support code '
                                   'for \'%s\'' % m)
            ...
            try:
                ...
                if len(resource_paths) != 1:
                    ...
                    raise QuantumError("Internal error: Referenced module_util package '%s' contains 0 or multiple "
                                       "import locations when we only expect 1." % n_package_name)
                mu_path = os.path.join(resource_paths[0], n_resource_name)
            except OSError as err:
                if err.errno == errno.ENOENT:
                    raise QuantumError('Could not find collection imported module support code for \'%s\''
                                       % to_native(m))
                else:
                    raise
...
def _slurp(path):
    if not os.path.exists(path):
        raise QuantumError("imported module support code does not exist at %s"
...
def _create_powershell_wrapper(b_module_data, module_path, module_args,
    ...
    if substyle != 'script':
        # don't scan the module for util dependencies and other Quantum related
    ...
    # make sure Quantum.ModuleUtils.AddType is added if any C# utils are used
    if len(finder.cs_utils_wrapper) > 0 or len(finder.cs_utils_module) > 0:
        finder._add_module((b"Quantum.ModuleUtils.AddType", ".psm1"),
                           wrapper=False)
