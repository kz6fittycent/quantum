...
begin {
    ...
    $script:common_functions = {
        Function ConvertFrom-QuantumJson {
            <#
            .SYNOPSIS
        ...
        Function Format-QuantumException {
            <#
            .SYNOPSIS
...
"@
    ...
    $script:wrapper_functions = {
        Function Write-QuantumError {
            <#
            .SYNOPSIS
            Writes an error message to a JSON string in the format that Quantum
            ...
            if ($null -ne $ErrorRecord) {
                ...
                $result.exception = (Format-QuantumException -ErrorRecord $ErrorRecord)
            }
    ...
    Write-QuantumLog "INFO - starting exec_wrapper" "exec_wrapper"
    if (-not $json_raw) {
        Write-QuantumError -Message "internal error: no input given to PowerShell exec wrapper"
        exit 1
    }

    Write-QuantumLog "INFO - converting json raw to a payload" "exec_wrapper"
    $payload = ConvertFrom-QuantumJson -InputObject $json_raw
    ...
    if ($payload.min_os_version) {
        ...
        Write-QuantumLog "INFO - checking if actual os version '$actual_os_version' is less than the min os version '$min_os_version'" "exec_wrapper"
        if ($actual_os_version -lt $min_os_version) {
            Write-QuantumError -Message "internal error: This module cannot run on this OS as it requires a minimum version of $min_os_version, actual was $actual_os_version"
            exit 1
        }
    }
    if ($payload.min_ps_version) {
        ...
        Write-QuantumLog "INFO - checking if actual PS version '$actual_ps_version' is less than the min PS version '$min_ps_version'" "exec_wrapper"
        if ($actual_ps_version -lt $min_ps_version) {
            Write-QuantumError -Message "internal error: This module cannot run as it requires a minimum PowerShell version of $min_ps_version, actual was $actual_ps_version"
            exit 1
        }
    }
    ...
    Write-QuantumLog "INFO - running action $action" "exec_wrapper"
    ...
    try {
        ...
        Write-QuantumError -Message "internal error: failed to run exec_wrapper action $action" -ErrorRecord $_
        exit 1
    }
    Write-QuantumLog "INFO - ending exec_wrapper" "exec_wrapper"
}
