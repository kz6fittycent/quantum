...
<#
.SYNOPSIS
Invokes an Quantum module in a new Runspace. This cmdlet will output the
...
Write-QuantumLog "INFO - creating new PowerShell pipeline for $ModuleName" "module_wrapper"
...
if ($host.Name -eq "ConsoleHost") {
    Write-QuantumLog "INFO - setting console input encoding to UTF8 for $ModuleName" "module_wrapper"
    $ps.AddScript('[Console]::InputEncoding = New-Object Text.UTF8Encoding $false').AddStatement() > $null
}

# set the variables
foreach ($variable in $Variables) {
    Write-QuantumLog "INFO - setting variable '$($variable.Name)' for $ModuleName" "module_wrapper"
...
if ($Environment) {
    foreach ($env_kv in $Environment.GetEnumerator()) {
        Write-QuantumLog "INFO - setting environment '$($env_kv.Key)' for $ModuleName" "module_wrapper"
...
if ($Modules) {
    foreach ($module in $Modules.GetEnumerator()) {
        Write-QuantumLog "INFO - create module util '$($module.Key)' for $ModuleName" "module_wrapper"
...
if ($Breakpoints.Count -gt 0) {
    Write-QuantumLog "INFO - adding breakpoint to runspace that will run the modules" "module_wrapper"
...
Write-QuantumLog "INFO - start module exec with Invoke() - $ModuleName" "module_wrapper"
...
try {
    ...
    Write-QuantumError @error_params
...
Write-QuantumLog "INFO - module exec ended $ModuleName" "module_wrapper"
$stdout = $sb.ToString()
if ($stdout) {
...
if ($module_output.Count -gt 0) {
    ...
    Write-QuantumLog "INFO - using the output stream for module output - $ModuleName" "module_wrapper"
...
if ($null -ne $rc) {
    Write-QuantumLog "INFO - got an rc of $rc from $ModuleName exec" "module_wrapper"
...
if ($ps.HadErrors -or ($PSVersionTable.PSVersion.Major -lt 4 -and $ps.Streams.Error.Count -gt 0)) {
    Write-QuantumLog "WARN - module had errors, outputting error info $ModuleName" "module_wrapper"
    ...
    foreach ($err in $ps.Streams.Error) {
        $error_msg = Format-QuantumException -ErrorRecord $err

        # need to use the current hosts's UI class as we may not have
        # a console to write the stderr to, e.g. psrp
        Write-QuantumLog "WARN - error msg for for $($ModuleName):`r`n$error_msg" "module_wrapper"
