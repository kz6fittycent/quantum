...
#Requires -Module Quantum.ModuleUtils.AddType
#QuantumRequires -CSharpUtil Quantum.AccessToken
#QuantumRequires -CSharpUtil Quantum.Become

$ErrorActionPreference = "Stop"

Write-QuantumLog "INFO - starting become_wrapper" "become_wrapper"
...
Function Get-BecomeFlags($flags) {
    $logon_type = [Quantum.AccessToken.LogonType]::Interactive
    $logon_flags = [Quantum.Become.LogonFlags]::WithProfile
    ...
    foreach ($flag in $flag_split) {
        ...
        if ($flag_key -eq "logon_type") {
            $enum_details = @{
                enum = [Quantum.AccessToken.LogonType]
                flag_type = $flag_key
                value = $flag_value
            }
            ...
            $logon_flags = 0 -as [Quantum.Become.LogonFlags]
            foreach ($logon_flag_value in $logon_flag_values) {
                if ($logon_flag_value -eq "") {
                    continue
                }
                $enum_details = @{
                    enum = [Quantum.Become.LogonFlags]
                    flag_type = $flag_key
    ...
    return $logon_type, [Quantum.Become.LogonFlags]$logon_flags
}

Write-QuantumLog "INFO - loading C# become code" "become_wrapper"
$add_type_b64 = $Payload.powershell_modules["Quantum.ModuleUtils.AddType"]
$add_type = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($add_type_b64))
New-Module -Name Quantum.ModuleUtils.AddType -ScriptBlock ([ScriptBlock]::Create($add_type)) | Import-Module > $null

$new_tmp = [System.Environment]::ExpandEnvironmentVariables($Payload.module_args["_quantum_remote_tmp"])
$access_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Quantum.AccessToken"]))
$become_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Quantum.Become"]))
$process_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Quantum.Process"]))
...
try {
    ...
    Write-QuantumError -Message "internal error: failed to parse become_flags '$($Payload.become_flags)'" -ErrorRecord $_
    $host.SetShouldExit(1)
    return
}
Write-QuantumLog "INFO - parsed become input, user: '$username', type: '$logon_type', flags: '$logon_flags'" "become_wrapper"
...
try {
    Write-QuantumLog "INFO - starting become process '$lp_command_line'" "become_wrapper"
    $result = [Quantum.Become.BecomeUtil]::CreateProcessAsUser($username, $password, $logon_flags, $logon_type,
        $null, $lp_command_line,  $lp_current_directory, $null, $exec_wrapper)
    Write-QuantumLog "INFO - become process complete with rc: $($result.ExitCode)" "become_wrapper"
    $stdout = $result.StandardOut
    try {
        $stdout = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($stdout))
    } catch [FormatException] {
        # output wasn't Base64, ignore as it may contain an error message we want to pass to Quantum
        Write-QuantumLog "WARN - become process stdout was not base64 encoded as expected: $stdout"
    }
    ...
    Write-QuantumError -Message "internal error: failed to become user '$username'" -ErrorRecord $_
    $host.SetShouldExit(1)
}

Write-QuantumLog "INFO - ending become_wrapper" "become_wrapper"
