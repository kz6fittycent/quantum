...
Write-QuantumLog "INFO - starting module_powershell_wrapper" "module_powershell_wrapper"

$module_name = $Payload.module_args["_quantum_module_name"]
Write-QuantumLog "INFO - building module payload for '$module_name'" "module_powershell_wrapper"
...
foreach ($csharp_util in $Payload.csharp_utils_module) {
    Write-QuantumLog "INFO - adding $csharp_util to list of C# references to compile" "module_powershell_wrapper"
...
try {
    ...
    # output a pretty error for Quantum to parse
    $result = @{
        msg = "Failed to invoke PowerShell module: $($_.Exception.Message)"
        failed = $true
        exception = (Format-QuantumException -ErrorRecord $_)
    }
    Write-Output -InputObject (ConvertTo-Json -InputObject $result -Depth 99 -Compress)
    $host.SetShouldExit(1)
}

Write-QuantumLog "INFO - ending module_powershell_wrapper" "module_powershell_wrapper"
