...
Write-QuantumLog "INFO - starting coverage_wrapper" "coverage_wrapper"
...
Function New-CoverageBreakpoint {
    ...
    # Create an object that tracks the Quantum path of the file and the breakpoints that have been set in it
    $info = [PSCustomObject]@{
        Path = $QuantumPath
...
$module_name = $Payload.module_args["_quantum_module_name"]
Write-QuantumLog "INFO - building coverage payload for '$module_name'" "coverage_wrapper"
...
$temp_path = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "quantum-coverage-$([System.IO.Path]::GetRandomFileName())"
Write-QuantumLog "INFO - Creating temp path for coverage files '$temp_path'" "coverage_wrapper"
...
try {
    ...
    $parse_util = {
        ...
        Write-QuantumLog "INFO - Outputting module_util $util_name to temp file '$util_path'" "coverage_wrapper"
        Set-Content -LiteralPath $util_path -Value $util_code -Encoding $file_encoding

        $quantum_path = $Payload.coverage.module_util_paths.$util_name
        if ((Compare-WhitelistPattern -Patterns $coverage_whitelist -Path $quantum_path)) {
            $cov_params = @{
                Path = $util_path
                Code = $util_sb
                QuantumPath = $quantum_path
            }
        ...
        Write-QuantumLog "INFO - Adding util $util_name to scripts to run" "coverage_wrapper"
    ...
    Write-QuantumLog "INFO - Ouputting module $module_name to temp file '$module_path'" "coverage_wrapper"
    Set-Content -LiteralPath $module_path -Value $module -Encoding $file_encoding
    $scripts.Add($module_path)

    $quantum_path = $Payload.coverage.module_path
    if ((Compare-WhitelistPattern -Patterns $coverage_whitelist -Path $quantum_path)) {
        $cov_params = @{
            Path = $module_path
            Code = [ScriptBlock]::Create($module)
            QuantumPath = $Payload.coverage.module_path
        }
        $breakpoints = New-CoverageBreakpoint @cov_params
        $breakpoint_info.Add($breakpoints)
    }
    ...
    try {
        ...
        # possible. Once all the tests have been run quantum-test will collect this info and process it locally in
        # one go.
        Write-QuantumLog "INFO - Creating coverage result output" "coverage_wrapper"
        $coverage_info = @{}
        foreach ($info in $breakpoint_info) {
            $coverage_info.($info.Path) = $info.Breakpoints | Select-Object -Property Line, HitCount
        }

        # The coverage.output value is a filename set by the Quantum controller. We append some more remote side
        ...
        Write-QuantumLog "INFO - Outputting coverage json to '$coverage_output_path'" "coverage_wrapper"
        # Quantum controller expects these files to be UTF-8 without a BOM, use .NET for this.
    ...
    try {
        ...
        Write-QuantumLog "INFO - Remove temp coverage folder '$temp_path'" "coverage_wrapper"
        Remove-Item -LiteralPath $temp_path -Force -Recurse
    }
}

Write-QuantumLog "INFO - ending coverage_wrapper" "coverage_wrapper"
